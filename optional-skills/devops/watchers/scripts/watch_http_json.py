...
Usage (via cron with --no-agent):
    ...
    hbm cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$HBM_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "HBM AGENT-Watcher/1.0"})
