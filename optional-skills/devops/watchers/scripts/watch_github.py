...
Usage (via cron with --no-agent):
    ...
    hbm cron create hbm-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HBM_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hbm-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the HBM AGENT .env file
(``${HBM_HOME:-~/.hbm}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "HBM AGENT-Watcher/1.0",
    }
