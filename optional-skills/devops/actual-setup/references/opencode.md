...
is typically only in `~/.hbm/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike HBM AGENT,
...
`hbm-custom-providers` skill, pitfall 2). HBM AGENT hits this because it forwards
