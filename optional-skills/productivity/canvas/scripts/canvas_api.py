#!/usr/bin/env python3
"""Canvas LMS API CLI for HBM AGENT.
...
def _check_config():
    ...
    if missing:
        hbm_env = os.path.join(
            os.environ.get("HBM_HOME", os.path.expanduser("~/.hbm")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {hbm_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for HBM AGENT"
    )
