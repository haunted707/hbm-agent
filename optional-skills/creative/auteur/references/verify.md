...
- Optional second opinion: an outside UI critique (upstream used a separate 'impeccable' skill, not vendored; in HBM AGENT, `vision_analyze` on the shoot.mjs screenshots works) — it measures UX heuristics auteur doesn't; disagreement between the two is signal, not noise.
