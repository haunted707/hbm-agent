...
| Real video | an image→video model on an approved keyframe (6–10s) — user-supplied backend or browser tool | animate an approved keyframe; HBM AGENT ships no native video tool |
...
An image→video model animates an approved keyframe (6 or 10 seconds); HBM AGENT has no native video tool, so use whatever image→video backend the user has (or a browser tool via `browser_exec`), save into `assets/gen/`, then optimize (§5). **Spend video like the motion budget spends attention** — one hero clip + at most a couple supporting; a video that isn't the wow peak is usually a still that should have stayed a still.
