...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (HBM AGENT uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — HBM AGENT has `image_generate`
...
- License: MIT (inherits from hbm-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: HBM AGENT contributors
