...
hbm kanban watch --tenant <project-slug>

# Snapshot of the board
hbm kanban list --tenant <project-slug>
hbm kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
hbm kanban stats --tenant <project-slug>

# Visual dashboard (browser)
hbm dashboard

# Inspect a specific task (includes comments + events)
hbm kanban show <task-id>

# Follow a single task's event stream
hbm kanban tail <task-id>
```

Verify available subcommands with `hbm kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `hbm kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `hbm kanban block <id>` then `hbm kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `hbm kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `hbm kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `hbm profile list` (profile exists?), `hbm status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(hbm kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
hbm kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
hbm kanban block <task-id>
hbm kanban archive <task-id>

# Diagnose what happened
hbm kanban show <task-id>      # task body, comments, recent events
hbm kanban tail <task-id>      # follow the live event stream
hbm kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `hbm kanban create` for the director
...
while true; do
    clear
    hbm kanban list --tenant <slug>
    echo "---"
    hbm kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `hbm kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `hbm profile list` and start
