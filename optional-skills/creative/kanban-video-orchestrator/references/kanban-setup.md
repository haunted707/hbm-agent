...
the actual `setup.sh` that creates the project workspace, configures HBM AGENT
...
2. **Create profiles** — `hbm profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.hbm/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `hbm kanban create` assigned to the director
...
```bash
hbm profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.hbm/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.hbm/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most HBM AGENT
...
Each profile gets a `SOUL.md` at `~/.hbm/profiles/<name>/SOUL.md` that
...
```bash
hbm kanban create "Direct production of <video title>" \
    --assignee director \
...
the HBM AGENT `.env` (`${HBM_HOME:-$HOME/.hbm}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hbm_env="${HBM_HOME:-$HOME/.hbm}/.env"
    if grep -q "^${var}=" "$_hbm_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_hbm_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_hbm_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY hbm ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY hbm OPENROUTER_API_KEY || exit 1
# ...
```
