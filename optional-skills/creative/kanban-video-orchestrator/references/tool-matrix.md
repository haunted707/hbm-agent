...
Maps each role archetype to the HBM AGENT skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public hbm-agent
...
## HBM AGENT skills relevant to video production

### Visual / rendering skills (`hbm-agent/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `hbm plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`hbm-agent/skills/creative/`)
...
### Audio / media skills (`hbm-agent/skills/creative/` + `skills/media/`)
...
These are **not** HBM AGENT skills but external CLIs / APIs that profiles invoke.
...
## Built-in HBM AGENT tools for media review

These are native HBM AGENT tools — not invoked via terminal but through their own
...
key is present in `${HBM_HOME:-~/.hbm}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every HBM AGENT profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HBM_HOME:-~/.hbm}/.env` → environment variable.
...
When a new HBM AGENT-public video skill ships:
