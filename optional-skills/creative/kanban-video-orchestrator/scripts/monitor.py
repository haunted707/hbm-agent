#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `hbm kanban list` and
...
def hbm_available() -> bool:
    return shutil.which("hbm-agent") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed hbm CLI."""
    try:
        out = subprocess.run(
            ["hbm", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `hbm kanban list`
    out = subprocess.run(
        ["hbm", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not hbm_available():
        print("ERROR: 'hbm' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
