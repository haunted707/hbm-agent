...
### 1. Start order: editor first, HBM AGENT second

HBM AGENT probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW HBM AGENT session.
...
restart the editor, restart the HBM AGENT session.
...
server fails to bind (Output Log) or HBM AGENT's probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.hbm/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for HBM AGENT

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. HBM AGENT's connection
lives in `~/.hbm/config.yaml` via `hbm mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms HBM AGENT — just don't
mistake it for the HBM AGENT setup step.
...
explicitly warns against overlapping calls. HBM AGENT executes same-turn tool
...
### 9. Timeouts: HBM AGENT gives up before Unreal does

HBM AGENT's default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If HBM AGENT runs elsewhere (SSH backend,
