...
## 5. GODMODE FAST — HBM AGENT 4 405B (Zero Refusal)

**Model:** `nousresearch/hbm-4-405b`
**Strategy:** HBM AGENT models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with HBM AGENT

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.hbm/config.yaml`:
