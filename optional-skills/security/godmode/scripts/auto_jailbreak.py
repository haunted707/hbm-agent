...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HBM_HOME", os.path.expanduser("~/.hbm")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HBM_HOME", Path.home() / ".hbm")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# HBM AGENT config paths
# ═══════════════════════════════════════════════════════════════════

HBM_HOME = Path(os.getenv("HBM_HOME", Path.home() / ".hbm"))
CONFIG_PATH = HBM_HOME / "config.yaml"
PREFILL_PATH = HBM_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/HBM AGENT models — already uncensored, just needs clean prompt
    "hbm": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "hbm" in model_lower or "nous" in model_lower:
        return "hbm"
...
def _get_current_model() -> tuple:
    """Read current model and provider from HBM AGENT config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hbm/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart HBM AGENT for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart HBM AGENT for changes to take effect.")
