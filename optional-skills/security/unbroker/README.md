...
The design is **HBM AGENT-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
hbm skills install official/security/unbroker
```

Then start a new HBM AGENT session and drive it (below). The skill works zero-config; a few optional
...
Drive it from an HBM AGENT session:
...
- **PII handling.** Dossiers live under the HBM AGENT home directory (`0600`, optionally
