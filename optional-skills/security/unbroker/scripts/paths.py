...
All per-subject data lives under PDD_DATA_DIR (default: $HBM_HOME/unbroker),
which is the same trust boundary HBM AGENT uses for .env and OAuth tokens.
"""
...
def hbm_home() -> Path:
    return Path(os.environ.get("HBM_HOME") or (Path.home() / ".hbm"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else hbm_home() / "unbroker"
