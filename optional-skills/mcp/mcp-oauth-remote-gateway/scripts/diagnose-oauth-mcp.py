...
Usage:
  ...
  <server_name> matches the files in $HBM_HOME/mcp-tokens/<server>.json etc.
...
def _hbm_home():
    # Prefer HBM AGENT's own resolver (profile-safe); fall back to env then ~/.hbm.
    try:
        from hbm_constants import get_hbm_home
        return str(get_hbm_home())
    except Exception:
        return os.environ.get("HBM_HOME") or os.path.expanduser("~/.hbm")


def _tokens_dir():
    return os.path.join(_hbm_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "hbm-diag", "version": "1.0"}},
        },
        headers={
