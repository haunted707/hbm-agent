...
_HBM_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `hbm-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _HBM_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_HBM_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _HBM_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `hbm tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``hbm tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _HBM_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (HBM_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _HBM_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in hbm_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full HBM AGENT toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. hbm-acp is the
    ...
    "hbm-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors hbm-cli; `hbm tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "hbm-cron": _bundle("Default cron toolset - same core tools as hbm-cli; gated by `hbm tools`"),
    "hbm-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "hbm-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "hbm-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "hbm-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "hbm-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "hbm-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "hbm-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "hbm-email": _bundle("Email bot toolset - interact with HBM AGENT via email (IMAP/SMTP)"),
    "hbm-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "hbm-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "hbm-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "hbm-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "hbm-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "hbm-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "hbm-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "hbm-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "hbm-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _HBM_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "hbm-sms": _bundle("SMS bot toolset - interact with HBM AGENT via SMS (Twilio)"),
    "hbm-webhook": _ts("Webhook toolset - receive and process external webhook events", _HBM_WEBHOOK_SAFE_TOOLS),
    "hbm-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "hbm-telegram", "hbm-discord", "hbm-whatsapp", "hbm-slack",
            "hbm-signal", "hbm-bluebubbles", "hbm-homeassistant", "hbm-email",
            "hbm-sms", "hbm-mattermost", "hbm-matrix", "hbm-dingtalk",
            "hbm-feishu", "hbm-wecom", "hbm-wecom-callback", "hbm-weixin",
            "hbm-qqbot", "hbm-webhook", "hbm-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _HBM_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only hbm-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_HBM_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `hbm-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("hbm-"):
        return []
    platform_name = name[len("hbm-"):]
    try:
