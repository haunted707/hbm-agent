...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with hbm-agent)
2. User plugins: ``$HBM_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``hbm_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$HBM_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$HBM_HOME/plugins/model-providers/`` if it exists."""
    try:
        from hbm_constants import get_hbm_home

        d = get_hbm_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$HBM_HOME/plugins/`` if it exists.

    This is where ``hbm plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple HBM_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_hbm_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``hbm_agent.plugins``
    ...
    * **Provider targets only.** The ``hbm_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$HBM_HOME``) keep their
    ...
    try:
        from hbm_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$HBM_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``hbm plugins install`` at
          ``$HBM_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``hbm_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$HBM_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with hbm-agent.
    ...
    # 2. User plugins — under $HBM_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``hbm plugins install`` / the plugin index.
    #     Those clone into $HBM_HOME/plugins/<name>/ — flat, NOT under
