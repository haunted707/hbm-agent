...
def _profile_user_agent() -> str:
    """Return a ``hbm-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from hbm_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"hbm-cli/{_ver}"
    except Exception:
        return "hbm-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; hbm_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.hbm/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # hbm-cli UA so the catalog endpoint is reachable.
