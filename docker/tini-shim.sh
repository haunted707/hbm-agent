...
# HBM AGENT WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: HBM_TINI_SHIM_TARGET overrides the `/init` path (and
# HBM_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${HBM_TINI_SHIM_TARGET:-/init}"
WRAPPER="${HBM_TINI_SHIM_WRAPPER:-/opt/hbm/docker/main-wrapper.sh}"
