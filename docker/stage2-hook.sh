...
#
# Wired into the image as /etc/cont-init.d/01-hbm-setup by the
...
HBM_HOME="${HBM_HOME:-/opt/data}"
INSTALL_DIR="/opt/hbm"

# Drop to hbm via s6-setuidgid, but skip it when already non-root.
as_hbm() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid hbm "$@"; }
...
# starts non-root. The baked install tree under /opt/hbm is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $HBM_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass HBM_UID/HBM_GID — or the PUID/PGID aliases — which the
...
# `id -u hbm` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# hbm UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u hbm)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-hbm UID).
...
and the baked /opt/hbm install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e HBM_UID=\$(id -u) -e HBM_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the hbm user to that UID/GID at boot and chowns the data
...
# --- Bootstrap HBM_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid hbm mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom HBM_HOME paths whose parents only
# root can create (e.g. `HBM_HOME=/home/hbm/.hbm` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for HBM_UID/HBM_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  HBM_UID/
# HBM_GID still win when both are set.  See #15290, salvages #25872.
HBM_UID="${HBM_UID:-${PUID:-}}"
HBM_GID="${HBM_GID:-${PGID:-}}"

if [ -n "${HBM_UID:-}" ] && validate_uid_gid "$HBM_UID" && [ "$HBM_UID" != "$(id -u hbm)" ]; then
    echo "[stage2] Changing hbm UID to $HBM_UID"
    usermod -u "$HBM_UID" hbm
fi
if [ -n "${HBM_GID:-}" ] && validate_uid_gid "$HBM_GID" && [ "$HBM_GID" != "$(id -g hbm)" ]; then
    echo "[stage2] Changing hbm GID to $HBM_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$HBM_GID" hbm 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised hbm user (UID 10000)
...
# hbm process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes hbm. Idempotent across container
...
#   - hbm is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G hbm 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] hbm already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" hbm 2>/dev/null; then
        echo "[stage2] Added hbm to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group hbm failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When HBM_UID is remapped or the top-level $HBM_HOME isn't owned by
# the runtime hbm UID, restore ownership to hbm — but ONLY for the
# directories hbm actually writes to. The full $HBM_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of hbm-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_hbm_uid=$(id -u hbm)
...
chown_hbm_tree() {
    ...
    chown -R hbm:hbm "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_hbm_owner() {
    target="$1"
    find "$target" \( ! -user hbm -o ! -group hbm \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$HBM_HOME" 2>/dev/null)" != "$actual_hbm_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $HBM_HOME (targeted) to hbm ($actual_hbm_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $HBM_HOME: chown the directory itself (not its contents)
    # so hbm can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$HBM_HOME"; then
        :
    else
        chown hbm:hbm "$HBM_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $HBM_HOME failed (rootless container?) — continuing"
    fi
    # HBM AGENT-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by hbm (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$HBM_HOME/$sub" ] && tree_has_non_hbm_owner "$HBM_HOME/$sub"; then
            chown_hbm_tree "$HBM_HOME/$sub"
        fi
    done
fi
...
# hbm user. Hosted/container instances keep mutable state under
# $HBM_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# HBM_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hbm root-owned and
...
# $HBM_HOME/lazy-packages dir on the data volume (Dockerfile sets
# HBM_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to hbm
...
# Always reset ownership of $HBM_HOME/profiles to hbm on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> hbm …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as hbm and walks
...
if [ -d "$HBM_HOME/profiles" ] && tree_has_non_hbm_owner "$HBM_HOME/profiles"; then
    chown_hbm_tree "$HBM_HOME/profiles"
fi

# Always reset ownership of $HBM_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged hbm runtime even
...
# gate as profiles/).
if [ -d "$HBM_HOME/cron" ] && tree_has_non_hbm_owner "$HBM_HOME/cron"; then
    chown_hbm_tree "$HBM_HOME/cron"
fi

# Always ensure logs/gateways is hbm-owned (#45258). Formerly healed by
...
# top-level $HBM_HOME is mis-owned, so a warm volume with hbm-owned
# HBM_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid hbm mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as hbm.
if [ -d "$HBM_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$HBM_HOME/logs/gateways"; then
        :
    else
        chown hbm:hbm "$HBM_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# and cron/.
if [ -d "$HBM_HOME/platforms/pairing" ] && tree_has_non_hbm_owner "$HBM_HOME/platforms/pairing"; then
    chown_hbm_tree "$HBM_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$HBM_HOME/pairing" ] && tree_has_non_hbm_owner "$HBM_HOME/pairing"; then
    chown_hbm_tree "$HBM_HOME/pairing"
fi

# Reset ownership of hbm-owned top-level state files on every boot.
# The targeted data-volume chown above only covers hbm-owned
# *subdirectories*; loose state files living directly under $HBM_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> hbm …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged hbm runtime then hits
...
# sweep so host-owned files in a bind-mounted $HBM_HOME are never
...
# entries of hbm_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$HBM_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$HBM_HOME/$f"; then
            :
        else
            chown hbm:hbm "$HBM_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the hbm runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$HBM_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$HBM_HOME/config.yaml"; then
        :
    else
        chown hbm:hbm "$HBM_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$HBM_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as hbm user ---
# Run as hbm via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $HBM_HOME values
...
as_hbm mkdir -p \
    ...
    "$HBM_HOME/home" \
    "$HBM_HOME/pairing" \
    "$HBM_HOME/platforms/pairing" \
    "$HBM_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/hbm/.install_method (see Dockerfile), NOT written here into
# $HBM_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $HBM_HOME: it is a shared DATA volume, commonly
...
if [ -f "$HBM_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$HBM_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$HBM_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$HBM_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$HBM_HOME/$dest"; then
            :
        else
            as_hbm cp "$INSTALL_DIR/$src" "$HBM_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/hbm/.env.example being present
...
# not generate one. HBM AGENT loads $HBM_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$HBM_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$HBM_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $HBM_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$HBM_HOME/.env" ] && ! refuse_symlinked_path "clean" "$HBM_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$HBM_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$HBM_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$HBM_HOME/.env"; then
        :
    else
        if [ ! -f "$HBM_HOME/.env" ]; then
            ...
            (umask 077 && as_hbm touch "$HBM_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$HBM_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$HBM_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$HBM_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $HBM_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $HBM_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$HBM_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$HBM_HOME/.env"; then
        :
    else
        chown hbm:hbm "$HBM_HOME/.env" 2>/dev/null || true
        chmod 600 "$HBM_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `hbm-agent` user — without this it gets EACCES on every
...
# hbm group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp hbm /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted hbm group access to the Fly Machines API socket"
    else
...
if [ -f "$HBM_HOME/config.yaml" ]; then
    s6-setuidgid hbm "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$HBM_HOME/auth.json" ] && [ -n "${HBM_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$HBM_HOME/auth.json"; then
        :
    else
        printf '%s' "$HBM_AUTH_JSON_BOOTSTRAP" > "$HBM_HOME/auth.json"
        chown hbm:hbm "$HBM_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$HBM_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via HBM_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$HBM_HOME/auth.json" ] && [ -n "${HBM_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$HBM_HOME/auth.json"; then
        :
    else
        s6-setuidgid hbm "$INSTALL_DIR/.venv/bin/python" \
...
# can set HBM_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# HBM_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$HBM_HOME/gateway_state.json" ] && \
        [ "${HBM_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$HBM_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$HBM_HOME/gateway_state.json"
        chown hbm:hbm "$HBM_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$HBM_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_hbm "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/hbm/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI HBM AGENT spawns for the
...
# main-wrapper.sh propagates it into the supervised ``hbm-agent`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-hbm, dashboard, etc.).
