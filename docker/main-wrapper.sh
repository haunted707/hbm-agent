#!/bin/sh
# shellcheck shell=sh
# /opt/hbm/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching HBM_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `hbm-agent` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `hbm-agent <args>` (subcommand passthrough)
#
# Drop to hbm via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${HBM_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${HBM_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export HBM_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset HBM_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid hbm "$@"; exec "$@"; }
...
# an arbitrary non-root, non-hbm UID, the bootstrap was skipped and the
# baked image dirs (owned by the hbm build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u hbm)" ]; then
    cat >&2 <<EOF
[hbm] ERROR: container started with --user $cur_uid (an arbitrary, non-hbm UID) — not supported.
...
# to the hbm user's home before dropping privileges so libraries that
...
_hbm_orig_cwd="${HBM_ORIG_CWD:-$PWD}"
...
# HBM AGENT subcommand pass-through.
drop hbm-agent "$@"
