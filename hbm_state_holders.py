...
an unlinked SQLite DB/WAL/SHM generation.  ``hbm_state`` supplies only the
...
_HBM_EXECUTABLES = frozenset({"hbm-agent", "hbm-agent-acp", "hbm-agent-run"})
_HBM_PYTHON_MODULES = frozenset({"acp_adapter", "hbm_cli.main"})
_HBM_PYTHON_SCRIPTS = frozenset({"hbm_cli/main.py", "run_agent.py"})
...
def _looks_like_hbm(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported HBM AGENT execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _HBM_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _HBM_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _HBM_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the HBM_HOME root, so an absolute-path token
    containing a ``/.hbm`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own HBM AGENT home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.hbm deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.hbm" in normalized or normalized.endswith("/.hbm"):
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # HBM_HOME would otherwise make every holder invisible and let maintenance proceed.
