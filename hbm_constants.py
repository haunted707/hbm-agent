"""Shared constants for HBM AGENT.
...
_UNSET = object()
_HBM_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_HBM_HOME_OVERRIDE", default=_UNSET)
...
def set_hbm_home_override(path: str | Path | None) -> Token:
    """Set a context-local HBM AGENT home override and return its reset token.

    Deliberately does not mutate ``os.environ`` (shared by every thread in the process).
    """
    value: str | object = _UNSET if path is None else str(path)
    return _HBM_HOME_OVERRIDE.set(value)


def reset_hbm_home_override(token: Token) -> None:
    """Restore the previous context-local HBM AGENT home override."""
    _HBM_HOME_OVERRIDE.reset(token)


def get_hbm_home_override() -> str | None:
    """Return the active context-local HBM AGENT home override, if any."""
    override = _HBM_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


def _get_platform_default_hbm_home() -> Path:
    """Return the platform-native default HBM AGENT home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "hbm"
    return Path.home() / ".hbm"


def sudo_invoker_default_home() -> Path | None:
    """The invoking user's native ``~/.hbm`` when this process is root under ``sudo``, else None.

    sudo strips HBM_HOME and sets HOME=/root, so the process's own default is root's; the profile
    ...
    try:
        return Path(pwd.getpwnam(sudo_user).pw_dir) / ".hbm"
    except KeyError:  # SUDO_USER not in passwd (chroot/container)
        return None


def _warn_profile_fallback_once() -> None:
    """Warn once when HBM_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    global _profile_fallback_warned
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_hbm_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[HBM_HOME fallback] HBM_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass HBM_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_hbm_home() -> Path:
    """HBM AGENT home: context-local override → ``HBM_HOME`` env var → platform default."""
    override = get_hbm_home_override()
    if override:
        return Path(override)
    if not os.environ.get("HBM_HOME", "").strip():
        _warn_profile_fallback_once()
    return get_process_hbm_home()
...
def hbm_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a HBM AGENT home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_hbm_home()
    raw = str(candidate)
...
def reset_hbm_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


def get_process_hbm_home() -> Path:
    """HBM AGENT home of the running process, ignoring task overrides.
    ...
    return Path(val) if val else _get_platform_default_hbm_home()


# HBM AGENT-managed runtime downloads at the root of a home (GGUF models, llama.cpp runtimes,
# managed Node): re-downloadable on demand and routinely tens to hundreds of GB. Shared by
# ``hbm backup`` (excludes them) and ``profile create --clone-all`` (skips them from the
# default profile) so the two lists cannot drift apart.
LOCAL_RUNTIME_ROOT_DIRS: frozenset[str] = frozenset({"models", "runtimes", "node"})

# get_default_hbm_root() memo keyed on (native home, HBM_HOME) so it stays
# fresh when a test or plugin mutates HBM_HOME; saves ~80us/call at 31+ sites.
_default_hbm_root_memo: "tuple[str, str, Path] | None" = None


def get_default_hbm_root() -> Path:
    """Root HBM AGENT dir for profile-level ops: ``<root>`` when ``HBM_HOME=<root>/profiles/<name>``."""
    global _default_hbm_root_memo
    native_home = _get_platform_default_hbm_home()
    env_home = os.environ.get("HBM_HOME", "")
    memo = _default_hbm_root_memo
    ...
    if env_home:
        ...
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.hbm (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else HBM_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_hbm_root_memo = (str(native_home), env_home, result)
    return result
...
# Files marking a real HBM AGENT home; arbitrary dirs with a ``profiles`` segment lack them.
_HBM_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_hbm_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<hbm-home>/profiles``.

    Accepts the classic ``~/.hbm`` layout, a root carrying HBM AGENT-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _HBM_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_hbm_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real HBM AGENT home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_hbm_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".hbm":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def profile_name_for_home(path: str | Path | None) -> str | None:
    ...
    The default home is the HBM AGENT root itself, so its basename is an installation detail (``.hbm``
    on POSIX and commonly ``hbm`` on Windows), not the profile id ``default``.
    """
...
def mkdir_under_hbm_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_hbm_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("HBM_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``HBM_OPTIONAL_MCPS``)."""
    return _packaged_dir("HBM_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``HBM_BUNDLED_SKILLS``)."""
    return _packaged_dir("HBM_BUNDLED_SKILLS", default, "skills")


def get_hbm_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a HBM AGENT subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
    ...
    """
    home = home or get_hbm_home()
    old_path = home / old_name
    return old_path if _legacy_path_has_content(old_path) else home / new_subpath


def iter_hbm_node_dirs(home: Path | None = None) -> list[Path]:
    """HBM AGENT-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with hbmManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_hbm_home()) / "node"
...
def _iter_managed_node_candidates(names: list[str], home: Path | None = None):
    ...
    for directory in iter_hbm_node_dirs(home):
        for name in names:
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the HBM AGENT-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_hbm_node_path())
    return result is not None and result.returncode == 0


_HBM_NODE_TARGET_MAJOR = int(os.environ.get("HBM_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/hbm_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks hbm-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def hbm_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any HBM AGENT-managed node/npm/npx shim exists on disk."""
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ HBM AGENT-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `hbm update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_HBM_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_HBM_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%HBM_HOME%\\node`` on Windows.
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the hbm.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HBM_HOME": str(get_hbm_home()), **extra_env},
...
def bootstrap_hbm_managed_node() -> str | None:
    """Install a HBM AGENT-managed Node tree under ``$HBM_HOME/node`` and return its npm path.

    HBM AGENT never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    existing = find_hbm_node_executable("npm")
    if existing:
        return existing
    if sys.platform == "win32":
        ok = _heal_managed_node_windows()
    else:
        # HBM_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, HBM_NODE_SKIP_LINKS="1")
    if not ok:
        return None
    return _first_runnable_managed(_candidate_node_command_names("npm"))[0]


def heal_hbm_managed_node() -> bool:
    """Redownload HBM AGENT-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not hbm_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _HBM_NODE_TARGET_MAJOR
    return False


def find_hbm_node_executable(command: str) -> str | None:
    """HBM AGENT-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
    ...
    if needs_heal and heal_hbm_managed_node():
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_hbm_node_executable(command)
    if managed:
        return managed
    if hbm_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_hbm_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with HBM AGENT-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    parts = [p for p in merged.get("PATH", "").split(os.pathsep) if p]
    for entry in reversed([str(path) for path in iter_hbm_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``hbm update``
...
def display_hbm_home() -> str:
    """User-facing ``~/`` display string for HBM_HOME (``~/.hbm/profiles/coder``)."""
    home = get_hbm_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\hbm/skills/
...
def profile_cli_selector() -> str:
    """``-p <name> `` (trailing space) pinning copy-pasteable ``hbm ...`` guidance to the
    active NAMED profile, else ``""``: a bare ``hbm`` follows the sticky ``active_profile``
    file, which can name a different database than the one that failed (#105887). A custom
    home outside the profile tree has no selector (only HBM_HOME names it)."""
    name = profile_name_for_home(get_hbm_home())
    return f"-p {name} " if name and name != "default" else ""


def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved HBM_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks hbm-user traversal in Docker (UID 10000).
    # A credential file here means HBM_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "hbm-agent install directory (%s). Credential files are "
            "normally stored under the hbm home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{HBM_HOME}/home`` when the profile-home directory exists."""
    hbm_home = get_hbm_home_override() or (env or {}).get("HBM_HOME") or os.getenv("HBM_HOME")
    if not hbm_home:
        return None
    profile_home = os.path.join(hbm_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "HBM_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the HBM AGENT profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={HBM_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{HBM_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply HBM AGENT' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["HBM_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when HBM AGENT runs in WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under HBM_HOME."""
    return get_hbm_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under HBM_HOME."""
    return get_hbm_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under HBM_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_hbm_ipv4_patched", False):
        return
    ...
    def _ipv4_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
        ...
        return _original_getaddrinfo(host, port, family, type, proto, flags)
    _ipv4_getaddrinfo._hbm_ipv4_patched = True  # type: ignore[attr-defined]
...
def openrouter_variant_base(model_id: str) -> str | None:
    ...
    Lives here rather than in ``hbm_cli.models`` so the metadata layer
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``hbm_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``hbm_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with HBM AGENT (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("hbm_"))
