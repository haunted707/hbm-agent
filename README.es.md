...
> **Nota:** HBM AGENT es un fork con nueva marca de
> [Hermes Agent](https://github.com/NousResearch/hermes-agent) de Nous Research.
> Se distribuye bajo la Licencia MIT, conservando el aviso de copyright original
> (consulta [LICENSE](LICENSE)). Este fork no está afiliado con Nous Research ni
...
<p align="center">
  <a href="https://hermes-agent.nousresearch.com/docs/"><img src="https://img.shields.io/badge/Docs-hbm--agent.nousresearch.com-FFD700?style=for-the-badge" alt="Documentación"></a>
...
> **Nota:** HBM AGENT se distribuye desde su propio repositorio — **no** se
> instala con los comandos de una línea de Hermes Agent
> (`hermes-agent.nousresearch.com/install.sh` / `.ps1`), que instalan el Hermes
> Agent original. Clona este repositorio y ejecuta el instalador local.
...
> **Nota:** En Windows nativo, HBM AGENT funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/NousResearch/hermes-agent/issues).
...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\hbm\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). HBM AGENT usa este Git Bash incluido para ejecutar comandos de shell.
...
> **Android / Termux:** La ruta manual probada está documentada en la [guía de Termux](https://hermes-agent.nousresearch.com/docs/getting-started/termux). En Termux, HBM AGENT instala el extra `.[termux]` curado porque el extra completo `.[all]` actualmente incluye dependencias de voz incompatibles con Android.
>
> **Windows:** Windows nativo es totalmente compatible — el comando de PowerShell de arriba instala todo. Si prefieres usar WSL2, el comando de Linux también funciona allí. La instalación nativa de Windows se encuentra en `%LOCALAPPDATA%\hbm`; WSL2 instala en `~/.hbm` como en Linux.

Después de la instalación:

```bash
source ~/.bashrc    # recargar shell (o: source ~/.zshrc)
hbm              # ¡empieza a chatear!
```
...
```bash
hbm              # CLI interactiva — inicia una conversación
hbm model        # Elige tu proveedor y modelo LLM
hbm tools        # Configura qué herramientas están habilitadas
hbm config set   # Establece valores de configuración individuales
hbm gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
hbm setup        # Ejecuta el asistente de configuración completo
hbm claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
hbm update       # Actualiza a la última versión
hbm doctor       # Diagnostica cualquier problema
```
...
HBM AGENT funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Esto te autentica vía OAuth, establece Nous como tu proveedor y activa el Tool Gateway. Comprueba qué está conectado en cualquier momento con `hbm portal info`. Detalles completos en la [página de documentación del Tool Gateway](https://hermes-agent.nousresearch.com/docs/user-guide/features/tool-gateway).
...
HBM AGENT tiene dos puntos de entrada: inicia la interfaz de terminal con `hbm`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `hbm`                                      | Ejecuta `hbm gateway setup` + `hbm gateway start`, luego envía un mensaje al bot |
...
Si vienes de OpenClaw, HBM AGENT puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`hbm setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
hbm claw migrate              # Migración interactiva (preset completo)
hbm claw migrate --dry-run    # Vista previa de qué se migraría
hbm claw migrate --preset user-data   # Migrar sin secretos
hbm claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.hbm/skills/openclaw-imports/`
...
Consulta `hbm claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-hbm.sh`:

```bash
git clone https://github.com/NousResearch/hermes-agent.git
cd hbm-agent
./setup-hbm.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/hbm
./hbm              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para HBM AGENT y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
- 🔌 [HermesClaw](https://github.com/AaronWong1999/hermesclaw) — Puente WeChat comunitario: Ejecuta HBM AGENT y OpenClaw en la misma cuenta de WeChat.
