"""Centralized logging setup for HBM AGENT.
...
# in HBM AGENT (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from hbm_constants import get_config_path, get_hbm_home, mkdir_under_hbm_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_hbm_session_injector", False):
        return
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.hbm_home = str(get_hbm_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.hbm_home = ""  # type: ignore[attr-defined]
        return record
    _session_record_factory._hbm_session_injector = True  # type: ignore[attr-defined]
...
# Logger name prefixes per component; used by _ComponentFilter and ``hbm logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "hbm_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("hbm_cli", "cli"),
    "cron": ("cron",),
    "gui": ("hbm_cli.web_server", "hbm_cli.pty_bridge", "tui_gateway", "uvicorn"),
}
...
def setup_logging(
    *,
    ...
    """Configure the HBM AGENT logging subsystem; returns the ``logs/`` directory.
    ...
    home = hbm_home or get_hbm_home()
    log_dir = mkdir_under_hbm_home(home / "logs")
    # A second HBM AGENT home in a process that already logs for another one — a dashboard or
    # ``hbm serve`` backend building agents for several profiles, a multiplexed gateway —
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_hbm_verbose", False) for h in root.handlers):
        return
    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._hbm_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from hbm_cli.config import is_managed
        ...
        if _is_unavailable_log_stream(exc):
            ...
            if not self._unavailable_reported:
                ...
                _quietly(lambda: print(
                    f"hbm_logging: {self.baseFilename} unavailable ({exc}); "
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their HBM AGENT home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._hbm_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "hbm_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._hbm_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="hbm-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_hbm_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # can pin logging.*) so this reuses hbm_cli.main's early parse (one config.yaml parse
        # per process); fall back to a direct parse for bare hbm_logging consumers.
        try:
