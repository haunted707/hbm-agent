# HBM AGENT - Development Guide

Instructions for AI coding assistants and developers working on the hbm-agent codebase.
...
HBM AGENT is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: HBM AGENT ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`hbm tools`, `hbm setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `HBM_HOME` — two of them (A→B→A) when the
...
- **New `HBM_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `hbm tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.hbm/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `hbm <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`hbm webhook`, `hbm cron`, `hbm tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.hbm/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or HBM AGENT Cloud, and
only the first two carry `HBM_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the HBM AGENT desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_HBM_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `HBM_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`hbm --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.hbm/hbm-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _HBM_CORE_TOOLS
├── cli.py                # HbmCLI (REPL, slash dispatch) + hbm_cli/cli_*_mixin.py
├── hbm_state.py       # SessionDB facade; hbm_state_*.py siblings
├── hbm_constants.py   # get_hbm_home(), display_hbm_home() — profile-aware paths
├── hbm_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── hbm_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `hbm --tui`
...
**User state:** `~/.hbm/config.yaml` (settings), `~/.hbm/.env` (secrets only),
`~/.hbm/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_hbm_home()`. Browse logs with `hbm logs [--follow] [--level] [--session]`.
...
families: `hbm_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`hbm_cli/kanban.py` (14), `hbm_cli/web_server.py` (13 + 24 routers), `hbm_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `hbm_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::hbm_cli.plugin_compat.HbmPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `hbm_cli.update_cmd._hbm_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `hbm_cli/AGENTS.md`.
- **Never hardcode `~/.hbm`.** `get_hbm_home()` for code paths, `display_hbm_home()`
  for user-facing text (both from `hbm_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.hbm/profiles`) so `hbm -p x profile list`
...
- **One process may serve many profiles; code that runs outside a turn binds the owning
  ...
  module constants are a bug class — key slots by `hbm_home_key()` or resolve at call time.
  ...
  `HBM_HOME`. Advisory lint: `scripts/check_profile_scope_patterns.py`.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `hbm webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `HBM_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `HBM_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `HBM_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/hbm_cli/`,
  `tests/agent/`, `tests/hbm_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `hbm_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.hbm/`.** The autouse `_isolate_hbm_home` fixture in
  `tests/conftest.py` redirects `HBM_HOME`; never hardcode `~/.hbm/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_hbm_home()` stay
  in the temp dir (pattern: `tests/hbm_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".hbm"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("HBM_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `HBM_HOME` — code reads the
  env var, not `Path.home()/.hbm`.
...
`tests/hbm_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `hbm_cli/`, `main.py` | `hbm_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `hbm update` pipeline, profiles / multiplex |
...
| `plugins/`, `hbm_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `hbm_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`hbm_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
