...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/hbm.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/hbm.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'hbm-update',
  error: 'E2E ASSERTION FAILED: hbm update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('hbm.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'hbm-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured hbm desktop spec; update completed',
      logs: { desktop: '[hbm] [updates] no staged updater; surfacing manual `hbm update` for CLI install at C:/install\n[hbm] [updates] manual: hbm update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: hbm update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> hbm-update (v2026.3.12 -> HEAD)'
