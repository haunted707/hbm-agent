 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `hbm serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. HBM AGENT should add
 ...
 * `E2E_SAY(code-farmer)[{at}hbm Reply with B.] E2E_SAY(hbm)[B]`.
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `hbm-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated HBM_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `hbm-dev-mock-${Date.now()}`))
  const hbmHome = nodePath.join(root, 'hbm-home')
...
function writeMockConfig(hbmHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(hbmHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(hbmHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.hbmHome, mock.url)
  console.log(`  HBM_HOME: ${sandbox.hbmHome}`)
  ...
  const env: Record<string, string> = {
    ...
    HBM_DESKTOP_HBM_ROOT: repoRoot,
    HBM_DESKTOP_APP_NAME: `HbmDevMock-${Date.now()}`,
  }
