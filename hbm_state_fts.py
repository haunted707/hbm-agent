...
from hbm_state_common import (FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS,
    routed_sessions_setting)
from hbm_state_errors import is_fts_scoped_corruption_error

# caplog tests pin the "hbm_state" logger name.
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("HBM_FTS5_CJK_SO")
    return Path(env).expanduser() if env else get_hbm_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on) for the profile being served."""
    value = routed_sessions_setting("cjk_fts", "HBM_CJK_FTS")
...
class SessionFtsSetupMixin:
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `hbm update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `hbm sessions optimize-storage` on a host "
    ...
    def _is_fts_write_corruption_error(exc: sqlite3.DatabaseError) -> bool:
        ...
        gateway transcript retry: see :func:`hbm_state_errors.is_fts_scoped_corruption_error`."""
