...
# 5. Symlinks the 'hbm' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☤ HBM AGENT Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/hbm-uv-install.$$.log")"
...
if is_termux; then
    # HBM AGENT currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; HBM AGENT requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink hbm into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up hbm command..."

HBM_BIN="$SCRIPT_DIR/venv/bin/hbm-agent"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HBM_BIN" "$COMMAND_LINK_DIR/hbm"
echo -e "${GREEN}✓${NC} Symlinked hbm → $COMMAND_LINK_DISPLAY_DIR/hbm"
...
# ============================================================================
# Seed bundled skills into ~/.hbm/skills/
# ============================================================================

HBM_SKILLS_DIR="${HBM_HOME:-$HOME/.hbm}/skills"
mkdir -p "$HBM_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.hbm/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$HBM_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  hbm status        # Check configuration"
if is_termux; then
    echo "  hbm gateway       # Run gateway in foreground"
else
    echo "  hbm gateway install # Install gateway service (messaging + cron)"
fi
echo "  hbm cron list     # View scheduled jobs"
echo "  hbm doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m hbm_cli.main setup
fi
