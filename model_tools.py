...
def _is_dispatcher_owned_worker() -> bool:
    """False when HBM_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # Carry profile + approval/sudo context so get_hbm_home() resolves correctly.
...
def _tool_defs_cache_key(
    ...
    return (
        ...
        bool(os.environ.get("HBM_KANBAN_TASK")), bool(skip_tool_search_assembly),
...
def _apply_toolset_selection(tools: set, names: List[str], quiet_mode: bool, *, disable: bool) -> None:
    ...
    for name in names:
        if validate_toolset(name):
            ...
            if disable and (name.startswith("hbm-") or (get_toolset(name) or {}).get("posture")):
                ...
                if not quiet_mode and name.startswith("hbm-") and name not in _WARNED_DISABLED_BUNDLES:
...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        if (os.environ.get("HBM_KANBAN_TASK") and not _is_delegated_child_context()
    ...
    # toolset is stripped even when a composite (hbm-cli) re-enables it.
    # This ensures that even if a composite toolset (like hbm-cli) is enabled, any tools belonging to a
