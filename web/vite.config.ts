...
const BACKEND = process.env.HBM_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `hbm dashboard` server injects a one-shot
 * session token into `index.html` (see `hbm_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__HBM_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function hbmDevToken(): Plugin {
  const TOKEN_RE = /window\.__HBM_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__HBM_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "hbm:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[hbm] Could not find session token in ${BACKEND} — ` +
              `is \`hbm dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__HBM_SESSION_TOKEN__="${match[1]}";` +
              `window.__HBM_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[hbm] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`hbm dashboard\` or set HBM_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@hbm/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `hbm dashboard` must serve these; Vite has no
