# web/ + hbm_cli/web_routers/ — the dashboard (`hbm dashboard` → `/chat`)

Applies on top of the root `AGENTS.md`. Backend routers: `hbm_cli/web_routers/*.py`, one file per
dashboard surface, mounted by `hbm_cli/web_server.py` (+ `web_server_*.py` siblings). Frontend:
`web/src/`. Shared JSON-RPC/WS client: `apps/shared` (`@hbm/shared`), also used by the desktop.

## The dashboard embeds the REAL `hbm --tui` — not a rewrite

`hbm_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `web_server.py`:
...
- The server spawns exactly what `hbm --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL
...
`cmd_dashboard` skips `_build_web_ui` and exports `HBM_SERVE_HEADLESS=1` so `mount_spa()`
...
- Routers are one-file-per-surface; a new surface is a new `web_routers/<surface>.py`, not a growing
  `web_server.py`.
- Tests: Python in `tests/hbm_cli/` (routers, pty bridge); JS in the `web/` vitest suite. Python
