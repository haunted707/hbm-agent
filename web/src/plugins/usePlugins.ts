...
import { api, HBM_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "hbm:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${HBM_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${HBM_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?hbm_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-hbm-plugin", manifest.name);
