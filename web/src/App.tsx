...
const SIDEBAR_COLLAPSED_KEY = "hbm-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="hbm-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="hbm-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateHbm = status?.can_update_hbm === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'hbm update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "hbm update";
    return (
      t.status.updateHbmConfirmMessage ??
      `This will run 'hbm update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateHbmConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateHbm) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateHbm,
      runningLabel: t.status.updatingHbm,
      spin: false,
    });
  }
  ...
  return (
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the HBM AGENT gateway process. Connected channels and active sessions will reconnect afterward.")
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateHbmConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateHbmConfirmTitle ?? `${t.status.updateHbm}?`}
    />
    </>
  );
}
