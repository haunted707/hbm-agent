...
interface HbmConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function HbmConsoleModal({ open, onClose }: HbmConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("hbm> ");
  const inputPromptRef = useRef("hbm> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "hbm> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to HBM AGENT Console...\x1b[0m");
    ...
    void (async () => {
      try {
        ...
        ws.onclose = (ev) => {
          ...
          const message =
            ev.code === 1006 && !hasReadyFrameRef.current
              ? "Console could not connect to the dashboard server. Check that `hbm dashboard` is running, then click Reconnect."
              : ev.code === 1000
                ? "Console closed."
        ...
        writeLine(
          term,
          "\x1b[31mConsole could not connect to the dashboard server. Check that `hbm dashboard` is running, then click Reconnect.\x1b[0m",
        );
      }
    })();
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="hbm-console-title"
    >
      <div
        className={cn(
          themedBody,
