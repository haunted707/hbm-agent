...
beforeEach(() => {
  ...
  Object.defineProperty(window, "__HBM_SESSION_TOKEN__", {
    configurable: true,
    value: "stale-token",
    writable: true,
  });
  Object.defineProperty(window, "__HBM_AUTH_REQUIRED__", {
    configurable: true,
    value: false,
    writable: true,
  });
...
describe("ChatPage", () => {
  ...
  it("reconnects on tab return after a hidden-tab close even when a stale upload banner is showing", async () => {
    ...
    const host = container.querySelector(".hbm-chat-xterm-host");
  ...
  it("stops retrying after the ladder is spent and offers Check server status", async () => {
    ...
    try {
      ...
      expect(container.textContent).toMatch(/Lost connection to the HBM AGENT dashboard server/);
      expect(container.textContent).toContain("hbm dashboard");
      const labels = Array.from(container.querySelectorAll("button")).map((b) => b.textContent?.trim());
...
describe("ChatPage side panel collapse", () => {
  ...
  it("collapses the desktop side panel and persists the choice", async () => {
    ...
    expect(localStorage.getItem("hbm-chat-panel-collapsed")).toBe("1");
    expect(
    ...
    expect(localStorage.getItem("hbm-chat-panel-collapsed")).toBe("0");
    expect(
      container.querySelector('[aria-label="Collapse chat side panel"]'),
    ).not.toBeNull();
  });
