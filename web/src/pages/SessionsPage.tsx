...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    hbm_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__hbm_dashboard_no_matching_source__";
    ...
    async (id: string) => {
      try {
        const res = await fetch(api.exportSessionUrl(id, rowProfile(id)), {
          credentials: "include",
          headers: {
            "X-HBM AGENT-Session-Token":
              (window as unknown as { __HBM_SESSION_TOKEN__?: string })
                .__HBM_SESSION_TOKEN__ ?? "",
          },
        });
        if (!res.ok) {
