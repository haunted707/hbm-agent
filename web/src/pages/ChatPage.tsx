/**
 * ChatPage — embeds `hbm --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (hbm_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "hbm.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const tokenMissing =
    ...
    !window.__HBM_SESSION_TOKEN__ &&
    !window.__HBM_AUTH_REQUIRED__;
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("hbm-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("hbm-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__HBM_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner HBM AGENT TUI has enabled xterm mouse-events
    ...
    //      payloads upload to HBM_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to HBM_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[hbm-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__HBM_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets HBM_HOME pointed at the
    ...
    // forwarding them into HBM AGENT. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="hbm-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
