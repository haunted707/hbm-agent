...
// (e.g. https://mission-control.tilos.com/hbm/). The Python backend
// injects ``window.__HBM_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__HBM_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const HBM_BASE_PATH = readBasePath();
const BASE = HBM_BASE_PATH;
...
declare global {
  interface Window {
    __HBM_SESSION_TOKEN__?: string;
    __HBM_BASE_PATH__?: string;
    ...
    __HBM_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-HBM AGENT-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__HBM_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``hbm update``, ``hbm gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__HBM_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__HBM_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__HBM_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``hbm_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__HBM_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-HBM AGENT-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``hbm_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__HBM_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateHbm: () =>
    fetchJSON<ActionResponse>("/api/hbm/update", { method: "POST" }),
  checkHbmUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/hbm/update/check${force ? "?force=true" : ""}`,
    ),
...
/** Preflight for `hbm gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** hbm-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``hbm update``. */
  can_update_hbm?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the HBM_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the HBM_HOME volume — no staleness dimension, so no
 * sampled_at. */
