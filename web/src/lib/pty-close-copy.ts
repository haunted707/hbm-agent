...
export const PTY_TOKEN_MISSING_BANNER: PtyBanner = {
  text: "Chat can't connect because this page was opened without a login token. Reload the page, or start it again with `hbm dashboard` in a terminal.",
  action: 'reload'
}
...
const REJECTION_BANNERS: Record<number, PtyBanner> = {
  ...
  4403: {
    text: 'The dashboard refused this chat connection because the page address does not match the server it was opened from. Open the dashboard from the address `hbm dashboard` printed.',
    action: null
  },
  4404: {
    text: 'This HBM AGENT server does not offer the terminal chat. Update HBM AGENT (`hbm update`) and reload the page.',
    action: 'reload'
  },
  4408: {
    text: 'This HBM AGENT server only accepts chat from the machine it runs on. Open the dashboard on that machine, or start it with a public bind.',
    action: null
  }
}
...
export const PTY_GAVE_UP_BANNER: PtyBanner = {
  text: 'Lost connection to the HBM AGENT dashboard server. If you stopped `hbm dashboard`, start it again; otherwise click Reconnect now.',
  action: 'check-server'
}

/** Overlay copy: the hbm --tui child exited; a crash looks identical to `/exit`. */
