...
export const API_UNREACHABLE_MESSAGE =
  "HBM AGENT dashboard cannot reach the HBM AGENT service. Is `hbm dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The HBM AGENT service hit an internal error.",
  502: "The dashboard proxy could not reach the HBM AGENT service.",
  503: "The HBM AGENT service is not ready yet. Try again in a moment.",
  504: "The HBM AGENT service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The HBM AGENT service answered with an unexpected error (${status}).`;
}
