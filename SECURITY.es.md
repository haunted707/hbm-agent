# Política de Seguridad de HBM AGENT

Este documento describe el modelo de confianza de HBM AGENT, identifica el
...
vulnerabilidades de seguridad. **HBM AGENT no opera un programa de
...
- Detalles del entorno (`hbm --version`, SHA del commit, SO, versión de Python).
...
HBM AGENT es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta HBM AGENT,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a HBM AGENT al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de HBM AGENT
...
HBM AGENT admite dos posturas de aislamiento a nivel de SO. Abordan
...
HBM AGENT admite esto de dos maneras:

- **La propia imagen Docker de HBM AGENT y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de HBM AGENT
...
HBM AGENT filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en HBM AGENT
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de HBM AGENT
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   HBM AGENT no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de HBM AGENT o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  HBM AGENT ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de hbm-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de HBM AGENT
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en HBM AGENT. Los errores en la ruta de
  instalación de habilidades o plugins de HBM AGENT que impidan al
...
- HBM AGENT incluye guardias de cadena de suministro para lanzamientos de servidores
