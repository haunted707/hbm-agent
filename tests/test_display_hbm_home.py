"""display_hbm_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom HBM_HOME under
the user profile (e.g. AppData/Local/hbm), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\hbm`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\hbm/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayHbmHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        nested = fake_userhome / "AppData" / "Local" / "hbm"
        with patch.object(hc, "get_hbm_home", return_value=nested):
            out = hc.display_hbm_home()
        self.assertEqual(out, "~/AppData/Local/hbm")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import hbm_constants as hc

        with patch.object(hc, "get_hbm_home",
                          return_value=Path.home() / ".hbm"):
            out = hc.display_hbm_home()
        self.assertEqual(out, "~/.hbm")

    def test_outside_home_falls_back_to_absolute(self):
        import hbm_constants as hc

        outside = Path("/opt/hbm-custom") if os.name != "nt" else Path("C:/opt/hbm-custom")
        with patch.object(hc, "get_hbm_home", return_value=outside):
            out = hc.display_hbm_home()
