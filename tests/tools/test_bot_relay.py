    ...
    return [
        {
            ...
            "connection_label": "HBM AGENT Cloud",
            "title": "Moxie",
...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "hbm", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@hbm", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("hbm@cloud-1", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("hbm@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "hbm" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "hbm", "connection_id": "cloud-1",
         "connection_label": "HBM AGENT Cloud", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="hbm", message="ping", agent=agent))
    assert out.get("status") == "sent"
    assert "HBM AGENT Cloud" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 hbm (@hbm): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@hbm` — on HBM AGENT Cloud — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="hbm",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("HBM_HOME", str(tmp_path / "nope"))
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "hbm", "connection_id": "cloud-1",
         "connection_label": "HBM AGENT Cloud", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "sent"}),
...
# ── delivery turn author (HBM_TURN_AUTHOR on the recipient turn) ──────────
...
def test_delivery_env_carries_only_the_given_author(monkeypatch):
    """The dispatcher's own HBM_TURN_AUTHOR never reaches the child: dropped without an author, replaced with one."""
    from agent.turn_author import TURN_AUTHOR_ENV

    monkeypatch.setenv("HBM_RELAY_TEST_MARKER", "kept")
    monkeypatch.setenv(TURN_AUTHOR_ENV, json.dumps({"id": "bot:previous", "name": "previous", "is_bot": True}))
    monkeypatch.setenv("HBM_SESSION_KEY", "session-A")
    monkeypatch.setenv("HBM_UI_SESSION_ID", "ui-A")
    monkeypatch.setenv("HBM_SESSION_ID", "session-A")
    monkeypatch.setenv("HBM_SESSION_PROFILE", "profile-A")
    # A session-* knob, not identity: stripping it would break the child's watcher tuning.
    monkeypatch.setenv("HBM_SESSION_STALL_TIMEOUT", "97")
    ...
    assert env["HBM_RELAY_TEST_MARKER"] == "kept"
    assert "HBM_SESSION_KEY" not in env
    assert "HBM_UI_SESSION_ID" not in env
    assert "HBM_SESSION_ID" not in env
    assert "HBM_SESSION_PROFILE" not in env
    assert env["HBM_SESSION_STALL_TIMEOUT"] == "97"
