...
def test_cannot_message_self(tmp_path):
    ...
    result = json.loads(
        bot_mode_dm.message_agent_tool(target="hbm", message="hi", agent=agent)
    )
    assert "error" in result
...
def test_local_delivery_command_and_ack(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(bot_relay, "_hbm_cli", lambda: "hbm")
...
def test_peer_delivery_command_pins_registry_profile_for_secondary_bots(
    ...
    assert transport_argv == ["hbm", "-p", "default", "peer", "dm", "spark"]
...
def test_peer_delivery_command(tmp_path, monkeypatch):
    ...
    mode, _dm_file, transport_argv = _runner_parts(calls[0]["command"])
    assert mode == "stdin"
    assert transport_argv == ["hbm", "-p", "default", "peer", "dm", "spark/researcher"]
    # the peer child reads the author from its env and forwards it in the request body
    assert _runner_author(calls[0]["command"]) == {"id": "bot:eri-mac.local/default", "name": "hbm", "is_bot": True}
...
def test_delivery_pins_the_hbm_entrypoint_beside_this_interpreter(tmp_path, monkeypatch):
    """A background delivery must not rely on PATH: the runner's service context
    lacks the gateway's venv bin dir, so a bare ``hbm`` resolves to a system
    ...
    hbm_entry = venv_bin / ("hbm.exe" if sys.platform == "win32" else "hbm")
    hbm_entry.write_text("#!/bin/sh\n", encoding="utf-8")
    ...
    mode, _dm_file, transport_argv = _runner_parts(calls[0]["command"])
    assert mode == "query-file"
    assert transport_argv[0] == str(hbm_entry)
    ...
    mode, _dm_file, transport_argv = _runner_parts(calls[1]["command"])
    assert mode == "stdin"
    assert transport_argv == [str(hbm_entry), "-p", "default", "peer", "dm", "spark"]
...
def test_named_profile_sender_prefix(tmp_path, monkeypatch):
    """A named-profile bot signs with its own handle, not @hbm."""
    calls = _capture_spawn(monkeypatch)
    home = _managed_home(tmp_path, teammates=("researcher", "coder"))
...
def test_delivery_command_author_json_survives_quoting_and_windows_slash_rewrite(tmp_path, monkeypatch):
    ...
    author = {"id": "bot:default", "name": "hbm", "is_bot": True}
    command = bot_mode_dm._delivery_command(["hbm", "-p", "x"], str(tmp_path / "dm.txt"),
    ...
    assert _runner_parts(command) == ("query-file", str(tmp_path / "dm.txt"), ["hbm", "-p", "x"])

    monkeypatch.setattr(sys, "platform", "win32")
    command = bot_mode_dm._delivery_command(["hbm", "-p", "x"], "C:\\Users\\me\\dm.txt",
    ...
    command = bot_mode_dm._delivery_command(["hbm"], "dm.txt", stdin_file=True)
...
def test_live_dm_admitted_before_waiter_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HBM_HOME", str(tmp_path / "wrong-home"))
    ...
    assert record["message"] == "Message from 🤖 hbm (@hbm): hello"
    assert record["author"] == {"id": "bot:default", "name": "hbm", "is_bot": True}
...
def test_delivery_runner_unlinks_when_child_launch_raises(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="child launch failed"):
        bot_mode_dm._run_delivery(["hbm"], str(dm_file), stdin_file=False)
    assert not dm_file.exists()
...
def test_local_turn_reemits_empty_stdout_for_a_bare_silence_marker(tmp_path, capsys):
    """#110782: the one-shot ``hbm chat -c "Bot Chat"`` transport applies the gateway's
...
def test_query_file_delivery_closes_stdin_for_initial_attempt_and_retry(
    ...
    returncode = bot_mode_dm._run_delivery(
        ["hbm", "-p", "researcher"], str(dm_file), stdin_file=False
    )
...
def test_delivery_main_child_env_carries_only_the_argv_author(tmp_path, monkeypatch, mode, author):
    """The ``--author`` payload becomes HBM_TURN_AUTHOR on the child. Without it the runner drops the
    ...
    monkeypatch.setattr(subprocess, "run", fake_run)
    monkeypatch.setenv("HBM_DM_TEST_MARKER", "kept")
    ...
    returncode = bot_mode_dm._delivery_main(
        ["--run-delivery", *author_args, mode, str(dm_file), "hbm", "-p", "researcher"])

    assert returncode == 0
    [(argv, kwargs)] = calls
    assert argv[:3] == ["hbm", "-p", "researcher"]
    assert kwargs["env"]["HBM_DM_TEST_MARKER"] == "kept"
...
def test_real_delivery_command_round_trip_carries_author(tmp_path):
    """Through a real subprocess, the runner argv built by ``_delivery_command`` sets HBM_TURN_AUTHOR on the child."""
    dm_file = tmp_path / "message.txt"
    dm_file.write_text("secret", encoding="utf-8")
    observed = tmp_path / "observed.txt"
    child = tmp_path / "child.py"
    child.write_text(
        ...
        "pathlib.Path(sys.argv[1]).write_text(os.environ.get('HBM_TURN_AUTHOR', 'unset'), encoding='utf-8')\n",
        encoding="utf-8",
    )
    author = {"id": "bot:default", "name": "hbm", "is_bot": True}
...
def test_sweeper_removes_only_stale_dm_files(tmp_path, monkeypatch):
    ...
    legacy_stale = tmp_path / "hbm-dm-stale.txt"
