...
class TestSensitivePathCheck:
    ...
    def test_hbm_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved_loaded", True)

        from tools.file_tools import write_file_tool
        result = json.loads(write_file_tool(str(fake_config), "approvals:\n  mode: off\n"))
        assert "error" in result
        assert "HBM AGENT config" in result["error"]

    def test_hbm_config_blocked_via_tilde_path(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved_loaded", True)
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved", "/some/other/path")
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved", "/home/user/.hbm/config.yaml")
        monkeypatch.setattr("tools.file_tools_write_guards._hbm_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
...
class TestSecretFileReadRedaction:
    ...
    def hbm_home(self, tmp_path, monkeypatch):
        """A HBM AGENT home with no ``.hbm`` segment, like ``%LOCALAPPDATA%\\hbm``."""
        import agent.file_safety as file_safety

        home = tmp_path / "hbm"
        home.mkdir()
        monkeypatch.setattr(file_safety, "_hbm_home_path", lambda: home)
        monkeypatch.setattr(file_safety, "_hbm_root_path", lambda: home)
        return home
    ...
    @patch("tools.file_tools._get_file_ops")
    def test_read_file_of_hbm_config_masks_opaque_token(self, mock_get, hbm_home):
        ...
        from tools.file_tools import read_file_tool
        out = json.loads(read_file_tool(str(hbm_home / "config.yaml"), task_id="secret-read"))
        ...
        out = json.loads(read_file_tool(str(hbm_home.parent / "proj-config.yaml"), task_id="plain-read"))
        assert self.SYNTH in out["content"]

    @patch("tools.file_tools._get_file_ops")
    def test_search_in_hbm_home_masks_opaque_token(self, mock_get, hbm_home):
        config = hbm_home / "config.yaml"
        ops = MagicMock()
        ...
        from tools.file_tools import search_tool
        raw = search_tool(pattern="ADS_API_TOKEN", path=str(hbm_home),
