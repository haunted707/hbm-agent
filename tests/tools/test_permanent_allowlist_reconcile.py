...
So a hand edit made while an HBM AGENT process is live was undone by the next
...
def fake_config(monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.config.load_config", _load, raising=False)
    monkeypatch.setattr("hbm_cli.config.save_config", _save, raising=False)
...
def test_a_second_process_writing_first_does_not_lose_this_ones_approval(fake_config):
    """Two live HBM AGENT processes. Whoever writes second must not drop the first."""
...
def test_save_failure_is_logged_not_raised(fake_config, monkeypatch, caplog):
    ...
    monkeypatch.setattr("hbm_cli.config.load_config", _boom, raising=False)
    approval.approve_permanent("docker *")
