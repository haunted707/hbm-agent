...
def two_profiles(tmp_path, monkeypatch):
    ...
    def enter(which):
        tokens.append(set_hbm_home_override(homes[which]))
        return hbm_home_key(homes[which])

    yield enter
    for tool_name in list(registry.get_tool_names_for_toolset("mcp-x")):
        for home in homes.values():
            registry.deregister(tool_name, scope=hbm_home_key(home))
...
def test_served_profile_without_multiplex_flag_gets_its_own_connection(two_profiles, monkeypatch):
    """A dashboard/desktop backend serves profiles through the HBM_HOME override with
    ...
    with patch("hbm_constants.get_hbm_home_override", return_value=None):
