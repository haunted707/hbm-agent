...
and the shared slash-command handlers (hbm_cli/write_approval_commands.py).
...
@pytest.fixture
def hbm_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="hbm_wa_test_")
    home = os.path.join(d, ".hbm")
    os.makedirs(home)
    monkeypatch.setenv("HBM_HOME", home)
    yield home
...
def test_default_gate_is_off(hbm_home):
...
def test_invalid_subsystem_is_off(hbm_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(hbm_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(hbm_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(hbm_home, monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.config.load_config", _boom)
...
def test_handle_approve_all(hbm_home):
...
def test_handle_approval_on(hbm_home):
    from hbm_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(hbm_home):
    from hbm_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(hbm_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(hbm_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(hbm_home):
