...
def _is_hbm_cli(argv) -> bool:
    """Match the delivery CLI by basename — local_delivery_command may
    resolve the venv-relative hbm next to the interpreter (#93590)."""
    name = str(argv[0]).rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    return name in ("hbm", "hbm.exe")
...
def _transport_calls(calls):
    ...
    return [argv for argv in calls if argv and _is_hbm_cli(argv)]
