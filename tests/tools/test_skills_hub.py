...
from tools.skills_hub_search import (
    HBM_INDEX_TTL, _load_hbm_index, create_source_router, parallel_search_sources, unified_search,
)
...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  hbm:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["hbm"]["tags"] == ["a", "b"]
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `hbm skills browse`."
            )
...
def _make_index_source(skills):
    """Build a HbmIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub_official import HbmIndexSource
    src = HbmIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestHbmIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "hbm-index"
...
class TestOptionalSkillSourceLiveRepoFallback:
    ...
    searchable and installable without `hbm update` (live-repo fallback)."""
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``hbm skills install … --name
...
# ---------------------------------------------------------------------------
# _load_hbm_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadHbmIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "hbm-index.json"
        monkeypatch.setattr("tools.skills_hub_search._hbm_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (HBM_INDEX_TTL + 100)
        import os
