...
``~/.hbm/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.hbm/.env`` would be told
...
class TestTranscribeCallSitesReadDotenv:
    ...
    def test_transcribe_groq_forwards_dotenv_key(self):
        ...
        with patch("hbm_cli.config.get_env_value", return_value="groq-dotenv-key"), \
             patch.object(tt, "_HAS_OPENAI", True), \
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``hbm_cli.config.load_env`` to simulate
    ``~/.hbm/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("hbm_cli.config.load_env",
