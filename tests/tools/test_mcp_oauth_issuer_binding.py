...
from tools.mcp_oauth import HbmTokenStorage  # noqa: E402
...
def test_legacy_file_without_issuer_adopts_current_issuer_once(tmp_path):
    ...
    assert json.loads(_token_file(tmp_path).read_text())["hbm_issuer"] == "https://as.example.com"
    # Now bound: a later issuer change is rejected.
    storage2 = HbmTokenStorage("srv", hbm_home=tmp_path)
...
def test_set_tokens_stamps_bound_issuer_and_get_tokens_keeps_it_out_of_the_sdk_model(tmp_path):
    ...
    assert json.loads(_token_file(tmp_path).read_text())["hbm_issuer"] == "https://as.example.com"
    fresh = HbmTokenStorage("srv", hbm_home=tmp_path)
    tokens = asyncio.run(fresh.get_tokens())
    assert fresh.loaded_issuer == "https://as.example.com"
    assert not hasattr(tokens, "hbm_issuer")  # never leaks into the wire model
