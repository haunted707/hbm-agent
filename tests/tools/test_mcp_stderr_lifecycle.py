...
def test_scoped_shutdown_releases_log_and_preserves_other_profile(tmp_path):
    ...
    for name in ("first", "second"):
        token = set_hbm_home_override(tmp_path / name)
        try:
    ...
    try:
        shutdown_mcp_servers(scope=hbm_home_key(tmp_path / "first"))
        assert first.closed
        second.write("other profile remains usable\n")
        second.flush()
        token = set_hbm_home_override(tmp_path / "first")
        try:
...
def test_rename_profile_releases_cached_log_handle(tmp_path, monkeypatch):
    ...
    token = set_hbm_home_override(old_dir)
    try:
    ...
    try:
        ...
        assert hbm_home_key(old_dir) not in mcp_tool_config._mcp_stderr_log_fh
