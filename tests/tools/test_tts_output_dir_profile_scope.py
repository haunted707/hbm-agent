...
``HBM_HOME`` or ``set_hbm_home_override()``. The call-time accessor
...
def _reload_tts_tool(import_home: Path, monkeypatch):
    monkeypatch.setenv("HBM_HOME", str(import_home))
...
def test_default_output_dir_follows_contextvar_profile_override(tmp_path, monkeypatch):
    """The web server scopes profiles via set_hbm_home_override() rather
