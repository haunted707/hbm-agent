...
Older images made /opt/hbm/ui-tui and /opt/hbm/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HBM_TUI_DIR=/opt/hbm/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hbm:hbm /opt/hbm/ui-tui" not in text
    assert "chown -R hbm:hbm /opt/hbm/node_modules" not in text
