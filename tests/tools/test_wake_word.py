"""Tests for tools.wake_word — the "Hey HBM AGENT" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey hbm"}) == "hey hbm"
    assert ww.wake_phrase({}) == "hey hbm"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_hbm.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "hbm_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey hbm"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey hbm"
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_hbm.onnx"}}
    )
    assert calls["download"] == [["/models/hey_hbm.onnx"]]
    assert eng._labels == ["hey_hbm"]


def test_bundled_hey_hbm_model_ships_on_disk():
    # The "hey hbm" wake word works out of the box only if the model is
