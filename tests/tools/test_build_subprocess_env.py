...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_hbm_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["HBM_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_hbm_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see HBM_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('HBM_HOME'), "
    ...
    assert result["home"] == str(hbm_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `hbm` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_hbm_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no hbm console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``hbm-agent``.
    ...
    bin_dir = local_mod._resolve_hbm_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "hbm-agent.exe" if os.name == "nt" else "hbm-agent")
    ):
        pytest.skip("no real hbm console-script install available")

    # Simulate the service-manager minimal PATH: hbm dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("hbm-agent", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("hbm-agent", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'hbm-agent' must resolve from the child PATH {env.get('PATH')!r}"
    )
