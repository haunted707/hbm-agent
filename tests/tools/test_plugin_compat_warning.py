...
Kept alongside the compat layer (tests/hbm_cli/test_compat_manifest_targets.py); both are deleted with it.
"""
import importlib
import json
import warnings
...
def test_compat_resolution_warns_once_per_name_with_the_new_location():
    from hbm_cli.plugin_compat import HbmPluginCompatWarning, _seen
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        getattr(mod, name)
        getattr(mod, name)
    ours = [w for w in rec if issubclass(w.category, HbmPluginCompatWarning)]
...
def test_importing_the_facade_itself_does_not_warn():
    ...
    from hbm_cli.plugin_compat import HbmPluginCompatWarning
    import sys
    sys.modules.pop("tools.web_tools", None)
    with warnings.catch_warnings(record=True) as rec:
        warnings.simplefilter("always")
        importlib.import_module("tools.web_tools")
    assert not [w for w in rec if issubclass(w.category, HbmPluginCompatWarning)]
