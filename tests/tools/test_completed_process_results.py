...
def test_headless_terminal_result_survives_cli_exit(tmp_path):
    ...
    env = {**os.environ, "HBM_HOME": str(home), "HOME": str(tmp_path),
    ...
    def read_result(profile):
        result = subprocess.run([sys.executable, "-c", consumer, process_id],
                                cwd=tmp_path, env={**env, "HBM_HOME": str(profile)},
    ...
    env["HBM_SESSION_ID"] = receipt["parent_session_id"]
...
def test_receipts_are_bounded_redacted_and_session_scoped(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HBM_SESSION_ID", "owner-session")
    sessions = []
    ...
    paths = list((get_hbm_home() / "logs" / "process-results").glob("*.json"))
    assert len(paths) == 2
    ...
    assert not (get_hbm_home() / "logs" / "process-results" / f"{child.id}.json").exists()
