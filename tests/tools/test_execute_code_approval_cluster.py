...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad HBM_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: HBM_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hbm_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "HBM_HOME": "/h", "HBM_PROFILE": "p",
        "HBM_CONFIG": "/c.yaml", "HBM_ENV": "/e",
        "HBM_DELEGATED_CHILD_CONTEXT": "1",
        # other HBM_* → dropped (broad prefix removed)
        "HBM_BASE_URL": "https://x", "HBM_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "HBM_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "HBM_BASE_URL", "HBM_INTERACTIVE", "HBM_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped HBM_* vars."""
    import logging
