...
def worker_env(monkeypatch, tmp_path):
    """Isolated HBM_HOME with a running task; returns the task id."""
    home = tmp_path / ".hbm"
    home.mkdir()
...
# ---------------------------------------------------------------------------
# Negative test — force=True bypasses HBM_REDACT_SECRETS=false
# ---------------------------------------------------------------------------

def test_scrub_respects_force_flag_regardless_of_config(worker_env, monkeypatch):
    """force=True must fire even when HBM_REDACT_SECRETS=false is set."""
