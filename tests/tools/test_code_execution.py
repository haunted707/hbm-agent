...
UDS socket lifecycle, hbm_tools generation, timeout enforcement,
...
class TestHbmToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_hbm_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"hbm_rpc\")", src)
        self.assertNotIn('os.environ.get("HBM_RPC_DIR", "/tmp/hbm_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_hbm_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "hbm_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "hbm_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/hbm_exec_", mkdir_cmd)
        self.assertIn("HBM_RPC_DIR=/data/data/com.termux/files/usr/tmp/hbm_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/hbm_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/hbm_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """HBM_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"HBM_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import hbm_constants; print(hbm_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("hbm_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from hbm_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from hbm_tools import json_parse
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_hbm_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("HBM_RPC_SOCKET", child_env)
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("hbm_cli.config.load_config_readonly", return_value={}):
            result = _load_config()
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching HBM_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated hbm_tools module reads HBM_RPC_TOKEN and sends it."""
        src = generate_hbm_tools_module(["terminal"], transport="uds")
        self.assertIn("HBM_RPC_TOKEN", src)
