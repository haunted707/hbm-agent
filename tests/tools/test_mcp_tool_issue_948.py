...
def test_resolve_stdio_command_falls_back_to_hbm_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool_config.shutil.which", return_value=None), \
         patch.dict("os.environ", {"HBM_HOME": str(tmp_path)}, clear=False):
...
def test_windows_managed_node_root_prefers_cmd_launchers(tmp_path):
    """Managed Windows Node lives directly in ``<HBM_HOME>/node`` (no ``bin``) as ``npx.cmd`` /
...
def test_node_fallback_uses_active_profile_home(tmp_path, monkeypatch):
    """The managed-Node lookup follows ``get_hbm_home()`` (context override), not raw ``HBM_HOME``:
    ...
    npx_path.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
    npx_path.chmod(0o755)
    monkeypatch.setenv("HBM_HOME", str(tmp_path / "launch-home"))
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$HBM_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the HBM AGENT Docker image
    ...
    # the other candidates ($HBM_HOME/node/bin/npx and ~/.local/bin/npx)
