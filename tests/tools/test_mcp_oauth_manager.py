...
def test_manager_isolates_same_named_servers_by_profile_home(tmp_path, monkeypatch):
    ...
    for home, access_token in ((profile_a, "TOKEN_A"), (profile_b, "TOKEN_B")):
        ...
        try:
            storage = HbmTokenStorage("shared")
...
def test_hbm_provider_subclass_exists():
    """HbmMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _HBM_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _HBM_PROVIDER_CLS is not None
    assert issubclass(_HBM_PROVIDER_CLS, OAuthClientProvider)
...
#
# Two HBM AGENT backends routinely share one HBM_HOME (desktop `serve` +
...
async def test_refresh_400_does_not_strand_a_rejected_token_in_the_context(
    ...
    recovered = await provider._hbm_reload_tokens_after_refresh_failure()
...
async def test_concurrent_refresh_presents_single_use_token_exactly_once(tmp_path, monkeypatch):
    ...
    assert a._hbm_fence is None and b._hbm_fence is None
...
async def test_refresh_fails_closed_while_a_peer_holds_the_fence(tmp_path, monkeypatch):
    ...
    assert provider._hbm_fence is None
