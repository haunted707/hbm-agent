...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "hbm_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "hbm_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    hbm_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["hbm_cli"] = hbm_cli
    sys.modules["hbm_cli.config"] = types.SimpleNamespace(
        get_hbm_home=lambda: Path(tempfile.gettempdir()) / "hbm-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.hbm/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
