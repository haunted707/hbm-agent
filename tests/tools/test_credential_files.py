...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        hbm_home.mkdir()
        (hbm_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"HBM_HOME": str(hbm_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(hbm_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.hbm/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.hbm/skills"


    def test_custom_container_base(self, tmp_path):
        hbm_home = tmp_path / ".hbm"
        (hbm_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"HBM_HOME": str(hbm_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.hbm")

        assert mounts[0]["container_path"] == "/home/user/.hbm/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.hbm/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.hbm/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.hbm/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.hbm/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape HBM_HOME."""
        monkeypatch.setenv("HBM_HOME", str(tmp_path / ".hbm"))
        (tmp_path / ".hbm").mkdir()

        # Create a sensitive file one level above hbm_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside hbm_home
    ...
    def test_nested_subdir_inside_hbm_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of HBM_HOME must be allowed."""
        hbm_home = tmp_path / ".hbm"
        hbm_home.mkdir()
        subdir = hbm_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside HBM_HOME pointing outside must be rejected."""
        hbm_home = tmp_path / ".hbm"
        hbm_home.mkdir()
        monkeypatch.setenv("HBM_HOME", str(hbm_home))

        # Create a sensitive file outside hbm_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside hbm_home pointing outside
        symlink = hbm_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes HBM_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, hbm_home: Path, cred_files: list):
        import yaml
        config_path = hbm_home / "config.yaml"
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape HBM_HOME."""
        hbm_home = tmp_path / ".hbm"
        hbm_home.mkdir()
        monkeypatch.setenv("HBM_HOME", str(hbm_home))
        ...
        sensitive.write_text("{}")
        self._write_config(hbm_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(hbm_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside HBM_HOME via config must still mount."""
        hbm_home = tmp_path / ".hbm"
        hbm_home.mkdir()
        monkeypatch.setenv("HBM_HOME", str(hbm_home))

        (hbm_home / "oauth.json").write_text("{}")
        self._write_config(hbm_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        hbm_home = tmp_path / ".hbm"
        hbm_home.mkdir()
        (hbm_home / "cache" / "documents").mkdir(parents=True)
        (hbm_home / "cache" / "audio").mkdir(parents=True)
        (hbm_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("HBM_HOME", str(hbm_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.hbm/cache/documents" in paths
        assert "/root/.hbm/cache/audio" in paths
        assert "/root/.hbm/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_hbm_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        hbm_home = tmp_path / ".hbm"
        hbm_home.mkdir()
        # Use legacy dir name with content — get_hbm_dir prefers
        # populated old over new.
        legacy_doc = hbm_home / "document_cache"
        legacy_img = hbm_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(hbm_home / "document_cache") in host_paths
        assert str(hbm_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.hbm/cache/documents" in container_paths
        assert "/root/.hbm/cache/images" in container_paths
    ...
    def test_empty_hbm_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.hbm/attachments" in container_paths
        assert "/root/.hbm/images" in container_paths
        assert "/root/.hbm/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``HBM_HOME/images``, not
        ...
        """
        hbm_home = tmp_path / ".hbm"
        (hbm_home / "images").mkdir(parents=True)
        monkeypatch.setenv("HBM_HOME", str(hbm_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.hbm/images" in by_container
        assert by_container["/root/.hbm/images"] == str(hbm_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        hbm_home = tmp_path / ".hbm"
        (hbm_home / "images").mkdir(parents=True)
        upload = hbm_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("HBM_HOME", str(hbm_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.hbm/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.hbm/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(hbm_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.hbm/cache/images/x.png"
        assert (hbm_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    HBM AGENT cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        hbm_home = tmp_path / ".hbm"
        (hbm_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("HBM_HOME", str(hbm_home))
        return str(hbm_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_hbm(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.hbm/attachments/drop.zip"

    def test_ssh_maps_to_tilde_hbm(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.hbm/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        hbm_home = tmp_path / ".hbm"
        doc_dir = hbm_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — HBM_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping HBM_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"HBM_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.hbm/google_token.json" in paths
        assert "/root/.hbm/.env" not in paths
