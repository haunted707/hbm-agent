...
def _worker_board(tmp_path, monkeypatch):
    ...
    for key, value in {
        "HBM_KANBAN_DB": str(db), "HBM_KANBAN_BOARD": "default",
        "HBM_KANBAN_TASK": own, "HBM_KANBAN_RUN_ID": str(task.current_run_id),
        "HBM_KANBAN_CLAIM_LOCK": task.claim_lock, "HOME": str(tmp_path),
    }.items():
...
def test_terminal_descendants_cannot_mutate_even_after_task_is_removed(tmp_path, monkeypatch):
    ...
    script.write_text(
        ...
        " 'db': os.getenv('HBM_KANBAN_DB'), 'board': os.getenv('HBM_KANBAN_BOARD')}\n"
        ...
        "os.environ.pop('HBM_KANBAN_TASK', None)\n"
        f"p = subprocess.run([sys.executable, '-m', 'hbm_cli.main', 'kanban', 'complete', foreign, '--result', 'must refuse'], cwd={str(ROOT)!r}, capture_output=True, text=True, stdin=subprocess.DEVNULL, timeout=45)\n"
    ...
    child_envs = [
        ...
        _build_safe_env({"HBM_HOME": os.environ["HBM_HOME"]}),
    ]
...
def test_worker_cli_cannot_use_foreign_task_to_drop_run_scope(tmp_path, monkeypatch):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "hbm_cli.main", "kanban", "complete", foreign, "--result", "no"],
    ...
    for arguments in (["attach", foreign, str(attachment)], ["unblock", foreign]):
        proc = subprocess.run([sys.executable, "-m", "hbm_cli.main", "kanban", *arguments],
                              cwd=ROOT, env=dict(os.environ), stdin=subprocess.DEVNULL,
                              capture_output=True, text=True, timeout=45)
...
def test_child_shell_can_write_a_kanban_board_outside_its_lineage_root(tmp_path, monkeypatch):
    ...
    DB its shell touches: a repro run against a scratch HBM_HOME got a silently read-only board
    ...
    monkeypatch.setenv("HBM_HOME", str(lineage_home))
    monkeypatch.delenv("HBM_DELEGATED_CHILD_CONTEXT", raising=False)
    for key in ("HBM_KANBAN_DB", "HBM_KANBAN_BOARD", "HBM_KANBAN_TASK", "HBM_KANBAN_HOME"):
    ...
    script.write_text(
        ...
        "from hbm_cli import kanban_db as kb\n"
        "from hbm_cli.kanban_db_connect import connect\n"
        "out = {'marker': os.environ.get('HBM_DELEGATED_CHILD_CONTEXT')}\n"
        "try:\n"
        "    conn = connect(kb.kanban_db_path()); kb.create_task(conn, title='lineage'); out['lineage'] = 'WROTE'\n"
        "except PermissionError as exc:\n"
        "    out['lineage'] = 'fenced: ' + str(exc)\n"
        f"os.environ['HBM_HOME'] = {str(scratch_home)!r}\n"
        "import hbm_constants; hbm_constants._default_hbm_root_memo = None\n"
