...
HBM AGENT state stays profile-scoped through HBM_HOME. Host subprocesses should
...
class TestGetSubprocessHome:
    """Unit tests for hbm_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(hbm_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("HBM_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        hbm_home = real_home / ".hbm" / "profiles" / "coder"
        profile_home = hbm_home / "home"
        profile_home.mkdir(parents=True)
        monkeypatch.setenv("HOME", str(real_home))
        monkeypatch.setenv("HBM_HOME", str(hbm_home))
        from hbm_constants import get_subprocess_home
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_REAL_HOME", str(real_home))

        from hbm_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("HBM_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HBM_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(hbm_home / "home")
        assert result["HBM_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_hbm_home_unset(self, monkeypatch):
