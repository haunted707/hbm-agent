...
def _restore_tool_modules():
    original_hbm_home = os.environ.get("HBM_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_hbm_home is None:
            os.environ.pop("HBM_HOME", None)
        else:
            os.environ["HBM_HOME"] = original_hbm_home
        _reset_modules(("tools", "hbm_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    hbm_cli = types.ModuleType("hbm_cli")
    hbm_cli.__path__ = [str(REPO_ROOT / "hbm_cli")]  # type: ignore[attr-defined]
    sys.modules["hbm_cli"] = hbm_cli
    hbm_home = tmp_path / "hbm-home"
    os.environ["HBM_HOME"] = str(hbm_home)
    sys.modules["hbm_cli.config"] = types.SimpleNamespace(
        get_hbm_home=lambda: hbm_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="hbm-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": hbm_home / "modal_snapshots.json",
