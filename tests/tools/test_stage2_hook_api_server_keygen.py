...
was gated on ``[ -f "$HBM_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'HBM_HOME="{home}"\n'
        # In tests we run unprivileged; as_hbm is a passthrough then.
        'as_hbm() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    HBM AGENT loads $HBM_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/hbm/.env.example -> $HBM_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $HBM_HOME/.env from it on first boot"
    )
