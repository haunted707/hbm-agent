"""Tests for file write safety and HBM_WRITE_SAFE_ROOT sandboxing.
...
class TestSafeWriteRoot:
    """HBM_WRITE_SAFE_ROOT should sandbox writes to a specific subtree."""
    ...
    def test_safe_root_does_not_override_static_deny(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_WRITE_SAFE_ROOT", os.path.expanduser("~"))
        assert _is_write_denied(os.path.expanduser("~/.ssh/id_rsa")) is True


class TestMultipleSafeWriteRoots:
    """HBM_WRITE_SAFE_ROOT with multiple colon-separated directories."""
    ...
    def test_write_inside_first_root_allowed(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_WRITE_SAFE_ROOT", f"{root_a}{os.pathsep}{root_b}")
        assert _is_write_denied(str(child)) is False
    ...
    def test_trailing_separator_ignored(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_WRITE_SAFE_ROOT", f"{root}{os.pathsep}")
        assert _is_write_denied(str(inside)) is False
...
class TestGetWriteDeniedError:
    ...
    def test_credential_path_message(self):
        ...
        assert "HBM_WRITE_SAFE_ROOT" not in err
    ...
    def test_safe_root_message(self, tmp_path: Path, monkeypatch):
        ...
        assert err is not None
        assert "outside HBM_WRITE_SAFE_ROOT" in err
...
class TestSafeRootDenialMessageIntegration:
    ...
    message when HBM_WRITE_SAFE_ROOT blocks a path.
    ...
    def test_write_file_safe_root_outside_shows_safe_root_message(
        ...
        assert "outside HBM_WRITE_SAFE_ROOT" in res.error
...
class TestAtomicWrite:
    ...
    def test_no_temp_file_leaked_on_success(self, ops, tmp_path: Path):
        ...
        assert [p for p in os.listdir(tmp_path) if ".hbm-tmp" in p] == []
...
class TestProtectedInstructionFiles:
    ...
    AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project-local .hbm
    ...
    def test_project_local_hbm_dir_is_gated(self, tmp_path, approvals):
        proj = tmp_path / "proj" / ".hbm"
        proj.mkdir(parents=True)
        approvals["answer"] = "deny"
        res = self._write(proj / "config.yaml")
        assert res.get("error") and "BLOCKED" in res["error"]

    def test_checkout_nested_under_hbm_dir_not_gated(self, tmp_path, approvals):
        """A repo living UNDER a .hbm dir (e.g. ~/.hbm/hbm-agent)
        must not have every write gated — only files directly inside a
        .hbm dir count as project config."""
        repo = tmp_path / ".hbm" / "some-repo" / "src"
        repo.mkdir(parents=True)
        res = self._write(repo / "module.py", "x = 1\n")
        assert not res.get("error"), res
        assert approvals["calls"] == []

    def test_real_hbm_home_not_gated_by_this_check(
        self, tmp_path, approvals, monkeypatch
    ):
        """~/.hbm itself is governed by existing guards, not this gate."""
        import tools.file_tools_write_guards as ft
        fake_home = tmp_path / ".hbm"
        (fake_home / "notes").mkdir(parents=True)
        monkeypatch.setattr(
            ft, "_get_real_hbm_home", lambda: str(fake_home.resolve())
        )
...
class TestProfileHomeExemptsHbmRoot:
    """issue #60: under ``hbm -p <name>`` (``HBM_HOME=<root>/profiles/<name>``)
    the exemption used to cover ONLY the profile dir, so the ROOT's direct files
    (LEDGER.md / MEMORY.md / SOUL.md ...) fell through to the ``.hbm`` component
    rule, were read as project-local ``.hbm`` config, and — having no approval
    channel headless — failed closed. That blocked #54 (LEDGER.md edit). The gate
    must exempt the whole HBM AGENT tree, exactly like the default profile does.
    """
    ...
    def _gate_on(self, monkeypatch):
        ...
        monkeypatch.setattr(ft, "_real_hbm_home_loaded", False)
        monkeypatch.setattr(ft, "_real_hbm_home_cached", None)
        yield
    ...
    def _profile_layout(self, tmp_path: Path):
        """A real-shaped HBM AGENT root: ``<tmp>/home/profiles/worker`` + root markers."""
        root = tmp_path / "home"
    ...
    def test_named_profile_scope_exempts_root_direct_files(self, tmp_path, monkeypatch, approvals):
        ...
        not project-local ``.hbm`` config: the write lands with no approval prompt."""
        import tools.file_tools_write_guards as ft
        ...
        token = set_hbm_home_override(str(profile))
        try:
            assert os.path.realpath(str(root)) in ft._hbm_exempt_homes()
    ...
    def test_only_a_real_hbm_root_is_exempt(self, tmp_path, monkeypatch, approvals):
        """Negatives hold with a named profile active: a checkout's ``.hbm/config.yaml`` and
        protected basenames stay gated (fail-closed, unwritten), and a coincidental
        ``.../profiles/<name>`` tree that is NOT an HBM AGENT root never exempts its parent."""
        import tools.file_tools_write_guards as ft
        from hbm_constants import reset_hbm_home_override, set_hbm_home_override

        root, profile = self._profile_layout(tmp_path)
        repo = tmp_path / "repo"
        (repo / ".hbm").mkdir(parents=True)
        monkeypatch.delenv("HBM_HOME", raising=False)
        token = set_hbm_home_override(str(profile))
        try:
            assert ft._protected_instruction_reason(str(repo / ".hbm" / "config.yaml"))
            assert ft._protected_instruction_reason(str(repo / "AGENTS.md")) == "AGENTS.md"
            target = repo / ".hbm" / "config.yaml"
        ...
        fake_profile = tmp_path / "not-a-hbm-root" / "profiles" / "worker"
        fake_profile.mkdir(parents=True)
        token = set_hbm_home_override(str(fake_profile))
        try:
            assert ft._hbm_exempt_homes() == (os.path.realpath(str(fake_profile)),)
        finally:
            reset_hbm_home_override(token)


class TestMultiplexProfileWriteGuardsAreProfileScoped:
    """#107327: a multiplexed gateway scopes ``HBM_HOME`` per turn via a
    ...
    def test_home_getter_tracks_active_profile_after_a_prior_scope(self, tmp_path):
        ...
        tok = set_hbm_home_override(str(a))
        try:
            assert ft._get_real_hbm_home() == os.path.realpath(str(a))
        finally:
            reset_hbm_home_override(tok)
        # The next turn is beta — the getter must now return beta's home, not alpha's.
        tok = set_hbm_home_override(str(b))
        try:
            assert ft._get_real_hbm_home() == os.path.realpath(str(b))
        finally:
            reset_hbm_home_override(tok)
    ...
    def test_config_hard_block_refuses_beta_config_even_after_alpha_turn(self, tmp_path):
        ...
        tok = set_hbm_home_override(str(a))
        try:
            ft._get_hbm_config_resolved()  # warm the (formerly poisoning) alpha lookup
        finally:
            reset_hbm_home_override(tok)
        ...
        try:
            ...
            reset_hbm_home_override(tok)
        assert err is not None
        assert "Refusing to write to HBM AGENT config file" in err
