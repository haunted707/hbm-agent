...
def test_output_path_rejects_hbm_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    hbm_home = tmp_path / "hbm-home"
    hbm_home.mkdir()
    monkeypatch.setattr(file_safety, "_hbm_home_path", lambda: hbm_home)
    monkeypatch.setattr(file_safety, "_hbm_root_path", lambda: hbm_home)

    target = hbm_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
