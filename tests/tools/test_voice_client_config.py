...
Real config files in a temp HBM_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated HBM_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".hbm"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "HBM_LOCAL_STT_LANGUAGE",
    ):
