...
class TestToolsetConsistency:
    ...
    def test_hbm_platforms_share_core_tools(self):
        """All hbm-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on hbm-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["hbm-cli", "hbm-telegram", "hbm-discord", "hbm-whatsapp", "hbm-slack", "hbm-signal", "hbm-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hbm_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hbm-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("hbm-cli")
        second = resolve_toolset("hbm-cli")
        ...
        assert (
            "hbm-cli", True, registry_id, generation, registry.current_scope_key()
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("hbm-cli", include_registry=False)
        second = resolve_toolset("hbm-cli", include_registry=False)
        assert first == second
