...
def _assert_hbm_identity_headers(headers):
    """Both Perplexity endpoints carry the HBM AGENT identity headers (same set as Kimi/OpenCode)
    ...
    assert headers["X-Pplx-Integration"] == "hbm-agent"
...
def test_search_dispatch_maps_search_api_shape():
    ...
    _assert_hbm_identity_headers(post.call_args.kwargs["headers"])
