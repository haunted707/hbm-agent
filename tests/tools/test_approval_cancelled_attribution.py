...
def gateway_session(monkeypatch):
    ...
    for k in ("HBM_CRON_SESSION", "HBM_YOLO_MODE", "HBM_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("HBM_GATEWAY_SESSION", "1")
    monkeypatch.setenv("HBM_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from HBM_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("HBM_CRON_SESSION", "HBM_YOLO_MODE", "HBM_GATEWAY_SESSION", "HBM_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)
