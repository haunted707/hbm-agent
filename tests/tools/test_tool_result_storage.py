...
class TestWriteToSandbox:
    def test_success(self):
        ...
        result = _write_to_sandbox("hello world", "/tmp/hbm-results/abc.txt", env)
    ...
    def test_large_content_via_stdin(self):
        ...
        env.execute.return_value = {"output": "", "returncode": 0}
        big = "x" * 200_000
        _write_to_sandbox(big, "/tmp/hbm-results/big.txt", env)
        cmd = env.execute.call_args_list[0][0][0]
    ...
    def test_size_probe_decides_lossless(self, stdin_mode, probed, ok):
        ...
        assert _write_to_sandbox(content, "/tmp/hbm-results/p.txt", env) is ok
        if not ok:
            rm_cmd = env.execute.call_args_list[2][0][0]
            assert rm_cmd.startswith("rm -f ") and "/tmp/hbm-results/p.txt" in rm_cmd
        else:
    ...
    def test_unprobeable_backend_is_best_effort_success(self):
        ...
        assert _write_to_sandbox("data", "/tmp/hbm-results/np.txt", env) is True
...
class TestResolveStorageDir:
    ...
    def test_uses_env_temp_dir_when_available(self):
        ...
        assert _resolve_storage_dir(env) == "/data/data/com.termux/files/usr/tmp/hbm-results"
...
class TestBuildPersistedMessage:
    def test_structure(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/hbm-results/test123.txt",
        )
        ...
        assert "/tmp/hbm-results/test123.txt" in msg
    ...
    def test_large_size_shows_mb(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/hbm-results/big.txt",
        )
        assert "MB" in msg
...
class TestMaybePersistToolResult:
    ...
    def test_tool_use_id_cannot_escape_storage_dir(self):
        ...
        assert "Full output saved to: /tmp/hbm-results/outside_whoami_x_" in result
        assert "/tmp/hbm-results/../" not in result
        assert target.startswith("/tmp/hbm-results/outside_whoami_x_")
...
# ── Host-side spillover ($HBM_HOME/cache/spillover) ────────────────
...
class TestSpillover:
    ...
    def test_remote_env_probe_failure_falls_back_to_sandbox_write(self):
        ...
        assert PERSISTED_OUTPUT_TAG in result
        assert "/tmp/hbm-results/tc_remote_2.txt" in result
...
class TestRecoveryHint:
    def test_preview_teaches_recovery_not_refetch(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/hbm-results/r.txt",
        )
