...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.hbm/hbm-agent/utils.py)
...
2. ``hbm update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``hbm_cli/main.py`` (the ``hbm update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``hbm_constants`` after the
