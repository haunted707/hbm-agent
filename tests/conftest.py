"""Shared fixtures for the hbm-agent test suite.
...
2. **Isolated HBM_HOME.** HBM_HOME points to a per-test tempdir so
   code reading ``~/.hbm/*`` via ``get_hbm_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hbm"`` instead of the canonical
   ``get_hbm_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No HBM_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox HBM_HOME before ANY test module is imported ──────────────────
# `hbm_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_hbm_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.hbm/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes HBM_HOME, but fixtures run
...
# the REAL HBM AGENT root — capture it BEFORE the sandbox rewires HBM_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.hbm (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("HBM_KANBAN_HOME", "").strip()
_PRE_SANDBOX_HBM_HOME = os.environ.get("HBM_HOME", "")


def _hbm_home_points_at_production(value: str) -> bool:
    """True when a pre-set HBM_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    HBM_HOME=~/.hbm``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``hbm_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.hbm`` — the escape vector that landed pytest
    ...
    HBM_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".hbm").resolve()
    except Exception:
        return True
...
if _hbm_home_points_at_production(os.environ.get("HBM_HOME", "")):
    _SESSION_HBM_HOME = tempfile.mkdtemp(prefix="hbm-test-home-")
    os.environ["HBM_HOME"] = _SESSION_HBM_HOME
    atexit.register(shutil.rmtree, _SESSION_HBM_HOME, True)
...
# like a real CLI") — which used to disarm hbm_state's live-DB guard in
# the child at the same moment the child lost the HBM_HOME redirect.
# HBM_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# hbm_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export HBM_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["HBM_TEST_ISOLATION"] = os.environ.get("HBM_HOME", "") or "1"

#: HBM_HOME as it stood when conftest was imported - i.e. before any test
...
HBM_HOME_AT_CONFTEST_IMPORT = os.environ.get("HBM_HOME", "")
...
# HBM_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_HBM_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``hbm_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM an HBM AGENT session inherits them and
    # hbm_constants home-resolution helpers prefer them over monkeypatched
    ...
    "HBM_QUIET",
    "HBM_TOOL_PROGRESS",
    "HBM_TOOL_PROGRESS_MODE",
    "HBM_MAX_ITERATIONS",
    "HBM_SESSION_PLATFORM",
    "HBM_SESSION_CHAT_ID",
    "HBM_SESSION_CHAT_NAME",
    "HBM_SESSION_CHAT_TYPE",
    "HBM_SESSION_THREAD_ID",
    "HBM_SESSION_SOURCE",
    "HBM_SESSION_KEY",
    "HBM_GATEWAY_SESSION",
    "HBM_CRON_SESSION",
    ...
    "HBM_MODEL",
    "HBM_INFERENCE_MODEL",
    "HBM_INFERENCE_PROVIDER",
    "HBM_TUI_PROVIDER",
    "HBM_MANAGED",
    "HBM_MANAGED_DIR",
    "HBM_DEV",
    "HBM_CONTAINER",
    "HBM_EPHEMERAL_SYSTEM_PROMPT",
    "HBM_TIMEZONE",
    "HBM_REDACT_SECRETS",
    "HBM_BACKGROUND_NOTIFICATIONS",
    "HBM_EXEC_ASK",
    "HBM_HOME_MODE",
    "HBM_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.hbm/kanban.db instead of the per-test HBM_HOME.
    "HBM_KANBAN_DB",
    "HBM_KANBAN_BOARD",
    "HBM_KANBAN_HOME",
    "HBM_KANBAN_WORKSPACES_ROOT",
    "HBM_KANBAN_LOGS_ROOT",
    "HBM_KANBAN_TASK",
    "HBM_KANBAN_WORKSPACE",
    "HBM_KANBAN_RUN_ID",
    "HBM_KANBAN_CLAIM_LOCK",
    "HBM_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "hbm" host.
    "HBM_HONCHO_HOST",
    ...
    "HBM_DASHBOARD_OAUTH_CLIENT_ID",
    "HBM_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and HBM_HOME to per-test tempdirs so code that
    reads ``~/.hbm/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral HBM_* vars that could change test semantics.
    for name in _HBM_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("HBM_HONCHO_HOST", "hbm")

    # 3. Redirect HBM_HOME to a per-test tempdir. Code that reads
    #    ``~/.hbm/*`` via ``get_hbm_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.hbm/*`` via
    #    ``Path.home() / ".hbm"`` instead of ``get_hbm_home()``
    #    is a bug to fix at the callsite.
    fake_hbm_home = tmp_path / "hbm_test"
    fake_hbm_home.mkdir()
    (fake_hbm_home / "sessions").mkdir()
    (fake_hbm_home / "cron").mkdir()
    (fake_hbm_home / "memories").mkdir()
    (fake_hbm_home / "skills").mkdir()
    monkeypatch.setenv("HBM_HOME", str(fake_hbm_home))
    ...
    # hbm_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("HBM_TEST_ISOLATION", str(fake_hbm_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("HBM_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. hbm_state computes ``DEFAULT_DB_PATH = get_hbm_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from hbm_state import ...``) that
    ...
    hbm_state_mod = sys.modules.get("hbm_state")
    if hbm_state_mod is not None and hasattr(hbm_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            hbm_state_mod, "DEFAULT_DB_PATH", fake_hbm_home / "state.db"
        )
    ...
    monkeypatch.setenv("HBM_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.hbm/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import hbm_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path HBM_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_hbm_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from hbm_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _neutralize_git_safe_directory_read(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_safe_directory"):
        return
    try:
        from hbm_cli import _subprocess_compat
    except Exception:
        return
...
def _close_leaked_session_dbs():
    ...
    Root cause of OOM incident 20260816: ~40 files under tests/hbm_cli/
    ...
    ``pytest tests/hbm_cli/`` accumulated 16-25 GB RSS and had to be
    OOM-killed three times in one day.

    Rather than editing every test file, ``SessionDB.__init__`` registers each
    instance in ``hbm_state_guard._test_instance_registry`` (a WeakSet,
    populated only when the ``HBM_TEST_ISOLATION`` marker is set — i.e.
    ...
    Instances opened through ``hbm_state_registry.acquire()`` are skipped:
    ...
    yield
    try:
        from hbm_state_guard import _test_instance_registry as registry
    except Exception:
        return
    if not registry:
        return
...
# invocation), kanban writes silently pollute the real ~/.hbm. This autouse
...
# allow-list because test-level fixtures legitimately move HBM_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session HBM_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``HBM_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) HBM AGENT root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_HBM_HOME and not _hbm_home_points_at_production(
        _PRE_SANDBOX_HBM_HOME
    ):
        # HBM_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_hbm_root().resolve()
    # No pre-existing HBM_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".hbm").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.hbm`` captured at import time. Hermetic tests that legitimately
    move HBM_HOME to sibling tempdirs are unaffected.

    Only patches when ``hbm_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("hbm_cli.kanban_db")
    _kdbc = sys.modules.get("hbm_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import hbm_cli.kanban_db`
...
# ``hbm_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL HBM AGENT root. This fixture wires the
# test-side knobs:
...
#     installs where HBM_HOME is not ~/.hbm) into the guard's
...
# so subprocess children that import hbm_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("hbm_state")
    ...
    if _PRE_SANDBOX_HBM_HOME and not _hbm_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_HBM_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local HBM AGENT home override redirects every later
    # ``get_hbm_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from hbm_constants import get_hbm_home_override, set_hbm_home_override

        if get_hbm_home_override() is not None:
            set_hbm_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal hbm config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``hbm-gateway`` process
...
# correlated with a ``tests/hbm_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> hbm-gateway``
...
def _wal_is_usable() -> bool:
    """True when HBM AGENT will actually put a database into WAL mode here.

    HBM AGENT refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running HBM AGENT can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside an HBM AGENT managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``hbm_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_hbm_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_hbm_home`` fixture
    redirects ``HBM_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.hbm/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``hbm_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["HBM_VOICE_TTS"] = "1"``. The
...
#      ``hbm_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``hbm_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``hbm_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``hbm_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# HBM AGENT runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.hbm``.
#
...
def _relocate_basetemp_outside_operator_home(config) -> None:
    """Move pytest's basetemp out of the operator's platform-native HBM AGENT home.

    Every per-test sandbox is ``<basetemp>/.../hbm_test``. ``get_default_hbm_root()``
    prefers the platform-native home whenever ``HBM_HOME`` sits *under* it, so a basetemp
    inside ``~/.hbm`` (or ``%LOCALAPPDATA%\\hbm``, where ``TEMP`` commonly lives on
    ...
    """
    from hbm_constants import _get_platform_default_hbm_home

    native = _get_platform_default_hbm_home().resolve()
    ...
    # HBM AGENT home). The repo is no escape either: the default install checks it
    # out *inside* the home (~/.hbm/hbm-agent). A sibling of the native
    ...
    safe = Path(tempfile.mkdtemp(prefix="hbm-pytest-basetemp-", dir=safe_root))
    assert not safe.resolve().is_relative_to(native), (
        f"pytest basetemp {safe} still resolves inside the operator's HBM AGENT home {native}; "
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where HBM AGENT falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — HBM AGENT uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart hbm-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting hbm/python patterns are also blocked.
    """
    ...
    def _matches_hbm_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _HBM_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_hbm_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch hbm-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m hbm_cli.main".
                if (
                    "hbm" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live hbm-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting hbm/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `hbm update` (or the
        # equivalent `python -m hbm_cli.main update`).  These commands
        ...
        if "update" in low and (
            # hbm update / hbm update --gateway / setsid bash -c ... hbm update
            ("hbm" in low and "update" in low.split())
            or
            # python -m hbm_cli.main update --gateway
            ("hbm_cli" in low and "update" in low.split())
            or
            # venv/bin/hbm update  (absolute path variant used in tests)
            (".venv/bin/hbm" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`hbm update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m hbm_cli.main
        # gateway run|start|restart``). ``_spawn_hbm_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp HBM_HOME, resolves the DEVELOPER's
        # ``hbm-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … hbm gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "hbm gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (hbm_cli.web_server_gateway."
                "_spawn_hbm_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_HBM_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import hbm_cli.voice as _voice
    except Exception:
