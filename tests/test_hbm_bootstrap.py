"""Tests for hbm_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every HBM AGENT entry point
(hbm, hbm-agent, hbm-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every HBM AGENT entry point imports hbm_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported hbm_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("hbm_bootstrap", None)
    import hbm_bootstrap  # noqa: WPS433
    return hbm_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the HBM AGENT CLI
...
class TestEntryPointsImportBootstrap:
    """Every HBM AGENT entry point must import hbm_bootstrap as its
    ...
    # Entry points that invoke HBM AGENT as a process.  Each one must
    # import hbm_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "hbm_cli/main.py",   # hbm CLI (console_script)
        "run_agent.py",          # hbm-agent (console_script)
        "acp_adapter/entry.py",  # hbm-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import hbm_bootstrap' and that
        ...
        half-finished ``hbm update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``hbm_bootstrap`` as a top-level module) leaves hbm
        ...
        """
        # Resolve relative to the hbm-agent repo root.  Tests live
        # at tests/test_hbm_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hbm start even when hbm_bootstrap hasn't been
        ...
        assert first_import_name == "hbm_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'hbm_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import hbm_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing HBM AGENT's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("HBM_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["HBM_PYTHON_SRC_ROOT"] = env
            elif "HBM_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["HBM_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hbm")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HBM_PYTHON_SRC_ROOT", None)
            else:
                os.environ["HBM_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/hbm", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # HBM AGENT root.  The guard must relocate HBM AGENT to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/hbm"])
        assert result[0] == "/opt/hbm"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the HBM AGENT root.
        assert result.index("/opt/hbm") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["HBM_PYTHON_SRC_ROOT"] = "/env/hbm"
            hb.harden_import_path()
            assert sys.path[0] == "/env/hbm"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HBM_PYTHON_SRC_ROOT", None)
