...
Override the image with ``HBM_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("HBM_TEST_IMAGE", "hbm-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``HBM_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("HBM_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged hbm user
# ---------------------------------------------------------------------------
#
# Background: every HBM AGENT runtime path inside the container drops to UID
# 10000 (the ``hbm`` user) via ``s6-setuidgid hbm``. ``docker exec``
...
# fails (PermissionError swallowed) for hbm, so a test that ran as root
...
# helpers below run every probe under ``-u hbm`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: hbm).
