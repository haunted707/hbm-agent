"""Runtime smoke test for Docker $HBM_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the hbm user after container boot.
...
registered later runs its log service as the dropped hbm user and
...
def test_logs_gateways_seeded_and_hbm_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by hbm after boot."""
    ...
    assert "logs=hbm" in r.stdout, (
        f"logs/ not owned by hbm: {r.stdout}"
    )
    assert "gateways=hbm" in r.stdout, (
        f"logs/gateways/ not owned by hbm: {r.stdout}"
    )
