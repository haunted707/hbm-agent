"""Harness: dashboard opt-in via HBM_DASHBOARD.

Today (tini): dashboard starts once when HBM_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``hbm`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without HBM_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "hbm dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without HBM_DASHBOARD"
    )
...
# whenever `HBM_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``HBM_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "HBM_DASHBOARD=1",
        "HBM_DASHBOARD_HOST=0.0.0.0",
        "HBM_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"HBM_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and HBM_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``HBM_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
