...
Build the real image and verify at runtime:
  ...
  1. /opt/hbm is not writable by the hbm user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and HBM_DISABLE_LAZY_INSTALLS are set
  3. /opt/hbm/.install_method contains "docker" (code-scoped stamp)
  4. $HBM_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $HBM_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_hbm(
    built_image: str, container_name: str,
) -> None:
    """The hbm user must not be able to modify /opt/hbm.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/hbm as the hbm user
        "touch /opt/hbm/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hbm user can write to /opt/hbm (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/hbm/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hbm user can write to /opt/hbm/.venv: {r.stdout}"
    )


def test_hbm_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    HBM_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $HBM_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $HBM_HOME was not healed on boot: "
        f"{r.stdout}"
    )
