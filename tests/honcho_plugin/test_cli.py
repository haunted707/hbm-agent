...
class TestResolveApiKey:
    ...
    def test_returns_api_key_from_root(self, monkeypatch):
        ...
        monkeypatch.setattr(honcho_cli, "_host_key", lambda: "hbm")
...
class TestCmdSetupLocalJwt:
    ...
    def test_local_setup_stores_jwt_under_host_block(self, monkeypatch, tmp_path):
        ...
        host_block = (cfg.get("hosts") or {}).get("hbm") or {}
...
class TestCmdStatus:
    ...
    def test_auth_line_detects_oauth_grant(self, monkeypatch, capsys, tmp_path):
        ...
        out = capsys.readouterr().out
        assert "Auth:           OAuth (hbm-agent" in out
...
class TestCloneHonchoForProfile:
    ...
    def test_user_peer_aliases_carry_into_cloned_profile(self, monkeypatch, tmp_path):
        ...
        assert ok is True
        new_block = written["cfg"]["hosts"]["hbm_coder"]
...
class TestSetupWizardDeploymentShape:
    ...
    calls, asserting the resulting hbm_host block so the tree's routing
    ...
    def _run_setup(self, monkeypatch, tmp_path, *, answers, initial_cfg=None,
        ...
        monkeypatch.setattr(
            "hbm_cli.config.load_config", lambda: {"memory": {}}, raising=False,
        )
        monkeypatch.setattr(
            "hbm_cli.config.save_config", lambda c: None, raising=False,
        )
    ...
    def test_unpin_steers_to_pooled_by_default(self, monkeypatch, tmp_path):
        ...
        initial_cfg = {
            "apiKey": "***",
            "hosts": {"hbm": {"pinPeerName": True, "peerName": "eri"}},
        }
        answers = [
            "cloud",           # deployment
            "",                # api key (keep)
    ...
    def test_mapping_step_points_at_peers_map(self, monkeypatch, tmp_path, capsys):
        self._run_setup(monkeypatch, tmp_path, answers=["cloud", "", "eri", "hermetika", "hbm", "s"])
        assert "hbm honcho peers map" in capsys.readouterr().out
    ...
    def test_host_pin_user_peer_true_is_detected_as_single(self, monkeypatch, tmp_path):
        ...
        initial_cfg = {
            "apiKey": "***",
            "hosts": {"hbm": {"pinUserPeer": True, "peerName": "eri"}},
        }
        ...
        answers = ["cloud", "", "eri", "hermetika", "hbm"]
        host = self._run_setup(monkeypatch, tmp_path, answers=answers, initial_cfg=initial_cfg)
    ...
    @pytest.mark.parametrize("initial_cfg, expected_pin", [
        (None, True),
        ({"apiKey": "***", "hosts": {"hbm": {}}}, True),
        ({"apiKey": "***", "hosts": {"hbm": {"pinUserPeer": False, "peerName": "eri"}}}, False),
        ({"apiKey": "***", "hosts": {"hbm": {"enabled": True, "workspace": "hbm", "peerName": "eri"}}}, False),
        ({"apiKey": "***", "enabled": True, "workspace": "hbm", "peerName": "eri"}, False),
    ...
    def test_no_gateway_connected_skips_mapping_when_declined(self, monkeypatch, tmp_path):
        ...
        answers = ["cloud", "", "eri", "hermetika", "hbm", "n"]
...
class TestCloneCarriesPinUserPeer:
    ...
    def test_clone_inherits_host_pin_user_peer(self, monkeypatch, tmp_path):
        ...
        assert ok is True
        new_block = written["cfg"]["hosts"]["hbm_partner"]
        assert new_block["pinUserPeer"] is True
...
class TestCmdSetupDeviceFlow:
    ...
    def _run_setup(self, monkeypatch, tmp_path, *, answers, device_available=True,
        ...
        cred = OAuthCredential(
            ...
            client_id="hbm-agent", token_endpoint="http://x/oauth/token",
    ...
    def test_headless_defaults_to_device(self, monkeypatch, tmp_path):
        ...
        assert cfg["hosts"]["hbm"]["apiKey"] == "hch-at-x"
    ...
    def test_denied_device_flow_aborts_without_grant(self, monkeypatch, tmp_path):
        ...
        assert len(calls) == 1
        assert "apiKey" not in cfg.get("hosts", {}).get("hbm", {})
...
class TestWriteRefusesUnparseableStore:
    ...
    def test_command_prints_one_sentence_asks_nothing_and_writes_nothing(self, monkeypatch, tmp_path, capsys, run):
        ...
        honcho_cli = _point_cli_at(monkeypatch, cfg_path, _host_key=lambda: "hbm_coder",
    ...
    def test_apikey_answer(self, monkeypatch, tmp_path, capsys, grant, root_key, answer, ok, host_key, root_after, shown):
        ...
        if grant:
            host = {"apiKey": "hch-at-dead", "oauth": {"refreshToken": "hch-rt-dead", "expiresAt": 1,
                                                       "clientId": "hbm-agent", "tokenEndpoint": "https://api.honcho.dev/oauth/token"}}
        cfg = {"hosts": {"hbm": host}, **({"apiKey": root_key} if root_key else {})}
...
_OAUTH_DEFAULT = {"peerName": "eri", "hosts": {"hbm": {
    "enabled": True, "apiKey": "hch-at-live", "workspace": "hbm", "peerName": "eri", "oauth": {"refreshToken": "hch-rt-live"},
}}}
_KEYLESS_DEFAULT = {"hosts": {"hbm": {"workspace": "hbm"}}}
...
class TestEnabledRequiresACredential:
    ...
    def _env(self, monkeypatch, tmp_path, cfg, *, env_key=None, host="hbm_dreamer", profile="dreamer"):
        import copy
    ...
    @pytest.mark.parametrize("cfg, env_key, enabled", [
        ...
        ({"hosts": {"hbm": {"enabled": True, "apiKey": "hch-v3-hostonly", "workspace": "hbm"}}}, None, False),
    ...
    def test_clone_is_enabled_only_by_an_on_disk_credential(self, monkeypatch, tmp_path, cfg, env_key, enabled):
        ...
        block = written["cfg"]["hosts"]["hbm_dreamer"]
        assert block.get("enabled") is (True if enabled else None)
        assert "apiKey" not in block and "oauth" not in block
        assert block["aiPeer"] == "dreamer" and block["workspace"] == "hbm"

    @pytest.mark.parametrize("cfg, env_key, profile, expect, enabled", [
        ({"hosts": {"hbm_dreamer": {"workspace": "hbm"}}}, "hch-v3-from-env", "dreamer", ["setup"], False),
        (_OAUTH_DEFAULT, None, "dreamer",
         ["stays disabled", "hbm honcho setup --target-profile dreamer", "hosts.hbm_dreamer"], False),
        ({"hosts": {"hbm_dreamer": {"enabled": True, "aiPeer": "dreamer", "workspace": "hbm"}}}, None, "dreamer",
         ["stays disabled"], False),
        ({"hosts": {}}, None, "default", ["Run 'hbm honcho setup' to sign in"], False),
    ...
    def test_enable_writes_enabled_only_for_an_on_disk_credential(self, monkeypatch, tmp_path, capsys,
        ...
        host = "hbm" if profile == "default" else "hbm_dreamer"
...
class TestWriteConfigMergesOntoDisk:
    ...
    def _rotate_on_disk(self, cfg_path):
        ...
        disk["hosts"]["hbm"].update(apiKey="hch-at-new", oauth={"refreshToken": "hch-rt-new"})
        cfg_path.write_text(json.dumps(disk))

    def test_untouched_keys_take_disk_and_the_commands_edits_apply(self, monkeypatch, tmp_path):
        disk = {"apiKey": "root", "hosts": {"hbm": {"apiKey": "hch-at-old", "oauth": {"refreshToken": "hch-rt-old"},
        ...
        cfg["hosts"]["hbm"]["recallMode"] = "tools"
        cfg["hosts"]["hbm"].pop("runtimePeerPrefix")
        cfg["dialecticCadence"] = 3
        honcho_cli._write_config(cfg)
        out = json.loads(cfg_path.read_text())
        assert out["hosts"]["hbm"] == {"apiKey": "hch-at-new", "oauth": {"refreshToken": "hch-rt-new"}, "recallMode": "tools"}
        assert out["apiKey"] == "root" and out["dialecticCadence"] == 3

    def test_a_credential_the_command_set_wins(self, monkeypatch, tmp_path):
        disk = {"hosts": {"hbm": {"apiKey": "hch-at-old", "oauth": {"refreshToken": "hch-rt-old"}}}}
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, disk)
        cfg = honcho_cli._read_config()
        self._rotate_on_disk(cfg_path)
        cfg["hosts"]["hbm"]["apiKey"] = "hch-v3-pasted"
        cfg["hosts"]["hbm"].pop("oauth")
        honcho_cli._write_config(cfg)
        assert json.loads(cfg_path.read_text())["hosts"]["hbm"] == {"apiKey": "hch-v3-pasted"}

    def test_a_second_write_on_the_same_read_applies_only_the_edits_made_since_the_first(self, monkeypatch, tmp_path):
        disk = {"hosts": {"hbm": {"apiKey": "hch-at-old", "oauth": {"refreshToken": "hch-rt-old"}, "workspace": "A"}}}
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, disk)
        cfg = honcho_cli._read_config()
        cfg["hosts"]["hbm"]["workspace"] = "B"
        honcho_cli._write_config(cfg)
        self._rotate_on_disk(cfg_path)
        cfg["hosts"]["hbm"]["workspace"] = "A"
        honcho_cli._write_config(cfg)
        out = json.loads(cfg_path.read_text())["hosts"]["hbm"]
    ...
    def test_a_grant_the_login_installed_yields_to_a_later_rotation(self, monkeypatch, tmp_path):
        ...
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, {"hosts": {"hbm": {"peerName": "alice"}}})
        monkeypatch.setattr(honcho_cli, "_host_key", lambda: "hbm")
        cfg = honcho_cli._read_config()
        grant = {"access_token": "hch-at-login", "refresh_token": "hch-rt-login", "expires_in": 3600}
        cred = oauth.install_grant(cfg_path, "hbm", grant, client_id="c", token_endpoint="e", apply_config=False)
        honcho_cli._apply_grant_to_host(cfg, cfg["hosts"]["hbm"], cred)
        self._rotate_on_disk(cfg_path)
        cfg["hosts"]["hbm"]["recallMode"] = "tools"
        honcho_cli._write_config(cfg)
        out = json.loads(cfg_path.read_text())["hosts"]["hbm"]
    ...
    _SEED = {"dialecticCadence": 3, "hosts": {"hbm": {"peerName": "alice"}}}
    ...
    def _seeded(self, monkeypatch, tmp_path):
        ...
        return _point_cli_at(monkeypatch, local, _config_path=lambda: seed, _host_key=lambda: "hbm"), local
    ...
    def test_a_read_seeded_from_another_file_applies_only_its_edits_onto_the_local_file(self, monkeypatch, tmp_path):
        ...
        cfg = honcho_cli._read_config()
        grant = {"access_token": "hch-at-login", "refresh_token": "hch-rt-login", "expires_in": 3600}
        cred = oauth.install_grant(local, "hbm", grant, client_id="c", token_endpoint="e", apply_config=False)
        honcho_cli._apply_grant_to_host(cfg, cfg["hosts"]["hbm"], cred)
        ...
        oauth._persist_credential(local, "hbm", rotated)
        cfg["hosts"]["hbm"]["recallMode"] = "tools"
        honcho_cli._write_config(cfg)
        out = json.loads(local.read_text())
        assert out["hosts"]["hbm"]["apiKey"] == "hch-at-new"
        assert out["hosts"]["hbm"]["oauth"]["refreshToken"] == "hch-rt-new"
        assert out["hosts"]["hbm"]["recallMode"] == "tools" and out["hosts"]["hbm"]["peerName"] == "alice"
    ...
    def test_a_read_seeded_from_another_file_is_written_whole_while_no_local_file_exists(self, monkeypatch, tmp_path):
        ...
        cfg["hosts"]["hbm"]["recallMode"] = "tools"
        honcho_cli._write_config(cfg)
        assert json.loads(local.read_text()) == {"dialecticCadence": 3, "hosts": {"hbm": {"peerName": "alice", "recallMode": "tools"}}}
    ...
    def test_a_plain_dict_is_written_whole(self, monkeypatch, tmp_path, build):
        honcho_cli, cfg_path = self._paths(monkeypatch, tmp_path, {"hosts": {"hbm": {"apiKey": "hch-at-old"}}})
