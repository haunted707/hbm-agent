...
from hbm_cli.profiles import _get_default_hbm_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "hbm"
        assert config.workspace_id == "hbm"
...
class TestFromGlobalConfig:
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"hbm": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="hbm-agent"
        ):
            result = config.resolve_session_name("/home/user/hbm-agent/subdir")
        assert result == "hbm-agent"


class TestResolveConfigPath:
    def test_prefers_hbm_home_when_exists(self, tmp_path):
        hbm_home = tmp_path / "hbm"
        hbm_home.mkdir()
        local_cfg = hbm_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: HBM_HOME points at ~/.hbm/profiles/<name>, so
        # _get_default_hbm_home() must resolve back to ~/.hbm — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".hbm"
        ...
        assert _get_default_hbm_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "hbm_coder"
        assert profile_host_key("default") == "hbm"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HBM_HONCHO_HOST": "hbm.coder"}):
            assert resolve_active_host() == "hbm.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("HBM_HONCHO_HOST", None)
            # Temporarily remove hbm_cli.profiles to simulate import failure
            saved = sys.modules.get("hbm_cli.profiles")
            sys.modules["hbm_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "hbm"
            finally:
                if saved is not None:
                    sys.modules["hbm_cli.profiles"] = saved
                else:
                    sys.modules.pop("hbm_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="hbm_coder")
        assert config.host == "hbm_coder"
        assert config.workspace_id == "hbm"  # shared workspace
        assert config.ai_peer == "hbm_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"hbm": {"enabled": True, "aiPeer": "hbm"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (hbm.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="hbm_work",
            workspace_id="hbm",
            raw={"hosts": {"hbm.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_hbm_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("hbm_cli.profiles.get_active_profile_name", return_value="default"), \
