...
Under a gateway (Telegram, Discord, Slack, ...) HBM AGENT passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to HBM AGENT via Telegram AND Discord
...
class TestPinTransition:
    ...
    def test_identity_signature_reflects_both_session_prefixes(self, tmp_path, monkeypatch):
        ...
        cfg_path = tmp_path / "honcho.json"
        monkeypatch.setenv("HBM_HOME", str(tmp_path))
        base = {"apiKey": "k", "peerName": "Igor", "aiPeer": "hbm"}
    ...
    def test_identity_signature_reflects_a_repointed_host_workspace(self, tmp_path, monkeypatch):
        """``hbm honcho peers map`` can repoint a host block's workspace; the cached agent's manager
        ...
        cfg_path.write_text(json.dumps({**base, "hosts": {"hbm": {"workspace": "old"}}}))
        sig_old = provider.identity_signature()["workspace"]
        cfg_path.write_text(json.dumps({**base, "hosts": {"hbm": {"workspace": "new"}}}))
...
class TestProfilePeerUniqueness:
    """Each HBM AGENT profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `hbm -p partner` pinned to a different person's peer).
