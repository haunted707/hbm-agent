...
def test_root_session_no_compression(db):
    ...
    assert prov["currentHbmSessionId"] == "root1"
    assert prov["rootHbmSessionId"] == "root1"
    assert prov["parentHbmSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_hbm_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentHbmSessionId"] == "old"
    assert prov["rootHbmSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousHbmSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootHbmSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"hbm"}
    assert "sessionProvenance" in meta["hbm"]
    assert meta["hbm"]["sessionProvenance"]["currentHbmSessionId"] == "root1"
