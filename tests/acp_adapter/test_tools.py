...
COMMON_HBM_TOOLS = ["read_file", "search_files", "terminal", "patch", "write_file", "process"]


class TestToolKindMap:
    def test_all_hbm_tools_have_kind(self):
        """Every common hbm tool should appear in TOOL_KIND_MAP."""
        for tool in COMMON_HBM_TOOLS:
...
class TestBuildToolTitle:
    ...
    def test_execute_code_title_includes_first_code_line(self):
        ...
        assert "from hbm_tools import terminal" in title
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("web_search", {"query": "hbm agent acp"}),
            ("execute_code", {"code": "\nfrom hbm_tools import terminal\nprint('done')"}),
