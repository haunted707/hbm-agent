"""Tests for pip entry-point provider discovery (hbm_agent.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``hbm_agent.plugins`` entry point, supporting both a
