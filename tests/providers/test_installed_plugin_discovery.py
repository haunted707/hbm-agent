"""A provider installed by ``hbm plugins install`` must actually be found.

The installer clones into ``$HBM_HOME/plugins/<name>/`` (flat), provider
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules):
        if mod.startswith(("plugins.model_providers", "_hbm_user_provider")):
            del sys.modules[mod]
...
def test_flat_installed_model_provider_plugins_are_discovered_alongside_nested_ones(hbm_home):
    _write_plugin(hbm_home / "plugins" / "installed-acp", name="installed-acp",
                  manifest='name: installed-acp\nkind: "model-provider"\n')
    _write_plugin(hbm_home / "plugins" / "model-providers" / "nested-acp", name="nested-acp",
...
def test_other_plugins_in_the_flat_directory_are_left_to_the_plugin_manager(hbm_home):
    _write_plugin(hbm_home / "plugins" / "other-standalone", name="other-standalone",
                  manifest="name: other-standalone\nkind: standalone\n")
    _write_plugin(hbm_home / "plugins" / "manifestless", name="manifestless", manifest=None)
    _write_plugin(hbm_home / "plugins" / "broken-manifest", name="broken-manifest",
