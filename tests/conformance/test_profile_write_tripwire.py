...
--------------------
When a non-default profile is active (``HBM_HOME`` points at
...
class ProfileTripwire:
    ...
    Attributes:
        root:     the default profile's home (``<tmp>/hbm``).
...
def profile_tripwire(tmp_path, monkeypatch):
    ...
    1. A temp hbm root acting as the *default* profile, seeded with the
       ...
       detectable.
    2. ``HBM_HOME`` (env var AND the context-local override) pointed at
       the testprof home — the exact activation shape ``--profile`` uses.
    3. ``hbm_state.DEFAULT_DB_PATH`` restored to its import-time snapshot.
       ...
       makes resolution flow through ``get_hbm_home()`` — the production
