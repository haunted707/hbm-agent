"""Secret stores under HBM_HOME are write-denied; control files stay writable (#110464).
...
@pytest.fixture()
def hbm_layout(tmp_path, monkeypatch):
    """Profile HBM_HOME plus a distinct global root, both patched."""
    root = tmp_path / "hbm_root"
...
def test_read_denied_secret_stores_are_write_denied_on_profile_and_root(hbm_layout):
    root, profile = hbm_layout
...
def test_control_files_and_lookalikes_outside_home_stay_writable(hbm_layout, tmp_path):
    root, profile = hbm_layout
    for base in (profile, root):
