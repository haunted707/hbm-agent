...
an autonomous agent reached through a client shim that cannot carry HBM AGENT tool
...
So: a client declaring ``SUPPORTS_HBM_TOOL_CALLS = False`` skips the fork with
...
def test_fork_is_skipped_when_the_provider_cannot_emit_tool_calls(caplog):
    client = MagicMock()
    client.SUPPORTS_HBM_TOOL_CALLS = False
...
def test_fork_is_spawned_when_the_provider_can_emit_tool_calls():
    client = MagicMock()
    client.SUPPORTS_HBM_TOOL_CALLS = True
