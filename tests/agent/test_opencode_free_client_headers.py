...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_hbm_attribution(mock_openai):
    """Keyless requests still identify as HBM AGENT (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "HBM AGENT"
    assert str(headers.get("User-Agent", "")).startswith("HbmAgent/")
