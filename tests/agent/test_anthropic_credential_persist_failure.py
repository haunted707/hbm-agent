...
``~/.hbm/.anthropic_oauth.json`` for ``hbm_pkce`` ones.  Those singletons
...
def hbm_home(tmp_path, monkeypatch):
    """Real on-disk HBM_HOME so ``load_pool()`` re-reads what we persisted."""
    home = tmp_path / "hbm"
...
def test_hbm_oauth_writer_raises_instead_of_swallowing(hbm_home, monkeypatch):
    ...
    with pytest.raises(CredentialPersistError):
        AA._write_hbm_oauth_credentials(
            _ROTATED_ACCESS, _ROTATED_REFRESH, _EXPIRED_MS + 3_600_000
        )
...
def test_pool_hbm_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "hbm_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
