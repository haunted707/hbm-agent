...
def test_free_tier_block_gets_its_own_code_and_carries_the_sentence():
    ...
    result = _failed_result("auth_permanent", error="HTTP 403: no permissions",
                            free_tier={"kind": "disabled", "message": "Using HBM AGENT without signing in is switched off."})
    ...
    assert surface["message"] == "Using HBM AGENT without signing in is switched off."
