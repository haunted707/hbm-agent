...
def backup_env(monkeypatch, tmp_path):
    """Isolate HBM_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".hbm"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_hbm_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd hbm_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under HBM_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level HBM_DIR picks up the tmp HOME."""
    import hbm_constants
    importlib.reload(hbm_constants)
