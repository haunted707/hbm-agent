...
Reports live under ``~/.hbm/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated HBM_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".hbm"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "HBM_DIR", home)
