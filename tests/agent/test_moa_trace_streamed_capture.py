...
path via ``hbm chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp HBM_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point HBM_HOME at a temp dir and turn moa.save_traces on."""
    hbm_home = tmp_path / ".hbm"
    hbm_home.mkdir()
    monkeypatch.setenv("HBM_HOME", str(hbm_home))

    # save_moa_turn reads config via hbm_cli.config.load_config; stub it to
