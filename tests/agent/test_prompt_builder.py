...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("hbm_cli.config.load_config", default_load_config)
        monkeypatch.setattr("hbm_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("hbm_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("hbm_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("hbm_cli.config.load_config_readonly", lambda: {})
...
class TestBuildContextFilesPrompt:
    ...
    def test_hbm_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".hbm.md").write_text("HBM AGENT-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "HBM AGENT-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HBM_HOME", str(tmp_path / "hbm_home"))
        hbm_home = tmp_path / "hbm_home"
        hbm_home.mkdir()
        (hbm_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
...
# =========================================================================
# .hbm.md helper functions
# =========================================================================


class TestFindHbmMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".hbm.md").write_text("rules")
        assert _find_hbm_md(tmp_path) == tmp_path / ".hbm.md"
    ...
    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".hbm.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_hbm_md(sub) == tmp_path / ".hbm.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .hbm.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_probe_remote_backend_ssh_is_probe_only_and_torn_down(self, monkeypatch):
        ...
        ~/.hbm and snapshot a session just to run `uname`, and its __del__ would
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """HBM_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("HBM_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._BACKEND_PROBE_CACHE.clear()
        result = _pb.build_environment_hints()
...
class TestContextFileReadTimeout:
    def test_slow_hbm_md_is_skipped_and_agents_md_still_loads(self, tmp_path, monkeypatch, caplog):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".hbm.md").write_text("HBM AGENT project rules.")
        ...
        assert "HBM AGENT project rules" not in result
