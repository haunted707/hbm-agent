...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"hbm-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "hbm-agent-dev", "declared": True},
        },
    )
...
# `hbm curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `hbm curator pin <umbrella>` in the rename block so
