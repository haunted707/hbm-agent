...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated HBM_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="hbm_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".hbm"))
    prev_home = os.environ.get("HBM_HOME")
    os.environ["HBM_HOME"] = os.path.join(test_home, ".hbm")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("hbm_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into hbm_cli.*/tools.* and their monkeypatches would otherwise
