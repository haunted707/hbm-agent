...
class TestParseConfigStringList:
    """#86661: `hbm config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `hbm config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    hbm_home = tmp_path / "da\\ta"
    subprocess_home = hbm_home / "home"
    subprocess_home.mkdir(parents=True)
    (hbm_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("HBM_HOME", str(hbm_home))
    monkeypatch.setenv("HOME", str(hbm_home))
