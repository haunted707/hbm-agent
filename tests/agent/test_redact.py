...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure HBM_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.hbm/.env")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.hbm/config.yaml", "api_key: hbmConfigSecret123", "hbmConfigSecret123"),
            (
                "head ~/.hbm/profiles/work/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.hbm/config.example.yaml",
            "cat ~/.hbm/config.template.yaml",
            "cat ~/.bashrc.example",
...
class TestHbmHomePathClassification:
    """``_is_secret_file_arg`` must see the RESOLVED HBM AGENT home: a managed Windows home
    (``%LOCALAPPDATA%\\hbm``) has no ``.hbm`` segment and a resolved path never spells
    ``$HBM_HOME``, so the literal test alone classified its ``config.yaml`` as ordinary YAML."""
    ...
    def test_resolved_home_config_is_secret_bearing_but_project_config_is_not(self, tmp_path, monkeypatch):
        ...
        home = tmp_path / "hbm"  # no ".hbm" segment
