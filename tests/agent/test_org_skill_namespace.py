...
class TestOrgPullIsWiredIn:
    ...
    def test_no_internal_jargon_in_user_facing_strings(self):
        ...
        targets = [
            root / "hbm_cli" / "subcommands" / "sync.py",
            root / "hbm_cli" / "subcommands" / "skills.py",
        ]
        banned = re.compile(
...
class TestSkillSyncIsOneCommand:
    ...
    belong elsewhere. `propose` in particular used to sit under `hbm
    ...
    def test_propose_is_not_under_skills(self):
        skills_src = self._src("hbm_cli", "subcommands", "skills.py")
        assert '"propose"' not in skills_src, (
            "`propose` must NOT remain under `hbm skills` — Skill Sync is "
...
class TestLocalEditsSurviveOrgUpdates:
    ...
    def test_auto_propose_defaults_off(self, monkeypatch):
        ...
        monkeypatch.delenv("HBM_SYNC_ORG_AUTO_PROPOSE", raising=False)
        monkeypatch.setattr(
    ...
    def test_auto_propose_can_be_enabled_by_env(self, monkeypatch):
        ...
        monkeypatch.setenv("HBM_SYNC_ORG_AUTO_PROPOSE", "1")
