...
def isolated_home(monkeypatch):
    """Temp HBM_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="hbm_test_31179_")
    hbm_home = os.path.join(test_home, ".hbm")
...
def _fresh_modules():
    """Drop cached hbm modules so each test reloads against current env."""
    _drop_reload_targets()
