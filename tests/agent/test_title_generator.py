...
class TestGenerateTitle:
    ...
    def test_title_language_reads_config(self):
        ...
        with patch("hbm_cli.config.load_config", return_value=cfg), patch("hbm_cli.config.load_config_readonly", return_value=cfg):
            assert _title_language() == "French"
        with patch("hbm_cli.config.load_config", return_value={}), patch("hbm_cli.config.load_config_readonly", return_value={}):
            assert _title_language() == ""
        with patch("hbm_cli.config.load_config", side_effect=RuntimeError("bad config")), \
         patch("hbm_cli.config.load_config_readonly", side_effect=RuntimeError("bad config")):
            assert _title_language() == ""
...
class TestMaybeAutoTitle:
    ...
    def test_kanban_worker_with_unreadable_card_falls_back_to_the_task_id(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HBM_KANBAN_TASK", "t_missing")
        db = SessionDB(tmp_path / "state.db")
        db.create_session(session_id="sess-1", source="kanban")
