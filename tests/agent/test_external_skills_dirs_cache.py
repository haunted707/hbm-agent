...
@pytest.fixture
def hbm_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.hbm/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".hbm"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(hbm_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = hbm_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different HBM_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".hbm"
