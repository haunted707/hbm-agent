...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("hbm_cli.config.load_config", return_value=config or {}),
...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "hbm doctor" not in lower
...
def test_explanation_persistence_corrupt_cause_never_says_free_space():
    ...
    assert "hbm doctor" in lower
    assert "free some space" not in lower
    assert "full disk" not in lower


def test_explanation_persistence_corrupt_backups_dir_follows_hbm_home(monkeypatch, tmp_path):
    """Step 3 must name the backups dir under the ACTIVE home, not ~/.hbm (#104250).

    Pre-update backups live at ``<hbm_root>/backups`` (``hbm_cli/backup.py``), so a
    custom-HBM_HOME deployment told to restore from ``~/.hbm/backups/`` is misdirected
    mid data-loss incident: that directory may not exist at all, or may hold an unrelated
    install's backups.
    """
    custom_home = tmp_path / "custom-hbm-home"
    monkeypatch.setenv("HBM_HOME", str(custom_home / "profiles" / "research"))
    out = AIAgent._format_turn_completion_explanation(
        "session_persistence_failed", "corrupt"
    )
    assert f"{custom_home / 'backups'}" in out
    assert "~/.hbm/backups" not in out
...
def test_deleted_wal_cause_is_plain_first_steps_not_a_forensic_runbook():
    """The WAL-generation runbook lives in the logger.error at hbm_state; the chat reply
    gives the two steps a user can take (stop, doctor) and points at the log."""
    from hbm_state_errors import PERSISTENCE_ERROR_CAUSES

    out = AIAgent._format_turn_completion_explanation(
        "session_persistence_failed", "deleted_wal"
    ).lower()
    assert "deleted_wal" in PERSISTENCE_ERROR_CAUSES
    assert "hbm gateway stop" in out and "hbm doctor" in out
    ...
    assert "~/.hbm" not in out  # display_hbm_home(), never a hardcoded path
...
def test_explanation_persistence_one_arg_backward_compat():
    ...
    assert out.strip() != ""
    assert "couldn't save" in out.lower() and "hbm doctor" in out.lower()
...
def test_classify_persistence_error_categories():
    ...
    from hbm_state import classify_persistence_error
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to hbm_state_errors.is_disk_full_error, so
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from hbm_state import SessionCompressionInProgressError
    from hbm_state_errors import CompressionSessionBusyError
    from hbm_state import classify_persistence_error
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from hbm_state import classify_persistence_error
    from hbm_state_errors import SessionTurnLeaseLostError
...
def test_classify_persistence_error_fts_provenance_order():
    ...
    from hbm_state import SessionDB, classify_persistence_error
    from hbm_state_errors import SQLITE_CORRUPT_VTAB, is_fts_scoped_corruption_error
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("HBM_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"HBM_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("HBM_TURN_COMPLETION_EXPLAINER", None)
        with patch("hbm_cli.config.load_config", counting_load):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from hbm_state import StateDbCorruptError, classify_persistence_error
