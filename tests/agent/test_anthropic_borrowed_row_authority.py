...
def hbm_home(tmp_path, monkeypatch):
    """Real on-disk HBM_HOME so ``load_pool()`` re-reads what it persisted."""
    home = tmp_path / "hbm"
    ...
    monkeypatch.setattr(
        "hbm_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_persisted_claude_code_row_carries_no_token_material(
    ...
    rows = [r for r in _persisted_rows(hbm_home) if r.get("source") == "claude_code"]
    assert len(rows) == 1
...
def test_pool_store_sync_never_adopts_a_borrowed_row(hbm_home, claude_credentials):
...
def test_refresh_from_persisted_sanitized_row_keeps_the_full_pair(
    hbm_home, claude_credentials, monkeypatch
):
...
def test_empty_oauth_entry_is_never_leased(hbm_home, claude_credentials):
...
def test_hbm_pkce_row_still_syncs_from_the_pool_store(monkeypatch):
    """The borrowed-source refusal must not disable pool-owned adoption.

    ``hbm_pkce`` *is* pool-owned, so its persisted row keeps its tokens and
