...
The official Codex endpoint must receive HBM AGENT's own harness identity, rather
...
These tests pin:
- the required HBM AGENT originator
- the versioned HBM AGENT User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_hbm_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("agent.process_bootstrap.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "hbm-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"HbmAgent/{__version__}"
