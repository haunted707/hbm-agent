...
conversation-affinity hint HBM AGENT sends — ``prompt_cache_key`` on both
...
bucket it warmed. HBM AGENT cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-HBM AGENT-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is an HBM AGENT-internal token; a session KEY embeds the
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-HBM AGENT-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``HBM_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("HBM_SESSION_SOURCE", "override-src")
