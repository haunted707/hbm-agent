...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate HBM_HOME under tmp_path."""
    monkeypatch.setenv("HBM_HOME", str(tmp_path / ".hbm"))
    (tmp_path / ".hbm").mkdir()

    # Force the constants/image cache helpers to re-read HBM_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("hbm_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hbm_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under HBM_HOME — gateway cleanup
