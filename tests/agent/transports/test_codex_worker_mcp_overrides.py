...
``mcp_servers.hbm-mcp.env.*`` while the migration writes ``[mcp_servers.hbm-tools]``;
...
from hbm_cli.codex_runtime_plugin_migration import migrate
...
def launch(monkeypatch, tmp_path):
    ...
    for key in (*KANBAN_ENV_KEYS, DELEGATED_CHILD_ENV_MARKER, "HBM_KANBAN_DB", "HBM_KANBAN_BOARD"):
...
def _migrated_server_names(tmp_path) -> set[str]:
    ...
    report = migrate({"mcp_servers": {}}, codex_home=codex_home, discover_plugins=False, expose_hbm_tools=True)
...
def test_worker_overrides_target_the_migrated_server(launch, tmp_path):
    ...
    overrides = launch({
        "HBM_KANBAN_TASK": "11111111-1111-4111-8111-111111111111",
        "HBM_KANBAN_RUN_ID": "42",
        "HBM_KANBAN_DB": str(tmp_path / "board" / "kanban.db"),
    })
    ...
    assert any(arg.startswith(f"mcp_servers.{next(iter(targeted))}.env.HBM_KANBAN_TASK=") for arg in overrides)
...
def test_only_dispatcher_owned_workers_get_mcp_overrides(launch):
    ...
    with non_dispatcher_owned_context():
        assert launch({"HBM_KANBAN_TASK": "11111111-1111-4111-8111-111111111111"}) == []
