...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_hbm_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for hbm_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": hbm_level}
            )
            assert kw["reasoning_effort"] == wire_level, hbm_level
