...
`hbm_cli/runtime_provider.py`), and the echo flag must still take effect for
it. A future refactor keying the flag on provider *name* would reintroduce that
miss; this test is the tripwire.

Uses a temp HBM_HOME + real `load_config_readonly` (the config cache is
...
def _write_home(tmp_path, monkeypatch, reasoning_echo: bool):
    """Point HBM_HOME at a temp profile declaring a named custom provider."""
    home = tmp_path / "hbm"
    home.mkdir()
    lines = [
        "model:",
    ...
    try:
        from hbm_cli import config as _cfg
