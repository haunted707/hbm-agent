...
binary download are exercised manually by `hbm secrets bitwarden
...
def hbm_home(tmp_path, monkeypatch):
    """Point HBM AGENT at an isolated home directory."""
    home = tmp_path / ".hbm"
    home.mkdir()
    monkeypatch.setenv("HBM_HOME", str(home))
    # Some modules cache get_hbm_home; clear if needed.
    import hbm_constants
    if hasattr(hbm_constants, "_HBM_HOME_CACHE"):
        hbm_constants._HBM_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(hbm_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from hbm_cli.env_loader import _apply_external_secret_sources
