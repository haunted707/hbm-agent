...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_hbm_home``,
    ``get_default_hbm_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``HBM_HOME`` and no resolver mocks.
    """

    def test_real_hbm_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_hbm_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active HBM AGENT profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """HBM_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".hbm"
        ...
        assert "Active HBM AGENT profile: default." in prompt
...
def test_coding_prompt_orders_shared_context_before_workspace(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "HBM_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "HBM_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_hbm_home", lambda: Path("/hbm"))

    # Production renders this as str(get_hbm_home()) + "/profiles/<name>/",
    # and str(Path("/hbm")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/hbm".
    _home_str = str(Path("/hbm"))
    expected_profile = (
    ...
    with (
        ...
        patch("hbm_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("hbm_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under HBM_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from hbm_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("hbm_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from hbm_time import now as hbm_now
        sid = hbm_now().strftime("%Y%m%d_%H%M%S_fresh")
