...
manipulation. HBM AGENT keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point HBM_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_hbm_home /
    get_default_hbm_root) instead of monkeypatching private helpers —
    ...
    HBM_HOME=<root>/profiles/<name> makes get_default_hbm_root()
