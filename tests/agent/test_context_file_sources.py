...
def test_manifest_matches_what_the_prompt_actually_loads(project, tmp_path_factory):
    """Every context type present at once; the ladder picks .hbm.md, the chain lists both AGENTS files,
    CLAUDE.md/.cursorrules/.cursor/rules/*.mdc are shadowed, an empty file never wins, SOUL.md rides along."""
    (project / ".hbm.md").write_text("hbm rules")
    ...
    assert statuses == {
        ".hbm.md": "loaded", "../AGENTS.md": "shadowed", "AGENTS.override.md": "empty", "AGENTS.md": "shadowed",
    ...
    # Same walk, other winner: drop .hbm.md and the whole AGENTS chain loads while the rest stays shadowed.
    (project / ".hbm.md").unlink()
...
def test_truncated_and_suppressed_statuses_follow_the_builder(project, monkeypatch, tmp_path_factory):
    ...
    # Install-tree guard: a fallback cwd (cwd=None) inside the HBM AGENT tree lists the file but never loads it.
