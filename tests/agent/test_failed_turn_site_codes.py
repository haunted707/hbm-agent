...
def test_persistence_failure_default_copy_is_actionable_and_profile_aware(monkeypatch):
    monkeypatch.setenv("HBM_HOME", "/srv/hbm-profile")
    text = TurnExplainersMixin._format_turn_completion_explanation("session_persistence_failed", "replaced")
    assert "hbm gateway stop" in text and "hbm doctor" in text
    assert "~/.hbm" not in text and "/srv/hbm-profile" in text
    assert "manifest" not in text  # the runbook stays in logger.error at hbm_state
