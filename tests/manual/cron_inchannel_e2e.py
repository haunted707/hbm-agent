...
Uses a throwaway HBM_HOME so it never touches ~/.hbm. No real names.
"""
...
def _fresh_home():
    """Point HBM_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hbm_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["HBM_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER HBM_HOME is set.
...
# a different get_hbm_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"HBM_HOME (throwaway): {HOME}")
