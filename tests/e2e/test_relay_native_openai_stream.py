...
Relay runs its finalizer as soon as the provider stream ends — concurrently with HBM AGENT's
...
def _stream_through_relay(tmp_path, monkeypatch, response_body: bytes, *, finalize_before):
    ...
    Returns ``(hbm_result, relay_llm_end_event)``."""
...
def test_openai_stream_final_tool_call_delta_reaches_relay_parent_event(tmp_path, monkeypatch):
    ...
    hbm_call = result.choices[0].message.tool_calls[0]
    assert (hbm_call.function.name, hbm_call.function.arguments) == ("read_file", '{"path": "/tmp/x"}')
