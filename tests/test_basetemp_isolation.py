"""pytest's basetemp must never sit inside the operator's platform-native HBM AGENT home.

Every per-test sandbox is ``<basetemp>/.../hbm_test`` and ``get_default_hbm_root()``
...
def test_basetemp_inside_the_native_home_is_relocated_outside_it(tmp_path, monkeypatch):
    ...
    native.mkdir()
    monkeypatch.setattr(hbm_constants, "_get_platform_default_hbm_home", lambda: native)
    ...
    monkeypatch.setenv("HBM_HOME", str(relocated / "t0" / "hbm_test"))
    assert hbm_constants.get_default_hbm_root() == relocated / "t0" / "hbm_test"
...
def test_fallback_root_escapes_a_repo_checked_out_inside_the_native_home(tmp_path, monkeypatch):
    # Default install: repo at ~/.hbm/hbm-agent and TEMP under the home (Windows).
    native = tmp_path / "native-home"
    (native / "hbm-agent").mkdir(parents=True)
    monkeypatch.setattr(hbm_constants, "_get_platform_default_hbm_home", lambda: native)
    monkeypatch.setattr(suite_conftest, "PROJECT_ROOT", native / "hbm-agent")
