"""Tests for hbm_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $HBM_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the HBM AGENT-managed node dir.
"""
...
def _make_hbm_node(hbm_home: Path) -> Path:
    """Create a fake $HBM_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = hbm_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    hbm_home = fake_home / ".hbm"
    _make_hbm_node(hbm_home)
    ...
    removed = uninstall.remove_node_symlinks(hbm_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    hbm_home = fake_home / ".hbm"
    node_bin = _make_hbm_node(hbm_home)
