...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_hbm_home(), so
        # point HBM_HOME at the temp dir (the _hbm_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "hbm_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in HBM_HOME/config.yaml
    ...
    install whose HBM_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read HBM_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh HBM_HOME with NO config.yaml (managed/desktop first launch).
        hbm_home = tmp_path / ".hbm"
        hbm_home.mkdir()
        monkeypatch.setenv("HBM_HOME", str(hbm_home))
