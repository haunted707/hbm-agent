...
pointer — ``model.key_env`` in config.yaml → the env var in ``$HBM_HOME/.env``
(e.g. ``HBM_CUSTOM_LMSTUDIO_API_KEY``) — while keeping ``model.provider`` on
...
def hbm_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HBM_CUSTOM_LMSTUDIO_API_KEY", raising=False)
    return home
...
def test_model_key_env_pointer_is_honored_for_registry_provider(hbm_home):
    _write(
        hbm_home,
        "model:\n  default: some-model\n  provider: lmstudio\n"
        "  base_url: http://127.0.0.1:1234/v1\n"
        "  key_env: HBM_CUSTOM_LMSTUDIO_API_KEY\n",
        "HBM_CUSTOM_LMSTUDIO_API_KEY=sk-lm-desktop-saved\n",
    )
    ...
    assert creds["source"] == "HBM_CUSTOM_LMSTUDIO_API_KEY"


def test_model_key_env_does_not_leak_across_providers(hbm_home):
    ...
    _write(
        ...
        "model:\n  default: gpt-x\n  provider: openai\n"
        "  key_env: HBM_CUSTOM_LMSTUDIO_API_KEY\n",
        "HBM_CUSTOM_LMSTUDIO_API_KEY=sk-lm-should-not-leak\n",
    )
    from hbm_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER, resolve_api_key_provider_credentials
