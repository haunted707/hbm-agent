...
``.hbm-catalog.json`` sidecar) and the install endpoint has NO kill-list bypass."""
...
from hbm_cli import plugin_catalog as pc_cat
...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_hbm_home):
    try:
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    from hbm_cli import plugins_cmd
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the HBM AGENT plugin catalog" in resp.json()["detail"]
