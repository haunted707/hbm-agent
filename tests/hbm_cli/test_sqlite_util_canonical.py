"""Invariants for the canonical SQLite connect/transaction layer (``hbm_cli/sqlite_util.py``).
...
from hbm_cli import sqlite_util
...
def test_every_store_opens_through_the_canonical_open_db(monkeypatch, tmp_path, module_name, attr):
    ...
    if module_name == "hbm_cli.projects_db":
...
def test_plugin_db_wal_goes_through_the_shared_fallback(monkeypatch, tmp_path):
    ...
    seen = []
    real = hbm_state_wal.apply_wal_with_fallback

    def spy(conn, **kwargs):
        seen.append(kwargs["db_label"])
        return real(conn, **kwargs)

    monkeypatch.setattr(hbm_state_wal, "apply_wal_with_fallback", spy)
