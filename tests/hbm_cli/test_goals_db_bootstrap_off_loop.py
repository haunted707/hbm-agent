...
def _patch_sessiondb(monkeypatch, cls=_RecordingDB):
    # goals.py acquires through hbm_state_registry (shared writer per home); patch its
    # construction seam, and keep every acquired fake out of the registry so tests don't share.
    import hbm_state_registry

    monkeypatch.setattr(hbm_state_registry, "_open_session_db", lambda path: cls(db_path=path))
...
def test_loop_thread_cache_hit_returns_cached_instance(monkeypatch):
    ...
    goals._DB_CACHE[str(get_hbm_home())] = sentinel
    result = "UNSET"
