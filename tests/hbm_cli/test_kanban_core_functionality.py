...
Complements tests/hbm_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/hbm_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/hbm_cli/test_kanban_db.py.
    monkeypatch.setenv("HBM_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    hbm subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("HBM_KANBAN_TASK") == tid
    assert env.get("HBM_PROFILE") == "some-profile"
...
def test_cli_daemon_help_marks_deprecated():
    ...
    from hbm_cli import kanban as kb_cli
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `hbm kanban --help` output"
    )
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``hbm_cli.kanban_db`` fresh and uses that single
...
_WORKER_LOG_TAIL = (
    ...
    "╰──────────────────────────────╯\n"
    "\nResume this session with:\n  hbm --resume 20260915_000000_abc\n\n"
