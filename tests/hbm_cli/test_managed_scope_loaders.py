...
hbm_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # hbm_time checks an env override first; ensure it's unset so config wins.
    ...
    assert hbm_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (HBM_TIMEZONE, HBM_REDACT_SECRETS, HBM_MAX_ITERATIONS,
    ...
    # writing HBM_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
