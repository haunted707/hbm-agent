"""Comprehensive tests for hbm_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and HBM_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.hbm/profiles)
    * HBM_HOME  -> tmp_path/.hbm  (so get_hbm_home() agrees)
    * Creates the bare-minimum ~/.hbm directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".hbm"
...
class TestGetProfileDir:
    ...
    def test_default_returns_hbm_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".hbm"
...
class TestCreateProfile:
    ...
    def test_clone_sync_imports_carries_manifest_but_never_links_profiles(self, profile_env):
        ...
        from hbm_cli.agent_import_sync import SYNC_MANIFEST_NAME, load_sync_manifest
...
class TestNoSkillsOptOut:
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["hbm-agent"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
        ...
        assert r1.get("copied") == ["hbm-agent"]
        assert len(called) == 1
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `hbm update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".hbm" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("hbm_cli.profiles._cleanup_gateway_service"), \
             patch("hbm_cli.profiles.time.sleep"), \
             patch("hbm_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "hbm_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "hbm_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "hbm_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "hbm_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_hbm_shim(self, profile_env, monkeypatch):
        """A `hbm` console-script shim spawned directly (e.g. Electron's
        findOnPath('hbm') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "hbm" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "hbm".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/hbm", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/hbm", "--profile", "other", "serve"]),
            # Non-hbm script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_hbm_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "hbm" (e.g.
        hbm-notes.py, hbm-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "hbm" -- only the actual known
        console-script entry points (hbm, hbm-agent, hbm-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/hbm-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/hbm-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (hbm-agent,
        hbm-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "hbm"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/hbm-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/hbm-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".hbm" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".hbm" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom HBM_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("HBM_HOME", str(custom))
        # With Docker-aware roots, a custom HBM_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("hbm_cli.profiles.shutil.which", lambda name: "/opt/hbm/bin/hbm")
        from hbm_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/hbm/bin/hbm -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from hbm_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from hbm_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from hbm_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "hbm.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["hbm_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["hbm_heimdall"]["peerName"] == "user-peer"
    ...
    def test_multiplexed_rename_unroutes_old_then_hot_serves_new(self, profile_env):
        ...
        create_profile("oldname", no_alias=True)
        old_dir = tmp_path / ".hbm" / "profiles" / "oldname"
        new_dir = tmp_path / ".hbm" / "profiles" / "newname"
        ...
        def _record_notify(name):
            ...
            if name == "oldname" and old_dir.exists():
                ...
                with pytest.raises(FileNotFoundError):
                    mkdir_under_hbm_home(old_dir / "logs")

        with patch("hbm_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("hbm_cli.profiles._live_default_multiplexer", return_value=True), \
             patch("hbm_cli.profiles._notify_multiplexer", side_effect=_record_notify):
    ...
    def test_unmultiplexed_rename_does_not_signal_multiplexer(self, profile_env):
        ...
        with patch("hbm_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("hbm_cli.profiles._live_default_multiplexer", return_value=False), \
             patch("hbm_cli.profiles._notify_multiplexer") as notify:
            new_dir = rename_profile("oldname", "newname")

        notify.assert_not_called()
        assert not (tmp_path / ".hbm" / "profiles" / ".deleted").exists()
        assert not old_dir.exists() and new_dir.is_dir()
    ...
    def test_rename_migrates_session_identity_without_live_gateway(self, profile_env):
        ...
        pdb.close()
        root_db = SessionDB(tmp_path / ".hbm" / "state.db")
        root_db.save_gateway_routing_entry(
            ...
            scope=str(tmp_path / ".hbm" / "sessions"))
        root_db.close()
        ...
        moved_db.close()
        root_db2 = SessionDB(tmp_path / ".hbm" / "state.db")
    ...
    def test_rename_delegates_identity_migration_to_live_gateway(self, profile_env):
        ...
        with patch("hbm_cli.profiles.check_alias_collision", return_value="skip"), \
             ...
             patch("hbm_state_registry.acquire") as acquire:
            rename_profile("oldname", "newname")
    ...
    def test_migrate_identity_command_repairs_a_failed_live_migration(self, profile_env, capsys):
        ...
        moved_db = SessionDB(tmp_path / ".hbm" / "profiles" / "newname" / "state.db")
        row = moved_db._read_one(
            "SELECT session_key, profile_name FROM sessions WHERE id = ?", ("sess1",))
...
class TestExportImport:
    ...
    def test_export_default_includes_profile_data(self, profile_env, tmp_path):
        ...
        # Write through HBM_HOME, not get_profile_dir("default"): the latter resolves to the
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultHbmHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_hbm_home()."""



    def test_default_hbm_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_hbm_home() returns HBM_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HBM_HOME", str(docker_home))
        home = _get_default_hbm_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under HBM_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HBM_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        from hbm_cli.profiles import _check_gateway_running
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="hbm gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with HBM_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("HBM_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_hbm_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured HBM_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("HBM_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No HBM_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("HBM_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_hbm_home()
...
class TestCloneAllExcludesRuntimeTrees:
    ...
    runtime trees the local-models flow puts under ``~/.hbm``: ``models/``
    ...
    def test_runtime_trio_is_one_constant_shared_with_backup(self):
        ...
        from hbm_cli import backup, profiles
        from hbm_constants import LOCAL_RUNTIME_ROOT_DIRS
