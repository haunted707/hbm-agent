...
def _fresh_memo(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("HBM_PORTAL_BASE_URL", raising=False)
...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    hbm_constants.set_hbm_home_override (gateway/run.py,
    tui_gateway/server.py), not the HBM_HOME env var — the memo must key
    ...
    reset_token = hbm_constants.set_hbm_home_override(str(profile_a))
    try:
        token_a = auth.resolve_nous_access_token()
    finally:
        hbm_constants.reset_hbm_home_override(reset_token)
    ...
    reset_token = hbm_constants.set_hbm_home_override(str(profile_b))
    try:
