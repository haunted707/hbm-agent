"""ensure_hbm_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".hbm"
    second = tmp_path / "b" / ".hbm"

    monkeypatch.setenv("HBM_HOME", str(first))
    cfg.ensure_hbm_home()

    # Profile switch: HBM_HOME moves → the new path is ensured too.
