"""A pooled Desktop backend (``hbm --profile X serve``, HBM_HOME=<root>/profiles/X) answers its
REST without ``?profile=``. Those UNSCOPED reads/verbs are about X — a profile the live default
multiplexer serves — and must agree with the scoped ``?profile=X`` answer and with ``hbm -p X status``:
...
restart`` under X's HBM_HOME that exited 78 while the UI reported success.
"""
...
def pooled_served_process(tmp_path, monkeypatch):
    """Process whose HBM_HOME is a served named profile; the default home records a live multiplexer."""
    root = tmp_path / "hbm"
    ...
    monkeypatch.setenv("HBM_HOME", str(root / "profiles" / "alpha"))
...
def test_unscoped_lifecycle_verbs_in_a_served_profile_process_address_the_multiplexer(pooled_served_process):
    ...
    # The child must run under the DEFAULT home (the multiplexer's), not inherit alpha's HBM_HOME.
    assert _profile_action_environment(restart)["HBM_HOME"] == str(pooled_served_process)
