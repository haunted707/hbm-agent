...
from hbm_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS, normalize_provider
...
class TestFireworksAliases:
    ...
    def test_providers_normalize_provider(self, alias):
        from hbm_cli.providers import normalize_provider as normalize_in_providers
...
class TestFireworksOverlay:
    def test_overlay_exists(self):
        ...
        assert "fireworks" in HBM_OVERLAYS
        overlay = HBM_OVERLAYS["fireworks"]
...
class TestFireworksAuxiliary:
    ...
    def test_client_sends_attribution_headers(self, monkeypatch):
        ...
        assert headers["X-Title"] == "HBM AGENT"
        assert kwargs["base_url"] == "https://api.fireworks.ai/inference/v1"

    def test_client_sends_hbm_user_agent(self, monkeypatch):
        ...
        headers = kwargs.get("default_headers", {})
        assert headers["User-Agent"].startswith("HbmAgent/")
