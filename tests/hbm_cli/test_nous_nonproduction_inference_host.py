...
* Profile authority — ``NOUS_INFERENCE_BASE_URL`` / ``HBM_PORTAL_BASE_URL`` are per-profile
...
@pytest.mark.parametrize("helper, var", [
    ...
    (auth_nous._nous_portal_env_override, "HBM_PORTAL_BASE_URL"),
])
...
def test_portal_override_alone_grants_nothing(monkeypatch):
    ...
    monkeypatch.setenv("HBM_PORTAL_BASE_URL", NONPROD_PORTAL)
    monkeypatch.delenv("NOUS_INFERENCE_BASE_URL", raising=False)
