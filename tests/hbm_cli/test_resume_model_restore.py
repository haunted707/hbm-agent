...
SessionDB.session_gateway_runtime (hbm_state.py) — the round trip that
makes `hbm --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare HbmCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.HbmCLI)
