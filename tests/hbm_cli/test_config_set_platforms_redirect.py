...
`hbm config set platforms.<name>.<display_setting> <value>` must write to
...
def _write_config(hbm_home: Path, data: dict) -> Path:
...
def _set(monkeypatch, hbm_home, key, value, force=False):
    """Isolated call to set_config_value against a temp HBM_HOME."""
    monkeypatch.setenv("HBM_HOME", str(hbm_home))
    # set_config_value resolves the home live via get_config_path()/get_hbm_home()
...
class TestPerPlatformDisplayRedirect:
    def test_streaming_redirects_to_display_platforms(self, hbm_home, monkeypatch):
        """platforms.telegram.streaming must land under display.platforms."""
        _set(monkeypatch, hbm_home, "platforms.telegram.streaming", "false")

        result = yaml.safe_load((hbm_home / "config.yaml").read_text())
    ...
    def test_show_reasoning_redirects(self, hbm_home, monkeypatch):
        _set(monkeypatch, hbm_home, "platforms.telegram.show_reasoning", "false")
        result = yaml.safe_load((hbm_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["telegram"]["show_reasoning"] is False

    def test_tool_progress_redirects(self, hbm_home, monkeypatch):
        ...
        # is the bool.
        _set(monkeypatch, hbm_home, "platforms.discord.tool_progress", "off")
        result = yaml.safe_load((hbm_home / "config.yaml").read_text())
        assert result["display"]["platforms"]["discord"]["tool_progress"] is False

    def test_connection_key_not_redirected(self, hbm_home, monkeypatch):
        """A real connection key (token) stays in top-level platforms.<name>."""
        _set(monkeypatch, hbm_home, "platforms.telegram.token", "new-token")
        result = yaml.safe_load((hbm_home / "config.yaml").read_text())
...
class TestRedirectSiblingSurfaces:
    ...
    def test_get_mirrors_gateway_resolution_after_set(self, hbm_home, monkeypatch, capsys):
        ...
        raw = yaml.safe_load((hbm_home / "config.yaml").read_text())
        assert resolve_display_setting(raw, "telegram", "streaming") is False

    def test_unset_removes_the_redirected_leaf(self, hbm_home, monkeypatch):
    ...
    def test_unset_missing_redirected_leaf_exits_nonzero(self, hbm_home, monkeypatch):
        from hbm_cli.config import unset_config_value
    ...
    def test_set_prints_redirect_note(self, hbm_home, monkeypatch, capsys):
        _set(monkeypatch, hbm_home, "platforms.telegram.streaming", "false")
    ...
    def test_redirect_helper_only_touches_known_display_keys(self):
        ...
        from hbm_cli.config import _redirect_platform_display_key
