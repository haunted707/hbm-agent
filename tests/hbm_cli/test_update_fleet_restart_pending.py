...
A ``hbm update`` killed after git pull advanced HEAD but before the
...
- next ``hbm update`` with git already up to date still runs the
...
import hbm_cli.main_web_build as main_web_build
import hbm_cli.main_install_repair as main_install_repair
from hbm_cli import update_cmd
import hbm_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(hbm_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(hbm_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(hbm_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(hbm_main, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(
        hbm_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(hbm_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        hbm_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        main_web_build, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(hbm_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        hbm_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        hbm_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(hbm_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(hbm_main, "_clear_update_incomplete_marker", lambda: None)
    ...
    monkeypatch.setattr(hbm_main, "_build_web_ui", lambda *a, **k: None)
    ...
    monkeypatch.setattr(update_cmd, "_purge_stale_hbm_modules", lambda: None)
    monkeypatch.setattr(hbm_main, "_purge_stale_hbm_modules", lambda: None)

    import hbm_cli.gateway as hbm_gateway

    monkeypatch.setattr(
        hbm_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(hbm_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        hbm_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "hbm_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_hbm_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_hbm_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_hbm_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "hbm_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(hbm_main, "_purge_stale_hbm_modules", lambda: None)
    ...
    monkeypatch.setattr("hbm_cli.gateway_windows.is_installed", lambda: False)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``hbm-gateway.service``) and an unmanaged ``serve`` on the same
    ...
    import hbm_cli.update_inventory as ui
    ...
    import hbm_cli.process_identity as pi
    ...
    latest = get_hbm_home() / "logs" / "update_receipts" / "latest.json"
...
def test_clean_update_defers_desktop_owned_serve_and_clears_marker(
    ...
    hbm_main.cmd_update(args)  # no SystemExit(1)
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(hbm_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "hbm gateway restart" in err
...
#
# `systemctl --user restart hbm-gateway` never runs this module's clear path, and an update
...
def test_startup_warn_kept_without_positive_evidence(monkeypatch, capsys, disk_sha, fleet):
    ...
    _patch_marker_sha(monkeypatch, disk_sha)
    monkeypatch.setattr("hbm_cli.update_receipt.collect_fleet_versions", lambda **kwargs: fleet)
