"""Flat-install runtime state must be gitignored so ``hbm update``'s untracked
autostash cannot sweep the live state.db (#110648).

On a flat install (checkout root == $HBM_HOME) the profile's runtime files
live inside the repo as untracked paths. ``git stash push --include-untracked``
(hbm_cli/update_cmd_stash.py) moves the whole untracked set into the stash and
...
.hbm-bootstrap-complete / .install_method precedent (#38529 / #66189).
"""
import shutil
import sqlite3
import subprocess
...
# Runtime state that lives at $HBM_HOME's root on a flat install, exactly as
# ``hbm update`` would sweep it: one representative per ignored class. The
...
def flat_install_repo(tmp_path: Path) -> Path:
    ...
    Built in a subdirectory of tmp_path: the suite-wide HBM_HOME isolation
    fixture (tests/conftest.py) materialises its own ``hbm_test/`` tree in
    ...
    (repo / "app.py").write_text("print('hbm')\n")
...
def test_flat_install_runtime_state_is_ignored(flat_install_repo):
    ...
    so `hbm update` never enters its stash step for runtime state alone."""
    status = _run_git(
...
def test_untracked_autostash_cannot_sweep_runtime_state(flat_install_repo):
    ...
    _run_git(
        flat_install_repo,
        "stash", "push", "--include-untracked", "-m", "hbm-update-autostash",
    )
    assert (flat_install_repo / "app.py").read_text() == "print('hbm')\n"
    missing = [
        rel
        for rel in FLAT_INSTALL_RUNTIME_STATE
...
def test_untracked_autostash_leaves_open_wal_database_readable(flat_install_repo):
    ...
    from hbm_cli.update_cmd_stash import _stash_local_changes_if_needed
