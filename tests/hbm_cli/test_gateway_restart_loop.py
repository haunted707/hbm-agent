...
Covers:
- Defense 1: gateway stop/restart refuse when _HBM_GATEWAY=1
...
from hbm_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    def test_hbm_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.hbm.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.hbm/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l hbm-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.hbm.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.hbm.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.hbm/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.hbm.gateway-apollo:/a.plist' "
            "'ai.hbm.gateway-cronus:/c.plist' 'ai.hbm.gateway-plutus:/p.plist' "
            "'ai.hbm.gateway:/Users/botuser/Library/LaunchAgents/ai.hbm.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated hbm note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.hbm.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.hbm.gateway',
        "launchctl kick'start' -k gui/501/ai.hbm.gateway",
        "launchctl kick\\start -k gui/501/ai.hbm.gateway",
        'launchctl "kickstart" -k gui/501/ai.hbm.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.hbm.gateway',
        "launchctl dis\\able gui/501/ai.hbm.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.hbm."gateway"',
        # Same class on the systemctl and hbm-CLI branches.
        'systemctl re"start" hbm-gateway',
        'hbm gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.hbm.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.hbm.update-checker',
        'systemctl re"start" hbm-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `hbm gateway start` is benign — starting a gateway from inside a
        ...
        "hbm gateway start",
        "hbm gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway hbm
        # services must not be falsely blocked (the old `.*hbm` matched any
        # hbm token).
        "launchctl unload ai.hbm.update-checker.plist",
        "launchctl restart ai.hbm.daemon",
        ...
        "systemctl restart hbm-meta.service",
        "systemctl restart hbm-cron-helper",
        ...
        "echo after the hbm gateway restarted cleanly",
        "the hbm gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "hbm skill view gateway-notes && echo hbm gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `hbm` is a path component
        # there, not a command.
        "cat '/docs/hbm gateway restart-notes.md'",
        "less /home/user/notes/hbm gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "hbm gateway restart",
        "hbm gateway restart; echo done",
        "hbm gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;hbm gateway restart",
        "true && hbm gateway stop",
        "echo $(hbm gateway restart)",
        "echo `hbm gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.hbm.gateway"])',
        'subprocess.run(["hbm", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.hbm.gateway")',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: hbm gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `hbm -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry HBM_HOME/HBM_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("HBM_PROFILE", "zeus")
        monkeypatch.delenv("HBM_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "hbm -p zeus gateway stop",
        "hbm -p zeus gateway restart",
        "hbm --profile zeus gateway restart",
        "hbm --profile zeus gateway stop",
        "hbm --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "hbm -v -p zeus gateway restart",
        "hbm -p zeus -v gateway restart",
        "hbm --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "hbm -p venus gateway stop",
        "hbm -p venus gateway restart",
        "hbm --profile venus gateway restart",
        "hbm --profile=venus gateway stop",
        "hbm -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "hbm -p zeus gateway start",
        "hbm -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("hbm gateway restart")
        assert _contains_gateway_lifecycle_command("hbm gateway stop")

    def test_hbm_home_derived_profile(self, monkeypatch):
        # Without HBM_PROFILE the guard falls back to the HBM_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("HBM_PROFILE", raising=False)
        monkeypatch.delenv("HBM_PROFILE_NAME", raising=False)
        import hbm_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("hbm -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("hbm -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("hbm -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("hbm gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hbm_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade hbm then run hbm gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_hbm_gateway_once.sh). The script must live under
        # HBM_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("HBM_HOME", str(tmp_path / ".hbm"))
        scripts_dir = tmp_path / ".hbm" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_HBM_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _HBM_GATEWAY=1.

    Issue #37453: systemctl --user restart hbm-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart hbm-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.hbm.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-hbm label: label-independent detection is the point
        # (#62891 second reproduction used `ai.hbm.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.hbm.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _HBM_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _HBM_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_HBM_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="hbm gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["hbm gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.hbm.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.hbm.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-hbm systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run hbm gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("hbm gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/hbm\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 hbm gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_hbm_home()` falls back to Path.home(), which raises
        RuntimeError when neither HBM_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart hbm-gateway' .",
        "grep -rn 'hbm gateway restart' ./logs",
        "rg 'hbm gateway restart' ../archive",
        "grep -c 'systemctl stop hbm-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM hbm_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell hbm gateway restart"',
        'sqlite3 ./db ".system systemctl restart hbm-gateway"',
        'psql ./x -c "\\! systemctl restart hbm-gateway"',
        "grep -r 'hbm gateway restart' . | sh",
        "grep -r 'hbm gateway restart' ./logs | bash",
        "grep -r 'hbm gateway restart' . | sudo sh",
        "grep -r 'x' . ; hbm gateway restart",
        "grep -r 'x' . && systemctl restart hbm-gateway",
        'grep -r "$(hbm gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart hbm-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell hbm gateway restart"',
        'sqlite3 db ".system systemctl restart hbm-gateway"',
        'psql -c "\\! systemctl restart hbm-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    hbm_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run hbm gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_HOME", str(tmp_path / ".hbm"))
        (tmp_path / ".hbm").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run hbm gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop hbm-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart hbm-gateway' /var/log/syslog",
        "rg 'hbm gateway restart' /home/user/.hbm/logs/",
        "journalctl -u hbm-gateway --grep 'systemctl restart hbm-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'hbm_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell hbm gateway restart"',
        'psql -c "\\! systemctl restart hbm-gateway"',
        "grep 'systemctl restart hbm-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart hbm-gateway",
        'grep "$(systemctl restart hbm-gateway)" f',
        "grep 'restart' log; systemctl restart hbm-gateway",
        'sqlite3 db "SELECT 1"; hbm gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "hbm gateway restart",
        "sudo systemctl stop hbm-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart hbm-gateway%'\""
        )
