...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_hbm_home", lambda: hbm_home)
    monkeypatch.setattr(uninstall, "_is_default_hbm_home", lambda home: False)
    ...
    assert str(hbm_home) in output
    assert project_root.exists()
    assert hbm_home.exists()
