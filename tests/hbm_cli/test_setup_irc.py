"""Tests for IRC gateway configuration via `hbm setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh HBM AGENT install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new HBM AGENT install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#hbm")
            monkeypatch.setenv("IRC_NICKNAME", "hbm-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `hbm setup gateway` experience with IRC present."""
