"""Tests for the `hbm memory reset` CLI command.
...
- Profile-scoped reset (uses HBM_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake HBM_HOME with memory files."""
    hbm_home = tmp_path / ".hbm"
    memories = hbm_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("HBM_HOME", str(hbm_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHbm repo is at ~/.hbm/hbm-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `hbm memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        hbm_home = tmp_path / ".hbm"
        (hbm_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("HBM_HOME", str(hbm_home))
