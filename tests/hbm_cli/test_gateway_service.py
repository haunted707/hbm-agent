...
import hbm_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`hbm gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `hbm gateway restart` in the background; when a
        ...
        assert "hbm gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``HBM_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hbm_test`` HBM_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "hbm-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir HBM_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="HBM_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/hbm_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="HBM_HOME=/tmp/hbm-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="HBM_HOME={tmp_path}/hbm-home"\n'
...
class TestServiceIdentityForForeignHome:
    """A HBM_HOME that is neither ``~/.hbm`` nor ``~/.hbm/profiles/<name>`` must never resolve to
    the default profile's ``hbm-gateway`` unit (a temp-home harness uninstalled the production gateway)."""
    ...
    def test_foreign_home_gets_its_own_unit(self, machine_home, tmp_path, monkeypatch):
        ...
        foreign.mkdir()
        monkeypatch.setenv("HBM_HOME", str(foreign))

        default_unit = machine_home / ".config" / "systemd" / "user" / "hbm-gateway.service"
        assert gateway_cli.get_service_name() != "hbm-gateway"
    ...
    def test_default_and_named_profile_homes_keep_their_names(self, machine_home, monkeypatch):
        default_home = machine_home / ".hbm"
        (default_home / "profiles" / "alpha").mkdir(parents=True)

        monkeypatch.setenv("HBM_HOME", str(default_home))
        assert gateway_cli.get_service_name() == "hbm-gateway"

        monkeypatch.setenv("HBM_HOME", str(default_home / "profiles" / "alpha"))
        assert gateway_cli.get_service_name() == "hbm-gateway-alpha"

    def test_sudo_user_default_home_keeps_bare_service_name(self, machine_home, tmp_path, monkeypatch):
        sudo_home = tmp_path / "alice"
        sudo_default = sudo_home / ".hbm"
        ...
        monkeypatch.delenv("HBM_HOME", raising=False)
        assert gateway_cli.get_service_name() == "hbm-gateway"

        # After unit sync, HBM_HOME points at the invoking user's native home.
        monkeypatch.setenv("HBM_HOME", str(sudo_default))
        assert gateway_cli.get_service_name() == "hbm-gateway"


class TestUninstallRefusesForeignUnit:
    """systemd_uninstall must not stop/disable/unlink a unit pinned to another HBM_HOME."""

    def test_unit_for_other_home_is_left_alone(self, tmp_path, monkeypatch, capsys):
        unit_path = tmp_path / "hbm-gateway.service"
        unit_path.write_text('[Service]\nEnvironment="HBM_HOME=/somewhere/else"\n', encoding="utf-8")
        monkeypatch.setenv("HBM_HOME", str(tmp_path / "mine"))
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path)
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("HBM_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("HBM_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("HBM_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("HBM_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_restart_exit_code_is_also_declared_a_success_status(self):
        ...
        and any OnFailure= alert unit fires on every routine restart (hbm update,
        hbm gateway restart, the in-app restart). SuccessExitStatus=75 keeps the same
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".hbm" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `hbm gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import hbm_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.hbm.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>HBM_HOME</key>"
                "<string>/Users/alice/.hbm</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_hbm_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.hbm.gateway")
        ...
        assert ("observe", "ai.hbm.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.hbm.gateway"])
            ),
        )
...
class TestSystemUnitHbmHome:
    """HBM_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".hbm" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".hbm")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_hbm = target_home / ".hbm"
        root_home = tmp_path / "root"
        root_hbm = root_home / ".hbm"
        managed_bin = target_hbm / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_hbm.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("HBM_HOME", str(root_hbm))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home), 1001),
        )
        monkeypatch.setattr(gateway_cli, "get_hbm_home", lambda: root_hbm)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "hbm_constants.iter_hbm_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "hbm_constants.hbm_managed_node_tree_present",
    ...
    def test_system_unit_orders_after_target_user_manager(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("HBM_HOME", str(tmp_path / ".hbm"))
        monkeypatch.setattr(
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HBM_HOME=/home/alice/.hbm' in unit
        assert '/root/.hbm' not in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's HBM_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hbm_home = str(gateway_cli.get_hbm_home().resolve())
        assert f'HBM_HOME={hbm_home}' in unit


class TestSystemUnitRefreshSyncsHbmHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.hbm."""

    def test_refresh_adopts_unit_hbm_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_hbm = root_home / ".hbm"
        alice_hbm = alice_home / ".hbm"
        root_hbm.mkdir(parents=True)
        alice_hbm.mkdir(parents=True)
        (root_hbm / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_hbm / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's HBM_HOME + drain timeout).
        monkeypatch.setenv("HBM_HOME", str(alice_hbm))
        ...
        # Simulate sudo without inherited HBM_HOME (falls back to root).
        monkeypatch.setenv("HBM_HOME", str(root_hbm))
        ...
        assert os.environ["HBM_HOME"] == str(alice_hbm)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned HBM_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_hbm_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_hbm_home_from_systemd_unit",
...
class TestHbmHomeForTargetUser:
    """Unit tests for _hbm_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("HBM_HOME", raising=False)

        result = gateway_cli._hbm_home_for_target_user("/home/alice")
        assert result == "/home/alice/.hbm"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "hbm gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".hbm" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("HBM_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_hbm_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home), 1001),
        ...
        assert f'HBM_HOME={target_home / ".hbm" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".hbm" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HBM_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_hbm_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".hbm" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.hbm.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hbm" / "hbm-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hbm" / "hbm-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".hbm" / "hbm-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("HBM_HOME", str(root_home / ".hbm"))
        monkeypatch.setattr(gateway_cli, "get_hbm_home", lambda: root_home / ".hbm")
        ...
        # WorkingDirectory is anchored at the target user's HBM_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.hbm" in unit
        assert "WorkingDirectory=/home/alice/.hbm/hbm-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(SystemctlUnavailableError):
            gateway_cli._run_systemctl(["start", "hbm-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "hbm-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'hbm gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHbmUnitDetection:
    """Tests for _find_legacy_hbm_units / has_legacy_hbm_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``hbm.service`` unit behind when the service was
    renamed to ``hbm-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``hbm.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``hbm-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=HBM AGENT Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m hbm_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "hbm.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "hbm.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_hbm_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m hbm_cli.main gateway run
          - python path/to/hbm_cli/main.py gateway run
          - hbm gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m hbm_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hbm/hbm_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/hbm gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hbm/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "hbm.service" if i == 0 else "hbm.service"  # same name
            # Test each variant fresh
            (user_dir / "hbm.service").write_text(
                f"[Unit]\nDescription=Old HBM AGENT\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_hbm_units()
...
class TestRemoveLegacyHbmUnits:
    """Tests for remove_legacy_hbm_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "hbm.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_hbm_units(interactive=False)
        ...
        assert any("--user stop hbm.service" in c for c in cmds_joined)
        assert any("--user disable hbm.service" in c for c in cmds_joined)
        assert any("--user daemon-reload" in c for c in cmds_joined)

    def test_does_not_touch_profile_units_during_migration(
        self, tmp_path, monkeypatch, capsys
    ):
        """Teknium's constraint: profile units (hbm-gateway-coder.service)
        ...
        profile_unit = user_dir / "hbm-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "hbm-gateway.service"
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import hbm_cli.main as cli_main
        ...
        # public, just confirm that `hbm gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import hbm_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "hbm_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_hbm_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "hbm_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_hbm_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_hbm_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_hbm_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_hbm_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_hbm_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "hbm-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "hbm-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "hbm-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``hbm gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "hbm-gateway")
...
class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``hbm gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (HBM_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_user_unit_workingdirectory_is_hbm_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".hbm"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_hbm_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_hbm_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.hbm.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.hbm.gateway"
...
class TestLaunchdUnloadedJobStderrStaysOffTerminal:
    ...
    def fake_launchctl(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.hbm.gateway")
        monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: tmp_path / "ai.hbm.gateway.plist")
        monkeypatch.setattr(gateway_cli, "_clear_launchd_unsupported_marker", lambda: None)
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        hbm = tmp_path / "home" / ".hbm"
        hbm.mkdir(parents=True)
        (hbm / "config.yaml").write_text(config_yaml, encoding="utf-8")
        monkeypatch.setenv("HBM_HOME", str(hbm))
        monkeypatch.delenv("HBM_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("HBM_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"HBM_CRON_DRAIN_TIMEOUT": "200"},
        )
        assert "TimeoutStopSec=240" in unit


class TestUnitAnchoredServiceIdentity:
    """The installed ``hbm-gateway.service`` owns the bare name: under ``sudo`` the naming basis moves
    mid-command when ``_sync_hbm_home_from_systemd_unit()`` adopts the unit's HBM_HOME (#108674).
    ...
    def test_home_not_pinned_by_unit_keeps_its_suffix(self, tmp_path, monkeypatch):
        alice_home = tmp_path / "alice" / ".hbm"
        alice_home.mkdir(parents=True)
        bob_home = tmp_path / "bob" / ".hbm"
        bob_home.mkdir(parents=True)
        root_home = tmp_path / "root" / ".hbm"
        root_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="HBM_HOME={alice_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(hbm_constants, "_get_platform_default_hbm_home", lambda: root_home)
        monkeypatch.setenv("HBM_HOME", str(bob_home))
    ...
    def test_unprivileged_profile_command_ignores_the_system_unit(self, tmp_path, monkeypatch):
        ...
        profile_home = tmp_path / "alice" / ".hbm" / "profiles" / "kimi"
        profile_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="HBM_HOME={profile_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        monkeypatch.setattr(os, "geteuid", lambda: 1000)
        monkeypatch.setenv("HBM_HOME", str(profile_home))
        assert gateway_cli.get_service_name() == "hbm-gateway-kimi"
    ...
    def test_real_unit_sync_keeps_the_name_it_validated(self, tmp_path, monkeypatch):
        ...
        before ``_sync_hbm_home_from_systemd_unit()`` must survive the mutation it performs."""
        alice_home = tmp_path / "alice" / ".hbm"
        alice_home.mkdir(parents=True)
        root_home = tmp_path / "root" / ".hbm"
        root_home.mkdir(parents=True)
        ...
        gateway_cli._sync_hbm_home_from_systemd_unit(system=True)

        assert os.environ["HBM_HOME"] == str(alice_home)  # the sync really ran
