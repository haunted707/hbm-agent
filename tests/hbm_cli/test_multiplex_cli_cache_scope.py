"""Multiplexed gateway: hbm_cli's process-wide caches must not hand profile A's value to profile B.

Every test builds two real profile homes (config.yaml / .env / cache files that differ), warms a
cache under ``set_hbm_home_override(A)`` and reads under B. Only the HTTP transport is canned —
...
def test_nous_reasoning_caps_follow_each_profiles_portal(homes, monkeypatch):
    ...
    import hbm_cli.models as models
    import hbm_cli.models_reasoning_caps as caps
...
def test_model_catalog_in_process_copy_is_bound_to_its_cache_file(homes, monkeypatch):
    a, b = homes
    import hbm_cli.model_catalog as mc
...
def test_failed_guest_mint_only_suppresses_that_profile(homes, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HBM_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    import hbm_cli.anon_auth as anon
    ...
    def client(timeout_seconds, verify):
        def handler(request):
            attempts.append(str(get_hbm_home()))
            if status["code"] == 429:
...
def test_active_skin_is_per_profile_and_leaves_launch_slot_alone(homes):
    a, b = homes
    from hbm_cli import skin_engine
