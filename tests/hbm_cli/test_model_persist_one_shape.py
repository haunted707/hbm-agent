...
four now go through ``hbm_cli.model_switch.persist_model_selection`` /
...
def _via_cli(home):
    from hbm_cli import cli_model_switch_mixin as mixin
    stub = type("Stub", (), {
...
def _via_dashboard(home):
    from hbm_cli.web_server_config import _apply_model_assignment_sync
...
def test_every_persist_surface_writes_the_same_model_block(seeded_home, monkeypatch, surface):
    monkeypatch.setattr("hbm_cli.model_switch.switch_model", lambda **_kw: _RESULT)
    monkeypatch.setattr("cli.HbmCLI._persist_model_switch_to_session", lambda *a, **k: None)
    monkeypatch.setattr("hbm_cli.cli_model_switch_mixin._print_switch_summary", lambda *a, **k: None)
    monkeypatch.setattr("hbm_cli.model_selection_guards.combined_selection_warning",
...
def test_provider_switch_drops_the_key_env_pointer_but_a_same_route_repick_keeps_it(seeded_home):
    ...
    # Registry providers get the pointer too (Desktop stores e.g. HBM_CUSTOM_LMSTUDIO_API_KEY
    ...
    (seeded_home / "config.yaml").write_text(
        seed.replace("provider: custom\n", "provider: lmstudio\n")
            .replace("CUSTOM_BOX_API_KEY", "HBM_CUSTOM_LMSTUDIO_API_KEY"), encoding="utf-8")
    ...
    assert block["key_env"] == "HBM_CUSTOM_LMSTUDIO_API_KEY"
