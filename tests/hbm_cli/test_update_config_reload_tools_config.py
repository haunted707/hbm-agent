"""``hbm update`` runs config migrations in the PRE-pull updater process. A migration step
that imports a helper at call time (``_migrate_to_45`` → ``hbm_cli.tools_config``) resolves
...
The class fix: the migration step evicts every cached HBM AGENT module (``_purge_stale_hbm_modules``,
...
def _restore_sys_modules():
    """The purge under test evicts real HBM AGENT modules; put the originals back for later tests."""
...
def test_reload_config_modules_restores_missing_tools_config_symbol():
    ...
    tools_config = sys.modules.get("hbm_cli.tools_config")
    if tools_config is None:
        import hbm_cli.tools_config as tools_config  # noqa: F811
    assert hasattr(tools_config, "_configurable_keys")

    del tools_config._configurable_keys
    try:
        from hbm_cli.update_cmd_config import _reload_config_modules
        _reload_config_modules()
        assert hasattr(sys.modules["hbm_cli.tools_config"], "_configurable_keys")
    finally:
        reloaded = sys.modules["hbm_cli.tools_config"]
...
def test_update_migration_survives_stale_module_missing_call_time_symbol(tmp_path, monkeypatch):
    ...
    from hbm_constants import set_hbm_home_override
    ...
    try:
        ...
        from hbm_cli.update_cmd import _check_and_apply_config_migration
        _check_and_apply_config_migration(assume_yes=True, gateway_mode=False, pre_update_snapshot_id=None)
    finally:
        from hbm_constants import reset_hbm_home_override
...
def test_update_migration_import_failure_after_purge_prints_fallback(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setenv("HBM_HOME", str(home))
    import hbm_cli.config  # noqa: F401  (pre-pull world has config cached)
    import hbm_constants
    # A pre-pull root module lacking a symbol the post-pull hbm_cli/config.py imports at module level.
    monkeypatch.delattr(hbm_constants, "get_process_hbm_home")
