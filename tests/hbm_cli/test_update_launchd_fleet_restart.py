"""Regression for #41403 — ``hbm update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.hbm.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed HBM_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.hbm.gateway",
            "ai.hbm.gateway-merit-ops",
            "ai.hbm.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(hbm_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.hbm.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.hbm.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.hbm.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.hbm.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.hbm.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.hbm.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.hbm.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        # box with a live ai.hbm.gateway* fleet would leak its PIDs into the assertion.
        monkeypatch.setattr(gw.subprocess, "run", lambda *a, **k: _completed(0, ""))
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.hbm.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.hbm.gateway", "ai.hbm.gateway-a", "ai.hbm.gateway-b"],
        )
        located = {
            "ai.hbm.gateway": (f"gui/{UID}", 100),
            "ai.hbm.gateway-a": (f"gui/{UID}", 200),
            "ai.hbm.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through hbm_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.hbm.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.hbm.gateway", current, "ai.hbm.gateway-user-scoped"],
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.hbm.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.hbm.gateway",
            f"user/{UID}/ai.hbm.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.hbm.gateway",
            f"user/{UID}/ai.hbm.gateway-user-scoped",
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.hbm.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.hbm.gateway-a"],
            located={"ai.hbm.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.hbm.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.hbm.gateway",
            labels=["ai.hbm.gateway", "ai.hbm.gateway-idle"],
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                "ai.hbm.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.hbm.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.hbm.gateway",
            labels=["ai.hbm.gateway", "ai.hbm.gateway-dormant"],
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                "ai.hbm.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.hbm.gateway-dormant"]
        assert restarted == ["ai.hbm.gateway", "ai.hbm.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.hbm.gateway",
            labels=["ai.hbm.gateway", "ai.hbm.gateway-a"],
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                "ai.hbm.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.hbm.gateway-a"]
        assert restarted == ["ai.hbm.gateway", "ai.hbm.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                "ai.hbm.gateway-bad": (f"gui/{UID}", 200),
                "ai.hbm.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.hbm.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.hbm.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.hbm.gateway-good"]
        assert restarted == ["ai.hbm.gateway", "ai.hbm.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.hbm.gateway",
                "ai.hbm.gateway-wedged",
                "ai.hbm.gateway-after",
            ],
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                "ai.hbm.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.hbm.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.hbm.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.hbm.gateway-after"]
        assert restarted == ["ai.hbm.gateway", "ai.hbm.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                "ai.hbm.gateway-wedged": (f"gui/{UID}", 200),
                "ai.hbm.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.hbm.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.hbm.gateway",
            labels=["ai.hbm.gateway", "ai.hbm.gateway-zombie"],
            located={
                "ai.hbm.gateway": (f"gui/{UID}", 100),
                "ai.hbm.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.hbm.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.hbm.gateway"]
        assert failed == ["ai.hbm.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.hbm.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.hbm.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )
...
class TestIncompleteWarningOnMacos:
    ...
    def test_launchd_labels_get_bootstrap_hint(self, capsys):
        _warn_incomplete_gateway_fleet_restart(["ai.hbm.gateway-merit-ops"])
