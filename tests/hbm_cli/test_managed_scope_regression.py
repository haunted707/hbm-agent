...
def hbm_home(tmp_path, monkeypatch):
    ...
    # /etc/hbm on the dev/CI box can't influence the test.
    monkeypatch.setenv("HBM_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(hbm_home, monkeypatch):
...
def test_env_expansion_in_user_config(hbm_home, monkeypatch):
    from hbm_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_hbm_dotenv(hbm_home=str(home))
