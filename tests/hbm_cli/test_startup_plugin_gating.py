...
``hbm_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``hbm --help``, ``hbm --version``,
``hbm logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
from hbm_cli._parser import build_top_level_parser, top_level_value_flag_sets
from hbm_cli.main import (
...
def _live_subcommand_names() -> set[str]:
    """Run ``hbm --help`` in-process and parse the subcommand block.
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``hbm photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain hbm-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level hbm <name> CLI command via PluginContext.
