...
from cli import HbmCLI, _build_compact_banner, _rich_text_from_ansi
from hbm_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_narrow_terminals_compact_voice_recording_prompt_fragments(self):
        ...
        with patch.object(HbmCLI, "_get_tui_terminal_width", return_value=50):
...
class TestCompactBannerSkinIntegration:
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_hbm(self):
        set_active_skin("poseidon")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "HBM AGENT v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "Poseidon Agent" in banner
        assert "NOUS HBM" not in banner
