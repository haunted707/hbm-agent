...
the SAME provider universe as ``hbm model`` (the CLI/TUI picker).
...
def test_every_hbm_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `hbm model` universe."""
    ...
    assert not missing, (
        "providers shown in `hbm model` but not configurable in the desktop "
