...
from hbm_cli.session_export_html import (
...
def test_sessions_export_cli_prompt_only_stdout(monkeypatch, capsys):
    ...
    monkeypatch.setattr(hbm_state, "SessionDB", lambda *args, **kwargs: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["hbm", "sessions", "export", "-", "--session-id", "sess", "--only", "user-prompts"],
    )
