...
Regression for the cross-profile leak class in ``hbm dashboard`` / ``hbm serve``:
``_config_profile_scope`` bound only HBM_HOME, so ``GET /api/config?profile=B`` expanded B's
``${VAR}`` refs to the DEFAULT profile's plaintext credentials (its ``os.environ``), and console
``send`` for B (``send_cmd._load_hbm_env``) copied B's ``.env`` into the shared process env with
...
def client(two_homes):
    from hbm_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    c = TestClient(app)
...
def test_console_send_for_named_profile_does_not_write_process_env(two_homes, monkeypatch):
    ...
    from hbm_cli.web_routers.chat_ws import _execute_console_line

    root, b = two_homes
    seen = {}

    def fake_send(args):
        import hbm_cli.send_cmd as send_cmd
        from gateway.config import _getenv
        send_cmd._load_hbm_env()
        seen["loader_sees"] = _getenv("TELEGRAM_BOT_TOKEN")
        seen["environ_has"] = "TELEGRAM_BOT_TOKEN" in os.environ
        seen["home"] = Path(os.environ.get("HBM_HOME", ""))
        return '{"success": true}'
