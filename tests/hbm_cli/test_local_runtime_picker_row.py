...
def test_no_staged_models_no_row(hbm_home):
    from hbm_cli.inventory import _local_runtime_row, load_picker_context

    assert _local_runtime_row(load_picker_context()) is None


def test_staged_models_make_a_selectable_row(hbm_home):
    from hbm_cli.inventory import _local_runtime_row, load_picker_context

    _stage(hbm_home, "Qwen3-4B-Instruct-2507-UD-Q8_K_XL", "Some-Other-Model")
...
def test_row_marks_current_when_config_points_at_llamacpp(hbm_home):
    from hbm_cli.inventory import _local_runtime_row, load_picker_context
...
def test_full_payload_includes_local_row(hbm_home):
...
def test_explicit_only_filter_keeps_local_row_on_any_profile(hbm_home):
    ...
    from hbm_cli.inventory import _filter_explicit_provider_rows, _local_runtime_row, load_picker_context

    _stage(hbm_home, "Qwen3.8-27B-UD-Q5_K_XL")
