"""Tests for the per-profile Projects store (hbm_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="HBM AGENT", folders=["/tmp/hbm"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "hbm-agent"
    assert proj.name == "HBM AGENT"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/hbm"
    assert [f.path for f in proj.folders] == ["/tmp/hbm"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "hbm-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' HBM_HOME.
