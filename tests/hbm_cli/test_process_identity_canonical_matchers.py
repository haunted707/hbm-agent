...
from hbm_cli.dashboard_procs import _is_desktop_local_serve_cmdline
from hbm_cli.update_cmd_windows import _hbm_holder_subcommand, _is_backend_argv
...
# value. The updater's kill set additionally requires the Desktop's `-m hbm_cli.main` spawn shape —
# a user-launched `hbm serve` / `hbm dashboard` is refused on, never tree-killed.
CMDLINES = [
    ("python -m hbm_cli.main serve " + LOOPBACK, "serve", True, True),
    ("python -m hbm_cli.main dashboard", "dashboard", False, True),
    ("/venv/bin/hbm serve --isolated --host=127.0.0.1 --port=0 --ssh-owner-nonce abc", "serve", True, False),
    (r"C:\hbm\.venv\Scripts\hbm.exe serve --host 100.106.105.2 --port 9119", "serve", False, False),
    ("hbm.exe dashboard", "dashboard", False, False),
    ("hbm --profile ops serve " + LOOPBACK, "serve", True, False),
    ("hbm -m serve kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("python -m hbm_cli.main kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("hbm --reasoning high dashboard " + LOOPBACK, "dashboard", False, False),
    ("hbm gateway run --replace", "gateway", False, False),
    ("hbm chat --model serve", "chat", False, False),
...
def test_kill_and_relaunch_predicates_agree_with_the_canonical_holder_matcher(
    ...
    assert _hbm_holder_subcommand(cmdline) == subcommand
...
def test_desktop_local_serve_spares_fixed_port_and_remote_hosts():
    assert not _is_desktop_local_serve_cmdline("hbm serve --host 100.106.105.2 --port 9119 --skip-build")
    assert not _is_desktop_local_serve_cmdline("hbm serve --host 127.0.0.1 --port 9119")
    assert _is_desktop_local_serve_cmdline("hbm serve --host localhost --port 0")
