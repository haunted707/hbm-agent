"""Tests for the top-level `./hbm-agent` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hbm-agent` should use `hbm_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hbm-agent"
    called = []

    fake_main_module = types.ModuleType("hbm_cli.main")

    def fake_main():
        called.append("hbm_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "hbm_cli.main", fake_main_module)
