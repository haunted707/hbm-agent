...
from hbm_cli.subcommands.dashboard import build_dashboard_parser
...
from hbm_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`hbm config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="hbm")
...
# ── deprecated `hbm login` fails gracefully, not with argparse error ────
#
# `hbm login` is a removed command; its handler (`login_command` in
# `hbm_cli/auth.py`) prints a deprecation notice pointing at `hbm auth` /
# `hbm model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `hbm --help`.
