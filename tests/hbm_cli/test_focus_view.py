...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        with patch.object(HbmCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(HbmCLI, "_is_session_yolo_active", return_value=False):
            text = HbmCLI._build_status_bar_text(host, width=width)
...
def _run_fake_turn(tool_progress_mode: str, dispatch_mode: str = "sequential"):
    ...
    messages: list = [
        {"role": "system", "content": "you are hbm"},
