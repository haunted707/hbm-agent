"""Fixtures shared across hbm_cli tests."""
...
def all_assignees_spawnable(monkeypatch):
    """Pretend every assignee maps to a real HBM AGENT profile.
...
def _suppress_concurrent_hbm_gate(request, monkeypatch):
    """Default ``_detect_concurrent_hbm_instances`` to ``[]`` for every test.

    The Windows update path now refuses to proceed when another
    ``hbm.exe`` is detected (issue #26670). On a developer's Windows
    machine running the test suite via ``hbm`` itself, this would
    flag the running agent as a concurrent instance and abort every
    ``cmd_update`` test. Tests that want to exercise the gate explicitly
    re-patch ``_detect_concurrent_hbm_instances`` with their own
    ...
    if request.node.get_closest_marker("real_concurrent_gate"):
        return
    try:
        from hbm_cli import main as _cli_main
    except Exception:
        return
    # raising=False: under pytest's per-test spawn isolation, a concurrent
    # xdist worker importing a module that transitively touches hbm_cli.main
    # can briefly expose a partially-initialized module object here — one where
    # _detect_concurrent_hbm_instances isn't defined yet. A bare setattr
...
def isolated_update_runtime(monkeypatch, tmp_path, request):
    ...
    from hbm_cli import gateway, main, update_cmd, update_cmd_fleet
    from hbm_cli import update_inventory, update_receipt
    ...
    monkeypatch.setattr(main, "_purge_stale_hbm_modules", lambda: None)
