...
Regression for #110820: the Restart Gateway button spawned ``hbm gateway restart`` as the
...
def system_scope_install(monkeypatch, tmp_path):
    ...
    system_unit = tmp_path / "etc" / "hbm-gateway.service"
    system_unit.parent.mkdir(parents=True)
    system_unit.write_text("[Service]\n", encoding="utf-8")
    user_unit = tmp_path / "user" / "hbm-gateway.service"
    monkeypatch.setattr(
        "hbm_cli.gateway.get_systemd_unit_path",
...
def _spawn(tmp_path, subcommand, *, sudo_ok: bool, targeted_only: bool = False, probes: list | None = None):
    """Run ``_spawn_hbm_action(subcommand)`` with Popen (and the ``sudo -n`` probes) faked;
    ...
    from hbm_cli import web_server_gateway
    ...
    ), patch("hbm_cli.web_server.PROJECT_ROOT", tmp_path):
        web_server_gateway._spawn_hbm_action(subcommand, "probe")
...
def test_only_system_scope_lifecycle_verbs_are_spawned_under_sudo(
    ...
    monkeypatch.setattr("hbm_cli.web_server_profiles._resolve_profile_dir", lambda name: tmp_path / name)
    argv = _spawn(tmp_path, subcommand, sudo_ok=True)
    plain = ["/venv/bin/python", "-m", "hbm_cli.main", *subcommand]
...
def test_targeted_nopasswd_sudoers_still_elevates(unprivileged, system_scope_install, tmp_path):
    # Same posture as the ``hbm update`` fleet restart: a refused blanket ``sudo -n true`` is
    ...
    plain = ["/venv/bin/python", "-m", "hbm_cli.main", "gateway", "restart"]
