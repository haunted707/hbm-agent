...
def test_quickstart_without_recommendation_requires_explicit_choice(client, monkeypatch):
    ...
    import hbm_cli.web_routers.local_models as lm
    ...
    monkeypatch.setattr(
        "hbm_cli.web_server_config._apply_model_assignment_sync",
...
def test_quickstart_refuses_when_nothing_fits(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_cli.local_runtime.catalog.select_variant", lambda *a, **k: None)
...
def capable_hardware(monkeypatch):
    ...
    from hbm_cli.local_runtime import hardware
    from hbm_cli.local_runtime.estimator import HardwareBudget
...
def test_quickstart_runs_all_three_legs(client, capable_hardware, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "hbm_cli.local_runtime.binaries.installed_tags", lambda: [])
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "hbm_cli.web_routers.local_models.download_file", _fake_download)

    # Leg 3: activation — stub the server start and the model assignment.
    monkeypatch.setattr(
        "hbm_cli.local_runtime.bootstrap.ensure_local_runtime",
        lambda config, force=False: calls.append("server") or None)
    monkeypatch.setattr(
        "hbm_cli.web_routers.local_models._state_endpoint",
...
def test_quickstart_skips_satisfied_legs(client, capable_hardware, monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_cli.local_runtime.binaries.installed_tags", lambda: ["b10362"])
    monkeypatch.setattr(
        "hbm_cli.local_runtime.binaries.ensure_runtime_installed",
        lambda tag, backend, progress=None: calls.append("install"))
    ...
    monkeypatch.setattr(
        "hbm_cli.local_runtime.bootstrap.staged_model_ids", lambda: all_ids)
    monkeypatch.setattr(
        "hbm_cli.web_routers.local_models.download_file",
...
def quickstart_ready(monkeypatch):
    ...
    from hbm_cli.local_runtime.catalog import VariantChoice

    monkeypatch.setattr(
        "hbm_cli.local_runtime.binaries.installed_tags", lambda: ["b10362"])
    monkeypatch.setattr(
        "hbm_cli.local_runtime.catalog.select_variant",
        lambda entry, budget: VariantChoice(variant=entry.variants[0],
                                            zero_spill=True,
                                            reason_key="best-fits"))
    monkeypatch.setattr(
        "hbm_cli.web_routers.local_models._engine_too_old",
