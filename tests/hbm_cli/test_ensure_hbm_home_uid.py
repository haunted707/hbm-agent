"""Regression tests for #34107 — Docker UID/GID handling in ensure_hbm_home.

When HBM AGENT runs in Docker with ``HBM_UID=1000`` / ``HBM_GID=911``,
the entrypoint chowns the top-level ``HBM_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_hbm_home()`` — especially
...
The fix is a ``_chown_to_hbm_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_hbm_uid_gid
# ---------------------------------------------------------------------------


class TestResolveHbmUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("HBM_UID", "1000")
        monkeypatch.setenv("HBM_GID", "911")
        from hbm_cli.config import _resolve_hbm_uid_gid
...
class TestChownToHbmUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_hbm_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("HBM_UID", raising=False)
        monkeypatch.delenv("HBM_GID", raising=False)
        from hbm_cli import config as cfg
