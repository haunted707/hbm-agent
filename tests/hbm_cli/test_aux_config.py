"""Tests for the auxiliary-model configuration UI in ``hbm model``.
...
from hbm_cli.config import DEFAULT_CONFIG, load_config
from hbm_cli.main_provider_setup import _AUX_TASKS, _DELEGATION_TASK_KEY, _delegation_cfg_as_task, _format_aux_current, _reset_aux_to_auto, _save_aux_choice
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".hbm").mkdir(exist_ok=True)
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from hbm_cli.config import load_config as _lc, save_config
...
def test_delegation_cfg_as_task_projection():
    ...
    shaped = _delegation_cfg_as_task(
        {"delegation": {"provider": "nous", "model": "HBM AGENT-4.5"}}
    )
    assert _format_aux_current(shaped) == "nous · HBM AGENT-4.5"
...
def test_leave_unchanged_replaces_cancel_label(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(hbm_cli_main_provider_setup, "_prompt_provider_choice", fake_prompt)
