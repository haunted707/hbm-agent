...
from hbm_cli.main_agent_cmds import cmd_insights
...
def _run_show_insights(command: str):
    ...
    with patch("hbm_state.SessionDB", return_value=db), \
...
def test_insights_skips_open_when_store_missing(capsys):
    ...
    for run in (lambda: HbmCLI.__new__(HbmCLI)._show_insights("/insights 7"),
                lambda: cmd_insights(SimpleNamespace(days=30, source=None))):
        with patch("hbm_state.SessionDB") as ctor:
            run()
...
def test_show_insights_opens_read_only():
    cli_obj = HbmCLI.__new__(HbmCLI)
    db = MagicMock()
    with patch("hbm_state.SessionDB", return_value=db) as ctor, \
         patch("agent.insights.InsightsEngine", _InsightsEngineStub):
        _InsightsEngineStub.calls = []
