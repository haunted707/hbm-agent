...
from hbm_cli.setup import setup_model_provider
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("hbm_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("hbm_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "hbm_cli.main._model_flow_named_custom",
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr(
        "hbm_cli.nous_subscription.get_nous_subscription_features",
    ...
    from hbm_cli.setup import setup_terminal_backend
...
def test_vercel_setup_prefills_project_and_team_from_link_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("hbm_cli.setup.prompt", fake_prompt)
