...
def hbm_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HBM_MANAGED", raising=False)
    return home
...
def test_env_var_names_the_managing_system(hbm_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("HBM_MANAGED", raising=False)
    else:
        monkeypatch.setenv("HBM_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    hbm_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the HBM_MANAGED of the service."""
    (hbm_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("HBM_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(hbm_home, monkeypatch):
    (hbm_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("HBM_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    hbm_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("HBM_MANAGED", managed_value)
    ...
    # `hbm update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "hbm update"
...
def test_unmanaged_install_offers_no_update_command(hbm_home, monkeypatch):
    monkeypatch.delenv("HBM_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(hbm_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = hbm_home / ".managed"
