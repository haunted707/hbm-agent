...
def _enable(hbm_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<hbm_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        hbm_home = Path(os.environ["HBM_HOME"])  # set by hermetic conftest fixture
        user_plugins = hbm_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(hbm_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        hbm_home = Path(os.environ["HBM_HOME"])  # set by hermetic conftest fixture
        _write_plugin(hbm_home / "plugins", ["p1"])
        _enable(hbm_home, "p1")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(hbm_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(hbm_home, "bad-img-plugin")
