...
install``. A locked ``hbm.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(hbm_main, "_is_windows", lambda: True)
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M hbm_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(hbm_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.hbm-update-staging/")
    assert is_artifact("?? cli.py.hbm-update-staging")
    assert is_artifact("?? tools.hbm-update-old/")
    ...
    assert not is_artifact(" M hbm_cli/update_cmd.py")
    assert not is_artifact("?? notes.hbm-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.hbm-update-staging/\n?? cli.py.hbm-update-old\n"),
    )
    assert (
