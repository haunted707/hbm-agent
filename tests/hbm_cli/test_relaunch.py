"""Tests for hbm_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from hbm_cli import relaunch as relaunch_mod


class TestResolveHbmBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/hbm"
        ...
        assert relaunch_mod.resolve_hbm_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'hbm' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_hbm_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/hbm" if name == "hbm" else None
        )
        assert relaunch_mod.resolve_hbm_bin() == "/usr/bin/hbm"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_hbm_bin", lambda: "/usr/bin/hbm")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/hbm"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/hbm", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/hbm", ["/usr/bin/hbm", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        hbm).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_hbm_bin", lambda: r"C:\Users\test\hbm.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\hbm.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\hbm.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_hbm_bin", lambda: r"C:\hbm.exe")
...
class TestResolveHbmBinWindowsPyGuard:
    """On Windows, resolve_hbm_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a hbm.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\hbm.exe" if name == "hbm" else None,
        )

        bin_path = relaunch_mod.resolve_hbm_bin()
        # Must NOT be the .py — must be the hbm.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\hbm.exe"
    ...
    def test_posix_python_launcher_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        The git installer invokes ``hbm`` with the venv interpreter while the
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: str(wrapper) if name == "hbm" else None,
        )
        assert relaunch_mod.resolve_hbm_bin() == str(wrapper)
    ...
    def test_posix_python_launcher_on_path_falls_back_to_current_python(
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil,
            "which",
            lambda name: str(script) if name == "hbm" else None,
        )

        assert relaunch_mod.resolve_hbm_bin() is None

        # A console script pinned to the running interpreter keeps the venv: still exec-able.
        pinned = tmp_path / "pinned" / "hbm"
        pinned.parent.mkdir()
        pinned.write_text(f"#!{sys.executable}\n")
        pinned.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(pinned), "chat"])
        assert relaunch_mod.resolve_hbm_bin() == str(pinned)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_hbm_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND hbm.exe
