...
Covers:
  ...
  - Namespace spoofing (hbm:, foreign, already-colon'd) is rejected
  ...
  - `hbm plugins show` output includes emits/listens
"""
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "hbm:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including hbm: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("hbm:core_event", lambda **p: got.append("hbm"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "hbm:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
...
def test_pending_budget_drops_new_event_without_blocking(monkeypatch, caplog):
    from hbm_cli import plugins_dispatch
