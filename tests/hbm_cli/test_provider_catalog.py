"""Tests for the unified provider catalog (hbm_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``hbm model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from hbm_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
