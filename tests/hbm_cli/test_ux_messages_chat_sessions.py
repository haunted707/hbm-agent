...
from hbm_cli.active_sessions import session_already_owned_message
from hbm_cli.cli_chat_error_copy import agent_init_failure_message, chat_error_response
...
@pytest.mark.parametrize("exc, pointer, absent", [
    (_StatusError("HTTP 401: Invalid API key", 401), "hbm model", "HTTP 401"),
    ...
    (Exception("Unknown error"), "hbm doctor", "Unknown error"),
])
...
def test_chat_error_response_accepts_plain_string_summary():
    ...
    assert "hbm model" in text.splitlines()[0]
...
def test_agent_init_failure_message_says_message_not_sent_and_points_to_doctor():
    ...
    assert "hbm doctor" in text and "/model" in text
...
def test_owned_message_first_line_is_plain_and_details_follow():
    ...
    assert first == ("This chat is open in another HBM AGENT window/terminal. "
...
def test_sessions_db_open_failure_points_to_repair(monkeypatch, capsys):
    import hbm_cli.sessions_cmd as sessions_cmd
    ...
    db_path = hbm_state._default_db_path()
