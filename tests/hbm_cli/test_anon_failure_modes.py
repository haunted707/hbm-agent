...
HBM AGENT does with each (the failure-mode contract behind the desktop's onboarding copy).

Driven through a fake NAS whose responses are the ones the real service sends (see the code table
in ``hbm_cli.anon_auth``), never through mocked-away client code.
"""
...
from hbm_cli import anon_auth, anon_sign_in, free_tier_bootstrap
from hbm_cli.auth import _load_auth_store

from tests.hbm_cli.anon_portal import PORTAL, WELCOME, install_portal  # noqa: F401
...
class TestNasRefusalCodes:
    ...
    def test_a_locked_account_is_never_replaced_through_connectors_either(self, nas):
        ...
        from tests.hbm_cli.anon_portal import make_jwt
...
class TestSignInFailures:
    ...
    def test_a_service_verdict_keeps_its_code_wait_and_copy(self, code, retry_after, retryable, needle):
        ...
        assert "HBM AGENT " not in state.copy and "http" not in state.copy
...
def test_extra_welcome_hosts_make_a_local_stand_in_the_welcome_host(monkeypatch):
    """``HBM_EXTRA_WELCOME_HOSTS`` (env-only) extends the ROUTE predicate so a rehearsal against a
    local stand-in gets the free tier's rules — including the route-keyed dark-tier 403."""
    monkeypatch.delenv("HBM_EXTRA_WELCOME_HOSTS", raising=False)
    assert anon_auth.route_is_welcome_host("http://127.0.0.1:8765/v1") is False
    monkeypatch.setenv("HBM_EXTRA_WELCOME_HOSTS", "127.0.0.1, localhost")
