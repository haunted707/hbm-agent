...
def test_noninteractive_guard_rejects_malformed_yaml(args, tmp_path, caplog, capsys):
    ...
    with caplog.at_level(logging.ERROR, logger="hbm_cli.config"):
    ...
    assert any(
        ...
        and "HBM AGENT stopped because your settings file" in record.getMessage()
...
def test_ignore_user_config_is_applied_before_oneshot_startup(monkeypatch):
    ...
    assert os.environ["HBM_IGNORE_USER_CONFIG"] == "1"
