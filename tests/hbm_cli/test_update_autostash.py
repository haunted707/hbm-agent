...
from hbm_cli import config as hbm_config
from hbm_cli import main as hbm_main
...
def _patch_gateway_discovery():
    ...
    ``_purge_stale_hbm_modules`` must also be stubbed: it evicts
    ``hbm_cli.gateway`` from ``sys.modules`` mid-update, and the restart
    phase's fresh ``from hbm_cli.gateway import ...`` then loads an
    ...
    it ``ai.hbm.gateway`` and the verify step exits 1 (#111866, #110701).
    """
    with patch("hbm_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("hbm_cli.gateway.supports_systemd_services", return_value=False), \
         patch("hbm_cli.update_cmd_fleet._restart_macos_launchd_gateways", lambda *a, **k: None), \
         patch("hbm_cli.gateway.find_profile_gateway_processes", return_value=[]), \
         patch("hbm_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("hbm_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         patch.object(hbm_main, "_fleet_probe_expected_runtimes", lambda *a, **kw: False), \
         patch.object(hbm_main, "_purge_stale_hbm_modules", lambda *a, **kw: None), \
         patch("hbm_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(hbm_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(hbm_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(hbm_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(hbm_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(hbm_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(hbm_config, "check_config_version", lambda **_kwargs: (5, 5))
    monkeypatch.setattr(hbm_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(hbm_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(hbm_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_cli.config.load_config",
        lambda: {"memory": {"provider": "mem0"}},
    )
    monkeypatch.setattr(
        "hbm_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    hbm_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_hbm_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import hbm_constants

    monkeypatch.delattr(hbm_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(hbm_constants, "apply_subprocess_home_env")

    hbm_main._reload_updated_runtime_modules()

    assert callable(hbm_constants.apply_subprocess_home_env)
...
def _make_update_side_effect(
    ...
    ``refs/hbm-update-backups/orphan-<branch>-*`` (oldest first) so the
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        hbm_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        hbm_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(hbm_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        hbm_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_orphan_history_backs_up_before_reset(monkeypatch, tmp_path, capsys):
    ...
    ``refs/hbm-update-backups/orphan-*`` ref before the reset proceeds."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert ref_name.startswith("refs/hbm-update-backups/orphan-main-")
...
def test_cmd_update_orphan_rescue_refs_pruned_beyond_keep_limit(monkeypatch, tmp_path, capsys):
    ...
    stale_refs = [
        "refs/hbm-update-backups/orphan-main-"
...
def test_prune_orphan_rescue_refs_leaves_unparseable_names_alone():
    ...
    weird = "refs/hbm-update-backups/orphan-main-not-a-timestamp-xyz"
    calls = []
    ...
    with mock_patch.object(hbm_main.subprocess, "run", side_effect=fake_run):
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        hbm_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        hbm_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "hbm_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    hbm_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    hbm_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``hbm update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(hbm_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".hbm-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by hbm update (hbm_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "hbm-update-autostash")

    assert marker.exists(), (
        ".hbm-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".hbm-bootstrap-complete" not in status
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    import subprocess
    from hbm_cli import update_cmd
    import hbm_cli.update_cmd_deps as update_cmd_deps
    ...
    assert "made the HBM AGENT unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert hbm_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
...
def test_restore_stays_parked_when_untracked_baseline_is_unknown(
    ...
    from hbm_cli import update_cmd
    import hbm_cli.update_cmd_stash as update_cmd_stash
...
def test_prune_orphan_rescue_refs_with_real_git_unpins_objects(tmp_path):
    ...
    old_ref = "refs/hbm-update-backups/orphan-main-20200101-000000-" + snap_sha[:12]
    ...
    remaining = git("for-each-ref", "refs/hbm-update-backups/").stdout
