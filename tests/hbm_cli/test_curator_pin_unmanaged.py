...
`hbm curator pin <name>` on an unmanaged skill (curation-eligible but no
...
becomes meaningful after `hbm-agent curator adopt`) and makes the message say
...
def test_pin_unmanaged_records_flag_and_prints_adopt_hint(monkeypatch, capsys):
    import hbm_cli.curator as curator_cli
    ...
    assert "hbm-agent curator adopt legacy-skill" in out
