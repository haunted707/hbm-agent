...
def test_observational_sessions_actions_open_a_read_only_store(monkeypatch, action):
    factory = MagicMock()
    monkeypatch.setattr("hbm_state.SessionDB", factory)
...
def test_doctor_write_probe_never_touches_a_store_a_live_writer_holds(monkeypatch, tmp_path):
    ...
    probed: list = []
    monkeypatch.setattr(hbm_state_repair, "_db_opens_cleanly", lambda path: probed.append(path))

    monkeypatch.setattr(hbm_state_holders, "live_writer_holds_db", lambda *_a, **_k: True)
    assert doctor_state._write_health_reason(state_db, should_fix=False) is None
    monkeypatch.setattr(hbm_state_holders, "live_writer_holds_db", lambda *_a, **_k: False)
    assert doctor_state._write_health_reason(state_db, should_fix=False) is None
