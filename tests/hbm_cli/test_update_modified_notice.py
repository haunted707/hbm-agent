"""Guard: every `hbm update` path that reports user-modified skills must
also tell the user how to find them.

`hbm update` keeps (does not overwrite) bundled skills the user edited and
...
import hbm_cli.update_cmd as update_mod
import hbm_cli.update_cmd_maint as update_maint_mod
import hbm_cli.update_cmd_zip as update_zip_mod
...
def _source_lines() -> list[str]:
    # The update pipeline was extracted to hbm_cli/update_cmd.py and then
