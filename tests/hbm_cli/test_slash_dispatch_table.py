"""Parity guard for the slash-command dispatch table in cli.HbmCLI.
...
def test_every_old_branch_resolves_to_a_handler():
    for name in OLD_CHAIN_COMMANDS:
        entry = HbmCLI._slash_handler(name)
        assert entry is not None, name
        method_name, pass_arg = entry
        assert callable(getattr(HbmCLI, method_name)), name
        assert isinstance(pass_arg, bool)
    # explicit table entries are only the ones the naming convention can't cover
    for name, (method_name, pass_arg) in HbmCLI._SLASH_DISPATCH.items():
...
def test_registry_names_resolve_into_the_table():
    from hbm_cli.commands import COMMAND_REGISTRY, resolve_command

    for name in HbmCLI._SLASH_DISPATCH:
        cmd = resolve_command(name)
        assert cmd is not None and HbmCLI._slash_handler(cmd.name) is not None, name
    # registry commands the CLI never handled inline must still fall through
    dispatched = {c.name for c in COMMAND_REGISTRY if HbmCLI._slash_handler(c.name)}
...
def test_dispatch_return_semantics_and_side_effects():
    c = _cli()
    with patch.object(HbmCLI, "_toggle_yolo", return_value=None) as m, \
            patch("hbm_cli.plugins.fire_pre_command_hook") as hook:
    ...
    c = _cli()
    with patch.object(HbmCLI, "_handle_resume_command") as m:
    ...
    c = _cli()
    with patch.object(HbmCLI, "_handle_handoff_command", return_value=False):
        assert c.process_command("/handoff telegram") is False
    with patch.object(HbmCLI, "_handle_handoff_command", return_value=True):
        assert c.process_command("/handoff telegram") is True
    with patch.object(HbmCLI, "_handle_update_command", return_value=True):
        assert c.process_command("/update") is False
    with patch.object(HbmCLI, "_handle_update_command", return_value=False):
...
def test_unknown_command_falls_through():
    ...
    with patch.object(HbmCLI, "_process_unregistered_slash", return_value=True) as m:
