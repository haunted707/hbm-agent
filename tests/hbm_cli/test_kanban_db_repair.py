...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HBM_KANBAN_DB", str(db_path))
...
def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `hbm kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated HBM_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".hbm"
    home.mkdir()
