...
def session_db(tmp_path):
    ...
    os.environ["HBM_HOME"] = str(tmp_path / ".hbm")
    os.makedirs(tmp_path / ".hbm", exist_ok=True)
    from hbm_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".hbm" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal HbmCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        HbmCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        HbmCLI._handle_branch_command(cli_instance, "/branch refactor approach")
