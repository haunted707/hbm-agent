...
from hbm_cli.web_models import MoaConfigPayload, MoaModelSlot, MoaPresetPayload
from hbm_cli.web_routers.models import set_moa_models
...
class TestSetMoaModelsPreservesUndeclaredKeys:
    ...
    def test_save_traces_preserved(self, tmp_path):
        ...
        with (
            patch("hbm_cli.config.load_config", side_effect=fake_load_config),
            patch("hbm_cli.config.save_config", side_effect=fake_save_config),
            patch("hbm_cli.web_server_profiles._profile_scope"),
        ):
...
def test_moa_save_writes_only_the_moa_section(tmp_path, monkeypatch):
    ...
    """
    import yaml
    from hbm_cli.config import get_config_path, load_config, read_raw_config
    ...
    with (
        patch("hbm_cli.config.load_config", return_value=stale),
        patch("hbm_cli.web_server_profiles._profile_scope"),
    ):
