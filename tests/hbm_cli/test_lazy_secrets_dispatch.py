...
def _run_hbm(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run hbm CLI as a subprocess from repo root.
    ...
    connectivity — but ``hbm update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-hbm-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-hbm-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`hbm secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_hbm(["secrets", "bitwarden", "status"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `hbm update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`hbm update --check` must not load cryptography._rust."""
        result = _run_hbm(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['hbm','update','--check']
...
# resolves the hbm_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('hbm_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
