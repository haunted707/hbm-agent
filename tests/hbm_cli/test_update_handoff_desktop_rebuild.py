...
A Windows git install runs `hbm update` from `hbm.exe`, which reexecs a
...
def test_current_checkout_repair_rebuilds_desktop_under_project_root():
    ...
    with (
        ...
        m.return_value.PROJECT_ROOT = update_cmd.Path("/fake/hbm")
    ...
    assert rebuild.call_args[0][0] == update_cmd.Path("/fake/hbm/apps/desktop")
