...
External secret backends are useful during normal HBM AGENT startup, but the
...
""",
    ...
    dispatch = (
        "hbm_main.cmd_update = lambda _args: 0\n"
        "hbm_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import hbm_cli.main as hbm_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "HBM_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_update_startup_does_not_import_bitwarden_or_cryptography(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["hbm", "update", "--check"])
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["hbm", "chat"])
...
def test_dotenv_loading_is_preserved_when_external_secrets_are_skipped(
    ...
    loaded = env_loader.load_hbm_dotenv(
        hbm_home=home,
...
def test_update_probe_children_skip_external_secret_sources(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, "-c",
         "import sys; sys.argv = ['hbm', 'update']\n"
         "from hbm_cli.update_cmd_deps import _validate_critical_modules_import\n"
         "print('PROBE=' + repr(_validate_critical_modules_import(__import__('os').getcwd())))"],
        capture_output=True, text=True, timeout=180, cwd=REPO_ROOT,
        env={**os.environ, "HBM_HOME": str(home)},
    )
    assert result.returncode == 0, result.stderr
