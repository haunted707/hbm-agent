...
def test_installed_tags_newest_first(hbm_home):
    from hbm_cli.local_runtime.binaries import installed_tags

    assert installed_tags() == []
    _install_fake_tag(hbm_home, "b10290")
    _install_fake_tag(hbm_home, "b10412")
    assert installed_tags() == ["b10412", "b10290"]


def test_default_tag_flows_from_default_config(hbm_home):
    """Unpinned users inherit the HBM AGENT-release default (deep-merge);
...
def test_preferred_b10964_update_offer_respects_explicit_pins(hbm_home, pin):
    ...
    _install_fake_tag(hbm_home, "b10679")
...
def test_update_available_requires_enabled_and_installed(hbm_home, monkeypatch):
...
def test_boot_never_downloads_missing_tag(hbm_home, monkeypatch):
...
def test_prune_keeps_n_minus_one(hbm_home):
    from hbm_cli.local_runtime.binaries import installed_tags, prune_old_tags

    for tag in ("b10100", "b10200", "b10290"):
        _install_fake_tag(hbm_home, tag)
    prune_old_tags(["b10290", "b10200"])
    assert installed_tags() == ["b10290", "b10200"]
    # downloads/ cache dir must survive pruning when present.
    downloads = hbm_home / "runtimes" / "llamacpp" / "downloads"
