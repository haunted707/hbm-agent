"""Tests for the Linux XDG desktop entry installed by ``hbm desktop``."""
...
from hbm_cli import linux_desktop_entry as lde
...
def xdg_home(tmp_path, monkeypatch) -> Path:
    ...
    # ~/.local/bin/hbm on the dev machine.
...
def _stub_install(tmp_path, monkeypatch) -> None:
    hbm_bin = tmp_path / "bin" / "hbm"
    hbm_bin.parent.mkdir(exist_ok=True)
    hbm_bin.write_text("", encoding="utf-8")
    monkeypatch.setattr(
        "hbm_cli.relaunch.resolve_hbm_bin", lambda: str(hbm_bin)
    )
...
def test_install_writes_entry_with_absolute_exec_and_icon(
    ...
    assert entry == xdg_home / "applications" / "hbm.desktop"
    values = _parse(entry.read_text(encoding="utf-8"))

    # Exec must be the absolute path of the resolved binary. The launcher
    # runs with a minimal PATH, so a bare `hbm` would not resolve.
    assert values["Exec"] == f"{hbm_bin} desktop"
...
def test_install_prefers_themed_icon_from_hicolor(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "hbm.png"
    assert dest.is_file()
...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr(
        "hbm_cli.relaunch.resolve_hbm_bin", lambda: "/usr/bin/hbm"
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_falls_back_to_interpreter_module(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr("hbm_cli.relaunch.resolve_hbm_bin", lambda: None)
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])

    entry = lde.install_desktop_entry(root)
    exec_line = _parse(entry.read_text(encoding="utf-8"))["Exec"]

    assert exec_line.endswith("-m hbm_cli.main desktop")
    assert Path(exec_line.split(" ")[0]).is_absolute()


# #90292: the shell installer's bash wrapper makes argv[0] the repo `hbm`
...
def test_exec_prefixes_interpreter_for_env_shebang_python_script(
    ...
    assert str(hbm_bin) in exec_line
...
def test_exec_leaves_shell_wrapper_launchers_alone(tmp_path, xdg_home, monkeypatch):
    ...
    assert exec_line == f"{hbm_bin} desktop"
...
def test_exec_leaves_venv_shebang_scripts_alone(tmp_path, xdg_home, monkeypatch):
    ...
    hbm_bin.parent.mkdir()
    interpreter = os.path.abspath(sys.executable)
    hbm_bin.write_text(f"#!{interpreter}\nimport hbm_cli\n", encoding="utf-8")
    hbm_bin.chmod(0o755)
    monkeypatch.setattr(
        "hbm_cli.relaunch.resolve_hbm_bin", lambda: str(hbm_bin)
    )
...
def test_exec_converges_from_repo_script_argv0_to_installed_wrapper(
    ...
    Launching with argv[0] = <checkout>/hbm (what the broken entry
    ...
    root = _make_project(tmp_path)
    repo_script = root / "hbm"  # checkout-internal launcher candidate
    repo_script.write_text(
        "#!/usr/bin/env python3\nimport hbm_cli\n", encoding="utf-8"
    )
    repo_script.chmod(0o755)
    wrapper = tmp_path / "installed" / "bin" / "hbm"
    ...
    monkeypatch.setattr(
        "shutil.which", lambda name: str(wrapper) if name == "hbm" else None
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_never_persists_a_bare_interpreter_command(
    ...
    """The `python -m hbm_cli.main` relaunch context must not write
...
def test_exec_keeps_resolver_fallback_when_no_wrapper_on_path(
    ...
    `sys.executable -m hbm_cli.main desktop` fallback. Persisting the
    ...
    def fake_resolve():
        # Mirror resolve_hbm_bin's chain: argv[0] → relative → PATH → None.
        return sys.argv[0] if sys.argv[0] else None

    monkeypatch.setattr("hbm_cli.relaunch.resolve_hbm_bin", fake_resolve)
...
def test_exec_uses_known_wrapper_when_path_lookup_misses(
    ...
    # checkout's hbm script (the aidiyet check requires it to target
    # the writing checkout).
    known_wrapper = tmp_path / "known-home" / ".local" / "bin" / "hbm"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "hbm"} "$@"\n',
        encoding="utf-8",
    )
...
def test_exec_finds_known_wrapper_when_resolver_has_no_candidate(
    ...
    `python -m`, and PATH has no `hbm`) makes resolve_hbm_bin return
    ...
    `<python> -m hbm_cli.main desktop` module form. Each flip between the
    wrapper and module forms rewrites hbm.desktop on the next launch; any
    ...
    known_wrapper = tmp_path / "cold-home" / ".local" / "bin" / "hbm"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "hbm"} "$@"\n',
        encoding="utf-8",
...
def test_exec_rejects_known_wrapper_from_another_checkout(
    ...
    On machines with multiple installs over time, ~/.local/bin/hbm may
    ...
    other_root.mkdir()
    foreign_wrapper = tmp_path / "known-home" / ".local" / "bin" / "hbm"
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "user",
            {},
            "HOME-SET-BY-TEST/.local/bin/hbm",
            id="user-layout",
        ),
        pytest.param(
            "termux",
            {"PREFIX": "PREFIX-SET-BY-TEST"},
            "PREFIX-SET-BY-TEST/bin/hbm",
...
def test_known_wrapper_candidates_cover_installer_layouts(
    ...
    if layout == "root-fhs":
        ...
        assert candidates.index("/usr/local/bin/hbm") < candidates.index(
            f"{sentinel_home}/.local/bin/hbm"
        )
    if layout == "non-root-no-fhs":
        # Non-root euid: /usr/local/bin must be excluded outright.
        assert "/usr/local/bin/hbm" not in candidates
...
def test_install_without_source_icon_uses_themed_name(tmp_path, xdg_home, monkeypatch):
    ...
    # when HBM AGENT is installed some other way.
    assert _parse(entry.read_text(encoding="utf-8"))["Icon"] == "hbm"
...
def test_exec_arg_quoting_handles_spaces(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    spaced = tmp_path / "my apps" / "hbm"
    spaced.parent.mkdir()
    spaced.write_text("", encoding="utf-8")
    monkeypatch.setattr("hbm_cli.relaunch.resolve_hbm_bin", lambda: str(spaced))
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_can_import_probe_runs_and_caches(tmp_path):
    ...
    deliberately NOT asserted - a CI host with hbm pip-installed
    ...
    try:
        first = lde._can_import_hbm_cli(real)
        assert isinstance(first, bool)
        assert str(real) in lde._probe_cache
        t0 = time.monotonic()
        second = lde._can_import_hbm_cli(real)
...
def test_exec_falls_back_to_running_interpreter_when_probe_fails(
    ...
    monkeypatch.setattr(lde, "_can_import_hbm_cli", lambda p: False)
...
def test_wrapper_ownership_accepts_shim_via_symlinked_home(tmp_path, monkeypatch):
    ...
    lexical_checkout = home_link / "hbm-agent"
    (home_real / "hbm-agent").mkdir()

    shim = home_link / ".local" / "bin" / "hbm"
    shim.parent.mkdir(parents=True)
    shim.write_text(
        f"#!/bin/bash\n"
        f"exec {lexical_checkout}/venv/bin/python "
        f'{lexical_checkout}/hbm "$@"\n',
        encoding="utf-8",
    )
...
def test_probe_skips_wrapper_with_escaping_python_shebang(
    ...
    broken_wrapper = xdg_home / ".local" / "bin" / "hbm"
...
def test_probe_accepts_shell_launcher_wrapper(tmp_path, xdg_home, monkeypatch):
    ...
    good_wrapper = xdg_home / ".local" / "bin" / "hbm"
...
def test_install_places_1024_png_in_256x256_not_scalable(
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "hbm.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "hbm.png"
    assert values["Icon"] == "hbm"
    assert dest.is_file()
    assert dest.read_bytes() == lde.icon_path(root).read_bytes()
...
def test_install_removes_stale_scalable_png(tmp_path, xdg_home, monkeypatch):
    """v2026.8.31 wrote the PNG into scalable/. A later hbm desktop
...
def test_install_exact_48_png_uses_48x48_dir(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "48x48" / "apps" / "hbm.png"
    assert dest.is_file()
    assert not (
        xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "hbm.png"
    ).exists()
...
def test_install_resizes_decodable_png_to_panel_sizes(
    ...
    dest_24 = xdg_home / "icons" / "hicolor" / "24x24" / "apps" / "hbm.png"
    dest_256 = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "hbm.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "hbm.png"
