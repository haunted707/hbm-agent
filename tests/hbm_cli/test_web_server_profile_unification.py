...
profile switcher can target any profile's HBM_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via HBM_HOME.
"""
import json
...
import hbm_cli.web_server_profiles as _web_server_profiles


@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_hbm_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_hbm_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import hbm_cli.mcp_config as mcp_config
    ...
    def test_mcp_test_resolves_profile_secret_source_scope(
        ...
        def _worker_sources(hbm_home):
            if Path(hbm_home).resolve() == worker_home.resolve():
    ...
    def test_mcp_list_expands_url_ref_from_profile_secret_scope(
        ...
        monkeypatch.setattr(
            ...
            lambda hbm_home: {"MCP_GH_URL": "http://worker/mcp"}
            if Path(hbm_home).resolve() == worker_home.resolve() else {},
        )
...
class TestProfileScopedModel:
    ...
    def _accept_any_model(self, monkeypatch):
        ...
        monkeypatch.setattr("hbm_cli.model_switch.switch_model", _switch)
    ...
    def test_model_options_uses_config_only_scope_for_selected_profile(
        ...
        monkeypatch.setattr(
            "hbm_cli.inventory.load_picker_context", lambda: object()
        )
        monkeypatch.setattr(
            "hbm_cli.inventory.build_model_options_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same HBM_HOME the rest
        ...
        monkeypatch.setattr(
            "hbm_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_hbm_home()))
            return None
        ...
        assert resp.json()["hbm_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_hbm_home(self, isolated_profiles, monkeypatch):
        import hbm_cli.web_server as web_server

        monkeypatch.setattr(
            "hbm_cli.main_tui_launch._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = _web_server_chat._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["HBM_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "HBM_TUI_GATEWAY_URL" not in env
