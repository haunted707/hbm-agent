"""Tests for the Kanban DB layer (hbm_cli.kanban_db)."""
...
import hbm_state
import hbm_state_wal
from hbm_cli import kanban_db as kb
from hbm_cli import kanban_db_connect as kbc
from hbm_cli import kanban_db_dispatch as kbd
from hbm_cli import kanban_db_workspace as kbw


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated HBM_HOME with an empty kanban DB."""
    home = tmp_path / ".hbm"
    home.mkdir()
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import hbm_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import hbm_cli.kanban_db as _kb
    from hbm_cli import kanban_db_dispatch as _kbd

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("HBM_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("HBM_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """HBM AGENT's own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` HBM AGENT's DB/metadata/logs on task completion.
    """
...
# spawned with `hbm -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's HBM_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's HBM_HOME."""

    def _set_home(self, monkeypatch, tmp_path, hbm_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HBM_HOME", str(hbm_home))
        monkeypatch.delenv("HBM_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.hbm/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.hbm/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.hbm/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # HBM_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile HBM_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # HBM_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is HBM_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["HBM_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["HBM_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["HBM_KANBAN_TASK"] == "t_dispatch_env"
        assert env["HBM_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "HBM_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see hbm_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import hbm_state_wal as _hbm_state_wal
    monkeypatch.setattr(
        _hbm_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _hbm_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    hbm_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("hbm_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="hbm_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        hbm_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "hbm_cli.kanban_db.sqlite3.connect",
...
def test_sqlite_connect_closes_tracked_conn_on_setup_failure(tmp_path, monkeypatch):
    ...
    from hbm_cli import sqlite_safe_read
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'hbm-agent kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from hbm_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_hbm_argv()
#
# Workers spawned by the dispatcher must use a `hbm` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["hbm", ...]`
...
# only falls back to the PATH shim when ``hbm_cli`` is not importable.
# ---------------------------------------------------------------------------


def test_resolve_hbm_argv_prefers_module_form_over_path_shim(monkeypatch):
    """A `hbm` on PATH must not shadow the running install (#111569):
    the module argv wins whenever ``hbm_cli`` is importable; only an
    explicit ``$HBM_BIN`` overrides it."""
    import shutil
    import sys
    from hbm_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("HBM_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/tmp/planted/hbm")
    monkeypatch.setattr(kbd, "_safe_which_no_cwd", lambda name: "/tmp/planted/hbm")
    assert kbd._resolve_hbm_argv() == [sys.executable, "-m", "hbm_cli.main"]

    monkeypatch.setenv("HBM_BIN", "/opt/hbm/bin/hbm")
    assert kbd._resolve_hbm_argv() == ["/opt/hbm/bin/hbm"]


def test_resolve_hbm_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m hbm_cli.main`.

    Pins the correct module name (NOT `hbm` — there is no top-level
    `hbm` package). Regression for #23198: the original PR shipped
    `python -m hbm` which fails with `No module named hbm` on every
    invocation.
    """
    import shutil
    import sys
    import hbm_cli.kanban_db as kb
    from hbm_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("HBM_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kbd._resolve_hbm_argv()
    assert argv == [sys.executable, "-m", "hbm_cli.main"]


def test_resolve_hbm_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `hbm_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m hbm_cli.main --version`
    ...
    assert "HBM AGENT" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="hbm_cli.kanban_db"):
        with kbc.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from hbm_cli.kanban_db_connect import connect
    from hbm_cli.sqlite_safe_read import file_length_matches_header
