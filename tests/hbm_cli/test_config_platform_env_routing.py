"""``hbm config set/get/unset`` route every name HBM AGENT registers as an environment variable to
...
def test_platform_env_key_round_trips_without_a_config_yaml_copy(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setenv("HBM_MANAGED_DIR", str(tmp_path / "managed"))
...
def test_unregistered_upper_snake_name_routes_to_env_by_shape(tmp_path, monkeypatch, capsys):
    """Any ``UPPER_SNAKE`` key is an environment setting even when HBM AGENT never registered it
    ...
    cfg.set_config_value("HBM_TIMEZONE", "Europe/Berlin")
    cfg.set_config_value("my_custom_flag", "hello")
    out = capsys.readouterr().out

    env_text = (tmp_path / ".env").read_text(encoding="utf-8")
    assert "TELEGRAM_GROUP_ALLOWED_USERS=222,333" in env_text and "HBM_TIMEZONE=Europe/Berlin" in env_text
    assert yaml.safe_load(config_path.read_text(encoding="utf-8")) == {
...
def test_env_writer_denylist_guards_upper_snake_names_and_unknown_names_get_a_note(
    ...
    with pytest.raises(SystemExit):
        cfg.set_config_value("HBM_YOLO_MODE", "true", force=True)
