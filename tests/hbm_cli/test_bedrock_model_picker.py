...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (HBM_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via HBM_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in HBM_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from hbm_cli.providers import HBM_OVERLAYS
        assert "bedrock" in HBM_OVERLAYS
