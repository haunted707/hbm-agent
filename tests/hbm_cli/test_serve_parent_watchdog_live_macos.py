...
calls ``os._exit(0)`` ~5ms after ``HBM_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            HBM_PARENT_PID=str(parent.pid),
            HBM_PARENT_START_MARKER=drifted_marker,
            HBM_PARENT_NONCE="nonce-95693",
            HBM_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("HBM_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "HBM_BACKEND_READY"), "backend never announced READY"
