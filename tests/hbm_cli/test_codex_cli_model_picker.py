...
Covers:
 - Normal path (tokens already in HBM AGENT auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — HBM AGENT
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `hbm auth openai-codex`. The old
...
@pytest.fixture()
def hbm_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in HBM AGENT auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(hbm_auth_only_env):
    """openai-codex appears when tokens are already in HBM AGENT auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or HBM AGENT
    auth store."""
    hbm_home = tmp_path / ".hbm"
    hbm_home.mkdir()
