...
from hbm_cli.auth import _auth_file_path, _load_auth_store
...
class FakePortal:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if path == "/api/oauth/token":
            ...
            self.token_grants += 1
            return httpx.Response(200, json={
                "access_token": _jwt(sub="nas_user:9", client_id="hbm-cli", account_tier="free"),
...
def portal(monkeypatch, tmp_path):
    fake = FakePortal()
    monkeypatch.setenv("HBM_PORTAL_BASE_URL", PORTAL)
    monkeypatch.setenv("HBM_ANON_API_SECRET", "test-secret")
...
def free_account(monkeypatch):
    ...
    from hbm_cli import models as m
...
class TestSignInCompletionSettlesTheModel:
    ...
    def test_no_eligible_recommendation_leaves_no_default_rather_than_a_model_the_account_may_not_use(
        ...
        assert "run `hbm model` to pick one" in out
