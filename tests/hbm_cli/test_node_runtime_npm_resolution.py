...
from hbm_cli.main_install_repair import _is_windows_npm_path, _resolve_node_runtime_npm
...
def test_windows_npm_path_refuses_windows_shims_but_not_native_data_mounts():
    ...
    assert not _is_windows_npm_path("/mnt/data/hbm/home/.local/bin/npm")
...
def test_resolve_node_runtime_npm_rescans_past_windows_drive_to_native_mount(monkeypatch):
    ...
    with (
        patch("hbm_constants.find_node_executable", return_value="/mnt/c/Program Files/nodejs/npm"),
        patch("hbm_cli.main_install_repair.shutil.which", side_effect=fake_which),
    ):
