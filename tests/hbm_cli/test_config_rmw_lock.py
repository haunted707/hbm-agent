...
@pytest.fixture
def client(monkeypatch, _isolate_hbm_home):
    ...
    monkeypatch.setattr("hbm_cli.model_cost_guard.expensive_model_warning", lambda *_a, **_k: None)
    cfg = load_config()
...
def _on_disk() -> dict:
    ...
    return yaml.safe_load((get_hbm_home() / "config.yaml").read_text(encoding="utf-8"))
