"""Tests for hbm_cli/completion.py — shell completion script generation."""
...
from hbm_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real hbm structure."""
    p = argparse.ArgumentParser(prog="hbm")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_hbm_completion()" in out
        assert "complete -F _hbm_completion hbm-agent" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_hbm_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        ...
        for line in lines:
            ...
            if in_profile_case and "_hbm_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _hbm_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__hbm_profiles" in out
        count = out.count("(__hbm_profiles)")
