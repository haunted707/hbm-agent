...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.hbm.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.hbm.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("hbm_constants.display_hbm_home", lambda: "~/.hbm-work")
    ...
    assert "~/.hbm-work/logs/launchd-reload.log" in out
