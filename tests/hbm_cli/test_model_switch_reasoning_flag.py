...
The parser is the single owner (hbm_cli.model_switch.parse_model_switch_args); the CLI and
...
def test_cli_commit_applies_effort_after_the_agent_swap(monkeypatch):
    ...
    cli._stage_and_swap_model = lambda result, old: cli_mod.HbmCLI._stage_and_swap_model(cli, result, old)
    monkeypatch.setattr(mixin, "_print_switch_summary", lambda *_a, **_k: None)
    monkeypatch.setattr(cli_mod.HbmCLI, "_persist_model_switch_to_session", lambda *_a: None)
    saved = {}
