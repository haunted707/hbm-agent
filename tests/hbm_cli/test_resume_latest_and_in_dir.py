...
`hbm --tui --resume latest --in ./dir` (and the classic-CLI equivalents)
...
def test_top_level_in_value_not_mistaken_for_subcommand(monkeypatch):
    # `hbm --in chat` — "chat" is the flag's value, not the subcommand.
    import sys

    import hbm_cli.main as mod

    monkeypatch.setattr(sys, "argv", ["hbm", "--in", "chat", "--resume", "latest"])
...
def test_in_dir_replaces_inherited_terminal_cwd(main_mod, monkeypatch, tmp_path):
    """A parent HBM AGENT surface, the shell or .env can export TERMINAL_CWD before
