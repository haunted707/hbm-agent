"""Tests for Bug #12905 fix — stale OAuth token detection in hbm model flow.

Bug 3: `hbm model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.hbm/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("hbm_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from hbm_cli.model_setup_flows import _model_flow_anthropic
        cfg = {}
