...
``hbm*.exe`` shims aside (``hbm.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``hbm`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("hbm", "hbm-agent", "hbm-acp", "hbm-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "hbm.exe" in names, "hbm.exe must be restored after a no-op install"
    assert "hbm-acp.exe" in names
    assert "hbm-gateway.exe" in names
    assert (scripts / "hbm.exe").read_bytes() == b"MZ-old-hbm"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "hbm.exe").read_bytes() == b"MZ-new-hbm"
...
# ---------------------------------------------------------------------------
# hbm_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "hbm.exe" in names, "hbm.exe must be restored after a no-op recovery install"
    assert (scripts / "hbm.exe").read_bytes() == b"MZ-old-hbm"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "hbm.exe").write_bytes(b"MZ-new-hbm")
