...
def _isolate(tmp_path, monkeypatch):
    """Isolate HBM_HOME so tests don't touch real config."""
    home = tmp_path / ".hbm"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal HbmCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("hbm_cli.banner.build_welcome_banner"):
        from cli import HbmCLI
        obj = HbmCLI.__new__(HbmCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["hbm-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below HBM AGENT's minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly HBM AGENT's minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above HBM AGENT's minimum context length."""
