...
from hbm_cli.tool_availability_notices import filter_to_enabled_toolsets, tool_availability_warning_lines
...
def test_generic_toolset_lists_vars_and_setup_hint():
    ...
    text = "\n".join(lines)
    assert "rl" in text and "TINKER_API_KEY" in text and "hbm setup" in text
...
class TestFilterToEnabledToolsets:
    _BUNDLE = {"hbm-cli": ["terminal", "process_manage", "web_search", "read_file"],
    ...
    def test_composite_bundle_keeps_terminal_notice_on_default_install(self):
        kept = filter_to_enabled_toolsets([_TERMINAL, _WEB, _RL], ["hbm-cli"], self._resolve)
    ...
    def test_resolver_error_is_not_fatal(self):
        ...
        assert filter_to_enabled_toolsets([_TERMINAL], ["hbm-cli"], boom) == []
