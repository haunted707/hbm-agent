...
1. The venv-repair import probes (early recovery + `hbm update`) must
...
2. ``hbm doctor`` must fail the certificate check in that state, and
   ``hbm doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `hbm update`'s venv repair must apply
...
# =========================================================================
# 2. hbm doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "hbm doctor --fix" in message
