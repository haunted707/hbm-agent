"""Tests for the Command Installation check in hbm doctor."""
...
import hbm_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal HBM_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".hbm"
    ...
    hbm_bin = venv_bin_dir / "hbm"
    hbm_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    hbm_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, hbm_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "hbm"
        wrong_target = tmp_path / "wrong_hbm"
        ...
        assert cmd_link.resolve() == hbm_bin.resolve()
