...
from hbm_cli.local_runtime.catalog import (
    CATALOG,
    catalog_by_id,
...
def test_catalog_and_preset_agree_on_identical_model_facts(tmp_path, monkeypatch):
    ...
    from hbm_cli.local_runtime import bootstrap, catalog, presets
    from hbm_cli.local_runtime.context_policy import RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from hbm_cli.local_runtime.estimator import ctx_bytes
    from hbm_cli.web_routers.local_models import _catalog_row

    monkeypatch.setenv("HBM_HOME", str(tmp_path))
    monkeypatch.setattr("hbm_cli.web_routers.local_models._engine_too_old", lambda tag: False)
...
def test_hybrid_long_context_stays_cheap():
    ...
    from hbm_cli.local_runtime.catalog import FLOOR
    from hbm_cli.local_runtime.estimator import ctx_bytes
