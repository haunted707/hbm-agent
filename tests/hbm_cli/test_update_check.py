"""Tests for the update check mechanism in hbm_cli.banner.
...
def git_repo(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "hbm-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    monkeypatch.setenv("HBM_HOME", str(tmp_path))
    monkeypatch.delenv("HBM_REVISION", raising=False)
    ...
    monkeypatch.setattr("hbm_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir
...
def test_passive_check_uses_the_api_and_never_fetches(git_repo, monkeypatch):
    ...
    tip.assert_called_once_with("nousresearch/hbm-agent", "main")
...
def test_cache_is_daily_but_invalidated_when_head_moves(git_repo, monkeypatch):
    """A fresh cache answers without any network; ``hbm update`` moving HEAD busts it at once;
