"""Regression tests for the OAuth dispatcher in hbm_cli.web_server.
...
HBM AGENT must not mint subscription OAuth tokens from an unattended HTTP
...
import hbm_cli.web_routers.oauth as _rt_oauth
import hbm_cli.web_server_oauth as _web_server_oauth
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "hbm_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "hbm_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "hbm_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_minimax_start_route_honors_poller_mock_on_owning_module(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "hbm_cli.auth._minimax_poll_token",
        fail_poll_token,
    ), patch(
        "hbm_cli.web_server_oauth._minimax_poller",
        fake_poller,
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from hbm_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_hbm_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "hbm auth add fake-oauth",
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "hbm auth add anthropic"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `hbm model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `hbm model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by HBM AGENT."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``hbm auth remove
    xai-oauth``.
    """
    from hbm_cli import auth as auth_mod
    from hbm_cli import web_server as ws
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to hbm_cli.auth.get_auth_status (the
    ...
    with patch("hbm_cli.auth.get_auth_status", return_value=fake_status):
