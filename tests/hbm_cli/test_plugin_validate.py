"""Tests for ``hbm plugins validate`` (hbm_cli/plugin_validate.py).
...
def test_requires_hbm_spec_is_validated(tmp_path):
    manifest = dict(BASE_MANIFEST, requires_hbm=">=0.21")
    d = _make_plugin(tmp_path, manifest=manifest)

    report = validate_plugin_dir(d)

    assert report.ok, report.failures
    assert ("requires_hbm", True, "spec '>=0.21' parses") in report.checks
...
class TestRequiresHbmSpec:
    """A typo'd ``requires_hbm`` clause must fail admission, not silently gate nothing."""

    def test_typoed_clause_fails_admission(self, tmp_path):
        d = _make_plugin(
            tmp_path, manifest={**BASE_MANIFEST, "requires_hbm": ">=0.21.1,<0.x"}
        )
        report = validate_plugin_dir(d)
        assert not report.ok
        assert any(
            "requires_hbm" in f and "does not parse" in f for f in report.failures
        ), report.failures
