...
Bug class: the checkout's ``.py`` files change (``hbm update``, manual
...
def _make_pycache(repo: Path, subdir: str = "hbm_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    hbm_main._sweep_stale_bytecode_if_checkout_changed()
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.hbm/plugins/<name>
# ---------------------------------------------------------------------------
