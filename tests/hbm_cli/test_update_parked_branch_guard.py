"""Regression tests for the parked-branch guard in ``hbm update``.
...
memory-limit``, days behind main) left there by earlier tooling. ``hbm
...
def _no_config(monkeypatch):
    ...
    import hbm_cli.config as hbm_config

    monkeypatch.setattr(hbm_config, "load_config", lambda: {})
...
def test_skip_warning_names_branch_behind_count_and_commands(repo_pair, capsys):
    ...
    assert f"git -C {repo_pair} checkout main && hbm update" in out
...
def test_print_update_completion_carries_branch_and_sha(
    ...
    monkeypatch.setattr(hbm_main, "PROJECT_ROOT", repo_pair)
...
def _patch_update_flow(monkeypatch, repo, run_real_git=True):
    ...
    """
    monkeypatch.setattr(hbm_main, "PROJECT_ROOT", repo)
    ...
    monkeypatch.setattr(hbm_main, "_record_bytecode_fingerprint", lambda *a, **k: None)
    ...
    monkeypatch.setattr(hbm_main, "_pause_windows_gateways_for_update", lambda: None)
    monkeypatch.setattr(
        hbm_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(hbm_main, "_capture_active_lazy_features", lambda: [])
    monkeypatch.setattr(hbm_main, "_capture_active_tool_dependencies", lambda: [])
