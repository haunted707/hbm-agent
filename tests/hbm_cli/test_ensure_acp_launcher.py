"""`hbm update` must self-heal the ``hbm-acp`` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) resolve the agent by the
``hbm-acp`` command name on the login-shell PATH. Fresh installs get the
launcher from ``scripts/install.sh``; existing installs get it from
``_ensure_acp_launcher()`` during ``hbm update``.
"""
...
from hbm_cli.update_cmd import _ensure_acp_launcher
...
def test_does_not_follow_symlink_into_venv(fake_home, tmp_path):
    """#21454 failure mode: never write through a symlinked hbm-acp."""
    (fake_home / "hbm-agent").write_text("#!/bin/sh\n", encoding="utf-8")
    console_script = tmp_path / "venv" / "bin" / "hbm-agent-acp"
    console_script.parent.mkdir(parents=True)
    marker = "#!/usr/bin/env python\n# real console script\n"
    console_script.write_text(marker, encoding="utf-8")
    (fake_home / "hbm-agent-acp").symlink_to(console_script)

    _ensure_acp_launcher()

    assert console_script.read_text(encoding="utf-8") == marker
    assert (fake_home / "hbm-agent-acp").is_symlink()
...
def test_unwritable_bin_dir_is_skipped(fake_home):
    ...
    if not hasattr(os, "geteuid"):
        ...
        _ensure_acp_launcher()
        assert not (fake_home / "hbm-agent-acp").exists()
        return
    if os.geteuid() == 0:
