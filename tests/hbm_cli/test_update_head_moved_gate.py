"""Tests for the post-pull HEAD-movement gate in ``hbm update``.
...
Before this guard ``hbm update`` printed "✓ Code updated!" and
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the hbm_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns hbm_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/hbm_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        hbm_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        hbm_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        hbm_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    hbm_main.cmd_update(args)  # completes normally (no SystemExit)
