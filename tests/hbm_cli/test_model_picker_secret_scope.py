...
class TestSwitchModelKeyEnvScope:
    ...
    def _run_switch(self, monkeypatch, user_cfg):
        ...
        monkeypatch.setattr(
            "hbm_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
        )
...
class TestPickerKeyEnvDotenv:
    ...
    that lives only in ``$HBM_HOME/.env`` authenticates the ``/model`` verification probe, and
    ...
    def test_switch_probe_uses_dotenv_key_over_stale_process_env(self, monkeypatch, tmp_path):
        ...
        import hbm_cli.model_switch as ms
        import hbm_cli.models_validate as mv
        ...
        monkeypatch.setattr("hbm_cli.runtime_provider.resolve_runtime_provider", _fake_runtime)
        monkeypatch.setattr(ms, "resolve_alias", lambda *a, **k: None)
