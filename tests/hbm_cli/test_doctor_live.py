"""Tests for ``hbm doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from hbm_cli import doctor_live
from hbm_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `hbm doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("hbm_cli.doctor.HBM_HOME", tmp_path / "home")
        monkeypatch.setattr("hbm_cli.doctor.PROJECT_ROOT", tmp_path / "root")
