...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (hbm_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from hbm_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "hbm" / "auth.json").read_text())
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((hbm_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("hbm_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from hbm_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from hbm_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `hbm auth add nous` must also populate providers.nous so the
    ...
    # raises "HBM AGENT is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`hbm auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``hbm auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``hbm auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("hbm_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from hbm_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from hbm_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """hbm auth add xai-oauth must set active_provider and write a pool entry.
    ...
    monkeypatch.setattr(
        "hbm_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``hbm auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from hbm_cli.auth_commands import auth_remove_command
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "hbm" / "auth.json").read_text(encoding="utf-8"))
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from hbm_cli.auth import clear_provider_auth
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`hbm-agent logout --provider openai-codex` must still clear model.provider.
    ...
    from hbm_cli.auth import logout_command
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from hbm_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source HBM AGENT reads from has a
...
def test_seed_from_singletons_respects_hbm_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hbm_pkce must not re-seed from ~/.hbm/.anthropic_oauth.json when suppressed."""
    hbm_home = tmp_path / "hbm"
    hbm_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("HBM_HOME", str(hbm_home))

    import yaml
    (hbm_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (hbm_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["hbm_pkce"]},
    }))

    # Stub the readers so only hbm_pkce is "available"; claude_code returns None
    import agent.anthropic_credentials as aa
    monkeypatch.setattr(aa, "read_hbm_oauth_credentials", lambda: {
    ...
    # hbm_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "hbm_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "hbm_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "hbm_cli.copilot_auth.get_copilot_api_token",
...
def test_qwen_oauth_login_marks_active_through_moved_owner(monkeypatch):
    ...
    import hbm_cli.auth_commands as auth_commands
    import hbm_cli.auth_qwen as auth_qwen
...
def test_auth_add_openrouter_oauth_persists_pkce_key_without_touching_api_key_default(tmp_path, monkeypatch):
    """`hbm auth add openrouter --type oauth` stores the PKCE-minted key as an ``api_key`` pool row
    ...
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("hbm_cli.auth._openrouter_pkce_login", lambda **kw: {"api_key": "sk-or-v1-from-pkce"})

    from hbm_cli.auth import resolve_provider
    from hbm_cli.auth_commands import auth_add_command
...
def test_openrouter_loopback_callback_binds_nonce_path_and_rejects_forged_redirect(monkeypatch):
    ...
    import hbm_cli.auth_openrouter as orm
