"""#103339 item 2: ``hbm doctor --fix`` never checkpoints state.db through a bare writable ``sqlite3.connect``
...
from hbm_cli.doctor_report import Finding
from hbm_cli.doctor_state import _state_db_wal
...
def test_doctor_checkpoint_runs_only_on_the_exclusive_repair_guard(tmp_path, monkeypatch):
    ...
    real_durable = hbm_state_repair._connect_repair_durable

    def _durable(path, **kwargs):
        guard_connects.append(Path(path))
        return real_durable(path, **kwargs)

    monkeypatch.setattr(hbm_state_repair, "_connect_repair_durable", _durable)
...
def test_session_count_reads_a_home_with_uri_reserved_characters(tmp_path):
    ...
    from hbm_cli.doctor_state import _session_count
