...
The bug class: on Windows, when `hbm.exe`/sibling shims could not be
...
def _make_shims(scripts_dir: Path, names=("hbm", "hbm-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # hbm.exe cannot be renamed; hbm-gateway.exe can
    real_rename = Path.rename

    def deny_hbm(self, target):
        if self.name == "hbm.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_hbm)
    monkeypatch.setattr(main_install_repair, "_hbm_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "hbm.exe" in exc_info.value.failed_shims
    # The successful rename (hbm-gateway.exe) was rolled back.
    assert (scripts / "hbm-gateway.exe").exists()
    assert not list(scripts.glob("hbm-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("hbm",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["hbm.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "hbm.exe" in out
