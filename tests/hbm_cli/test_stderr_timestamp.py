"""Tests for hbm_cli.stderr_timestamp."""
...
from hbm_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.hbm.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.hbm.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "hbm_cli.main", "gateway", "run", "--replace"]
