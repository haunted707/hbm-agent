"""Tests for _purge_stale_hbm_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `hbm update`.

Field failure (2026-08-20, Teknium's Linux box): `hbm update` pulled a
checkout where hbm_cli/gateway.py newly imports `line_input` from
hbm_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from hbm_cli.gateway
...
def _restore_sys_modules():
    ...
    The purge under test evicts real HBM AGENT modules from the cache; later
...
def test_purge_evicts_hbm_prefixed_modules():
    victims = [
        "hbm_cli.cli_output",
        "hbm_cli.gateway",
        "gateway.status",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_hbm_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_hbm_modules()
    assert sys.modules.get("hbm_cli.update_cmd") is update_cmd
    assert sys.modules.get("hbm_cli.main") is cli_main
    assert "hbm_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import hbm_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_hbm_modules()
        post_purge_receipt = importlib.import_module("hbm_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["hbm_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_hbm_modules()
    finally:
        sys.modules.pop("hbm_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from hbm_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from hbm_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_hbm_modules()
    from hbm_cli.update_inventory import RuntimeRecord as after
    assert after is before


def test_stale_top_level_utils_scenario_end_to_end():
    """The 2026-09-12 field failure: `hbm update` from a pre-`base_url_origin`
    checkout kept the old top-level `utils` cached, and the restart phase's import of
    `hbm_cli.gateway` died on `from utils import base_url_origin`."""
...
def test_purge_protects_hbm_logging():
    # A second copy of hbm_logging starts a second QueueListener over the same log
    # files while the first keeps running: its listener/handler state is module-global.
    real = sys.modules.get("hbm_logging")
    sentinel = _fake_module("hbm_logging")
    sys.modules["hbm_logging"] = sentinel
    try:
        cli_main._purge_stale_hbm_modules()
        assert sys.modules.get("hbm_logging") is sentinel
    finally:
        sys.modules.pop("hbm_logging", None)
        if real is not None:
            sys.modules["hbm_logging"] = real
