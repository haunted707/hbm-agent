...
managers acquire the same path through ``hbm_state_registry`` from the REPL thread a
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = hbm_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(hbm_state_registry, "_open_session_db", recording_open)

    cli = SimpleNamespace()
    try:
        HbmCLI._init_session_store(cli)
