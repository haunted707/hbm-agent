"""User-facing message contracts for `hbm skills/plugins/mcp/cron/doctor` failure paths.

Every failure line must say what happened and name an existing command to type next; none may
lead with a raw exception, a status literal, or a hardcoded ``~/.hbm`` path (UX message audit,
...
def test_skill_fetch_failure_names_search_and_doctor():
    from hbm_cli.skills_hub import _print_fetch_failure
    c, buf = _console()
...
def test_scan_block_message_explains_force_and_points_at_inspect():
    from hbm_cli.skills_hub import _scan_block_message
    ...
    assert "hbm skills inspect acme/evil-skill" in msg
...
def test_plugin_clone_failure_leads_with_next_steps_and_escapes_git_output():
    from hbm_cli.plugins_cmd import _clone_failure_message
    ...
    assert "hbm plugins search" in msg and "gh auth login" in msg
...
def test_unknown_plugin_message_points_at_list_and_full_key_rule():
    from hbm_cli.plugins_cmd import _unknown_plugin_message
    msg = _unknown_plugin_message("firecrawl")
    assert "No plugin named 'firecrawl'" in msg
    assert "hbm plugins list" in msg and "web/firecrawl" in msg and "hbm plugins install" in msg
...
def test_banner_failed_mcp_line_shows_reason_and_command():
    from hbm_cli.banner import _mcp_server_line
    ...
    assert "hbm mcp login linear" in auth


def test_mcp_probe_failure_unwraps_group_and_points_at_login_for_401():
    from hbm_cli.mcp_config import _probe_failure_next_step, _probe_failure_reason
    ...
    assert "hbm mcp login linear" in _probe_failure_next_step("linear", group)

    missing = FileNotFoundError(2, "No such file or directory", "npx")
    assert "config.yaml" in _probe_failure_next_step("gh", missing)
    assert "~/.hbm" not in _probe_failure_next_step("gh", missing) or "profiles" in _probe_failure_next_step("gh", missing)


# ── cron ──────────────────────────────────────────────────────────────────────────────────────

def test_cron_delivery_failure_lines_have_no_status_literals_and_name_the_fix():
    from hbm_cli.cron import _cron_doctor_issues_for_job, _job_warnings, _last_run_display
    ...
    assert "hbm cron edit job42 --deliver" in joined
    assert "hbm gateway restart" in joined and "hbm cron run job42" in joined
    assert "gateway was unreachable" in joined


def test_cron_queued_status_reads_as_in_progress():
    from hbm_cli.cron import _last_run_display
    assert "in progress" in _last_run_display({"last_status": "delivery_queued"})


# ── doctor / advisories / paths ───────────────────────────────────────────────────────────────

def test_advisory_remediation_uses_active_hbm_home(monkeypatch, tmp_path):
    from hbm_cli import security_advisories as sa
    monkeypatch.setattr(sa, "display_hbm_home", lambda: "~/.hbm/profiles/work")
    hit = SimpleNamespace(advisory=sa.ADVISORIES[0], package="mistralai", installed_version="2.4.6")
    text = "\n".join(sa.full_remediation_text(hit))
    assert "~/.hbm/profiles/work/.env" in text
    assert "{hbm_home}" not in text
    assert not re.search(r"~/\.hbm/\.env", text)


def test_doctor_ack_failure_names_config_path_and_retry(monkeypatch, capsys):
    import hbm_cli.doctor as doctor
    monkeypatch.setattr("hbm_cli.security_advisories.ack_advisory", lambda _id: False)
    monkeypatch.setattr(doctor, "_DHH", "~/.hbm/profiles/work")
    from hbm_cli.security_advisories import ADVISORIES
    try:
...
def test_backup_recovery_hint_cites_the_slash_command_not_a_fake_subcommand():
    from hbm_cli.backup import _snapshot_recovery_hint
    hint = _snapshot_recovery_hint()
    assert "/snapshot list" in hint and "/snapshot restore" in hint
    assert "hbm-agent snapshot" not in hint  # `hbm-agent snapshot` is not a CLI subcommand
