...
from hbm_cli import cli_commands_mixin as commands
...
def test_the_in_chat_and_terminal_completion_use_their_own_copy():
    ...
    assert "run `hbm model` to pick one" in anon_auth.drain_sign_in_copy(iter([state]), chat=False)


def test_the_command_resolves_through_the_cli_fallback():
    from cli import HbmCLI
    assert HbmCLI._slash_handler("login") == ("_handle_login_command", True)
...
def test_upgrade_guest_keeps_the_terminal_timeout(monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.auth_device_flow._is_remote_session", lambda: True)
