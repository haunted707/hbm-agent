"""Venv ownership preflight for ``hbm update`` (#83529).

A venv touched by ``sudo pip`` / ``sudo hbm`` contains root-owned files
(e.g. ``site-packages/hbm_agent-*.dist-info/INSTALLER``). A later normal
``hbm update`` then dies mid-mutation inside ``uv pip install -e .``
("Permission denied (os error 13)") with ``venv/bin/hbm`` already deleted,
...
def _make_fake_venv(tmp_path):
    ...
    (venv / "bin" / "hbm").write_text("#!stub\n")
    (venv / "bin" / "python").write_text("#!stub\n")
    sp = venv / "lib" / "python3.12" / "site-packages"
    dist_info = sp / "hbm_agent-1.0.0.dist-info"
...
def test_foreign_owned_dist_info_child_detected(tmp_path, monkeypatch):
    ...
    installer = str(
        ...
        / "hbm_agent-1.0.0.dist-info" / "INSTALLER"
    )
...
def test_foreign_owned_refuses_with_chown_hint(tmp_path, monkeypatch, capsys):
    venv = _make_fake_venv(tmp_path)
    hbm_bin = str(venv / "bin" / "hbm")
    real_uid = update_cmd._path_uid
    monkeypatch.setattr(
        update_cmd,
        "_path_uid",
        lambda p: 0 if str(p) == hbm_bin else real_uid(p),
    )
    monkeypatch.setattr(
        update_cmd_deps,
        "_path_uid",
