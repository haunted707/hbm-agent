"""Invariant tests for the plain-language startup messages of the ``hbm`` CLI.

Contracts (not snapshots): a mistyped subcommand names the typo, offers the closest
match and ``hbm --help`` without dumping the 70-entry choice list; a rejected
profile name explains the rule; missing optional dependencies point at ``hbm update``.
"""
...
def test_unknown_subcommand_names_typo_suggests_closest_and_hides_choice_list():
    ...
    assert "'sesions' is not a `hbm` command" in text
    assert "Did you mean: sessions" in text
    assert "hbm --help" in text
...
def test_unknown_subcommand_without_close_match_still_points_at_help():
    ...
    assert "'zzqx' is not a `hbm` command" in text
...
def test_nested_group_typo_names_the_group_and_suggests():
    ...
    assert "'stat' is not a `hbm gateway` command" in text
...
def test_non_choice_errors_keep_argparse_usage_line():
    ...
    assert text.startswith("usage: hbm")
...
def test_invalid_profile_flag_value_explains_rule_and_exits(monkeypatch):
    from hbm_cli import main as _main

    monkeypatch.setattr(sys, "argv", ["hbm", "-p", "Work Bot", "status"])
    err = io.StringIO()
    with redirect_stderr(err), pytest.raises(SystemExit) as exc:
...
def test_option_looking_dash_p_value_is_a_silent_skip_even_under_hbm(monkeypatch):
    from hbm_cli import main as _main

    # `-p no:xdist` reaching us through a differently named runner (tox, nox, python -m) must not exit.
    monkeypatch.setattr(sys, "argv", ["hbm", "-p", "no:xdist", "tests/"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
    monkeypatch.setattr(sys, "argv", ["hbm", "-p", "--flag"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_invalid_dash_p_after_a_subcommand_is_left_to_that_subcommand(monkeypatch):
    ...
    # A plugin/subcommand flag such as `hbm kanban serve -p "Work Bot"` is not our profile selector.
    monkeypatch.setattr(sys, "argv", ["hbm", "kanban", "serve", "-p", "Work Bot"])
    assert _main._scan_profile_flag(sys.argv[1:]) == (None, 0, None)
...
def test_bare_continue_with_no_session_names_the_next_step(monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_cli.terminal_breadcrumbs.resolve_breadcrumb_session", lambda: None, raising=False
    )
    out = io.StringIO()
...
def test_model_picker_does_not_warn_about_missing_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("hbm_cli.auth.resolve_provider", _raise)
...
def test_missing_dashboard_deps_point_at_hbm_update():
    ...
    assert "hbm update" in text
    assert "-m pip install -e '.[all]'" in text
    assert "metadata" not in text


def test_missing_node_message_names_cli_fallback(monkeypatch, capsys):
    from hbm_cli import main_tui_launch as _tui

    monkeypatch.setattr("hbm_constants.find_node_executable", lambda _b: None)
    monkeypatch.setattr("hbm_cli.dep_ensure.ensure_dependency", lambda _n: False, raising=False)
    monkeypatch.delenv("HBM_NODE", raising=False)
    with pytest.raises(SystemExit):
        _tui._tui_node_bin("node")
    text = capsys.readouterr().out
    assert "hbm --cli" in text
