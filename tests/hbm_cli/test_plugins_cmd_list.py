...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for HBM AGENT"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.hbm.cli_plugin",
    ...
    monkeypatch.setattr(
        "hbm_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
        importlib.metadata,
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for HBM AGENT",
            "entrypoint",
            "adapters.hbm.cli_plugin",
            "wiki",
        )
    ]
...
def test_declared_capabilities_for_entrypoint_uses_distribution_metadata(
    ...
    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr(
        "hbm_cli.plugins.get_bundled_plugins_dir", lambda: bundled_dir
    )
    monkeypatch.setattr(
        importlib.metadata,
        "entry_points",
...
def test_unreadable_plugin_dir_is_skipped_by_every_manifest_scan(monkeypatch, tmp_path, caplog):
    ...
    from hbm_cli.plugins_discovery import scan_directory
    ...
    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr("hbm_cli.plugins.get_bundled_plugins_dir", lambda: bundled_dir)
