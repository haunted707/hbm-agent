...
from hbm_cli import managed_uv as uv
from hbm_cli import update_receipt as receipts
...
def test_runtime_repair_outcome_reaches_persisted_receipt(tmp_path, monkeypatch, status, ok, bucket):
    ...
    # Outside `hbm update` (setup/bootstrap) there is no receipt; the hook must stay a no-op.
