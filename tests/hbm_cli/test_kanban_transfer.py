"""Tests for kanban board export / import (``hbm_cli.kanban_transfer``).
...
from hbm_cli import kanban_transfer as kt
from hbm_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("HBM_KANBAN_HOME", str(root))
        for var in ("HBM_KANBAN_DB", "HBM_KANBAN_WORKSPACES_ROOT",
                    "HBM_KANBAN_ATTACHMENTS_ROOT", "HBM_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this HBM AGENT"):
