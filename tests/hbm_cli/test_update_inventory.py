"""Tests for hbm_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("hbm_cli.profiles._get_default_hbm_home", lambda: default_home)
    monkeypatch.setattr("hbm_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("hbm_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    ...
    monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: {
        100: "hbm gateway run", 200: "hbm --profile work gateway run"}.get(pid))
    monkeypatch.setattr("hbm_cli.gateway._get_service_pids", lambda all_profiles=False: {100})
    monkeypatch.setattr("hbm_cli.gateway.supports_systemd_services", lambda: True)
    monkeypatch.setattr("hbm_cli.gateway.find_profile_gateway_processes", lambda exclude_pids=None: [])
    monkeypatch.setattr(
        "hbm_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": "a" * 40, "short_sha": "a" * 8, "version": "1.0", "source": "git"},
    )
    monkeypatch.setattr("hbm_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("hbm_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        from hbm_cli.update_inventory import describe_restart_mechanism

        assert "hbm -p work gateway restart" in describe_restart_mechanism(
            by_profile["work"].restart_via, "work"
        )

    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        monkeypatch.setattr("hbm_cli.config.detect_install_method", lambda *a, **k: "docker")
        monkeypatch.setattr(
            "hbm_cli.config.recommended_update_command_for_method",
            lambda m: "docker pull nousresearch/hbm-agent:latest",
        )
        plan = ui.collect_runtime_inventory()
    ...
    def test_stopped_record_with_recycled_pid_is_not_a_runtime(self, fleet, monkeypatch):
        ...
        gateway the restart phase can never touch (that phantom made `hbm update` exit partial)."""
        work_home = fleet / "home" / "profiles" / "work"
        _write_state(work_home, 200, gateway_state="stopped")
        monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: {
            100: "hbm gateway run", 200: "C:/Windows/system32/dllhost.exe /Processid:{X}"}.get(pid))
        plan = ui.collect_runtime_inventory()
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from hbm_cli.gateway import ProfileGatewayProcess
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "hbm_cli.config.detect_install_method",
            "hbm_cli.build_info.get_code_identity",
            "hbm_cli.profiles._get_default_hbm_home",
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        ...
        home.mkdir()
        monkeypatch.setattr("hbm_cli.config.get_hbm_home", lambda: home, raising=False)
        ur._current = None
