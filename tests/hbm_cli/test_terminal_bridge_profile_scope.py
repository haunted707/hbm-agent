...
process profile, never a per-task ``set_hbm_home_override``.
...
a secondary profile (via ``set_hbm_home_override``) must NOT cause that
...
``get_process_hbm_home()`` so the comparison reflects the launch scope,
...
def test_process_hbm_home_ignores_task_override(tmp_path, monkeypatch):
    ...
    token = set_hbm_home_override(str(other_home))
    try:
        assert (
            env_loader._process_hbm_home().resolve() == launch_home.resolve()
