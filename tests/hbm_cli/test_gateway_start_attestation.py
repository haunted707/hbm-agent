...
def _install_pid_sequence(monkeypatch, snapshots):
    ...
    monkeypatch.setattr("hbm_cli.gateway.find_gateway_pids", _fake)
    return calls
...
def test_report_gateway_start_failure_is_loud_not_checkmark(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "hbm_cli.config.get_hbm_home", lambda: str(tmp_path)
    )
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Hbm_Gateway_x")

    gateway_windows._report_gateway_start("direct spawn (PID 7)")
    out = capsys.readouterr().out
    assert "✓" not in out
    assert "FAILED" in out
    assert "schtasks /Run /TN Hbm_Gateway_x" in out
...
def test_attestation_warning_includes_schtasks_recovery(monkeypatch, attest_home):
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(
        gateway_windows, "get_task_name", lambda: "Hbm_Gateway_arthur_tutor"
    )
    gateway_windows._write_start_attestation([888], "direct spawn (PID 888)")
    warning = gateway_windows.check_start_attestation(current_pids=[])
    assert "schtasks /Run /TN Hbm_Gateway_arthur_tutor" in warning
...
def test_breakaway_fallback_warns_even_on_success(monkeypatch, attest_home, capsys):
    ...
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Hbm_Gateway")
    ...
    assert "schtasks /Run /TN Hbm_Gateway" in out
...
def test_attested_probe_fails_closed_without_a_well_formed_dead_attestation(attest_home):
    """``hbm update`` uses this probe to override Desktop-owned lifecycle suppression
...
def test_attestation_bound_to_create_time_is_no_authority_once_the_sentinel_moved_on(monkeypatch, attest_home):
    ...
    monkeypatch.setattr("hbm_cli.process_identity._process_create_time", lambda pid=None: 1000.0)
