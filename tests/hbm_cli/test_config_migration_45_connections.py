...
``hbm tools`` persists an explicit per-platform toolset list, and absence from
that list reads as "unchecked" — so a toolset that ships after the list was saved
stays off for picker users while composite (``[hbm-cli]``) users inherit it.
...
class TestConnectionsToolsetMigration:
    ...
    def test_global_disable_is_not_overridden_or_claimed(self, tmp_path, disabled):
        """Blank Slate and `hbm tools --disable` write agent.disabled_toolsets, which the resolver
