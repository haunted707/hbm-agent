...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "hbm_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # HBM_HOME.  For a standard install (HBM_HOME unset) that root is
        # the platform-native default (~/.hbm), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from hbm_constants import get_default_hbm_root
        assert env.get("HBM_HOME") == str(get_default_hbm_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (HBM_DESKTOP=1) must NOT
