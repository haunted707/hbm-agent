...
def hbm_home(tmp_path, monkeypatch):
    ...
    # Clear any cached hbm_home computation
    import hbm_constants
    if hasattr(hbm_constants, "_hbm_home_cache"):
        hbm_constants._hbm_home_cache = None
    return home
...
def test_save_conversation_writes_under_hbm_home(hbm_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.hbm/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the HBM_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "hbm_constants"]:
    ...
    cli.HbmCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("hbm_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.hbm/sessions/saved/
    saved_dir = hbm_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("hbm_conversation_*.json"))
    ...
    assert "hbm --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(hbm_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "hbm_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(hbm_home, capsys):
    ...
    cli.HbmCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(hbm_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "hbm_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.HbmCLI.save_conversation(stub, "/save pdf")
