...
Canonical owners:
  ...
  * ``hbm_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._hbm_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/hbm/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "hbm_cli/managed_scope.py",
