...
from hbm_cli import gateway as hbm_gateway
...
from hbm_cli import process_identity
...
def test_control_plane_argv_is_not_a_gateway():
    ...
    serve = "C:\\HBM AGENT\\.venv\\Scripts\\python.exe -m hbm_cli.main serve --host 127.0.0.1"
    run = "C:\\HBM AGENT\\.venv\\Scripts\\python.exe -m hbm_cli.main gateway run"
...
def test_control_plane_classifier_is_token_based_not_substring():
    ...
    py = "C:\\HBM AGENT\\.venv\\Scripts\\python.exe -m hbm_cli.main"
...
def test_pause_skips_cold_start_plan_when_desktop_owns_lifecycle(monkeypatch):
    ...
    monkeypatch.setattr(hbm_gateway, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
        hbm_gateway, "find_windows_gateway_services", lambda **_k: []
    )
...
def test_attested_dead_gateway_survives_desktop_ownership_and_marker_is_consumed_on_spawn(
    ...
    monkeypatch.setattr("hbm_cli.config.get_hbm_home", lambda: str(tmp_path))
    monkeypatch.setattr(cli_main, "_is_windows", lambda: True)
    monkeypatch.setattr(main_install_repair, "_is_windows", lambda: True)
    monkeypatch.setattr(hbm_gateway, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(hbm_gateway, "find_windows_gateway_services", lambda **_k: [])
    monkeypatch.setattr(gateway_windows, "is_installed", lambda: True)
    monkeypatch.setattr(update_cmd, "_desktop_owns_gateway_lifecycle", lambda: True)
    monkeypatch.setattr(update_cmd_windows, "_desktop_owns_gateway_lifecycle", lambda: True)
...
def test_cold_start_is_authorized_by_the_token_generation_not_the_mutable_marker(
    ...
    """#110020 review (a): the marker is a one-shot that a concurrent ``hbm gateway status``
    consumes between plan and execution. The spawn must still happen (authority lives on the
    token), and a *newer* marker written by a concurrent ``hbm gateway start`` must not be
    ...
    # Concurrent ``hbm gateway status`` consumed the marker...
    assert gateway_windows.check_start_attestation(current_pids=[]) is not None
    assert not marker.exists()
    # ...and a concurrent ``hbm gateway start`` wrote a fresh one for its own PID.
...
def _running_beta_pause_fixture(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(hbm_gateway, "launch_detached_profile_gateway_restart", lambda p, o: True)
    ready_probes: list = []
...
def test_service_supervised_running_profile_is_not_cold_started(monkeypatch, tmp_path):
    ...
    service = SimpleNamespace(name="HbmGw-beta", profile="beta", service_pid=800, gateway_pid=900,
    ...
    assert token["services"] == ["HbmGw-beta"]
    assert token["profiles"] == {}
    assert "cold_start_profiles" not in token
