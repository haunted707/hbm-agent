...
- Probe: ``named_profile_served_by_running_multiplexer`` reads the pid-verified default
  ``gateway_state.json`` first (env-only opt-in on the default profile is invisible to ``hbm -p X``);
...
- ``hbm status`` / ``cron status`` on a satellite say running-via-multiplexer; the default lists served.
"""
...
def served_root(tmp_path, monkeypatch):
    ...
    (root / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "hbm_home": str(root)}))
    (root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "hbm_home": str(root), "gateway_state": "running",
         "served_profiles": ["default", "coder"]}))
    monkeypatch.setenv("HBM_HOME", str(root / "profiles" / "coder"))
    monkeypatch.delenv("GATEWAY_MULTIPLEX_PROFILES", raising=False)
    import hbm_constants
    import gateway.status as status
    monkeypatch.setattr(hbm_constants, "_default_hbm_root_memo", None)
    ...
    monkeypatch.setattr(status, "_read_process_cmdline", lambda pid: (
        "python -m hbm_cli.main gateway run" if pid == os.getpid() else real_cmdline(pid)))
    return root
...
def test_probe_falls_back_to_config_only_without_recorded_key(served_root):
    ...
    (served_root / "gateway_state.json").write_text(json.dumps(
        {"pid": os.getpid(), "hbm_home": str(served_root), "gateway_state": "running"}))
...
def test_probe_survives_a_missing_default_pid_file(served_root):
    ...
    (``hbm -p X status``, ``cron list``, the dashboard ladder) say "not running" about the gateway
...
def test_setup_gateway_service_step_skips_install_for_served_profile(served_root, monkeypatch, capsys):
    """``hbm -p <profile> setup gateway`` (and ``hbm setup`` / ``hbm import``) reach the service
    ...
    monkeypatch.setenv("HBM_HOME", str(served_root / "profiles" / "other"))  # not in the live record
    assert gw.ensure_gateway_service(context="setup") is True
...
def test_recycled_pid_does_not_lend_a_stale_record_its_served_profiles(served_root):
    ...
    once did, so `hbm -p coder gateway start` exited 78 for a multiplexer that was long gone."""
    import subprocess
    import gateway.status as status
    from hbm_cli.gateway import named_profile_served_by_running_multiplexer
    ...
    try:
        ...
        for name in ("gateway.pid", "gateway_state.json"):
            (served_root / name).write_text(json.dumps({
                "pid": child.pid, "hbm_home": str(served_root), "gateway_state": "running",
...
def test_status_surfaces_agree_for_a_satellite_profile(served_root, monkeypatch):
    import hbm_cli.gateway as gw
    import hbm_cli.status as st
    import hbm_cli.cron as cr
...
def test_dashboard_liveness_ladder_reports_served_profile_running(served_root):
    ...
    "stopped" while `hbm -p X status` said running. Alpha's `<X>:<platform>` entries project as its own."""
...
def test_dashboard_lifecycle_verbs_target_the_multiplexer(served_root, monkeypatch):
    ...
    from hbm_cli.web_server_gateway import _gateway_subcommand, _profile_action_environment, multiplexed_profile_refusal
    monkeypatch.setattr(profiles_mod, "_check_gateway_running", lambda home: False)
    # This process's own HBM_HOME is coder's; the restart child must still run under the DEFAULT
    ...
    assert _profile_action_environment(restart)["HBM_HOME"] == str(served_root)
