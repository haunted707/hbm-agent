"""Tests for hbm_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import hbm_cli.gui_uninstall as gu


def _make_agent(hbm_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = hbm_home / "hbm-agent"
    (agent_root / "hbm_cli").mkdir(parents=True)
    (agent_root / "hbm_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(hbm_home: Path) -> None:
    """Create the source-built GUI artifacts a `hbm desktop` run produces."""
    desktop = hbm_home / "hbm-agent" / "apps" / "desktop"
    ...
    (hbm_home / "hbm-agent" / "node_modules").mkdir(parents=True)
    (hbm_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(hbm_home: Path) -> None:
    (hbm_home / "config.yaml").write_text("x: 1\n")
    (hbm_home / ".env").write_text("KEY=secret\n")
    (hbm_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(hbm_home)
    _make_gui_build(hbm_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(hbm_home)
    ...
    assert summary["hbm_home"] == str(hbm_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `hbm desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    hbm_home = tmp_path / ".hbm"
    _make_agent(hbm_home)
    icon = lde.icon_path(hbm_home / "hbm-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(hbm_home)
    ...
    assert lde.icon_path(hbm_home / "hbm-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (hbm_home / "hbm-agent" / "hbm_cli").is_dir()
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import hbm_cli.uninstall as uninstall
