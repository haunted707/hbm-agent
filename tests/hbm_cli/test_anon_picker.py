...
against a temp ``HBM_HOME`` with the network catalog fetch stubbed.
"""
...
from hbm_cli.auth import _save_auth_store
...
def guest_home(monkeypatch, tmp_path):
    ...
    from hbm_cli import model_switch_providers as msp
    ...
    return Path(os.environ["HBM_HOME"])
...
def _cli_nous_rows(config):
    from hbm_cli.main_provider_setup import _build_provider_picker_rows
...
def test_guest_identity_shows_free_tier_row_with_only_welcome_model(guest_home, monkeypatch):
    ...
    # The `hbm model` provider picker applies the same rule from the same helper.
