"""Tests for the stale-dashboard handling run at the end of ``hbm update``.

``hbm update`` detects ``hbm dashboard`` processes left over from the
...
from hbm_cli.main_dashboard import _find_stale_dashboard_pids
from hbm_cli.dashboard_procs import _kill_stale_dashboard_processes
from hbm_cli import dashboard_procs
from hbm_cli import main_dashboard
from hbm_cli import update_cmd
from hbm_cli.update_cmd import _finish_dashboard_update_cleanup
from hbm_cli.main_dashboard import _restart_managed_dashboard_service
from hbm_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "hbmPath": "/opt/hbm/bin/hbm",
        "hbmHome": str(tmp_path),
    ...
    monkeypatch.delenv("HBM_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "hbm_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m hbm_cli.main dashboard"),
                    _ps_line(12345, "hbm dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("hbm_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "hbm-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="hbm-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "hbm-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "hbm-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "hbm-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("hbm_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "hbm-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "hbm-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "hbm-dashboard.service"],
            ["systemctl", "--user", "restart", "hbm-dashboard.service"],
        ]
        ...
        assert "✓ restarted hbm-dashboard.service" in capsys.readouterr().out
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("hbm_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("hbm_cli._subprocess_compat.pid_is_hbm", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "hbm_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `hbm update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("hbm_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m hbm_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("hbm_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/hbm-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="hbm-serve.service"), \
        ...
        restart.assert_called_once_with(
            "hbm-serve.service", "/system.slice/hbm-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service hbm-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: hbm update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"hbm-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("hbm_cli.dashboard_procs._hbm_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["hbm", "dashboard", "--port", "8300"],
                ["hbm", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["hbm", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["hbm", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["hbm", "serve"]])

        assert failed == [["hbm", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from hbm_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.hbm/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["hbm", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from hbm_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "hbm_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "hbm_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["hbm", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["hbm", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["hbm", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_hbm_home(self):
        from hbm_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/hbm-home-a"
        a = ["hbm", "dashboard", "--port", "8300"]
        b = ["hbm", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["hbm", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["hbm", "dashboard", "--port", "8301"]
        home = "/home/u/.hbm"
        out = _filter_dashboard_respawn_candidates(
            [
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another HBM_HOME is never respawned (#94030).
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.hbm-sidecar/hbm-agent/venv/bin/python",
            "-m", "hbm_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.hbm-lifeos")],
            own_home="/Users/u/.hbm",
        )
        assert out == []

    def test_matching_hbm_home_is_kept(self):
        from hbm_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["hbm", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.hbm")],
            own_home="/Users/u/.hbm",
        )
        assert out == [argv]

    def test_symlinked_hbm_home_compares_equal(self, tmp_path):
        from hbm_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable HBM_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from hbm_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_hbm_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            hbm_constants, "get_hbm_home", lambda: Path("/home/u/.hbm")
        )
        argv = ["hbm", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.hbm-lifeos"),
        ])
        assert foreign == []
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["hbm", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00hbm_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "hbm_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``hbm update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``hbm_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from hbm_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("hbm_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import hbm_cli._subprocess_compat as compat
        from hbm_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["hbm_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["hbm_cli._subprocess_compat"])
            importlib.reload(sys.modules["hbm_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "hbm_cli._subprocess_compat" in reloaded
        assert "hbm_cli.dashboard_procs" in reloaded
...
class TestLaunchdSupervisedBackends:
    ...
    ARGV = [
        "/opt/hbm/venv/bin/python", "-m", "hbm_cli.main",
    ...
    def _run(self, pid, jobs, *, restart_ok=True, restart_managed=True):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("hbm_cli.dashboard_procs._process_ancestors", return_value=[]), \
             patch("hbm_cli.dashboard_procs._hbm_home_for_pid", return_value=None), \
             patch.object(live, "_respawn_dashboard_processes", return_value=[]) as respawn, \
    ...
    def test_launchd_owned_backend_restarts_through_launchd_never_as_a_detached_respawn(self, capsys):
        ...
        job = ("system", "ai.hbm.dashboard", list(self.ARGV), None)
        result, restart, respawn = self._run(9102, [job])
        respawn.assert_not_called()
        restart.assert_called_once_with("system", "ai.hbm.dashboard", None)
        assert result["killed"] == [9102] and result["unrecovered"] == []
        out = capsys.readouterr().out
        assert "✓ restarted launchd job system/ai.hbm.dashboard" in out
        assert "when you're ready" not in out

        result, restart, respawn = self._run(9103, [("gui/501", "ai.hbm.dashboard", list(self.ARGV), 9103)],
                                             restart_ok=False)
        respawn.assert_not_called()
        assert result["unrecovered"] == [9103]
        assert "run: launchctl kickstart -k gui/501/ai.hbm.dashboard" in capsys.readouterr().out

        # A LaunchDaemon lives in the system domain: kickstart needs root, so a non-root hint says sudo.
        with patch("hbm_cli.dashboard_procs.os.geteuid", return_value=501, create=True):  # windows-footgun: ok — patch target string, create=True
            self._run(9105, [("system", "ai.hbm.serve", list(self.ARGV), None)], restart_ok=False)
        assert "run: sudo launchctl kickstart -k system/ai.hbm.serve" in capsys.readouterr().out

        other = ("gui/501", "ai.hbm.other", ["hbm", "dashboard", "--port", "8300"], 777)
    ...
    def test_loaded_backend_jobs_scans_plists_and_attributes_pids_host_independently(
        ...
        agents.mkdir()
        daemons.mkdir()
        backend_argv = ["/opt/hbm/venv/bin/python", "-m", "hbm_cli.main", "dashboard", "--port", "9119"]
        serve_argv = ["/opt/hbm/venv/bin/python", "-m", "hbm_cli.main", "serve", "--port", "8642"]
        gateway_argv = ["/opt/hbm/venv/bin/python", "-m", "hbm_cli.main", "gateway", "run"]
        for path, label, argv in (
            (agents / "ai.hbm.dashboard.plist", "ai.hbm.dashboard", backend_argv),
            (agents / "ai.hbm.gateway.plist", "ai.hbm.gateway", gateway_argv),
            (agents / "ai.hbm.serve-old.plist", "ai.hbm.serve-old", serve_argv),
            (daemons / "ai.hbm.serve.plist", "ai.hbm.serve", serve_argv),
        ):
        ...
        def fake_print(domain, label):
            ...
            if (domain, label) == (f"user/{uid}", "ai.hbm.dashboard"):
                return (True, 4242)
            if (domain, label) == ("system", "ai.hbm.serve"):
                return (True, None)
            return (False, None)
        ...
        with patch("hbm_cli.gateway._launchd_print_service_pid", side_effect=fake_print):
            jobs = _no_real_launchd_jobs([("agent", agents), ("daemon", daemons)])

        assert jobs == [
            (f"user/{uid}", "ai.hbm.dashboard", backend_argv, 4242),
            ("system", "ai.hbm.serve", serve_argv, None),
        ]
        assert not any(label == "ai.hbm.gateway" for _domain, label in probed)
        assert probed[:2] == [(f"gui/{uid}", "ai.hbm.dashboard"), (f"user/{uid}", "ai.hbm.dashboard")]
        assert not any(domain == "system" and label != "ai.hbm.serve" for domain, label in probed)

        owning = main_dashboard._launchd_job_owning_backend
        assert owning(4242, ["something", "else"], jobs) == (f"user/{uid}", "ai.hbm.dashboard", 4242)
        assert owning(9999, ["python"], jobs, ancestors=[4242, 1]) == (f"user/{uid}", "ai.hbm.dashboard", 4242)
        assert owning(9999, serve_argv, jobs) == ("system", "ai.hbm.serve", None)
        ...
        assert owning(9999, backend_argv + ["--no-open"], jobs) == (f"user/{uid}", "ai.hbm.dashboard", 4242)
