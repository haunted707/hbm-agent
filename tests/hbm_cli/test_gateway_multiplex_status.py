...
``hbm gateway status`` / ``gateway list`` / ``profile list`` keyed liveness
...
def _fake_multiplexer(monkeypatch, tmp_path, *, multiplex: bool, pid_file: bool = True):
    ...
    (tmp_path / "gateway_state.json").write_text(json.dumps({
        "pid": os.getpid(), "kind": "hbm-gateway", "gateway_state": "running",
        "start_time": status._get_process_start_time(os.getpid()), "hbm_home": str(tmp_path),
    }))
    monkeypatch.setenv("HBM_HOME", str(tmp_path / "profiles" / "beta"))
    monkeypatch.setattr(hbm_constants, "_default_hbm_root_memo", None)
    monkeypatch.setattr(
        status, "_read_process_cmdline", lambda pid: "python -m hbm_cli.main gateway run --replace"
    )
