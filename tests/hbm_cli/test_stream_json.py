"""``hbm chat -q … --format stream-json`` emits a parseable JSONL event stream and nothing else on stdout."""
...
def _run_stream_json_chat(monkeypatch, capsys, run_conversation, credentials_ok=True):
    ...
    import cli
    import hbm_cli.main as cli_entry
    ...
    monkeypatch.setattr(cli, "HbmCLI", FakeCLI)
    ...
    monkeypatch.setattr("hbm_cli.free_tier_bootstrap.run_bootstrap", lambda **_k: None)
    monkeypatch.setattr("hbm_cli.quiet_single_query.continue_quiet_notify_completions", lambda *_a, **_k: None)
