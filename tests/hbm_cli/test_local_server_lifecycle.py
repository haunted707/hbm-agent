...
def test_stop_disables_and_tears_down(client, monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.local_runtime.bootstrap.get_supervisor",
                        lambda: _FakeSup())
    monkeypatch.setattr("hbm_cli.local_runtime.bootstrap.shutdown_local_runtime",
                        _shutdown)
...
def test_start_enables_and_boots(client, monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.local_runtime.bootstrap.ensure_local_runtime",
                        _ensure)
...
def test_stop_forwards_recovery_and_preserves_conflict(client, tmp_path, monkeypatch, refuse):
    ...
    def recover():
        called.append(True)
        if refuse:
            raise HTTPException(409, "Another HBM AGENT process owns this server, or its ownership could not be verified")
