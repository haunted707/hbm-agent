...
#88251: ``hbm update`` treated a failed desktop pack as non-fatal, printed
...
import hbm_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_constants.with_hbm_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "hbm_constants.display_hbm_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_hbm_home", lambda: tmp_path)
