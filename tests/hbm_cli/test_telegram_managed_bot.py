"""Tests for hbm_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestQRCode:
    ...
    def test_print_qr_code_tip_targets_active_interpreter(self, capsys):
        ...
        # environment when HBM AGENT runs in an isolated venv (which has no pip
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "hbm_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/HbmSetupBot/hbm_abcdefghijklmnop_bot?name=HBM AGENT+Agent",
            "qr_payload": "https://t.me/newbot/HbmSetupBot/hbm_abcdefghijklmnop_bot?name=HBM AGENT+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "hbm_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="HBM AGENT")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="hbm_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/HbmSetupBot/hbm_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/HbmSetupBot/hbm_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "hbm_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "hbm_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("hbm_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("hbm_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("hbm_cli.telegram_managed_bot.time.sleep"):
                result = poll_for_setup_result(
...
class TestSetupTelegramAuto:
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from hbm_cli import setup

        seen = {}
        profile_home = tmp_path / ".hbm" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_hbm_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "hbm_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from hbm_cli.setup_platforms import _profile_name_from_hbm_home

        assert (
            _profile_name_from_hbm_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\hbm\profiles\oracle")
            )
            == "oracle"
        )
