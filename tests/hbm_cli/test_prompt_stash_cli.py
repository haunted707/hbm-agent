"""Tests for the Ctrl+S prompt stash wiring inside HbmCLI.

The state machine itself is covered by tests/hbm_cli/test_prompt_stash.py. These
tests verify the cli.py side:
  - HbmCLI.__init__ creates a PromptStash
...
tests/hbm_cli/test_cli_extension_hooks.py.
"""
...
def _make_cli(**kwargs):
    """Create a HbmCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports hbm_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``HbmCLI.run()``'s local ``KeyBindings`` instance, so they are not
