...
second HBM AGENT holder the updater's drain did not stop — survives the copy and is
...
def test_post_update_guard_covers_sibling_profiles(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(update_cmd, "get_hbm_home", lambda: root_home)
    monkeypatch.setattr(
        "hbm_cli.backup._sibling_profile_homes",
