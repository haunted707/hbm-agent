...
@pytest.mark.parametrize("subdir", (".", *config._HBM_HOME_SUBDIRS))
def test_unavailable_directory_links_are_diagnosed_without_creating_targets(tmp_path, monkeypatch, subdir):
    ...
    monkeypatch.setattr(config, "get_hbm_home", lambda: home)
    monkeypatch.setattr(config, "is_managed", lambda: False)
    config._HBM_HOME_ENSURED.discard(str(home))
    ...
    assert str(home) not in config._HBM_HOME_ENSURED

    target.mkdir(parents=True, mode=0o750)
    config.ensure_hbm_home()
    assert link.is_symlink() and target.stat().st_mode & 0o777 == 0o750
    assert str(home) in config._HBM_HOME_ENSURED
...
def test_initialization_preserves_external_directory_modes(tmp_path, monkeypatch, linked):
    ...
    config.ensure_hbm_home()
    monkeypatch.setattr(config, "get_hbm_home", lambda: home.resolve())
    config.ensure_hbm_home()

    assert all((home / name).is_dir() for name in config._HBM_HOME_SUBDIRS)
...
def test_symlinked_parent_above_home_is_not_an_operator_home_link(
    ...
    """A link above HBM_HOME is not an operator-owned home link.
    ...
    if existing:
        ...
        for name in config._HBM_HOME_SUBDIRS:
    ...
    monkeypatch.setattr(config, "is_managed", lambda: False)
    config._HBM_HOME_ENSURED.discard(str(home))
    config._HBM_HOME_ENSURED.discard(str(home.resolve()))
