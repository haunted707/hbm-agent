...
def test_doctor_restores_global_tool_policy_and_module_state(tmp_path: Path) -> None:
    ...
    before_modules = {
        ...
        if name == "hbm_plugins" or name.startswith("hbm_plugins.")
    }
...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from hbm_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    from hbm_cli import plugin_dev
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = hbm_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_hbm_home", lambda: hbm_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a hbm-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded hbm-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("hbm-plugin-doctor-*"))
