"""``hbm update`` / ``hbm migrate relay``: legacy ``HBM_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from hbm_cli.relay_plugin_migrate import (
...
HBM_NEMO_RELAY_ATOF_ENABLED=1
HBM_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
HBM_NEMO_RELAY_ATOF_FILENAME=hbm-atof.jsonl
HBM_NEMO_RELAY_ATOF_MODE=append
HBM_NEMO_RELAY_ATIF_ENABLED=1
HBM_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
HBM_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
HBM_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "hbm-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file HBM AGENT will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: HBM_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")
