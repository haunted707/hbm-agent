...
The notice prints the most recent curator run summary on `hbm update`,
...
session start) so users normally never see the rename map. `hbm update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from hbm_cli import update_cmd_maint
