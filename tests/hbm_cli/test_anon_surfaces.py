...
from hbm_cli.auth import _load_auth_store  # noqa: F401  (store import name kept for parity with core tests)
...
def _account_state() -> dict:
    return {
        "auth_method": "oauth_device_code", "client_id": "hbm-cli",
        "access_token": _jwt(sub="nas_user:real", client_id="hbm-cli", account_tier="standard", paid_access=True),
...
def isolated_store(monkeypatch, tmp_path):
    monkeypatch.setenv("HBM_SHARED_AUTH_DIR", str(tmp_path / "shared-store"))
...
def _render_all(capsys) -> dict[str, str]:
    ...
    out["hbm status"] = capsys.readouterr().out
...
def test_every_in_chat_free_tier_string_names_the_slash_command(monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.auth.resolve_provider", lambda _requested: "nous")
    ...
    for text in (*command_copy, *refusal_copy):
        # The ruled refusal uses HBM AGENT as the grammatical subject; only that exact product-name
        # phrase is exempt from the broad top-level-command gate.
        assert "hbm " not in text.replace("this HBM AGENT can", "this product can").lower()
...
def test_no_chat_copy_of_any_sign_in_state_leaks_a_terminal_verb_or_a_forbidden_word():
    ...
    terminal_or_url = re.compile(r"hbm |https?://", re.IGNORECASE)
...
def test_terminal_only_strings_keep_the_terminal_verb(monkeypatch, capsys):
    assert "hbm " in nous_account.FREE_TIER_NEEDS_ACCOUNT
    assert "hbm " in anon_auth.UPGRADE_UNAVAILABLE
    assert "hbm " in anon_auth.FREE_TIER_NOT_SIGNED_IN

    monkeypatch.setattr("hbm_cli.auth.get_provider_auth_state", lambda _provider: {"access_token": "token"})
    monkeypatch.setattr("hbm_cli.model_switch_providers._free_tier_nous_row", lambda _provider: None)
    model_setup_flows._model_flow_nous({})
    assert "hbm " in capsys.readouterr().out
...
async def test_the_wait_line_is_only_composed_on_the_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_cli.auth_device_flow._print_device_code_instructions", lambda *_args, **_kwargs: None
    )
    cli_copy = []
    ...
    for rel in ("gateway/slash_commands_login.py", "hbm_cli/cli_commands_mixin.py"):
