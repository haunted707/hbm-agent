"""``hbm chat -Q`` passes the dispatcher's HBM_TURN_AUTHOR to ``run_conversation`` as ``turn_author``.
...
def _plain_one_shot_env(monkeypatch):
    monkeypatch.delenv("HBM_KANBAN_GOAL_MODE", raising=False)
    monkeypatch.delenv("HBM_KANBAN_TASK", raising=False)


def _run(monkeypatch, env_value, run_conversation=None):
    """One quiet turn with HBM_TURN_AUTHOR set to ``env_value`` (unset for None); returns the recorded call kwargs."""
...
def test_quiet_one_shot_resumes_nested_notify_on_this_session_not_parent(monkeypatch, capsys):
    ...
    ``chat -Q`` used to inherit the dispatcher's HBM_SESSION_KEY and exit after the
...
def test_quiet_notify_loop_shares_one_linger_budget(monkeypatch):
    ...
    """
    from hbm_cli import quiet_single_query as qsq
