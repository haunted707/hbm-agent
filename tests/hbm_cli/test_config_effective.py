"""Invariants for ``hbm_cli.config_effective.load_user_config_effective`` — the one loader every
defaults-free config reader (gateway runtime, TUI gateway, cron, ``hbm send`` bridge, doctor,
...
def _reset_caches():
    ...
    from hbm_cli import config_effective, managed_scope
...
def _reset_caches_keep_last_good():
    import hbm_cli.config as cfg
    from hbm_cli import config_effective
