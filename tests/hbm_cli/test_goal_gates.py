...
def test_failing_gate_short_circuits_judge():
    ...
    mgr.add_gate("exit 5")
    with patch("hbm_cli.goals.judge_goal") as mock_judge:
...
def test_failed_gate_reruns_when_untracked_file_content_changes(tmp_path, monkeypatch):
    ...
    with patch("hbm_cli.goals.judge_goal", return_value=("done", "ok", False, None, False)) as judge:
