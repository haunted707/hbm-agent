"""Regression tests for Desktop-owned ``hbm serve`` lifecycle tracking."""

import pytest

from hbm_cli.web_server_lifecycle import (
    _is_serve_orphaned,
...
def test_parent_watchdog_treats_empty_marker_env_as_absent(monkeypatch):
    """Blank inherited HBM_PARENT_START_MARKER/NONCE degrade to PID-only tracking."""
    from hbm_cli import web_server_lifecycle

    monkeypatch.setenv("HBM_PARENT_PID", "4242")
    monkeypatch.setenv("HBM_PARENT_START_MARKER", "")
    monkeypatch.setenv("HBM_PARENT_NONCE", "")
    seen = {}
...
def test_parent_watchdog_warns_when_disarmed_by_unusable_marker(monkeypatch, caplog):
    ...
    monkeypatch.setenv("HBM_PARENT_PID", "4242")
    monkeypatch.setenv("HBM_PARENT_START_MARKER", "ps:Sat")
    monkeypatch.setenv("HBM_PARENT_NONCE", "n")
    started = []
    monkeypatch.setattr(
