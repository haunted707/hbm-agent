...
from hbm_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    HBM_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def hbm_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("hbm_cli.config") or mod == "hbm_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, hbm_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, hbm_home, capsys, monkeypatch):
