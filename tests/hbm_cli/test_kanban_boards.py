...
def fresh_home(tmp_path, monkeypatch):
    """Isolated HBM_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test HBM_HOME plus a path-init cache reset
    ...
    # Also reset hbm_constants cache so get_default_hbm_root() re-reads.
    try:
        import hbm_constants
        hbm_constants._cached_default_hbm_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "hbm-agent", "proj_1", "a",
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``HBM_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("HBM_KANBAN_DB", str(forced))
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("HBM_KANBAN_BOARD", "envwin")
        with kbc.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``HBM_KANBAN_BOARD`` / DB / workspaces on spawn.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``hbm kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "hbm_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
