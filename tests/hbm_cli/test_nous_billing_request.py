"""Tests for the hbm_cli.nous_billing HTTP client's response handling.
...
from hbm_cli import nous_billing as nb
...
def test_billing_token_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    multiplex gateway, where the per-turn HBM_HOME override selects the auth.json."""
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: f"tok-{hbm_constants.get_hbm_home().name}")
    a, b = tmp_path / "a", tmp_path / "b"
    a.mkdir(); b.mkdir()
    seen = []
    for home in (a, b):
        tok = hbm_constants.set_hbm_home_override(str(home))
        try:
