...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_hbm_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from hbm_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """hbm -z bypasses HbmCLI, but recall still needs SessionDB."""
    from hbm_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "hbm_state_registry", mod("hbm_state_registry", acquire=fake_acquire))
    monkeypatch.setitem(
        sys.modules,
        "hbm_cli.config",
        mod("hbm_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "hbm_cli.models",
        mod("hbm_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("hbm_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["HBM_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/hbm-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["HBM_MODEL"] == "nous/hbm-test"
    assert env["HBM_INFERENCE_MODEL"] == "nous/hbm-test"
    assert env["HBM_TUI_PROVIDER"] == "nous"
    assert env["HBM_INFERENCE_PROVIDER"] == "nous"
    assert env["HBM_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["HBM_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("hbm-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_hbm_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "hbm-ink"
    ...
    monkeypatch.delenv("HBM_TUI_DIR", raising=False)
