"""
Tests for --yes / --force flag separation in `hbm skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("hbm_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["hbm", "skills", "install", "official/email/agentmail", "--yes"],
    )
