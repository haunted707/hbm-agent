...
_GATEWAY_CMD = "python -m hbm_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m hbm_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("hbm_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("hbm_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("hbm_cli.gateway.is_macos", return_value=True),
            patch("hbm_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "hbm_cli.gateway.get_launchd_label",
                return_value="ai.hbm.gateway.myprofile",
            ),
            patch(
                "hbm_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.hbm.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.hbm.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.hbm.gateway": 123,
            "ai.hbm.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "hbm_cli.gateway.get_launchd_label",
                return_value="ai.hbm.gateway",
            ),
            patch(
                "hbm_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.hbm.gateway", "ai.hbm.gateway-profile-b"],
            ),
            patch(
                "hbm_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.hbm.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("hbm_cli.gateway.is_macos", return_value=True),
            patch("hbm_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.hbm.gateway.broken\n"
                    "  123  \t0\tai.hbm.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every hbm-gateway* unit unconditionally."""
        with (
