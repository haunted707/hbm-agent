...
from hbm_cli import model_switch_providers as picker
...
def test_picker_command_auth_is_lazy_and_credential_scoped(monkeypatch, route, static):
    ...
    monkeypatch.setattr("hbm_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: False)
...
def test_setup_probe_credentials_never_become_saved_credentials(monkeypatch, route, discover, static):
    from agent import command_token_source
    from hbm_cli import config as config_module, models
    from hbm_cli import model_setup_flows_custom as setup
    ...
    from hbm_cli.main_provider_setup import _named_custom_provider_map
