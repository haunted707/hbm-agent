...
next command, and every ``hbm <sub> <cmd>`` it cites is a registered subcommand.
"""
...
from hbm_cli import gateway as gateway_cli
from hbm_cli import profiles
from hbm_cli.update_lock import UpdateHolder, describe_holder
...
def test_systemctl_failure_is_explained_not_tracebacked(monkeypatch, capsys):
    exc = subprocess.CalledProcessError(1, ["systemctl", "--user", "start", "hbm-gateway"])
    out = _run_gateway_command_raising(monkeypatch, capsys, exc)
    assert not out.lstrip().startswith("Command '")
    assert "hbm gateway status --deep" in out
    assert "hbm gateway install --force" in out
...
def test_existing_gateway_guard_says_bots_are_online_and_names_status(monkeypatch, capsys):
    ...
    for cmd in ("hbm gateway status", "hbm gateway restart", "hbm gateway stop"):
        assert cmd in out
...
def test_unsupported_platform_copy_says_what_is_unsupported(subcommand):
    ...
    assert "hbm gateway run" in text or subcommand == "uninstall"
...
def test_describe_holder_avoids_jargon_and_names_next_steps():
    ...
    assert "hbm logs" in message
    assert "hbm update" in message
