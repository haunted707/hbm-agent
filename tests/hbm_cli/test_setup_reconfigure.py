...
- Bare `hbm setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `hbm setup --quick` runs the narrower "fill in missing items" flow.
- `hbm setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("hbm_cli.setup.ensure_hbm_home", {}),
        ("hbm_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("hbm_cli.config.is_managed", {"return_value": False}),
        ("hbm_cli.setup.load_config", {"return_value": {}}),
        ("hbm_cli.setup.save_config", {}),
        ("hbm_cli.setup.get_env_value", {"return_value": None}),
        ("hbm_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("hbm_cli.setup._print_setup_summary", {}),
        ("hbm_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("hbm_cli.setup.load_config", {"return_value": {}}),
        ("hbm_cli.setup.save_config", {}),
        ("hbm_cli.auth.get_active_provider", {"return_value": None}),
        ("hbm_cli.setup.get_env_value", {"return_value": None}),
        ("hbm_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `hbm setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="hbm_cli.setup.prompt_choice",
                quick="hbm_cli.setup._run_quick_setup",
                model="hbm_cli.setup.setup_model_provider",
                terminal="hbm_cli.setup.setup_terminal_backend",
                agent="hbm_cli.setup.setup_agent_settings",
                gateway="hbm_cli.setup.setup_gateway",
                tools="hbm_cli.setup.setup_tools",
            )
            from hbm_cli.setup import run_setup_wizard
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("hbm_cli.setup.prompt_choice", {"return_value": 0}),
                first="hbm_cli.setup_quick._run_first_time_quick_setup",
            )
            from hbm_cli.setup import run_setup_wizard
            from hbm_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("hbm_cli.setup.prompt_choice", {"return_value": 2}),
                blank="hbm_cli.setup_quick._run_blank_slate_setup",
            )
            from hbm_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "hbm_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["hbm", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
