...
session restoration / cron / the gateway):
    ...
    * There is NO quiet/non-interactive gating in the scanner itself; the
      gateway calls it before setting HBM_QUIET (gateway/run.py), so the
      warning fires regardless of HBM_QUIET.

Exercises the real scanner against a real temp HBM_HOME .env file.
"""
...
TRUTH_TABLE = [
    ...
    pytest.param(
        "TERMINAL_CWD=.\n", {"HBM_QUIET": "1"}, None, ("TERMINAL_CWD",),
        id="quiet-mode-no-gating-dotenv-warns",
    ),
    pytest.param(
        "API_KEY=x\n",
        {"HBM_QUIET": "1", "TERMINAL_CWD": "."},
        None,
        (),
...
def test_deprecated_env_warning_truth_table(
    ...
    if env_file is not None:
        (hbm_home / ".env").write_bytes(env_file.encode("utf-8"))
    ...
    for key in ("TERMINAL_CWD", "MESSAGING_CWD", "HBM_QUIET"):
        monkeypatch.delenv(key, raising=False)
    for key, value in proc_env.items():
        monkeypatch.setenv(key, value)

    import hbm_cli.config as config_module
