...
CASES = {
    ...
    "product python → nix": (["hbm_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every hbm_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["hbm_state.py"], _lanes(python=True, scan=True)),
