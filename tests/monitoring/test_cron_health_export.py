...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("hbm.gateway.up"), _M("hbm.gateway.active_agents"),
        _M("hbm.gateway.busy"), _M("hbm.gateway.drainable"),
        _M("hbm.gateway.restart_requested"),
        _M("hbm.platform.up"), _M("hbm.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("hbm.cron.scheduler.heartbeat_age_seconds"),
        _M("hbm.cron.scheduler.last_success_age_seconds"),
        _M("hbm.cron.scheduler.catch_up_occurrences"),
        _M("hbm.cron.jobs.enabled"), _M("hbm.cron.jobs.running"),
        _M("hbm.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "HBM AGENT-owned Relay transport health" in text
