...
The ``hbm update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m hbm_cli.main gateway run`` — image name
    ``pythonw``, not ``hbm-agent.exe`` — so the ``taskkill /IM hbm-agent.exe`` guard
    ...
    # The hbm.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM hbm-agent.exe' in text
