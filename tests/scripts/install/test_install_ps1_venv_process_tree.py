"""Windows installer regression for HBM AGENT children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected HBM AGENT process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    runtime_dir = install_dir / ".hbm-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real HBM AGENT launchers
    ...
    _write_cmd(
        ...
        '/C:"/IM hbm-agent.exe" >nul\n'
        "if not errorlevel 1 exit /b 0\n"
        '"%SystemRoot%\\System32\\taskkill.exe" %*\n',
    )
    uv = hbm_home / "bin" / "uv.exe"
    ...
    wrapper.write_text(
        ...
        f"-InstallDir '{install_dir}' -HbmHome '{hbm_home}'\n"
        "exit $LASTEXITCODE\n",
        encoding="utf-8",
    )
