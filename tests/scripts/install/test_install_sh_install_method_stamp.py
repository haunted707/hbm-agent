...
($INSTALL_DIR), not into the shared $HBM_HOME.

Background (shared-$HBM_HOME bug)
------------------------------------
$HBM_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.hbm:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$HBM_HOME the two installs would fight over one slot — and the container,
...
``~/.hbm/hbm-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$HBM_HOME/\.install_method"', text), (
        "install.sh must not stamp $HBM_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `hbm update`"
    )
