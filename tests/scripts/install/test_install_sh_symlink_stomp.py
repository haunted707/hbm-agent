...
Older versions of ``install.sh`` created ``$command_link_dir/hbm`` as a
symlink to the pip-generated entry point at ``$HBM_BIN`` (i.e.
``venv/bin/hbm``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/hbm" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$HBM_BIN" "$@"`` then self-recursed and ``hbm`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/hbm``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/hbm-agent\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/hbm-agent"')
    cat_idx = block.find('cat > "$command_link_dir/hbm-agent" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/hbm before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/hbm        <- pip entry point (the one we must preserve)
          local_bin/hbm       <- symlink → ../venv/bin/hbm  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``HBM_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/hbm`` still contains its original pip-script body
      * ``local_bin/hbm`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "hbm-agent"
    ...
    shim_path = command_link_dir / "hbm-agent"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/hbm was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/hbm must be replaced with a regular file, not "
