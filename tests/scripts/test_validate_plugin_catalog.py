...
def test_bad_requires_hbm_spec_fails(tmp_path):
    _expect_error(tmp_path, {"requires_hbm": "banana"}, "requires_hbm")


def test_comma_separated_requires_hbm_passes(tmp_path):
    entry = {**VALID_ENTRY, "requires_hbm": ">=0.19, <2.0"}
    path = write_entry(tmp_path, entry)
    result = run_validator(str(path))
    assert result.returncode == 0, result.stdout + result.stderr
