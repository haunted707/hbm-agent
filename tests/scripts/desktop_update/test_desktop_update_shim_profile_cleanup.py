...
def test_shim_removes_only_its_owned_profile(tmp_path, outcome):
    ...
    install = tmp_path / "hbm-agent"
    install.mkdir()
    env = {**os.environ, "HOME": str(tmp_path), "HBM_HOME": str(tmp_path),
           "XDG_CONFIG_HOME": str(config), "TMPDIR": str(tmp_path),
           "PATH": f"{bin_dir}:/usr/bin:/bin", "HBM_SELFTEST_HOLD_SECONDS": "1",
           "HBM_UPDATE_SHIM_GRACE_SECONDS": "1", "HBM_SELFTEST_FAIL": "1" if outcome == "error" else ""}
    ...
    collision = tmp_path / f"hbm-update-ui-{process.pid}"
    collision.mkdir()
