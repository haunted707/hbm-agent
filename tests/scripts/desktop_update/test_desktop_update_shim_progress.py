...
def progress(tmp_path):
    ...
    status = tmp_path / "hbm-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen HBM AGENT to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `hbm update`, and reports the stage that was on screen
...
n="$(cat "$HBM_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$HBM_TEST_CALLS"
for f in "$TMPDIR"/hbm-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$HBM_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$HBM_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "hbm-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    hbm = install_root / "venv" / "bin" / "hbm"
    hbm.write_text(FAKE_HBM)
    hbm.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "HBM_TEST_CAPTURE": str(capture),
        "HBM_TEST_CALLS": str(calls),
        "HBM_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".hbm-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`hbm update` is the longest wait in the hand-off and the one the
