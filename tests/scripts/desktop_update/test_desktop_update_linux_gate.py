...
``--install-root`` keeps whatever spelling the app's HBM AGENT root had (a symlinked
``~/.hbm/hbm-agent``, or ``/home`` on Fedora/ostree where it is a link to
...
def test_symlinked_spelling_on_either_side_still_relaunches(tmp_path, root_spelling, target_spelling):
    ...
    target = tmp_path / target_spelling / "apps" / "desktop" / "release" / "linux-unpacked" / "HBM AGENT"
...
def test_target_outside_the_checkout_is_still_skew(tmp_path):
    ...
    (tmp_path / "link").symlink_to(real)
    foreign = tmp_path / "opt" / "HBM AGENT"
    foreign.mkdir(parents=True)
    (foreign / "hbm").touch()
    assert _gate(tmp_path / "link", foreign / "hbm") == "skew"
    # A sibling directory sharing the prefix must not be mistaken for the checkout either.
    assert _gate(tmp_path / "link", Path(str(unpacked) + "-evil") / "HBM AGENT") == "skew"
