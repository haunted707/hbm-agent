"""Tests for the bundled hbm-achievements dashboard plugin.
...
``plugins/hbm-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the hbm-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.hbm so state/snapshot files don't collide.
    ...
    # swap ``sys.modules['hbm_state']`` with auto-restoration. Without
    # this, a raw ``sys.modules[...] = fake`` assignment would leak the
    # fake into later tests in the same xdist worker — breaking every
    # test that does ``from hbm_state import SessionDB``.
    module._test_monkeypatch = monkeypatch
    yield module


class _FakeSessionDB:
    """Stand-in for hbm_state.SessionDB that records scan calls."""
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['hbm_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("hbm_state")
    fake_module.SessionDB = lambda **_kw: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "hbm_state", fake_module)
...
def test_scan_sessions_never_opens_a_writable_session_db(plugin_api, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(hbm_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
