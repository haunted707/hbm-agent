"""Plugin data paths follow the active profile's HBM_HOME, including the ContextVar override.

Several plugins carried a ``~/.hbm`` fallback (guarding an ImportError of ``hbm_constants``
...
def _openviking_log(home):
    ...
    return ov.get_hbm_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
