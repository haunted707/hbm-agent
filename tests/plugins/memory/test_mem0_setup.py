...
def _inject_fake_hbm_cli(monkeypatch):
    """Inject fake hbm_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("hbm_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("hbm_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_hbm_cli = types.ModuleType("hbm_cli")
    fake_hbm_cli.config = fake_config_mod
    fake_hbm_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "hbm_cli", fake_hbm_cli)
    monkeypatch.setitem(sys.modules, "hbm_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "hbm_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["hbm", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_hbm_home", lambda: tmp_path)
        _inject_fake_hbm_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "hbm-user"
...
def test_discovery_loaded_setup_module_exposes_post_setup(monkeypatch):
    """`hbm memory setup mem0` reaches the wizard when the package is first imported by plugin
