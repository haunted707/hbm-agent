...
Profile isolation is a ContextVar-scoped HBM_HOME override; a plain ``threading.Thread`` starts
...
def _retaindb(seen, tmp_path):
    ...
    p._client = MagicMock()
    p._context_overlay = lambda query: {"context": seen.setdefault("home", get_hbm_home()) and "ctx"}
...
def _hindsight(seen, tmp_path):
    ...
    p._do_recall = lambda query: (seen.setdefault("home", get_hbm_home()) and "text", 1)
