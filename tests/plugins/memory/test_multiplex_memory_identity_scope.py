...
OPENVIKING_* / HINDSIGHT_BANK_ID / HBM_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "HBM_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("hbm", "", "platform")
    ...
    provider.initialize("s1", hbm_home=str(secondary_profile))
    assert provider._client.project == "hbm-b"
    ...
    assert (hcfg["banks"]["hbm"]["bankId"], hcfg["mode"]) == ("hbm", "cloud")
