...
class FakeEntryPoint:
    ...
    group = "hbm_agent.memory_providers"
...
def test_project_dir_is_ignored_without_opt_in(tmp_path, monkeypatch):
    ...
    _write_provider_dir(tmp_path / ".hbm" / "plugins", "projectmem")
...
def test_project_dir_is_discovered_when_opted_in(tmp_path, monkeypatch):
    provider = _write_provider_dir(tmp_path / ".hbm" / "plugins", "projectmem")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("HBM_ENABLE_PROJECT_PLUGINS", "1")
...
def test_bundled_still_wins_over_project(tmp_path, monkeypatch):
    ...
    _write_provider_dir(tmp_path / ".hbm" / "plugins", "honcho")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("HBM_ENABLE_PROJECT_PLUGINS", "1")
...
def test_find_provider_dir_resolves_a_package_entry_point(entry_points, tmp_path, monkeypatch):
    ...
    dashboard config panel and its `hbm <provider>` subcommands — both are
...
def test_unreadable_user_plugin_does_not_abort_memory_discovery(tmp_path, monkeypatch):
    """One mode-000 / ACL-denied ``$HBM_HOME/plugins/<x>`` must not hide the bundled
    providers or its readable siblings from the dashboard / ``hbm memory`` pickers (#111804)."""
    plugins_root = tmp_path / "plugins"
