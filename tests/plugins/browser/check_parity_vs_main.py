...
Run from the PR worktree:
    ...
    cd ~/.hbm/hbm-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.hbm/hbm-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.hbm/hbm-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a hbm-agent checkout"
)
...
# Isolated HBM_HOME for the config write.
