...
The contract under test: data lives under ``<hbm home>/plugin-data/<name>/``
...
def test_data_dir_lives_outside_the_install_tree(hbm_home):
    root = plugin_data_dir("my-plugin")

    assert root == hbm_home / "plugin-data" / "my-plugin"
    assert root.is_dir()
    # The invariant that motivated the module: data must not live under the
    # install tree that `hbm plugins remove` deletes.
    assert (hbm_home / "plugins") not in root.parents


def test_data_dir_is_stable_across_calls(hbm_home):
    assert plugin_data_dir("p") == plugin_data_dir("p")


@pytest.mark.parametrize("bad", ["", ".", "..", "../escape", "a/b", "a\\b", "x" * 65])
def test_hostile_names_are_rejected(hbm_home, bad):
    with pytest.raises(ValueError):
        plugin_data_dir(bad)


def test_plugin_db_journal_mode_is_the_shared_fallback_verdict(hbm_home):
    ...
    assert (hbm_home / "plugin-data" / "board" / "data.db").exists()


def test_plugin_db_rejects_path_shaped_filenames(hbm_home):
    with pytest.raises(ValueError):
