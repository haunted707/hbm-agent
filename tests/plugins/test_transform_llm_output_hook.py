...
from hbm_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(hbm_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <hbm_home>/plugins/<name> and opt it in."""
    plugin_dir = hbm_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HBM_HOME", str(tmp_path / "hbm_empty"))
