...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs HBM AGENT the way
...
# actually touches:
#
#   INSTALL   - downloads the production HBM AGENT-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron HBM AGENT.exe must appear.
...
#   installer's `git clone` and `hbm update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and hbm-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "hbm-desktop-app-update", "hbm-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published HBM AGENT-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:HBM_E2E_WORKROOT) { $env:HBM_E2E_WORKROOT } else { Join-Path $env:TEMP "hbm-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://hbm-assets.nousresearch.com/HBM AGENT-Setup.exe",
...
$HbmHome  = Join-Path $WorkRoot "hbm-home"
$InstallDir  = Join-Path $HbmHome "hbm-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if hbm ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit hbm thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\HBM AGENT.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\HBM AGENT.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-HbmRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $HbmHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "hbm-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $HbmHome ".hbm-update-result.json"
    ...
    Get-ChildItem -LiteralPath $HbmHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "hbm-home-ls.txt")
}

function Test-HbmRuns([string]$Label) {
    Save-InstallSideState $Label
    $hbmExe = Join-Path $InstallDir "venv\Scripts\hbm.exe"
    Assert-True (Test-Path -LiteralPath $hbmExe) "$Label -- venv\Scripts\hbm.exe exists"
    & $hbmExe --version 2>&1 | ForEach-Object { Write-Host "    hbm --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- hbm --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-HbmHome", $HbmHome, "-InstallDir", $InstallDir)
...
function Invoke-HbmUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $hbmExe = Join-Path $InstallDir "venv\Scripts\hbm.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $hbmExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $hbmExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "hbm update transcript" $log
    Assert-True ($updateExit -eq 0) "hbm update exited $updateExit (expected 0)"
}

function Invoke-HbmDesktopAppUpdate([string]$TargetSha) {
    # The hbm-desktop launch surface: `hbm desktop` runs its whole
    ...
    $prevCap = $env:HBM_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:HBM_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $hbmExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:HBM_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "hbm desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "hbm desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $HbmHome ".hbm-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured hbm desktop spec; update completed"
}
...
function Stop-HbmAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only HBM AGENT.exe (Electron) -- never hbm.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "HBM AGENT" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) HBM AGENT.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `hbm update`/desktop builds use the HBM AGENT-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $HbmHome "node\node.exe"),
        (Join-Path $HbmHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website HBM AGENT-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): HBM AGENT-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "HBM AGENT-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "HBM AGENT-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:HBM_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $HbmHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  HBM AGENT-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> HBM AGENT.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "HBM AGENT-Setup.exe", (Join-Path $HbmHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "HBM AGENT" -ErrorAction SilentlyContinue)) "HBM AGENT.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "HBM AGENT-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $HbmHome "logs\bootstrap-installer.log"
    ...
    Stop-HbmAppProcesses "post-install"
    ...
    Test-HbmRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop HBM AGENT.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged HBM AGENT.exe present before update"
    ...
    try {
        ...
        # `hbm update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .hbm-update-result.json on every exit;
        #   * older checkouts -> the staged hbm-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/hbm/relaunch
        ...
        $updateLog = Join-Path $HbmHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-HbmRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "HBM AGENT.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched HBM AGENT.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "HBM AGENT" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched HBM AGENT window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "HBM AGENT" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $HbmHome "logs\desktop-update-handoff.log"
        ...
        Stop-HbmAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, hbm runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-HbmRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-HbmRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $HbmHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "hbm-desktop-app-update" {
            Invoke-HbmDesktopAppUpdate $state.current
        }
        "hbm-update" {
            Invoke-HbmUpdate
        }
        ...
        "desktop-installer@latest" {
            # A user re-downloading HBM AGENT-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $HbmHome "logs\desktop.log"))) {
