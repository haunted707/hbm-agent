...
# (HBM AGENT-Setup.dmg from the website) can reach HEAD.
#
...
#              hbm-desktop-app-update  capture `hbm desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              hbm-update              CLI update from the installed venv
...
#     --update-method open-app-update|hbm-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://hbm-assets.nousresearch.com/HBM AGENT-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|hbm-desktop-app-update|hbm-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${HBM_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/hbm-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$HBM_HOME/hbm-agent"
}
...
phase_stage() {
  ...
  arm_redirect
  mkdir -p "$HBM_HOME"
  touch "$HBM_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/HBM AGENT.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/HBM AGENT.app" \
    "/Applications/HBM AGENT.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published HBM AGENT-Setup.dmg"

  local dmg="$WORK_ROOT/HBM AGENT-Setup.dmg"
  ...
  # "Install HBM AGENT" button with native input.
  local rc=0
  ...
  log_group "HBM AGENT-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$hbm" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "hbm --version failed after install"
  ok "hbm --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed HBM AGENT.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    hbm-update)
      ...
      # update subcommand only in later releases; ask the installed hbm.
      local hbm="$INSTALL_DIR/venv/bin/hbm"
      local update_cmd=("$hbm" update)
      ...
      [ "$rc" -eq 0 ] || fail "hbm update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/HBM AGENT.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/HBM AGENT.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built HBM AGENT.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, hbm update) need.
      local app app_bin
...
PYEOF
    ...
    hbm-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "hbm desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "hbm desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.hbm/logs" "$ildest/hbm-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/HBM AGENT"
  ...
  "$INSTALL_DIR/venv/bin/hbm" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "hbm --version failed after update"
  ok "hbm --version works post-update"
