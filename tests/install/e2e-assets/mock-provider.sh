...
#   mock_start <workroot>      start, write config into $HBM_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, HBM_HOME, ok/fail helpers.
...
EOF
  ...
  ok "provider 'mock' configured in $HBM_HOME (api $url/v1)"
}
