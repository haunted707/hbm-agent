#!/usr/bin/env bash
# Drive the HBM AGENT-Setup dmg bootstrap through its first-run GUI.
#
...
# the redirect env), click "Install HBM AGENT ->" with native input, then watch
...
tell application "System Events"
  set procs to (every process whose name contains "HBM AGENT")
...
HBM_BIN="$INSTALL_DIR/venv/bin/hbm"
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$HBM_BIN" ] && installed_app
}
...
BOOTSTRAP_LOG="$HOME/.hbm/logs/bootstrap-installer.log"
bootstrap_error() {
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + HBM AGENT.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "HBM AGENT-Setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then
