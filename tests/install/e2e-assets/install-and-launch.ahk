...
; Drive the REAL HBM AGENT-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real HBM AGENT.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-hbm-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "HBM AGENT-Setup.exe"
...
Log("Launch clicked; waiting for the HBM AGENT desktop app window")
...
try {
    ...
    throw Error("HBM AGENT.exe real-sized window did not appear within 300s of clicking Launch")
}
