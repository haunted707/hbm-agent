// drive-update.cjs — launch the INSTALLED HBM AGENT.exe (real Electron desktop
...
//
//   node <this file> <path-to-HBM AGENT.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <HBM AGENT.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: HBM_HOME (isolated install) and
  ...
  // The real backend (`hbm serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting HBM AGENT... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.hbmDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating HBM AGENT — this window will close" overlay should appear,
  ...
  // the staged binary), which claims HBM_HOME/.hbm-update-in-progress
  ...
  // proved the desktop had exited and `hbm update` was already running).
  //
  ...
  const hbmHome = process.env.HBM_HOME
  const markerPath = hbmHome ? path.join(hbmHome, '.hbm-update-in-progress') : null
  const resultPath = hbmHome ? path.join(hbmHome, '.hbm-update-result.json') : null
