// @ts-check
/**
 * Launch the HBM AGENT desktop app from a captured launch spec and click the
 * real update flow: Settings -> About -> "Update now".
 *
 * The spec is written by launch-capture/sitecustomize.py at `hbm
...
async function main() {
  ...
  // an "update complete, reopen HBM AGENT to finish" overlay and never exits;
  ...
  const boundedClose = async (application, label) => {
    ...
    // (`hbm serve` python + node helpers) survives and keeps writing
  ...
  phase('relaunch');
  log('relaunching the updated app (the "reopen HBM AGENT" step)');
