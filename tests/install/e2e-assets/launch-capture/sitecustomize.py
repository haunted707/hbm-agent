"""Driver-side spawn interception for hbm desktop E2E legs.

The installed ``hbm`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``hbm
...
Opt-in: ``HBM_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "hbm
...
_SPEC_PATH = os.environ.get("HBM_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/HBM AGENT.exe, linux-unpacked/...,
        # mac*/HBM AGENT.app/Contents/MacOS/...).
