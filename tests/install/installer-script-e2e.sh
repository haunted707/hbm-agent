...
#              install landed on OLD with a working `hbm`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `hbm`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method hbm-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  hbm-update|installer-script|installer-script+desktop|hbm-desktop-app-update) ;;
  *) echo "error: --update-method must be hbm-update, installer-script, installer-script+desktop or hbm-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/hbm-installer-script-e2e"
LOG_DIR="${HBM_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled hbm or a
# developer config, and old installer scripts hardcode $HOME/.hbm (the
# HBM_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export HBM_HOME="$HOME/.hbm"
mkdir -p "$HBM_HOME"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/HBM AGENT" \
    "$release_dir/linux-unpacked/hbm" \
    "$release_dir/mac-arm64/HBM AGENT.app" \
    "$release_dir/mac/HBM AGENT.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local hbm="$INSTALL_DIR/venv/bin/hbm"
  [ -x "$hbm" ] || fail "no hbm console script at $hbm"
  "$hbm" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "hbm --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "hbm --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `hbm desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `hbm update` itself makes. Probe the
  # INSTALLED hbm for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `hbm desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local hbm="$INSTALL_DIR/venv/bin/hbm"
  if ! "$hbm" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "hbm desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$hbm" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "hbm desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "hbm desktop --build-only ($1) exited $rc; transcript above"
  ok "hbm desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  hbm-update)
    ...
    # installed hbm; older ones read the prompt from stdin, so close it.
    HBM="$INSTALL_DIR/venv/bin/hbm"
    if "$HBM" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$HBM" update --yes)
    else
      update_cmd=("$HBM" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "hbm update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "hbm update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  hbm-desktop-app-update)
    # The real user surface: `hbm desktop` launches the app, the user
    ...
    # suite's own) is configured into HBM_HOME exactly like the dev:mock
    ...
    step "capturing the hbm desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      HBM_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$HBM" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "hbm desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "hbm desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "hbm desktop exited 0 but no launch was captured at $SPEC"
    ...
    (cd "$PW_DIR" && node launch-from-spec.mjs \
      --spec "$SPEC" \
      --result "$HBM_HOME/.hbm-update-result.json" \
      --expect-sha "$HEAD_SHA" \
...
mkdir -p "$ildest"
cp -R "$HBM_HOME/logs" "$ildest/hbm-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/hbm/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$HBM_HOME/.hbm-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$HBM_HOME" > "$ildest/hbm-home-ls.txt" 2>/dev/null || true
