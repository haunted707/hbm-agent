...
(``run_agent``, ``hbm_state``, ``toolsets``...) reach the wheel through
...
``ModuleNotFoundError`` on ``import hbm_state``. This test pins the two halves of that
...
PACKAGES = ("agent", "tools", "hbm_cli", "gateway", "tui_gateway", "cron", "acp_adapter", "plugins", "providers")


def _root_py_modules() -> set[str]:
    spec = importlib.util.spec_from_file_location("_hbm_setup_py", REPO_ROOT / "setup.py")
...
def test_every_root_module_imported_by_packaged_code_is_shipped():
    ...
    assert "hbm_state" in shipped and "setup" not in shipped
