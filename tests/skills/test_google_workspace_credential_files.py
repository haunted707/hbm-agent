...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        hbm_home.mkdir()
        (hbm_home / "google_token.json").write_text("{}")
        (hbm_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.hbm/google_token.json" in container_paths
            assert "/root/.hbm/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
