...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["hbm"]["related_skills"]:
        hits = (
...
def test_tools_wired_and_home_not_hardcoded():
    ...
    no-change ticks stay silent, and the HBM AGENT home path is resolved, never assumed."""
    _, body = _frontmatter_and_body()
...
def test_frontmatter_blueprint_is_a_valid_installed_blueprint():
    ...
    assert "[SILENT]" in job["prompt"] and "~/.hbm" not in job["prompt"]