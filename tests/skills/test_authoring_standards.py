...
def test_required_frontmatter_fields(p):
    ...
    hbm = (fm.get("metadata") or {}).get("hbm") or {}
    if not (hbm.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.hbm.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    hbm = (fm.get("metadata") or {}).get("hbm") or {}
    dangling = [
        rs for rs in (hbm.get("related_skills") or []) if rs not in _all_names()
    ]
