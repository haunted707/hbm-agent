...
def test_cli_keeps_discovered_home_when_launch_selection_changes(tmp_path, monkeypatch, profile):
    ...
    token = set_hbm_home_override(str(root))
    ...
    def run(argv, **kwargs):
        ...
        code = ('import json,sys; sys.argv=["hbm"]+json.loads(sys.argv[1]); '
                'import hbm_cli.main; from hbm_constants import get_hbm_home; '
                'print(json.dumps(str(get_hbm_home())))')
        # Everything after the launcher (binary or ``python -m hbm_cli.main``) is the CLI argv.
        cli_argv = argv[3:] if argv[1:3] == ["-m", "hbm_cli.main"] else argv[1:]
    ...
    try:
        ...
        assert seen == [home]
        assert not (tmp_path / ".hbm").exists()
    finally:
