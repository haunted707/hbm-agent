...
- Unlimited timeout (HBM_CRON_TIMEOUT=0)
- Backward compat: HBM_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """HBM_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """HBM_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("HBM_CRON_TIMEOUT", "1200")
        raw = os.getenv("HBM_CRON_TIMEOUT", "").strip()
...
class TestSysPathOrdering:
    ...
    def test_hbm_time_importable(self):
        """hbm_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _hbm_now
        assert callable(_hbm_now)
