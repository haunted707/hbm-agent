...
@pytest.mark.parametrize("error", [None, subprocess.TimeoutExpired("hbm", 1)])
def test_cli_owner_deferral_and_attempt_fence(tmp_path, monkeypatch, error):
    ...
    monkeypatch.setattr(delivery.shutil, "which", lambda _: "/bin/hbm")
...
def test_delivery_exception_retains_attempt_and_continues_siblings(tmp_path, monkeypatch):
    ...
    def resolve_cli(name, *args, **kwargs):
        # Delivery resolves the CLI (the running install's ``hbm_cli``) right after discovery.
        nonlocal armed
        armed = True
        return object()
    ...
    def run(*args, **kwargs):
        calls.append(kwargs["env"]["HBM_HOME"])
