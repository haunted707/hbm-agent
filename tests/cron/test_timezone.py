"""
Tests for timezone support (hbm_time module + integration points).
...
def _reset_hbm_time_cache():
    """Reset the hbm_time module cache."""
    hbm_time.reset_cache()


# =========================================================================
# hbm_time.now() — core helper
# =========================================================================

class TestHbmTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_hbm_time_cache()

    def teardown_method(self):
        _reset_hbm_time_cache()
        os.environ.pop("HBM_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["HBM_TIMEZONE"] = "Asia/Kolkata"
        result = hbm_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["HBM_TIMEZONE"] = "UTC"
        result = hbm_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["HBM_TIMEZONE"] = "America/New_York"
        result = hbm_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["HBM_TIMEZONE"] = "Europe/London"
        tz = hbm_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching HBM_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("HBM_TIMEZONE", raising=False)

        monkeypatch.setenv("HBM_HOME", str(first_home))
        assert str(hbm_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch HBM_HOME in one process.
        monkeypatch.setenv("HBM_HOME", str(second_home))
        assert str(hbm_time.get_timezone()) == "America/New_York"
    ...
    def test_multiplex_prefers_routed_profile_config_over_env(self, tmp_path, monkeypatch):
        """Under the multiplexed gateway HBM_TIMEZONE holds only the DEFAULT profile's value
        ...
        monkeypatch.setenv("HBM_TIMEZONE", "America/New_York")

        # Single-profile process: env stays authoritative.
        assert hbm_time.get_timezone_name() == "America/New_York"

        set_multiplex_active(True)
        try:
            assert hbm_time.get_timezone_name() == "America/New_York"  # default profile turn
            token = set_hbm_home_override(str(routed_home))
            try:
                assert hbm_time.get_timezone_name() == "Asia/Tokyo"
                assert str(hbm_time.get_timezone()) == "Asia/Tokyo"
            finally:
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_hbm_home_override(str(homes[key]))
                try:
                    tz = hbm_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When HBM_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) HBM_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("HBM_TIMEZONE=" + os.environ.get("HBM_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "HBM_TIMEZONE=NOT_SET" in result["output"], (
            "HBM_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When HBM_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("HBM_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=hbm_tz) which shifted
        absolute time when system-local tz != HBM AGENT tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when HBM AGENT tz
        ...
        # Use an HBM AGENT timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _hbm_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["HBM_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hbm_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of HBM AGENT timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["HBM_TIMEZONE"] = "US/Eastern"
        _reset_hbm_time_cache()
