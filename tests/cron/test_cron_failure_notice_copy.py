"""User-facing cron failure notices: plain words, the real output path, and the exact `hbm cron`
...
def test_generic_failure_names_runs_and_pause_commands_and_the_real_output_dir():
    ...
    for cmd in ("hbm cron runs ab12cd34", "hbm cron run ab12cd34", "hbm cron pause ab12cd34"):
        assert f"`{cmd}`" in msg
    assert f"{display_hbm_home()}/cron/output/ab12cd34/" in msg
...
def test_auth_failure_points_at_login_and_a_retry_command(monkeypatch):
    ...
    assert "/login" in msg and "`hbm auth add <provider>`" in msg
    assert "hbm login" not in msg  # that command was removed
    assert "`hbm cron run ab12cd34`" in msg
...
def test_rate_and_usage_limit_phrases_still_yield_a_provider_notice(monkeypatch):
    ...
    for text in (
        ...
        assert "`hbm cron run ab12cd34`" in msg or "`hbm cron edit ab12cd34" in msg, msg
...
def test_cron_cause_gloss_is_the_shared_table():
    ...
    for reason in FAILURE_CAUSE_GLOSS:
        ...
        assert notice is not None and "`hbm cron" in notice, reason
...
def test_transient_provider_failures_never_lead_with_jargon(monkeypatch):
    ...
    for err in ("Request timed out.", "HTTP 429: Too Many Requests"):
        ...
        assert "`hbm cron run ab12cd34`" in msg
        assert "`hbm cron runs ab12cd34`" in msg
...
def test_blocked_config_notice_says_it_did_not_run_and_will_self_heal():
    ...
    assert "`hbm cron doctor`" in text
