...
def _fire(home, mode):
    env = {k: v for k, v in os.environ.items()
           if not k.startswith(('HBM_', '_HBM_'))
...
def test_completion_before_occurrence_does_not_prove_slot_completed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(executions, '_hbm_now', lambda: datetime(2026, 1, 1, tzinfo=timezone.utc))
    ...
    monkeypatch.setattr(executions, '_hbm_now', lambda: datetime(2026, 1, 5, tzinfo=timezone.utc))
