...
def test_standalone_fallback_pool_keeps_profile_scope(tmp_path, monkeypatch):
    ...
    from hbm_constants import get_hbm_home, set_hbm_home_override
    ...
    async def _inside_running_loop():
        ...
        set_hbm_home_override(str(sec_home))
...
def test_desktop_ticker_gates_on_profile_gateway_running(tmp_path, monkeypatch, profile_count):
    ...
    running = {homes[-1][1]}
    monkeypatch.setattr(
        "hbm_cli.profiles.profiles_to_serve", lambda multiplex=False: list(homes)
    )
    monkeypatch.setattr(
        "hbm_cli.profiles._check_gateway_running", lambda home: home in running
    )
    monkeypatch.setattr("hbm_cli.profiles._served_by_running_multiplexer", lambda name: False)
    captured = {}
    ...
    monkeypatch.setattr("hbm_logging.enable_profile_log_routing", lambda homes: None)
