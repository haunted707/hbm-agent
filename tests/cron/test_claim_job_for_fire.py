...
These exercise the real store against a temp HBM_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated HBM_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("HBM_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_hbm_home() reads the env live.
    yield tmp_path
...
def test_unclassified_off_tick_claim_does_not_stamp_a_future_occurrence(temp_home, monkeypatch):
    ...
    monkeypatch.setattr(
        jobs, "_hbm_now", lambda: datetime.fromisoformat(pending) - timedelta(minutes=1))
...
def test_claim_seconds_before_the_slot_owns_it_once(temp_home, monkeypatch):
    ...
    monkeypatch.setattr(jobs, "_hbm_now", lambda: slot_dt - timedelta(seconds=2))
    monkeypatch.setattr(executions, "_hbm_now", lambda: slot_dt - timedelta(seconds=2))
    claimed = jobs.claim_job_for_fire(job["id"], return_job=True)
    ...
    monkeypatch.setattr(jobs, "_hbm_now", lambda: backstop)
    monkeypatch.setattr(executions, "_hbm_now", lambda: backstop)
...
def test_fresh_claim_from_a_dead_same_host_owner_is_reclaimable(temp_home):
    ...
    ``hbm cron run`` killed mid-flight must not block the next manual run for the whole TTL
