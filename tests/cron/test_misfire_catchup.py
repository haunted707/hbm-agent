...
from cron.jobs import _hbm_now, create_job, get_job, load_jobs, save_jobs
...
def _park_in_past(job_id, minutes):
    ...
    for j in jobs:
        if j["id"] == job_id:
            j["next_run_at"] = (
                _hbm_now() - timedelta(minutes=minutes)
            ).isoformat()
    save_jobs(jobs)
...
class TestFireOverdueJobs:
    ...
    def test_claim_advances_next_run_no_refire(self, tmp_cron_dir):
        ...
        assert stamped["next_run_at"] > _hbm_now().isoformat()
    ...
    def test_estop_skips_sweep_and_next_sweep_after_resume_catches_up(
        ...
        """`hbm pause` must silence the backstop too — otherwise it force-fires every job
        that ESTOP held back. Nothing to unwind: the first sweep after `hbm resume`
