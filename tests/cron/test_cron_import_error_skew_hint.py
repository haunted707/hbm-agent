...
Field incident: an interrupted `hbm update` (pull done, restart never ran)
...
had no way to know the fix was one `hbm gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "hbm gateway restart" not in msg
