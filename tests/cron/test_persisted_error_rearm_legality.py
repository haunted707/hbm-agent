...
def cron_store(tmp_path, monkeypatch):
    ...
    (hbm_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("HBM_HOME", str(hbm_home))
    monkeypatch.setattr(J, "HBM_DIR", hbm_home)
    monkeypatch.setattr(J, "CRON_DIR", hbm_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", hbm_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", hbm_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_hbm_now", lambda: now):
            due = J.get_due_jobs()
