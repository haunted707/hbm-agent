...
builds must resolve exactly as it would under a standalone ``hbm -p <name> gateway run``.
"""
...
def two_homes(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text(
        "HBM_CRON_TIMEOUT=111\nHBM_MODEL=default-model\nTERMINAL_ENV=docker\n"
        "TERMINAL_DOCKER_IMAGE=default-only-image\nHBM_LANGUAGE=en\n")
    (alpha / ".env").write_text("HBM_CRON_TIMEOUT=222\nHBM_LANGUAGE=zh\n")
    ...
    for key, val in (("HBM_CRON_TIMEOUT", "111"), ("HBM_MODEL", "default-model"),
                     ("TERMINAL_ENV", "docker"), ("TERMINAL_DOCKER_IMAGE", "default-only-image"),
                     ("HBM_LANGUAGE", "en")):
        monkeypatch.setenv(key, val)
    set_multiplex_active(True)
    home_token = set_hbm_home_override(str(alpha))
    try:
        yield root, alpha
    finally:
...
def test_cron_env_settings_resolve_from_the_served_profile(two_homes):
    """``HBM_CRON_TIMEOUT`` / ``HBM_MODEL`` are alpha's (or absent) both with the fire-time
    ...
    try:
        ...
        try:
            ...
            assert captured["target_model"] == ""  # alpha has no HBM_MODEL; default's must not leak
...
def test_child_env_for_served_profile_drops_launch_profile_settings(two_homes):
    ...
    try:
        ...
        reset_secret_scope(token)
    for key in ("HBM_MODEL", "TERMINAL_ENV", "TERMINAL_DOCKER_IMAGE", "HBM_LANGUAGE", "HBM_CRON_TIMEOUT"):
        assert key not in env, key
    assert env["HBM_HOME"] == str(alpha)
    assert "PATH" in env

    # No-op when the scope IS the launch profile (standalone / default profile's own children).
    reset_hbm_home_override(set_hbm_home_override(None))
    home_token = set_hbm_home_override(str(root))
    try:
        env = strip_launch_profile_env(build_subprocess_env(scrub_secrets=True, inherit_profile_home=True))
    finally:
        reset_hbm_home_override(home_token)
    assert env["TERMINAL_ENV"] == "docker"
    assert env["HBM_MODEL"] == "default-model"
