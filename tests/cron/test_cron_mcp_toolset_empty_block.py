...
def _run(job, tmp_path):
    ...
    with patch("run_agent.AIAgent") as agent_cls, \
         ...
         patch("hbm_state_registry.acquire", return_value=MagicMock()), \
         patch("tools.mcp_tool_discovery.discover_mcp_tools", return_value=[]), \
         patch("hbm_cli.runtime_provider.resolve_runtime_provider", return_value=dict(_RUNTIME)):
...
def test_requested_mcp_server_owned_by_other_profile_blocks_run(tmp_path):
    ...
    from hbm_constants import hbm_home_key, reset_hbm_home_override, set_hbm_home_override

    set_multiplex_active(True)
    token = set_hbm_home_override(tmp_path / "other")
    try:
        undo = _register_notion_in_scope(hbm_home_key())
    finally:
