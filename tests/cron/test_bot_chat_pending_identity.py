...
def test_deferred_destination_does_not_follow_root_changes(tmp_path, monkeypatch, recipient):
    ...
    monkeypatch.setenv("HBM_HOME", str(source))
    monkeypatch.setattr("hbm_cli.profiles.get_profile_dir", lambda _: home)
    ...
    try:
        ...
        lease.release()
        db.close()
    monkeypatch.setattr("hbm_cli.profiles.get_profile_dir", lambda _: other)
    ...
    try:
        with monkeypatch.context() as changed:
            changed.setenv("HBM_HOME", str(tmp_path / "new-source"))
        ...
        if recipient == "cli":
            ...
            assert Path(run.call_args.kwargs["env"]["HBM_HOME"]) == home
