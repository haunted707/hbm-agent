...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_hbm_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "hbm_constants.get_default_hbm_root", lambda: root
    )
    token = set_hbm_home_override(str(grant_home))
