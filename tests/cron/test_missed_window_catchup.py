...
Drives the REAL ``tick()`` against a throwaway HBM_HOME with a ``no_agent`` script job that
...
def slot_env(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HBM_MACHINE_ID", raising=False)

    import cron.executions as E
    import cron.jobs as J
    import cron.scheduler as S

    monkeypatch.setattr(J, "HBM_DIR", home)
    ...
    monkeypatch.setattr(S, "_hbm_home", home)
    ...
    slot = (J._hbm_now() - timedelta(minutes=1)).replace(microsecond=0).isoformat()
    stored = J.load_jobs()
    ...
    def crash_before_dispatch() -> None:
        ...
        env = dict(os.environ, HBM_HOME=str(home))
...
class TestMissedWindowCatchUp:
    def test_slot_lost_before_dispatch_fires_once_after_restart(self, slot_env):
        ...
        assert J._ensure_aware(J.datetime.fromisoformat(after_crash["next_run_at"])) > J._hbm_now()
        ...
        assert J._ensure_aware(J.datetime.fromisoformat(rec["next_run_at"])) > J._hbm_now()
