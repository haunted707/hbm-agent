...
- Path resolution (absolute, relative to HBM_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp HBM_HOME."""
    hbm_home = tmp_path / ".hbm"
    hbm_home.mkdir()
    (hbm_home / "cron").mkdir()
    (hbm_home / "cron" / "output").mkdir()
    (hbm_home / "scripts").mkdir()
    monkeypatch.setenv("HBM_HOME", str(hbm_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "HBM_DIR", hbm_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", hbm_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", hbm_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", hbm_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_missing_script_names_the_profile_folder(self, cron_env):
        ...
        assert "hbm cron edit" in output


    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit HBM AGENT provider env (SECURITY.md §2.3)."""
        from tools.environments.local_env_policy import _HBM_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler_script import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_HBM_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.hbm/scripts/ must be rejected."""
        from cron.scheduler_script import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("HBM_SESSION_PLATFORM") is None
        assert os.environ.get("HBM_SESSION_CHAT_ID") is None
        assert os.environ.get("HBM_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("HBM_CRON_SCRIPT_TIMEOUT", "2")
