...
profile's HBM_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.hbm/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active HBM_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever HBM_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    HBM_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import hbm_constants

    monkeypatch.setattr(
        hbm_constants, "_get_platform_default_hbm_home", lambda: root
    )
    monkeypatch.setenv("HBM_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile HBM_HOME (<root>/profiles/<name>), the cron store
    ...
    assert hbm_constants.get_hbm_home().resolve() == profile_home.resolve()
    assert hbm_constants.get_default_hbm_root().resolve() == root.resolve()

    # cron/jobs.py computes HBM_DIR from get_hbm_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.HBM_DIR.resolve() == profile_home.resolve()
        assert (
