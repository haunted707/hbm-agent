...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(hbm_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "hbm_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("HBM_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("HBM_TEST_RUNTIME_KEY", ""),
            "base_url": None,
