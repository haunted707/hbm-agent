"""Guard: HBM AGENT-owned subprocesses must not resolve managed runtimes by bare PATH.

HBM AGENT installs runtimes for itself — ``uv`` at ``$HBM_HOME/bin/uv``, Node at
``$HBM_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in HBM AGENT's own
...
* when a system copy also exists, the one HBM AGENT does not own wins — which is
...
``iter_hbm_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes HBM AGENT provisions into HBM_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not HBM AGENT-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("hbm_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (hbm_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for an HBM AGENT-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$HBM_HOME/bin (uv) and $HBM_HOME/node are not on an "
        ...
        "  node/npm -> hbm_constants.find_node_executable()\n"
        "  PATH env -> hbm_constants.iter_hbm_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(hbm_constants, helper))


def test_managed_uv_helpers_exist():
    from hbm_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
