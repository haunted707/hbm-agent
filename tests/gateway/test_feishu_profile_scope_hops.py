...
The adapter is constructed and connected inside ``_profile_runtime_scope`` (HBM_HOME override +
...
def _observe(routed_home: Path) -> tuple:
    try:
        ...
        secret = "<unscoped>"
    return ("routed" if Path(get_hbm_home()) == routed_home else "launch", secret)
...
def routed_scope(tmp_path):
    ...
    home_token = set_hbm_home_override(str(routed_home))
