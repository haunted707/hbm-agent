...
def _install_secondary(monkeypatch, runner, stamps):
    ...
    monkeypatch.setattr("hbm_cli.env_loader.hydrate_profile_secret_sources", lambda home: {})
...
async def test_secondary_whatsapp_without_primary_is_reported_unserved(monkeypatch, caplog):
    ...
    # Stamped so `hbm gateway status --profile work` / the dashboard can show the reason.
    ...
    from hbm_cli import gateway_multiplex_served as served
