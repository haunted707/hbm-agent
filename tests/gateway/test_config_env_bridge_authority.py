...
Guards against the 60-vs-500 bug where a stale `.env HBM_MAX_ITERATIONS=60`
...
def _run_gateway_import(hbm_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            "HBM_MAX_ITERATIONS",
            "HBM_AGENT_TIMEOUT",
            "HBM_AGENT_TIMEOUT_WARNING",
            "HBM_TURN_LEASE_TIMEOUT",
            "HBM_SESSION_STALL_TIMEOUT",
            "HBM_GATEWAY_BUSY_INPUT_MODE",
            "HBM_GATEWAY_BUSY_TEXT_MODE",
            "HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "HBM_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    # otherwise intentionally clean so stale HBM AGENT settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
@pytest.fixture
def hbm_home(tmp_path: Path) -> Path:
    home = tmp_path / ".hbm"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(hbm_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(hbm_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(hbm_home, {
        "HBM_AGENT_TIMEOUT": "60",
        "HBM_AGENT_TIMEOUT_WARNING": "30",
        "HBM_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(hbm_home, initial_env={})

    assert env.get("HBM_AGENT_TIMEOUT") == "1800"
    assert env.get("HBM_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("HBM_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(hbm_home: Path) -> None:
    ...
    _write_env(
        hbm_home,
        {"HBM_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(hbm_home, initial_env={})

    assert env.get("HBM_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("HBM_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(hbm_home: Path) -> None:
    ...
    _write_config(hbm_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(hbm_home, initial_env={})

    assert env.get("HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(hbm_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        hbm_home,
        initial_env={"HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
