...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("hbm_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("hbm_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("hbm_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/hbm/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/hbm/profiles/from-source")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("hbm_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("hbm_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/hbm/profiles/nonexistent")
                with patch("hbm_cli.profiles.profile_exists", return_value=False):
                    with patch("hbm_constants.get_hbm_home", return_value=Path("/hbm")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global HBM_HOME
                            assert result == Path("/hbm")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("hbm_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("hbm_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("hbm_constants.get_hbm_home", return_value=Path("/hbm")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
...
class TestRoutingConsultation:
    ...
    def test_routing_consulted_when_source_profile_empty(self, mock_runner, discord_source):
        ...
        with patch("hbm_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("hbm_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/hbm/profiles/routed")
...
class TestNonDiscordProfileRouting:
    ...
    def test_telegram_route_resolves(self, mock_runner, telegram_source):
        ...
        with patch(
            "hbm_cli.profiles.profiles_to_serve",
