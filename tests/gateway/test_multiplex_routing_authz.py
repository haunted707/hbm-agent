...
Every test builds a bare ``GatewayRunner`` with stub adapters against a temp ``HBM_HOME`` and
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("hbm_cli.profiles.profiles_to_serve", return_value=served), \
            patch("hbm_cli.profiles.get_profile_dir", side_effect=lambda n: home / "profiles" / n), \
            patch("hbm_cli.profiles.profile_exists", return_value=True):
