...
async def test_session_hygiene_turn_hold_budget_abandons_streaming_wait(
    ...
    # The idle-timeout copy is the only one that adds the `hbm doctor` pointer.
    assert not any(
        "took too long" in c.lower() and "hbm doctor" in c.lower()
...
async def test_session_hygiene_idle_timeout_still_takes_failure_path(
    ...
    # Behavior witness: idle timeout MUST send the idle-timeout message (with the `hbm doctor` pointer).
    sent_contents = [m["content"] for m in adapter.sent]
    assert any(
        "took too long" in c.lower() and "hbm doctor" in c.lower()
        for c in sent_contents
    ), f"idle timeout must send the took-too-long + hbm doctor message, got: {sent_contents}"
