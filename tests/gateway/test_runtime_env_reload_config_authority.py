...
HBM_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env HBM_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``hbm-agent config get terminal.backend`` still says local.
    """
    hbm_home = tmp_path / ".hbm"
    hbm_home.mkdir()
    (hbm_home / "config.yaml").write_text(
    ...
    (hbm_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
