...
class TestPressureSignalScope:
    ...
    def _cgroup(self, monkeypatch, tmp_path, stat_text):
        ...
        monkeypatch.setattr(cleanup, "_own_cgroup_path", lambda: "/hbm.service")
        real_read_text = acp.Path.read_text

        def read_text(self, *args, **kwargs):
            if str(self) == "/sys/fs/cgroup/hbm.service/memory.high":
                return "6979321856\n"
            if str(self) == "/sys/fs/cgroup/hbm.service/memory.stat":
    ...
    def test_same_cgroup_child_anon_counts_against_the_budget(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("hbm_cli.mem_trim.collect_memory_snapshot", lambda: {"rss_anon_kib": 1_600 * 1024})
...
class TestSalvageFollowups:
    ...
    def test_release_batch_drains_plan_before_trim(self, monkeypatch):
        ...
        import hbm_cli.mem_trim as mem_trim_mod
