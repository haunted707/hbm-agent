...
(HBM_HOME override), not the launch home the module constants were frozen from."""
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (HBM_HOME) and a routed profile B with different config everywhere."""
    a = tmp_path / ".hbm"
    b = a / "profiles" / "b"
...
def test_max_turns_and_fallback_chain_follow_routed_profile(two_homes, monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_hbm_home", a)
...
async def test_gateway_hooks_fire_per_routed_profile(two_homes):
    ...
    try:
        ...
        reset_hbm_home_override(token)
    assert ctx_a.get("seen") == ["hbm_hook_hook-a"]
    assert ctx_b.get("seen") == ["hbm_hook_hook-b"]
