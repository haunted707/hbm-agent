...
(/opt/hbm/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable HBM_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_hbm_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable HBM_HOME."""
    ...
    monkeypatch.setattr(
        "hbm_constants.get_hbm_home", lambda: hbm_home
    )
    ...
    expected = hbm_home / "scripts" / "whatsapp-bridge"
