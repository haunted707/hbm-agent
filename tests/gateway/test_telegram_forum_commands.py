...
async def test_ensure_forum_commands_registers_once():
    ...
    with patch("hbm_cli.commands_platforms.telegram_menu_commands") as mock_menu:
...
async def _menu_build_leaves_loop_free(adapter, run_site):
    ...
    with patch("hbm_cli.commands_platforms.telegram_menu_commands", _blocking_menu), \
            patch("hbm_cli.commands_platforms.telegram_menu_max_commands", lambda: 60), \
