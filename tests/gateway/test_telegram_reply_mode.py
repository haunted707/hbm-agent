...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via HBM AGENT-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    response in a different chat lane than the HBM AGENT topic the session runs
    in. The no-anchor branch must prefer the HBM AGENT topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_hbm_topic_thread_id(self):
