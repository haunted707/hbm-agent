...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("HBM_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("HBM_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("HBM_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("HBM_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("HBM_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
