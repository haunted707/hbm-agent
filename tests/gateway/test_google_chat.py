...
def adapter(tmp_path):
    ...
    ~/.hbm/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.hbm/...)
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the HBM AGENT host. The relay flattens Chat
...
class TestSend:
    ...
    async def test_send_clarify_posts_choice_card(self, adapter):
        ...
        assert buttons[0]["onClick"]["action"]["function"] == "hbm_clarify"
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'HBM AGENT is thinking…' stuck in chat. Race fix via
    ...
    async def test_orphan_typing_cards_reaped_on_completion(self, adapter):
        ...
        'HBM AGENT is thinking…' messages."""
        from plugins.platforms.google_chat.adapter import _TYPING_CONSUMED_SENTINEL
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/hbm-chat"
            ),
        ...
        monkeypatch.setattr("hbm_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("hbm_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("hbm_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "hbm_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "hbm_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "hbm_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "hbm_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
