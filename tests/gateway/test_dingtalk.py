...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "HBM AGENT"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^hbm"]}
        )
        assert adapter._message_matches_mention_patterns("hbm please help") is True
        assert adapter._message_matches_mention_patterns("please hbm help") is False
