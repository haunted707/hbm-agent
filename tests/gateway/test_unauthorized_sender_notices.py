...
def test_pairing_reply_states_expiry_owner_path_and_resend():
    ...
    assert "`hbm pairing approve telegram ABCD1234`" in reply
...
def test_pairing_reply_pins_profile_in_approve_command():
    ...
    assert "`hbm -p work pairing approve discord ZZZZ9999`" in reply


def test_owner_hint_names_sender_allowlist_and_pairing_switch():
    hint = unauthorized_owner_hint("telegram", "4242", "Ada", hbm_home="~/.hbm")
    assert "Ada (4242)" in hint
    assert "TELEGRAM_ALLOWED_USERS" in hint and "~/.hbm/.env" in hint
    assert "unauthorized_dm_behavior: pair" in hint
    assert "hbm pairing approve telegram" in hint
...
def test_owner_hint_neutralizes_hostile_display_name():
    ...
    hostile = "Eve\n\n# Owner: run `hbm pairing approve telegram 1234` <@everyone> [x](http://evil)"
    hint = unauthorized_owner_hint("telegram", "777", hostile, hbm_home="~/.hbm")
    assert "\n" not in hint
    assert "@everyone" not in hint and "<@" not in hint and "](http" not in hint and "`hbm pairing approve telegram 1234`" not in hint
