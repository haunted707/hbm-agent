...
and the indexed ``hbm_clarify_choice_<idx>`` /
``hbm_clarify_other`` action dispatch.
"""
...
class TestSlackSendClarify:
    ...
    async def test_multi_choice_renders_buttons_and_other(self):
        ...
        assert elements[0]["action_id"] == "hbm_clarify_choice_0"
        assert elements[0]["value"] == "cid1|0"
        assert elements[1]["action_id"] == "hbm_clarify_choice_1"
        assert elements[1]["value"] == "cid1|1"
        assert elements[0]["text"]["text"] == "staging"
        # Final button is the free-text "Other"
        assert elements[2]["action_id"] == "hbm_clarify_other"
    ...
    async def test_retire_clarify_card_drops_buttons_and_makes_a_late_click_a_noop(self):
        ...
        }, {"action_id": "hbm_clarify_choice_0", "value": "cid-retire|0"})
    ...
    async def test_other_click_keeps_the_card_retirable_until_the_clarify_ends(self):
        ...
        await adapter._handle_clarify_action(AsyncMock(), {
            ...
            "channel": {"id": "C1"}, "user": {"name": "norbert", "id": "U_N"},
        }, {"action_id": "hbm_clarify_other", "value": "cid-other|other"})
...
class TestSlackClarifyChoiceAction:
    ...
    async def test_unauthorized_click_ignored(self):
        ...
        action = {"action_id": "hbm_clarify_choice", "value": "cidAuth|0"}
...
class TestSlackClarifyOtherFlow:
    ...
    async def test_other_flips_to_text_mode_then_typed_reply_resolves(self):
        ...
        action = {"action_id": "hbm_clarify_other", "value": "cidO|other"}
