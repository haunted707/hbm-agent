...
async def test_run_blocking_executes_on_owned_pool():
    ...
    assert captured["thread"].startswith("hbm-feishu-sdk")
    adapter._shutdown_sdk_executor()
...
async def test_run_blocking_propagates_caller_contextvars(tmp_path, monkeypatch):
    ...
    multiplexed profile's HBM_HOME override is a contextvar, and a worker that lost it
    ...
    adapter = FeishuAdapter(PlatformConfig())
    token = set_hbm_home_override(str(tmp_path / "profiles" / "secondary"))
    try:
        assert await adapter._run_blocking(get_hbm_home_override) == str(tmp_path / "profiles" / "secondary")
    finally:
