...
@pytest.mark.parametrize(
    ("cmdline", "expected"),
    [
        ("/v/python -m hbm_cli.main -p ops-2 gateway run", False),
        ("/v/python -m hbm_cli.main --profile ops2 gateway run", False),
        ("/v/python -m hbm_cli.main -p ops gateway run", True),
        ("/v/python -m hbm_cli.main --profile=ops gateway run", True),
    ],
)
...
@pytest.mark.parametrize("cmdline", [
    "/v/python -m hbm_cli.main --profile=ops gateway run",
    "/v/python -m hbm_cli.main -p ops gateway run",
    "/v/python -m hbm_cli.main --profile ops gateway run",
])
