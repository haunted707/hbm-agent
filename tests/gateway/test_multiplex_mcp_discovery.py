...
async def test_gateway_boot_discovers_mcp_under_every_profile_home(
    ...
    def fake_discover() -> list[str]:
        seen.append((get_hbm_home(), threading.current_thread().name))
        return []
...
async def test_reload_mcp_only_touches_requesting_profile(
    ...
    worker_scope = hbm_home_key(worker_home)
    ...
    monkeypatch.setattr(
        ...
        {"default-srv": hbm_home_key(tmp_path), "worker-srv": worker_scope},
    )
    seen: list[tuple] = []

    def fake_shutdown(*, scope=None) -> None:
        seen.append(("shutdown", scope, get_hbm_home()))

    def fake_discover() -> list[str]:
        seen.append(("discover", get_hbm_home()))
        return []
...
async def test_reload_mcp_formats_scoped_connection_keys_before_refreshing_cached_agents(
    ...
    launch_scope = hbm_home_key(tmp_path / "default")
    worker_home = tmp_path / "profiles" / "worker"
    worker_home.mkdir(parents=True)
    worker_scope = hbm_home_key(worker_home)
...
def test_scope_visibility_rejects_a_foreign_or_differently_authenticated_route(
    ...
    worker_scope = hbm_home_key(tmp_path / "worker")
    launch_scope = hbm_home_key(tmp_path / "default")
...
def test_shared_server_tools_are_callable_and_removed_on_non_owner_reload(
    ...
    worker_home.mkdir(parents=True)
    launch_home.mkdir()
    worker_token = set_hbm_home_override(worker_home)
    previous_multiplex = set_multiplex_active(True)
    worker_scope = hbm_home_key()
    launch_scope = hbm_home_key(launch_home)
    ...
    try:
        ...
        reset_hbm_home_override(worker_token)
...
def test_deregister_scope_kwarg_targets_overlay_and_keeps_plugin_confinement() -> None:
    ...
    reg._plugin_module_scopes["hbm_plugins.p"] = {"/home/p1"}
    reg._caller_module = staticmethod(lambda: "hbm_plugins.p")
