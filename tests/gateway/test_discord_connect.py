...
class FakeBot:
    def __init__(self, *, intents, proxy=None, allowed_mentions=None, **_):
        ...
        self.user = SimpleNamespace(id=999, name="HBM AGENT")
        self._events = {}
        self.tree = FakeTree()
...
async def test_safe_sync_slash_commands_only_mutates_diffs():
    ...
    desired_same = {
        "name": "status",
        "description": "Show HBM AGENT session status",
        "type": 1,
        "options": [],
        "nsfw": False,
        "dm_permission": True,
...
async def test_skill_catalog_scan_runs_off_the_event_loop(monkeypatch):
    ...
    async def _scan_leaves_loop_free(run_site):
        ...
        monkeypatch.setattr("hbm_cli.commands_platforms.discord_skill_commands_by_category", _blocking_scan)
