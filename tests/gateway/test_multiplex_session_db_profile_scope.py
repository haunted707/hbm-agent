...
redirected ``get_hbm_home()`` for that turn.  The rows carried the correct
...
def multiplex_homes(tmp_path, monkeypatch):
    """A root home plus a named profile home, with HBM_HOME on the root.
    ...
    # The suite-wide fixture in conftest re-points ``hbm_state.DEFAULT_DB_PATH``
    ...
    # the hatch is closed and resolution goes through ``get_hbm_home()``,
    # which is what production does.  ``HBM_HOME`` above still keeps that
...
def test_primary_handler_rejected_route_falls_back_and_marks_sentinel(multiplex_homes):
    ...
    async def capture_scope(event):
        seen.append((Path(get_hbm_home()), event.source.profile_route_rejected))
...
def test_crash_marker_from_a_secondary_profile_survives_restart(multiplex_homes):
    ...
    scope = set_hbm_home_override(str(profile))
    try:
        entry = store.get_or_create_session(_profile_source())
        assert store.mark_turn_active(entry.session_key) is not None
    finally:
        reset_hbm_home_override(scope)
...
def test_profile_named_main_keeps_its_own_namespace_and_store(multiplex_homes):
    ...
    scope = set_hbm_home_override(str(main_home))
    try:
