...
closure the PR changed, against a real temp ``HBM_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "hbm_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_hbm_home() / config.yaml`` — point it here.
    monkeypatch.setattr("hbm_constants.get_hbm_home", lambda: hbm_home)
    monkeypatch.setattr("hbm_cli.config.get_hbm_home", lambda: hbm_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_hbm_home", default_home)
    _stub_picker_dependencies(monkeypatch)
