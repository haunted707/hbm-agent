...
async def test_reply_to_bot_under_require_mention_keeps_reply_author(monkeypatch):
    ...
    body = "> <@hbm:example.org> did you check the logs?\n\nhello there"
    ...
    assert msg.reply_to_author_id == "@hbm:example.org"
    assert msg.reply_to_author_name == "hbm"
...
async def test_plain_blockquote_without_reply_is_stripped_whole(monkeypatch):
    ...
    event = _make_event("> @hbm:example.org please summarise\n\nthanks", event_id="$evt_quote")
    await adapter._on_room_message(event)

    msg = adapter.handle_message.await_args.args[0]
    assert msg.reply_to_message_id is None
    assert "@hbm:example.org" not in msg.text
