...
def test_session_store_default_db_uses_runtime_hbm_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime HBM_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    hbm_state before a fixture redirected HBM_HOME used to pin every
    default SessionDB() at the developer's real ~/.hbm/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_hbm_home"
    fake_home.mkdir()
    monkeypatch.setenv("HBM_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "hbm-agent sessions list" in raw["_README"]
