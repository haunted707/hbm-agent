...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_hbm_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("hbm_constants.display_hbm_home", return_value="~/.hbm/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.hbm/profiles/coder/cron/output/" in prompt
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_group_participant_aliases_share_session_key(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HBM_HOME", str(tmp_home))
