...
def adapter():
    ...
    a._client = SimpleNamespace(user=SimpleNamespace(id=99999, name="HbmBot"), guilds=[])
    return a
...
def _capture_skill_registration(adapter, monkeypatch, entries):
    ...
    import hbm_cli.commands_platforms as _hc
    monkeypatch.setattr(
...
async def test_skill_handler_rejects_before_dispatch_for_unauthorized(
    ...
    assert "hbm pairing approve discord" in (
