...
``hbm -p X`` dispatcher would build, and X's notifications are rendered/filtered under X's
config.
"""
...
def served(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text("HBM_MODEL=default-model\nTERMINAL_ENV=docker\n")
    ...
    monkeypatch.setenv("HBM_MODEL", "default-model")
    monkeypatch.setenv("TERMINAL_ENV", "docker")
    monkeypatch.setenv("HBM_KANBAN_DB", str(tmp_path / "board.db"))
...
def test_worker_for_served_profile_gets_its_own_env_and_toolset_pin(served, monkeypatch):
    ...
    monkeypatch.setattr(kbd, "_hbm_argv", lambda: ["hbm"], raising=False)
    kbd._default_spawn(task, str(served.alpha), board=None)

    env = spawned["env"]
    assert env["HBM_HOME"] == str(served.alpha)
    assert "HBM_MODEL" not in env and "TERMINAL_ENV" not in env
...
class RecordingAdapter:
    ...
    async def send(self, chat_id, text, metadata=None):
        ...
        self.sent.append({"text": text, "home": str(get_hbm_home()), "strict": media_delivery_strict()})
