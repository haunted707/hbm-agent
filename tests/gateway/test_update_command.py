...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _hbm_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._hbm_home", tmp_path), \
        ...
        with patch("gateway.run._hbm_home", tmp_path):
            ...
            # project_root will be the real hbm-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_resolve_hbm_bin_module_argv(self):
        """_resolve_hbm_bin uses the running interpreter's module argv when hbm_cli is
        importable, even when PATH also offers a ``hbm`` binary (#111569: a PATH-first
        lookup would re-exec an attacker-planted executable on /update and /restart)."""
        import sys
        from gateway.run import _resolve_hbm_bin

        fake_spec = MagicMock()
        with patch("shutil.which", return_value="/tmp/attacker/hbm"), \
             patch("importlib.util.find_spec", return_value=fake_spec):
            result = _resolve_hbm_bin()

        assert result == [sys.executable, "-m", "hbm_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_hbm_bin_falls_back_to_path_then_none(self):
        """Without an importable hbm_cli the argv degrades to PATH, then to None — never a
        bare ``hbm`` string that a hostile PATH entry could shadow."""
        from gateway.run import _resolve_hbm_bin

        with patch("shutil.which", return_value="/usr/local/bin/hbm"), \
             patch("importlib.util.find_spec", return_value=None):
            assert _resolve_hbm_bin() == ["/usr/local/bin/hbm"]

        with patch("shutil.which", return_value=None), \
             patch("importlib.util.find_spec", side_effect=ImportError):
            assert _resolve_hbm_bin() is None
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._hbm_home", hbm_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/hbm" if x == "hbm" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = hbm_home / ".update_pending.json"
        ...
        assert not (hbm_home / ".update_exit_code").exists()
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting HBM AGENT update" in result
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("HBM_MANAGED", "")
        ...
        # Later steps may legitimately return success ("Starting HBM AGENT
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (hbm_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = hbm_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (hbm_home / ".update_output.txt").write_text("done")
        (hbm_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (hbm_home / ".update_pending.json").write_text(json.dumps(pending))
        (hbm_home / ".update_output.txt").write_text(
    ...
    async def test_cleans_up_on_error(self, tmp_path):
        ...
        pending_path = hbm_home / ".update_pending.json"
        output_path = hbm_home / ".update_output.txt"
        exit_code_path = hbm_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (hbm_home / ".update_pending.claimed.json").exists()
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        assert "HBM AGENT update finished" in sent_text
        assert not pending_path.exists()
        assert not output_path.exists()
        assert not exit_code_path.exists()
    ...
    async def test_failed_update_notice_says_still_running_and_trims_log(self, tmp_path):
        ...
        hbm_home = tmp_path / "hbm"
        hbm_home.mkdir()
        (hbm_home / ".update_pending.json").write_text(
            json.dumps({"platform": "discord", "chat_id": "111", "user_id": "222"}))
        (hbm_home / ".update_output.txt").write_text("x" * 3000 + "\nERROR: pip failed\n")
        (hbm_home / ".update_exit_code").write_text("1")
        mock_adapter = AsyncMock()
        runner.adapters = {Platform.DISCORD: mock_adapter}
        ...
        assert "hbm update" in sent_text and "/update" in sent_text
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        (hbm_home / ".update_pending.json").write_text(json.dumps({
            "platform": "telegram",
            "chat_id": "67890",
            "user_id": "12345",
        }))
        (hbm_home / ".update_output.txt").write_bytes(
        ...
        assert "HBM AGENT update finished" in sent
        assert not (hbm_home / ".update_pending.json").exists()
