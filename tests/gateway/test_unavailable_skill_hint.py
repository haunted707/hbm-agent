...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`hbm_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + HBM_HOME so the real user config is untouched."""
    home = tmp_path / ".hbm"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `hbm skills config` writes).
    monkeypatch.setattr(
    ...
    assert "hbm skills config" in msg
