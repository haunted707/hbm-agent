...
from hbm_cli.heartbeat import HeartbeatManager, HeartbeatState, save_heartbeat, migrate_heartbeat_to_session
...
async def test_watches_read_and_admit_in_each_owner_profile(tmp_path, monkeypatch):
    ...
    class Adapter:
        ...
        async def handle_message(self, event):
            rows.append((event.source.profile, str(get_hbm_home()), event.text))
            self._active_sessions[event.metadata['gateway_session_key']] = True

    runner._adapter_for_source = lambda source: Adapter()
    watch = {}
    for name in ('alpha', 'beta'):
        home = tmp_path / '.hbm' / 'profiles' / name
    ...
    with _profile_runtime_scope(tmp_path / '.hbm' / 'profiles' / 'alpha'):
        await runner._heartbeat_poll_once(watch)
    assert {row[0] for row in rows} == {'alpha', 'beta'}
    assert all(row[1] == str(tmp_path / '.hbm' / 'profiles' / row[0]) for row in rows)
    assert len(watch) == 2
