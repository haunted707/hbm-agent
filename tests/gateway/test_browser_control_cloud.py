...
from hbm_cli.dashboard_auth.ws_tickets import _reset_for_tests, mint_ticket
...
def test_dashboard_ticket_subprotocol_carries_the_same_server_identity(gated_dashboard):
    ...
    assert ws._hbm_ws_subprotocol == _web_server_chat._GATEWAY_WS_PROTOCOL
...
def test_cloud_agent_context_binds_registration_principal_and_transport_family():
    ...
    try:
        ...
        assert get_session_env("HBM_BROWSER_CONTROL_PRINCIPAL") == _principal_digest(
            identity
        )
        assert (
            get_session_env("HBM_BROWSER_CONTROL_TRANSPORT_FAMILY")
            == "cloud-ticket-ws"
        )
    finally:
