...
def test_in_turn_cap_eviction_of_another_profiles_agent_enters_the_owners_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HBM_HOME", str(default_home))
    secret_scope.set_multiplex_active(True)
    home_token = set_hbm_home_override(str(prof_a))
...
def test_default_profile_owner_is_the_root_even_when_launched_under_a_named_profile(tmp_path, monkeypatch):
    """``hbm -p x gateway`` sets HBM_HOME to x's home and serves the default profile as a
    ...
    monkeypatch.setenv("HBM_HOME", str(prof_x))  # launched under x
    secret_scope.set_multiplex_active(True)
    home_token = set_hbm_home_override(str(prof_x))
