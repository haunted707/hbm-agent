...
The slash worker that parses ``/heartbeat`` runs a HbmCLI whose watchdog queues the due prompt
...
@pytest.fixture()
def server(hbm_home):
    with patch.dict("sys.modules", {"hbm_cli.env_loader": MagicMock(), "hbm_cli.banner": MagicMock()}):
...
def _arm_due(key: str):
    from hbm_cli.heartbeat import HeartbeatManager, save_heartbeat
...
def test_notification_poller_fires_due_heartbeat_when_idle(server, session):
    ...
    from hbm_cli.heartbeat import load_heartbeat
...
def test_desktop_poller_leaves_gateway_owned_heartbeat_for_gateway(server, session, hbm_home):
    ...
    sid, _, s = session
    s["source"] = "desktop"
    store = SessionStore(hbm_home / "sessions", GatewayConfig())
...
def test_heartbeat_tick_defers_when_busy_or_not_due(server, session, running, due):
    ...
    if not due:
        ...
        from hbm_cli.heartbeat import save_heartbeat
...
def test_abandon_fire_never_overwrites_a_concurrent_pause(hbm_home):
    from hbm_cli.heartbeat import HeartbeatManager, load_heartbeat
