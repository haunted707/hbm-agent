...
``platform="tui"`` / ``chat_id=HBM_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        hbm_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ``get_default_hbm_root()``, which resolves the process env and
        ...
        token = set_hbm_home_override(str(other_profile_home))
        try:
