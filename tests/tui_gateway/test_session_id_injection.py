...
session id into HBM_SESSION_ID so terminal/execute_code subprocesses can
...
saw an empty HBM_SESSION_ID even though agent_init had set it via
...
def test_set_session_context_injects_agent_session_id(monkeypatch):
    """HBM_SESSION_ID must equal the live agent.session_id after binding."""
    _install_session(
    ...
    assert get_session_env("HBM_SESSION_ID") == "20260722_deadbeef"
