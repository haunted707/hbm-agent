...
Regression for the silent cross-profile secret leak class: ``hbm serve`` hosted many profile
...
only HBM_HOME. And the launch-profile asymmetry: a default-member hosted-room turn in a
...
def two_homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_hbm_home", root)
...
def test_rpc_scope_reaches_llm_oneshot_and_model_options(two_homes, monkeypatch):
    ...
    def fake_oneshot(**kwargs):
        seen["oneshot"] = (Path(os.environ.get("HBM_HOME", "")), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        from hbm_constants import get_hbm_home
        seen["oneshot_home"] = Path(get_hbm_home())
        return "t"
    ...
    def build_payload(ctx, **kwargs):
        from hbm_constants import get_hbm_home
        seen["options"] = (Path(get_hbm_home()), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        return {"providers": []}

    monkeypatch.setattr("hbm_cli.inventory.build_model_options_payload", build_payload)
