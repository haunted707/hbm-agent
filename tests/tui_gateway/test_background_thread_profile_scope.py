...
Under multiplex the HBM_HOME override and secret scope are contextvars bound per turn; a bare
...
def test_auto_title_thread_runs_in_the_turns_profile_scope(served_home, monkeypatch):
    ...
    home_token = set_hbm_home_override(str(b))
...
def test_ws_orphan_reap_tears_down_under_the_sessions_profile(served_home, monkeypatch):
    ...
    assert Path(get_hbm_home()) == a  # the test thread's own context is untouched
