...
def hbm_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves HBM_HOME.
...
def server(hbm_home, monkeypatch):
    ...
    # Pin config resolution to the isolated HBM_HOME. Sibling test
    ...
    # module-level ``_hbm_home = get_hbm_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, hbm_home):
    _write_moa_config(hbm_home, """
moa:
