...
def reload_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(_mcp_discovery, "discover_mcp_tools",
                        lambda: discovered_homes.append(hbm_constants.hbm_home_key()))
...
def test_reload_rediscovers_under_each_live_profile_scope(reload_env):
    ...
    assert hbm_constants.hbm_home_key() in reload_env.discovered_homes
    assert hbm_constants.hbm_home_key(reload_env.profile_b) in reload_env.discovered_homes
