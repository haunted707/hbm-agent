...
from hbm_cli.dashboard_auth.ws_tickets import INTERNAL_PROVIDER, INTERNAL_USER_ID
...
def home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HBM_HOME", str(h))
    return h
...
def test_outbox_drain_returns_each_envelope_once(home):
    ...
    env = bot_relay.enqueue_envelope(
        home, target=target, message="m", sender_profile="default", sender_handle="hbm"
    )
...
def test_deliver_validates_profile_and_runs_transport(home, monkeypatch):
    ...
    assert argv[0].rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("hbm", "hbm.exe")
    assert "Bot Chat" in argv and "--query-file" in argv

    # 'hbm' alias resolves to default
    _result(srv._methods["bot_relay.deliver"](2, {"profile": "hbm", "message": "x"}))
...
def test_deliver_lands_in_live_bot_chat_instead_of_subprocess(home, monkeypatch):
    ...
    def _fake_run(argv, *a, **k):
        ...
        # daemon thread; only the relay's `hbm` CLI spawn is under test.
...
def test_deliver_write_failure_still_removes_tempfile(home, monkeypatch, tmp_path):
    ...
    assert not glob.glob(str(tmp_path / "hbm-relay-dm-*")), "tempfile leaked"
...
def test_deliver_child_env_carries_the_envelope_sender_on_every_attempt(home, monkeypatch, fake_runs, sender, expected):
    """HBM_TURN_AUTHOR on the child comes from the envelope's sender fields alone: the retry gets the same
    ...
    assert all(e["HBM_RELAY_TEST_MARKER"] == "kept" for e in envs)
...
def test_gateway_drains_the_mailbox_the_tools_write_to(tmp_path, monkeypatch, subdir):
    """Both ends of the relay mailbox derive the install root from HBM_HOME with ONE formula.
    The writer side (``message_agent``'s ``_hbm_root``) and the drain side
    (``methods_bot_relay._relay_root``) must agree for a ``profiles/<name>`` home AND for an
    arbitrary subdir of the native ``~/.hbm`` — a split here is silent non-delivery."""
    from tools.bot_mode_probe import _default_home, _hbm_root
    from tui_gateway import methods_bot_relay

    monkeypatch.setenv("HOME", str(tmp_path))
    home = tmp_path / ".hbm" / subdir
    home.mkdir(parents=True)
    monkeypatch.setenv("HBM_HOME", str(home))

    writer_root = _hbm_root(Path(_default_home()))
    target = {"profile": "scout", "handle": "scout", "connection_id": "cloud-1",
              "connection_label": "", "title": "", "description": ""}
    env = bot_relay.enqueue_envelope(
        writer_root, target=target, message="m", sender_profile="default", sender_handle="hbm")
