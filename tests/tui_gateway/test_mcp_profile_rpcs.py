...
``HBM_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def hbm_root(tmp_path, monkeypatch):
    """A temp HBM_HOME root with two named profiles: 'work' and 'other'.

    Pointing HBM_HOME at a dir outside ~/.hbm makes it the profile ROOT
    (get_default_hbm_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "hbm_home"
...
def test_add_lands_in_named_profile_only(hbm_root):
    root = hbm_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(hbm_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(hbm_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_does_not_mix_launch_runtime_into_another_profile(hbm_root):
...
def test_status_includes_named_profile_runtime_in_multiplex(hbm_root):
    ...
    work_token = set_hbm_home_override(hbm_root / "profiles" / "work")
    try:
        work_scope = hbm_home_key()
    finally:
        reset_hbm_home_override(work_token)
...
def test_set_api_key_writes_env_and_header_to_right_profile(hbm_root):
    root = hbm_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(hbm_root):
    root = hbm_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(hbm_root):
    root = hbm_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(hbm_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(hbm_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(hbm_root):
    root = hbm_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == HBM_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
...
def test_test_resolves_env_refs_from_requested_profile_secret_scope(hbm_root, monkeypatch):
