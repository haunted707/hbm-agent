...
launch home, not whatever ``get_hbm_home()`` resolves to at first-touch
...
def launch_db_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_hbm_home", str(launch_home))
...
def test_get_db_first_touch_under_foreign_override_uses_launch_path(launch_db_env):
    ...
    token = set_hbm_home_override(str(foreign_home))
    try:
        db = server._get_db()
...
def test_insights_get_reads_the_requested_profile_store_not_the_launch_handle(launch_db_env, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("hbm_cli.profiles.get_profile_dir", lambda name: profiles_root / name)
