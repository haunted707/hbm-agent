...
def server():
    ...
    # e.g. hbm_cli.active_sessions would bind the mocked get_hbm_home
    ...
    with patch.dict("sys.modules", {
        "hbm_constants": MagicMock(get_hbm_home=MagicMock(return_value="/tmp/hbm_test")),
        "hbm_cli.env_loader": MagicMock(),
        "hbm_cli.banner": MagicMock(),
        "hbm_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `HBM_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("HBM_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("HBM_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_idle_reaper_rearms_missing_ws_orphan_timer(server, monkeypatch, tmp_path, closed_transport):
    ...
    successor = subprocess.run(
        [
            ...
            "-c",
            (
                "from hbm_cli.active_sessions import try_acquire_active_session; "
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/hbm-agent-dev": {"name": "hbm-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "hbm-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient HBM_HOME
    ...
    # The gateway's own HBM_HOME (the test-isolation tempdir, no
...
def test_slash_exec_routes_a_secondary_only_bundle_to_dispatch(server, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HBM_BUNDLES_DIR", raising=False)
    profile_b = tmp_path / "profile_b"
    external_b = tmp_path / "external_b"
    for name in ("one", "two"):
        d = external_b / name
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `hbm config set` does)
    ...
    import hbm_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_hbm_home", lambda: tmp_path)
    ...
    # Activate midnight, as `hbm config set display.skin midnight` would.
