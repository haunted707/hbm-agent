    ...
    with patch.dict(
        "sys.modules",
        {
            "hbm_constants": MagicMock(
                get_hbm_home=MagicMock(return_value="/tmp/hbm_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'hbm-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'hbm-release' patched"
    }
