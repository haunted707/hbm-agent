...
* the Desktop/dashboard backend serves ``?profile=B`` by installing a HBM_HOME override WITHOUT
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HBM_HOME", str(a))
    for key, val in (("A_MARKER", "a"), ("HBM_MODEL", "a-model"), ("FIRECRAWL_API_KEY", "a-fc"),
...
def test_ambient_only_launch_credential_never_reaches_a_routed_child(homes):
    ...
    assert env["HBM_HOME"] == str(b) and env["B_MARKER"] == "b"
...
def test_routed_home_with_multiplex_flag_off_gets_no_launch_residue(homes, monkeypatch):
    """Desktop/dashboard topology: B served via the HBM_HOME override only. The slash-worker /
    helper child sees B's env, and the browser passthrough resolves B's (absent) key as no key."""
    from tools.browser_tool import _build_browser_env

    a, b = homes
    token = set_hbm_home_override(str(b))
    try:
        env = served_profile_child_env(inherit_credentials=True)
        assert env["HBM_HOME"] == str(b) and env["B_MARKER"] == "b"
        assert "A_MARKER" not in env and "HBM_MODEL" not in env and "OPENAI_API_KEY" not in env
        browser_env = _build_browser_env()
        assert "FIRECRAWL_API_KEY" not in browser_env and browser_env["HBM_HOME"] == str(b)
    finally:
    ...
    assert served_profile_child_env(inherit_credentials=True)["HBM_MODEL"] == "a-model"
...
def test_real_child_observes_only_the_routed_profile(homes):
    ...
    try:
        ...
        reset_hbm_home_override(token)
    probe = "import json,os;print(json.dumps({k:os.environ.get(k) for k in ('HBM_HOME','A_MARKER','B_MARKER','OPENAI_API_KEY')}))"
    out = subprocess.run([sys.executable, "-c", probe], env=env, capture_output=True, text=True, encoding="utf-8", timeout=60)
    seen = json.loads(out.stdout.strip().splitlines()[-1])
    assert seen == {"HBM_HOME": str(b), "A_MARKER": None, "B_MARKER": "b", "OPENAI_API_KEY": None}
    assert Path(seen["HBM_HOME"]) == b
