...
* releasing a runtime scope is per-reset best-effort: a failing terminal reset must not leave the
  previous profile's secrets / HBM_HOME installed for the next body in that context.
"""
...
def test_send_keeps_external_source_value_over_raw_dotenv(two_homes, monkeypatch):
    ...
    wins) survives ``_load_hbm_env`` for the routed ``send``."""
    from hbm_cli import env_loader
    from hbm_cli.send_cmd import _load_hbm_env
...
def test_launch_body_survives_first_secondary_activation_on_the_dashboard(two_homes, monkeypatch):
    ...
    from hbm_cli import web_server_profiles as wsp
...
def test_release_resets_every_scope_when_one_reset_fails(two_homes, monkeypatch):
    ...
    assert current_secret_scope() is not None and get_hbm_home_override() == str(b)
    ...
    assert Path(server._hbm_home) == root
