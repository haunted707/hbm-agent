...
by surface:
  ...
  * ``tui_gateway.entry`` — the stdio ``hbm --tui`` path.
  * ``hbm_cli.mcp_startup`` — the desktop app + dashboard WebSocket sidecar
    (``tui_gateway/ws.py``) and ``hbm dashboard``.
...
``hbm_cli.mcp_startup``), so the scheduler bailed immediately and a slow MCP
...
import hbm_cli.mcp_startup as startup
...
def test_entry_in_flight_sees_startup_thread(clean_discovery_globals):
    """Desktop/dashboard surface: discovery thread lives on hbm_cli.mcp_startup.

    The entry-level in-flight check must report True so the late-refresh
    scheduler does not bail (the #51587 bug).
    """
    stop = threading.Event()
    startup._mcp_discovery_thread[hbm_home_key()] = _alive_thread(stop)
    try:
