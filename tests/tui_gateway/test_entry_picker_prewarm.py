...
guard is covered in ``tests/hbm_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``hbm_cli.model_switch_providers.prewarm_picker_cache_async``
...
import hbm_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from hbm_cli.model_switch,
