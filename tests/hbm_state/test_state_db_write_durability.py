...
The defect: hbm_state already knows macOS `fsync()` does not guarantee
...
import hbm_state
from hbm_state import repair_state_db_schema
from hbm_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in hbm_state_repair; hbm_state only
    # re-imports them.
    import hbm_state_repair

    source = Path(hbm_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(hbm_state_repair.__file__))
