...
A raw single-process ``pytest tests/hbm_cli/`` used to accumulate every
...
* ``hbm_state_guard._register_test_instance`` adds every successfully
  constructed SessionDB to a WeakSet registry when the
  ``HBM_TEST_ISOLATION`` marker is set (test-isolation runs only).
...
def test_constructed_sessiondb_is_registered(tmp_path):
    ...
    try:
        assert db in hbm_state_guard._test_instance_registry
    finally:
        db.close()
