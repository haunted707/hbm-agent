...
from hbm_state_dbfile import _pread_db_header, iter_deleted_sqlite_sidecar_holders
from tests.hbm_state._wal_generation_harness import (
...
def test_iter_holders_empty_on_non_linux(monkeypatch, tmp_path):
    monkeypatch.setattr(hbm_state.sys, "platform", "win32")
...
def test_iter_holders_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(hbm_state_dbfile.os, "readlink", fake_readlink)
    try:
...
def test_iter_holders_ignores_descriptor_closed_during_scan(tmp_path, monkeypatch, vanish_errno):
    ...
    monkeypatch.setattr(hbm_state_dbfile.sys, "platform", "linux")
    monkeypatch.setattr(
        hbm_state_dbfile,
    ...
    monkeypatch.setattr(hbm_state_dbfile.os, "stat", stat_vanished)
...
def test_write_path_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(hbm_state_readpool.os, "readlink", fake_readlink)
    try:
