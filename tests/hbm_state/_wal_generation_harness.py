...
def write_second_generation(db_path: Path, n_rows: int) -> int:
    ...
    (as any non-hbm opener would), write ``n_rows`` messages and checkpoint them into the main file.
...
_GATEWAY_CHILD = textwrap.dedent(
    ...
    from pathlib import Path
    repo, hbm_home, db_path = sys.argv[1], sys.argv[2], sys.argv[3]
    sys.path.insert(0, repo)
    os.environ["HBM_HOME"] = hbm_home
    import hbm_state_wal
    if hbm_state_wal.is_sqlite_wal_reset_vulnerable():
        hbm_state_wal.is_sqlite_wal_reset_vulnerable = lambda version_info=None: False
    hbm_state_wal.resolve_journal_mode = lambda: "wal"
    from hbm_state import DeletedWalGenerationError, SessionDB
...
def gateway_writer(tmp_path: Path):
    ...
    repo_root = os.path.dirname(os.path.abspath(hbm_state.__file__))
    ...
    with stderr_path.open("w", encoding="utf-8") as stderr:
        proc = subprocess.Popen(
            [sys.executable, "-c", _GATEWAY_CHILD, repo_root, str(hbm_home), str(path)],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=stderr,
            text=True, encoding="utf-8", bufsize=1,
            env={**os.environ, "HBM_STATE_DB_GUARD_BYPASS": "1"},
        )
