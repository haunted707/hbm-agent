...
#88532 made SessionStore follow HBM_HOME; this covers the TUI/agent
...
def test_open_profile_session_db_does_not_fallback_on_open_failure(homes, monkeypatch):
    ...
    monkeypatch.setattr("hbm_state.SessionDB", boom_db)
...
def _stub_rebuild_env(monkeypatch, server, launch) -> list[str]:
    """Neutralize the rebuild's side paths; returns the HBM_HOME seen by each _make_agent call."""
    homes_seen: list[str] = []

    def fake_make_agent(_sid, _key, *, session_db=None, **_kw):
        from hbm_constants import get_hbm_home

        homes_seen.append(str(get_hbm_home()))
...
def test_bot_capability_rebuild_stays_on_the_profile_store(homes, monkeypatch):
    ...
    ``_make_agent`` with neither the session's ``state.db`` handle nor its HBM_HOME, so the replacement
    agent bound the launch ``_get_db()`` handle and built its prompt/skills from the launch profile: every
    later turn of a named-profile bot appended to ``~/.hbm/state.db`` under the same session id while the
