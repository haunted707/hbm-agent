...
from hbm_state_wal import WalUnsupportedError, _detect_cross_vm_fs, apply_wal_with_fallback
...
class TestDetectCrossVmFs:
    ...
    def test_ordinary_filesystems_never_flagged(self, tmp_path, fstype):
        ...
        assert _detect_cross_vm_fs("/home/user/.hbm", mountinfo_path=mi) is False
...
class TestWalRefusalOnCrossVmFs:
    ...
    def _isolate(self, monkeypatch):
        ...
        monkeypatch.setattr(hbm_state_wal, "is_sqlite_wal_reset_vulnerable", lambda *a, **k: False)
        monkeypatch.setattr(hbm_state_wal, "resolve_journal_mode", lambda: "wal")
        hbm_state_wal._cross_vm_warned_paths.clear()
        hbm_state_wal._cross_vm_existing_wal_warned_paths.clear()

    def test_fresh_db_on_cross_vm_fs_gets_delete_and_without_detection_gets_wal(self, tmp_path, monkeypatch):
        monkeypatch.setattr(hbm_state_wal, "_path_on_cross_vm_fs", lambda p: True)
        ...
        monkeypatch.setattr(hbm_state_wal, "_path_on_cross_vm_fs", lambda p: False)
    ...
    def test_existing_wal_db_on_cross_vm_fs_warns_operator_once(self, tmp_path, monkeypatch, caplog,
        ...
        monkeypatch.setattr(hbm_state_wal, "is_sqlite_wal_reset_vulnerable", lambda *a, **k: wal_reset_vulnerable)
        db = tmp_path / "already-wal.db"
        seed = sqlite3.connect(str(db))
        if str(seed.execute("PRAGMA journal_mode=WAL").fetchone()[0]).lower() != "wal":
            seed.close()
        ...
        seed.close()
        monkeypatch.setattr(hbm_state_wal, "_path_on_cross_vm_fs", lambda p: True)
        with caplog.at_level(logging.ERROR, logger=hbm_state_wal.logger.name):
            for _ in range(2):
                conn = sqlite3.connect(str(db))
