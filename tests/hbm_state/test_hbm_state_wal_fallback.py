...
(SQLITE_PROTOCOL — typical on NFS/SMB), HBM AGENT must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``HBM_HOME`` silently lose
...
from hbm_state import SessionDB, format_session_db_unavailable, get_last_init_error
from hbm_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
def _reset_last_init_error():
    ...
    hbm_state._set_last_init_error(None)
    yield
    hbm_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    hbm_state_wal._wal_delete_fallback_failed_paths.clear()
    yield
    hbm_state_wal._wal_fallback_warned_paths.clear()
    hbm_state_wal._wal_delete_fallback_failed_paths.clear()
...
def _assume_fixed_sqlite(monkeypatch):
    ...
    hbm_state_wal._wal_reset_bug_warned_paths.clear()
    yield
    hbm_state_wal._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see hbm_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("hbm_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        hbm_state._set_last_init_error("OperationalError: locking protocol")
        ...
        # a move-it action (`hbm doctor --fix` cannot repair a mount; a WAL-docs link is not
        # something a chat user can act on).
        lead, details = msg.splitlines()
        assert "network" in lead
        assert "local disk" in lead
        assert "hbm doctor --fix" not in lead
    ...
    def test_known_bucket_keeps_table_action_and_appends_network_hint(self):
        ...
        hbm_state._set_last_init_error("OperationalError: disk I/O error")
        msg = format_session_db_unavailable()
