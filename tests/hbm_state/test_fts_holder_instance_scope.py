...
HBM AGENT instances: a main gateway (user ``ubuntu``,
HBM_HOME=/home/ubuntu/.hbm) and a demo gateway (user ``demo``,
HBM_HOME=/home/demo/.hbm).  The demo gateway runs as another user, so
its ``/proc/<pid>/fd`` table is unreadable from the main instance and the
holder scan falls back to ``/proc/<pid>/cmdline`` + ``_looks_like_hbm``.
The demo process's argv matches the HBM AGENT patterns exactly, so the fallback
...
rebuild in ``hbm_state_schema._recover_stale_fts`` was deferred 42 times
...
hbm); a genuine second instance with a DIFFERENT HBM_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* HBM AGENT
...
# token lives under /home/demo/.hbm, the binary name matches the HBM AGENT
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.hbm/hbm-agent/hbm",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + hbm launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.hbm/hbm-agent/venv/bin/python",
    "/home/demo/.hbm/hbm-agent/hbm_cli/main.py",
    "gateway",
]

# A HBM AGENT-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["hbm", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT HBM AGENT home → not a holder of our state.db."""
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["hbm", f"--db={db_path}", "gateway"],
            ["hbm", "checkpoint", f"{db_path}-wal"],
            ["hbm", "--home", our_home, "gateway"],
        ):
            assert hbm_state_holders._looks_like_hbm(argv) or argv[0] == "hbm"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
