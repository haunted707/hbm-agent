"""Plain-language contracts for hbm_state user-facing errors (CLI UX message campaign, cluster D)."""

import hbm_state
from hbm_state import SessionResumeTooLargeError, format_session_db_unavailable
...
def test_db_unavailable_points_to_doctor_without_sqlite_internals():
    hbm_state._set_last_init_error("OperationalError: database is locked")
    try:
...
def test_db_unavailable_unknown_cause_on_network_drive_points_at_moving_not_doctor_fix():
    ...
    assert "hbm doctor --fix" not in text


def test_db_unavailable_without_cause_still_names_doctor():
    hbm_state._set_last_init_error(None)
    text = format_session_db_unavailable(details=True)
    assert "hbm doctor" in text
