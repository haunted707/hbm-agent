...
tests/hbm_state/test_state_db_repair_live_writer_guard.py and are NOT
duplicated here).
...
isolation guard in hbm_state hard-fails on production-shaped paths — never
...
def test_live_writer_probe_detects_real_holder(tmp_path):
    ...
    db = _make_state_db(tmp_path)
    _require_wal(db)
    assert hbm_state_repair._live_writer_holds_db(db) is False
    with _LiveWriter(db):
        assert hbm_state_repair._live_writer_holds_db(db) is True, (
...
def test_quick_snapshot_flags_corrupt_state_db(tmp_path):
    ...
    A fake HBM_HOME carries a truncated state.db plus a config.yaml. The
    ...
    snap_id = create_quick_snapshot(label="conformance", hbm_home=home)
    if snap_id is None:
