...
- files inside the HBM AGENT home tree are repaired with ``chmod u+rw``
...
import hbm_state
from hbm_state import SessionDB, preflight_db_writability
...
def hbm_home(tmp_path, monkeypatch):
    """Isolated HBM_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".hbm"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, hbm_home):
        db = hbm_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, hbm_home):
        db = hbm_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, hbm_home):
        sub = hbm_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, hbm_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, hbm_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, hbm_home):
        db = hbm_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, hbm_home):
        db_path = hbm_home / "state.db"
