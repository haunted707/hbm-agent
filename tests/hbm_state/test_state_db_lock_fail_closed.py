...
work on a file several HBM AGENT processes share (gateway service, the Desktop
app's `hbm serve` backend, CLI sessions, the TUI slash worker):

* `hbm_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `hbm_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
