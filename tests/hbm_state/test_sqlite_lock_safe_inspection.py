"""POSIX advisory locks must survive HBM AGENT's own database inspection.
...
HBM AGENT used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `hbm-agent sessions
...
out across HBM AGENT's inspection calls.
"""
...
from hbm_cli.sqlite_safe_read import (
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_hbm_fail_close", False):
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import hbm_cli.sqlite_safe_read as ssr
