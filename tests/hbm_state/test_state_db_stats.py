"""Tests for state.db health/stats collection (hbm doctor section).

Covers:
- ``hbm_state_dbfile.collect_state_db_stats``: read-only, best-effort stats
  (page_count, freelist, WAL size, journal mode, row counts, FTS presence,
  pending v23 FTS-rebuild bookkeeping).
- ``hbm_state_dbfile.count_db_holders``: best-effort probe for how many processes
  hold the DB file open (/proc on Linux, libproc on macOS; None elsewhere/on error).
- ``hbm_cli.doctor_state._render_state_db_stats``: formatting/threshold helper
...
def test_count_db_holders_sees_open_connection_macos(populated_db):
    # #109641: the doctor's holder count was Linux-only, so `hbm doctor` on the platform with
...
def test_render_warns_on_large_db():
    from hbm_cli.doctor_state import STATE_DB_SIZE_WARN_BYTES, _render_state_db_stats
