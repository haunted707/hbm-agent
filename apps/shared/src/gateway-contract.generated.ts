...
/** One persisted reaction row (``hbm_state_messages.set_message_reaction``); ``seen`` is stamped once announced. */
...
/** One ``hbm_cli/inventory.py::build_models_payload`` provider row (the union of every field the builder sets; ``pricing_pending`` / ``free_tier_pending`` mark the cached-only path). */
...
/** ``hbm_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown, ``"free"``); the sale fields are Nous Portal-only. */
...
/** ``hbm_cli/foreign_sessions_browser.py::list_foreign_sessions`` — ``id`` is an opaque handle, never a path. */
...
/** ``list`` → ``plugins`` + counts; ``toggle`` → ``ok``/``unchanged``/``name``/``plugin``; ``install`` → ``hbm_cli.plugins_cmd.dashboard_install_plugin``'s ok payload; ``update`` → ``ok``/``unchanged``/``sha``. */
...
/** ``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``HbmSkin``). ``{}`` when the skin engine failed to load. Colour maps are token → colour string. */
...
export interface RpcMethods {
  ...
  /** Run ``hbm <argv>`` non-interactively and capture its output; ``blocked`` explains a refusal. */
  ...
  /** Re-read ~/.hbm/.env (CLI /reload parity); built agents keep their pool until /new. */
  ...
  /** Export the transcript to ~/.hbm/sessions/saved (classic /save). */
