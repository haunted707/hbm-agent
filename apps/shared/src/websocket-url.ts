...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update HBM AGENT Desktop and try again.')
    }
...
export interface HbmWebSocketUrlOptions {
...
export function buildHbmWebSocketUrl(options: HbmWebSocketUrlOptions): string {
