...
//   D=$(mktemp -d) && env -u HBM_SHARED_AUTH_DIR HBM_GUEST_ONBOARDING=1 HBM_SKIP_INTRO=1 \
//     HBM_HOME="$D/home" HBM_DESKTOP_USER_DATA_DIR="$D/userdata" hbm desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'hbm-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.HBM_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set HBM_DESKTOP_PYTHON to your installed HBM AGENT Python executable.')
...
for (const dir of ['.hbm/shared', 'electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, HBM_HOME: path.join(sandbox, '.hbm'), HBM_SHARED_AUTH_DIR: path.join(sandbox, '.hbm/shared'),
  HBM_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), HBM_DESKTOP_CWD: path.join(sandbox, 'work'),
  HBM_DESKTOP_HBM_ROOT: root, HBM_DESKTOP_PYTHON: python,
  HBM_DESKTOP_IGNORE_EXISTING: '1', HBM_DESKTOP_APP_NAME: 'HBM AGENT Connector Rehearsal',
  HBM_GUEST_ONBOARDING: '1', HBM_SKIP_INTRO: '1', HBM_DESKTOP_DEV_SERVER: url, HBM_DESKTOP_CDP_PORT: '9344'
})
