...
// no longer bundles the HBM AGENT Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'HBM AGENT.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'HBM AGENT'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'HBM AGENT.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'HBM AGENT'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default HBM_HOME for non-sandboxed runs -- matches main.ts's
// resolveHbmHome(). On Windows it's %LOCALAPPDATA%\hbm; elsewhere
// it's ~/.hbm. The fresh-install sandbox launchFresh() sets its own
// HBM_HOME and never touches this.
const DEFAULT_HBM_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hbm')
  }
  return path.join(os.homedir(), '.hbm')
})()
const VENV_ROOT = path.join(DEFAULT_HBM_HOME, 'hbm-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'hbm-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.HBM_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `HBM AGENT-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `HBM AGENT-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `HBM AGENT-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'HBM AGENT-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.HBM_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.HBM_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const hbmHome = path.join(sandbox, 'hbm-home')
  ...
  env.HBM_DESKTOP_CWD = cwd
  env.HBM_DESKTOP_IGNORE_EXISTING = '1'
  env.HBM_DESKTOP_TEST_MODE = 'fresh-install'
  env.HBM_DESKTOP_USER_DATA_DIR = userDataDir
  env.HBM_HOME = hbmHome
  delete env.HBM_DESKTOP_HBM
  delete env.HBM_DESKTOP_HBM_ROOT
  ...
  console.log(`  HBM_HOME: ${hbmHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hbmHome, 'hbm-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The HBM AGENT Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of hbm_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'hbm-agent', 'hbm_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing HBM AGENT
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + HBM_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  HBM_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
