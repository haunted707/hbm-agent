 ...
 * product name (`HBM AGENT`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/HBM AGENT'
 *
 * This is a hard failure with no obvious cause for the user — `hbm desktop`
 ...
 * (the mac-unpacked HBM AGENT.app bundle) and Windows (win-unpacked), so we
 ...
 * HBM AGENT.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `hbm desktop --build-only` (hbm_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'HBM AGENT.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'HBM AGENT'}.exe`
