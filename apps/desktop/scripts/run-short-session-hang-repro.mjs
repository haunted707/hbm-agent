...
const LEGACY_STORAGE_KEY = 'hbm.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'hbm.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.hbmDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `hbm-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const hbmHome = join(sandbox, 'hbm-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(hbmHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(hbmHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        HBM_DESKTOP_APP_NAME: `HbmShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        HBM_DESKTOP_HBM_ROOT: target.targetRoot,
        HBM_DESKTOP_IGNORE_EXISTING: '1',
        HBM_DESKTOP_USER_DATA_DIR: userData,
        HBM_HOME: hbmHome,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'hbm-short-session-ab-'))
  const prepared = []
