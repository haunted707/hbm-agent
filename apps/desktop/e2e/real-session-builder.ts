 ...
 * AIAgent loop, using the E2E mock provider configured in `hbmHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(hbmHome: string) {
  ...
  static async start(hbmHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(hbmHome)
