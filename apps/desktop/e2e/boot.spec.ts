 ...
 * packaged binary) with a real `hbm serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → hbm serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with HBM AGENT title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('HBM AGENT')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { hbmDesktop?: Record<string, unknown> }).hbmDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → hbm serve is spawned → WS connects → config
