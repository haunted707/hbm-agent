...
// #100406: in a Bot Mode group room a teammate's `@hbm` handoff must give
// the primary profile (internal name `default`) its turn, exactly like
// `@hbm → @code-farmer` already does. The live roster stamps the primary
// row's handle as the bare profile id ("default"), and the mention parser let
// that stamped handle shadow the `@hbm` alias — so the room settled with
// HBM AGENT never driven. The mock inference server scripts each member's line
...
/** The plugin's persisted room log (`hbm.plugin.hbm-bots.group-chats`). */
async function roomLog(page: Page, group: string): Promise<RoomLogEntry[]> {
  return page.evaluate(name => {
    const raw = window.localStorage.getItem('hbm.plugin.hbm-bots.group-chats')
...
test.afterEach(async ({}, info) => {
  ...
  await info.attach('room-log', {
    body: JSON.stringify(await roomLog(fixture.page, 'HBM AGENT, Code Farmer'), null, 2),
  ...
  await info.attach('runtime-source', {
    body: JSON.stringify(
      await fixture.app.evaluate(() => ({
        cwd: process.cwd(),
        argv: process.argv,
        root: process.env.HBM_DESKTOP_HBM_ROOT,
        home: process.env.HBM_HOME
      })),
      null,
      2
    ),
  ...
  await info.attach('desktop-log', {
    body: readFileSync(join(fixture.sandbox.hbmHome, 'logs/desktop.log')),
...
test('a teammate handing off with @hbm drives the primary profile', async () => {
  test.setTimeout(420_000)
  const page = fixture!.page
  const group = 'HBM AGENT, Code Farmer'
  ...
  for (const title of ['HBM AGENT', 'Code Farmer']) {
  ...
  // to @hbm; HBM AGENT' scripted reply is "B". Neither script token carries
  // a literal `@`, so the user send itself never mentions HBM AGENT.
  await composer.fill(
    '@code-farmer Please reply with one line only. ' +
      'E2E_SAY(code-farmer)[{at}hbm Please reply with the letter B.] E2E_SAY(hbm)[B]'
  )
  await composer.press('Enter')

  // Code Farmer's handoff line lands first (the reverse direction is not in
  // question); then the room must NOT settle without HBM AGENT' turn.
  await expect
    .poll(
      async () =>
        (await roomLog(page, group)).some(e => e.from?.name === 'code-farmer' && /@hbm/.test(e.text || '')),
      {
        timeout: 180_000
      }
    )
  ...
  await expect
    .poll(
      ...
      {
        timeout: 180_000,
        message: 'the primary profile (default / @hbm) never took its turn after being @mentioned by a teammate'
      }
    )
    .toBe(true)
  ...
  await composer.fill(
    '@hbm Begin the reverse handoff. E2E_SAY(hbm)[{at}code-farmer Reply with D.] E2E_SAY(code-farmer)[D]'
  )
  await composer.press('Enter')
  await expect
