...
test.beforeAll(async () => {
  scratch = await fs.mkdtemp(path.join(os.tmpdir(), 'hbm-glass-'))
...
test('glass tabs fade the label beneath the close button without repainting the field', async ({ page }) => {
  ...
  await page.evaluate(() => {
    document.documentElement.setAttribute('data-hbm-glass-scope', 'sidebar')
  ...
  await page.evaluate(() => document.documentElement.removeAttribute('data-hbm-glass'))
