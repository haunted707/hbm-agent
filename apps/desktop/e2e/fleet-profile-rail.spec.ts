 ...
 * second gateway, "Homelab", is a REAL second `hbm serve` this spec spawns
 * with its own HBM_HOME, profiles and session token, registered in the v2
...
function findHbmBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'hbm')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['hbm'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('hbm binary not found: create the repo venv (uv sync) or put hbm on PATH')
}
...
/**
 * Spawn a second, fully real `hbm serve` as the remote gateway. Its
 * session token is pinned through HBM_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        HBM_HOME: home,
        HBM_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote hbm serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-HBM AGENT-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote hbm serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.hbmHome, mock.url)
    ...
    seedProfiles(sandbox.hbmHome, ['research'])
