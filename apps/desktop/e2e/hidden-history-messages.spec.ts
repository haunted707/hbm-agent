...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.hbmHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.hbmHome)
  const builder = await RealSessionBuilder.start(sandbox.hbmHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.hbmHome, mock.url)
  fs.appendFileSync(path.join(sandbox.hbmHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
