...
async function seedBot(hbmHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(hbmHome, 'profiles', name)
  fs.mkdirSync(dir, { recursive: true })
...
test.beforeAll(async () => {
  ...
  for (const name of ['alpha', 'beta', 'gamma']) {
    await seedBot(sandbox.hbmHome, mock.url, name)
  }
...
test('file bots into user sections by menu and drag; rename; delete returns them to Unassigned', async () => {
  ...
  const alphaProfile = path.join(fixture!.sandbox.hbmHome, 'profiles', 'alpha', 'profile.yaml')
