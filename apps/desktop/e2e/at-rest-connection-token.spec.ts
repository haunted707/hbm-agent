 ...
 * dir (macOS `~/Library/Application Support/HBM AGENT/connection.json`, Windows
 * `AppData\Roaming\HBM AGENT\connection.json`). Anything that can read the file
 ...
 * `X-HBM AGENT-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add HBM AGENT desktop app (#20059)"), the commit
 ...
 */
const SENTINEL_TOKEN = 'hbm-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'HbmE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-HBM AGENT-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote HBM AGENT gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-hbm-session-token']
 ...
 * backend, so we skip spawning `hbm serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startHbm() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    HBM_DESKTOP_APP_NAME: STABLE_APP_NAME,
    HBM_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { hbmDesktop?: Record<string, unknown> }).hbmDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture HBM AGENT should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { hbmDesktop: any }).hbmDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('with keychain encryption opted IN, a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // HBM_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.hbmHome, needles),
      'the gateway session token leaked into a HBM_HOME file (logs included)',
    ).toEqual([])
