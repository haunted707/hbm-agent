  ...
  test(`unsigned gateway ${status} leaves recovery settings usable`, async () => {
    ...
    const server = http.createServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        if (pathname === '/api/status') {
          ...
          signedIn = true
          res.setHeader('Set-Cookie', 'hbm_session_at=fixture-session; Path=/; HttpOnly; SameSite=Lax')
          res.end('{}')
    ...
    try {
      const launched = await launchDesktop(buildAppEnv(sandbox, {
        HBM_DESKTOP_DEV_SERVER: ''
      }))
      app = launched.app
      const page = launched.page
      await expect(page.getByRole('button', { name: /gateway settings/i })).toBeVisible({ timeout: 60_000 })
      const snapshot = await page.evaluate(() => (window as unknown as DesktopWindow).hbmDesktop.getBootProgress())
      ...
      const gatewayUrl = page.getByPlaceholder('https://gateway.example.com/hbm')
      ...
      await page.evaluate(async () => {
        await Promise.allSettled(Array.from({ length: 20 }, () => (window as unknown as DesktopWindow).hbmDesktop.getConnection()))
      })
      await expect(back).toBeVisible()
      await expect(gatewayUrl).toHaveValue(url)
      ...
      await expect.poll(async () => {
        try {
          return await page.evaluate(() => (window as unknown as DesktopWindow).hbmDesktop.getConnection())
        } catch {
          return null
        }
