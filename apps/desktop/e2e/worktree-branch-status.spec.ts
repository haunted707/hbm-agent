...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'HBM AGENT E2E'], { cwd: repo })
...
function configureRepoCwd(hbmHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(hbmHome, mockUrl)
  fs.appendFileSync(path.join(hbmHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(hbmHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.hbmHome, mock.url, repo)
