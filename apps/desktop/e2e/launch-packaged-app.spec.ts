...
/**
 * E2E smoke tests for the packaged HBM AGENT desktop app.
 *
 ...
 * (credential stripping, isolated HBM_HOME + userData, unique app name).
 *
...
test('window opens with the HBM AGENT title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      hbmDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).hbmDesktop?.hud?.open({ sessionId: null })
  )
