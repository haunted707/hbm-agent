/**
 * Shared E2E fixtures for the HBM AGENT desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → hbm serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `hbm desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `hbm-e2e-${prefix}-${Math.random()}`))
  const hbmHome = path.join(root, 'hbm-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(hbmHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(hbmHome, 'config.yaml')
 ...
 */
export function writeEnvFile(hbmHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(hbmHome, '.env')
 ...
 */
function writeEmptyConfig(hbmHome: string): void {
  const configPath = path.join(hbmHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - HBM_HOME → sandbox hbm-home (isolated config/sessions)
 *  - HBM_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - HBM_DESKTOP_IGNORE_EXISTING=1 → don't pick up `hbm` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - HBM_DESKTOP_HBM_ROOT → REPO_ROOT (dev checkout resolution)
 *  - HBM_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    HBM_HOME: sandbox.hbmHome,
    HBM_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    HBM_DESKTOP_IGNORE_EXISTING: '1',
    HBM_DESKTOP_HBM_ROOT: REPO_ROOT,
    HBM_DESKTOP_APP_NAME: `HbmE2E-${Date.now()}`,
    ...
    HBM_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated HBM_HOME + userData
 * @param env      - the process environment (already has HBM_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.hbmHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via HBM_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`hbm serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.hbmHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { HBM_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to HBM AGENT backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'HBM AGENT.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'HBM AGENT.app', 'Contents', 'MacOS', 'HBM AGENT')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'hbm')
}
 ...
 * progress without spawning a real HBM AGENT backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * HBM_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    HBM_DESKTOP_BOOT_FAKE: '1',
    HBM_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + hbm-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).HBM_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).HBM_DESKTOP_HBM
  delete (env as Record<string, string | undefined>).HBM_DESKTOP_HBM_ROOT
