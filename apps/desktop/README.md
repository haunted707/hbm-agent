...
**The native desktop app for [HBM AGENT](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other HBM AGENT surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to HBM AGENT and hear it back.</td></tr>
...
### Install with HBM AGENT (recommended)

Already have the HBM AGENT CLI? Just run:

```bash
hbm desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing HBM AGENT gateway or install HBM AGENT locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the HBM AGENT Desktop website.](https://hermes-agent.nousresearch.com/).
...
```bash
# throwaway HBM_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the HBM AGENT runtime into `HBM_HOME`
(`~/.hbm`, or `%LOCALAPPDATA%\hbm` on Windows), using the same layout as a
CLI install.
...
- **HBM AGENT** runs as a headless `hbm serve` process and exposes the
...
1. `HBM_DESKTOP_HBM_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `HBM_DESKTOP_HBM`, or `hbm` on `PATH`
5. a system Python that can import the HBM AGENT runtime
...
Desktop supports a managed local backend, explicit remote gateways, and HBM AGENT
...
screen offers **Connect to existing HBM AGENT** before starting the local installer.
...
terminal commands, and file operations run against the remote HBM AGENT host, not
...
transport- or HBM AGENT-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-HBM AGENT-Session-Token`.
...
Boot logs land in `HBM_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\hbm\hbm-agent\.hbm-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\hbm\hbm-agent\venv"
```

> The default HBM AGENT home on Windows is `%LOCALAPPDATA%\hbm`. Set the `HBM_HOME` env var if you've relocated it.
