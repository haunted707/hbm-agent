...
How to build HBM AGENT Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another HBM AGENT surface can also
...
- A **runtime home change** (switching the underlying `HBM_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across HBM AGENT — do not assume
...
`env -u NODE_ENV HBM_GUEST_ONBOARDING=1 HBM_HOME=<tmp>/.hbm HBM_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). To use the portal stand-in, add
`HBM_PORTAL_BASE_URL=http://127.0.0.1:8765 HBM_ANON_API_SECRET=test-secret HBM_SHARED_AUTH_DIR=<tmp>/.hbm/shared`
...
The free tier (a Nous identity with no account, `hbm_cli/anon_auth.py`) reaches the renderer
