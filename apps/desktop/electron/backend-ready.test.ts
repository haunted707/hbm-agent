 ...
 * HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS, clamped to a 45s floor.
 */
...
test('honors a valid HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS override', () => {
  const env = { HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '120000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), 120_000)
})

test('clamps an override below the floor up to the 45s minimum', () => {
  const env = { HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '1000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), MIN_PORT_ANNOUNCE_TIMEOUT_MS)
})

test('rounds a fractional override', () => {
  const env = { HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '60000.7' }
...
test('falls back to the default for malformed / non-positive overrides', () => {
  for (const bad of ['', 'abc', '0', '-5', 'NaN', undefined]) {
    const env = bad === undefined ? {} : { HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: bad }
    assert.equal(
...
test('resolves with the announced port', async () => {
  ...
  child.stdout.emit('data', 'noise before\nHBM_DASHBOARD_READY port=54321\n')
  assert.equal(await p, 54321)
})

test('resolves with a HBM_BACKEND_READY port (headless `serve`)', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'HBM_BACKEND_READY port=43210\n')
...
test('parses the port even when the line arrives split across chunks', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'HBM_DASHBOARD_READY po')
...
test('rejects with the timeout message after the deadline', async () => {
  ...
  await assert.rejects(
    ...
    /Timed out waiting for HBM AGENT backend port announcement \(20ms\)/
  )
})
...
test('a late announcement after timeout does not throw (listeners torn down)', async () => {
  ...
  assert.doesNotThrow(() => {
    child.stdout.emit('data', 'HBM_DASHBOARD_READY port=9999\n')
  })
})
...
function mkTmpReadyFile() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'hbm-ready-test-'))
...
test('exit-before-announcement error carries the buffered output tail (stdout path)', async () => {
  ...
  const wait = waitForDashboardPortAnnouncement(child, {
    describeOutputTail: () => '\nRecent backend output:\nModuleNotFoundError: hbm_cli'
  })

  child.emit('exit', 1, null)

  await assert.rejects(wait, /exited before port announcement \(1\)[\s\S]*ModuleNotFoundError: hbm_cli/)
})

test('exit-before-announcement error carries the buffered output tail (ready-file path)', async () => {
  const child = makeFakeChild()
  const readyFile = path.join(os.tmpdir(), `hbm-ready-${process.pid}-${Date.now()}.json`)
...
test('bufferedOutput accepts the legacy HBM_DASHBOARD_READY sentinel too', async () => {
  const child = makeFakeChild()

  const port = await waitForDashboardPortAnnouncement(child, {
    bufferedOutput: () => 'HBM_DASHBOARD_READY port=43212\n',
    timeoutMs: 500
  })
...
test('bufferedOutput without a sentinel still resolves from later live stdout', async () => {
  ...
  child.stdout.emit('data', Buffer.from('HBM_BACKEND_READY port=43213\n'))
...
test('the merged-tail seed recovers a sentinel spliced onto a partial stderr line (#103792)', async () => {
  ...
  const port = await waitForDashboardPortAnnouncement(child, {
    bufferedOutput: () => 'INFO  Started server process [4711]HBM_BACKEND_READY port=65238',
    timeoutMs: 500
  })
...
test('the merged-tail seed does not match prose that merely names the sentinel', async () => {
  ...
  const wait = waitForDashboardPort(
    ...
    50,
    () => '',
    () => 'still waiting for HBM_BACKEND_READY from the backend\n'
  )
