...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_hbm_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\HBM AGENT\\hbm-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\HBM AGENT', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\HBM AGENT', 'hbm-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'hbm-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\HBM AGENT', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'hbm-setup.exe',
      options: { cwd: 'C:\\HBM AGENT', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\hbm\\AppData\\Local\\hbm'
  const staged = path.join(home, 'hbm-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when hbm-setup is staged (#74836)', () => {
  const home = '/Users/hbm/.hbm'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages hbm-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\hbm\\AppData\\Local\\hbm', {
    fileExists: () => false,
    isWindows: true
  })
...
test('resolveUpdateScriptHandoff prefers the repo script on Windows when present', () => {
  const root = String.raw`C:\Users\hbm\AppData\Local\hbm\hbm-agent`
...
test('resolveUpdateScriptHandoff returns null when the checkout predates the script', () => {
  const handoff = resolveUpdateScriptHandoff(String.raw`C:\Users\hbm\AppData\Local\hbm\hbm-agent`, {
    isWindows: true,
    fileExists: () => false
  })
...
test('resolveUpdateScriptHandoff is Windows-only (POSIX updates in place)', () => {
  const handoff = resolveUpdateScriptHandoff('/home/hbm/.hbm/hbm-agent', {
    isWindows: false,
    fileExists: () => true
  })
...
test('resolvePosixScriptHandoff returns the bash recipe when the script exists', () => {
  const root = '/home/hbm/.hbm/hbm-agent'
...
test('resolvePosixScriptHandoff is null when the checkout predates the script', () => {
  const handoff = resolvePosixScriptHandoff('/home/hbm/.hbm/hbm-agent', {
    isWindows: false,
    fileExists: () => false
  })
...
test('resolvePosixScriptHandoff is null on Windows', () => {
  const handoff = resolvePosixScriptHandoff(String.raw`C:\Users\hbm\AppData\Local\hbm\hbm-agent`, {
    isWindows: true,
    fileExists: () => true
  })
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--no-sandbox',
    'hbm://open/session/abc',
    '--profile=work'
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'hbm://open/session/abc', '--profile=work'])
...
test('describeUpdaterHandoffFailure leads with plain copy and confines the raw outcome to Details', () => {
  for (const raw of ['updater exited 127 before the settle window elapsed', 'updater spawn failed: ENOENT']) {
    ...
    assert.match(lead, /HBM AGENT keeps running/)
    assert.match(lead, /Try again/)
    assert.doesNotMatch(lead, /exited|spawn|ENOENT|settle window|hbm update|\d/)
