 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `hbm update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.hbmHome, 'HBM AGENT home')
  const hbmPath = validateRemoteValue(target.hbmPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(hbmPath)

  const updateCommand =
    `env HBM_HOME=${homeWord} ` +
    `HBM_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'HBM_UPDATE_ORIGIN_PROFILE=default ' +
    `HBM_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `HBM_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:HBM_HOME=${psLiteral(home)}`,
    `$env:HBM_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:HBM_UPDATE_ORIGIN_PROFILE="default"',
    `$env:HBM_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:HBM_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:HBM_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:HBM_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:HBM_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(hbmPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.hbm-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote HBM AGENT updated and every managed SSH profile is ready.'
        : restoreOk
