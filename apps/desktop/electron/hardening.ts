...
// Relative, not `@hbm/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so HBM AGENT Desktop cannot save remote gateway tokens. ' +
        ...
        'or set HBM_DESKTOP_REMOTE_URL and HBM_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set HBM_DESKTOP_REMOTE_URL and HBM_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
