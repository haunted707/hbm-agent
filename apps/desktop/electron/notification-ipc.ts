...
export function registerNativeNotifications({ getMainWindow, focusWindow }: NotificationHost): void {
  ...
  ipcMain.handle('hbm:notify', (event, payload: HbmNotification) => {
    ...
    notification.on('click', () => {
      ...
      if (focusSessionId) {
        window.webContents.send('hbm:focus-session', focusSessionId)
      }

      if (payload?.activate || payload?.notifyId) {
        window.webContents.send('hbm:notification-activate', {
    ...
    notification.on('action', (actionEvent, index) => {
      ...
      if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
        ...
        window.webContents.send('hbm:notification-action', { sessionId: payload.sessionId, actionId: action.id })
