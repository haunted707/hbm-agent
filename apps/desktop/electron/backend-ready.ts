...
// `hbm serve` announces HBM_BACKEND_READY; the legacy `hbm dashboard`
// backend announces HBM_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^HBM_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
...
// spliced onto them (`...process [4711]HBM_BACKEND_READY port=65238`) and `^` never lines up
// (#103792). Match on a token boundary instead; `port=<digits>` keeps prose mentions out.
export const READY_IN_MERGED_OUTPUT_RE = /(?<!\w)HBM_(?:BACKEND|DASHBOARD)_READY port=(\d+)/

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `hbm_cli.main` → `web_server` → FastAPI/
 ...
 * HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.HBM_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `HBM_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(
  ...
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`HBM AGENT backend: exited before port announcement (${signal || code})${describeOutputTail()}`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for HBM AGENT backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
