 ...
 *      top of startHbm() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.hbm/active_profile',
    hbmHome: '/home/u/.hbm',
    profilesRoot: '/home/u/.hbm/profiles',
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.hbm/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.hbm/profiles', ['coder'], {
      ...fs,
      isHbmProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-hbm) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live hbm process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.hbm/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.hbm/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isHbmProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.hbm/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.hbm/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.hbm/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.hbm/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.hbm/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.hbm/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isHbmProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.hbm/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.hbm/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isHbmProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.hbm/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.hbm/profiles/default': { dir: true },
    '/home/u/.hbm/profiles/coder': { dir: true },
    '/home/u/.hbm/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.hbm/profiles/default': { dir: true },
    '/home/u/.hbm/profiles/some-file.txt': { dir: false },
    '/home/u/.hbm/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $HBM_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.hbm/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.hbm/profiles/coder': { dir: true },
    '/home/u/.hbm/profiles/writer': { dir: true },
    '/home/u/.hbm/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.hbm/active_profile': { content: 'coder' },
    '/home/u/.hbm/profiles/coder': { dir: true },
    '/home/u/.hbm/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.hbm/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.hbm/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.hbm/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.hbm/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.hbm/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.hbm/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isHbmProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.hbm, not ~/.hbm/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at hbmHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.hbm', '/home/u/.hbm/profiles'), '/home/u/.hbm/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.hbm', '/home/u/.hbm/profiles'),
    '/home/u/.hbm/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at hbmHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.hbm', '/home/u/.hbm/profiles'),
    '/home/u/.hbm/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.hbm', '/home/u/.hbm/profiles'),
    '/home/u/.hbm/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at hbmHome', () => {
  const fs = makeFs({
    '/home/u/.hbm/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.hbm/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.hbm/profiles', ['default', 'coder'], {
      ...fs,
      hbmHome: '/home/u/.hbm',
      isHbmProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.hbm/profiles/<name>, never scored ~/.hbm/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.hbm/profiles/conduit': { dir: true },
    '/home/u/.hbm/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.hbm/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.hbm/profiles/work': { dir: true },
    '/home/u/.hbm/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.hbm/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.hbm/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.hbm/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isHbmProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
