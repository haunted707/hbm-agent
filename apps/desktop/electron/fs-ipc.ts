...
// hardening, HBM_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('hbm:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('hbm:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('hbm:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('hbm:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<HBM_HOME>/desktop-plugins`,
  // resolved from the main-process HBM_HOME (see resolveHbmHome) — NOT from
  // the connected backend. A remote backend reports its own `hbm_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(hbmHome, 'profiles', profile) : hbmHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(hbmHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(hbmHome, root)
    await reconcileUnifiedDesktopHalves(hbmHome, root)

    return root
  }

  ipcMain.handle('hbm:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('hbm:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(hbmHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(hbmHome, root)
  })

  // The LOCAL logs root (`<HBM_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where HBM_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('hbm:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('hbm:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('hbm:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('hbm:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('hbm:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('hbm:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
