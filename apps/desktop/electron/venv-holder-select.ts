 ...
 *
 * The pre-update handoff kills HBM AGENT-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `hbm`, unrelated scripts) must NOT be
...
/**
 * True when a process is a HBM AGENT-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives HBM AGENT and holds venv shims mapped).
 */
export function isHbmOwnedVenvDaemon(
