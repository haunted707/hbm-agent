...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.hbm-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing HBM AGENT', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('HBM_HOME')) {
          return '/home/alice/.hbm\n'
        }

        if (command.includes('.hbm-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.hbm/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.hbm-update-in-progress/)
})
...
test('listRemoteHbmProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/HBM_HOME/, '/Users/zillajr/.hbm\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteHbmProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteHbmProfiles rejects a hostile HBM_HOME', async () => {
  const ssh = fakeSsh([[/HBM_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteHbmProfiles(ssh),
    (err: any) => {
...
test('locateHbm prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/hbm/, 'OK']])
  assert.equal(await locateHbm(ssh, '/opt/hbm'), '/opt/hbm')
})

test('locateHbm throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong hbm" bug.
  const ssh = fakeSsh([
    [/command -v hbm/, '/home/u/.local/bin/hbm\n'],
    [/\[ -x .*\.local\/bin\/hbm/, 'OK']
  ])

  await assert.rejects(
    () => locateHbm(ssh, '/bad/path/hbm'),
    (err: any) => {
      assert.equal(err.kind, 'hbm-not-found')
      assert.match(err.message, /\/bad\/path\/hbm/)
...
test('locateHbm falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v hbm/, '/home/u/.local/bin/hbm\n'],
    [/\[ -x .*\.local\/bin\/hbm/, 'OK']
  ])

  assert.equal(await locateHbm(ssh, ''), '/home/u/.local/bin/hbm')
})

test('locateHbm preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$HBM_BIN" "$HBM_ENTRYPOINT" "$@",
  // where $HBM_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.hbm/hbm-agent/venv/bin/python\n']
  ])

  assert.equal(await locateHbm(ssh, ''), '/home/u/.local/bin/hbm')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateHbm must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateHbm returns an explicit remoteHbmPath unchanged', async () => {
  // The override half of #74411: an explicit remoteHbmPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/hbm changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/hbm/, 'OK'],
    [/python3 -c/, '/home/u/.hbm/hbm-agent/venv/bin/python\n']
  ])

  assert.equal(await locateHbm(ssh, '~/.local/bin/hbm'), '~/.local/bin/hbm')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteHbmPath must never be rewritten')
})

test('locateHbm falls back to ~/.local/bin/hbm when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateHbm(ssh, ''), '~/.local/bin/hbm')
})

test('locateHbm tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/hbm/, 'OK']])
  assert.equal(await locateHbm(ssh, ''), '~/.hbm/hbm-agent/venv/bin/hbm')
})

test('locateHbm throws a hbm-not-found error with an install hint', async () => {
...
test('locateHbm uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v hbm/, '/x/hbm'],
    [/\[ -x/, 'OK']
  ])
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.hbm/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.hbm/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.hbm/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/hbm'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/hbm serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/hbm'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/hbm', '/Users/cd9c/.hbm', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /hbm-agent.*venv.*bin.*hbm/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'hbm wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'hbm')
    const launcher = path.join(temp, 'hbm launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/hbm-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/hbm', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('HBM_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/hbm', 'work', {
    hbmHome: '~/.hbm',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.hbm-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"hbm-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /hbm-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'hbm-update-mutex-'))
  const hbmPath = path.join(directory, 'hbm')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *hbm-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(hbmPath, '', {
      hbmHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { hbmPath: '/x/hbm', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('HBM_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('HBM_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]HBM_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { hbmPath: '/x/hbm', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'HBM_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'HBM_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.hbm\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'HBM AGENT v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'HBM_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile hbmPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ hbmPath: '/old/stale/hbm', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteHbmPath: '/new/hbm', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'HBM_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes hbmHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/HBM_HOME/, '/home/alice/.hbm\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'HBM_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"hbmHome":"\/home\/alice\/\.hbm"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'HBM_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'HBM_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/hbm'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.hbm/hbm-agent/venv/bin/hbm'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/hbm'))
  assert.doesNotThrow(() => validateRemotePath('~/.hbm/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/hbm'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('hbm'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/hbm'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/hbm\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/hbm\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/hbm\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/hbm', '/usr/`id`/hbm', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.hbm/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/hbm')
  assert.ok(result.includes('/usr/local/bin/hbm'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/hbm')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('HBM_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/hbm', 'work', {
    tokenFilePath: `~/.hbm/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.hbm\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/hbm', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing HBM AGENT', () => {
  const cmd = buildSpawnCommand('/x/hbm', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env HBM_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { hbmPath: '/x/hbm', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { hbmPath: '/x/hbm', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.hbm/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.hbm/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/hbm serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/hbm serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/hbm'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/hbm'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/hbm'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/hbm'), false)
})
...
test.skipIf(process.platform === 'win32')('capability probe survives a zsh login shell on the remote (#111949)', async t => {
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), 'hbm-zsh-probe-'))

  try {
    const hbm = path.join(dir, 'hbm')
    await writeFile(hbm, '#!/bin/sh\necho "--ssh-session-token-file --ssh-owner-nonce"\n', { mode: 0o700 })

    const ssh = { exec: async (command: string) => (await exec(command, { shell: zsh })).stdout }

    assert.equal(await remoteSupportsSshOwnership(ssh, hbm), true)
  } finally {
...
test('probes run under the remote watchdog so a hung CLI cannot orphan (#110478)', async () => {
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'HBM AGENT v0.18.2 (abc123)\n'
      }
    ]
  ])

  assert.equal(await probeHbmVersion(versionSsh, '/x/hbm'), 'HBM AGENT v0.18.2 (abc123)')
  ...
  assert.equal(await remoteSupportsSshOwnership(helpSsh, '/x/hbm'), true)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'hbm-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.hbm/.hbm-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.hbm/desktop-ssh/${OWNERSHIP_ID}`
...
test('connect() post-spawn cleanup that cannot prove ownership keeps the original boot error', async () => {
  ...
  await assert.rejects(
    connect(connectDeps(ssh, { platform: { os: 'Linux', arch: 'x86_64' }, waitForHbm: async () => { throw boot } })),
