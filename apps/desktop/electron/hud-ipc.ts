...
export function registerHudIpc({
  ...
  ipcMain.on('hbm:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('hbm:hud:windowing', event => {
  ...
  ipcMain.on('hbm:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('hbm:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('hbm:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('hbm:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('hbm:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('hbm:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('hbm:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('hbm:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('hbm:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('hbm:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('hbm:hud:close', async () => {
    closeHudWindow()
