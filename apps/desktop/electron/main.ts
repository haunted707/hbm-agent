...
import { buildDesktopBackendEnv, hbmManagedNodePathEntries, normalizeHbmHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForHbmReady } from './backend-health'
import { backendCommandMatches, createBackendOwnership, createBackendShutdownCoordinator } from './backend-ownership'
import { canImportHbmCli, PROBE_TIMEOUT_MS, shouldTrustHbmOverride, verifyHbmCli } from './backend-probes'
...
import { isHbmOwnedVenvDaemon } from './venv-holder-select'
...
const USER_DATA_OVERRIDE = process.env.HBM_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.HBM_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.HBM_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with HBM_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[hbm] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[hbm] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. HBM_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[hbm] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[hbm] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via HBM_DESKTOP_PASSWORD_STORE, bridged by the
// `hbm desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[hbm] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[hbm] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `hbm desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose HBM AGENT sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[hbm] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[hbm] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[hbm] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[hbm] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[hbm] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('hbm:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized HBM AGENT burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[hbm] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[hbm] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[hbm] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[hbm] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// HBM_HOME — the user-facing root for everything HBM AGENT-related. Mirrors
// scripts/install.ps1's $HbmHome and scripts/install.sh's $HBM_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\hbm (matches install.ps1)
//   macOS / Linux: ~/.hbm (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.hbm directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\hbm yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// HBM_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// HBM_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.hbm / %LOCALAPPDATA%\hbm.
function resolveHbmHome() {
  if (process.env.HBM_HOME) {
    return normalizeHbmHomeRoot(process.env.HBM_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'hbm-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a HBM_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\hbm and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('HBM_HOME')

    if (fromRegistry) {
      return normalizeHbmHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'hbm')
    const legacy = path.join(app.getPath('home'), '.hbm')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.hbm setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.hbm')
}

const HBM_HOME = resolveHbmHome()

function pathWithHbmManagedNode(...entries) {
  const managed = hbmManagedNodePathEntries(HBM_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_HBM_ROOT — the canonical mutable HBM AGENT install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_HBM_ROOT = path.join(HBM_HOME, 'hbm-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_HBM_ROOT, 'venv')
...
// boots, going straight to `resolveHbmBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_HBM_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HBM_ROOT, '.hbm-bootstrap-complete')
...
// active-profile.json records which HBM AGENT profile the desktop launches its
// local backend as. When set, startHbm() passes `hbm --profile <name>
// dashboard …`, which deterministically pins HBM_HOME (see
// _apply_profile_override in hbm_cli/main.py) and bypasses the sticky
// ~/.hbm/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors hbm_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// hbmDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under HBM_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by hbm_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(HBM_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (hbm_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.HBM_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.HBM_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.HBM_DESKTOP_SKIP_QUIT_CONFIRM === '1'
// Nous free tier gate, decided ONCE here and stamped onto every backend spawn
// (desktopBackendSpawnEnv) and the renderer (hbm:launch-flags).
const GUEST_ONBOARDING = guestOnboardingEnabled()
const SKIP_INTRO = skipIntroEnabled()

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.HBM_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.HBM_DESKTOP_APP_NAME || 'HBM AGENT'
...
// 'hbm:native-theme' ('dark' | 'light' | 'system'); we pin
...
// an in-place `hbm update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveHbmVersion(),
...
// startHbm(); this pool only holds EXTRA profile
...
// The legacy HBM_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.HBM_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.HBM_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `HBM AGENT backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `hbm:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.HBM_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startHbm() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning hbm serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start HBM AGENT backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (hbmLog.length > 300) {
    hbmLog.splice(0, hbmLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-hbm-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('hbm:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the hbm:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('hbm:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_HBM_ROOT
  })
}
...
// The Tauri updater writes HBM_HOME/.hbm-update-in-progress for the whole
...
// (`force_kill_other_hbm`, taskkill /IM hbm.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(HBM_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--hbm-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `hbm update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — HBM AGENT will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(HBM_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      const handoffLogPath = path.join(HBM_HOME, 'logs', 'desktop-update-handoff.log')
      ...
      // and the same reveal primitive as 'hbm:logs:reveal'.
      void dialog
        .showMessageBox({
          type: 'error',
          title: 'HBM AGENT update',
          message: "HBM AGENT couldn't finish updating",
          detail:
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting HBM AGENT to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `hbm` — must not shadow `hbm.cmd`/`hbm.exe`.
...
async function unwrapWindowsVenvHbmCommand(command, backendArgs) {
  return resolveVenvHbmCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `hbm serve`; runtimes older than serve only have `dashboard`. We
...
const backendSupportsServe = createBackendServeSupportResolver(HBM_HOME, rememberLog)
...
function isHbmSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'hbm_cli', 'main.py'))
}

async function findPythonForRoot(root) {
  const override = process.env.HBM_DESKTOP_PYTHON
...
async function findSystemPython() {
  ...
  // CPython but several HBM AGENT deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves HBM_GIT_BASH_PATH
...
// normal HBM_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\hbm\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'hbm', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'hbm', 'git', 'bin', 'git.exe'))
  }
...
function recentHbmLog() {
  return hbmLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's hbm root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_HBM_ROOT.
// HBM_DESKTOP_HBM_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.HBM_DESKTOP_HBM_ROOT && path.resolve(process.env.HBM_DESKTOP_HBM_ROOT),
    !IS_PACKAGED && isHbmSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isHbmSourceRoot(ACTIVE_HBM_ROOT) ? ACTIVE_HBM_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_HBM_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('hbm:updates:progress', merged)
  }
}
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of HBM AGENT can't update itself from inside the app. Download the latest version from the HBM AGENT website, " +
        `or reinstall HBM AGENT to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
      hbmRoot: updateRoot,
      branch
    }
  }
...
// #105855 was a run of GitHub outages that read as a HBM AGENT bug because the
// UI hid the cause.
    ...
    const req = https.get(
      url,
      {
        headers: {
          ...
          'User-Agent': 'hbm-desktop-update-check'
        },
        timeout: 10_000
      },
      res => {
...
// Windows that binary owns ALL repo mutation — running `hbm update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(HBM_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `hbm update` can write
// fresh entry points. On Windows this is the file the running backend
// `hbm.exe` holds open; on POSIX it's never mandatory-locked.
function venvHbmShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'hbm.exe')
    : path.join(updateRoot, 'venv', 'bin', 'hbm')
}
...
// Kill only HBM AGENT-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `hbm`,
...
function killHbmOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isHbmOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping HBM AGENT-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `hbm.exe` that
// spawned its own grandchildren (a `hbm` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `HBM AGENT backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live HBM AGENT backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.hbmBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the HBM AGENT backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.hbmBackendIdentity
...
// `hbmProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `hbm.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a hbm
// REPL, a pty terminal, the gateway) keep `hbm.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const hbmProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\hbm.exe, but the
  // backend is `python.exe -m hbm_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (hbmProcess && Number.isInteger(hbmProcess.pid)) {
    initialPids.push(hbmProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(hbmProcess, {
  ...
  // `hbm gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvHbmShimPath(updateRoot), HBM_HOME)

  // Reap HBM AGENT-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killHbmOwnedVenvDaemons(updateRoot)

  const shim = venvHbmShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentHbmProcess = backendConnectionState.getProcess()

        if (currentHbmProcess && Number.isInteger(currentHbmProcess.pid)) {
          stragglers.push(currentHbmProcess.pid)
        }
...
// `hbm update`). Instead we spawn the staged HBM AGENT-Setup binary with
// --update and quit, so it can run `hbm update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `hbm update` → relaunch),
      ...
      // in: the HBM_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `hbm update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `hbm desktop`, never the Tauri installer that self-copies
      // hbm-setup.exe into HBM_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `hbm` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `hbm update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `hbm update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, hbmRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(HBM_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating HBM AGENT — this window will close and the updater will open. Don’t reopen HBM AGENT yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(HBM_HOME, rememberLog)
    ...
    // hbm.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running hbm, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the HBM AGENT install open ' +
        '(a second HBM AGENT window or a terminal running hbm?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvHbmShimPath(updateRoot))
      }
    ...
    // HBM AGENT processes running from this venv.  The updater normally refuses
    ...
    // `hbm update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `hbm update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          HBM_HOME,
          HBM_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithHbmManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `hbm update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(HBM_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another HBM AGENT update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(HBM_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHbm = path.join(venvBin, IS_WINDOWS ? 'hbm.exe' : 'hbm')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's HBM AGENT launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the hbm
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.hbm-bootstrap-complete')),
      hasVenvHbm: fileExists(venvHbm),
...
// Python-level pre-update snapshot inside `hbm update`.
function preflightStateDb(hbmHome, rememberLog) {
  const stateDbPath = path.join(hbmHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(hbmHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(hbmHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(hbmHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `hbm update`, swaps/relaunches the app bundle, and writes
// .hbm-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'hbm update', percent: null })

    return { ok: true, manual: true, command: 'hbm update', hbmRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      HBM_HOME,
      HBM_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithHbmManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating HBM AGENT — this window will close. Don’t reopen HBM AGENT yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_HBM_ROOT actually
...
async function isActiveRuntimeUsable() {
  ...
  return (
    isHbmSourceRoot(ACTIVE_HBM_ROOT) &&
    ...
    (await canImportHbmCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_HBM_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    }))
  )
}
...
async function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `hbm
...
function resolveWebDist() {
  const override = process.env.HBM_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set HBM_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndexWithMissing(): { index: string; missing: string[] } {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: hbm desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: hbm desktop --force-build`
  )
...
function resolveHbmCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/HBM AGENT.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveHbmCwd(), sanitized: Boolean(trimmed) }
  }
...
async function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'hbm_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_HBM_ROOT, the
...
async function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `HBM AGENT at ${ACTIVE_HBM_ROOT}`,
    command,
    args: ['-m', 'hbm_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hbmHome: HBM_HOME,
      pythonPathEntries: [ACTIVE_HBM_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_HBM_ROOT,
    bootstrap: true,
    shell: false
  }
}

async function resolveHbmBackend(backendArgs) {
  // 1. Explicit override -- HBM_DESKTOP_HBM_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.HBM_DESKTOP_HBM_ROOT && path.resolve(process.env.HBM_DESKTOP_HBM_ROOT)

  if (overrideRoot && isHbmSourceRoot(overrideRoot)) {
    const backend = await createPythonBackend(overrideRoot, `HBM AGENT source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `hbm` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isHbmSourceRoot.)
  if (!IS_PACKAGED && isHbmSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `HBM AGENT source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_HBM_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\hbm\\hbm-agent (Windows) or ~/.hbm/hbm-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active HBM AGENT runtime at ${ACTIVE_HBM_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `hbm` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    HBM_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.HBM_DESKTOP_IGNORE_EXISTING !== '1') {
    let hbmCommand = null
    const hbmOverride = process.env.HBM_DESKTOP_HBM

    if (hbmOverride) {
      const resolvedOverride = findOnPath(hbmOverride)

      if (resolvedOverride) {
        hbmCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(hbmOverride, { isWsl: IS_WSL })) {
        hbmCommand = hbmOverride
      } else {
        rememberLog(`Ignoring Windows HBM AGENT override under WSL: ${hbmOverride}`)
      }
    } else {
      hbmCommand = findOnPath('hbm')
    }

    if (hbmCommand) {
      if (looksLikeDesktopAppBinary(hbmCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving HBM AGENT CLI: ${hbmCommand}`)
        hbmCommand = null
      }
    }

    if (hbmCommand) {
      const unwrapped = await unwrapWindowsVenvHbmCommand(hbmCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `hbm` shim
      ...
      const shellForProbe = isCommandScript(hbmCommand)

      // HBM_DESKTOP_HBM is an explicit deployment override (used by
      ...
      if (
        shouldTrustHbmOverride(hbmOverride) ||
        (await verifyHbmCli(hbmCommand, { shell: shellForProbe }))
      ) {
      ...
      rememberLog(
        `Ignoring existing HBM AGENT CLI at ${hbmCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed hbm_cli module via system Python.
  ...
  if (python) {
    ...
    // having hbm_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\hbm\hbm-agent\venv.
    if (await canImportHbmCli(python)) {
      return {
        kind: 'python',
        label: `installed hbm_cli module via ${python}`,
        command: python,
        args: ['-m', 'hbm_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: hbm_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveHbmBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'HBM AGENT not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolveHbmBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no HBM AGENT install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'HBM AGENT recovery was handed off to HBM AGENT Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      hbmHome: HBM_HOME,
      logRoot: path.join(HBM_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('HBM AGENT install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startHbm() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // hbm:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(await resolveHbmBackend(backend.args), assertStillOwned)
  }
  ...
  if (!isHbmSourceRoot(ACTIVE_HBM_ROOT)) {
    throw new Error(missingInstallPartMessage(`HBM AGENT source files are missing or incomplete at ${ACTIVE_HBM_ROOT}`))
  }

  // On Windows, preflight Git Bash. HBM AGENT' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\hbm\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `hbm` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "HBM AGENT needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen HBM AGENT.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isHbmSourceRoot()
    // plus an importable hbm_cli before it hands back the active runtime.
  ...
  backend.label = `HBM AGENT at ${ACTIVE_HBM_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'HBM AGENT runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----hbm-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported HBM AGENT backend URL protocol: ${parsed.protocol}`))
        ...
        const req = client.request(
          parsed,
          {
            ...
            headers: {
              ...
              'X-HBM AGENT-Session-Token': token,
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to HBM AGENT backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-HBM AGENT-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-HBM AGENT-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('hbm:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveHbmCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('hbm:preview-file-changed', payload)
}
...
// waitForHbmReady how to read a 401 — rejected session vs gated route.
...
async function waitForHbm(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForHbmReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('hbm:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('hbm:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('hbm:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('hbm:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting HBM AGENT...".
function sendPowerResume() {
  ...
  webContents.send('hbm:power-resume')
}
...
// 'hbm:power-battery' push below.
ipcMain.handle('hbm:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('hbm:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(HBM_HOME, app.getPath('userData'))
  } catch (err) {
  ...
  try {
    ...
    // 'Open Plugins' rides the existing deep-link channel (hbm://open/…),
    ...
    if (response === 0) {
      handleDeepLink(`${HBM_PROTOCOL}://open/skills?tab=plugins`)
    }
  } finally {
    try {
...
function sendOpenUpdatesRequested() {
  ...
  // before the window is up) queue the request; 'hbm:deep-link-ready' flushes it.
  ...
  webContents.send('hbm:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('hbm:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('hbm:context-menu-spellcheck', {
...
//
// Hosted HBM AGENT gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``hbm_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / hbm #37247).
//     Both are set as HttpOnly cookies (``hbm_session_at`` ~15 min,
//     ``hbm_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startHbm → the renderer's
// single-shot boot() with no retry) surfaced as the "HBM AGENT couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "HBM AGENT
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to HBM AGENT Cloud agent…' : 'Sign in to HBM AGENT gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to HBM AGENT backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in hbm_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- HBM AGENT Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:hbm-remote-oauth`:
...
// convention (hbm_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every HBM AGENT surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.HBM_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the HBM AGENT gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for hbm_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing HBM AGENT Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a HBM AGENT Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to HBM AGENT Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (HBM AGENT Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to HBM AGENT Cloud. Open Settings → Gateway, choose HBM AGENT Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your HBM AGENT Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your HBM AGENT Cloud session has expired. Sign in to HBM AGENT Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the HBM AGENT Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every hbm:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteHbmPath`/`remoteProfile` —
...
// a valid stored value (pins the root HBM_HOME explicitly); null means "no
...
// contains "hbm", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isHbmProcess(pid) {
  try {
...
//   1. Legacy ~/.hbm/active_profile (explicit CLI choice via hbm profile use)
//   2. Running gateway (gateway.pid with verified liveness + hbm identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(HBM_HOME, 'active_profile'),
    hbmHome: HBM_HOME,
    profilesRoot: path.join(HBM_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.HBM_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.HBM_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted HBM AGENT Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteHbmPath: (ssh || savedSsh)?.remoteHbmPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by HBM_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the HBM AGENT Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered HBM AGENT Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteHbmPath: input.sshRemoteHbmPath ?? existingBlock.remoteHbmPath,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote HBM AGENT gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.HBM_DESKTOP_REMOTE_TOKEN,
      url: process.env.HBM_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      hbmPath: result.hbmPath,
      hbmHome: result.hbmHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { hbmPath: result.hbmPath, hbmHome: result.hbmHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteHbmPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteHbmPath: sshConfig.remoteHbmPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForHbm: (baseUrl, token) => waitForHbm(baseUrl, token, lease.signal, 'token'),
      ...
      // spawn command adds HBM_GUEST_ONBOARDING=1 only when this is on.
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        // per-profile pools also use bare keys. Only startHbm' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.hbmVersion || 'hbm (version unknown)'} at ${result.hbmPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteHbmVersion: result.hbmVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (HBM_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.HBM_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startHbm() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every HBM AGENT
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteHbmPath: input.sshRemoteHbmPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteHbmPath || '')
          let hbmPath
          let hbmVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            hbmPath = runtime.hbmPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.hbmPath])
            hbmVersion = inspection.version
            supported = inspection.supported
          } else {
            hbmPath = await remoteLifecycle.locateHbm(ssh, sshConfig.remoteHbmPath || '')
            hbmVersion = await remoteLifecycle.probeHbmVersion(ssh, hbmPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, hbmPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update HBM AGENT on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteHbmPath: hbmPath,
            remoteHbmVersion: hbmVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startHbm())
  ...
  // connect to HBM AGENT gateway". Mirror the renderer's connect here so the test
...
function resetHbmConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startHbm() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const hbmProcess = backendConnectionState.invalidate()
  stopBackendChild(hbmProcess)
...
// startHbm() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetHbmConnection() invalidates it.
  const hbmProcess = backendConnectionState.getProcess()
  const dying = hbmProcess && !hbmProcess.killed ? hbmProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('hbm:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('hbm:connections:changed', payload)
    }
  }
}
...
// startHbm() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(profile, opts: { passive?: boolean; spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startHbm()
  ...
  // spawning, so two children never share one profile's HBM_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteHbmPath: source.remoteHbmPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. hbm:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: hbmLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForHbm(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.hbmPath || !state?.hbmHome) {
  ...
  return {
    ...
    hbmPath: state.hbmPath,
    hbmHome: state.hbmHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteHbmPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.hbmPath ? platform : await probeWindowsRemote(ssh, config.remoteHbmPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          hbmPath: runtime.hbmPath,
          hbmHome: runtime.hbmHome,
          pythonPath: runtime.python
        }
      }
    }

    const hbmPath = await remoteLifecycle.locateHbm(ssh, config.remoteHbmPath || '')
    const hbmHome = await remoteLifecycle.probeRemoteHbmHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, hbmPath, hbmHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { hbmPath: state.hbmPath, hbmHome: state.hbmHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForHbm(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startHbm() but without the boot-progress UI,
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForHbm(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited HBM_HOME env (see _apply_profile_override
  // step 3 in hbm_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']

  const backend = await ensureRuntime(await resolveHbmBackend(backendArgs), () =>
    assertPoolEntryStillOwned(poolKey, entry)
  )
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(HBM_HOME, 'profiles', key))
  )
  rememberLog(`Starting HBM AGENT backend for profile "${profile}" via ${backend.label}`)
      ...
      env: desktopBackendSpawnEnv(
        {
          ...
          TERMINAL_CWD: hbmCwd,
          HBM_DASHBOARD_SESSION_TOKEN: token,
          ...
          ...parentIdentityEnv,
          HBM_WEB_DIST: webDist,
          ...(readyFile ? { HBM_DESKTOP_READY_FILE: readyFile } : {})
        },
        GUEST_ONBOARDING
      ),
      shell: backend.shell,
  ...
  child.once('error', error => {
    rememberLog(`HBM AGENT backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `HBM AGENT backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`HBM AGENT backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `HBM AGENT backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForHbm(baseUrl, token), startFailed])
  assertPoolEntryStillOwned(poolKey, entry, { releaseSlot: false })
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `HBM AGENT backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `HBM AGENT backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_hbm_home() recreates the deleted profile directory.
//
...
function startHbm() {
  return localBackendLifecycle.start(runHbmStart)
}

async function runHbmStart() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('HBM AGENT Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startHbm() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving HBM AGENT backend', 8)
  ...
  // later hbm:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote HBM AGENT backend at ${remote.baseUrl}`, 24)
      await waitForHbm(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote HBM AGENT backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, hbmLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.hbm/active_profile file) and
    // resolves HBM_HOME the same way `hbm -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving HBM AGENT runtime', 28)

        return resolveHbmBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting HBM AGENT backend via ${backend.label}`, 84)
    rememberLog(`Starting HBM AGENT backend via ${backend.label}`)
    ...
    const hbmProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: hbmCwd,
        env: desktopBackendSpawnEnv(
          {
            ...process.env,
            // Explicitly pin HBM_HOME for the child so Python's get_hbm_home()
            // resolves to the SAME location our resolveHbmHome() picked. Without
            // this pin, Python falls back to ~/.hbm on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\hbm, which differs from C:\Users\<u>\.hbm.
            // Mismatch would split config / sessions / .env / logs across two
            // directories. install.ps1 sets HBM_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(hbmProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(hbmProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, hbmProcess)

    if (!processOwner) {
      stopBackendChild(hbmProcess)
      await waitForBackendExit(hbmProcess)
      releaseBackendChild(hbmProcess)
      throw new Error('HBM AGENT backend start was superseded by a newer connection attempt.')
    }

    hbmProcess.stdout.on('data', rememberLog)
    hbmProcess.stderr.on('data', rememberLog)
    ...
    hbmProcess.once('error', error => {
      releaseBackendChild(hbmProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale HBM AGENT backend error: ${error.message}`)
        rejectBackendStart?.(new Error('HBM AGENT backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`HBM AGENT backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `HBM AGENT backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hbmProcess.once('exit', (code, signal) => {
      releaseBackendChild(hbmProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale HBM AGENT backend exit (${signal || code})`)
      ...
      rememberLog(`HBM AGENT backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `HBM AGENT backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `HBM AGENT backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHbmLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for HBM AGENT backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for HBM AGENT backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    await Promise.race([waitForHbm(baseUrl, token), backendStartFailed])
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => hbmProcess.exitCode === null && !hbmProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local HBM AGENT backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'HBM AGENT backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed HBM_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startHbm() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `hbm:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('hbm:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while HBM AGENT is
...
// pushes pet state over IPC (hbm:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via hbm:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main HBM AGENT app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the HBM AGENT icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see hbm:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep HBM AGENT in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('hbm:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so HBM AGENT can be driven
...
// left the user with NO HBM AGENT window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('hbm:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('hbm:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[hbm] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('hbm:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `hbm:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, hbm:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('hbm:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('hbm:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('hbm:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'hbm desktop --force-build',
  ...
  startHbm().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('hbm:connection', async (_event, profile, extra) => {
...
ipcMain.handle('hbm:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'hbm:connection', keyed by the composite
...
ipcMain.on('hbm:connection:active-route', (event, route) => {
...
// "Starting HBM AGENT…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('hbm:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetHbmConnection({ soft: true }),
...
ipcMain.handle('hbm:backend:touch', async (_event, profile) => {
...
ipcMain.handle('hbm:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('hbm:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('hbm:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('hbm:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('hbm:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('hbm:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('hbm:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = await resolveHbmBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'HBM AGENT is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `hbm-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, HBM_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('hbm:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('hbm:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('hbm:zoom:get', event => {
...
ipcMain.on('hbm:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('hbm:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('hbm:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startHbm() call restarts the
...
ipcMain.handle('hbm:bootstrap:repair', async () => {
  // Forceful repair: force the next startHbm() through the full installer
  ...
  // "bypass active runtime" path inside startHbm() and fall through
...
ipcMain.handle('hbm:boot-progress:get', async () => bootProgressState)
ipcMain.handle('hbm:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('hbm:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('hbm:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('hbm:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('hbm:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('hbm:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('hbm:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('hbm:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('hbm:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('hbm:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('hbm:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('hbm:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('hbm:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('hbm:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('hbm:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteHbmPath: entry.remoteHbmPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteHbmPath: connection.remoteHbmPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteHbmProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root HBM_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('hbm:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// hbm:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('hbm:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('hbm:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `hbm update` to every eligible registered connection at once.
...
ipcMain.handle('hbm:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/hbm/update', {}, { timeoutMs: 15_000 })
...
// analogue of the hbm:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('hbm:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('hbm:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startHbm() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('hbm:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- HBM AGENT Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('hbm:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('hbm:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('hbm:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('hbm:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of hbm:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('hbm:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('hbm:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new HBM_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('hbm:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('hbm:requestMicrophoneAccess', async () => {
...
ipcMain.handle('hbm:window:readBelow', async event => {
...
async function handleHbmApiRequest(request) {
  ...
  // backend calls ensure_hbm_home() which recreates the profile directory,
...
ipcMain.handle('hbm:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its HBM_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleHbmApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleHbmApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('hbm:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('hbm:data-url-read-max:get', () => ({
...
ipcMain.handle('hbm:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('hbm:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('hbm:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('hbm:readFileText', async (_event, filePath) => {
...
// `hbm:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('hbm:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('hbm:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('hbm:writeClipboard', (_event, text) => {
...
ipcMain.handle('hbm:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('hbm:readClipboard', () => clipboard.readText())

ipcMain.handle('hbm:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('hbm:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('hbm:context-menu:edit', (event, command) => {
...
ipcMain.handle('hbm:context-menu:copy-image', event => {
...
ipcMain.handle('hbm:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('hbm:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('hbm:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('hbm:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('hbm:savePastedText', async (_event, payload) => {
...
ipcMain.handle('hbm:saveClipboardImage', async () => {
...
ipcMain.handle('hbm:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('hbm:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('hbm:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('hbm:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('hbm:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('hbm:titlebar-theme', (_event, payload) => {
...
ipcMain.on('hbm:native-theme', (_event, mode) => {
...
ipcMain.on('hbm:translucency:support', event => {
...
// it arrives from `hbm desktop --local` or directly on HBM AGENT.exe (a
// shortcut edit), and survives self-relaunches because collectRelaunchArgs
// only strips internal flags.
ipcMain.on('hbm:launch-flags', event => {
  event.returnValue = {
...
ipcMain.on('hbm:translucency', (_event, payload) => {
...
ipcMain.on('hbm:keep-awake', (_event, on) => {
...
ipcMain.handle('hbm:quick-entry:settings:get', async () => {
...
ipcMain.handle('hbm:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('hbm:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('hbm:quick-entry:submit', {
...
ipcMain.on('hbm:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('hbm:quick-entry:state', payload)
  }
})

ipcMain.on('hbm:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('hbm:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('hbm:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `hbm:window:openSession`).
...
ipcMain.handle('hbm:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('hbm:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('hbm:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('hbm:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveHbmCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('hbm:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveHbmCwd()
}))

ipcMain.handle('hbm:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('hbm:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('hbm:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('hbm:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('hbm:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('hbm:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('hbm:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: hbmLog.slice(-200) }))
...
ipcMain.on('hbm:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (hbm:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('hbm:updates:check', async (_event, opts) =>
...
ipcMain.handle('hbm:updates:apply', async (_event, payload) =>
...
ipcMain.handle('hbm:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('hbm:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical HBM AGENT version (the one `release.py` bumps in
// hbm_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real HBM AGENT version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveHbmVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'hbm_cli', '__init__.py')
...
// Renderer-bundle skew: `hbm update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live HBM AGENT version and push it into the native About panel
// just before showing it, so an in-place `hbm update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveHbmVersion()} — app build out of date, update the desktop app`
        : resolveHbmVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
...
ipcMain.handle('hbm:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveHbmVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    hbmRoot: resolveUpdateRoot(),
...
// The About page's "Restart HBM AGENT" button (shown when bundleSwapPending):
...
ipcMain.handle('hbm:app:relaunch', async () => {
...
ipcMain.handle('hbm:machine:profile', async () => {
...
// the actual removal via `hbm uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (hbm_cli/uninstall.py + hbm_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_HBM_ROOT
  ...
  const fallback = () => ({
    hbm_home: HBM_HOME,
    agent_installed: isHbmSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'hbm_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, HBM_HOME, NO_COLOR: '1' },
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no HBM AGENT venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import hbm_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_HBM_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `hbm-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `hbm-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('hbm:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('hbm:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('hbm:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('hbm:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// hbm:// deep links (e.g. hbm://blueprint/morning-brief?time=08:00,
// hbm://mcp/install?name=NAME&config=B64 — the vendor "Add to HBM AGENT"
// button, or hbm://plugin/install?repo=owner/repo). Dev
// (`HBM_DESKTOP_DEV_SERVER`) registers hbm-dev:// instead — bare
// Electron or a stale OS handler often owns hbm:// on dev machines.
...
// ---------------------------------------------------------------------------
const HBM_PROTOCOL = DEV_SERVER ? 'hbm-dev' : 'hbm'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['hbm-dev', 'hbm'] : ['hbm']
...
function handleDeepLink(url) {
  ...
  // hbm://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('hbm:deep-link', payload)
...
ipcMain.handle('hbm:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${HBM_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(HBM_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(HBM_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${HBM_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `hbm://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startHbm), whose
...
app.whenReady().then(() => {
  ...
  // startHbm() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching hbm:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting HBM AGENT never keeps another app's chord hostage.
