...
function tmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'hbm-compat-'))
}
...
const REPORT = {
  ...
  plugins: {
    ...
    beta: [
      { file: 'a.py', line: 1, old: 'hbm_cli.kanban_db.connect', new: 'hbm_cli.kanban_db_connect.connect' },
      {
        file: 'a.py',
        line: 9,
        old: 'hbm_cli.kanban_db.connect_closing',
        new: 'hbm_cli.kanban_db_connect.connect_closing'
      }
    ]
  },
  lines: [
    '2 plugins use import paths that stop working on 2026-09-14 (10 days): alpha (1), beta (2)',
    'Details: hbm plugins compat'
  ]
}
...
test('report → one notice naming the plugins and the date, with no config keys or CLI commands', () => {
  ...
  for (const text of [n.title, n.message, n.detail]) {
    assert.doesNotMatch(text, /config\.yaml|hbm plugins compat|allow_deprecated_imports|module path/)
  }
})
...
test('dismissal is remembered for the same report and forgotten for a different one', () => {
  ...
  assert.match(third.message, /were turned off: alpha, beta\. HBM AGENT works normally without them\./)
  assert.doesNotMatch(third.detail, /config\.yaml|hbm plugins compat|allow_deprecated_imports/)
