...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/hbm-agent.git'), 'someone/hbm-agent')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('nousresearch/hbm-agent', 'bb/gui'),
    'https://api.github.com/repos/nousresearch/hbm-agent/commits/bb%2Fgui'
  )
})
