 ...
 *
 * The staged Tauri `hbm-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `hbm update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `hbm update`, swaps/relaunches
 * the app, and writes .hbm-update-result.json. With the app gone before
 * the update starts, the HBM_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `hbm update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into HBM_HOME on *every* platform
 * (`hbm-setup.exe` on Windows, `hbm-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_hbm_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `hbm update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(hbmHome, 'hbm-setup.exe')
 ...
 *
 * `copy_self_to_hbm_home` deliberately no-ops during `--update`
 ...
 * "Another HBM AGENT update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `hbm update`, which
 ...
 * the installer is written to HBM_HOME at install/repair time, making mtime
 ...
 * The lead sentence is plain: nothing changed and HBM AGENT keeps running. The raw
...
export function describeUpdaterHandoffFailure(outcome: Pick<UpdaterHandoffOutcome, 'message'>): string {
  const lead =
    "The updater couldn't start, so nothing was changed and HBM AGENT keeps running as before. " +
