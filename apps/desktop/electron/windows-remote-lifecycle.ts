...
const READY_RE = /^HBM_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitHbmPath = '') {
  const explicit = psLiteral(explicitHbmPath)
  ...
  const script = [
    ...
    '$hbmHome=$env:HBM_HOME',
    'if(-not $hbmHome){$hbmHome=Join-Path $env:LOCALAPPDATA "hbm"}',
    'Assert-NoReparse $hbmHome $true',
    '$candidate=[IO.Path]::Combine($hbmHome, "hbm-agent\\venv\\Scripts\\hbm.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "hbm-agent\\.venv\\Scripts\\hbm.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $hbmHome "hbm-agent\\venv\\Scripts\\hbm.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "hbm-agent\\.venv\\Scripts\\hbm.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command hbm.exe -ErrorAction SilentlyContinue',
    ...
    '$hbm=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$hbm=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $hbm){throw "HBM AGENT is not installed on the remote Windows host."}',
    'Assert-NoReparse $hbm $false',
    'if($explicit -and $hbm -ne $explicit){throw "The configured HBM AGENT path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($hbm), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;hbmHome=$hbmHome;hbmPath=$hbm;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(hbmHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class HbmMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(hbmHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".hbm-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[HbmMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, hbmHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(hbmHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitHbmPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitHbmPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'hbm_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'hbm_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'hbm_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.hbmHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".hbm-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};hbmPath=${psLiteral(reservation.hbmPath)};hbmHome=${psLiteral(reservation.hbmHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.hbmPath === 'string' &&
    typeof lock.hbmHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.hbmPath === runtime.hbmPath &&
    lock.hbmHome === runtime.hbmHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteHbmPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.hbmHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.hbmPath])

  if (!inspection.supported) {
    const error: any = new Error('Update HBM AGENT on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.hbmPath = inspection.path
  const hbmVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located hbm at ${runtime.hbmPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, hbmPath: runtime.hbmPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForHbm(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="HBM AGENT SSH"', 'powershell.exe -NoLogo')
