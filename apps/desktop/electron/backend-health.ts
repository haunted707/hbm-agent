...
export interface HbmReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForHbmReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote HBM AGENT gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForHbmReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed HBM AGENT Cloud instance
 ...
 * `isReauthRequired` so startHbm latches; a bare `needsOauthLogin` (the
...
export async function waitForHbmReady(baseUrl: string, options: HbmReadyOptions): Promise<void> {
  ...
  throw new Error(`HBM AGENT backend did not become ready: ${detail}`)
}
