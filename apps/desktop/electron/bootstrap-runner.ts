 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of HBM AGENT by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.hbm-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(hbmHome) {
  return path.join(hbmHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.hbm/hbm-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(hbmHome) {
  if (!hbmHome) {
    return null
  }

  const candidate = path.join(hbmHome, 'hbm-agent', 'scripts', installScriptName())
...
function cachedScriptPath(hbmHome, commit) {
  return path.join(bootstrapCacheDir(hbmHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(hbmHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(hbmHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, hbmHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass HBM_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          HBM_HOME: hbmHome || process.env.HBM_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, hbmHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, hbmHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--hbm-home', hbmHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, hbmHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, hbmHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(hbmHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, hbmHome, emit })
    abortSignal?.throwIfAborted()
