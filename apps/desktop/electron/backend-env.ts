 ...
 * `iter_hbm_node_dirs()` in hbm_constants.py, which the Electron main
 * process cannot import.
 */
function hbmManagedNodePathEntries(
  hbmHome,
  ...
  const root = pathModule.join(hbmHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const hbmNodeDirs = hbmManagedNodePathEntries(hbmHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([hbmNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeHbmHomeRoot(hbmHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!hbmHome) {
    return hbmHome
  }

  const resolved = pathModule.resolve(String(hbmHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). hbm_bootstrap sets this inside the child too, but only
