...
export const BOOTSTRAP_STAGE_LABELS: ReadonlyMap<string, string> = new Map([
  ...
  ['repository', 'HBM AGENT source code'],
  ...
  ['path', 'HBM AGENT command'],
  ['gateway', 'HBM AGENT service'],
...
const BOOTSTRAP_FAILURE_REMEDY =
  'Common causes: no internet connection, antivirus blocking the installer, or another copy of HBM AGENT running. ' +
  'Close other HBM AGENT windows and choose Reload and retry; if it fails again, open the logs and send them to support.'
...
export function describeBootstrapFailure(failedStage: string | null | undefined, rawError: unknown): string {
  ...
  const lead = label
    ? `Setting up HBM AGENT stopped during the '${label}' step.`
    : 'Setting up HBM AGENT stopped before it could finish.'
...
/**
 * Error.message for an installed HBM AGENT with a piece missing (source tree,
 * Python environment). The renderer's install overlay offers the Repair install
 * button ('hbm:bootstrap:repair'), so the copy points there. `whatIsMissing`
...
export function missingInstallPartMessage(whatIsMissing: string): string {
  return (
    "Part of HBM AGENT' installation is missing (it may have been deleted or quarantined by antivirus). " +
