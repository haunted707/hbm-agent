// Backend subcommand routing for the desktop-managed HBM AGENT process.
//
// The desktop app launches its own headless backend via `hbm serve` — it
...
// app hasn't updated yet, or an older `hbm` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional HBM AGENT profile to pin via `--profile`.
 */
 ...
 * `-m hbm_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `hbm_cli/subcommands/dashboard.py` source registers
