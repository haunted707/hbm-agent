...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  ...
  assert.equal(canonicalGitHubRemote('git@github.com:nousresearch/hbm-agent.git'), OFFICIAL_REPO_CANONICAL)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  ...
  assert.equal(isOfficialSshRemote('git@github.com:nousresearch/hbm-agent.git'), true)
})
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('git@github.com:someuser/hbm-agent.git'), false)
