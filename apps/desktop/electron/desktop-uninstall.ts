 ...
 * the three user-facing uninstall modes to the `hbm uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m hbm_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `hbm_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'hbm_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/HBM AGENT.app/Contents/MacOS/HBM AGENT  → …/HBM AGENT.app
 *   Windows: …\HBM AGENT\HBM AGENT.exe                 → …\HBM AGENT  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/HBM AGENT.app/Contents/MacOS/HBM AGENT → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/HBM AGENT.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs HBM AGENT.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]HBM AGENT$/i.test(dir) || /[\\/]hbm-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/hbm
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import hbm_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hbmHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import hbm_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but HBM AGENT install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "HBM_HOME=${String(hbmHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
