...
export function createIntroRevealWindowController({
  ...
  function spawnIntroRevealWindow() {
    ...
    win.on('closed', () => {
      ...
      if (main && !main.isDestroyed()) {
        main.webContents.send('hbm:intro-reveal:closed')
      }
    })
  ...
  ipcMain.handle('hbm:intro-reveal:open', (_event, payload?: IntroRevealOpenPayload) => openIntroReveal(payload))
  ipcMain.handle('hbm:intro-reveal:close', (_event, payload?: IntroRevealClosePayload) => closeIntroReveal(payload))
  ipcMain.on('hbm:intro-reveal:ready', event => {
    if (event.sender === introRevealWindow?.webContents) {
      introRevealShow?.reveal()
    }
  })
  ipcMain.on('hbm:intro-reveal:skip', event => {
    const main = mainWindow()

    if (event.sender === introRevealWindow?.webContents && main && !main.isDestroyed()) {
      main.webContents.send('hbm:intro-reveal:skip')
    }
  })
