...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'hbm_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'hbm_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'hbm_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/HBM AGENT.app/Contents/MacOS/HBM AGENT', 'darwin'),
    '/Applications/HBM AGENT.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/HBM AGENT.app/Contents/MacOS/HBM AGENT', 'darwin'),
    '/Users/x/Applications/HBM AGENT.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\HBM AGENT\\HBM AGENT.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\HBM AGENT'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hbm-desktop\\HBM AGENT.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hbm-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\HBM AGENT.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_HbmXXXX/hbm', 'linux', { APPIMAGE: '/home/x/Apps/HBM AGENT.AppImage' }),
    '/home/x/Apps/HBM AGENT.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/hbm/linux-unpacked/hbm', 'linux', {}), '/opt/hbm/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/hbm', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/HBM AGENT.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/HBM AGENT.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.hbm/hbm-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.hbm/hbm-agent',
    uninstallArgs: ['-m', 'hbm_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/hbm/linux-unpacked',
    hbmHome: '/home/x/.hbm'
  })
  ...
  assert.match(script, /'-m' 'hbm_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/hbm\/linux-unpacked'/)
  assert.match(script, /export HBM_HOME='\/home\/x\/\.hbm'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.hbm/hbm-agent',
    agentRoot: '/home/x/.hbm/hbm-agent',
    uninstallArgs: ['-m', 'hbm_cli.uninstall', '--mode', 'full'],
    appPath: null,
    hbmHome: '/home/x/.hbm'
  })

  // System python + source on PYTHONPATH so import hbm_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.hbm\/hbm-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'hbm_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'hbm_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    hbmHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'hbm_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hbm',
    uninstallArgs: ['-m', 'hbm_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\HBM AGENT',
    hbmHome: 'C:\\Users\\x\\AppData\\Local\\hbm'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import hbm_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\hbm;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "hbm_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\HBM AGENT" >nul 2>&1/)
