 ...
 * runtime, remote gateways, HBM AGENT Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@hbm/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteHbmPath || '').trim() !== String(activeSsh.remoteHbmPath || '').trim() ||
...
/** Turn `ls ~/.hbm/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. HBM AGENT Cloud
 * instances are platform-managed — we never run `hbm update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteHbmPath: input.remoteHbmPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/HBM AGENT-managed names; an empty result stores nothing.
 ...
 * `remoteHbmPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'HBM AGENT Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'HBM AGENT Cloud' : 'Remote gateway')
