export type ParentWatchdogEnv = {
  HBM_PARENT_PID: string
  HBM_PARENT_START_MARKER?: string
  HBM_PARENT_NONCE?: string
  /** Spawn tag consumed by hbm_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { HBM_PARENT_PID: String(pid) }
  env.HBM_SPAWN = spawnTag(pid, startMarker)
  ...
  env.HBM_PARENT_START_MARKER = startMarker
  env.HBM_PARENT_NONCE = nonce

  return env
}

/** Build the `HBM_SPAWN` tag mirrored by hbm_cli/process_identity.py.
