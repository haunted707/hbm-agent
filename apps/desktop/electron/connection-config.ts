 ...
 *     `X-HBM AGENT-Session-Token` header; WS uses `?token=`.
...
// hbm_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - hbm_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - hbm_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / hbm #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-hbm_session_at', '__Secure-hbm_session_at', 'hbm_session_at']
const RT_COOKIE_VARIANTS = ['__Host-hbm_session_rt', '__Secure-hbm_session_rt', 'hbm_session_rt']

// The Nous portal (NAS) does NOT use HBM AGENT gateway session cookies — it is a
...
// Keep this aligned with hbm_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['hbm', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startHbm latches
...
/** Which HBM AGENT profile the remote SSH dashboard should actually run as.
...
// HBM AGENT Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteHbmPath = String(entry.remoteHbmPath || '').trim()

  if (remoteHbmPath) {
    out.remoteHbmPath = remoteHbmPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote HBM AGENT installation. Preserve an explicit mapping
  // when it is a valid HBM AGENT profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_hbm_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `HBM_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process HBM_HOME can belong to a different
 ...
 * `hbm:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://hbm.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `hbm:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a hbm session ACCESS-token cookie
 ...
 * the HBM AGENT gateway session cookies, so cloud sign-in / discovery liveness
