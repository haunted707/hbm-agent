// The standalone desktop-plugin root (`<HBM_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.hbm-package.json` marker
...
export const PACKAGE_MARKER = '.hbm-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.hbm-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every hbm home the app knows about locally: the default plus each profile. */
export async function localHomes(hbmHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(hbmHome, 'profiles'))

  return [hbmHome, ...profiles.map(name => path.join(hbmHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(hbmHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(hbmHome, 'profiles'))) {
    const scopedRoot = path.join(hbmHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(hbmHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(hbmHome)) {
