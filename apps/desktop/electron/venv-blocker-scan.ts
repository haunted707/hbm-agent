...
const SCAN_MODULE = 'hbm_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/HBM AGENT processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'hbm_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another HBM AGENT process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the HBM AGENT installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other HBM AGENT windows and terminals, then retry.  If the problem\n' +
    'persists, run `hbm update` in a terminal for detailed diagnostics.'
  )
}
