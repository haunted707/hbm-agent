 ...
 *   - the browser sees only a minimal "return to HBM AGENT" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to HBM AGENT.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('hbm:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('hbm:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('hbm:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
