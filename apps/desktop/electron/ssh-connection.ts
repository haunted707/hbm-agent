...
// orphan (ppid=1) — a hung remote CLI (e.g. a wedged `hbm --version`)
...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(HBM_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-HBM AGENT-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.hbm/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'hbm-desktop-ssh')
  }

  return path.join(os.homedir(), '.hbm', 'desktop-ssh')
}
