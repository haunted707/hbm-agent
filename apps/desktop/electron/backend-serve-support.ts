...
// spawns `hbm serve`; runtimes older than serve only have `dashboard`, so
...
// (covers a bare `hbm` resolved from PATH with no known source root). Result
...
export function createBackendServeSupportResolver(hbmHome: string, rememberLog: (message: string) => void) {
    ...
    const pending = (async () => {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'hbm_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          // `hbm --version` (~10.5s measured cold), and a false negative here
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, HBM_HOME: hbmHome, ...(backend.env || {}) },
            timeout: PROBE_TIMEOUT_MS,
            stdio: 'ignore',
            // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
            // (see resolveHbmBackend step 4); execFileSync of a .cmd without
