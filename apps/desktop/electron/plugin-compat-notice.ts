 ...
 *
 * The Python side (hbm_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes HBM_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(hbmHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(hbmHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(hbmHome: string, userData: string): PendingNotice | null {
  const report = readReport(hbmHome)
  ...
  const copy = report.in_effect
    ? {
        ...
        message: `These plugins were built for an older HBM AGENT and were turned off: ${nameList}. HBM AGENT works normally without them.`,
        detail: `Look for an updated version of each plugin or ask its author.\n\n${list}`
      }
    : {
        title: 'Some plugins need an update',
        message: `These plugins were built for an older HBM AGENT and will stop working on ${report.removal_date}: ${nameList}.`,
