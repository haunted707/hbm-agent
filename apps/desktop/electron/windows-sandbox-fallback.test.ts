...
test('sandbox marker round-trips through the userData file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'hbm-sandbox-marker-'))
...
test('buildIcaclsGrantArgs targets ALL APPLICATION PACKAGES with inherited RX', () => {
  assert.deepEqual(buildIcaclsGrantArgs('C:\\HBM AGENT\\win-unpacked'), [
    'C:\\HBM AGENT\\win-unpacked',
    '/grant',
...
test('grantAllApplicationPackagesAcl is a no-op off Windows and reports exec failures', () => {
  ...
  const ok = grantAllApplicationPackagesAcl('C:\\HBM AGENT', {
    platform: 'win32',
  ...
  assert.deepEqual(calls[0]?.args, buildIcaclsGrantArgs('C:\\HBM AGENT'))

  const failed = grantAllApplicationPackagesAcl('C:\\HBM AGENT', {
    platform: 'win32',
    execFileSync() {
...
test('buildNoSandboxRelaunchArgs appends a single --no-sandbox flag', () => {
  assert.deepEqual(buildNoSandboxRelaunchArgs(['--foo', '--no-sandbox', 'hbm://x']), [
    '--foo',
    'hbm://x',
    '--no-sandbox'
  ])
