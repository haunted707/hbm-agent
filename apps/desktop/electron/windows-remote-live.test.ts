...
//   HBM_WIN_SSH_HOST   ssh alias/host of the Windows box
//   HBM_WIN_SSH_USER   remote user
//   HBM_WIN_SSH_HBM absolute path to the remote hbm.exe under test
const liveHost = process.env.HBM_WIN_SSH_HOST || ''
const liveUser = process.env.HBM_WIN_SSH_USER || ''
const configuredHbm = process.env.HBM_WIN_SSH_HBM || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-HBM AGENT-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredHbm)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredHbm.replace('hbm.exe', 'python.exe')}' -m hbm_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredHbm.replace('hbm.exe', 'python.exe')
        const terminate = `& '${python}' -m hbm_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.hbmPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m hbm_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m hbm_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
