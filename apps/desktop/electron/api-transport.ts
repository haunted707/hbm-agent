/**
 * Shared HTTP transport policy for the Electron main process's HBM AGENT REST
...
function htmlResponseError(url: string, statusCode: unknown, location?: unknown) {
  ...
  return new Error(
    `Expected JSON from ${url} but got HTML (status ${statusCode}). The endpoint is likely missing on the HBM AGENT backend.`
  )
}
