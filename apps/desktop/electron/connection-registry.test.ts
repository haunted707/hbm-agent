...
test('primary SSH reuse rejects a descriptor with a different remote HBM AGENT path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteHbmPath: '/srv/hbm', user: 'alice' },
    profiles: {}
  })
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    connections: [
      ...
      { id: 'hbm-vps', kind: 'ssh', label: 'HBM AGENT VPS', host: 'hbm-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'hbm-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'hbm-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'hbm-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteHbmPath: '/opt/hbm' },
  ...
  assert.equal(resolve({ ...base, remoteHbmPath: '/opt/hbm' }), 'ssh-path')
...
// The electron and @hbm/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@hbm/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'hbm-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('token only persists on token-auth remotes; oauth/cloud drop it', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'C', url: 'https://c.hbm.cloud', authMode: 'oauth', token: { enc: 'x' } },
    registry
  )
...
test('merge preserves fields the editor does not carry (org, ssh extras)', () => {
  ...
  assert.equal(labelOnly.remoteHbmPath, '/opt/hbm')
...
test('remote input normalizes URL and auth mode; cloud keeps org', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'Cloud', url: 'https://foo.hbm.cloud', authMode: 'oauth', org: 'nous' },
    registry
  )
  ...
  const input = {
    ...
    connections: [
      ...
      },
      {
        id: 'cloud-1',
        kind: 'cloud',
        label: 'HBM AGENT Cloud',
        url: 'https://a.hbm.cloud',
        authMode: 'oauth',
        org: 'nous'
      },
...
test('migrate: v1 cloud keeps cloud provenance + org', () => {
  const registry = migrateV1ToRegistry({
    mode: 'cloud',
    remote: { url: 'https://a.hbm.cloud', authMode: 'oauth', org: 'nous' }
  })
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'hbm-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'hbm-alex')
  assert.equal(applied.lastUsed, 'hbm-alex')
})
