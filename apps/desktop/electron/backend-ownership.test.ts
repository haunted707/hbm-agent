...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'hbm serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/hbm serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m hbm_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/hbm --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\HBM AGENT Runtime\\hbm.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('hbm chat --query serve'), false)
