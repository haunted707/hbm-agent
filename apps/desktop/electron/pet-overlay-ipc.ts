...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('hbm:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('hbm:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('hbm:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('hbm:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('hbm:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('hbm:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('hbm:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('hbm:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('hbm:pet-overlay:control', payload)
  })
}
