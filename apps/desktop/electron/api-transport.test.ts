/**
 * Unit + live-transport tests for the Electron main process's HBM AGENT REST
...
describe('htmlResponseError', () => {
  ...
  it('does not blame credentials when the redirect only fixes the scheme or a trailing slash', () => {
    for (const [url, location] of [
      ...
      ['https://gateway.example.com/hbm/api/health', 'https://gateway.example.com/hbm/api/health/']
    ]) {
