...
test('rewrites an archived node-pty root to the matching unpacked tree exactly once', () => {
  assert.equal(
    writableNodePtyRoot('/HBM AGENT.app/Contents/Resources/app.asar/dist/node_modules/node-pty'),
    '/HBM AGENT.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'
  )
  assert.equal(
    writableNodePtyRoot('/HBM AGENT.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'),
    '/HBM AGENT.app/Contents/Resources/app.asar.unpacked/dist/node_modules/node-pty'
  )
})

test('uses the unpacked helper when resolution reports an app.asar node-pty root', () => {
  const archivedRoot = '/HBM AGENT.app/Contents/Resources/app.asar/dist/node_modules/node-pty'
