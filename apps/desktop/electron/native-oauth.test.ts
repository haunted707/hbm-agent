...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/hbm', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/hbm/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/hbm'), 'https://gw.example.com/hbm/auth/native/refresh')
})
