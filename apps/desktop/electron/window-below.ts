// window-below.ts — which OS window sits directly underneath a HBM AGENT window.
//
...
export function enumerationFailureNote(platform: string, env: NodeJS.ProcessEnv, detail?: string): string {
  ...
  if (env.HYPRLAND_INSTANCE_SIGNATURE) {
    return (
      ...
      'Check that `hyprctl clients` works from the same session HBM AGENT is ' +
      'running in.'
    )
  }
  ...
  if (wayland) {
    return (
      ...
      'X11/Xorg session, or run HBM AGENT under XWayland with DISPLAY set.'
    )
  }
 ...
 *
 * Walks past every window owned by our own process (all HBM AGENT windows share
...
export async function readWindowBelow(
  ...
  if (process.platform === 'darwin' && !titlesAvailable) {
    result.note =
      ...
      'Screen Recording permission, which HBM AGENT does not request for this.'
  }
