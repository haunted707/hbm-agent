 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote HBM AGENT dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the HBM AGENT install on the remote (login-shell probe),
 ...
 *     `HBM_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForHbm in.
 *
 * The minted HBM_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.hbm/desktop-ssh'
...
// Resolve the remote hbm executable. An EXPLICIT path is honored strictly
...
async function locateHbm(ssh, remoteHbmPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The hbm binary or wrapper script
  ...
  if (remoteHbmPath) {
    if (await isExecutable(remoteHbmPath)) {
      return resolveLauncher(remoteHbmPath)
    }

    const err: any = new Error(
      `The HBM AGENT path you set is not an executable on the remote host: "${remoteHbmPath}". ` +
        'Check the path (it must be the full path to the `hbm` binary on the remote, e.g. ' +
        '~/hbm-agent/.venv/bin/hbm), or clear it to auto-detect.'
    )

    err.kind = 'hbm-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v hbm')}`)).trim()
  ...
  candidates.push('~/.local/bin/hbm')
  candidates.push('/usr/local/bin/hbm')
  candidates.push('~/.hbm/hbm-agent/venv/bin/hbm')
  ...
  const err: any = new Error(
    'HBM AGENT is not installed on the remote host (could not find a `hbm` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the HBM AGENT path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<hbm> --version`,
// e.g. "HBM AGENT v0.18.2 ..."), or '' on failure. Surfaces WHICH hbm a
// connection uses, so a stale/unexpected install is visible.
async function probeHbmVersion(ssh, hbmPath) {
  try {
    // Watchdogged: a hung remote CLI must die remotely instead of orphaning
    // when the local ssh child is SIGKILLed (#110478).
    const out = (await ssh.exec(withRemoteTimeout(`${expandRemotePath(hbmPath)} --version 2>&1`))).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". HBM AGENT Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The HBM_HOME the remote dashboard will use (explicit env wins, else
// ~/.hbm). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteHbmHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${HBM_HOME:-$HOME/.hbm}"')).trim().split('\n').pop()

    return out || '~/.hbm'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote HBM AGENT home.')
...
marker=home/'.hbm-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing HBM AGENT checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, hbmHome) {
  const home = assertSafeRemoteHome(hbmHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote HBM AGENT install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote HBM AGENT update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote HBM AGENT update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteHbmProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteHbmHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote HBM AGENT profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote HBM AGENT home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'hbmPath', 'hbmHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !hbmPath) {
    return false
  }
  ...
  const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(hbmPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/hbm`, leaving neither
      // launcher nor HBM_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `hbm_home=os.path.expanduser(${shq(hbmHome)}) if ${shq(hbmHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if hbm_home:\n' +
      ' expected_entries.add(os.path.join(hbm_home,"hbm-agent","venv","bin","hbm"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.hbmPath)
  const expectedHome = lock.hbmHome ? expandRemotePath(lock.hbmHome) : "''"
  ...
  const executableMatch = lock.hbmHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.hbmPath === expected.hbmPath &&
    lock.hbmHome === expected.hbmHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.hbmPath)})
hbm_home=os.path.expanduser(${py(lock.hbmHome)})
expected_entries={expected_path,os.path.join(hbm_home,"hbm-agent","venv","bin","hbm")}
...
// execing HBM AGENT. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"hbm-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(hbmPath, profile, opts: any = {}) {
  const hbm = expandRemotePath(hbmPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.hbmHome || '~/.hbm')}/.hbm-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.hbmHome || '~/.hbm')}/.hbm-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env HBM_DESKTOP=1${opts.guestOnboarding === true ? ' HBM_GUEST_ONBOARDING=1' : ''} ${hbm} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} hbm-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, hbmPath) {
  const hbm = expandRemotePath(hbmPath)
  ...
  const out = await ssh.exec(
    `help="$( ${withRemoteTimeout(`${hbm} serve --help 2>&1`)} )"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, hbmPath))) {
    const err: any = new Error(
      'The remote HBM AGENT install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update HBM AGENT on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(hbmPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        hbmHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForHbm, a token-gated
...
async function connect(deps) {
  ...
  const hbmHome = await probeRemoteHbmHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, hbmHome)
  const hbmPath = await locateHbm(ssh, remoteHbmPath)
  log(`located hbm at ${hbmPath}`)
  const hbmVersion = await probeHbmVersion(ssh, hbmPath)

  if (hbmVersion) {
    log(`remote hbm version: ${hbmVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this HBM AGENT Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.hbmPath === hbmPath &&
      lock.hbmHome === hbmHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    hbmHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, hbmHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForHbm(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
