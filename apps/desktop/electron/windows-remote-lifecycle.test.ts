...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command hbm.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          hbmHome: 'C:\\Users\\alice\\.hbm',
          hbmPath: 'C:\\HBM AGENT\\hbm.exe',
          python: 'C:\\HBM AGENT\\python.exe'
        })
      }

      if (script.includes('.hbm-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('hbm_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.hbm\\profiles\\research')
  assert.match(script, /\.hbm-update-in-progress/)
...
test('Windows probe validates HBM AGENT and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $hbmHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('HBM AGENT is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /HBM AGENT is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\HBM AGENT's\\python.exe" }, 'inspect', [
    'C:\\x y\\hbm.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'hbm_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /HBM AGENT''s/)
  assert.match(script, /C:\\x y\\hbm\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { hbmPath: lock.hbmPath, hbmHome: lock.hbmHome }
