 ...
 * windows-hbm-path.ts
 *
 * Pure, dependency-injected pieces of Windows `hbm` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvHbmCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `hbm` shim shadowed the real
 *      hbm.cmd/hbm.exe; the shim then failed the --version probe and
 ...
 *      and HBM AGENT launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvHbmCommand() — unwrapWindowsVenvHbmCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `hbm`) must
 * not shadow `hbm.cmd`/`hbm.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the HBM AGENT launcher that drives `hbm update`.
...
export function chooseUpdaterArgs(signals: BootstrapRecoverySignals, branch: string): string[] {
  const canRunUpdater = signals.hasVenvHbm && signals.hasVenvPython
...
export interface ResolveVenvHbmCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportHbmCli: (python: string, opts?: { env?: Record<string, string> }) => Promise<boolean>
...
/**
 * If `command` is a Windows venv `hbm`/`hbm.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/hbm(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m hbm_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportHbmCli(). A
 * venv whose update died mid-`pip install` still has python.exe + hbm.exe
 ...
 *
 * Returns null when `command` is not a venv hbm shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export async function resolveVenvHbmCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvHbmCommandDeps
): Promise<{
  label: string
  command: string
  args: string[]
  ...
  if (!/^hbm(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv HBM AGENT at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing HBM AGENT Python at ${python}`,
    command: python,
