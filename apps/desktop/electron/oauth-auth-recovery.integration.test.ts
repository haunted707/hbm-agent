...
async function fixture(beforeRefreshResponse?: () => Promise<void>) {
  const home = mkdtempSync(join(tmpdir(), 'hbm-auth-recovery-'))
...
test('HTTP outage, dead-refresh and cookie coexistence reach the correct ticket/readiness/media verdicts', async () => {
  ...
  try {
    ...
    try {
      await waitForHbmReady(f.baseUrl, {
    ...
    const mediaRequest = { url: 'hbm-media://remote/%2Ftmp%2Fclip.mp4', headers: new Headers(), method: 'GET' }
