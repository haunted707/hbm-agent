 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `hbm update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `hbm desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'HBM AGENT couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>hbm desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>hbm desktop --force-build</code>, then restart the app.</p>
