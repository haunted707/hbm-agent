 ...
 * console. It writes HBM_HOME/.hbm-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen HBM AGENT within 30 minutes.
...
export function handoffResultPath(hbmHome: string): string {
  return path.join(hbmHome, '.hbm-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(hbmHome)
  let raw: string
