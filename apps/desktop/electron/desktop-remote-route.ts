...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'HBM_DESKTOP_REMOTE_URL is set but HBM_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote HBM AGENT backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `hbm serve` the desktop never
