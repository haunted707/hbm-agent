...
export function markerPath(hbmHome) {
  return path.join(hbmHome, '.hbm-update-in-progress')
}
 ...
 *
 * The Tauri-based hbm-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(hbmHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(hbmHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * HBM AGENT to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(hbmHome, opts)
