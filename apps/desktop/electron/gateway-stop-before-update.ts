 ...
 *
 * So we delegate to `hbm gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `hbm update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(hbmCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(hbmCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(hbmCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(hbmCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(hbmCliPath, args)
  }

  if (!existsSync(hbmCliPath)) {
    return false
  }

  try {
    exec(hbmCliPath, args, {
