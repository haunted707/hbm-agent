 ...
 *   1. HBM_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\hbm\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect HBM_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.HBM_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'hbm', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'hbm', 'git', 'usr', 'bin', 'bash.exe'))
  }
