// Launching the HBM AGENT TUI in the user's OWN terminal emulator.
//
...
// `hbm --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's HBM AGENT runtime is often a venv Python invoked
//     as `python -m hbm_cli.main`, not a `hbm` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The HBM AGENT
