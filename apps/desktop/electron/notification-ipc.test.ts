...
import type { HbmNotification } from './notification-types'
...
it('returns native clicks and approval actions to the emitting window, not the primary', () => {
  ...
  const notify = host.handle.mock.calls[0][1] as (event: IpcMainInvokeEvent, payload: HbmNotification) => boolean
  ...
  expect(source.webContents.send).toHaveBeenCalledWith('hbm:focus-session', payload.focusSessionId)
  host.shown[0].emit('action', { actionIndex: 1 }, undefined)
  expect(source.webContents.send).toHaveBeenCalledWith('hbm:notification-action', {
  ...
  expect(primary.webContents.send).not.toHaveBeenCalled()
  host.shown[0].emit('click')
  expect(primary.webContents.send).toHaveBeenCalledWith('hbm:focus-session', payload.focusSessionId)
})
