...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'hbm-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/hbm-runner-test',
    sourceRepoRoot: null,
    hbmHome: '/tmp/hbm-runner-test',
    logRoot: '/tmp/hbm-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'hbm-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'hbm-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/hbm-agent',
      hbmHome: '/tmp/hbm'
    }),
    ['--dir', '/tmp/hbm-agent', '--hbm-home', '/tmp/hbm', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/hbm-agent', '--hbm-home', '/tmp/hbm', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/hbm', '--hbm-home', '/tmp/home', '--branch', 'main']
  )
})
