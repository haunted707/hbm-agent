...
export const ZOOM_STORAGE_KEY = 'hbm:desktop:zoomLevel'
 ...
 * but forgot to emit 'hbm:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('hbm:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `hbm:zoom:changed`, which never fired —
