// Profile-delete routing logic for the `hbm:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_hbm_home() and
...
/** Parse a `hbm:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its HBM_HOME.
 *
...
/**
 * Route the next `hbm:api` request away from the primary/window backend
 ...
 * ensure_hbm_home() recreates the directory the delete just removed.
 */
