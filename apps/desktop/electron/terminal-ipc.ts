...
// registry, and the hbm:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // HBM_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.HBM_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip HBM AGENT' TUI into
    ...
    env.TERM_PROGRAM = 'HBM AGENT'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a hbm/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from HBM_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.HBM_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `hbm:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('hbm:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('hbm:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('hbm:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('hbm:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('hbm:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('hbm:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
