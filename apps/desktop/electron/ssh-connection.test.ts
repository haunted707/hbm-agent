...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env HBM_DASHBOARD_SESSION_TOKEN=abc123deadbeef HBM_DESKTOP=1 hbm dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /HBM_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /HBM_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-HBM AGENT-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-HBM AGENT-Session-Token: hdr_888'), /X-HBM AGENT-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-HBM AGENT-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('hbm', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v hbm', 15000)
  assert.equal(args[args.length - 1], 'command -v hbm')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `hbm-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `hbm-ssh-test-${process.pid}-${Date.now()}`)
...
test('withRemoteTimeout kills a hung probe remotely instead of orphaning it (#110478)', async () => {
  ...
  const wrapped = withRemoteTimeout('hbm --version 2>&1', 15)
