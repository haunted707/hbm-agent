...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.hbm`). Default's state.db and gateway.pid live here. */
  hbmHome: string
  /** Named-profile root (`~/.hbm/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isHbmProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `hbmHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, hbmHome: string, profilesRoot: string): string {
  return name === 'default' ? `${hbmHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, hbmHome: string, profilesRoot: string): string {
  return name === 'default' ? `${hbmHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveHbmHome(profilesRoot: string, hbmHome?: string): string {
  if (hbmHome) {
    return hbmHome
  }

  // Tests that predate hbmHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live hbm process.
 ...
 *
 * `hbmHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isHbmProcess'> & { hbmHome?: string }
): string[] {
  const hbmHome = resolveHbmHome(profilesRoot, deps.hbmHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, hbmHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.hbmHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$HBM_HOME`, not `$HBM_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `hbm --profile default` and is worse than writing nothing
