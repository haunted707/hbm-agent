...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'hbm-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'hbm-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'HBM AGENT Test'], { cwd: dir })
