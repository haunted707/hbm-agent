...
beforeEach(() => {
  ...
  document.querySelector('hbm-watch')?.remove()
  delete (window as unknown as Record<string, unknown>).__hbmWatch
})

/** The overlay lives in a closed shadow root, so a test can only see the host. */
const host = () => document.querySelector('hbm-watch')
...
const drawn = () =>
  (window as unknown as { __hbmWatch: { parts: Record<string, HTMLElement> & { shadow: ShadowRoot } } })
    .__hbmWatch.parts
...
describe('watchInPage', () => {
  it('mounts one host on documentElement and reuses it across stages', () => {
    ...
    expect(document.querySelectorAll('hbm-watch')).toHaveLength(1)
  ...
  it('rebuilds its chrome when the injected source changes underneath it', () => {
    ...
    const stamp = (value: number) => {
      ;(window as unknown as Record<string, unknown>).__hbmWatchTag = value
    }
