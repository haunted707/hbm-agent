...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __hbmWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__hbmWatch || (win.__hbmWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('hbm-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('hbm-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __hbmWatchTag?: number }).__hbmWatchTag
