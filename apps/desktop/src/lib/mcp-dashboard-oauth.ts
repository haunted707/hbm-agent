...
const UPDATE_BACKEND = 'Update the HBM AGENT backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.hbmDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update HBM AGENT Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.hbmDesktop.openExternal(authUrl)
    let pollFailures = 0
