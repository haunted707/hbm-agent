...
afterEach(() => {
  ...
  document.querySelectorAll('hbm-annotate').forEach(node => node.remove())
})
...
describe('annotateInPage overlay', () => {
  it('installs a host and tears it down completely', () => {
    ...
    expect(document.querySelector('hbm-annotate')).toBeTruthy()

    api.teardown()

    expect(api.isInstalled()).toBe(false)
    expect(document.querySelector('hbm-annotate')).toBeNull()
  })
  ...
  it('keeps a pin glued to its element after the element moves', () => {
    ...
    const pin = () =>
      document.querySelector('hbm-annotate')!.shadowRoot!.querySelector('[data-annotate-pin="1"]') as HTMLElement
  ...
  it('keeps the picked element when the draft becomes a pin, even if the selector is wrong', () => {
    ...
    const api = annotateInPage(document)
    api.install()
    const host = document.querySelector('hbm-annotate') as HTMLElement
  ...
  it('shows the draft marker before a capture can photograph it', async () => {
    ...
    const shadow = document.querySelector('hbm-annotate')!.shadowRoot!
