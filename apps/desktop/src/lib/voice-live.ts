...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to HBM AGENT.
 *
 ...
 * that into an ordinary HBM AGENT turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * HBM AGENT keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (HBM AGENT) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await hbmApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by HBM AGENT; late results for an
  ...
  /** Quiet progress for the live model ("HBM AGENT is running the tests…"). */
