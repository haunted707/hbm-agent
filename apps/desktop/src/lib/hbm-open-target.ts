 ...
 * `hbm://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `hbm://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `hbm://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type HbmOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const HBM_PROTOCOL = 'hbm:'
...
export function normalizeHbmOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('hbm://') || trimmed.startsWith(`${HBM_PROTOCOL}//`)) {
    try {
      ...
      // hbm://index-network/intent/1 → /index-network/intent/1
...
export function resolveHbmOpenPath(target: HbmOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeHbmOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeHbmOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeHbmOpenString(target.path)
 ...
 * (`hbm://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromHbmDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveHbmOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveHbmOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `hbm://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromHbmDeepLink('open', name, params)
}
