...
/** Why a known HBM AGENT command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active HBM AGENT profile', surface: action('profile') },
  {
    name: '/skin',
 ...
 * `hbm_cli/commands.py::desktop_surface_registry` by
...
function isKnownHbmSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * HBM AGENT' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownHbmSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with HBM AGENT and were never asked for. It is
