...
// HBM AGENT is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.hbmMode
 ...
 * the only carriers of HBM AGENT' palette — are dropped so the paste target's
