...
//
// When someone is in a hands-free "Hey HBM AGENT" voice chat, the natural way to
...
// phrase (optionally addressed to HBM AGENT), so a real turn that merely contains
...
// Optional address prefixes so "hbm stop" / "ok stop" / "hey hbm, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey hbm', 'hey hbm,', 'hbm', 'hbm,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("hbm") is not a stop command on its own.
      continue
    }
 ...
 * to HBM AGENT). Returns false for anything that merely contains "stop" as part of
