...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.hbmDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.hbmDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.hbmDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.hbmDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `hbm-media://remote/${file}${scope ? `?${scope}` : ''}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `hbm-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.hbm/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.hbmDesktop?.saveGatewayFile) {
    throw new Error('Desktop file download bridge is unavailable')
  }

  return window.hbmDesktop.saveGatewayFile({
