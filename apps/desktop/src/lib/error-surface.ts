...
export interface ErrorRecoveryPlan {
  ...
  editMessage: boolean
  /** Reveal the HBM AGENT data folder so the user can free space (disk_full). */
  openHbmFolder: boolean
...
const CODE_PLANS: Partial<Record<ErrorCodeKey, Partial<ErrorRecoveryPlan>>> = {
  ...
  disk_full: { openHbmFolder: true, retry: true },
