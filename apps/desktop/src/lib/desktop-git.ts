...
// (window.hbmDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['hbmDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.hbmDesktop

  if (!desktop) {
    throw new Error('HBM AGENT Desktop bridge is unavailable')
  }
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: HbmGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: HbmGitBranch[] }>('branches', { path: repoPath })).branches,

  baseBranchList: async repoPath =>
    (await gitGet<{ branches: HbmGitBaseBranch[] }>('base-branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<HbmRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<HbmReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<HbmReviewShipInfo>('review/ship-info', { path: repoPath }),

    prList: (repoPath, branches, numbers) =>
      gitPost<HbmRepoPullRequests>('review/pr-list', { branches, numbers: numbers ?? [], path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  ...
  return isDesktopFsRemoteMode() ? remoteGit : window.hbmDesktop?.git
}
