...
function harness() {
  ...
  Object.defineProperty(window, 'hbmDesktop', { configurable: true, value: { mcpOauth: bridge, api, openExternal } })
  let relayed = false
...
describe('Desktop MCP client callback lifecycle', () => {
  ...
  it.each(['cancel', 'legacy'])('cleans up the owner before opening an unsafe authorization on %s', async outcome => {
    ...
    if (outcome === 'cancel') {
      ...
      await expect(action).rejects.toThrow('Update the HBM AGENT backend')
    }
