import { hbmApi } from '@/api/client'
import type {
  HbmConnection,
  HbmReadDirResult,
  HbmReadFileTextResult,
  HbmSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: HbmConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: HbmConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<HbmReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<HbmReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<HbmReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<HbmReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.hbmDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: HbmSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
