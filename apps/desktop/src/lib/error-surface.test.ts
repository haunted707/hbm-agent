...
describe('free-tier refusals', () => {
  const surface = parseErrorSurface({
    ...
    layer: 'provider',
    message: '  Using HBM AGENT without signing in is switched off right now. To sign in: /login. ',
    provider: 'nous',
    retryable: false
  })!

  it('carries the backend sentence and offers the free sign-in, never an OAuth re-login', () => {
    expect(surface.message).toBe('Using HBM AGENT without signing in is switched off right now. To sign in: /login.')
    const plan = errorRecoveryPlan(surface)
