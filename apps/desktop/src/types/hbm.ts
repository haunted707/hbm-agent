import type { ConnectionRequestPayload } from '@hbm/shared'
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when HBM AGENT doesn't know how to remove it. */
...
export interface FreeTierStatus {
   ...
   *  (`hbm_cli/anon_auth.py`), `error` its sentence, `retryable` whether a
...
export interface EnvVarInfo {
  ...
  // in hbm_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `hbm model` uses — instead of
...
export type HbmConfigRecord = Record<string, unknown>
...
// non-desktop) HBM AGENT simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (hbm.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors hbm_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with HBM AGENT, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with HBM AGENT but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `hbm tools` active-provider detection). */
  is_active: boolean
 ...
 *  not HBM AGENT); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** hbm-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** hbm-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/hbm/update/check` — the backend's own update state.
