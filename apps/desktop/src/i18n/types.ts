...
export interface Translations {
  ...
  settings: {
    ...
    gateway: {
      ...
      sshKeyTitle: string
      sshKeyDesc: string
      sshHbmPathTitle: string
      sshHbmPathDesc: string
      sshHbmPathPlaceholder: string
  ...
  commandCenter: {
    ...
    hbmActiveSessions: (version: string, count: number) => string
  ...
  freeTier: {
    ...
    // One sentence per backend code (`hbm_cli/anon_auth.py::ANON_*`); the copy never says
    // the free MODEL is off — what is unavailable is using HBM AGENT without signing in.
    setupFailed: {
      gateClosed: string
      paused: string
  ...
  assistant: {
    thread: {
      ...
      errorOpenHbmFolder: string
      errorOpenHbmFolderFailed: string
