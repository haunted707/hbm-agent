...
export const ja = defineLocale({
  sessionImport: {
    title: '別のアプリから続ける',
    subtitle: '会話をHBM AGENTに取り込み、続きを始めましょう。',
    action: 'セッションを取り込む',
    readingFrom: '読み込み元',
  ...
  boot: {
    ready: 'HBM AGENT Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'HBM AGENT の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      retryingRemoteBackend: 'リモート HBM AGENT バックエンドに再接続中…',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingHbmDesktop: 'HBM AGENT Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'HBM AGENT バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に HBM AGENT バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'HBM AGENT バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateHbm: 'HBM AGENT を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。HBM AGENT Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'HBM AGENT が応答を待っています。',
      turnDoneTitle: 'HBM AGENT が完了しました',
      turnDoneBody: '',
  ...
  sendDiagnostics: {
    ...
    failedHint:
      'ターミナルから `hbm debug share --nous` を実行するか、`hbm debug share --local` でアップロードせずにレポートを表示することもできます。',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を HBM AGENT のデフォルトに戻しますか？',
    ...
    vault: {
      ...
      otpHint: '2FA を有効にするときにサイトが表示する「セットアップキー」。保存すると HBM AGENT がコードを生成します。',
      ...
      sources: {
        ...
        notInstalled: name =>
          `未検出です。${name} のコマンドラインツールをインストールしてサインインすると、HBM AGENT が自動的に検出します。`,
        disabledDesc: '検出済みですが、HBM AGENT では無効になっています。',
        lockedDesc:
          '検出済み。エージェントがログイン情報を必要とするときにロック解除を求めます。今すぐ解除することもできます。',
        unlockedDesc: 'このセッションでロック解除済み。30分間操作がないか HBM AGENT を閉じると自動的にロックされます。',
        statusLocked: 'ロック中',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は HBM AGENT がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'HBM AGENT が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'HBM AGENT がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'HBM AGENT がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'HBM AGENT',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、HBM AGENT をシステム設定に合わせます。',
      ...
      reactionsDesc:
        'iMessage風の絵文字タップバック — メッセージにリアクションでき、HBM AGENTもあなたのメッセージにリアクションします。',
      tipsTitle: 'アプリ内ヒント',
      tipsDesc:
        'アプリや HBM AGENT からのヒントをときどき表示します。各ヒントは一度だけ表示されます。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      tipsReset: (count: number) => `${count}件のヒントをもう一度表示`,
      toursTitle: 'ガイドツアー',
      toursDesc:
        '各ステップを強調しながら、HBM AGENT がアプリを案内します。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、HBM AGENT の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。HBM AGENT を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'HBM AGENT がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'HBM AGENT が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'HBM AGENT が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを HBM AGENT がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから HBM AGENT 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'HBM AGENT ランタイムは更新されましたが、デスクトップアプリ自体は古いビルドのままです。アプリを更新するまで、新しいインターフェース機能(Bot Mode など)は表示されません。下の更新を実行してアプリを再ビルドしてください。それでもこの警告が消えない場合は、最新のデスクトップインストーラーから再インストールしてください。',
      bundleOutOfSyncAction: 'インストーラーを入手',
      bundleSwapPending: '再起動して更新を完了',
      bundleSwapPendingDesc:
        '更新されたアプリはすでにインストール済みです。HBM AGENT を再起動するだけで新しいビルドが読み込まれます。チャットや設定はそのまま保持されます。',
      bundleSwapPendingAction: 'HBM AGENT を再起動',
      updates: '更新',
      checkNow: '今すぐ確認',
      checking: '確認中…',
      seeWhatsNew: '新機能を見る',
      ...
      automaticUpdatesDesc: 'HBM AGENT はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、HBM AGENT を開かずにプロンプトを送信します。',
    ...
    gateway: {
      ...
      intro:
        'HBM AGENT Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している HBM AGENT バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      envOverrideTitle: '環境変数がこのデスクトップセッションを制御しています。',
      envOverrideDesc:
        '保存された設定を使用するには HBM_DESKTOP_REMOTE_URL と HBM_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      localTitle: 'ローカルゲートウェイ',
      localDesc:
        'ローカルホストでプライベートな HBM AGENT バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの HBM AGENT バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      remoteUrlTitle: 'リモート URL',
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/hbm などのパスプレフィックスもサポートしています。',
      ...
      restartingMessage: 'HBM AGENT Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · HBM AGENT ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'HBM AGENT は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshHbmPathTitle: 'HBM AGENT パス（任意）',
      sshHbmPathDesc: 'リモートの hbm バイナリへのフルパス。空欄 = 自動検出。',
      sshHbmPathPlaceholder: '自動検出',
      sshTestConnection: 'SSH をテスト',
      sshConnect: '接続',
      sshButtonsHint: '「保存」は次回起動時に適用され、「接続」は今すぐ再接続します。',
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— HBM AGENT を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。HBM AGENT は非対話的に ssh を実行します。',
      sshErrHostKey:
        '前回の接続以降、ホスト鍵が変更されています。想定どおりか確認し、ssh-keygen -R <host> を実行してから再接続してください。',
      sshErrNotInstalled:
        'リモートホストに HBM AGENT がインストールされていません。リモートでインストールする（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）か、HBM AGENT パスを設定してください。',
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。HBM AGENT Desktop の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
      sshErrTimeout: 'SSH 接続がタイムアウトしました。ホストが到達不能、またはスリープ中の可能性があります。',
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの HBM AGENT を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを HBM AGENT に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `ビルド ${tag}、${backend} バックエンド。サーバーは HBM AGENT が起動・管理します。`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。HBM AGENT がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          ...
          body: 'HBM AGENT が既定ブラウザプロファイルのスナップショットでブラウジングできるようにすると、サイトはログイン済みの状態で開きます。',
  ...
  skills: {
    ...
    skillArchivedMessage: 'hbm curator restore で復元できます。',
  ...
  starmap: {
    ...
    emptyDesc: 'HBM AGENT がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには HBM AGENT を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには HBM AGENT を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら HBM AGENT が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'HBM AGENT デスクトップを設定' },
    ...
    hbmActiveSessions: (version, count) => `HBM AGENT ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: 'どちらの方法でも、あなたが管理するボットを接続し、資格情報はこの HBM AGENT にのみ保存されます。',
      quickSetup: 'クイックセットアップ',
      recommended: '推奨',
      quickHelp:
        'QR コードをスキャンして Telegram で確認します。HBM AGENT がボットを作成し、あなたの Telegram ユーザー ID を自動検出します。',
      createWithQr: 'QR で作成',
      starting: '開始中…',
      replaceWarning:
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@hbm:example.org' },
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: 'このプロファイルのセッションは、このパソコンではなく指定したリモートの HBM AGENT で実行されます。',
      urlLabel: 'リモートアドレス',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した HBM AGENT 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'HBM AGENT はモデルの変更を保存しませんでした。',
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。HBM AGENT が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには HBM AGENT バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'HBM AGENT から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには HBM AGENT バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'HBM AGENT を起動中...',
    placeholderReconnecting: 'HBM AGENT に再接続中…',
    ...
    attachUrlDesc: 'HBM AGENT がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'HBM AGENT にコミットと PR を任せる',
  ...
  updates: {
    stages: {
      ...
      pull: 'もうすぐ完了…',
      pydeps: '仕上げ中…',
      update: 'HBM AGENT を更新中…',
      rebuild: 'デスクトップアプリを再ビルド中…',
      restart: 'HBM AGENT を再起動中…',
      done: '更新が完了しました',
      manual: 'ターミナルから更新',
    ...
    unsupportedMessage: 'このバージョンの HBM AGENT はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの HBM AGENT をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の HBM AGENT バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'HBM AGENT をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'HBM AGENT は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために HBM AGENT デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'HBM AGENT アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に HBM AGENT を再度開きます。更新中はご自分で HBM AGENT を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると HBM AGENT が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 HBM AGENT が自動的に再度開きます。',
    errorTitle: '更新が完了しませんでした',
    errorBody: 'ご安心ください。何も失われていません。今すぐ再試行できます。',
    blockerTitle: 'HBM AGENT を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて HBM AGENT を更新',
    foreignBlockerBody:
      'HBM AGENT はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'HBM AGENT は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  guidedGreeting: {
    line: 'やあ、どうぞ。HBM AGENT です。二分だけください、あなたに合わせて整えます。それから、本当にやりたいことに取りかかりましょう。\n\nまずは、何とお呼びすればいいですか。',
  ...
  install: {
    ...
    oneTimeTitle: 'HBM AGENT には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'HBM AGENT Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している HBM AGENT ゲートウェイに接続するか、このコンピューターに HBM AGENT をローカルインストールします。',
    connectExistingTitle: '既存の HBM AGENT に接続',
    connectExistingShort: '既存環境に接続',
    connectExistingDesc:
      'セッショントークンまたはブラウザーサインインでリモートバックエンドを使用します。ローカルインストールは開始されません。',
    installLocalTitle: 'HBM AGENT をローカルにインストール',
    installLocalDesc: 'HBM AGENT をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。',
    localStartUnavailable:
      'ローカルインストールを開始できません。HBM AGENT Desktop を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の HBM AGENT に接続',
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。HBM AGENT Desktop がトークンとブラウザーサインインのどちらが必要かを検出します。',
    remoteUrlTitle: 'ゲートウェイ URL',
    remoteUrlDesc: 'HBM AGENT ゲートウェイのベース URL を使用します。リモートの場合は https:// を含めてください。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hbm',
    probing: 'ゲートウェイ認証方式を検出中...',
    probeError: 'その HBM AGENT ゲートウェイに到達できませんでした。',
    identityProvider: 'ID プロバイダー',
    authTitle: '認証',
    ...
    settingUpTitle: 'HBM AGENT を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の HBM AGENT CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の HBM AGENT インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。HBM AGENT インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'HBM AGENT のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'HBM AGENT はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'HBM AGENT を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — HBM AGENT を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に HBM AGENT を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — HBM AGENT が自動接続します',
    ...
    authorizeThere: 'そこで HBM AGENT を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。HBM AGENT をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。HBM AGENT は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'HBM AGENT を再起動中...',
      askRestart: 'HBM AGENT にサーバーの再起動を依頼',
      lookingRestart: taskId => `HBM AGENT は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'HBM AGENT はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、HBM AGENT にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `HBM AGENT がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'HBM AGENT がサーバーを再起動できませんでした。',
      stillWorking:
        'HBM AGENT はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'HBM AGENT が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'HBM AGENT ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.hbm/config.yaml) に追加されます。HBM AGENT はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    secretDesc: 'HBM AGENT は続行するための認証情報が必要です。',
    ...
    vaultSaveDesc: origin =>
      `HBM AGENT は ${origin} のサインインページに到達しましたが、保存されたログイン情報がありません。ここで一度入力すると、このマシン上で暗号化して保存され、ページに直接入力されます。モデルはパスワードを一切見ません。`,
    ...
    vaultCodeDesc: site =>
      `${site} がワンタイムコード（SMS、メール、または認証アプリ）を求めています。ここに入力すると HBM AGENT がページに入力します。モデルはコードを一切見ません。`,
    vaultCodeLabel: 'コード',
    vaultCodeFootnote:
      'ヒント：「設定 → パスワードとログイン」でこのログインに認証キーを保存すると、HBM AGENT がコードを自動入力します。',
    vaultCodeSkip: 'スキップ',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには HBM AGENT Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには HBM AGENT Desktop を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`hbm gateway` は起動していますか？'
    }
  },
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で HBM AGENT が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても HBM AGENT',
        text: 'Telegram、Discord、Slack などに接続。同じエージェント、同じ記憶のままです。'
      },
      artifacts: {
        title: 'HBM AGENT が作ったものすべて',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の HBM AGENT です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
