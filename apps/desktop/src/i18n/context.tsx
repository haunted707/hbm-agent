import { applyDocumentLocale, isRecord } from '@hbm/shared/i18n'
import { createContext, type ReactNode, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'

import { getHbmConfigRecord, type HbmConfigRecord, saveHbmConfig } from '@/hbm'
...
export interface I18nConfigClient {
  getConfig: () => Promise<HbmConfigRecord>
  saveConfig: (config: HbmConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.hbmDesktop?.api) {
    ...
    return getHbmConfigRecord(undefined, { includeDefaults: false })
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.hbmDesktop?.api) {
    ...
    return saveHbmConfig(config, undefined, { preserveLanguage: true })
  }
}

export function getConfigDisplayLanguage(config: HbmConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: HbmConfigRecord, locale: Locale): HbmConfigRecord {
    ...
    const loadLocale = () => {
      ...
      return configClient
        ...
        .then(async config => {
          ...
          const machineProfile = await window.hbmDesktop?.getMachineProfile?.().catch(() => null)
