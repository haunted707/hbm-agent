...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в HBM AGENT и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в HBM AGENT.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в HBM AGENT. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в HBM AGENT',
    continue: 'Продолжить в HBM AGENT',
  ...
  boot: {
    ready: 'HBM AGENT Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек HBM AGENT',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду HBM AGENT…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingHbmDesktop: 'Запуск HBM AGENT Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс HBM AGENT завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс HBM AGENT завершился при запуске.',
    ...
    failure: {
      title: 'HBM AGENT не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд HBM AGENT старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updateHbm: 'Обновить HBM AGENT',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить HBM AGENT Desktop.',
    ...
    native: {
      ...
      inputBody: 'HBM AGENT ожидает ваш ответ.',
      turnDoneTitle: 'HBM AGENT завершил',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям HBM AGENT по умолчанию?',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока HBM AGENT в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'HBM AGENT задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока HBM AGENT был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока HBM AGENT был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте HBM AGENT следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и HBM AGENT сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия HBM AGENT — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из HBM AGENT и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого HBM AGENT останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые HBM AGENT может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как HBM AGENT обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда HBM AGENT обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Рантайм HBM AGENT обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'HBM AGENT автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      loading: 'Загрузка конфигурации HBM AGENT…',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая HBM AGENT.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами HBM AGENT, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется HBM AGENT Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'HBM AGENT Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм HBM AGENT, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз HBM AGENT, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт HBM AGENT Cloud.',
      kindSshDesc: 'Установка HBM AGENT, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет HBM AGENT (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в HBM AGENT Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом HBM AGENT в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      envOverrideTitle: 'Переменные окружения управляют этой сессией приложения.',
      envOverrideDesc:
        'Сбросьте HBM_DESKTOP_REMOTE_URL и HBM_DESKTOP_REMOTE_TOKEN, чтобы использовать сохранённую настройку ниже.',
      modeTitle: 'Режим подключения',
      localTitle: 'Локальный шлюз',
      localDesc: 'Запускает приватный бэкенд HBM AGENT на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду HBM AGENT.',
      remoteAuthHint:
      ...
      cloudDesc: 'Войдите в HBM AGENT Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'HBM AGENT Cloud',
      cloudSignIn: 'Войти в HBM AGENT Cloud',
      cloudSignedIn: 'Вы вошли в HBM AGENT Cloud',
      cloudNeedsSignIn: 'Войдите в HBM AGENT Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов HBM AGENT Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в HBM AGENT Cloud',
      cloudSignedOutTitle: 'Выход из HBM AGENT Cloud',
      cloudSignedOutMessage: 'Сессия HBM AGENT Cloud сброшена.',
      ...
      remoteUrlDesc: 'Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /hbm.',
      ...
      restartingMessage: 'HBM AGENT Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · HBM AGENT ${version}` : ''}`,
      ...
      sshDesc:
        'HBM AGENT запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshHbmPathTitle: 'Путь к HBM AGENT (необязательно)',
      sshHbmPathDesc: 'Полный путь к бинарнику hbm на удалённой машине. Пусто = автоопределение.',
      sshHbmPathPlaceholder: 'автоопределение',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — HBM AGENT найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — HBM AGENT запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'HBM AGENT не установлен на удалённой машине. Установите его там (curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh) или задайте путь к HBM AGENT.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим HBM AGENT Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите HBM AGENT на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в HBM AGENT. Проверьте конфигурацию ниже — она пришла из ссылки, а не из HBM AGENT.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. HBM AGENT проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте HBM AGENT на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
  ...
  skills: {
    ...
    skillArchivedMessage: 'Восстановить через hbm curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как HBM AGENT создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите HBM AGENT, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите HBM AGENT, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — HBM AGENT уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка HBM AGENT desktop' },
    ...
    hbmActiveSessions: (version, count) => `HBM AGENT ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle:
        'Оба способа подключают бота под вашим контролем и сохраняют его данные только в этой установке HBM AGENT.',
      quickSetup: 'Быстрая настройка',
      recommended: 'Рекомендуется',
      quickHelp:
        'Отсканируйте QR-код и подтвердите в Telegram. HBM AGENT создаст бота и определит ваш Telegram ID автоматически.',
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ID пользователя бота', placeholder: '@hbm:example.org' },
  ...
  profiles: {
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды HBM AGENT: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'HBM AGENT не сохранил это изменение модели.'
    },
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. HBM AGENT выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд HBM AGENT, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из HBM AGENT. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд HBM AGENT, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск HBM AGENT...',
    placeholderReconnecting: 'Переподключение к HBM AGENT…',
    placeholderFollowUp: 'Отправить продолжение',
    newSessionPlaceholders: [
      'Что будем делать?',
      'Дайте HBM AGENT задачу',
      'О чём вы думаете?',
      'Опишите, что нужно',
      'Что обсудим?',
    ...
    attachUrlDesc: 'HBM AGENT загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить HBM AGENT открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем HBM AGENT…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем HBM AGENT…',
    ...
    unsupportedMessage: 'Эта версия HBM AGENT не может обновлять себя из приложения.',
    ...
    availableBody: 'Новая версия HBM AGENT готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда HBM AGENT готова к установке.',
    ...
    manualBody:
      'HBM AGENT установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'HBM AGENT подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение HBM AGENT (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор HBM AGENT возьмёт процесс на себя в своём окне и автоматически перезапустит HBM AGENT, когда закончит. Пожалуйста, не открывайте HBM AGENT вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. HBM AGENT переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем HBM AGENT откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить HBM AGENT?',
    blockerBody:
      'HBM AGENT нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить HBM AGENT',
    foreignBlockerBody:
      'HBM AGENT не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'HBM AGENT может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  guidedGreeting: {
    line: 'Заходите. Я HBM AGENT. Дайте мне пару минут — обустрою тут всё под вас, а потом займёмся тем, что вам правда нужно.\n\nДля начала: как к вам обращаться?',
  ...
  install: {
    ...
    oneTimeTitle: 'HBM AGENT требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка HBM AGENT Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу HBM AGENT или установите HBM AGENT локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему HBM AGENT',
    ...
    installLocalTitle: 'Установить HBM AGENT локально',
    installLocalDesc: 'Скачайте HBM AGENT, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите HBM AGENT Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему HBM AGENT',
    remoteSetupDesc: 'Введите URL вашего шлюза. HBM AGENT Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза HBM AGENT, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/hbm',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу HBM AGENT.',
    ...
    settingUpTitle: 'Настройка HBM AGENT',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция HBM AGENT CLI или desktop. Остановите все работающие инстанции HBM AGENT и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик HBM AGENT скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас HBM AGENT',
    headerDesc: 'Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.',
    preparingInstall: 'HBM AGENT завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем HBM AGENT…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать HBM AGENT',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите HBM AGENT локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — HBM AGENT подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте HBM AGENT там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте HBM AGENT там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. HBM AGENT покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'HBM AGENT перезапускается...',
      askRestart: 'Попросить HBM AGENT перезапустить сервер',
      lookingRestart: taskId => `HBM AGENT ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'HBM AGENT работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите HBM AGENT открыть страницу.',
      finishedRestarting: message => `HBM AGENT завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'HBM AGENT не смог перезапустить сервер.',
      stillWorking:
        'HBM AGENT всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'HBM AGENT загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз HBM AGENT не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.hbm/config.yaml). HBM AGENT больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
  ...
  prompts: {
    ...
    secretDesc: 'HBM AGENT нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите HBM AGENT Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите HBM AGENT Desktop, чтобы сохранять изображения',
    ...
    handoff: {
      ...
      timedOut: 'Превышено время ожидания шлюза. Выполняется ли `hbm gateway`?'
    }
  },
  errors: {
