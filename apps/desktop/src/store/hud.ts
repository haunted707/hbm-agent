 ...
 * composer with the reply scrolling above it, so HBM AGENT can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.hbmDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.hbmDesktop?.hud
...
export function resetHudLayout(): void {
  void window.hbmDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.hbmDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.hbmDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
