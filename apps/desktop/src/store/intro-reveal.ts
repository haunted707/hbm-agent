 ...
 *
 * This store is the only writer of hbm-intro-reveal-seen-v1. First-run eligibility is guest onboarding
...
const SEEN_KEY = 'hbm-intro-reveal-seen-v1'
...
/** The film will never play for this identity (HBM_SKIP_INTRO). Record it
...
export function isIntroRevealEnabled(): boolean {
  return isOnboardingEnabled() && window.hbmDesktop?.skipIntro !== true
}

/** The guided flow is on but its intro film was turned off at launch
 * (HBM_SKIP_INTRO). Distinct from isIntroRevealEnabled(): false there also
...
export function startIntroReveal(): void {
  ...
  void window.hbmDesktop?.introReveal?.open({ hideMain: true }).catch(finishIntroReveal)
}
...
export function finishIntroReveal(): void {
  ...
  void window.hbmDesktop?.introReveal?.close({ showMain: true }).catch(() => undefined)
}

export function installIntroRevealBridgeListeners(): () => void {
  const bridge = window.hbmDesktop?.introReveal
