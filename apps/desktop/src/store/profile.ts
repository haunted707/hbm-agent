import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@hbm/shared'
import { atom, batch, computed } from 'nanostores'

import type { HbmConnection } from '@/global'
import { getProfiles, hbmApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/hbm'
...
import type { ProfileInfo } from '@/types/hbm'
...
// /api/profiles/active `current`). "default" is the root ~/.hbm. This is the
...
function profileListSource(connection: HbmConnection | null): null | string {
  if (!connection) {
    return null
  }
...
const PROFILE_ORDER_STORAGE_KEY = 'hbm.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'hbm.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await hbmApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new HBM_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.hbmDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<HbmConnection | null> {
  const getConnection = window.hbmDesktop?.getConnection
...
export async function ensureGatewayProfile(profile: string | null | undefined): Promise<void> {
  ...
  // #89206 split-brain — observed live as atom 'default' over a hbm-setup
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<HbmConnection | null> {
  const getConnectionFor = window.hbmDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'hbm.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.hbmDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.hbmDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.hbm) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.hbmDesktop?.touchBackend?.(target).catch(() => undefined)
}
