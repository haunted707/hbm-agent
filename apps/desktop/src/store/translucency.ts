 ...
 * `@hbm/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-hbm-glass]`
 * block in styles.css).
 */
 ...
 * sets `hbmDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.hbmDesktop?.glassSupported === 'boolean'
    ? window.hbmDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.hbmDesktop?.translucencySupported === 'boolean'
    ? window.hbmDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-hbm-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-hbm-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-hbm-glass-on', glassLive)
  root.toggleAttribute('data-hbm-glass', glassOn)
  root.toggleAttribute('data-hbm-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-hbm-glass-scope', scope)
  } else {
    root.removeAttribute('data-hbm-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.hbmDesktop?.setTranslucency?.(state)
  })
