...
vi.mock('@/hbm', () => ({
  getHbmConfigRecord: vi.fn(async () => ({})),
  saveHbmConfig: vi.fn(async () => undefined)
}))
...
it('keeps the desktop toggle local across config refreshes', async () => {
  for (const fails of [false, true]) {
    for (const enabled of [false, true]) {
      ...
      vi.mocked(saveHbmConfig).mockClear()
      ...
      try {
        ...
        expect(saveHbmConfig).not.toHaveBeenCalled()
        expect(localStorage.getItem('hbm.desktop.autoSpeakReplies')).toBe(fails ? null : String(enabled))
      } finally {
        write.mockRestore()
      }
    }
  ...
  for (const fails of [false, true]) {
    for (const enabled of [false, true]) {
      ...
      try {
        ...
        expect(localStorage.getItem('hbm.desktop.autoSpeakReplies')).toBeNull()
        prefs.applyAutoSpeakFromConfig({ voice: { auto_tts: enabled } })
        prefs.applyAutoSpeakFromConfig({ voice: { auto_tts: !enabled } })
        expect(prefs.$autoSpeakReplies.get()).toBe(enabled)
        expect(localStorage.getItem('hbm.desktop.autoSpeakReplies')).toBe(fails ? null : String(enabled))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye hbm', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye hbm')
  })
