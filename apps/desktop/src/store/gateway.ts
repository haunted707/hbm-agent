...
import type { HbmConnection } from '@/global'
import { HbmGateway, setApiRequestConnection } from '@/hbm'
...
function dialProfile(
  desktop: NonNullable<typeof window.hbmDesktop>,
  profile: string,
...
const isOpen = (gateway: HbmGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: HbmConnection) => void
  /**
...
interface GatewayRegistryState {
  ...
  primaryGateway: HbmGateway | null
  ...
  $gateway: ReturnType<typeof atom<HbmGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('hbm.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<HbmGateway | null>(null),
...
export function setPrimaryGateway(gateway: HbmGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<HbmConnection, 'connectionId'> | null): void {
...
export function activeGateway(): HbmGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the hbm module (null
...
function publishActiveConnection(connection: HbmConnection): void {
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: HbmGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`HBM AGENT gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.hbmDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update HBM AGENT Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<HbmGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "HBM AGENT gateway is not connected" on the Sessions `+` action (#88880).
