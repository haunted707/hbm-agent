import type { ConnectionState } from '@hbm/shared'
...
import type { SessionInfo, UsageStats } from '@/types/hbm'
...
const COMPOSER_MODEL_KEY = 'hbm.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'hbm.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'hbm.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'hbm.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'hbm.desktop.composer.fast'
...
function composerScopeForConnection(connection: HbmConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'hbm.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'hbm.desktop.lastRoute'
...
function workspaceCwdKey(connection: HbmConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.hbmDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.hbmDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<HbmConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'hbm.desktop.sessionOwnerHints.v1'
 ...
 * HbmConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<HbmConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useHbmConfig.
