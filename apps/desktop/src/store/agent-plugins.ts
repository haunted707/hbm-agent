...
/**
 * Feature store for backend (agent) plugins — the native HBM AGENT plugins plus
 ...
 * primitives `hbm plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native HBM AGENT. */
  portable?: boolean
 ...
 *  HBM_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's HBM_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
