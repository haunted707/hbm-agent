...
test('405 method-not-allowed toasts a restart in plain words with a Restart HBM AGENT action', () => {
  ...
  expect($notifications.get()[0]?.action?.label).toBe(en.notifications.actions.restartHbm)
  $notifications.get()[0]?.action?.onClick()
...
test('code-skew 503 unwraps to a restart-required summary, not raw IPC JSON', () => {
  notifyError(
    new Error(
      'Error invoking remote method \'hbm:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066."}'
    ),
    'Could not load models'
  )

  expect(lastMessage()).toMatch(/still running the old version/i)
  expect(lastMessage()).not.toMatch(/hbm:api|systemctl|backend/i)
  const before = $backendRestartRequest.get()
  expect($notifications.get()[0]?.action?.label).toBe(en.notifications.actions.restartHbm)
  $notifications.get()[0]?.action?.onClick()
  expect($backendRestartRequest.get()).toBe(before + 1)
