...
export function setZoomPercent(percent: number): void {
  window.hbmDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.hbmDesktop?.zoom) {
  void window.hbmDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.hbmDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
