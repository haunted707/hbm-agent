...
// Current CLI (`hbm_cli/skills_hub.py::_scan_block_message`): "Not installed:"
...
test('the current CLI "Not installed:" tail parses into findings + trust', () => {
  const hardBlock = [
    ...
    "Not installed: the security scan found 3 high-risk pattern(s) in 'org/skill' (listed above). " +
      'HBM AGENT never installs unverified skills with high-risk findings, even with --force. ' +
      'Review the findings or ask the author to fix them; to read the skill without installing, run `hbm skills inspect org/skill`.'
  ]
