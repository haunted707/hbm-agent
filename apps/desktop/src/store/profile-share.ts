 ...
 *
 * The archive is the CLI's own `hbm profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/hbm'
...
import type { ProfileDesktopOverlay } from '@/types/hbm'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'HBM AGENT profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.hbmDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.hbmDesktop?.selectPaths?.({
