 ...
 *  Working in `~/www/hbm-agent`, the draft says "hbm-agent" constantly
 *  — a "Use skill: hbm-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`hbm-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/hbm-agent-dev`, `plugin:skill`). */
