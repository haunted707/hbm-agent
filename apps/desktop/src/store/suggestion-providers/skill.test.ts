...
describe('collidesWithWorkspace', () => {
  it('suppresses a skill named exactly like the cwd folder', () => {
    expect(collidesWithWorkspace('hbm-agent', '/Users/b/www/hbm-agent')).toBe(true)
  })

  it('suppresses inside worktree-suffixed folders too', () => {
    expect(collidesWithWorkspace('hbm-agent', '/Users/b/www/hbm-agent-suggest')).toBe(true)
  })
  ...
  it('does not suppress on substring-only overlap', () => {
    ...
    expect(collidesWithWorkspace('clean', '/Users/b/www/hbm-agent')).toBe(false)
  })

  it('never collides when detached (empty cwd)', () => {
    expect(collidesWithWorkspace('hbm-agent', '')).toBe(false)
  })
})
...
describe('skillTouchedInMessages', () => {
  ...
  it('matches qualified skill names (category/name, plugin:name)', () => {
    expect(
      skillTouchedInMessages('hbm-agent-dev', [toolCall('skill_view', { name: 'github/hbm-agent-dev' })])
    ).toBe(true)
    expect(
