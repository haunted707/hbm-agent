...
const LAST_PROFILE_STORAGE_KEY = 'hbm.desktop.lastProfileByConnection'
...
async function rememberConnection(connectionId: string): Promise<void> {
  const setLastUsed = window.hbmDesktop?.connections?.setLastUsed
...
export async function selectConnection(connectionId: string, options: SelectConnectionOptions = {}): Promise<void> {
  ...
  // The primary local descriptor (startHbm) historically publishes without
