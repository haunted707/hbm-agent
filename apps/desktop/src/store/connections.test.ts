...
beforeEach(() => {
  ...
  ensureGatewayAgent.mockImplementation(async (connectionId, profile, options) => {
    ...
    $connection.set({
      ...
      // The primary local descriptor from startHbm() historically carried
  ...
  vi.stubGlobal('window', { hbmDesktop: { api, connections: { list, setLastUsed } }, localStorage })
})
...
describe('selectConnection', () => {
  ...
  it('restores the last-used profile on switch-back even when the primary local descriptor is profile-less', async () => {
    ...
    expect(JSON.parse(localStorage.getItem('hbm.desktop.lastProfileByConnection') || '{}')).toEqual({
      local: 'mac'
    })

    // A later resync republishes a profile-less primary descriptor (the
    // startHbm shape). The remembered pair is the authority for "what was
