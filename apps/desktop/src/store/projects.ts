...
import type { HbmGitBaseBranch, HbmGitBranch } from '@/global'
import { getHbmConfig, hbmApi, type HbmGateway } from '@/hbm'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/hbm'
...
const PROJECT_SCOPE_KEY = 'hbm.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active HBM AGENT profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await hbmApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<HbmGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<HbmGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero HBM AGENT
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getHbmConfig(context.profile))
...
// repo, returning where HBM AGENT should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<HbmGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<HbmGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.hbmDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.hbmDesktop?.writeClipboard?.(path)
  }
}
