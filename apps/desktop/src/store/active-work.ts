...
import type { HbmActiveWork } from '@/global'
import { $sessions } from '@/store/session'
import { $workingSessionIds } from '@/store/session-states'

const $activeWork = computed([$workingSessionIds, $sessions], (workingIds, sessions): HbmActiveWork => {
...
if (typeof window !== 'undefined') {
  ...
  $activeWork.subscribe(work => {
    ...
    lastSent = next
    window.hbmDesktop?.setActiveWork?.(work)
  })
}
