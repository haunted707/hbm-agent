...
vi.mock('@/hbm', () => ({
  getHbmConfigRecord: mocks.getHbmConfigRecord,
...
afterEach(() => {
  ...
  mocks.getHbmConfigRecord.mockReset()
...
it('shows the toast with Sign in + Disable, then stays quiet for a day and re-nudges after it', async () => {
  ...
  mocks.getHbmConfigRecord.mockResolvedValue(servers)
...
it('honors a persisted snooze in a fresh module session, then re-notifies after it expires', async () => {
  const servers = { mcp_servers: { linear: { url: 'https://mcp.linear.app/mcp', auth: 'oauth' } } }
  const key = 'hbm:mcp-health-snooze-until:default::linear'
  let clock = 1_700_000_000_000
...
it('coalesces reconnects during a sweep into one fresh follow-up sweep', async () => {
  ...
  mocks.getHbmConfigRecord.mockReturnValueOnce(first).mockResolvedValue({ mcp_servers: {} })

  startMcpHealthChecker()
  mocks.gatewayState.set('open')
  await flush()
  expect(mocks.getHbmConfigRecord).toHaveBeenCalledTimes(1)
  ...
  await flush()
  await flush()
  expect(mocks.getHbmConfigRecord).toHaveBeenCalledTimes(2)
})
