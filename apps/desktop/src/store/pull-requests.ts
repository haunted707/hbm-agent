...
import type { HbmBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/hbm'
...
export const $pullRequestsByBranch = atom<Record<string, HbmBranchPullRequest>>({})
...
const $prScannedSessions = persistentAtom<string[]>('hbm.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: HbmBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
