import type { ModelOptionProvider } from '@hbm/shared'
...
import type { OAuthProvider, OAuthStartResponse } from '@/types/hbm'
...
export interface DesktopOnboardingState {
   ...
   *  overlay show its "HBM AGENT is ready" screen once even though the app is
...
const CONFIGURED_CACHE_KEY = 'hbm-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'hbm-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'HBM AGENT is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring hbm_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `hbm model` does (for Nous it honors the user's free/paid tier, so a
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'HBM AGENT could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but HBM AGENT still cannot resolve a usable provider. ${detail}`
    : 'Connected, but HBM AGENT still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext, stillWanted?: () => boolean) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'HBM AGENT Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.hbmDesktop?.openExternal) {
    try {
      await window.hbmDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `HBM AGENT still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but HBM AGENT still cannot reach ${url}.` }
    }
