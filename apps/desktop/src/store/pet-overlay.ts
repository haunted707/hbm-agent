 ...
 * app's bounds and stays visible while HBM AGENT is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'hbm.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'hbm.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.hbmDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.hbmDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.hbmDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.hbmDesktop?.petOverlay?.close()
}
