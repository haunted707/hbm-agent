...
const agentConn = (over: Partial<HbmConnection> = {}): HbmConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as HbmConnection

const localConn = (over: Partial<HbmConnection> = {}): HbmConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as HbmConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<HbmConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<HbmConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { hbmDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<HbmConnection>(() => undefined))
