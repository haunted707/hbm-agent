...
const { getProfiles } = await import('@/hbm')
...
const remoteConn = (over: Partial<HbmConnection> = {}): HbmConnection =>
  ({ baseUrl: 'https://hbm-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as HbmConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { hbmDesktop: { getConnection } })
