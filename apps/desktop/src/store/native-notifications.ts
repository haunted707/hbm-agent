...
import { type HbmOpenTarget, resolveHbmOpenPath } from '@/lib/hbm-open-target'
...
const STORAGE_KEY = 'hbm:native-notifications'
...
// "Backgrounded" = the user isn't on HBM AGENT. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `hbm://index-network/intent/1`.
   */
  activate?: string
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `hbm://index-network/intent/1`). */
  activate?: HbmOpenTarget
 ...
 *  user is away from HBM AGENT — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveHbmOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveHbmOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.hbmDesktop
