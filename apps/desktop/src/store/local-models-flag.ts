 ...
 *
 * Local models ship on main behind `--local` (either `hbm desktop --local`
 * or the flag on HBM AGENT.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.hbmDesktop?.localModelsEnabled === true
)
