...
import type { HbmReviewFile, HbmReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — HBM AGENT' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'hbm.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'hbm.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'hbm.desktop.reviewTreeMode'
const SELECTED_KEY = 'hbm.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<HbmReviewFile[]>([])
...
export const $reviewShipInfo = atom<HbmReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['hbmDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: HbmReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly HbmReviewFile[], path: string): HbmReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.hbmDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.hbmDesktop?.openExternal?.(url)
    }
