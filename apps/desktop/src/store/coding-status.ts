...
import type { HbmGitWorktree, HbmRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: HbmGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, HbmRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, HbmGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<HbmRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<HbmGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<HbmRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<HbmGitWorktree[]>>()
const $noRepoStatus = atom<HbmRepoStatus | null>(null)
const $noWorktrees = atom<HbmGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<HbmRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<HbmGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: HbmRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: HbmGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<HbmRepoStatus | null>
  seq: number
  target: string
}
