...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@hbm/shared'
...
const TILES_KEY = 'hbm.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'hbm.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'hbm.desktop.botChatSessions.v1'
 ...
 *  (hbm-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_hbm_home() re-creates
 * the profile directory the delete just removed (hbm-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (hbm-agent#94235). Tiles persisted
...
// Dev hook for automation (mirrors __HBM_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__HBM_SESSION_TILES__ = {
