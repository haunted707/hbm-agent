...
describe('local boot failure classification', () => {
  it('classifies the raw main-process failures into plain causes', () => {
    expect(
      classifyLocalBootFailure(
        'HBM AGENT backend exited before it became ready (1).\nRecent backend output:\nTraceback (most recent call last):\n  File "x.py"'
      )
    ).toBe('exitedEarly')
    expect(classifyLocalBootFailure('Timed out connecting to HBM AGENT backend after 45000ms')).toBe('timedOut')
    expect(classifyLocalBootFailure("EACCES: permission denied, open '/home/x/.hbm/state.db'")).toBe('permission')
  ...
  it('keeps the raw output out of the headline and behind details', () => {
    const raw = 'HBM AGENT backend exited before it became ready (1).\nRecent backend output:\nTraceback (most recent call last):'
