 ...
 *
 * The chat belongs to a persistent `hbm-setup` profile, so it survives onboarding and can be found again. `setup`
 * is the internal name throughout this module (the profile key, the atoms, the hidden `[setup]` notes); the user sees
 * only HBM AGENT and the title `Welcome to HBM AGENT`.
 *
...
export const SETUP_PROFILE = 'hbm-setup'
...
export const SETUP_CHAT_TITLE = 'Welcome to HBM AGENT'
...
export function buildFirstTaskRunbook(
  ...
  return [
    `You are HBM AGENT. The user's welcome chat just opened this session so one task can have room to run: ${task.trim()}.`,
...
const pluginRunbook = (root: string) => [
  'THIS IS A PLUGIN JOB: the thing you are building is a piece of the HBM AGENT app itself, and it will appear in the window the user is looking at right now. That is the whole point — do not let it become a script in a folder.',
  `A plugin is ONE file: \`${root}/<name>/plugin.js\`. Plain ESM, no build step, no package.json, no install. It imports from \`@hbm/plugin-sdk\` and calls \`jsx()\` from \`react/jsx-runtime\` directly (there is no JSX compiler in this path — writing \`<div>\` will not work). It default-exports \`{ id, name, register(ctx) }\` and \`register\` calls \`ctx.register({ id, area, order, render })\`. The runtime loads it the moment you save, and reloads it on every later save, so there is no restart to ask them for.`,
  'LOOK BEFORE YOU WRITE. Read the `building-hbm-desktop-plugins` skill first — it has the SDK surface, the areas you can render into, and the traps. If the machine has a checkout of NousResearch/plugins, read a plugin close to what you are making; those thirteen are reviewed and show the real shapes (a statusbar chip, a composer action, a full pane).',
  ...
  'Never ask them to restart the app, never edit anything outside their plugin folder, and never touch the HBM AGENT install itself. If the plugin errors on load, the app toasts it and keeps running — read the error, fix the file, save again.'
]
...
export async function buildFirstTaskSeedMessages(
  ...
  const root = plan === 'plugin' ? await window.hbmDesktop?.desktopPluginsRoot?.() : undefined
...
export async function ensureSetupProfile(request: GatewayRequest): Promise<void> {
  try {
    await request('profiles.create', {
      description: 'Where HBM AGENT met you — walks your first run, then checks in as you find your feet.',
      name: SETUP_PROFILE,
      clone_from: 'default',
      share_auth: true,
      no_alias: true,
