...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'HBM AGENT Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'HBM AGENT docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('HBM AGENT Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'HBM AGENT docs', url: 'https://example.com/docs' }
    ])
  })
})
