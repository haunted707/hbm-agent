...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's HBM_HOME/logs. On a
  ...
  // HBM_HOME/logs, and its parent is the HBM AGENT data folder itself (what
  ...
  const openLocalDir = useCallback(async (resolve: (logsRoot: string) => string, failedMessage: string) => {
    try {
      const root = await window.hbmDesktop?.logsRoot?.()
      ...
      const result = await window.hbmDesktop?.openDir?.(resolve(root))
  ...
  const openHbmFolder = useCallback(
    () => openLocalDir(root => root.replace(/[\\/]+logs[\\/]*$/, ''), copy.errorOpenHbmFolderFailed),
    [copy.errorOpenHbmFolderFailed, openLocalDir]
  )
  ...
  const localFolders = Boolean(window.hbmDesktop?.logsRoot)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {plan.openHbmFolder && localFolders && (
        <button className="aui-error-action" onClick={() => void openHbmFolder()} type="button">
          {copy.errorOpenHbmFolder}
        </button>
      )}
      {plan.retry && (
