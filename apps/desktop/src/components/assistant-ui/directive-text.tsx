...
const HBM_REF_TYPES = WIRE_REFERENCE_KINDS
type HbmRefType = (typeof HBM_REF_TYPES)[number]
...
const HBM_DIRECTIVE_RE = referenceRe()
...
export const hbmDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(HBM_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...hbmDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing HBM AGENT directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.hbmDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.hbmDesktop?.readFileDataUrl(id)
