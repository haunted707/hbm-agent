 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.hbm.send(prompt)` (or declarative
 * `data-hbm-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'hbm-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'hbm-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `hbm.send(prompt)`.
 ...
 *  wires `data-hbm-send` so declarative HTML works with zero script:
 *  `<button data-hbm-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.hbm={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-hbm-send]"):null;' +
    'if(el)send(el.getAttribute("data-hbm-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `hbm serve` — renders live.)
