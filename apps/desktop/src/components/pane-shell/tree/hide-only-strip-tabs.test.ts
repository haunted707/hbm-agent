...
function sessionsBotsTree() {
  ...
  registerPane('hbm-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'hbm-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('hbm-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('hbm-bots:pane')
    expect($hiddenStripTabs.get()).toContain('hbm-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('hbm-bots:pane')

    expect(setStripTabHidden('hbm-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('hbm-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('hbm-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('hbm-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('hbm.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('hbm-bots:pane')
    ...
    revealTreePane('hbm-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('hbm-bots:pane')
    expect(window.localStorage.getItem('hbm.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'hbm-bots:pane', title: 'hbm-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'hbm-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
