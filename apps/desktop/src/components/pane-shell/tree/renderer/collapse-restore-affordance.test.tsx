...
describe('Sessions/Bots strip — #91223', () => {
  beforeEach(() => {
    ...
    registerPane('hbm-bots:pane', { hideOnly: true, placement: 'left' }, 'Bots')
  ...
  it('double-clicking the Sessions tab leaves the strip and both chips', () => {
    ...
    expect(tabEl('hbm-bots:pane')).toBeTruthy()
  ...
  it('double-clicking the Bots tab leaves the strip too', () => {
    ...
    tap(tabEl('hbm-bots:pane')!)
    doubleTap(tabEl('hbm-bots:pane')!)
...
describe('docked tool tile — collapsing keeps the restore chip', () => {
  beforeEach(() => {
    registerPane('workspace', { placement: 'main', uncloseable: true }, 'Chat')
    registerPane('hbm-bots:routines', { placement: 'main', width: '250px' }, 'Cronjobs')
    $layoutTree.set(
      split('row', [
        group(['workspace'], { active: 'workspace', id: 'g-main' }),
        group(['hbm-bots:routines'], { active: 'hbm-bots:routines', id: 'g-routines' })
      ])
    )
  })
  ...
  it('clicking the active tab does not collapse the tile or drop its label', () => {
    ...
    expect(tabEl('hbm-bots:routines')).toBeTruthy()
    tap(tabEl('hbm-bots:routines')!)

    expect(zoneAt(1).minimized).toBeFalsy()
    expect(tabEl('hbm-bots:routines')).toBeTruthy()
    expect(tabEl('hbm-bots:routines')?.textContent).toMatch(/cronjobs/i)
  })
  ...
  it('collapsing via the store does not dismiss the pane from the tree', () => {
    collapseTreePane('hbm-bots:routines')

    expect(zoneAt(1).minimized).toBe(true)
    expect($dismissedPanes.get().has('hbm-bots:routines')).toBe(false)
    expect(zoneAt(1).panes).toContain('hbm-bots:routines')
