...
describe('mergePluginPackages', () => {
  it('shows a unified package as ONE row with both halves, never two rows', () => {
    const rows = mergePluginPackages(
      [desktop({ id: 'media', name: 'Media Studio', packageName: 'hbm-media-studio' })],
      [agent({ name: 'hbm-media-studio', has_desktop_half: true, description: 'Generate media.' })]
    )
    ...
    expect(rows[0].agent?.name).toBe('hbm-media-studio')
  })

  it('a desktop half whose agent half is absent from THIS profile offers the install-here affordance', () => {
    const rows = mergePluginPackages([desktop({ id: 'media', packageName: 'hbm-media-studio' })], [])
