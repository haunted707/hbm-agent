...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/hbm'
...
export function SkillsView({
  ...
  const registryBridge = window.hbmDesktop?.connections
  const rosterBridge = window.hbmDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'HBM AGENT (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill HBM AGENT ships but
