...
import { getHbmConfig, getHbmConfigDefaults } from '@/hbm'
...
interface HbmConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useHbmConfig({ activeSessionIdRef }: HbmConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshHbmConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getHbmConfig(), getHbmConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying HBM AGENT' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshHbmConfig, sttEnabled, voiceMaxRecordingSeconds }
}
