...
import { useHbmConfig } from './use-hbm-config'

vi.mock('@/hbm', () => ({
  getHbmConfig: vi.fn(),
  getHbmConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'hbm.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getHbmConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getHbmConfig>>)

describe('useHbmConfig refreshHbmConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to HBM AGENT' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useHbmConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshHbmConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useHbmConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getHbmConfig>>>()
    vi.mocked(getHbmConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useHbmConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshHbmConfig(true)
    })
    expect(getHbmConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getHbmConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getHbmConfig>>>()
    vi.mocked(getHbmConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useHbmConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshHbmConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getHbmConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getHbmConfig>>>()
    vi.mocked(getHbmConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshHbmConfig(true)
      refreshC = result.current.refreshHbmConfig(true)
    })
