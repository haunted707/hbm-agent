import type { GatewayEvent } from '@hbm/shared'
...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'setup.ready') {
    // The boot bootstrap (hbm_cli/free_tier_bootstrap.py) resolved the
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (HBM AGENT activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as HbmSkin | undefined, { apply: true })
    }
