import type { BillingBlock } from '@hbm/shared'
 ...
 * billing-specific toast — never the generic "HBM AGENT error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'HBM AGENT reported an error',
