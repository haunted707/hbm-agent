...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshHbmConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshHbmConfig — two REST calls (config + defaults) per event, per
