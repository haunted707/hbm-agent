...
describe('gateway `error` event → error card + toast', () => {
  ...
  it('toasts the plain explanation, not the lease jargon, and keeps the raw text as detail', () => {
    ...
    expect(toast.title).toBe("HBM AGENT couldn't finish the reply")
    expect(toast.message).toMatch(/open in another HBM AGENT window or terminal/)
  ...
  it('keeps the server\'s own plain copy as the toast message when no code was recovered', () => {
    ...
    const serverCopy = 'HBM AGENT could not start the assistant for this chat. Check your model settings and try again.'
