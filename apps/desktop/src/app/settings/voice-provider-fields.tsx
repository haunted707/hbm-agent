...
export function VoiceProviderFields({
  ...
  const { data: loadedConfig } = useHbmConfigRecord(profile)
  ...
  const writeConfigCache = useMemo(() => hbmConfigCacheWriter(profile), [scopeKey])

  const { data: schemaResponse } = useQuery({
    queryKey: ['hbm-config-schema'],
    queryFn: () => getHbmConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  const [baseline, setBaseline] = useState<HbmConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveHbmConfigRecord(diffConfig(baseline ?? {}, config), profile)
        .then(() => {
          setBaseline(config)
