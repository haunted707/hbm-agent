...
import { disconnectOAuthProvider, listOAuthProviders } from '@/hbm'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/hbm'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.hbmDesktop?.terminal)
...
//      in hbm_cli/provider_catalog.py) — the SAME provider identity
//      `hbm model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // HBM AGENT can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider HBM AGENT can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // HBM AGENT never deletes creds another tool owns behind a silent API call.
