...
import { DEFAULT_REASONING_EFFORT, REASONING_EFFORT_VALUES } from '@hbm/shared'
...
import { hbmConfigCacheWriter, invalidateHbmConfig, useHbmConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `hbm model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in hbm_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = useHbmConfigRecord(scopeProfile)
  const setConfig = useMemo(() => hbmConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateHbmConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveHbmConfig(setNested({}, key, value), scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `hbm model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.hbmDesktop?.recycleBackend?.(scopeProfile)
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — HBM AGENT runs the flow for you.`}
          </p>
        )}
