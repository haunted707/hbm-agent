...
import type { ToolsetConfig } from '@/types/hbm'
...
const getHbmConfigSchema = vi.fn()
const saveHbmConfig = vi.fn()
const saveHbmConfigRecord = vi.fn()
...
vi.mock('@/hbm', () => ({
  ...
  getHbmConfigSchema: () => getHbmConfigSchema(),
  saveHbmConfig: (config: unknown) => saveHbmConfig(config),
  saveHbmConfigRecord: (config: unknown, profile?: unknown) => saveHbmConfigRecord(config, profile),
...
beforeEach(() => {
  ...
  getHbmConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveHbmConfig.mockResolvedValue({ ok: true })
  saveHbmConfigRecord.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveHbmConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveHbmConfigRecord.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
    ...
    expect(saveHbmConfigRecord.mock.calls.at(-1)?.[1]).toBeUndefined()
  })
  ...
  it('autosaves the inline voice fields into the profile the panel is scoped to', async () => {
    ...
    await waitFor(() => expect(saveHbmConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const [saved, forwarded] = saveHbmConfigRecord.mock.calls.at(-1) as [
