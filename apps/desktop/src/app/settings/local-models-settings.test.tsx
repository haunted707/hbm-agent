...
import type { LocalCatalogModel, LocalHardware, LocalModelsStatus, LocalRuntimeJob } from '@/types/hbm'
...
import * as hbm from '@/hbm'

const mocked = vi.mocked(hbm)
...
describe('BrowseSection', () => {
  ...
  it('searches HF after a pause and shows fit-priced files on demand', async () => {
    ...
    try {
      vi.mocked(hbm.searchHFModels).mockResolvedValue({
        hits: [{ downloads: 872724, gated: false, likes: 47, repo: 'unsloth/Qwen3.8-27B-GGUF', updated: '2026-08-18' }]
      })
      vi.mocked(hbm.listHFRepoFiles).mockResolvedValue({
        files: [
      ...
      expect(hbm.searchHFModels).not.toHaveBeenCalled()
      await act(async () => {
        await vi.advanceTimersByTimeAsync(400)
      })
      expect(hbm.searchHFModels).toHaveBeenCalledWith('qwen')
      ...
      vi.mocked(hbm.downloadBrowsedModel).mockResolvedValue({ job_id: 'j1', model_id: 'Qwen3.8-27B-Q4_K_M' })
      fireEvent.click(q4Btn)
      await act(async () => {
        await vi.runOnlyPendingTimersAsync()
      })
      expect(hbm.downloadBrowsedModel).toHaveBeenCalledWith('unsloth/Qwen3.8-27B-GGUF', ['Qwen3.8-27B-Q4_K_M.gguf'])
    } finally {
      vi.useRealTimers()
    }
  })
...
describe('added-by-you rows', () => {
  it('staged models outside the catalog get the full action set', async () => {
    vi.mocked(hbm.getLocalModelsStatus).mockResolvedValue({
      ...BASE_STATUS,
      loaded_models: { 'HBM AGENT-4.3-36B-Q5_K_M': 'loaded' },
      models: [{ id: 'HBM AGENT-4.3-36B-Q5_K_M', size_bytes: 25 * 2 ** 30, size_label: '25.0 GB' }],
      placement: {
        'HBM AGENT-4.3-36B-Q5_K_M': {
    ...
    vi.mocked(hbm.getLocalCatalog).mockResolvedValue({ models: [] })

    renderPane()
    await screen.findByText('HBM AGENT-4.3-36B-Q5_K_M')
