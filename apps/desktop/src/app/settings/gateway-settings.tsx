...
// HBM AGENT Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.hbmDesktop.setSecretStorageEncryption(on)
  ...
  // --- HBM AGENT Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.hbmDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteHbmPath: state.sshRemoteHbmPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.hbmDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.hbmDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'hbm-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.hbmDesktop.saveConnectionConfig({
        mode: state.mode,
      ...
      const result = await window.hbmDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.hbmDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.hbmDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.hbmDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.hbmDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.hbmDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.hbmDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.hbmDesktop.testConnectionConfig({
        mode: 'remote',
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
        remoteUrl: trimmedUrl
  ...
  if (!window.hbmDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* HBM AGENT Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/hbm"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteHbmPath: event.target.value }))}
                placeholder={g.sshHbmPathPlaceholder}
                value={state.sshRemoteHbmPath}
              />
            }
            description={g.sshHbmPathDesc}
            title={g.sshHbmPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.hbmDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
