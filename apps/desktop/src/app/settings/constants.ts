import { REASONING_EFFORTS } from '@hbm/shared'
...
// lib/personalities (mirrors hbm_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__hbm_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted HBM AGENT & Nous-trained models',
  ...
  { prefix: 'HBM_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // hbm_cli/config.py (local/groq/openai/mistral/elevenlabs).
  'stt.provider': ['local', 'groq', 'openai', 'mistral', 'xai', 'elevenlabs'],
  // How the desktop voice conversation is wired — tools/voice_live.py owns the
  // gpt-live branch (one full-duplex voice model delegating to HBM AGENT).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  browser: {
    useRealProfile:
      "Local browsing uses your real logins. HBM AGENT copies your default browser's profile (cookies, logins, preferences) into a managed snapshot and drives it with its packaged Chromium — your live profile is never opened directly, and the copy is refreshed from it on each run. Also lets the agent open a local real-profile session on request even when a cloud browser backend is configured. Only Chromium browsers (Chrome, Edge, Brave, Brave Origin, Chromium) are supported; a non-Chromium default fails with a clear message. Off by default."
  },
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before HBM AGENT stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters HBM AGENT can read from one file request.',
  approvals: {
    mode: 'How HBM AGENT handles commands that need explicit approval.',
  ...
  voice: {
    ...
    voiceChatMode:
      'chained: speech-to-text → HBM AGENT → text-to-speech with the providers below. gpt-live: one full-duplex OpenAI voice model (gpt-live-1) listens and talks, and hands every real request to HBM AGENT — any model you have selected answers with the full toolset. Needs an OpenAI API key; the voice layer bills $0.05 per minute.',
    gptLive: {
      voice: 'Voice for GPT-Live mode. Custom voice IDs are accepted.',
      instructions:
        'Extra sentences for the live voice persona (tone, pace, language). HBM AGENT keeps its own system prompt.'
    }
  },
  tts: {
    xai: {
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When HBM AGENT updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
