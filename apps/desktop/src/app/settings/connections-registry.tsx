 ...
 * agent sources (local runtime + any number of remote gateways / HBM AGENT Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.hbmDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.hbmDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `hbm update` to every eligible source; per-connection results
