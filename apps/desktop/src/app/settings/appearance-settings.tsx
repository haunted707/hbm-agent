...
function ResumeLastSessionSetting() {
  ...
  const configQuery = useHbmConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void saveHbmConfig(setNested({}, 'display.resume_last_session', on))
      .then(result => {
        if (!result.ok) {
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.hbmDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
