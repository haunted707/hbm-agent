...
import { type ProfileScope, saveHbmConfigRecord } from '@/hbm'
 ...
 * the default browser's profile (cookies/logins) into a HBM AGENT-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useHbmConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await saveHbmConfigRecord({ browser: { use_real_profile: on } }, profile)
        notify({
          kind: 'info',
