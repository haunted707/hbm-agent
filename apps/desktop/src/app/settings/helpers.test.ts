...
import type { HbmConfigRecord } from '@/types/hbm'
...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: HbmConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // HBM_QWEN_ shares the HBM_ stem with other integrations.
      expect(providerGroup('HBM_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: HbmConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: HbmConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: HbmConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: HbmConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: HbmConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: HbmConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: HbmConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: HbmConfigRecord = { toolsets: ['memory'] }
      const next: HbmConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: HbmConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: HbmConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: HbmConfigRecord = { toolsets: [] }
      const next: HbmConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: HbmConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: HbmConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `hbm config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: HbmConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: HbmConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: HbmConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: HbmConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: HbmConfigRecord = {}
      const draft: HbmConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: HbmConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: HbmConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: HbmConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: HbmConfigRecord = { toolsets: ['memory'] }
