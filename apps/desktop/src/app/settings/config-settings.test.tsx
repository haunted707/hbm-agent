...
vi.mock('@/hbm', () => ({
  ...
  saveHbmConfig: (config: unknown, profile?: string) => saveHbmConfig(config, profile),
...
beforeEach(() => {
  ...
  getHbmConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('renders and saves the Codex compression auto-raise setting', async () => {
    ...
    getHbmConfigSchema.mockResolvedValue({
      fields: {
    ...
    try {
      ...
      await vi.waitFor(() =>
        expect(saveHbmConfig).toHaveBeenCalledWith({ compression: { codex_gpt55_autoraise: false } }, undefined)
      )
    } finally {
      vi.useRealTimers()
    }
  ...
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getHbmConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await vi.waitFor(() => expect(saveHbmConfig).toHaveBeenCalledTimes(1))
      expect(saveHbmConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveHbmConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveHbmConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
