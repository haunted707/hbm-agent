...
import { saveHbmConfig } from '@/hbm'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { HbmConfigRecord } from '@/types/hbm'

import { setHbmConfigCache, useHbmConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: HbmConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig, dataUpdatedAt } = useHbmConfigRecord()
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setHbmConfigCache, so
    ...
    const timeout = window.setTimeout(() => {
      ...
      void saveHbmConfig(setNested({}, 'terminal.font_family', value))
        .then(result => {
          if (!result.ok) {
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="hbm-terminal-font-families"
          ...
          <datalist id="hbm-terminal-font-families">
