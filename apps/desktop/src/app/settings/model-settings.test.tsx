...
const getHbmConfigRecord = vi.fn()
const saveHbmConfig = vi.fn()
...
vi.mock('@/hbm', () => ({
  ...
  getHbmConfigRecord: () => getHbmConfigRecord(),
  saveHbmConfig: (config: unknown) => saveHbmConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'hbm-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['hbm-4', 'hbm-4-mini'],
        authenticated: true,
        capabilities: { 'hbm-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'hbm-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'nous', model: 'hbm-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nous', model: 'hbm-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getHbmConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveHbmConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'nous', model: 'hbm-4' })
  ...
  it('writes the profile default speed (service_tier) as a sparse patch, never the cached snapshot', async () => {
    ...
    getHbmConfigRecord.mockResolvedValue({
      agent: { reasoning_effort: 'medium', service_tier: 'normal' },
      auxiliary: { curator: { provider: 'auto', model: '', reasoning_effort: 'high' } }
    })
    await renderModelSettings()
    await waitFor(() => expect(getHbmConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() => expect(saveHbmConfig).toHaveBeenCalledWith({ agent: { service_tier: 'fast' } }))
  })
      ...
      providers: [
        {
          ...
          models: ['hbm-4'],
          authenticated: true,
          capabilities: { 'hbm-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('edits auxiliary reasoning effort below the selected model and applies it with the assignment', async () => {
    getAuxiliaryModels.mockResolvedValueOnce({
      main: { provider: 'nous', model: 'hbm-4' },
      tasks: [{ task: 'vision', provider: 'nous', model: 'hbm-4', base_url: '', reasoning_effort: null }]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'hbm-4' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('nous · hbm-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'nous', model: 'hbm-4', enabled: false }),
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'hbm:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in HBM AGENT Desktop, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/hbm:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { hbmDesktop: { recycleBackend: typeof recycleBackend } }).hbmDesktop = {
      recycleBackend
    }
