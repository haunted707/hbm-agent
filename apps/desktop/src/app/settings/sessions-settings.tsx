...
import type { HbmConfigRecord, SessionInfo } from '@/types/hbm'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.hbmDesktop) {
      return
    }

    let alive = true

    void getHbmConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        ...
        await saveHbmConfig({ sessions: { auto_archive: autoArchive, auto_archive_days: archiveDays } })
      } catch (err) {
...
// / Program Files), which buried any files HBM AGENT wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.hbmDesktop` is
    ...
    const settings = window.hbmDesktop?.settings
