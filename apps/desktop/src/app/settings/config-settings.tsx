...
import { getElevenLabsVoices, getHbmConfigSchema, saveHbmConfig } from '@/hbm'
...
import type { ConfigFieldSchema, HbmConfigRecord } from '@/types/hbm'

import { hbmConfigCacheWriter, useHbmConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<HbmConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useHbmConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => hbmConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['hbm-config-schema'] : ['hbm-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getHbmConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `hbm config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<HbmConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveHbmConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: HbmConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: HbmConfigRecord) => {
