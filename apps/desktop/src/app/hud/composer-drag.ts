...
function setWorkspaceTransfer(transferring: boolean): void {
  window.hbmDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.hbmDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.hbmDesktop?.hud?.beginMove?.()
}
 ...
 * (see hbm:hud:move-by — a transparent frameless window drifts wider on
