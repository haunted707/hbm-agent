...
export const HBM_PATHS_MIME = 'application/x-hbm-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal HBM AGENT sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.hbmDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(HBM_PATHS_MIME)
...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.hbmDesktop?.saveImageBuffer(data, blobExtension(blob), name)
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.hbmDesktop?.saveClipboardImage()
   ...
   * HBM AGENT-managed composer-pastes file via the main process, then attached
  ...
  const attachPastedText = useCallback(
    async (text: string) => {
      const save = window.hbmDesktop?.savePastedText
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.hbmDesktop?.getPathForFile ? window.hbmDesktop.getPathForFile(file) : ''
