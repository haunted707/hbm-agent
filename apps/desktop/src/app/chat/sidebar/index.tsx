...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/hbm'
...
// highlight markers (sqlite snippet() delimiters — see hbm_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('hbm:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('hbm:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask HBM AGENT to
