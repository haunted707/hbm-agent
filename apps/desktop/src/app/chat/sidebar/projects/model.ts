...
import type { SessionInfo } from '@/hbm'
...
// zero HBM AGENT sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, HbmGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, HbmGitWorktree[]>>({})
