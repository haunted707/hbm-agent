...
export function overlayInstallScript(source: string): string {
  return '(function(){var api=' + source + ';window.__hbmAnnotate=api;api.install();})()'
}
...
export async function teardownAnnotateOverlay(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    (function () {
      if (window.__hbmAnnotate && window.__hbmAnnotate.teardown) {
        window.__hbmAnnotate.teardown();
      }
      window.__hbmAnnotate = null;
    })()
  `)
}
...
export async function waitAnnotateEvent(guest: PreviewAnnotateGuest): Promise<AnnotatePageEvent> {
  const event = await guest.executeJavaScript(`
    window.__hbmAnnotate ? window.__hbmAnnotate.wait() : Promise.resolve({ type: 'end' })
  `)
...
export async function syncAnnotatePins(guest: PreviewAnnotateGuest, pins: AnnotatePinChrome[]): Promise<void> {
  await guest.executeJavaScript(`
    window.__hbmAnnotate && window.__hbmAnnotate.showPins(${JSON.stringify(pins)})
  `)
}
...
export async function showAnnotateDraft(
  ...
  await guest.executeJavaScript(`
    window.__hbmAnnotate && window.__hbmAnnotate.showDraft(${JSON.stringify(rect)}, ${number})
  `)
}

export async function hideAnnotateDraft(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    window.__hbmAnnotate && window.__hbmAnnotate.hideDraft()
  `)
}
...
export async function captureAnnotateCrop(
  ...
  await tryGuest(guest, 'window.__hbmAnnotate ? window.__hbmAnnotate.beginCapture() : null')

  try {
    return await guest.capture(padRect(rect))
  } finally {
    await tryGuest(guest, 'window.__hbmAnnotate && window.__hbmAnnotate.endCapture()')
  }
}
