        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.hbmDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.hbmDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask HBM AGENT to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.hbmDesktop?.watchPreviewFile ||
      !window.hbmDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.hbmDesktop.onPreviewFileChanged(payload => {
    ...
    void window.hbmDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.hbmDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.hbmDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:hbm-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.hbmDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.hbmDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.hbmDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.hbmDesktop?.openExternal(currentUrl)
                : undefined
            }
