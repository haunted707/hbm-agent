...
import { HBM_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'hbm:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.hbmDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(HBM_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
