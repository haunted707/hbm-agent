...
const preamble = () => `  var w = window;
  ...
  w.__hbmActHolder = w.__hbmActHolder || {};
  w.__hbmAct = ${actEngineSource()};
  w.__hbmWatch_fn = (${watchInPage.toString()});
  w.__hbmWatchTag = ${WATCH_TAG};
  var holder = w.__hbmActHolder;
  var act = function (a) { return w.__hbmAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__hbmWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__hbmHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'HBM-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
