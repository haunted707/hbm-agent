...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__hbmTourEngine) {
    ${driverIife}
    w.__hbmTourHolder = {};
    w.__hbmTourCollect = (${collectTourTargets.toString()});
    w.__hbmTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__hbm-tour-style')) {
    var style = document.createElement('style');
    style.id = '__hbm-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__hbmTourEngine(
    w.driver.js.driver,
    w.__hbmTourHolder,
