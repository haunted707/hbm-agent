...
describe('HUD mode', () => {
  it('keeps the mic inline, fans the toggles on hover, and offers no exit in the docked composer', async () => {
    ...
    expect(screen.getByLabelText('Wake word "hey hbm"')).toBeTruthy()
...
describe('wake-word ear visibility', () => {
  ...
  const findEar = async () => {
    ...
    return screen.findByLabelText('Wake word "hey hbm"')
  }

  it('stays reachable during a busy agent turn', async () => {
    applyWakeStatus({ available: true, enabled: true, listening: true, phrase: 'hey hbm' })
  ...
  it('stays reachable (enabled in config) even when a start was refused', async () => {
    applyWakeStatus({ available: true, enabled: true, listening: false, phrase: 'hey hbm' })
  ...
  it('stays reachable (never hides) even when unavailable and not enabled', async () => {
    applyWakeStatus({ available: false, enabled: false, listening: false, phrase: 'hey hbm' })
    applyWakeStartResult({ hint: 'run `hbm tools` (Voice section)', reason: 'unavailable', started: false })
    renderControls()
  ...
  it('shows a disabled paused ear inside the voice-conversation pill', () => {
    ...
    const ear = screen.getByLabelText('Wake word: "hey hbm" — paused during voice chat')
