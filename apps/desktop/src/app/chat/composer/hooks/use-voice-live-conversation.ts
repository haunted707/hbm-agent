...
interface VoiceLiveConversationOptions {
  ...
  enabled: boolean
  onFatalError?: () => void
  /** Interrupt the in-flight HBM AGENT turn (Stop-button seam). Fired when a new
   *  delegation supersedes one still running. */
  onInterrupt?: () => Promise<void> | void
  onStopWord?: () => void
  /** Submit a HBM AGENT turn: `text` is the user's last words (the bubble and the
...
/** Turn transcript fragments into the HBM AGENT turn: `prompt` is what the user
 ...
 * HBM AGENT. There is no `transcribing` phase: the voice model owns speech.
 */
export function useVoiceLiveConversation({
    ...
    const session = new VoiceLiveSession({
      ...
      onDelegation: (delegationId, context) => {
        ...
        void Promise.resolve(latest.current.onSubmit(prompt, voiceContext)).catch(error => {
          ...
          session.speak(delegationId, 'Sorry, I could not reach HBM AGENT for that request.')
          setDelegation(null)
          refreshStatus()
        })
      },
  ...
  // Drive the reply back into the voice: stream commentary as HBM AGENT writes
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (tool && tool !== lastToolLabelRef.current) {
        ...
        session.think(delegationId, `HBM AGENT is working: ${tool}. Not done yet.`)
      }
      ...
      if (
        ...
        if (spokenLengthRef.current === 0) {
          session.think(delegationId, 'HBM AGENT finished that request without a spoken result.')
        }
