...
describe('startNewSessionDrag', () => {
  ...
  it('labels the ghost with the project name for a project-row drag', () => {
    startNewSessionDrag(vi.fn(), fakePointerEvent(), { cwd: '/repo', label: 'New session in HBM AGENT Browser' })
    expect(captured.spec?.ghost).toEqual({ label: 'New session in HBM AGENT Browser' })
  })

  it('pins the created session to the project cwd on a center drop', () => {
    const onCreate = vi.fn()
    subZonePosition.mockReturnValue('center')
    const spec = engage(onCreate, { cwd: '/repo/hbm-browser' })
