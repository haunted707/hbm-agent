import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@hbm/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as HbmGateway | null
  const gatewayRef = useRef<HbmGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.hbmDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<HbmGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as HbmGateway | null
      }),
    []
  )
