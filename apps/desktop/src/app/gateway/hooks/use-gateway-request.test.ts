...
vi.mock('@/hbm', async importOriginal => {
  const actual = await importOriginal<typeof HbmModule>()

  class FakeHbmGateway {
  ...
  return { ...actual, HbmGateway: FakeHbmGateway }
})

import type * as HbmModule from '@/hbm'
import type { HbmGateway } from '@/hbm'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as HbmGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as HbmGateway, 'default')
  $gateway.set(primary as unknown as HbmGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.hbmDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as HbmGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { hbmDesktop: unknown }).hbmDesktop = { getConnection }
    $gateway.set(dropped)
