...
// bug that drives the REAL useGatewayBoot hook + REAL HbmGateway through a
...
function Harness({
  ...
  refreshHbmConfig = async () => undefined,
  refreshSessions
}: {
  beforeConnectionSwitch?: () => void
  refreshHbmConfig?: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
beforeEach(() => {
  ...
  ;(window as { hbmDesktop?: unknown }).hbmDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { hbmDesktop?: unknown }).hbmDesktop
  window.localStorage.removeItem('hbm.desktop.workspace-cwd')
  $currentCwd.set('')
  $busy.set(false)
  $awaitingResponse.set(false)
})
...
describe('primary failure foreground isolation', () => {
  it('ignores a boot snapshot superseded by a successful connection', async () => {
    ...
    ;(window as { hbmDesktop?: unknown }).hbmDesktop = desktop
    render(<Harness />)
    await flushAsync()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForHbm) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startHbm()'s remote branch awaits waitForHbm() for 45s before it
    ...
    // After ~45s waitForHbm gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('HBM AGENT backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshHbmConfig = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshHbmConfig={refreshHbmConfig} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForHbm, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting HBM AGENT…" forever; the renderer needs to own its own bound
  ...
  it('manual reconnect replaces only the active secondary route', async () => {
    ;(window as { hbmDesktop?: unknown }).hbmDesktop = {
      ...fakeDesktop(),
      getConnectionFor: vi.fn(async () => coderConn),
      getGatewayWsUrlFor: vi.fn(async () => coderConn.wsUrl)
    }
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote HBM AGENT gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "HBM AGENT couldn't start" overlay — the socket is open
  ...
  it('a backend exit while the boot overlay is up fails the overlay and does not add a dead-button toast', async () => {
    ...
    // HBM AGENT" toast here would do nothing when clicked; the overlay's own
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\HBM AGENT')
    expect($currentCwd.get()).toBe('C:\\HBM AGENT')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startHbm() rejects with a
  ...
  it('RETRY CONTRACT: a post-connect failure stays terminal even when its socket closes before boot catches it — a closed socket after a good dial is not a dial failure', async () => {
    ...
    const refreshHbmConfig = vi.fn(async () => {
    ...
    expect(refreshHbmConfig).toHaveBeenCalledTimes(1)
    expect($desktopBoot.get().error).toBeTruthy()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
    await advanceBackoff()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
