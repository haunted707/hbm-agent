...
export function markFirstBuildSession(storedId: string): void {
  writeKey('hbm.onboarding.first-build.v1', storedId)
}

export function endFirstBuildConnect(storedId: string): void {
  writeKey('hbm.onboarding.first-build.done.v1', storedId)
}

export function isFirstBuildSession(storedId: string | null | undefined): boolean {
  return (
    !!storedId &&
    readKey('hbm.onboarding.first-build.v1') === storedId &&
    readKey('hbm.onboarding.first-build.done.v1') !== storedId
  )
}

export function handoffReceiptKey(connection: null | string, guideStoredId: string): string {
  return `hbm.onboarding.handoff.v1.connection.${encodeURIComponent(connection ?? 'ambient')}.profile.default.guide.${encodeURIComponent(guideStoredId)}`
}
