...
import { getLatestSessionMessages } from '@/hbm'
...
import { useHbmConfigRecord } from '../hooks/use-config-record'
...
import { useHbmConfig } from '../session/hooks/use-hbm-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // "Restart HBM AGENT" from a toast: recycle the local backend the user is
  ...
  useEffect(() => {
    ...
    if (backendRestartRequest > 0) {
      if ($connection.get()?.mode === 'remote') {
        void reconnectGateway().catch(err => notifyError(err, translateNow('notifications.errors.restartHbmFailed')))

        return
      }

      void window.hbmDesktop?.recycleBackend?.(normalizeProfileKey($activeGatewayProfile.get())).catch(err =>
        notifyError(err, translateNow('notifications.errors.restartHbmFailed'))
      )
    }
  ...
  // Without this pin, the owner ladder sends session.create to hbm-setup
  ...
  const { refreshHbmConfig, sttEnabled, voiceMaxRecordingSeconds } = useHbmConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('hbm:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('hbm:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshHbmConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshHbmConfig])
  ...
  const configRecord = useHbmConfigRecord()
