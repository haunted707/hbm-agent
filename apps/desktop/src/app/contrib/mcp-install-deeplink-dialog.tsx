...
import { getHbmConfigRecord, saveMcpServers } from '@/hbm'
...
import { setHbmConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `hbm://mcp/install` deep links. The payload is
 ...
 * confirming lets HBM AGENT spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getHbmConfigRecord())
      ...
      setHbmConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
