...
import { pathFromHbmDeepLink, resolveHbmOpenPath } from '@/lib/hbm-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About HBM AGENT" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.hbmDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.hbmDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.hbmDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.hbmDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // hbm://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.hbmDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future hbmDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveHbmOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.hbmDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (hbm://index-network/intent/1, hbm://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromHbmDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.hbmDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.hbmDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.hbmDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.hbmDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
