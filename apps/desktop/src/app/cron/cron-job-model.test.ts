...
describe('lastErrorSummary', () => {
  it('strips the exception wrapper and markers, keeping only the first sentence', () => {
    const raw =
      "RuntimeError: Cron job 'x' has no model configured (job.model=None, HBM_MODEL=''). Set a per-job model via `hbm cron edit x --model <name>`."

    expect(lastErrorSummary(raw)).toBe("Cron job 'x' has no model configured (job.model=None, HBM_MODEL='').")
