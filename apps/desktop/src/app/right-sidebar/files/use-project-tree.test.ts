...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { hbmDesktop: { readDir: typeof readDir } }).hbmDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): HbmReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { hbmDesktop: unknown }).hbmDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: HbmReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<HbmReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: HbmReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: HbmReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: HbmReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<HbmReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.hbmDesktop is missing', async () => {
    delete (window as unknown as { hbmDesktop?: unknown }).hbmDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: HbmReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: HbmReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: HbmReadDirResult) => void) | undefined
