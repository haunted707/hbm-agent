...
const HBM_PATHS_MIME = 'application/x-hbm-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(HBM_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(HBM_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.hbmDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (hbm --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.hbmDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[hbm-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.hbmDesktop?.terminal?.write(sessionId, `${command}\r`)
