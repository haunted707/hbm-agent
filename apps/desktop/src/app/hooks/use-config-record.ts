...
import { getHbmConfigRecord, type ProfileScope, profileScopeKey } from '@/hbm'
...
//
// Distinct from session/hooks/use-hbm-config.ts, which is side-effecting —
// it pushes personality/cwd/voice/… into the session stores for live chat.
export const HBM_CONFIG_KEY = ['hbm-config-record'] as const
...
// share a cache row.
export const hbmConfigKey = (profile?: ProfileScope) =>
  profile == null ? HBM_CONFIG_KEY : ([...HBM_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useHbmConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    queryKey: hbmConfigKey(profile),
    ...
    queryFn: () => getHbmConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setHbmConfigCache writes the app-wide (base-key) record. Pass a profile to
...
export const setHbmConfigCache = writeCache<HbmConfigRecord>(HBM_CONFIG_KEY)
export const hbmConfigCacheWriter = (profile?: ProfileScope) =>
  writeCache<HbmConfigRecord>(hbmConfigKey(profile))

export const invalidateHbmConfig = (profile?: ProfileScope) =>
  queryClient.invalidateQueries({ queryKey: hbmConfigKey(profile) })
