...
## The desktop is its own chat surface on a `hbm serve` backend
...
(`@hbm/shared`: `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard also consumes.
The desktop has **no build/runtime dependency on the dashboard frontend**: it spawns a headless
`hbm serve` (`headless_backend=True` → `cmd_dashboard` skips `_build_web_ui` and exports
`HBM_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists).
`dashboard` and `serve` share `cmd_dashboard`/`start_server` but neither launches the other. It does
NOT embed `hbm --tui` — own composer, transcript, slash pipeline.
...
and ONLY when it does not (older managed install / PATH `hbm` not yet updated) rewrites argv to
...
The backend the app spawns is a **pooled `hbm serve --port 0` per (connection, profile)**: its
launch home is that profile, `HBM_DESKTOP=1` is set, and its in-process cron ticker stands down
...
- `src/lib/desktop-slash-commands.ts` is the load-bearing file: `DESKTOP_COMMAND_SPECS` (built-ins
  ...
  `CommandDef` in `hbm_cli/commands.py`; the live `commands.catalog` carries it, and
  `src/lib/desktop-slash-registry.json` (regenerate with `scripts/dump_desktop_slash_registry.py`;
  `tests/hbm_cli/test_desktop_slash_registry.py` + the vitest file fail on drift) is the offline
...
## Bot Mode (`src/plugins/hbm-bots/`) — one bot = ONE canonical forever-chat, identified by NAME

Each bot is a HBM AGENT **profile** with a persistent identity. This invariant regressed repeatedly,
...
**There is NO session-id pin.** The old design stored a pointer in `ui_meta['hbm-bots'].chat`;
...
in `src/plugins/hbm-bots/`: `canonical-chat-registry.test.ts` (tripwire: the open path never
