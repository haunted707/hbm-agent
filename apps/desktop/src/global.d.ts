import type { GatewayWsUrlResult } from '@hbm/shared'
import type { TranslucencyState } from '@hbm/shared/translucency'

import type { HbmNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    hbmDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`hbm --tui
      ...
      // HBM AGENT Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new HBM_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: HbmApiRequest) => Promise<T>
      notify: (payload: HbmNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<HbmReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<HbmReadFileTextResult>
      selectPaths: (options?: HbmSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<HbmPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<HbmPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<HbmPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: HbmActiveWork) => void
      setTitleBarTheme?: (payload: HbmTitleBarTheme) => void
       ...
       *  (HBM_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (HBM_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh HBM_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      readDir: (path: string) => Promise<HbmReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<HBM_HOME>/desktop-plugins),
      ...
      /** LOCAL `<HBM_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<HbmGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<HbmGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<HbmGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<HbmRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: HbmReviewScope, baseRef?: null | string) => Promise<HbmReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: HbmReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<HbmReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<HbmRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<HbmPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: HbmTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<HbmTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: HbmWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: HbmPreviewFileChanged) => void) => () => void
...
export interface HbmTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface HbmTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `hbm update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  hbmRoot?: string
...
export interface HbmConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface HbmTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface HbmActiveWork {
  count: number
  titles: string[]
}

export interface HbmWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a HBM AGENT Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted HBM AGENT Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected HBM AGENT Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the hbm gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered HBM AGENT Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface HbmApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface HbmPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface HbmReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface HbmPreviewWatch {
  id: string
  path: string
}
...
export interface HbmGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
// preselect it.
export interface HbmGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface HbmRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface HbmRepoStatus {
  branch: null | string
...
// turn began.
export type HbmReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface HbmReviewFile {
  path: string
  added: number
  removed: number
...
export interface HbmReviewList {
  files: HbmReviewFile[]
...
export interface HbmBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface HbmRepoPullRequests {
  ghReady: boolean
  prs: HbmBranchPullRequest[]
}
...
export interface HbmReviewShipInfo {
  ghReady: boolean
  pr: HbmReviewPr | null
}

export interface HbmReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface HbmReadDirResult {
  entries: HbmReadDirEntry[]
  error?: string
}

export interface HbmPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface HbmSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
