import { reconnectBackoffDelayMs } from '@hbm/shared'

import type { HbmConnection } from '@/global'
import { RECONNECT_ATTEMPT_TIMEOUT_MS, withTimeout } from '@/lib/with-timeout'

import { getApiRequestConnection, getApiRequestProfile, hbmApi, profileScoped } from './client'
 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<HbmConnection> {
  ...
  return withTimeout(
    window.hbmDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `hbmDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see HbmApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.hbmDesktop?.api) {
    throw new Error('HBM AGENT desktop bridge unavailable')
  }
