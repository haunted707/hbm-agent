...
import { capabilityScoped, hbmApi, type ProfileScope, profileScoped } from './client'
...
export function getMemoryProviderConfig(provider: string, profile?: null | string): Promise<MemoryProviderConfig> {
  return hbmApi<MemoryProviderConfig>({
...
export function startMemoryProviderOAuth(
  ...
  return hbmApi<MemoryProviderOAuthStatus>({
    ...profileScoped(profile),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `hbm memory` / `hbm curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return hbmApi<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return hbmApi<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return hbmApi<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return hbmApi<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
export function updateHbm(): Promise<ActionResponse> {
  return hbmApi<ActionResponse>({
    ...profileScoped(),
    path: '/api/hbm/update',
    method: 'POST'
...
export function checkHbmUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return hbmApi<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/hbm/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: ProfileScope): Promise<ActionStatusResponse> {
  return window.hbmDesktop.api<ActionStatusResponse>({
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return hbmApi<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return hbmApi<AudioSpeakResponse>({
    ...profileScoped(),
...
export function setTtsLease(lease: string, active: boolean): Promise<AudioTtsLeaseResponse> {
  return hbmApi<AudioTtsLeaseResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(profile?: null | string): Promise<ElevenLabsVoicesResponse> {
  return hbmApi<ElevenLabsVoicesResponse>({
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return hbmApi<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `hbm doctor` / `hbm security
// audit` / `hbm backup` / `hbm debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return hbmApi<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return hbmApi<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return hbmApi<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return hbmApi<DebugShareResponse>({
