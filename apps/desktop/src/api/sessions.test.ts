...
const hbmApi = vi.mocked(client.hbmApi)
...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    hbmApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'hbm-pi' })

    await deleteSession('sess-1', { connectionId: 'hbm-pi', profile: 'tommy' })

    expect(hbmApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((hbmApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('omits the profile from the body when none is given', async () => {
    ...
    const req = hbmApi.mock.calls[0][0] as { body: Record<string, unknown> }
