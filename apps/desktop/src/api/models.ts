import type { ModelOptionsResult } from '@hbm/shared'
...
import { capabilityScoped, hbmApi, type ProfileScope, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'

export function getGlobalModelInfo(profile?: null | string): Promise<ModelInfoResponse> {
  return hbmApi<ModelInfoResponse>({
...
export function getUsageAnalytics(days = 30, profile?: ProfileScope): Promise<AnalyticsResponse> {
  return window.hbmDesktop.api<AnalyticsResponse>({
...
export function getGlobalModelOptions(
  ...
  return hbmApi<ModelOptionsResult>({
    ...profileScoped(profile),
...
// curation `hbm model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(
  provider: string,
  profile?: null | string
): Promise<RecommendedDefaultModel> {
  return hbmApi<RecommendedDefaultModel>({
    ...profileScoped(profile),
...
export function getAuxiliaryModels(profile?: null | string): Promise<AuxiliaryModelsResponse> {
  return hbmApi<AuxiliaryModelsResponse>({
    ...profileScoped(profile),
...
export function getMoaModels(profile?: null | string): Promise<MoaConfigResponse> {
  return hbmApi<MoaConfigResponse>({
    ...profileScoped(profile),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(
  ...
  return hbmApi<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(profile),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(
  ...
  return hbmApi<ModelAssignmentResponse>({
    ...profileScoped(profile),
    path: '/api/model/set',
    method: 'POST',
    body
