import { JsonRpcGatewayClient } from '@hbm/shared'

import type { HbmApiRequest } from '@/global'
...
// "Timed out connecting to HBM AGENT backend" that hangs the UI (#48504).
//
...
export class HbmGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'HBM AGENT gateway connection closed',
      connectErrorMessage: 'Could not connect to HBM AGENT gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'HBM AGENT gateway is not connected',
...
// process to route them to that host (hbm:api's registry branch). Null
...
export function hbmApi<T>(request: HbmApiRequest): Promise<T> {
  return window.hbmDesktop.api<T>({ ...connectionScoped(), ...request })
}
