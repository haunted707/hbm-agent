 ...
 *   (`@hbm/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default HbmPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<hbm home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<hbm home>/plugins/<name>/desktop/
...
import { createPluginContext, type HbmPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.hbm/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@hbm/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default HbmPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of hbm-bots predating its adoption in-tree) must
...
// ---------------------------------------------------------------------------
// The on-disk plugin door — ONE app-level root, `<hbm home>/desktop-plugins/`:
//  - `<id>/plugin.js` — a standalone desktop plugin (agent- or user-written);
//  - `<package>/plugin.js` + `.hbm-package.json` — the desktop HALF of a
 ...
 *  agent row. */
const PACKAGE_MARKER = '.hbm-package.json'
...
async function readPackageMarker(desktop: Window['hbmDesktop'], folder: string): Promise<null | PackageMarker> {
  try {
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update HBM AGENT Desktop to load larger plugins"
    )
  }
