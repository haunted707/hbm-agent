 ...
 * `HbmPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/hbm'
...
export type { PluginRestOptions } from '@/hbm'
 ...
 *  under `hbm.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.hbmDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from HBM AGENT — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `hbm://index-network/intent/1`),
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `hbm.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.hbmDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.hbmDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.hbmDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.hbmDesktop
