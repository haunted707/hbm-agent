...
import type { HbmReadDirResult } from '@/global'
import type * as HbmModule from '@/hbm'
...
// getStatus would supply the connected backend's hbm_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ hbm_home: '/remote/box/.hbm' }))

vi.mock('@/hbm', async importActual => ({
  ...(await importActual<typeof HbmModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<HbmReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { hbmDesktop: unknown }).hbmDesktop = {
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { hbmDesktop?: unknown }).hbmDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend hbm_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.hbm/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.hbm/desktop-plugins')
    ...
    expect(readDir).not.toHaveBeenCalledWith('/local/.hbm/plugins')
    // The remote backend's hbm_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.hbm/desktop-plugins')
  })
  ...
  it('treats a folder without plugin.js as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.hbm/desktop-plugins') {
        return {
          entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.hbm/desktop-plugins/my-feature' }]
        }
      }

      if (dir === '/local/.hbm/desktop-plugins/my-feature') {
        return {
          entries: [
            { isDirectory: false, name: 'README.md', path: '/local/.hbm/desktop-plugins/my-feature/README.md' }
          ]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.hbm/desktop-plugins/my-feature')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.hbm/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.hbm/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.hbm/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.hbm/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('loads a unified desktop half (app-root copy + package marker) OPT-IN and tags it with its package', async () => {
    ...
    const root = '/local/.hbm/desktop-plugins'
    ...
    readDir.mockImplementation(async dir => {
      ...
      if (dir === `${root}/uni`) {
        return {
          entries: [
            { isDirectory: false, name: '.hbm-package.json', path: `${root}/uni/.hbm-package.json` },
    ...
    readFileText.mockImplementation(async file =>
      file.endsWith('.hbm-package.json')
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved app root, never the backend hbm_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.hbm/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.hbm/desktop-plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.hbm/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.hbm/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.hbmDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.hbm/desktop-plugins')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.hbm/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.hbm/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'hbm-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "hbm-bots", name: "Bot Mode", register() {} }',
        'hbm-bots',
        { file: '/local/.hbm/desktop-plugins/hbm-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['hbm-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['hbm-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.hbm/desktop-plugins/hbm-bots/plugin.js'
      })
