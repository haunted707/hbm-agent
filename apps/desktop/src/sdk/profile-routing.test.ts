...
vi.mock('@/hbm', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), hbmApi: vi.fn() }))
...
const { deleteProfile, hbmApi } = await import('@/hbm')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its HBM_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { hbmDesktop: unknown }).hbmDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(hbmApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(hbmApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(hbmApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
