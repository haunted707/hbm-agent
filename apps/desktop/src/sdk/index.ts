/**
 * @hbm/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__HBM_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, hbmApi, type HbmGateway } from '@/hbm'
...
import type { PaginatedSessions, UsageStats } from '@/types/hbm'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend HBM AGENT profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.hbmDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.hbmDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and HbmGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (hbm-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.hbmDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update HBM AGENT Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.hbmDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update HBM AGENT Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update HBM AGENT Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (hbm-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.hbmDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('HBM AGENT Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return hbmApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return hbmApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('HBM AGENT gateway unavailable')
    }
 ...
 *  "HBM AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { HbmGateway } from '@/hbm'
...
export type { HbmOpenTarget } from '@/lib/hbm-open-target'
...
/** Compact labels for the reasoning levels exported from @hbm/shared, so a
...
export type { StatusResponse } from '@/types/hbm'
/** Public SDK name for the shared gateway wire event; kept stable for plugins. */
export type { GatewayEvent as RpcEvent } from '@hbm/shared'
/** THE compact-number formatter — every user-facing count/token figure goes
 *  through here (1230 → "1.2k", 1_500_000 → "1.5M"). Don't hand-roll `/1000`. */
export { compactNumber } from '@hbm/shared'
/** HBM AGENT' reasoning levels, so a plugin surfacing a thinking depth uses the
...
export { contrastRatio } from '@hbm/shared/color'
