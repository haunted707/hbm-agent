...
import { ensureContrast, mix, parseColor } from '@hbm/shared/color'
...
const SKIN_KEY = 'hbm-desktop-theme-v2'
const MODE_KEY = 'hbm-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'hbm-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'hbm-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'hbm-desktop-active-profile-v1'
...
// mix / ensureContrast live in @hbm/shared/color (shared with the TUI);
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark', chatFontFamily = $chatFontFamily.get()) {
  ...
  root.dataset.hbmTheme = skinName
  root.dataset.hbmMode = rendered
  ...
  window.hbmDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('hbm-boot-background', chromeBg)
    window.localStorage.setItem('hbm-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.hbmThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.hbmDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (HBM AGENT authoring/activating a skin from a
