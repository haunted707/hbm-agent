/**
 * Live skin sync from the HBM AGENT backend.
 *
 * The backend resolves the active skin (built-in or `$HBM_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (HBM AGENT
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { HbmSkin } from '@hbm/shared/skin'
...
const BACKEND_THEMES_KEY = 'hbm-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: HbmSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as HbmSkin)
