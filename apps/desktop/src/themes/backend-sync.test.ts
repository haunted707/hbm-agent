...
describe('ingestBackendSkin', () => {
  ...
  it('seed does not paint, but a later same-name skin.changed applies (missed-activation recovery)', () => {
    ...
    // restarted). HBM AGENT re-affirms it — `hbm config set display.skin neon`
    // or a `hbm skin set` recolor. That explicit event must repaint even
    ...
    ingestBackendSkin(skin('forest'), { apply: true }) // HBM AGENT authored a new skin
    expect($pendingSkinApply.get()).toBe('forest')
  })
  ...
  it('applies a runtime switch back to default (repaints the desktop to its own default)', () => {
    ...
    ingestBackendSkin(skin('default'), { apply: true }) // HBM AGENT switched back to default
  ...
  it('hydrates the registry from storage on load, so a persisted pick resolves before the gateway connects', async () => {
    ingestBackendSkin(skin('neon'), { apply: false })
    expect(JSON.parse(window.localStorage.getItem('hbm-desktop-backend-themes-v1') ?? '{}').neon?.name).toBe('neon')
  ...
  it('drops junk and built-in names from the cached registry', async () => {
    ...
    window.localStorage.setItem('hbm-desktop-backend-themes-v1', JSON.stringify({ mono, bad: { name: 'bad' } }))
