...
Bundled HBM AGENT Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@hbm/plugin-sdk`, `react`, and
...
`$HBM_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use HBM AGENT theme
...
Stream availability, regional restrictions, and metadata vary. HBM AGENT is not
