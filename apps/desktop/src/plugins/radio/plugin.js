...
const CSS = `
.hbm-radio-bar{display:flex;align-items:center;gap:2px;height:100%;color:var(--ui-text-tertiary)}
.hbm-radio-waveform{position:relative;height:24px;width:100%;font-family:var(--font-mono,monospace);font-size:12px;line-height:12px;color:var(--ui-text-tertiary);opacity:.45;overflow:hidden;flex-shrink:0}
.hbm-radio-waveform-layer{position:absolute;inset:0;display:grid;grid-template-columns:repeat(var(--radio-columns),minmax(0,1fr));grid-auto-rows:12px}
.hbm-radio-waveform-layer>span{text-align:center;min-width:0}
.hbm-radio-waveform-layer:nth-child(1){opacity:.12}
.hbm-radio-waveform-layer:nth-child(2){opacity:.28}
.hbm-radio-waveform[data-compact=true]{width:40px;height:12px;margin-inline:2px}
.hbm-radio-next{display:flex;align-items:center;width:12px;height:12px}
.hbm-radio-waveform[data-active=true]{color:var(--ui-accent);opacity:1}
.hbm-radio-signal{height:48px;padding:5px 6px 0;min-width:0}
.hbm-radio-track{display:flex;align-items:center;gap:5px;height:19px;min-width:0;font-size:11px;line-height:16px;color:var(--ui-text-secondary)}
.hbm-radio-track-text{overflow:hidden;white-space:nowrap;text-overflow:ellipsis;min-width:0}
.hbm-radio-track[data-track=true]{color:var(--ui-text-primary)}
.hbm-radio-artist{min-width:0;max-width:55%;flex-shrink:1;font-size:inherit;font-weight:inherit}
.hbm-radio-artist>span{overflow:hidden;text-overflow:ellipsis}
.hbm-radio-track-separator{flex-shrink:0;color:var(--ui-text-quaternary)}
.hbm-radio-bar .radio-name{max-width:112px;min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis;text-align:left}
.hbm-radio-bar .hbm-radio-action{width:16px;height:16px;padding:0;flex-shrink:0}
.hbm-radio-action-icon{display:flex;align-items:center;justify-content:center;width:12px;height:12px;flex-shrink:0;overflow:hidden}
.hbm-radio-website{display:flex;width:24px;height:24px;flex-shrink:0}
.hbm-radio-panel{width:292px;max-width:calc(100vw - 24px)}
.hbm-radio-search{display:flex;align-items:center;height:32px;padding:0 4px 4px}
.hbm-radio-search-field{flex:1;min-width:0}
.hbm-radio-search-field input{flex:1;width:0;min-width:0}
.hbm-radio-search .hbm-radio-search-field{border:0}
.hbm-radio-search-loader{display:flex;align-items:center;justify-content:center;width:16px;flex-shrink:0;font-size:12px;color:var(--ui-text-tertiary)}
.hbm-radio-list{height:168px;max-height:calc(100dvh - 180px);min-height:80px;overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable}
.hbm-radio-row{display:flex;align-items:center;gap:4px;border-radius:4px}
.hbm-radio-row[data-current=true]{background:var(--chrome-action-hover)}
.hbm-radio-row[data-current=true] .hbm-radio-row-name{color:var(--ui-accent)}
.hbm-radio-row:hover{background:var(--chrome-action-hover)}
.hbm-radio-row [aria-pressed=true]{color:var(--ui-accent)}
.hbm-radio-pin{opacity:0;pointer-events:none}
.hbm-radio-row:hover .hbm-radio-pin,.hbm-radio-row:focus-within .hbm-radio-pin{opacity:1;pointer-events:auto}
@media(hover:none){.hbm-radio-pin{opacity:1;pointer-events:auto}}
.hbm-radio-row-main{display:flex;align-items:center;gap:8px;flex:1;min-width:0;text-align:left;height:24px;padding:2px 6px;cursor:pointer;border-radius:4px}
.hbm-radio-row-main:focus-visible{outline:1px solid var(--ui-accent);outline-offset:-1px}
.hbm-radio-row-icon{display:flex;align-items:center;justify-content:center;width:14px;flex-shrink:0;color:var(--ui-text-tertiary);opacity:0}
.hbm-radio-row:hover .hbm-radio-row-icon,.hbm-radio-row:focus-within .hbm-radio-row-icon,.hbm-radio-row[data-current=true] .hbm-radio-row-icon{opacity:1}
.hbm-radio-row[data-current=true] .hbm-radio-row-icon{color:var(--ui-accent)}
.hbm-radio-row-copy{min-width:0;flex:1}
.hbm-radio-row-name{display:block;font-size:12px;line-height:17px;color:var(--ui-text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.hbm-radio-volume{display:flex;align-items:center;gap:4px;height:28px;padding:2px 2px 0}
.hbm-radio-volume-space{flex:1;min-width:6px}
.hbm-radio-volume input{width:76px;min-width:0;height:3px;accent-color:var(--ui-accent);cursor:pointer}
.hbm-radio-error{font-size:11px;line-height:17px;color:var(--ui-text-secondary);padding:4px 4px 12px}
`
...
function createPlayer(ctx) {
  ...
  const bus = new BroadcastChannel('hbm:radio:playback')
  ...
  async function play(next = station.get(), analyse = true) {
    ...
    audio = element
    element.dataset.hbmRadioAudio = 'true'
    element.hidden = true
...
function Waveform({ player, compact = false }) {
  ...
  return jsx('div', { ref, className: 'hbm-radio-waveform', style: { '--radio-columns': width }, 'data-radio-meter': width, 'data-compact': compact, 'data-meter-mode': mode, 'data-active': active, 'aria-hidden': true,
    children: [0, 1, 2].map(layer => jsx('span', { className: 'hbm-radio-waveform-layer', children: baseline.map((cell, i) => jsx('span', { children: layer === 2 ? cell : '⠀' }, i)) }, layer)) })
}

function NextArrow() {
  return jsxs('span', { className: 'hbm-radio-next', 'aria-hidden': true, children: [
...
function SmallAction({ label, icon, onClick, size = 'micro', pressed, className, busy = false }) {
  return jsx(Tip, { label, children: jsx(Button, {
    variant: 'ghost', size, className: `hbm-radio-action ${className ?? ''}`, 'aria-label': label, 'aria-pressed': pressed, onClick,
    children: jsx('span', { className: 'hbm-radio-action-icon', children: busy ? jsx(GlyphSpinner, { ariaLabel: label }) : jsx(icon, {}) })
  }) })
}
...
async function artistRequest(path, signal, ctx) {
  ...
  return navigator.locks.request('hbm:radio:artist-lookup', { signal }, async () => {
    ...
    const response = await fetch(`https://musicbrainz.org/ws/2/${path}`, {
      ...
      headers: { 'User-Agent': 'HbmRadio/1.0 (https://github.com/NousResearch/hermes-agent)' }
    })
...
function Signal({ player, ctx }) {
  ...
  return jsxs('div', { className: 'hbm-radio-signal', children: [
    jsx(Waveform, { player }),
    jsxs('div', { className: 'hbm-radio-track', 'data-track': Boolean(track), role: 'status', children: [
      ...
      credit.artist && (artistUrl ? jsx(Tip, { label: `${t('artistProfile')}: ${credit.artist}`, children: jsx(Button, {
        variant: 'text', size: 'inline', asChild: true, className: 'hbm-radio-artist',
      ...
      }) }) : jsx('span', { className: 'hbm-radio-track-text', children: credit.artist })),
      credit.artist && jsx('span', { className: 'hbm-radio-track-separator', 'aria-hidden': true, children: '—' }),
      jsx(Tip, { label: text || undefined, children: jsx('span', { className: 'hbm-radio-track-text', children: credit.artist ? credit.title : text }) })
    ] })
  ] })
}
...
function StationRow({ station, player }) {
  ...
  return jsxs('div', { className: 'hbm-radio-row', 'data-current': current, 'data-pinned': saved, children: [
    jsxs(RowButton, { className: 'hbm-radio-row-main', 'aria-label': `${t(failed ? 'retry' : current && active ? 'pause' : 'play')}: ${station.name}`, title: failed ? t('streamError') : undefined, 'aria-current': current ? 'true' : undefined,
      onClick: () => current ? player.toggle() : void player.play(station), children: [
        jsx('span', { className: 'hbm-radio-row-icon', role: failed ? 'status' : undefined, 'aria-label': failed ? t('error') : undefined, children: jsx(failed ? icons.AlertCircle : current && active ? icons.Pause : icons.Play, { size: 12 }) }),
        jsx('span', { className: 'hbm-radio-row-copy', children: jsx('span', { className: 'hbm-radio-row-name', children: station.name }) })
      ] }),
    jsx(SmallAction, { label: `${t(saved ? 'unsave' : 'save')}: ${station.name}`, icon: icons.Pin, onClick: () => player.favorite(station), size: 'icon-xs', pressed: saved, className: 'hbm-radio-pin' })
  ] })
}
...
function Stations({ player }) {
  ...
  return jsxs('div', { children: [
    jsx('div', { className: 'hbm-radio-search', 'data-filled': Boolean(search), children: jsx(SearchField, {
      value: search, onChange: value => { setBrowseStation(player.station.get()); setSearch(value) }, placeholder: t('search'),
      containerClassName: 'hbm-radio-search-field',
      trailingAction: pending ? jsx('span', { className: 'hbm-radio-search-loader', children: jsx(GlyphSpinner, { ariaLabel: t('searching') }) }) : null
    }) }),
    jsxs('div', { ref: listRef, className: 'hbm-radio-list', 'data-radio-pending': pending, children: [
      ...items.map(station => jsx(StationRow, { station, player }, station.id)),
      searchFailed && jsx('div', { className: 'hbm-radio-error', role: 'status', children: t('searchError') }),
      !pending && !searchFailed && text && results.length === 0 && jsx('div', { className: 'hbm-radio-error', role: 'status', children: t('noResults') })
    ] })
  ] })
}
...
function Transport({ player, ctx }) {
  ...
  const status = useValue(player.status)
  const active = status === 'live' || status === 'connecting'
  return jsxs('div', { className: 'hbm-radio-volume', children: [
    ...
    jsx('span', { className: 'hbm-radio-volume-space' }),
    ...
    jsx('span', { className: 'hbm-radio-website', children: httpsUrl(station.homepage) && jsx(SmallAction, { label: t('visit'), icon: icons.ExternalLink, size: 'icon-xs', onClick: () => void ctx.os.openExternal(station.homepage) }) })
  ] })
}
...
function RadioBar({ player, ctx }) {
  ...
  return jsxs('div', { className: 'hbm-radio-bar', 'data-tour': 'radio-player', 'data-radio-status': status, children: [
    ...
    jsxs(Popover, { open, onOpenChange, children: [
      ...
      jsx(PopoverContent, { side: 'top', align: 'end', className: 'hbm-radio-panel', 'aria-label': t('radio'), 'data-tour': 'radio-panel', children:
        jsxs('div', { children: [
