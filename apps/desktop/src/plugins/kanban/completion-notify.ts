 ...
 *    desktop shell fires only while the user is AWAY from HBM AGENT. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@hbm/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from HBM AGENT (the toast above covers the foreground case). Isolated:
