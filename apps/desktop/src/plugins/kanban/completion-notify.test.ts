 ...
 * renderer process. The @hbm/plugin-sdk host is mocked: notify/navigate
 * calls are asserted, never really executed.
 */

import type { PluginRestOptions } from '@hbm/plugin-sdk'
...
describe('terminal kinds beyond completed', () => {
  ...
  it('gave_up: plain-words body, raw payload error only in detail; crashed and timed_out fall back to the task id', async () => {
    ...
    expect(gaveUp.message).toBe('HBM AGENT couldn’t finish this task. Open Kanban to see why and reassign it.')
  ...
  it('gave_up without a payload error still gets the plain-words body and no empty detail noise', async () => {
    ...
    expect(lastNotify()).toMatchObject({
      kind: 'error',
      message: 'HBM AGENT couldn’t finish this task. Open Kanban to see why and reassign it.',
      detail: 't101'
    })
  })

  it('retrying kinds (crashed/timed_out) say HBM AGENT will retry and never expose worker/gateway vocabulary', async () => {
    const m = await loadModule()
    m.bindCompletionNotify(makeRest(() => 100) as never)
    ...
    for (const call of hostMock.notify.mock.calls) {
      ...
      expect(toast.title).toMatch(/HBM AGENT will retry it automatically/)
...
describe('i18n routing', () => {
  ...
  it('falls back to the English bundle when the translator echoes the key', async () => {
    ...
    expect(lastNotify().title).toBe('Task took too long — HBM AGENT will retry it automatically')
  })
})
