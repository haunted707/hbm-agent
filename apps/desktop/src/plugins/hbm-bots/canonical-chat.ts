...
import * as sdk from '@hbm/plugin-sdk'
import { host } from '@hbm/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['hbm-bots'].chat) were
 ...
 *  click path's tile-staleness probe (hbm-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This HBM AGENT Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (hbm-agent#89617) — clicking Retry
  ...
  // until an app restart (hbm-agent#93604). A resume is cheap and
...
export function notifyBotOpenFailure(error: unknown, bot: RosterRow, step: BotOpenStep, botName?: string) {
  ...
  if (botModeGatewayNeedsUpdate(error)) {
    const connectionLabel = bot.connectionLabel || bot.connectionId || 'HBM AGENT'
    host.notify?.({
      kind: 'error',
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update HBM AGENT Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['hbm-bots']
