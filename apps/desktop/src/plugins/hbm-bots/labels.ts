...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "HBM AGENT". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `hbm profile rename
  ...
  // never generic "HBM AGENT" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as HBM AGENT (the agent it is)
