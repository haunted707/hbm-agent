/**
 * Routines: the HBM AGENT cron jobs scoped to the bot you're chatting with — the
...
// hbm -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `hbm -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// HBM AGENT-native schedule string; Advanced exposes it raw.
...
/** Compose the HBM AGENT schedule string from picker state. */
