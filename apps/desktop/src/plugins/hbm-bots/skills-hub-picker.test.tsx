 ...
 * `{type:'hbm-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as HbmSdk from '@hbm/plugin-sdk'
...
vi.mock('@hbm/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof HbmSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'nous/web-research', name: 'Web Research', type: 'hbm-skill-pick' },
      {
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'hbm-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'nous/web-research', type: 'hbm-skill-pick' }, { source: frame.contentWindow })
