...
describe('routing', () => {
  ...
  it('resolves @hbm to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@hbm take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @hbm', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @hbm,/)
    ...
    expect(peer).toMatch(/group chat with @hbm/)
  })
