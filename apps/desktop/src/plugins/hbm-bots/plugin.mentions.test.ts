...
async function contributions({
  ...
  cache.set(JSON.stringify(['hbm-bots', 'roster', cacheKeyConnection]), {
    key: ['hbm-bots', 'roster', cacheKeyConnection],
    value: { profiles }
  })
...
describe('@-mention completions', () => {
  ...
  it('surfaces default as @hbm and prefix-filters on the handle', async () => {
    const { provide } = await contributions({
      focused: 'researcher',
      profiles: [
        { name: 'default' },
    ...
    expect(provide('').map(item => item.insert)).toEqual(expect.arrayContaining(['@hbm', '@writer-homelab']))
...
describe('the mention middleware', () => {
  it('identifies a remote @name-device mention without delivering anything', async () => {
    ...
    expect(result.text).not.toMatch(/hbm -p '?default/)
  ...
  it('annotates the resolvable handle for a local row whose UI alias differs', async () => {
    ...
    // The local resolver only knows bare profile names / 'hbm'.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
    ...
    expect(result.text).toMatch(/message_agent target: "hbm"/)
  ...
  it('teaches no shellout and forbids forwarding the user’s text verbatim', async () => {
    ...
    // a `hbm -p …` handoff, giving the model a second send path and a way
    // to relay the raw draft.
    const { handler } = await contributions({ focused: 'research', profiles: [{ name: 'research' }, { name: 'ops' }] })
    const result = await handler({ text: 'ask @ops to summarize' })

    expect(result.text).not.toMatch(/hbm -p/)
  ...
  it('keeps a poisoned bot title inert prose', async () => {
    ...
    // the invariant that matters is that no hbm command is ever emitted.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
        { name: 'ops' },
    ...
    expect(result.text).not.toMatch(/`hbm/)
  })
