...
import { atom, host } from '@hbm/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'hbm-bots-groups'
...
/** The v3 envelope stored under the default profile's `hbm-bots-groups`
 ...
 *  clients (HBM AGENT Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "HBM AGENT"
 *  (community report, Aug 21 2026: renamed default still read "HBM AGENT is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as HBM AGENT (matching displayName and the
 *  @hbm handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`hbm profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'HBM AGENT' : trimmed
}
