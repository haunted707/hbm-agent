...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['hbm-bots-groups']
}
...
describe('speaker labels', () => {
  it('relabels HBM AGENT control-frame openers only in member-authored transcript lines', async () => {
  ...
  it('the default profile speaks as HBM AGENT in transcripts, not @default', async () => {
    ...
    expect(line).toBe('HBM AGENT: hello room')
    ...
    ).toBe('HBM AGENT (you): hi')
    expect(
      formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' } as GroupMessage, 'research')
    ).toBe('builder: yo')
  })

  it('honor friendly identity: Bot Mode title, then display_name, never a stale HBM AGENT', async () => {
    const { chat, rounds } = await loadRoom()
    const { formatGroupChatLine } = await import('./group-round-prompt')
    const data = await import('./data')

    // A renamed default (core display_name via `hbm profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "HBM AGENT is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → HBM AGENT, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('HBM AGENT')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['hbm-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['hbm-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['hbm-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'hbm-bots-groups': 1 } } }
      }
