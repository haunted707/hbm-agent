...
import { host, LruCache } from '@hbm/plugin-sdk'
...
// to (local, remote URL, SSH, HBM AGENT Cloud, docker) must be able to find
...
// hbm_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['hbm-bots']?.title || profile?.display_name || ''),
