...
import { forgetSessionUnread, host, queryClient } from '@hbm/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['hbm-bots']
...
/** Permanently delete a bot's HBM AGENT profile, then remove plugin-local state
 ...
 * directory (hbm-agent#52279). That is the "can't delete a bot" error. */
