...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `HbmPlugin` and
...
`$HBM_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`hbm-desktop-plugins` skill.
