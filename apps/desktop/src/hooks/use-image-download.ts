...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'hbm:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.hbmDesktop?.saveImageFromUrl) {
        if (await window.hbmDesktop.saveImageFromUrl(src)) {
