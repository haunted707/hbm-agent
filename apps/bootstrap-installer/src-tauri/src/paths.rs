...
//!
//! Mirrors `hbm_constants.get_hbm_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\hbm
//!   macOS:   ~/.hbm
//!   Linux:   ~/.hbm  (override via $HBM_HOME)
//!
//! NOTE (macOS): Python's get_hbm_home(), scripts/install.sh, and the
//! Electron desktop's resolveHbmHome() ALL use ~/.hbm on macOS — there
...
/// Returns the canonical HBM AGENT home directory, respecting $HBM_HOME if set.
pub fn hbm_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("HBM_HOME") {
    ...
    {
        // %LOCALAPPDATA%\hbm — matches scripts/install.ps1's $HbmHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("hbm");
        }
    }

    // macOS + Linux + fallback: ~/.hbm (matches Python get_hbm_home(),
    // install.sh, and the Electron desktop's resolveHbmHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".hbm");
    }
...
/// HBM_HOME so it survives repo checkout deletion (unlike anything under
/// hbm-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\hbm\hbm-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_hbm`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under HBM_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves HBM_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    hbm_home().join(".hbm-update-in-progress")
}
...
pub fn copy_self_to_hbm_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to HBM_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_HBM_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".hbm-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under HBM_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[hbm-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("HBM_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_hbm_home() -> String {
    hbm_home().to_string_lossy().into_owned()
}
