...
/// read side does not see EOF until the last of them closes it. `hbm update`
...
/// stranded with it.
///
/// Same bound `Invoke-HbmStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `hbm_home_override` propagates to the child as $HBM_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, hbm_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = hbm_home_override {
        cmd.env("HBM_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, hbm_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = hbm_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_hbm_home() {
    ...
    /// child. `hbm update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
