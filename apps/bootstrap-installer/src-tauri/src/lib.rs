//! HBM AGENT Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `HBM AGENT-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under HBM_HOME/logs/ so install
    ...
    // HBM AGENT is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "HBM AGENT installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // HBM AGENT is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "HBM AGENT" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "HBM AGENT" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_hbm_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hbm_home().join("hbm-agent");
                if bootstrap::hbm_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_hbm_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "hbm already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running HBM AGENT Setup");
}
