...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating HBM AGENT' : 'Setting up HBM AGENT'

  const description = isUpdate
    ? 'HBM AGENT is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The HBM AGENT installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="hbm-fade-in flex h-full flex-col">
