...
export const $hbmHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.hbm/logs/bootstrap-installer.log')
    $hbmHome.set('~/.hbm')
  ...
  try {
    const [logPath, hbmHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_hbm_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch HBM AGENT" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (HBM AGENT-Setup.exe --update);
...
export async function launchHbmDesktop(): Promise<void> {
  ...
  await invoke('launch_hbm_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'HBM AGENT repository' },
