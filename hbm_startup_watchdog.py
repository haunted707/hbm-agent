...
timeout. Config is env-only (``HBM_STARTUP_WATCHDOG=0``,
``HBM_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "HBM_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "HBM_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (hbm_cli.main / hbm_cli.gateway /
...
def _process_hbm_home() -> Path:
    """HBM_HOME for diagnostic files — stdlib-only replica of the hbm_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<HBM_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_hbm_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``HBM_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``HBM_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
