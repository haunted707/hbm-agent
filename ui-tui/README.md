...
React + Ink terminal UI for HBM AGENT. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `HBM_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `hbm setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.hbm/.hbm_history` or under `HBM_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `hbm setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/hbm-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external hbm setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.hbm/tui-parent.log
    ...
    types/
      hbm-ink.d.ts            type declarations for @hbm/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent HbmCLI subprocess for slash commands
```
