...
describe('backend lifecycle copy', () => {
  it('names the exit code, the last real stderr line, /logs and hbm doctor', () => {
    const tail =
    ...
    expect(text).toContain('/logs')
    expect(text).toContain('hbm doctor')
    expect(text).toContain('/resume')
    expect(text).not.toMatch(/\bgateway\b/)
  })
  ...
  it('only failure-looking stderr earns an activity row', () => {
    ...
    expect(stderrLooksLikeProblem('INFO hbm.mcp: discovered 3 servers')).toBe(false)
  })
