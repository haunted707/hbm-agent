import { JsonRpcGatewayError } from '@hbm/shared/json-rpc-channel'
...
// DASHBOARD_TUI_MODE resolves once at module load from HBM_TUI_DASHBOARD,
...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ output: 'HBM AGENT TUI Status' }))
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('HBM AGENT TUI Status', 'Status')
    })
  })
  ...
  it('falls through to command.dispatch for skill commands, sending the body but showing the invocation', async () => {
    const skillMessage =
      '[IMPORTANT: The user has invoked the "hbm-agent-dev" skill, indicating they want you to follow its instructions.\n' +
    ...
    const h = createSlashHandler(ctx)
    expect(h('/hbm-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.send).toHaveBeenCalledWith(skillMessage, true, '/hbm-agent-dev')
    })
  ...
  it('/history pages the current TUI transcript (user + assistant)', () => {
    ...
    expect(body).toContain('[HBM AGENT #2]')
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/hbm_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/hbm_conversation_test.json')
    })
  })
