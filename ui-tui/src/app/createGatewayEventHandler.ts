...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@hbm/ink'
import { stripAnsi } from '@hbm/shared/ansi'
import { relativeLuminance } from '@hbm/shared/color'
import type { StreamDeltaPayload, SubagentStatus, Usage } from '@hbm/shared/gateway-events'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.HBM_TUI_THEME : undefined

  writeBootTheme(theme, process.env.HBM_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to HBM_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit HBM_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the HBM_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.HBM_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.HBM_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a HBM_TUI_THEME the user
    ...
    delete process.env.HBM_TUI_THEME
  }
 ...
 * is cached into HBM_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * HBM_TUI_LIGHT / HBM_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.HBM_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.HBM_TUI_THEME || process.env.HBM_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.HBM_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.HBM_TUI_BACKGROUND ||
      process.env.HBM_TUI_THEME ||
      process.env.HBM_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.HBM_TUI_BACKGROUND || process.env.HBM_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.HBM_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: AnyGatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    ...
    setTimeout(async () => {
      ...
      // launches, `hbm --tui -q "…"`) — submit LITERALLY, bypassing the
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey HBM AGENT" if this surface owns it (server gates on config).
    ...
    // forging a brand-new one.  Mirrors classic CLI's `hbm -c` /
    // `hbm --tui` muscle memory and addresses the audit's "session
  ...
  return (ev: AnyGatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey HBM AGENT": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: hbm -p ${wakeProfile} --tui`)
