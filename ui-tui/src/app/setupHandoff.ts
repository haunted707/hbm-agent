import type { RunExternalProcess } from '@hbm/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`hbm ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching hbm: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`hbm ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
