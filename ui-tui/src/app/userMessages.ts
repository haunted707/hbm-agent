...
// /sessions, /quit) and `hbm doctor` is a real subcommand.

import type { ErrorSurface } from '@hbm/shared/gateway-events'
...
export const BACKEND_RESTARTING =
  'HBM AGENT stopped unexpectedly — restarting and reopening your chat (the reply in progress was lost).'

export const BACKEND_RESTARTING_ACTIVITY = 'HBM AGENT stopped unexpectedly · restarting…'

// Attached (dashboard / embedded) mode: only the socket dropped; HBM AGENT and any
// reply in progress are still alive on the backend and come back on reconnect.
export const CONNECTION_LOST = 'Connection to HBM AGENT lost — reconnecting and reopening your chat…'
...
export const backendGaveUp = (code: null | number, lastLine?: string): string => {
  ...
  return [
    `HBM AGENT stopped${exit} and could not be restarted. Your chat is saved.`,
    detail,
    'HBM AGENT keeps trying to reconnect in the background and reopens this chat when it succeeds; if it does not, type /resume.',
    'Type /logs for the full log, or /quit and run `hbm doctor` to check the install.'
  ]
    .filter(Boolean)
    .join('\n')
}

export const BACKEND_GAVE_UP_ACTIVITY = 'HBM AGENT stopped · /logs for details'
...
export const BACKEND_SLOW_START =
  'HBM AGENT is taking longer than usual to start. Still waiting… If it never connects: /logs shows the last backend output; /quit and run `hbm doctor`.'
...
export const stderrProblemActivity = (line: string): string => {
  ...
  return `Something went wrong inside HBM AGENT${what} · /logs for details`
}
...
export const VERSION_SKEW_MESSAGE =
  'The terminal UI and the HBM AGENT backend are out of sync (different versions). Run /update, or exit and run `hbm update`, then start the TUI again.'
...
const RPC_ERROR_ROWS: RpcErrorRow[] = [
  ...
  [
    ...
    () =>
      'HBM AGENT is not connected right now, so that was not sent. It reconnects automatically; wait a moment and try again, or type /logs if this persists.'
  ],
  [
    (_code, text) => TIMED_OUT_RE.exec(text),
    m => `HBM AGENT did not answer within ${m?.[1] ?? '?'}s. Try again; if it keeps happening, type /logs and report the last lines.`
  ]
]
...
const TURN_LAYER_COPY: Record<string, [string, string]> = {
  ...
  billing: ['The model provider reports no credit left', 'Top up the account or switch with /model.'],
  disk: ['The disk is full, so HBM AGENT could not save the turn', 'Free some space, then /retry.'],
  endpoint: ['Your custom model endpoint did not answer', 'Check the endpoint is running, then /retry.'],
  gateway: ['HBM AGENT hit an internal error while running this turn', 'Send /retry; type /logs for the trace.'],
