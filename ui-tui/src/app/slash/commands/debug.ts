...
import { terminalBackgroundHex } from '@hbm/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $HBM_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see HBM_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['HBM_TUI_BACKGROUND', process.env.HBM_TUI_BACKGROUND ?? '(unset)'],
            ['HBM_TUI_THEME', process.env.HBM_TUI_THEME ?? '(unset)'],
