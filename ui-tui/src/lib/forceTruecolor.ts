...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.HBM_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep HBM AGENT on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via HBM_TUI_TRUECOLOR=1.
