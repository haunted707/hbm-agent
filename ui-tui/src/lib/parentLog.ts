...
const logDir = join(process.env.HBM_HOME?.trim() || join(homedir(), '.hbm'), 'logs')
...
// a real ~/.hbm (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('hbm-tui: parent lifecycle log unavailable\n')
    }
  }
}
