 ...
 * `hbm-boot-background` / `hbm-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports HBM_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.hbm matches get_hbm_home()'s default.
const bootFilePath = () => join(process.env.HBM_HOME ?? join(homedir(), '.hbm'), 'tui-theme-boot.json')

// Never touch the user's real ~/.hbm from test runs (the TS suite has no
// HBM_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into HBM_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.HBM_TUI_THEME || env.HBM_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.HBM_TUI_BACKGROUND
  ) {
    env.HBM_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.HBM_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.HBM_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into HBM_TUI_THEME.
