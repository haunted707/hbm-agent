...
/**
 * User widget apps — HBM AGENT authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$HBM_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.hbm/plugins/`: files under HBM_HOME execute
...
const widgetsDir = () => join(process.env.HBM_HOME?.trim() || join(homedir(), '.hbm'), 'tui-widgets')
 ...
 *  every change, so a widget HBM AGENT writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or HBM AGENT) ever writes must hot-load too — a 10s poll here read
