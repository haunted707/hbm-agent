import type { InputEvent, Key } from '@hbm/ink'
import * as Ink from '@hbm/ink'
 ...
 * but that accepts `key.meta` on macOS — and hbm-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.HBM_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which hbm-ink decodes as meta+'d'; without this branch it
