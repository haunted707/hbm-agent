import { Box, Text, useInput, useStdout } from '@hbm/ink'
import { fuzzyRank } from '@hbm/shared/fuzzy'
import type { ModelOptionProvider, ModelOptionsResult } from '@hbm/shared/gateway-events'
import { modelSearchText } from '@hbm/shared/model-search-text'
import { REASONING_EFFORTS } from '@hbm/shared/reasoning-effort'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `hbm model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run hbm model)
          return
        }
    ...
    // filter. With Ctrl held, @hbm/ink reports `ch` as the key name ('g'),
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Paste your API key below (saved to ~/.hbm/.env)
        </Text>
