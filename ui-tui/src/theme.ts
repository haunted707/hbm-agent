import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@hbm/shared/color'
import type { SkinBranding, SkinColors } from '@hbm/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @hbm/shared/color (the primitives,
...
// Mirrors @hbm/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @hbm/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic HBM AGENT navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode HBM AGENT look was never hand-authored: for years the
 ...
 *  answer when known (cached in HBM_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.HBM_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit HBM_TUI_THEME / COLORFGBG signals above still win,
...
// `HBM_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `HBM_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `HBM_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `HBM_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default HBM AGENT palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.HBM_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.HBM_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.HBM_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into HBM_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into HBM_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (HBM AGENT navy
  ...
  // the derived value already carries the identity seeds (e.g. HBM AGENT navy).
  const activeRow =
