...
describe.each([
  ...
  it('under the dashboard PTY, re-asserts modes but never clears or repaints', async () => {
    ...
    // every OS app-switch (hbm-agent#94337). The focus report itself must
    // still be processed (this handler runs), only the repaint is skipped.
    const { beforeFocus, afterFocus, chunks } = await focusRegain(altScreen, { HBM_PTY_HOST: 'dashboard' })
