...
from hbm_state_errors import classify_persistence_error, is_disk_full_error
...
_DOCTOR = "Run `hbm doctor --fix` to diagnose and repair."

# cause -> (code, gloss, action). "disk" is split by is_disk_full_error at lookup time.
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        "storage_locked",
        "the session database is locked by another HBM AGENT process",
        "Wait a moment and try again; if it persists, stop the other HBM AGENT process (`hbm gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "corrupt": (
        ...
        f"{_DOCTOR} Recovery: `hbm sessions recover --source <state.db> --inspect-only`.",
    ),
    "fts_index": (
        "storage_index_corrupt",
        "the session search index is damaged (the messages themselves are intact)",
        "Run `hbm doctor --fix` (or `hbm sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
        "the session database file was replaced while HBM AGENT was running",
        "Stop HBM AGENT (`hbm gateway stop`), run `hbm doctor`, then start it again.",
    ),
    "deleted_wal": (
        "storage_replaced",
        "the session database file was changed or replaced while HBM AGENT was running",
        "Stop HBM AGENT (`hbm gateway stop`), run `hbm doctor`, then start it again.",
    ),
    "compression": (
        "storage_busy",
    ...
    "turn_lease": (
        "storage_busy",
        "another HBM AGENT process took over this session",
