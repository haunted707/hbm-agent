...
# The cache is also persisted under the HBM AGENT home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``hbm_constants.get_hbm_home()`` so the cache follows the
    active profile and is isolated across HBM AGENT homes. The cache directory
    is created on demand. Returns ``None`` when HBM AGENT home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "hbm-agent-osv-check/1.0"},
        method="POST")
