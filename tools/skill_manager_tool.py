...
broad, declarative). New skills land in ~/.hbm/skills/ (or ``skills.create_dir``);
...
# All skills live in ~/.hbm/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    An explicitly patched module-level ``SKILLS_DIR`` (tests) wins over the live HBM_HOME.

    Long-lived multi-profile runtimes (Dashboard/TUI/Desktop backend, cron, kanban workers) import this
    module once under the launch HBM_HOME and later bind a different profile per session (#40677).
    """
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "— fix them with skill_manage(action='patch') to match HBM AGENT skill standards.")
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    from hbm_constants import mkdir_under_hbm_home
    mkdir_under_hbm_home(skill_dir)
