...
def _home_candidates() -> list:
    ...
    try:
        ...
        candidates.append(Path(get_hbm_home()) / "bin" / "lightpanda")
    except Exception as e:  # pragma: no cover - defensive
        logger.debug("hbm home unavailable for lightpanda lookup: %s", e)
    return candidates
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    fallbacks), then installer/agent-browser locations, then ``$HBM_HOME/bin``. No Windows build."""
    if os.name == "nt":
...
def _state_dir() -> Path:
    from hbm_constants import get_hbm_home
    path = Path(get_hbm_home()) / "cache" / "browser-use" / "lightpanda"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this HBM AGENT spawns.
    ...
    bounds it without HBM AGENT managing eviction.
    """
...
def _cdp_ready(url: str, timeout: float = 0.2) -> bool:
    try:
        from hbm_cli.browser_connect import is_browser_debug_ready
    ...
    if not binary:
        if os.name == "nt":
            return None, ("browser.engine is 'lightpanda' but Lightpanda has no Windows "
                          "build. Set browser.engine to auto (or run HBM AGENT under WSL2).")
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning HBM AGENT is gone; return the count. A live owner is
