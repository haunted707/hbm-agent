...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("HBM_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from HBM AGENT YOLO. Lazy import so approval-only
...
def _permanent_set() -> set:
    ...
    profile (HBM_HOME override) → its own set, lazily loaded from ITS ``command_allowlist``: the
...
def _presence(approval_callback=None) -> tuple:
    ...
    is_ask = env_var_enabled("HBM_EXEC_ASK")
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set HBM_INTERACTIVE or "
                               "HBM_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set HBM_INTERACTIVE or "
                       "HBM_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_hbm_interactive_context': ('tools.approval_context', 'reset_hbm_interactive_context'),
    ...
    'set_hbm_interactive_context': ('tools.approval_context', 'set_hbm_interactive_context'),
}
