"""Skills Hub discovery: the centralized HBM AGENT index fetch (cached, stale-
...
from tools.skills_hub_official import HbmIndexSource, OptionalSkillSource
...
HBM_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
HBM_INDEX_TTL = 6 * 3600  # 6 hours


def _hbm_index_cache_file() -> Path:
    from tools.skills_hub import _index_cache_dir
    return _index_cache_dir() / "hbm-index.json"


def _load_hbm_index() -> Optional[dict]:
    """Fetch the centralized skills index (docs site, rebuilt daily), cached
    locally for HBM_INDEX_TTL; on any failure serve the stale cache.
    ...
    cache_file = _hbm_index_cache_file()
    cached = _read_json_if_fresh(cache_file, HBM_INDEX_TTL)
    if cached is not None:
        return cached
    data = None
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(HBM_INDEX_URL, timeout=15, follow_redirects=True,
                             headers={"Accept-Encoding": accept_encoding})
            if resp.status_code != 200:
                logger.debug("HBM AGENT index fetch returned %d", resp.status_code)
                return _load_stale_index_cache()
            data = resp.json()
            break
        except httpx.DecodingError as e:
            logger.debug("HBM AGENT index decode failed (Accept-Encoding=%s): %s", accept_encoding, e)
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("HBM AGENT index fetch failed: %s", e)
            return _load_stale_index_cache()
...
def _load_stale_index_cache() -> Optional[dict]:
    ...
    from tools.skills_hub import _read_json_if_fresh
    return _read_json_if_fresh(_hbm_index_cache_file(), float("inf"))
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    return [
        ...
        HbmIndexSource(auth=auth),     # centralized index (search + resolved install paths)
...
def _search_one_source(
    ...
    try:
        ...
        if provider_filter and isinstance(src, (HbmIndexSource, GitHubSource)):
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "hbm-index" and getattr(src, "is_available", False) for src in sources
    )
