...
# HBM AGENT session id (observability identity, distinct from the gateway routing session_key), forwarded to approval
...
# os.environ["HBM_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_hbm_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("hbm_interactive", None)


def set_hbm_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context instead of mutating os.environ."""
    return _hbm_interactive_ctx.set("1" if interactive else "")


def reset_hbm_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_hbm_interactive_context`."""
    _hbm_interactive_ctx.reset(token)


def _is_interactive_cli() -> bool:
    """True for an interactive CLI/ACP session (contextvar first, env fallback)."""
    ctx_val = _hbm_interactive_ctx.get()
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("HBM_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HBM_SESSION_KEY", default)
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    Webhook, msgraph_webhook, and api_server sessions bind ``HBM_SESSION_PLATFORM`` like chat gateways
...
def _is_single_query_approval_context() -> bool:
    ...
    return is_truthy_value(_session_env("HBM_SINGLE_QUERY_SESSION"))


def _is_gateway_approval_context() -> bool:
    """True inside a gateway/API session that can answer an approval.

    Legacy integrations set HBM_GATEWAY_SESSION; concurrent paths bind
    HBM_SESSION_PLATFORM via contextvars. Cron is NEVER a gateway approval
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("HBM_GATEWAY_SESSION") or bool(_get_session_platform())
...
def _should_fall_through_to_cli_approval(*, is_cli: bool, approval_callback, notify_cb) -> bool:
    ...
    ``HBM_EXEC_ASK`` (or a platform marker) can leak into an interactive CLI
