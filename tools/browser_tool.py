...
from hbm_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every HBM AGENT secret from process.env.
...
# than a concrete path (also compared in hbm_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``hbm_home_key()``): the multiplexed
...
_REAL_PROFILE_SESSION = "hbm-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-hbm_…``
...
# Owner HBM AGENT home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. HBM AGENT' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = hbm_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_hbm_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_hbm_home_override': ('hbm_constants', 'get_hbm_home_override'),
    'hbm_home_key': ('hbm_constants', 'hbm_home_key'),
    ...
    'node_tool_runnable': ('hbm_constants', 'node_tool_runnable'),
