...
# Written by `hbm profile create --no-skills` / installer `--no-skills`: sync seeds only
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `hbm skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `hbm skills reset {skill_name}` to "
