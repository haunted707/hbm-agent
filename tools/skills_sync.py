...
~/.hbm/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
from hbm_constants import get_bundled_skills_dir, get_hbm_home, get_optional_skills_dir
...
# retarget HBM_HOME after import, and frozen constants would resolve (and for
...
# once under the launch HBM_HOME and later scope requests to a different profile via
# set_hbm_home_override(). See #65828.
_HBM_HOME_AT_IMPORT = HBM_HOME
...
def _hbm_home() -> Path:
    return _live(HBM_HOME, _HBM_HOME_AT_IMPORT, get_hbm_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _hbm_home() / "skills")
...
# Written by `hbm-agent profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors hbm_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # HBM_BUNDLED_SKILLS env first, then repo-relative
...
def _write_manifest(entries: Dict[str, str]):
    ...
    mkdir_under_hbm_home(_manifest_file().parent)
    try:
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `hbm-agent skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `hbm-agent skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.hbm/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``hbm-agent``)."""
    essential_only = (_hbm_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing HBM_HOME / malicious manifest entry).
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.hbm/skills/ ...")
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_hbm_home() / NO_BUNDLED_SKILLS_MARKER).exists()
