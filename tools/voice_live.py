"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to HBM AGENT.
...
delegates every real request to HBM AGENT as its *client-delegation* backend. HBM AGENT stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — HBM AGENT' answer is what the voice speaks.
...
# backend (HBM AGENT) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are HBM AGENT, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- HBM AGENT: a full AI agent with tools — it can run commands, read and edit files, "
