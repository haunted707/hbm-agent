...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_hbm_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
_BROWSER_IDENTITY_KEYS = (
    "HBM_SESSION_ID",
    "HBM_BROWSER_CONTROL_PRINCIPAL",
    "HBM_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    processes keep the process-wide cache; a multiplex gateway installs a HBM AGENT-home override
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved HBM_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("hbm_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``hbm_plugins.{name}``), not
                # the exact module: a submodule's handler is still the package's to remove.
                # A handler defined in ``hbm_plugins.pkg.handlers`` is still owned by the
                # ``hbm_plugins.pkg`` package — exact string equality would wrongly block root-module
