...
# match hbm_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _load_config() -> dict:
    ...
    (follows HBM_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``HBM_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("HBM_IGNORE_USER_CONFIG") != "1":
        try:
...
def _resolve_child_runtime(
    ...
    # would pin a child HBM AGENT/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
