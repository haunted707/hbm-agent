...
def nous_tool_gateway_unavailable_message(capability: str = "the Nous Tool Gateway", *,
    ...
    try:
        ...
        pass
    return (f"{capability} is unavailable. Run `hbm model` to refresh your "
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    Resolution order (fixes #68003 — keys added via ``hbm auth add <provider>`` were invisible to the
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `hbm tools` selection: ``"nous"`` (managed
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via hbm "
            f"tools), but {failure}. Run 'hbm tools' to change it.")
