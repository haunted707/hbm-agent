"""Lazy dependency installer for opt-in HBM AGENT backends.
...
(never system Python); durable-target mode (``HBM_LAZY_INSTALL_TARGET``, sealed images)
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey HBM AGENT") engines (sync with the `wake` extra) ──────
...
_LAZY_TARGET_ENV = "HBM_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    modes; (2) the sealed venv (``HBM_DISABLE_LAZY_INSTALLS=1``) blocks only without a durable
    ...
    if cfg is not None and not bool((cfg.get("security") or {}).get("allow_lazy_installs", True)):
        return False
    if os.environ.get("HBM_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    if sys.platform == "win32" and feature == "platform.matrix":
        return ("unsupported on Windows: Matrix E2EE depends on python-olm, which has no Windows wheel and "
                "requires make + libolm to build from sdist. Run HBM AGENT under WSL to use Matrix on Windows.")
    return None
...
def _core_constraints_file() -> Optional[Path]:
    ...
    try:
        ...
        if not pins:
            return None
        fd, path = tempfile.mkstemp(prefix="hbm-core-constraints-", suffix=".txt")
...
def _uv_binary() -> Optional[str]:
    """Managed uv first ($HBM_HOME/bin is never on PATH), then PATH. A lookup, not ensure_uv():
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    ``hbm_cli.tools_config._pip_install`` (no CLI dependency)."""
    if not specs:
    ...
    try:
        ...
        uv_env = hbm_subprocess_env(inherit_credentials=False)
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    if _lazy_install_target() is None:
        ...
        with contextlib.suppress(Exception):
            from hbm_cli.config import get_managed_system
            managed_by = get_managed_system()
        if managed_by:
            raise FeatureUnavailable(
                feature, missing,
                f"unsupported on {managed_by}-managed installs: this build's packages come from {managed_by}, "
                f"so HBM AGENT cannot install them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of HBM AGENT instead).")
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    target = _lazy_install_target()
    if not _allow_lazy_installs():
        sealed = os.environ.get("HBM_DISABLE_LAZY_INSTALLS") == "1" and target is None
        reason = ("runtime installs are disabled on this deployment: the agent environment is immutable "
                  "and no writable install target is configured (HBM_LAZY_INSTALL_TARGET)"
