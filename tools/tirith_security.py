...
GitHub releases to $HBM_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
from hbm_constants import get_hbm_home, get_hbm_home_override, hbm_home_key
...
def _cached_path() -> str | None:
    ...
    if get_hbm_home_override() is not None:
        return _resolved_path_by_home.get(hbm_home_key())
...
def _store_resolved(path: str) -> None:
    ...
    if get_hbm_home_override() is not None:
        _resolved_path_by_home[hbm_home_key()] = path
    else:
        _resolved_path = path
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_hbm_home()), ".tirith-install-failed")
...
# --- Auto-install ---
def _hbm_bin_dir() -> str:
    """$HBM_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_hbm_home()), "bin"), exist_ok=True)
    return d
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HBM_HOME/bin/tirith -> ``(installed_path,
    ...
    try:
        ...
        if src is None:
            return None, reason
        dest = os.path.join(_hbm_bin_dir(), "tirith")
        try:
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $HBM_HOME/bin."""
    hbm_bin = os.path.join(_hbm_bin_dir(), "tirith")
    return shutil.which("tirith") or (hbm_bin if _is_executable(hbm_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $HBM_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
