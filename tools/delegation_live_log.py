...
One append-only log per child under ``<hbm_home>/cache/delegation/live/
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.hbm)."""
    from hbm_constants import get_hbm_dir
    return get_hbm_dir("cache/delegation", "delegation_cache") / "live"
...
class LiveTranscriptWriter:
    ...
    def __init__(self, delegation_id: str, task_index: int, goal: str,
        ...
        with _best_effort(f"init ({delegation_id} task {task_index})"):
            ...
            path.write_text(
                "=== HBM AGENT subagent live transcript ===\n"
