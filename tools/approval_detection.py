...
# Sensitive write targets, matched via ~ / $HOME / $HBM_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once HBM_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HBM_ENV_PATH = (
    r'(?:~\/\.hbm/|(?:\$home|\$\{home\})/\.hbm/|(?:\$hbm_home|\$\{hbm_home\})/)' r'\.env\b'
)
# ~/.hbm/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_HBM_CONFIG_PATH = (
    r'(?:~\/\.hbm/|(?:\$home|\$\{home\})/\.hbm/|(?:\$hbm_home|\$\{hbm_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_HBM_ENV_PATH}|{_HBM_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hbm[^\n]*\.env\b', "access to HBM AGENT secrets (Windows path)"),
    # ── end of Windows tier
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart hbm-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart hbm-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hbm|gateway|cli\.py)\b', "kill hbm/gateway process (self-termination)"),
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.hbm.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.hbm...'; do
    # launchctl bootout "$label"`) never has "hbm" after the verb, and that slipped past and restarted 4 gateways
    # with zero approval. Erring broad is correct for an approval gate: an extra prompt is cheap.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:hbm|ai\.hbm)\b)', "stop/restart hbm launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/HBM AGENT file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.hbm/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on HBM AGENT config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a HBM AGENT-managed security file (~/.hbm/config.yaml or .env). sed -i bypasses the
    # redirection/tee patterns above because it mutates the file directly. See #14639.
    (rf'\bsed\s+-[^\s]*i.*(?:{_HBM_CONFIG_PATH}|{_HBM_ENV_PATH})', "in-place edit of HBM AGENT config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HBM_CONFIG_PATH}|{_HBM_ENV_PATH})', "in-place edit of HBM AGENT config/env (long flag)"),
    ...
    # coverage from #14639.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HBM_CONFIG_PATH}|{_HBM_ENV_PATH})', "in-place edit of HBM AGENT config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/HBM AGENT home prefixes to ~/ and ~/.hbm/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/HBM_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). HBM AGENT home
    # first: on Windows it nests under the user home, and folding the user home first would eat the prefix it needs.
    command = _rewrite_resolved_hbm_home(command)
...
def _rewrite_resolved_hbm_home(command: str) -> str:
    """Resolved HBM_HOME (and its realpath) -> ``~/.hbm/`` so the _HBM_CONFIG_PATH /
    _HBM_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.hbm")
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one HBM AGENT ad-hoc temp script."""
    try:
    ...
    return (
        ...
        and re.fullmatch(r"hbm-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.hbm.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a hbm-gateway
