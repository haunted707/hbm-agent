...
def _hbm_tags(fm: dict) -> Any:
    """``metadata.hbm.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    hbm_meta = metadata.get("hbm", {}) if isinstance(metadata, dict) else None
    return hbm_meta.get("tags", []) if isinstance(hbm_meta, dict) else []
