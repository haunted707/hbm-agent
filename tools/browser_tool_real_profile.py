...
hbm-owned copy, launch the real browser binary on it, and attach agent-browser.
...
def _real_profile_daemon_env() -> dict:
    ...
    Chrome is launched by HBM AGENT, not the daemon, so a self-exiting daemon would leave Chrome
...
def _real_profile_unsupported_reason(browser) -> Optional[str]:
    ...
    """
    from hbm_cli.browser_connect import UNSUPPORTED_CHANNEL
    if browser is None:
...
def _real_profile_cdp() -> tuple:
    ...
    if not _cloud._use_real_profile():
        ...
        try:
            from hbm_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    from hbm_cli.browser_connect import (chromium_executable, detect_default_chromium,
    ...
    with _bt._real_profile_cdp_lock:
        ...
        # hbm process) holds the user-data-dir open — that corrupts the databases.
        ...
        # A Chrome from an earlier hbm process can still hold the copy dir after its attach
