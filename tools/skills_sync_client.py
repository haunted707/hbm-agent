...
cross-device state is the ``sync-manifest`` blob in the plane. Only ~/.hbm/skills/ skills qualify."""
...
# Configuration -- env-first so HBM AGENT Cloud can enable sync via environment alone. Every knob:
# HBM_SYNC_<KEY> env -> config.yaml ``sync.<key>`` -> default (base_url = the sync plane, NOT
...
def resolve_sync_base_url() -> Optional[str]:
    """HBM_SYNC_BASE_URL -> ``sync.base_url`` -> production plane, without trailing slash
    (``/v1/sync/`` is appended by the client). None only if the default is blanked out."""
    env = os.getenv("HBM_SYNC_BASE_URL")
...
def sync_feature_enabled() -> bool:
    ...
    return _sync_config_bool("HBM_SYNC_ENABLED", "enabled", default=False)
...
def _rel_to_skills_dir(skill_dir: Path) -> Optional[Path]:
    """*skill_dir* relative to ~/.hbm/skills/, or None if outside/unresolvable."""
    try:
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """The skill's path relative to ~/.hbm/skills/ (posix), or None."""
...
def _all_local_skill_names() -> List[str]:
    """Every local skill name (dir under ~/.hbm/skills/ with SKILL.md; frontmatter ``name`` or dir name)."""
    names: List[str] = []
    root = _skills_dir()
    try:
...
def stable_device_id() -> str:
    """Per-device label at ~/.hbm/skills/.sync_device_id. An existing file always wins; else seeded
    from HBM_SYNC_DEVICE_NAME (first use only, for HBM AGENT Cloud) or a friendly default, then persisted."""
    with suppress(OSError):
        val = _device_id_path().read_text(encoding="utf-8").strip()
        if val:
            return val
    val = (os.environ.get("HBM_SYNC_DEVICE_NAME") or "").strip() or _default_device_label()
    try:
...
# the bundled manifest (skills_sync.py) and the plane's `sync-manifest`. ~/.hbm/skills/.sync_state.
...
def push_skills(client: Optional[SyncClient] = None, *, skill_names: Optional[List[str]] = None,
                identity: Optional[Dict[str, Any]] = None, message: str = "hbm-agent skill sync") -> Dict[str, Any]:
...
def maybe_push_skills(*, message: str = "hbm-agent skill sync") -> Optional[Dict[str, Any]]:
