...
HBM_HOME = get_hbm_home()  # all skills live in ~/.hbm/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped HBM_HOME (long-lived runtimes may import before profile set)."""
    configured = Path(SKILLS_DIR)
    return configured if configured != _SKILLS_DIR_AT_IMPORT else get_hbm_home() / "skills"
...
def load_env() -> Dict[str, str]:
    """Snapshot of HBM_HOME/.env for the post-skill secret-capture diff (same tokenizer that
    installs the profile scope, so a captured value never differs from the served one)."""
    from agent.secret_scope import load_env_file

    return load_env_file(get_hbm_home() / ".env")
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    """``~/.hbm/skills/mlops/axolotl/SKILL.md`` -> ``"mlops"``; active profile dir first
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    """Disabled in config? Platform precedence: explicit arg, ``HBM_PLATFORM``, session
    ``HBM_SESSION_PLATFORM``. A globally-disabled skill stays disabled on every platform
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("HBM_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("HBM_SESSION_PLATFORM") or ""
...
def _log_security_warnings(name: str, skill_md: Path, content: str, all_dirs, active_skills_dir):
    ...
    if not _under_any(skill_md, trusted_dirs):
        warnings.append(f"skill file is outside the trusted skills directory (~/.hbm/skills/): {skill_md}")
...
def skill_view(
    ...
    try:
        ...
        # tags/related_skills: metadata.hbm.* (agentskills.io) first, then top-level.
        metadata = frontmatter.get("metadata")
        hbm_meta = (metadata.get("hbm", {}) or {}) if isinstance(metadata, dict) else {}
        tags, related_skills = (
            _parse_tags(hbm_meta.get(k) or frontmatter.get(k, "")) for k in ("tags", "related_skills"))
