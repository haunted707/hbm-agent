...
``metadata.hbm.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.hbm.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.hbm.blueprint`` is a mapping with a
    ...
    hbm = meta.get("hbm") if isinstance(meta, dict) else None
    blueprint = hbm.get("blueprint") if isinstance(hbm, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.hbm.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.hbm.blueprint`` block) ready for ``hbm-agent skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"hbm": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
