...
_HBM_CONFIG_FILES = r'\.hbm/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.hbm/\.env|\~/\.hbm/\.env',
     "hbm_env_access", "critical", "exfiltration", "directly references HBM AGENT secrets file"),
    ...
    # invocation: any client plugin that relays HBM AGENT' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for HBM AGENT/other config (setup docs routinely say
    ...
    (_prose_modify_re(_HBM_CONFIG_FILES),
     "hbm_config_mod", "high", "persistence", "modification language aimed at HBM AGENT configuration files (verify intent)"),
    (_shell_write_re(_HBM_CONFIG_FILES),
     "hbm_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting HBM AGENT configuration files"),
    (r'\.hbm/config\.yaml|\.hbm/SOUL\.md',
     "hbm_config_ref", "low", "persistence", "references HBM AGENT configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is HBM AGENT-native; `.clawhubignore` is honored for skills published through ClawHub.
