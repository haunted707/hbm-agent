...
# Config-based allowlist, keyed by HBM AGENT home: under gateway.multiplex_profiles one process serves
...
def _is_hbm_provider_credential(name: str) -> bool:
    """True if ``name`` is a HBM AGENT-managed provider credential per
    ``_HBM_PROVIDER_ENV_BLOCKLIST`` or a dynamic HBM AGENT-internal secret
    ...
    in the ``execute_code`` child); non-HBM AGENT keys (TENOR_API_KEY, …) stay
    registerable. Fails closed when the blocklist cannot be imported."""
    try:
        from tools.environments.local_env_policy import (
            _HBM_PROVIDER_ENV_BLOCKLIST, _is_hbm_internal_secret)
        ...
        return True
    return _is_hbm_internal_secret(name) or name in _HBM_PROVIDER_ENV_BLOCKLIST


def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register env var names as allowed in sandboxed environments (typically a
    skill's ``required_environment_variables``). HBM AGENT-managed provider credentials
    ...
    for name in _accepted((n.strip() for n in var_names), (
        "env passthrough: refusing to register HBM AGENT provider "
        "credential %r (blocked by _HBM_PROVIDER_ENV_BLOCKLIST). "
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not HBM AGENT provider credentials; refused
    names are logged with *refusal_msg* (``%r`` = name)."""
    for name in names:
        if not name:
            continue
        if _is_hbm_provider_credential(name):
...
def _load_config_passthrough() -> frozenset[str]:
    ...
    try:
        ...
        result.update(_accepted((i.strip() for i in items if isinstance(i, str)), (
            "env passthrough: refusing to register HBM AGENT "
            "provider credential %r from config.yaml (blocked "
            "by _HBM_PROVIDER_ENV_BLOCKLIST). Operator "
