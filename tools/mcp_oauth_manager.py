...
class HbmMCPOAuthProvider(HbmProviderMixin, *_SDK_BASES):
    ...
    fixes come from ``HbmProviderMixin``. Only usable when the SDK's OAuth module imported.
    ...
    def __init__(self, *args: Any, server_name: str = "", preregistered: bool = False, **kwargs: Any):
        ...
        self._hbm_server_name = server_name
        self._hbm_home = ""
        # A config-supplied client_id rejected as invalid_client means the *config* is wrong — only DCR clients auto-heal.
        self._hbm_preregistered = preregistered

    def _hbm_storage(self):
        """The context storage when it is a ``HbmTokenStorage``, else None."""
        from tools.mcp_oauth import HbmTokenStorage
        return self.context.storage if isinstance(self.context.storage, HbmTokenStorage) else None

    def _log_nonfatal(self, what: str, exc: BaseException) -> None:
        logger.debug("MCP OAuth '%s': %s failed (non-fatal): %s", self._hbm_server_name, what, exc)
    ...
    async def _initialize(self) -> None:
        ...
        Bearer tokens as "valid"; seeding the expiry (``HbmTokenStorage`` persists absolute
        ...
        await super()._initialize()  # HbmProviderMixin: restores metadata from disk, enforces issuer binding
    ...
    async def _prefetch_oauth_metadata(self) -> None:
        ...
        # The SDK's httpx flavour, not HBM AGENT': `create_oauth_metadata_request` returns *its* (httpx2) Request objects.
        ...
        async def _send(client, url: str, label: str):
            try:
                ...
                logger.debug("MCP OAuth '%s': %s discovery to %s failed: %s", self._hbm_server_name, label, url, exc)
                return None
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            for url in build_oauth_authorization_server_metadata_discovery_urls(self.context.auth_server_url, server_url):
                ...
                if asm:
                    ...
                    storage = self._hbm_storage()  # persist now so a later cold-load skips discovery
                    if storage is not None:
                        storage.save_oauth_metadata(asm)
                    logger.debug("MCP OAuth '%s': pre-flight ASM discovered token_endpoint=%s",
                                 self._hbm_server_name, asm.token_endpoint)
                    break
    ...
    async def _maybe_flag_poisoned_client(self, response: Any) -> None:
        ...
        """
        try:
            if (self._hbm_preregistered or getattr(response, "status_code", None) not in (400, 401)
            ...
            if cimd_url and getattr(self.context.client_info, "client_id", None) == cimd_url:
                logger.warning("MCP OAuth '%s': authorization server rejected our Client ID Metadata Document (%s) "
                               ...
                               self._hbm_server_name, cimd_url)
    ...
    async def async_auth_flow(self, request):  # type: ignore[override]
        try:  # pre-flow hook: reload from disk if it changed (non-fatal on error)
            await get_manager().invalidate_if_disk_changed(self._hbm_server_name, hbm_home=self._hbm_home)
...
_HBM_PROVIDER_CLS: Optional[type] = HbmMCPOAuthProvider if _SDK_BASES else None
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._hbm_home = key[0]
            return entry.provider

    @staticmethod
    def _key(server_name: str, hbm_home: str | Path | None = None) -> tuple[str, str]:
        from hbm_constants import get_hbm_home
        home = Path(hbm_home) if hbm_home is not None else get_hbm_home()
    ...
    def _build_provider(self, server_name: str, entry: _ProviderEntry) -> Optional[Any]:
        """Build a ``HbmMCPOAuthProvider``; None if the SDK's OAuth support is unavailable."""
        if _HBM_PROVIDER_CLS is None:
    ...
    def restore_entry(self, server_name: str, entry: _ProviderEntry | None, *, hbm_home: str | Path | None = None) -> None:
        """Restore a provider entry removed for a failed reauthorization."""
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, hbm_home), entry)

    def evict(self, server_name: str, *, hbm_home: str | Path | None = None) -> _ProviderEntry | None:
        """Drop only the in-process provider, preserving persisted OAuth state."""
        with self._entries_lock:
            return self._entries.pop(self._key(server_name, hbm_home), None)

    async def invalidate_if_disk_changed(self, server_name: str, *, hbm_home: str | Path | None = None) -> bool:
        """Force the SDK provider to reload when the tokens file mtime changed (e.g. a cron refresh); True if so."""
        from tools.mcp_oauth import _get_token_dir, _safe_filename
        entry = self._entries.get(self._key(server_name, hbm_home))
        if entry is None or entry.provider is None:
            return False
        async with entry.lock:
            try:
                mtime_ns = (_get_token_dir(hbm_home) / f"{_safe_filename(server_name)}.json").stat().st_mtime_ns
            except OSError:
                return False
