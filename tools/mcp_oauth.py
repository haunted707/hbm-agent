...
refresh; this module supplies ``HbmTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is HBM AGENT' Client ID Metadata
...
from hbm_constants import secure_parent_dir
...
async def acquire_refresh_fence(path: "Path", *, timeout: float = _REFRESH_FENCE_TIMEOUT_SECONDS) -> int:
    ...
    try:
        ...
        mkdir_under_hbm_home(lock_path.parent)
...
_USER_SKIPPED_SENTINEL = "__hbm_user_skipped__"


def _get_token_dir(hbm_home: str | Path | None = None) -> Path:
    """``HBM_HOME/mcp-tokens/`` — per-profile token directory."""
    from hbm_constants import get_hbm_home

    return Path(hbm_home if hbm_home is not None else get_hbm_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "HbmTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "HbmTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``hbm-agent
...
class HbmTokenStorage:
    """Persist OAuth state as ``HBM_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, hbm_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._hbm_home = Path(hbm_home) if hbm_home is not None else None
    ...
    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._hbm_home) / f"{self._server_name}{suffix}"
    ...
    def _fixup_loaded_tokens(self, data: dict) -> None:
        # ``hbm_issuer`` is HBM AGENT bookkeeping, not an SDK OAuthToken field: pop before validation.
        self.loaded_issuer = data.pop("hbm_issuer", None)
    ...
    async def set_tokens(self, tokens: "OAuthToken") -> None:
        ...
        if self._bound_issuer:  # which authorization server granted these tokens (never sent on the wire)
            payload["hbm_issuer"] = self._bound_issuer
    ...
    def stamp_issuer(self, issuer: str) -> None:
        """Backfill ``hbm_issuer`` onto a pre-binding token file: adopt the currently discovered
        issuer once instead of forcing a re-login, so the *next* read is protected."""
        data = _read_json(self._tokens_path())
        if data is None or data.get("hbm_issuer"):
            return
        data["hbm_issuer"] = str(issuer)
        try:
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._hbm_home)
        from hbm_constants import mkdir_under_hbm_home

        mkdir_under_hbm_home(token_dir)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to HBM AGENT.</p>" if parsed["code"]
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected HBM AGENT' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
HbmOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, hbm_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    HbmTokenStorage(server_name, hbm_home=hbm_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; HBM AGENT only decides
...
_CIMD_CLIENT_METADATA_URL = "https://nousresearch.github.io/hbm-agent/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "HbmTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after HBM AGENT must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "HbmTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "HbmTokenStorage | None" = None) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "HBM AGENT"),
...
def _invalidate_tokens_on_client_change(
    storage: "HbmTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
...
def _maybe_preregister_client(storage: "HbmTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older HBM AGENT is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    if HbmOAuthClientProvider is None:
        from tools.mcp_oauth_provider import HbmProviderMixin

        HbmOAuthClientProvider = type("HbmOAuthClientProvider", (HbmProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus HBM AGENT' token-endpoint fixes (see ``HbmProviderMixin``).",
            "__module__": __name__, "_hbm_logger": logger})
    return HbmOAuthClientProvider(server_url=server_url, **kwargs)
