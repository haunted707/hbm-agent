...
_terminal_scope_var: ContextVar = ContextVar("hbm_terminal_scope", default=None)
...
def build_profile_terminal_scope(
    hbm_home: "Any", *, env_overlay: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    *env_overlay* is a TRUSTED ``TERMINAL_*`` mapping captured from the launch process before
    ...
    """
    from hbm_cli.config import TERMINAL_CONFIG_ENV_MAP, _terminal_env_value
    ...
    if config_exists:
        from hbm_cli.config import fast_safe_load
...
def install_profile_terminal_scope(
    hbm_home: "Any", *, env_overlay: Optional[Dict[str, str]] = None) -> Token:
    """Build AND install a profile's policy; on failure install the refusal scope. Never raises."""
    try:
        return set_terminal_scope(build_profile_terminal_scope(hbm_home, env_overlay=env_overlay))
...
@contextmanager
def install_and_reset_profile_terminal_scope(hbm_home: "Any") -> Iterator[None]:
    """Install the profile's terminal policy for a bounded turn/fire. Never raises."""
    token = install_profile_terminal_scope(hbm_home)
    try:
        yield
    finally:
