...
When any profile carries ``ui_meta['hbm-bots']`` in profile.yaml (Bot-Mode-managed),
...
def _default_home() -> str:
    """Ambient process HBM_HOME (env, else the platform default) as a string."""
...
def _hbm_root(home: Path) -> Path:
    """Root ~/.hbm for both the default profile and named profiles."""
...
def _handle(name: str) -> str:
    # The mention middleware aliases the default profile as @hbm.
    return "hbm" if name == "default" else name
...
def _bots_meta(data: dict | None) -> dict | None:
    """The ``ui_meta['hbm-bots']`` block of a parsed profile.yaml, if a dict."""
    ui_meta = data.get("ui_meta") if data else None
    bots = ui_meta.get("hbm-bots") if isinstance(ui_meta, dict) else None
    return bots if isinstance(bots, dict) else None


def _is_bot_managed(profile_dir: Path) -> bool:
    return _bots_meta(_read_yaml_dict(profile_dir / "profile.yaml", "hbm-bots")) is not None
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    return _swallow(lambda: _any_managed(_hbm_root(_resolve_home(home))), False)
...
def _build_section(home: Path) -> str:
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each HBM AGENT profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    the AGENT'S OWN resolved home (session-db derived), not ambient HBM_HOME — build threads
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        token = set_hbm_home_override(str(resolved))
        try:
