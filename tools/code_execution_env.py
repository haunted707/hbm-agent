...
# prefix, the exact-name HBM_ allowlist, or (Windows) an OS-essential name.
# The broad "HBM_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (HBM_BASE_URL, HBM_KANBAN_DB, *_WEBHOOK).
# HBM_RPC_SOCKET / HBM_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. HBM_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports HBM AGENT code loses the Kanban mutation guard
# while still inheriting HBM_HOME.
_HBM_CHILD_ALLOWED = frozenset({
    "HBM_HOME", "HBM_PROFILE", "HBM_CONFIG", "HBM_ENV", "HBM_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational HBM_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret HBM_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _HBM_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("HBM_"):
            _dropped_hbm.append(k)
    if _dropped_hbm:
        logger.debug(
            "execute_code: dropped %d non-allowlisted HBM_* var(s) from the "
            ...
            len(_dropped_hbm), ", ".join(sorted(_dropped_hbm)),
        )
    ...
    # non-allowlisted HBM_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "HBM_KANBAN_DB", "HBM_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["HBM_RPC_SOCKET"] = rpc_endpoint
    child_env["HBM_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; HBM_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — hbm_time resolves the routed profile's).
    from hbm_time import get_timezone_name

    _tz_name = get_timezone_name()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("HBM_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # Multiplexed gateway/Desktop (#110303): the server process env carries the machine-default
    # HBM_HOME, but this turn runs under a per-profile override (ContextVar bound per turn).
    ...
    _home_override = get_hbm_home_override()
    if _home_override:
        child_env["HBM_HOME"] = _home_override
    # PYTHONPATH: the staging dir (hbm_tools.py) must always be importable even when project
    # mode changes CWD. HBM AGENT's root is added ONLY when the child runs in HBM AGENT's Python env —
    # exposing HBM AGENT's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited HBM AGENT-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip HBM AGENT-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-HBM AGENT-
    ...
    _strip_hbm_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_hbm_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import hbm_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the HBM AGENT "
                    "environment; hbm-agent root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the HBM AGENT environment.
...
def _uses_hbm_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to HBM AGENT's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the hbm-agent root."""
