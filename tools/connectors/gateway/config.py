...
def load_config() -> ConnectorConfig:
    try:
        from hbm_cli.config import load_config_readonly as _load
...
def operation_session_key(session_id: Optional[str]) -> str:
    ...
    (``HBM_SESSION_KEY``), falling back to the agent's session id where no gateway bound one.
    ...
    return get_session_env("HBM_SESSION_KEY", "") or str(session_id or "")
...
def session_platform() -> str:
    ...
    platform = get_session_env("HBM_SESSION_PLATFORM", "") or get_session_env("HBM_SESSION_SOURCE", "")
