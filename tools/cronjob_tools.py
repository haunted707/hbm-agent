...
# `cronjob(action="run")` executes in-process (comfortably below HBM_AGENT_TIMEOUT).
# Mirrors the 10s cadence of tools/environments/base.py::touch_activity_if_due (delegate_task's heartbeat
# uses 30s) — comfortably below the 1800s default HBM_AGENT_TIMEOUT. See #76502.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0
# Hard ceiling: with HBM_CRON_TIMEOUT=0 a truly hung run would otherwise mask the
# gateway watchdog forever; past this the heartbeat stops and the watchdog regains authority.
# The child cron run has its own inactivity watchdog (HBM_CRON_TIMEOUT, default 600s) that bounds a
# wedged job, but with HBM_CRON_TIMEOUT=0 (explicit "unlimited") a truly hung run_one_job would otherwise
...
def _try_dispatch_background_run(
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("HBM_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
def _cronjob_schema_overrides() -> dict:
    ...
    params["properties"]["script"]["description"] = _script_description(display_hbm_home())
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": _script_description("the profile HBM_HOME")
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("HBM_INTERACTIVE")
        or env_var_enabled("HBM_GATEWAY_SESSION")
        or env_var_enabled("HBM_EXEC_ASK")
        or is_truthy_value(get_session_env("HBM_CRON_SESSION", ""))
    )
