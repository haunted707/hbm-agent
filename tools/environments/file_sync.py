...
_FORCE_SYNC_ENV = "HBM_FORCE_FILE_SYNC"
...
_SYNC_BACK_TEMP_PREFIX = "hbm-sync-back-"
...
def iter_sync_files(container_base: str = "/root/.hbm") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.hbm to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.hbm", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``HBM_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def _sync_transaction(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="hbm-sync-push-") as staging:
                staged_files = []
                pushed_hashes = {}
    ...
    def sync_back(self, hbm_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.hbm/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(hbm_home=hbm_home)

    def _sync_back_transaction(self, hbm_home: Path | None = None) -> None:
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.hbm/skills/b.md`` -> ``~/.hbm/skills/b.md``)."""
