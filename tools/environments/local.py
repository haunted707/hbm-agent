...
from hbm_constants import get_process_hbm_home
...
from tools.environments.local_env_policy import (
    _ALWAYS_STRIP_KEYS, _HBM_PROVIDER_ENV_BLOCKLIST, _HBM_PROVIDER_ENV_FORCE_PREFIX,
    _is_hbm_internal_secret, _is_terminal_first_party_env,
...
from tools.environments.local_pythonpath import (
    _build_hbm_repo_root_aliases, _strip_hbm_owned_pythonpath_and_runtime_markers)
...
# get_temp_dir() defaults to HBM_HOME/cache/terminal (real storage, not tmpfs), so
...
# Background artifacts come in triplets (hbm_bg_<id>.log/.pid/.exit). A live
...
_BG_GROUP_RE = re.compile(r"^(hbm_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return HBM_HOME/cache/terminal, or None if unresolvable."""
    try:
        from hbm_constants import get_hbm_home
        return get_hbm_home() / "cache" / "terminal"
    except Exception:
        return None
...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local HBM_HOME override, then the subprocess HOME contract."""
    from hbm_constants import apply_subprocess_home_env, get_hbm_home_override
    try:
        if value := get_hbm_home_override():
            env["HBM_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    """Bridge gateway session ContextVars (HBM_SESSION_*) into a child env.
...
def _filter_secret_env(
    ...
    """Copy *items* into *out*, dropping HBM AGENT-managed secrets. ``_HBM_FORCE_<NAME>``
    ...
    for key, value in items.items():
        if key.startswith(_HBM_PROVIDER_ENV_FORCE_PREFIX):
            if not unwrap_force:
                continue
            key = key[len(_HBM_PROVIDER_ENV_FORCE_PREFIX):]
            if not _is_hbm_internal_secret(key):
                out[key] = value
            continue
        if _is_hbm_internal_secret(key) or key in plugin_strip:
            continue
        first_party = _is_terminal_first_party_env(key)
        passthrough = is_env_passthrough(key)
        if key in _HBM_PROVIDER_ENV_BLOCKLIST and not (passthrough or first_party):
            continue
...
def _finalize_child_env(env: dict) -> dict:
    ...
    bridging, HBM AGENT-owned PYTHONPATH + venv-marker strip, MSYS defaults, delegate_task
    Kanban scrub. Returns the (possibly new) dict."""
    _apply_profile_home(env)
    _inject_session_context_env(env)
    _strip_hbm_owned_pythonpath_and_runtime_markers(env)
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the hbm-agent install dir so bare ``hbm-agent`` resolves
    ...
    # Keep bare ``hbm-agent`` invocations available to child jobs even when the gateway was launched by a
    ...
    if path_key is not None:
        out[path_key] = _prepend_hbm_bin_dir(fix_path(out.get(path_key, "")))
    return _finalize_child_env(out)


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter HBM AGENT-managed secrets from a subprocess environment (background/PTY
...
def hbm_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
...
def _scrub_credentials(env: dict, *, inherit_credentials: bool) -> dict:
    ...
    if not inherit_credentials:
        strip |= _HBM_PROVIDER_ENV_BLOCKLIST
    for key in list(env):
        if (key in strip or key.startswith(_HBM_PROVIDER_ENV_FORCE_PREFIX)
                or _is_hbm_internal_secret(key)):
            del env[key]
    return env
...
def build_subprocess_env(
    ...
    bridges HBM_HOME + HOME and ``extra`` is applied last so caller overrides win."""
...
def served_profile_child_env(
    ...
    env = dict(base) if base is not None else hbm_subprocess_env(inherit_credentials=inherit_credentials)
    target = str(target_home or get_hbm_home_override() or "")
    if target:
        env["HBM_HOME"] = target
...
def _is_routed_home(target_home: "str | Path") -> bool:
    ...
    from hbm_constants import get_process_hbm_home
    try:
        return Path(target_home).resolve() != get_process_hbm_home().resolve()
    except OSError:
        return True
...
def strip_launch_profile_env(env: dict, target_home: "str | Path | None" = None) -> dict:
    ...
    installing a HBM_HOME override without that flag."""
    from agent.secret_scope import _is_global_env, load_env_file
    from hbm_constants import get_hbm_home_override, get_process_hbm_home
    target = target_home or get_hbm_home_override()
    if not target or not _is_routed_home(target):
        return env
    launch_home = get_process_hbm_home()
    from hbm_cli.config import TERMINAL_CONFIG_ENV_MAP
...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    """Ordered bash.exe candidates on Windows: HBM_GIT_BASH_PATH, our portable Git
    under %LOCALAPPDATA%\\hbm\\git (PortableGit ``bin`` and MinGit ``usr\\bin``),
    ...
    roots = [
        lad and os.path.join(lad, "hbm", "git", "bin"),
        lad and os.path.join(lad, "hbm", "git", "usr", "bin"),
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("HBM_GIT_BASH_PATH")
    candidates = _windows_bash_candidates(custom)
    # First candidate that can actually start wins: a stale HBM_GIT_BASH_PATH
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "HBM_GIT_BASH_PATH=%s fails to start; using %s instead", custom, candidate)
            return candidate
    if candidates:
        probe_details = "\n".join(
        ...
        return candidates[0]
    raise RuntimeError(
        "Git Bash not found. HBM AGENT requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set HBM_GIT_BASH_PATH to your bash.exe location.")
...
# Cached directory containing the ``hbm-agent`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_HBM_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_hbm_bin_dir() -> str | None:
    """Directory holding the ``hbm-agent`` console-script, or None (cached). A gateway
    launched by systemd/cron/a desktop launcher lacks the install dir on PATH and bare
    ``hbm-agent`` exits 127. Order: ``which``; absolute ``sys.argv[0]`` naming a real
    hbm executable; ``sys.executable``'s dir if it holds the shim."""
    global _HBM_BIN_DIR
    if _HBM_BIN_DIR is not _SENTINEL:
        return _HBM_BIN_DIR  # type: ignore[return-value]
    which = shutil.which("hbm-agent")
    argv0 = sys.argv[0] if sys.argv else ""
    base = os.path.basename(argv0).lower()
    exe_dir = os.path.dirname(sys.executable) if sys.executable else ""
    shim = "hbm-agent.exe" if _IS_WINDOWS else "hbm-agent"
    if which:
        candidate = os.path.dirname(which)
    elif (os.path.isabs(argv0) and (base == "hbm-agent" or base.startswith("hbm-agent."))
    ...
    _HBM_BIN_DIR = candidate if candidate and os.path.isdir(candidate) else None
    return _HBM_BIN_DIR


def _prepend_hbm_bin_dir(existing_path: str) -> str:
    """Prepend the hbm-agent install dir to ``existing_path`` if missing."""
    bin_dir = _resolve_hbm_bin_dir()
    return _prepend_missing_path_entries(existing_path, [bin_dir] if bin_dir else [])


def _managed_runtime_path_entries() -> list[str]:
    """Existing HBM AGENT-managed runtime dirs: ``$HBM_HOME/node`` (+``/bin``) and
    ``$HBM_HOME/bin`` (managed ``uv``). Per call, not cached: home is
    profile-scoped and a managed tree can appear mid-process."""
    try:
        from hbm_constants import get_hbm_home, iter_hbm_node_dirs
        return [str(d) for d in (*iter_hbm_node_dirs(), get_hbm_home() / "bin") if d.is_dir()]
    except Exception:
        return []
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    and ``wmic``. HBM AGENT runs terminal commands through bash on Windows, so set the standard MSYS opt-out by
    default. Refs #56700.
...
# --- HBM AGENT venv / repo-root detection (module-level, computed once) ---
...
# spellings — the Windows gateway launcher renders HBM AGENT-owned paths under the
# configured HBM_HOME spelling (possibly a junction to another drive).
_hbm_repo_root: Path = Path(__file__).resolve().parents[2]
_hbm_repo_root_aliases: tuple[Path, ...] = _build_hbm_repo_root_aliases(
    _hbm_repo_root, Path(__file__).absolute().parents[2], get_process_hbm_home())
_in_venv: bool = (getattr(sys, "base_prefix", sys.prefix) != sys.prefix
                  or hasattr(sys, "real_prefix"))  # real_prefix: virtualenv<20
_hbm_site_packages: list[Path] | None = None  # lazily cached by local_pythonpath
...
def _kill_process_group_posix(proc) -> None:
    ...
    try:
        ...
        if (pgid := getattr(proc, "_hbm_pgid", None)) is None:
            raise
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the HBM AGENT host itself — controller-side platform behavior
    ...
    def get_temp_dir(self) -> str:
        ...
        (Termux has no /tmp), ``HBM_HOME/cache/terminal`` (real storage: tmpfs /tmp
        fills under HBM AGENT load; pruned by ``cleanup_terminal_temp_cache``), /tmp,
        ...
        so always the HBM_HOME cache dir with forward slashes (bash- and Python-valid)."""
        if _IS_WINDOWS:
            cache_dir = (_default_terminal_temp_dir()
                         or Path(tempfile.gettempdir()) / "hbm_terminal")
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        if not _IS_WINDOWS:
            with contextlib.suppress(ProcessLookupError):
                proc._hbm_pgid = os.getpgid(proc.pid)
