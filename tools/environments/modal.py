...
def _snapshot_store() -> Path:
    ...
    return get_hbm_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str = "/root", timeout: int = 60,
        ...
        def _create(image_spec: Any) -> None:
            async def _create_sandbox():
                app = await _modal.App.lookup.aio("hbm-agent", create_if_missing=True)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .hbm/ as a tar archive (sandboxes run as root, so /root/.hbm)."""
        data = self._exec("tar cf - -C / root/.hbm", timeout=120, fail_label="bulk download", capture=True)
