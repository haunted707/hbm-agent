...
def _ensure_docker_available() -> None:
    ...
    if not docker_exe:
        raise _docker_unavailable(
            ...
            hint="Install Docker (or fix PATH) and retry, or run `hbm setup terminal` to switch to Local.")
    try:
        ...
        raise _docker_unavailable(
            ...
            hint="Start Docker (e.g. `systemctl start docker` or launch Docker Desktop), then retry — "
                 "or run `hbm setup terminal` to switch to Local.")
    except Exception:
...
class DockerEnvironment(BaseEnvironment):
    ...
    def _find_reusable_container(
        ...
        reused after ``hbm egress disable`` (baked-in proxy env and CA mounts), and every
