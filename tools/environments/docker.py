...
from tools.environments.remote_common import (
    bash_argv, client_env_with, load_hbm_env_vars, prepend_unset, resolve_passthrough_env, run_capture)
...
# Module-level binding: tests patch ``docker._load_hbm_env_vars`` to fake the .env file.
_load_hbm_env_vars = load_hbm_env_vars
...
def _get_active_profile_name() -> str:
    """Active HBM AGENT profile name, or ``"default"`` on any error. Resolved at container-create
...
def reap_orphan_containers(
    *, max_age_seconds: int = 600, profile_filter: str | None = None, docker_exe: str | None = None,
    ...
    """Remove stale hbm-tagged containers left behind by prior processes (SIGKILL/OOM
    ...
    filters = ["--filter", "label=hbm-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=hbm-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    """Locate the docker/podman CLI (cached): ``HBM_DOCKER_BINARY`` override, ``docker``
    on PATH, ``podman`` on PATH, then macOS Docker Desktop locations; ``None`` if absent."""
    global _docker_executable
    if _docker_executable is not None:
        return _docker_executable

    override = os.getenv("HBM_DOCKER_BINARY")
    if override and _is_executable(override):
        logger.info("Using HBM_DOCKER_BINARY override: %s", override)
        found = override
...
def _ensure_docker_available() -> None:
    ...
    if not docker_exe:
        raise _docker_unavailable(
            ...
            hint="Install Docker (or fix PATH) and retry, or run `hbm-agent setup terminal` to switch to Local.")
    try:
        ...
        raise _docker_unavailable(
            ...
            hint="Start Docker (e.g. `systemctl start docker` or launch Docker Desktop), then retry — "
                 "or run `hbm-agent setup terminal` to switch to Local.")
    except Exception:
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # s6-overlay images (e.g. hbm-agent:latest) already use /init as PID 1 and exec
        ...
        # Labels identify hbm containers to the orphan reaper (hbm-agent=1),
        ...
        self._labels = {
            "hbm-agent": "1",
            "hbm-task-id": task_label,
            "hbm-profile": profile_name,
    ...
    def _mount_args(self, volumes, host_cwd, auto_mount_cwd, task_id) -> tuple[list[str], list[str]]:
        ...
        Persistent mode bind-mounts from TERMINAL_SANDBOX_DIR (default ~/.hbm/sandboxes/)."""
    ...
    def _docker_run(self, cwd: str) -> str:
        ...
        container_name = f"hbm-{uuid.uuid4().hex[:8]}"
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        return resolve_passthrough_env(self._forward_env, hbm_env_loader=_load_hbm_env_vars)
    ...
    def _recreate_container(self) -> bool:
        ...
        existing = self._find_reusable_container(
            self._labels.get("hbm-task-id", ""),
            self._labels.get("hbm-profile", ""),
        ...
        if not self._container_id:
            if not self._image:
                ...
                return False
            try:
                new_name = f"hbm-{uuid.uuid4().hex[:8]}"
                result = run_capture(
    ...
    def _find_reusable_container(
        ...
        reused after ``hbm-agent egress disable`` (baked-in proxy env and CA mounts), and every
        ...
        filters = [
            "--filter", "label=hbm-agent=1",
            "--filter", f"label=hbm-task-id={task_label}",
            "--filter", f"label=hbm-profile={profile_label}",
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"hbm-cleanup-{log_id}")
        with _TEARDOWN_LOCK:
