...
# Module-level binding: tests patch ``ssh._load_hbm_env_vars`` to fake the .env file.
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "hbm-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.hbm"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.hbm directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.hbm"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="hbm-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .hbm/ as a tar archive."""
        # Tar from / with the full path so archive entries keep absolute paths
        # (home/user/.hbm/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.hbm".lstrip("/")
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        values, unset_names = resolve_passthrough_env(hbm_env_loader=_load_hbm_env_vars)
