...
from tools.environments.local_env_policy import _HBM_PROVIDER_ENV_BLOCKLIST, _is_hbm_internal_secret


def load_hbm_env_vars() -> dict[str, str]:
    """``~/.hbm/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            hbm_env_loader: Callable[[], dict[str, str]] = load_hbm_env_vars,
    ...
    is filtered through the HBM AGENT provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_hbm_internal_secret(k)}
    forward_keys = set(explicit_forward) | (implicit_forward - _HBM_PROVIDER_ENV_BLOCKLIST)
    hbm_env = hbm_env_loader() if forward_keys else {}
    exec_env: dict[str, str] = {}
    unset_names: set[str] = set()
    for key in sorted(forward_keys):
        value = os.getenv(key) or hbm_env.get(key)
