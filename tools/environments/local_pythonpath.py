"""HBM AGENT-owned PYTHONPATH stripping for child processes. Launchers prepend the repo
root and the HBM AGENT venv's site-packages so the backend can ``import tools``; leaked
into a child Python of a DIFFERENT version they load the backend's C extensions and
crash. Only entries proven HBM AGENT-owned by *path provenance* are removed — never by a
cross-version heuristic. Module state (``_hbm_repo_root_aliases``, ``_in_venv``,
``_hbm_site_packages``) lives in ``tools.environments.local`` (via :func:`_state`)
...
def _build_hbm_repo_root_aliases(
    resolved_root: Path, lexical_root: Path, configured_home: Path,
) -> tuple[Path, ...]:
    """Exact repo-root spellings emitted by HBM AGENT launchers. Mirrors
    ``gateway_windows._preserve_hbm_home_path`` (physical path under the resolved
    HBM_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary HBM_HOME descendants as HBM AGENT-owned. A repo-level junction
    ...
    # and the repo lives beside the profiles dir (as get_default_hbm_root() does).
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    if not env.get("VIRTUAL_ENV") or not any(
            _same_path(candidate, root / "venv") for root in _state()._hbm_repo_root_aliases):
        return None
    try:
...
def _get_hbm_site_packages(env: dict) -> list[Path]:
    """Exact site-packages dirs owned by the HBM AGENT runtime (cached):
    ...
    local = _state()
    if local._hbm_site_packages is None:
        result: list[Path] = []
        if local._in_venv:
            try:
                import site
        ...
        local._hbm_site_packages = list(result)
    result = list(local._hbm_site_packages)
...
def _strip_hbm_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip HBM AGENT-owned PYTHONPATH entries, then the runtime marker vars. Order is
...
def _strip_hbm_owned_pythonpath(env: dict) -> None:
    """Remove HBM AGENT-owned PYTHONPATH entries: only exact matches of the repo root
    ...
    if not pp:
        return
    owned_paths = [*_get_hbm_site_packages(env), *_state()._hbm_repo_root_aliases]
    ...
    if stripped:
        logger.debug("Stripped HBM AGENT-owned entries from PYTHONPATH: %s", stripped)
