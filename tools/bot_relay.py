...
from tools.bot_mode_probe import _default_home, _hbm_root
...
# Mirrors RELAY_DELIVER_TIMEOUT_MS in apps/desktop/src/plugins/hbm-bots/relay.ts; both test suites pin it.
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import hbm_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("hbm" if profile == "default" else profile)
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_hbm_root(Path(_default_home())))
...
def _hbm_cli() -> str:
    """hbm CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "hbm" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``hbm-agent`` entrypoint. A bare ``"hbm"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("hbm-agent.exe" if sys.platform == "win32" else "hbm-agent")
    return str(sibling) if sibling.is_file() else shutil.which("hbm-agent") or "hbm-agent"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_hbm_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]
...
def _delivery_child_session_env_names() -> "tuple[str, ...]":
    ...
    ``HBM_SESSION_*`` prefix match, which would also strip non-identity knobs
    (e.g. ``HBM_SESSION_STALL_TIMEOUT``)."""
...
def delivery_env(author: Optional[dict], profile_home: "str | Path | None" = None) -> dict[str, str]:
    ...
    ``os.environ``; ``-p`` alone only pinned HBM_HOME. ``profile_home`` is the target's home when the
...
# Deliveries are separate ``hbm-agent`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``hbm-agent``
