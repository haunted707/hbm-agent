"""Shared ``OAuthClientProvider`` customizations for HBM AGENT MCP OAuth.
...
if TYPE_CHECKING:
    from tools.mcp_oauth import HbmTokenStorage
...
class HbmProviderMixin:
    """Token-endpoint fixes layered over the SDK's ``OAuthClientProvider`` (must precede it in
    the MRO; subclasses set ``_hbm_logger`` to keep their own logger name).
    ...
    def _prepare_token_request(self, request):
        ...
        ua = getattr(self, "_hbm_token_user_agent", None)  # tests build via __new__
        if ua:
            request.headers["User-Agent"] = ua
        return request

    def _coerce_client_secret_post(self) -> None:
        """Same rule as ``HbmTokenStorage._coerce_secret_auth_method``, applied to the
        ...
        if HbmTokenStorage._coerce_secret_auth_method(data):
    ...
    # _hbm_release_refresh_fence. Never shared across instances.
    _hbm_fence: int | None = None
    ...
    async def async_auth_flow(self, request):
        ...
        while True:
            ...
            try:
                ...
                await self._hbm_release_refresh_fence()
    ...
    async def _refresh_token(self):
        ...
        """
        self._coerce_client_secret_post()
        await self._hbm_acquire_refresh_fence()
        try:
            ...
            candidate = await self._hbm_rotated_candidate()
            if candidate is not None:
                if not self._hbm_install_disk_pair(candidate):
                ...
                if self._hbm_live_ttl() and self.context.is_token_valid():
    ...
    async def _hbm_acquire_refresh_fence(self) -> None:
        ...
        if tokens_path is None:  # pragma: no cover - non-HBM AGENT storage
            return
        self._hbm_fence = await acquire_refresh_fence(tokens_path())

    async def _hbm_release_refresh_fence(self) -> None:
        """Release the fence if held. Idempotent and never raises."""
        from tools.mcp_oauth import release_refresh_fence

        fd, self._hbm_fence = self._hbm_fence, None
        if fd is not None:
            release_refresh_fence(fd)

    async def _hbm_rotated_candidate(self):
    ...
    def _hbm_install_disk_pair(self, tokens) -> bool:
    ...
    async def _initialize(self) -> None:
        ...
        storage = self.context.storage
        from tools.mcp_oauth import HbmTokenStorage
        if isinstance(storage, HbmTokenStorage) and self.context.oauth_metadata is None:
    ...
    async def _handle_refresh_response(self, response) -> bool:
        ...
        """
        try:
            return await self._hbm_handle_refresh_response(response)
        finally:
            await self._hbm_release_refresh_fence()
    ...
    async def _hbm_handle_refresh_response(self, response) -> bool:
        if not (200 <= response.status_code < 300):
            ...
            if await self._hbm_reload_tokens_after_refresh_failure():
                self._hbm_logger.info(
        ...
        try:
            ...
            self._hbm_logger.warning("Invalid refresh response: %s", response.status_code)
            self.context.clear_tokens()
            return False
    ...
    async def _hbm_reload_tokens_after_refresh_failure(self) -> bool:
        ...
        if (
            self._hbm_install_disk_pair(candidate)
            and self._hbm_live_ttl()
...
def bind_issuer_from_context(context: Any) -> None:
    ...
    it. No-op when metadata is not discovered yet or storage is not HBM AGENT'."""
    from tools.mcp_oauth import HbmTokenStorage
    storage = getattr(context, "storage", None)
    issuer = _metadata_issuer(context)
    if isinstance(storage, HbmTokenStorage) and issuer:
...
def enforce_refresh_token_issuer(context: Any) -> None:
    ...
    if not isinstance(storage, HbmTokenStorage) or tokens is None or not getattr(tokens, "refresh_token", None):
        return
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "HbmTokenStorage"]:
    ...
    return cfg, mo.HbmTokenStorage(server_name)


def build_provider_kwargs(cfg: dict, storage: "HbmTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
