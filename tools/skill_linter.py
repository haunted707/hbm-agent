...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    hbm_meta = meta.get("hbm") if isinstance(meta, dict) else None
    if not isinstance(hbm_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.hbm.{tags, related_skills}.")
    elif "tags" not in hbm_meta:
        yield _warn("missing-metadata", "metadata.hbm.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("hbm", "agent", "hbm agent") and (
        author != "HBM AGENT"):
        yield _warn("author-caps", f"author '{author}' should be 'HBM AGENT' (proper caps) "
