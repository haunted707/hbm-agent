...
The LLM writes a Python script that calls HBM AGENT tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform HBM AGENT does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_hbm_dir("cache/exec", "exec_spill")
...
def _missing_hbm_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
    if missing in {"json_parse", "shell_quote", "retry"}:
        return (f"Import helpers with `from hbm_tools import {missing}`. "
                "If that import failed, the generated module may be stale or another "
                "hbm_tools may be first on sys.path. Check hbm_tools.__file__ "
...
def generate_hbm_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """Source of the hbm_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
"""Auto-generated HBM AGENT tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["HBM_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("HBM_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (HBM_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("HBM_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated HBM AGENT tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("HBM_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hbm_rpc")
_seq = 0
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage hbm_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/hbm_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/hbm_tools.py",
                             generate_hbm_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"HBM_RPC_DIR={quoted_rpc_dir} HBM_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (hbm-agent#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of HBM AGENT tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.hbm.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.hbm/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "HBM AGENT's own python (the common case — stdlib plus HBM AGENT's "
    ...
    description = (
        "Run Python that calls HBM AGENT tools programmatically. Use when you "
        ...
        f"Available via `from hbm_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        f"{cwd_note}\n\n"
        "Helpers require imports: `from hbm_tools import json_parse, shell_quote, retry`. "
        ...
        "parameters": {
            ...
            "properties": {
                "code": {"type": "string", "description": (
                    ...
                    f"`from hbm_tools import {import_str}` "
