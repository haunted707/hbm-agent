"""Skills Hub official sources: repo-shipped optional skills and the centralized HBM AGENT index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _hbm_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    ...
    activated by default — absent from the system prompt and not copied to ~/.hbm/skills/ at
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.hbm.upstream); install pulls the live content from there.
        try:
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.hbm.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        hbm_meta = meta_block.get("hbm") if isinstance(meta_block, dict) else None
        upstream = hbm_meta.get("upstream") if isinstance(hbm_meta, dict) else None
...
class HbmIndexSource(SkillSource):
    """Skill source backed by the centralized HBM AGENT Skills Index: a JSON catalog on the docs site,
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_hbm_index
            self._index, self._loaded = _load_hbm_index(), True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "hbm-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "hbm-index"), identifier=entry.get("identifier", ""),
