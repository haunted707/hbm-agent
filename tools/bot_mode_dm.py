...
# delivery runner (``python bot_mode_dm.py --run-delivery …``); HBM AGENT helpers import lazily.
...
def _default_home() -> str:
    ...
    return str(get_process_hbm_home())
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'hbm' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('hbm' → 'default')."""
...
def message_agent_tool(target: str = "", message: str = "", task_id: Optional[str] = None, agent: Any = None) -> str:
    ...
    try:
        from tools.bot_mode_probe import (
            BOT_CHAT_TITLE, _handle, _hbm_root, _peers, _profile_name as _self_profile_name, _roster,
            is_bot_mode_managed,
        )
        from tools.bot_relay import BOT_CHAT_TURN_ARGS, _hbm_cli
    ...
    root, me = _hbm_root(Path(home)), _self_profile_name(Path(home))
    ...
    if peer_match or raw_target.lower() in peers:
        ...
        # venv bin dir, so a bare "hbm" resolves to a system install and dies on import
        # under the wrong interpreter (#108628). _hbm_cli pins the entrypoint beside
        # this interpreter; _delivery_lock/_local_delivery_home match argv[0] by basename,
        # so the absolute path stays compatible.
        return _start_delivery([_hbm_cli(), "-p", _self_profile_name(root), "peer", "dm", dm_target], content,
    ...
    return _start_delivery([_hbm_cli(), "-p", resolved, *BOT_CHAT_TURN_ARGS], content, f"@{_handle(resolved)}",
...
def cleanup_bot_dm_cache(max_age_hours: float = _DM_STALE_SECONDS / 3600, *, now: float | None = None) -> int:
    ...
    locations = [(temp_root, "hbm-dm-*.txt"), (temp_root, "hbm-relay-dm-*.txt")]
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    if stdin_file or len(argv) < 3 or cli not in ("hbm-agent", "hbm-agent.exe") or argv[1] != "-p":
        return contextlib.nullcontext()
    from tools.bot_mode_probe import _hbm_root
    from tools.bot_relay import acquire_turn_lock

    return acquire_turn_lock(_hbm_root(Path(_default_home())), argv[2])
...
def _run_local_turn(argv: list[str], dm_file: str, *, env: Optional[dict[str, str]] = None) -> int:
    ...
    reason = next((line.removeprefix("hbm-refusal-reason: ").strip()
                   for line in stderr_text.splitlines()
                   if line.startswith("hbm-refusal-reason: ")), None)
...
def _local_delivery_home(argv: list[str]) -> Path | None:
    cli = (argv[0] if argv else "").rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    if len(argv) < 3 or cli not in ("hbm-agent", "hbm-agent.exe") or argv[1] != "-p":
        return None
    from tools.bot_mode_probe import _hbm_root, _roster

    return dict(_roster(_hbm_root(Path(_default_home())))).get(argv[2])
