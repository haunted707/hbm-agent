...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        ...
        if get_hbm_home_override() is not None:
            lock_path = str(get_hbm_home() / ".mcp-discovery.lock")
        else:
            if _origin._MCP_DISCOVERY_LOCK_PATH is None:
                _origin._MCP_DISCOVERY_LOCK_PATH = str(get_hbm_home() / ".mcp-discovery.lock")
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local HBM_HOME override into ``coro`` (task-local on the MCP
    loop, so concurrent scopes don't interfere)."""
    try:
        from hbm_constants import get_hbm_home_override, reset_hbm_home_override, set_hbm_home_override
        home_override = get_hbm_home_override()
    except Exception:
        home_override = None
    ...
    async def _scoped():
        token = set_hbm_home_override(home_override)
        try:
            return await coro
        finally:
