...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_hbm_home() / "auth.json"
...
def hbm_xai_user_agent() -> str:
    """Return a stable HBM AGENT-specific User-Agent for xAI HTTP calls."""
    try:
        from hbm_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"HBM AGENT-Agent/{__version__}"


def hbm_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": hbm_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per HBM AGENT home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from hbm_constants import get_hbm_home
        marker_dir = get_hbm_home() / "state"
...
def _xai_base_url_override() -> str:
    """``HBM_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    from hbm_cli.config import get_env_value
    return str(get_env_value("HBM_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: HBM AGENT-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.hbm/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``HBM_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers HBM AGENT-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``hbm_cli.config.get_env_value`` so keys stored in ``~/.hbm/.env`` (the standard HBM AGENT
    ...
    scoping is identical to the fallback branch, and the base URL honors ``HBM_XAI_BASE_URL`` /
