...
# startup notice and `hbm-agent doctor` read it through terminal_backend_unavailable_reason() so the user
...
def _build_docker_env(*, image, cwd, timeout, cc, task_id, host_cwd, **_):
    ...
    # One-shot reaper for labeled containers orphaned by prior HBM AGENT processes that died before
...
def _build_plugin_env(*, env_type, image, cwd, timeout, cc, task_id, **_):
    ...
    if provider is not None:
        ...
        try:
            env_obj._hbm_backend_name = provider.name.strip().lower()
        except AttributeError:
            pass
        return env_obj
    try:
...
def _ssh_pre(config: Dict[str, Any]) -> bool:
    ...
    return _reject("the SSH host and user are not configured (TERMINAL_SSH_HOST / TERMINAL_SSH_USER); "
                   "run `hbm-agent setup terminal` to enter them or pick the 'local' backend")
