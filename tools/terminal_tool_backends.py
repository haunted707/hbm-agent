...
# startup notice and `hbm doctor` read it through terminal_backend_unavailable_reason() so the user
...
def _ssh_pre(config: Dict[str, Any]) -> bool:
    ...
    return _reject("the SSH host and user are not configured (TERMINAL_SSH_HOST / TERMINAL_SSH_USER); "
                   "run `hbm setup terminal` to enter them or pick the 'local' backend")
