...
NOT_REGULAR_SENTINEL = "__hbm_not_regular__"
...
MISSING_SENTINEL = "__hbm_missing__"

_READ_SENTINEL_PREFIX = "__HBM_RF_"
_WRITE_SENTINEL_PREFIX = "__HBM_WF_"
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        understand the MSYS ``/c/...`` form and HBM AGENT disables MSYS argument
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        tmpl = self._escape_shell_arg(".hbm-tmp.XXXXXX")
        script = (
            "set -e; "
            ...
            '|| mktemp "$d/.hbm-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.hbm-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        marker = stdout.find("HBM_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("HBM_UTF16:OK"):].strip()
        try:
    ...
    def _native_read_enabled(self) -> bool:
        ...
        file_operations holds Git-Bash-style paths there). ``HBM_NATIVE_FILE_READ=0``
        turns the fast path off."""
        flag = os.environ.get("HBM_NATIVE_FILE_READ", "1").strip().lower()
