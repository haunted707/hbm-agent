...
def _get_default_output_dir() -> str:
    ...
    return str(get_hbm_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    HBM_HOME and later scope requests to a different profile via
    ``hbm_constants.set_hbm_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped HBM_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("hbm_cli.config not available, using default TTS config")
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("HBM_SESSION_PLATFORM", "").lower()
...
def _tts_schema_overrides() -> dict:
    ...
    params["properties"]["output_path"]["description"] = _output_path_description(display_hbm_home())
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": _output_path_description("the profile HBM_HOME")
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'hbm_xai_user_agent': ('tools.xai_http', 'hbm_xai_user_agent'),
