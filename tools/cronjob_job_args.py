...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("HBM_SESSION_PLATFORM")
    origin_chat_id = get_session_env("HBM_SESSION_CHAT_ID")
    if not (origin_platform and origin_chat_id):
        return None
    thread_id = get_session_env("HBM_SESSION_THREAD_ID") or None
    ...
    if thread_id and origin_platform == "slack":
        message_id = get_session_env("HBM_SESSION_MESSAGE_ID") or None
    ...
    return {
        ...
        "chat_name": get_session_env("HBM_SESSION_CHAT_NAME") or None, "thread_id": thread_id,
        # Lets a delivery mirror resolve the participant's session in per-user-isolated groups.
        "user_id": get_session_env("HBM_SESSION_USER_ID") or None,
        ...
        "scope_id": get_session_env("HBM_SESSION_SCOPE_ID") or None,
    }
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    TUI/CLI sessions cannot be captured as a cron ``origin`` (no ``HBM_SESSION_PLATFORM``/``CHAT_ID`` is
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    from HBM_CRON_AUTO_DELIVER_*, or ``local`` when it has no concrete target; other
    elements pass through. Otherwise the scheduler would guess a home channel."""
    from gateway.session_context import get_session_env
    from utils import is_truthy_value
    if not is_truthy_value(get_session_env("HBM_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("HBM_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("HBM_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("HBM_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    """Scripts must be relative paths within HBM_HOME/scripts/ (absolute / ~ / drive-letter
    ...
    raw = script.strip()
    scripts_dir = get_hbm_home() / "scripts"
...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``hbm_cli.cron._builtin_gateway_liveness`` (#87033) so the
    ...
    """
    try:
        from hbm_cli.cron import _builtin_gateway_liveness
