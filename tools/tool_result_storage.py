...
is ALWAYS host-side ``$HBM_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $HBM_HOME/cache/spillover as a Path (not created)."""
    from hbm_constants import get_hbm_home
    return get_hbm_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $HBM_HOME/cache/spillover; returns path str or None.
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/hbm-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "HBM_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
