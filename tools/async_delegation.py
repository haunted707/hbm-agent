...
def _connect() -> sqlite3.Connection:
    from hbm_cli.sqlite_util import open_db
    # Same state.db as hbm_state.SessionDB -- reuse its owner-only (0600)
    # hardening so this writer doesn't create/leave the file (and its WAL
    # sidecars) at the process umask. See hbm_state._secure_state_db_files.
    from hbm_state import _secure_state_db_files
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from hbm_state_repair import apply_durability_barriers
    from hbm_state_schema import reconcile_state_schema
...
def _transaction():
    from hbm_cli.sqlite_util import transaction
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {k: v for k in _ROUTING_KEYS if (v := get_session_env(f"HBM_SESSION_{k.upper()}", ""))}
...
def _current_origin_session_id() -> str:
    """Raw session id of the ORIGINATING api_server request, or ``""``. ``HBM_SESSION_ID``
    ...
    request-scoped ``HBM_SESSION_CHAT_ID`` (raw X-HBM AGENT-Session-Id on api_server) survives
    child construction; on push platforms chat_id is a chat, not a session => ``""``."""
    try:
        from gateway.session_context import get_session_env
        is_api = get_session_env("HBM_SESSION_PLATFORM", "") == "api_server"
        return (get_session_env("HBM_SESSION_CHAT_ID", "") or "") if is_api else ""
    except Exception:
        return ""
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    try:
        # Propagate the dispatching profile so the detached child resolves get_hbm_home() correctly.
