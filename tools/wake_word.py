"""Wake-word ("Hey HBM AGENT") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey hbm" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey hbm", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled "hey hbm" model (tools/wakewords/) — the default; alias names resolve
# to it, not to an openWakeWord built-in.
_BUNDLED_MODEL_NAME = "hey_hbm"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_hbm", "hey hbm", "hbm"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_hbm model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey hbm"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from hbm_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the HBM AGENT backend "
...
# ── Process-wide singleton (mirrors hbm_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_hbm_root() / "runtime" / "wake-word.lock"
