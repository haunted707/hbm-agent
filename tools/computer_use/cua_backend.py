...
def sanitized_cua_driver_env() -> Dict[str, str]:
    """``cua_driver_child_env()`` with HBM AGENT provider secrets stripped — cua-driver is a third-party binary and must
...
def _maybe_repair_runtime_contract(contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    attempted / it failed). Never raises. An explicit ``HBM_CUA_DRIVER_CMD`` override is authoritative even
...
class CuaDriverBackend(_CaptureMixin, _InputMixin, ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Public session label (one per HBM AGENT run) sent as `session` on every call: owns the cursor color and
        # gives config/recording state a stable owner across transport restarts. Part of the 0.20 runtime contract.
        self._session_id: str = f"hbm-{uuid.uuid4().hex[:12]}"
