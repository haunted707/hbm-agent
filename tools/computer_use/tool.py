...
# Hard-blocked regardless of approval level (e.g. logout kills the session HBM AGENT runs in). Alt is
...
# Per-HBM AGENT-session cached backends (own cua-driver session, native target, refs, grant namespace).
...
def _cua_permission_mode(session_id: str) -> str:
    """Map HBM AGENT's approval bypass onto Cua's immutable mode; fails closed. Both identity namespaces are consulted
...
def _new_backend(permission_mode: str) -> ComputerUseBackend:
    backend_name = os.environ.get("HBM_COMPUTER_USE_BACKEND", "cua").lower()
    if backend_name in {"cua", "cua-driver", ""}:
        from tools.computer_use.cua_backend import CuaDriverBackend
        return CuaDriverBackend(permission_mode=permission_mode)
    if backend_name != "noop":
        raise RuntimeError(f"Unknown HBM_COMPUTER_USE_BACKEND={backend_name!r}")
...
def _scoped_sid(session_id: str) -> str:
    """Cache key for one HBM AGENT session's backend. Outside a served-profile scope it is the bare id
    ...
    return sid if get_hbm_home_override() is None else f"{sid}@{hbm_home_key()}"
...
def _get_backend(session_id: str = "") -> ComputerUseBackend:
    ...
    while True:
        with _backend_lock:
            ...
            permission_mode = _cua_permission_mode(bare_sid)  # approval state is keyed by the HBM AGENT session id
...
def _shutdown_backend_atexit() -> None:
    ...
    the HBM AGENT process that spawned it (#28152 item 3). #69903 kept the orphan from burning a core by
...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    """Test/CI stub (HBM_COMPUTER_USE_BACKEND=noop). Records ``(name, kwargs)`` calls; returns trivial results."""
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$HBM_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from hbm_constants import get_hbm_dir  # lazy so tests can patch get_hbm_dir
    cache_dir = get_hbm_dir(subdir, legacy)
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Copy of the capture in HBM AGENT' media cache so attachment surfaces can deliver it (None without an image)."""
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        if (cached := _AUX_VISION_ROUTE_CACHE.get(key := (hbm_home_key(), str(provider), str(model)))) is not None:
            return cached
        stage = "decision"
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$HBM_HOME/cache/vision/``) and merge
