...
def cua_driver_install_hint() -> str:
    ...
    return ("cua-driver is not installed. Install with one of:\n  hbm computer-use install\n"
            ...
            "Or run `hbm tools` and enable the Computer Use toolset to install it automatically.")
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (f"cua-driver {state.get('latest_version') or '?'} is available "
            f"(you have {state.get('current_version') or '?'}); update with `hbm computer-use install --upgrade`.")
