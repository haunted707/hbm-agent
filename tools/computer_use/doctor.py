...
def _build_identity(binary: str, report: Report) -> Report:
    """HBM AGENT-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(driver_cmd: Optional[str] = None, *, include: Sequence[str] = (), skip: Sequence[str] = (), json_output: bool = False,
    ...
    if json_output:
        # Additive envelope: upstream keys preserved, identity under hbm_identity (and environment under
        # hbm_environment when present) so overall/checks parsers keep working.
        payload = {**report, "hbm_identity": identity}
        if environment:
            payload["hbm_environment"] = environment
