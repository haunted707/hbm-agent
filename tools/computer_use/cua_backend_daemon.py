...
class _EmbeddedCuaDaemon:
    ...
    startup, so reusing the machine-wide daemon would let one HBM AGENT session's YOLO choice affect another. A
    ...
    HBM AGENT YOLO (``--dangerously-bypass-approvals``); ``bounded`` = a user-reviewed capability manifest approved
    ...
    def __init__(self, driver_cmd: str, permission_mode: str, capability_manifest: Optional[str] = None) -> None:
        ...
        self.socket_path = (rf"\\.\pipe\hbm-cua-{token}" if sys.platform == "win32"
    ...
    def start(self) -> None:
        ...
        threading.Thread(target=self._drain_stderr, args=(self._process,), name="hbm-cua-daemon-stderr", daemon=True).start()
