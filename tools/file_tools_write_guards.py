...
# resolve per call in production (below) because ``get_hbm_home()`` /
...
_hbm_config_resolved: str | None = None
_hbm_config_resolved_loaded = False
_real_hbm_home_cached: str | None = None
_real_hbm_home_loaded = False
...
def _hbm_home_real() -> str:
    from hbm_constants import get_hbm_home
    return os.path.realpath(str(get_hbm_home()))


def _get_hbm_config_resolved() -> str | None:
    """Resolved absolute path of the HBM AGENT config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``HBM_HOME`` scope (#107327);
    a test may pin it via ``_hbm_config_resolved`` + ``_hbm_config_resolved_loaded``."""
    if _hbm_config_resolved_loaded:
        return _hbm_config_resolved
    try:
        ...
        # home rather than the active multiplex ``HBM_HOME`` — comparing
        ...
        # ``get_hbm_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from hbm_constants import get_hbm_home
            return str((Path(str(get_hbm_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_hbm_home() -> str | None:
    """Realpath of the authoritative HBM AGENT home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``HBM_HOME`` scope (#107327);
    a test may pin it via ``_real_hbm_home_cached`` + ``_real_hbm_home_loaded``.
    Consumers exempting a whole TREE want ``_hbm_exempt_homes()``: under a named
    profile this home is ``<root>/profiles/<name>`` and the root is exempt too."""
    if _real_hbm_home_loaded:
        return _real_hbm_home_cached
    try:
        return _hbm_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_hbm_config_resolved``): re-derive from ``get_hbm_home()``
        # rather than ``_expand_tilde("~/.hbm")`` so the protected-instruction
        ...
        # the consumer: the ``~/.hbm`` exemption is skipped, so the gate runs.
        try:
            from hbm_constants import get_hbm_home
            return os.path.realpath(str(get_hbm_home()))
        except Exception:
...
def _hbm_exempt_homes() -> tuple[str, ...]:
    """Realpaths of the HBM AGENT home tree(s) the protected-instruction gate must stay out of:
    the ACTIVE profile's home, plus the HBM AGENT ROOT when that home is a named profile
    (``<root>/profiles/<name>``). Exempting only the profile dir left the root's DIRECT files
    (LEDGER.md / MEMORY.md / SOUL.md / AGENTS.md ...) to the ``.hbm`` component rule, which
    gated them like a project-local ``<repo>/.hbm/config.yaml`` — fail-closed headless
    (#110630). They are the agent's own store, governed by their own guards, exactly like
    ``~/.hbm`` under the default profile. The root is added only when the shape really is a
    named profile (``named_profile_home``), so a coincidental ``profiles/`` dir elsewhere never
    exempts its parent; the home comes from the ACTIVE scope, never ``HBM_HOME`` alone."""
    home = _get_real_hbm_home()
    if not home:
        return ()
    try:
        from hbm_constants import named_profile_home
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    hbm_config = _get_hbm_config_resolved()
    if hbm_config and hbm_config in candidates:
        return (
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .hbm tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from hbm_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.hbm itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.hbm`` component rule, which would match the home.
    # ``_hbm_exempt_homes`` also covers the ROOT when the active home is a named
    # profile, so ~/.hbm/<file> cannot read as project-local ``.hbm`` config.
    for real_home in _hbm_exempt_homes():
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .hbm config dirs (<repo>/.hbm/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.hbm.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".hbm":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side HBM AGENT mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.hbm" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    HBM AGENT state (a write the host never reads). Not profile isolation — that guard was
