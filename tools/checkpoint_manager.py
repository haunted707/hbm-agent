...
config / ``--checkpoints`` flag.  One store under ``~/.hbm/checkpoints/`` so git dedupes
blobs across projects (pre-v2 one-repo-per-workdir re-stored ~40 MB each): ``store/`` bare
repo with per-project ``refs/hbm/<hash16>``, ``indexes/<hash16>``, ``projects/<hash16>.json``
...
CHECKPOINT_BASE = get_hbm_home() / "checkpoints"
...
def _resolve_checkpoint_base() -> Path:
    ...
    changed it, else live profile-scoped HBM_HOME — under the multiplexed gateway one process
    ...
    return CHECKPOINT_BASE if CHECKPOINT_BASE != _CHECKPOINT_BASE_AT_IMPORT else get_hbm_home() / "checkpoints"

_STORE_DIRNAME, _INDEXES_DIRNAME, _PROJECTS_DIRNAME, _LEDGERS_DIRNAME = "store", "indexes", "projects", "ledgers"
_REFS_PREFIX, _LEGACY_PREFIX, _PRUNE_MARKER_NAME = "refs/hbm", "legacy-", ".last_prune"
...
DEFAULT_EXCLUDES = [
    ...
    ".git/", ".hg/", ".svn/", ".worktrees/",  # VCS + worktrees (HBM AGENT convention — don't snapshot siblings)
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("HBM_CHECKPOINT_TIMEOUT", 30)))
...
_STORE_GIT_CONFIG = (("user.email", "hbm@local"), ("user.name", "HBM AGENT Checkpoint"),
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file``/``patch`` produced, so restores can tell HBM AGENT' writes from later user edits."""
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir() if base.exists() else ():
        ...
        try:
            if (child / "HBM_WORKDIR").exists():
                workdir = (child / "HBM_WORKDIR").read_text(encoding="utf-8").strip()
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file HBM AGENT just wrote (agent-write ledger), so safe-mode
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        """Classify files changed since ``commit_hash``: ``restore`` = still matching what HBM AGENT
        last wrote (or deleted since); ``skipped`` = user-edited afterwards or never written by
        HBM AGENT.  ``ledger_empty`` => no ledger, callers fall back to a full restore."""
        ...
        for rel in filter(None, names_out.split("\x00")):
            ...
            hbm_authored = recorded is not None and _hash_file(abs_path) in (None, recorded)
            out["restore" if hbm_authored else "skipped"].append(rel)
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None,
        ...
        the user hand-edited after HBM AGENT' last write untouched (agent-write ledger); the result
    ...
    def _apply_safe_restore_deletes(self, p: _ProjectRefs, commit_hash: str,
        ...
        from the checkpoint is HBM AGENT-created (delete to restore) — unless ``max_file_size_mb`` kept
