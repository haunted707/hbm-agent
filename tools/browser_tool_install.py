...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    local node_modules binary, which vanishes on the next ``hbm update`` and leaves a dangling link ``which``
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    """Best-effort pre-fetch of the agent-browser npm package via npx (``hbm update`` / ``doctor --fix``).
    ...
    registry-lookup/fetch cost. Calling this during ``hbm update`` (or ``hbm doctor --fix``) warms
