...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(hbm_home).expanduser() if hbm_home else Path.home() / ".hbm") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite HBM AGENT's live source checkout "
        ...
        "is pushed. To change this checkout, stop HBM AGENT, run the command externally, then restart "
        "HBM AGENT.")
