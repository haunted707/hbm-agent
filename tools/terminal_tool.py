...
processes survive cleanup, idle reaping, or HBM AGENT exit.
...
def _current_session_key() -> str:
    ...
    return get_session_env("HBM_SESSION_KEY", "")


def _current_session_profile() -> str:
    """Active session's HBM AGENT profile name, or "" (same lookup discipline as
    :func:`_current_session_key`)."""
    from gateway.session_context import get_session_env

    return get_session_env("HBM_SESSION_PROFILE", "")
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``hbm-agent=1`` for the current
    profile — leftovers of HBM AGENT processes that died without firing
    ...
    HBM AGENT processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    context-local ``HBM_HOME`` override (multiplexed dashboard / gateway
    ...
    with _quiet("terminal config → env fallback bridge failed"):
        from hbm_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    from hbm_cli.config import _is_ssh_remote_tilde_cwd
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for HBM AGENT-owned control-plane
