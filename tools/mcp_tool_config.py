...
def _get_mcp_stderr_log() -> Any:
    ...
    from hbm_constants import get_hbm_home, hbm_home_key, mkdir_under_hbm_home
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from hbm_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("HBM_KANBAN_DB", "HBM_KANBAN_BOARD"):
...
def _node_fallback(command: str, *, windows: Optional[bool] = None) -> str:
    ...
    The managed tree comes from ``iter_hbm_node_dirs`` (Windows unpacks into ``<home>\\node``, POSIX into
    ``<home>/node/bin``) under the active profile's ``get_hbm_home()``; on Windows the real files are
    ...
    # /usr/local/bin: canonical Node location (from-source Linux, HBM AGENT Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    directories = [*map(str, iter_hbm_node_dirs(get_hbm_home())), os.path.join(home, ".local", "bin"),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``hbm_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); HBM AGENT already supervises the
