...
For custom/local-only tools do NOT edit core: create `~/.hbm/plugins/<name>/plugin.yaml` +
...
1. `tools/your_tool.py`:
   ...
   ```
2. `toolsets.py`: add the name to `_HBM_CORE_TOOLS` (all platforms) or a new toolset. **Required**
   — discovery registers the schema, but a tool is only exposed if a toolset names it.
   `_HBM_CORE_TOOLS` is the default bundle every platform's base toolset inherits, not dead code.
...
- **Paths in schema descriptions use `display_hbm_home()`** (schema is built at import; under
  multiplex it shows the launch home, which is display-only). **State files use `get_hbm_home()`**
  at call time, never `Path.home()/.hbm` and never a module constant, so each served profile gets
  its own state.
...
- **`check_fn` answers reachability/opt-in for the profile it runs under, never surface.** Results
  are TTL-cached in `registry.py::_check_fn_cache` keyed by `hbm_home_key()`, and one process
...
imports against a temp `HBM_HOME` (see `tests/tools/test_approval_config_readonly.py`).
