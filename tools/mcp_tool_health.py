...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into HBM AGENT logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into HBM AGENT' logging
        (agent.log via hbm_logging), tagged with the server name. Without this, the SDK's default
