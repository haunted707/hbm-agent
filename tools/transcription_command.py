...
# "No STT provider available". The single-env-var HBM_LOCAL_STT_COMMAND escape
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"hbm-cmd-stt-{provider_name}-") as tmpdir:
...
def _apply_pre_transcription_hook(
    *, file_path: str, provider: str, model: Optional[str], language: Optional[str],
    ...
    try:
        from hbm_cli.plugins import has_hook, invoke_hook
