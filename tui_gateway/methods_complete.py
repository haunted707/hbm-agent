...
def _profile_mention_items(prefix: str) -> list[dict]:
    ...
    is also offered as 'hbm' when no real profile claims that name."""
    out: list[dict] = []
    try:
        ...
        if "hbm".startswith(prefix.lower()) and "hbm" not in seen:
            out.append(_item("@hbm", "agent profile (primary)"))
    except Exception:
        return []
    return out
...
def _(rid, params: dict) -> dict:
    ...
    if not text.startswith("/"):
        return _ok(rid, {"items": []})
    from hbm_cli.commands_completion import SlashCommandCompleter
...
@_catch(5033)
def _(rid, params: dict) -> dict:
    from hbm_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    if pconfig.auth_type != "api_key":
        return _err(rid, 4003, f"{pconfig.name} uses {pconfig.auth_type} auth — run `hbm model` to configure")
    if not pconfig.api_key_env_vars:
        return _err(rid, 4004, f"no env var defined for {pconfig.name}")
    # Save the key to ~/.hbm/.env via the unified credential lifecycle so any stale config.yaml mirror of
    # the previous key (model.api_key, custom_providers[*].api_key) is rotated in the same action (#62269).
    env_var = pconfig.api_key_env_vars[0]
    from hbm_cli.credential_lifecycle import save_provider_env_credential  # also rotates stale config.yaml mirrors
...
def _(rid, params: dict) -> dict:
    ...
    from hbm_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
