...
# name; under ``HBM_TEST_ISOLATION`` (set by ``scripts/run_tests.sh`` / ``tests/conftest.py``) it
# raises, which is what makes the suite the gate.

STRICT = bool(os.environ.get("HBM_TEST_ISOLATION"))
        ...
        for err in exc.errors():
            if err.get("type") == "extra_forbidden":
                ...
                return None, (f"invalid params for {contract.name}: {loc}: {err.get('msg')} — the client and "
                              "the HBM AGENT backend are out of sync (different versions); run `hbm update` "
                              "and restart both")
    return params, None
