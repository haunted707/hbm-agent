...
class ConfigProviderRef(OpenModel):
    """``hbm_cli/models.py::list_available_providers`` row."""
...
class ModelPricing(Result):
    """``hbm_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
...
class ModelCapabilities(Result):
    """``hbm_cli/inventory.py::_apply_capabilities``."""
...
class ModelOptionProvider(OpenModel):
    """One ``hbm_cli/inventory.py::build_models_payload`` provider row (the union of every field
...
class GoalContractSnapshot(Result):
    """``hbm_cli/goals.py::GoalContract.to_dict``."""
