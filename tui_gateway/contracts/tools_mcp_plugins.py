...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.hbm/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class SkillBrowseItem(OpenModel):
    """``hbm_cli.skills_hub.browse_skills`` row."""
...
class SkillInspectInfo(OpenModel):
    """``hbm_cli.skills_hub.inspect_skill``; ``{}`` when the identifier resolves nowhere."""
...
class PluginsManageResult(Result):
    ...
    ``install`` → ``hbm_cli.plugins_cmd.dashboard_install_plugin``'s ok payload; ``update`` →
    ``ok``/``unchanged``/``sha``."""
