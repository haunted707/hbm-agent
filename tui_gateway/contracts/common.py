...
class OpenModel(Result):
    ...
    (the closed set is owned elsewhere: hbm_state rows, provider inventories)."""
...
class StoredSessionRow(OpenModel):
    """One ``sessions`` row as ``hbm_state`` lists it (``session.list`` / ``session.info`` rows /
    ``sessions.changed``)."""
...
class MessageReaction(OpenModel):
    """One persisted reaction row (``hbm_state_messages.set_message_reaction``); ``seen`` is
