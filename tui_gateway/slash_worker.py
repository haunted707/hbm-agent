"""Persistent slash-command worker — one HbmCLI per TUI session.

Protocol: reads JSON lines from stdin {id, command}, writes {id, ok, output|error} to stdout.
"""

# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory from shadowing HBM AGENT's own
...
# retry loop. ``hbm_bootstrap`` lives at the repo root (no collision risk), so importing it first is safe.
# ``hbm_bootstrap`` lives at the repo root, so importing it is safe before the guard runs (its name won't
...
_WATCHDOG_POLL_S = max(0.05, env_float("HBM_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, env_float("HBM_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before HbmCLI snapshots tools: each slash_worker child is its
    own process — the parent ``hbm serve`` discovery thread does not populate this registry.

    See #61891.
    """
    from hbm_cli.mcp_startup import start_background_mcp_discovery, wait_for_mcp_discovery
...
def _run(cli: HbmCLI, command: str) -> str:
...
def main():
    ...
    args = p.parse_args()
    os.environ["HBM_SESSION_KEY"] = args.session_key
    os.environ["HBM_INTERACTIVE"] = "1"
    # Start before the (hundreds-of-ms) HbmCLI build — that window is itself an orphan risk if the
    # gateway dies mid-spawn.
    _start_parent_death_watchdog(os.getppid())
    _prepare_slash_worker_runtime()
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = HbmCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
