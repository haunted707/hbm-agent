...
class HostSupervisor:
    ...
    def __init__(
        ...
        expected_hbm_home: str | None = None, autostart: bool = True) -> None:
        self.registry_path = (
            Path(registry_path) if registry_path is not None
            else get_hbm_home() / "state" / _REGISTRY_NAME)
        ...
        self.expected_hbm_home = (
            str(get_hbm_home()) if expected_hbm_home is None else expected_hbm_home)
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        self._hello = {}
        env = {**hbm_subprocess_env(inherit_credentials=True), **os.environ, **(self.env or {})}
        env["HBM_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
        root = str(_repo_root())
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("hbm_home") or "")
        if got_home and got_home != self.expected_hbm_home:
            raise RuntimeError(
                f"compute host HBM_HOME mismatch: {got_home} != {self.expected_hbm_home}")
