...
def resolve_skin() -> dict:
    try:
        from hbm_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _watcher_home() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _hbm_home)
...
def _ensure_skin_watcher() -> None:
    ...
    threading.Thread(target=_loop, name="hbm-change-watcher", daemon=True).start()
