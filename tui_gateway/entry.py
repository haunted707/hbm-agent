...
# Stop a ``utils/``-style package in the launch directory from shadowing HBM AGENT's own
# top-level modules; ``hbm_bootstrap``'s name can't collide, so importing it first is safe.
import hbm_bootstrap

hbm_bootstrap.harden_import_path()
...
# hbm_cli.mcp_startup (current path). The wait/in-flight/join helpers consult both.
...
def _install_sidecar_publisher() -> None:
    ...
    url = os.environ.get("HBM_TUI_SIDECAR_URL")
    if not url:
        return
...
# strand the process; ``HBM_TUI_GATEWAY_SHUTDOWN_GRACE_S`` overrides.
_DEFAULT_SHUTDOWN_GRACE_S = 1.0


def _shutdown_grace_seconds() -> float:
    value = env_float("HBM_TUI_GATEWAY_SHUTDOWN_GRACE_S", _DEFAULT_SHUTDOWN_GRACE_S)
    return value if value > 0 else _DEFAULT_SHUTDOWN_GRACE_S


def _mcp_startup_call(name: str, *args, default=None, log=None, **kwargs):
    """Call ``hbm_cli.mcp_startup.<name>`` (lazy import); ``default`` on any failure,
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # Discovery is spawned via the shared owner (ensure_mcp_discovery_started → hbm_cli.mcp_startup);
    ...
    # CALLER's profile context (agent build binds the session profile's HBM_HOME first), so a launch
...
def mcp_discovery_in_flight() -> bool:
    ...
    There are two independent discovery-thread owners by surface: the stdio ``hbm --tui`` path spawns ITS
    thread here (``_mcp_discovery_thread``), while the desktop app + dashboard WebSocket sidecar
    (``tui_gateway/ws.py``) and ``hbm dashboard`` spawn theirs via
    ``hbm_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh scheduler imports this
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    ``hbm_cli.mcp_startup`` thread used by the desktop/dashboard surfaces. See #51587.
    """
    entry_done = True
    thread = _mcp_discovery_thread
...
def _has_configured_mcp_servers() -> bool:
    ...
    from hbm_cli.mcp_startup import _has_configured_mcp_servers as configured
    return configured()
...
def ensure_mcp_discovery_started() -> None:
    ...
    session profile's HBM_HOME.

    WebSocket/Desktop entrypoints can accept sessions without running ``main()``, so the agent-build path
    (``server._start_agent_build``) also calls it AFTER binding the session profile's HBM_HOME override —
    the shared owner in ``hbm_cli.mcp_startup`` captures the caller's context-local override and
    propagates it into the discovery thread, so discovery reads the SELECTED profile's ``mcp_servers``, not
    the launch profile's. The discovery slot in ``hbm_cli.mcp_startup`` is keyed by profile home, so
...
def main():
    ...
    # Live-apply skins HBM AGENT activates mid-conversation.
