...
def _relay_root() -> Path:
    ...
    every HBM_HOME, including non-``profiles/`` subdirs of ``~/.hbm``."""
    from tools.bot_mode_probe import _default_home, _hbm_root
    return _hbm_root(Path(_default_home()))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``hbm -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "hbm" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="hbm-relay-dm-", suffix=".txt", text=True)
        try:
