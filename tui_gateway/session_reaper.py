...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    hung SQLite write can't block exit past ``HBM_TUI_EXIT_FLUSH_BUDGET_S`` (default 5s). Running sessions
    ...
    worker = threading.Thread(target=_run, daemon=True, name="hbm-exit-flush")
    worker.start()
    worker.join(budget)
...
def _transport_is_dead(transport) -> bool:
    ...
    # standalone `hbm --tui` and must NOT count as dead.
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from hbm_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from hbm_cli.active_sessions import coerce_max_concurrent_sessions
        cfg = _load_cfg() or {}
...
_HEARTBEAT_REFRESH_S = max(0.0, env_float("HBM_GATEWAY_HEARTBEAT_REFRESH_S", 60.0))
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    threading.Thread(target=_loop, name="hbm-gateway-heartbeat", daemon=True).start()
