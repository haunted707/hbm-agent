...
from hbm_constants import display_hbm_home as _display_hbm_home
...
def _cfg_get_provider(params):
    from hbm_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _hbm_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_hbm_home), "display": _display_hbm_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's HBM_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``HBM_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from hbm_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No HBM AGENT provider is configured.", source)
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``hbm debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from hbm_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
