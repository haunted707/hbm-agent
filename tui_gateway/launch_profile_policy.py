"""Launch-profile policy for a process that hosts several profile homes (``hbm serve`` /
``hbm dashboard`` pooling, ``?profile=``, hosted rooms; the multiplexed gateway's own worker).
