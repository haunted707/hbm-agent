...
def _hosted_room_turn_timeout_seconds() -> float:
    try:
        agent_timeout = float(os.getenv("HBM_AGENT_TIMEOUT", "1800"))
...
class HostedRoomService:
    ...
    def local_profiles(self) -> tuple[str, ...]:
        from hbm_constants import named_profile_is_deleted

        profiles, profiles_dir = {"default"}, self.root / "profiles"
        if profiles_dir.is_dir():
            # ``profiles/.deleted/`` is the tombstone dir `hbm profile delete` leaves behind, not a
