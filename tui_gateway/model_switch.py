...
def _profile_runtime_scope_tokens(profile_home) -> "_TurnScopes":
    """Bind HBM_HOME + secret + terminal scope for ``profile_home`` (None = launch profile) and
    ...
    if profile_home:
        ...
        overlay = None
        scopes.home = set_hbm_home_override(str(home))
    else:
        # No home override: the launch home IS get_hbm_home() (``_profile_home`` answers None for
...
def _release_profile_runtime_scope_tokens(scopes: "_TurnScopes | None") -> None:
    ...
    not leave the previous profile's secrets / HBM_HOME installed for the next body in this
    ...
    for token, reset in ((scopes.terminal, reset_terminal_scope), (scopes.secret, reset_secret_scope),
                         (scopes.home, reset_hbm_home_override)):
        if token is None:
            continue
        try:
            reset(token)
...
def _apply_model_switch(
    ...
    # Deliberately NOT written to process-global env (HBM_MODEL & co.): the desktop hosts
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        ...
        # how `hbm --tui -m` once leaked into config.yaml).
        _apply_model_switch(
