...
and a green off it is fake. Test seam: dead unless ``HBM_ISO_CERTIFY_SYNTH_TURN=1``; when armed,
...
_SPEC_FIELDS = (
    ("duration_s", float, lambda: _env_float("HBM_ISO_CERTIFY_DURATION_S", 8.0)),
    ("chunk", int, lambda: _env_int("HBM_ISO_CERTIFY_CHUNK", 20_000)),
    ("delta_interval_s", float, lambda: _env_float("HBM_ISO_CERTIFY_DELTA_S", 0.05)),
    ("tokens_per_delta", int, lambda: _env_int("HBM_ISO_CERTIFY_TPD", 512)),
...
def synth_turn_armed() -> bool:
    ...
    return os.environ.get("HBM_ISO_CERTIFY_SYNTH_TURN") == "1"
