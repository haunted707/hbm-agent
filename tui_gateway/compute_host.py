...
class ComputeHost:
    ...
    def __init__(
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("HBM_COMPUTE_HOST_HEARTBEAT_SECS") or "15"))
    ...
    def _run_real_turn(self, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                import hbm_undo
                hbm_undo.on_user_message_appended(session["session_key"])
            with contextlib.suppress(Exception):
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("HBM_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["HBM_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit({
        ...
        "hbm_home": os.environ.get("HBM_HOME", "")})
