...
def busy_message(command: str) -> str:
    ...
    return (f"session busy — HBM AGENT is still replying. Stop the current reply first (Stop button, "
            f"or Ctrl+C in a terminal), then run /{command.lstrip('/')}.")


def agent_init_failed_message(exc: Any) -> str:
    return (f"HBM AGENT could not start the assistant for this session. Details: {exc}. "
            "Check the model and provider with /model, or run `hbm setup` in a terminal to reconfigure.")


AGENT_STILL_STARTING = (
    "HBM AGENT is still starting this session (loading tools), so this command could not run yet. "
...
AGENT_MISSING_FOR_TURN = (
    "HBM AGENT could not start the assistant for this session, so your message was not run. "
