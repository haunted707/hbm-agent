...
def _legacy_group_fence_error(rid, session, params):
    ...
    if hosted or peer:
        ...
        return _err(
            rid, 4122, f"This room is managed by {owner}. Update HBM AGENT Desktop to continue it.")
    return None
...
def _parse_truncation_params(rid, sid, session, params, history):
    ...
    return None, None, _err(
        ...
        "(update your HBM AGENT client if a rewind was intended)")
...
def _(rid, params: dict) -> dict:
    from hbm_cli.input_sanitize import sanitize_user_prompt_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from hbm_cli.clipboard import has_clipboard_image, save_clipboard_image
...
_PREVIEW_RESTART_RULES = (
    "Restart exactly the app intended for the Preview URL, not HBM AGENT Desktop itself.",
    ...
    "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the HBM AGENT/Desktop process cwd.",
    ...
    "Before declaring success, verify the Preview URL responds with the intended app, not HBM AGENT Desktop. If it serves HBM AGENT/Desktop UI or another unrelated app, stop that process and report failure.",
