...
activate with `hbm --tui` or `HBM_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `HBM_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.hbm`. The first non-launch home hosted flips
...
**A method that sets only `get_hbm_home_override()` is half-bound**: config paths resolve to the
...
(`hbm -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `hbm_cli/commands.py` (`hbm_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds hbm-ink + tsx --watch)
npm start         # production
npm run build     # full build (hbm-ink + tsc)
