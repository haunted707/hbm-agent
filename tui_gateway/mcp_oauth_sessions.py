...
land. Reuses ``hbm mcp login``'s probe under ``force_interactive_oauth`` plus
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — stdlib naming
            ...
            if parsed.path.rstrip("/") not in ("/callback", ""):
                ...
                self.end_headers()
                return
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to HBM AGENT.</p>"
            status = 200
            try:
...
def _probe_with_rollback(
    server_name: str, cfg: dict, hbm_home: str, flow, reconnect_live: bool) -> None:
    """Run the OAuth probe; on ANY failure restore the prior token file + manager entry."""
    from hbm_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    from tools.mcp_oauth import HbmTokenStorage
    from tools.mcp_oauth_manager import get_manager
    manager = get_manager()
    storage = HbmTokenStorage(server_name)
    backup = storage.snapshot()
    previous_entry = None
    try:
        previous_entry = manager.remove(server_name, hbm_home=hbm_home)
        ...
        manager.restore_entry(server_name, previous_entry, hbm_home=hbm_home)
        raise


def _worker(
        session_id: str, hbm_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    try:
        ...
        home_token = set_hbm_home_override(hbm_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(hbm_home)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                _probe_with_rollback(server_name, cfg, hbm_home, flow, reconnect_live)
        finally:
...
def start_flow(
    hbm_home: str, server_name: str, cfg: dict, *, reconnect_live: bool = False,
    ...
    with _sessions_lock:
        ...
        if any(r["server_name"] == server_name and r["hbm_home"] == hbm_home for r in active):
            raise RuntimeError(f"MCP OAuth for '{server_name}' is already in progress")

    session_id = secrets.token_urlsafe(24)
    flow = DashboardOAuthFlow(
        flow_id=session_id, server_name=server_name, profile=None, hbm_home=hbm_home,
    ...
    rec = {
        "session_id": session_id, "server_name": server_name, "hbm_home": hbm_home,
        "flow": flow, "httpd": httpd, "created_at": time.time()}
    with _sessions_lock:
        _sessions[session_id] = rec
    threading.Thread(
        target=_worker, args=(session_id, hbm_home, server_name, dict(cfg), reconnect_live),
...
def _lookup(
    session_id: str, server_name: str, hbm_home: Optional[str] = None,
    ...
    if hbm_home_key(rec["hbm_home"]) != hbm_home_key(hbm_home):
...
def cancel_flow(session_id: str, server_name: str, hbm_home: str) -> Dict[str, Any]:
    """Cancel only the owning profile's flow and release its callback waiter."""
    rec, err = _lookup(session_id, server_name, hbm_home)
    if rec is None:
