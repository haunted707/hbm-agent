...
def _is_repo_junk(root: str) -> bool:
    ...
    HBM_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    from hbm_constants import get_hbm_home
    real = os.path.realpath(root)
    hbm_home = os.path.realpath(str(get_hbm_home()))
    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == hbm_home
        or real.startswith(hbm_home + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of HBM_HOME may be an
    intentional prose/data workspace, so only HBM_HOME itself is excluded here."""
    if not cwd:
        return True
    from hbm_constants import get_hbm_home
    real = os.path.normcase(os.path.realpath(cwd))
    hbm_home = os.path.normcase(os.path.realpath(str(get_hbm_home())))
    return real in _non_workspace_dirs() or real == hbm_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero HBM AGENT sessions still appear in the sidebar (#81723).
