...
def _session_home(session: dict) -> Path:
    """The HBM_HOME the session's durable state lives in (profile-aware)."""
    return Path(session.get("profile_home") or _hbm_home)
...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        # Ownership admission BEFORE message.start: a sibling backend sharing this HBM_HOME may have written the
