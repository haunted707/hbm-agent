...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("HBM_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("HBM_SESSION_SOURCE", "")
...
_hbm_home = get_hbm_home()  # read by agent_init via _ra()._hbm_home
_loaded_env_paths = load_hbm_dotenv(hbm_home=_hbm_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[hbm-agent: tool call arguments were corrupted in this session and "
    ...
    def _session_row_model_config(self) -> Any:
        ...
        /yolo toggle for ``hbm-agent --resume``.
        """
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from hbm_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``HBM_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("HBM_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``HBM_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("HBM_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("HBM_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        return (
            ...
            "See hbm-agent#21444 for symptom history."
        )
