...
class GatewayAgentCacheMixin:
    ...
    def _bind_adapter_run_generation(self, adapter: Any, session_key: str, generation: int | None) -> None:
        ...
        with suppress(Exception):
            ...
            if interrupt_event is not None:
                interrupt_event._hbm_run_generation = int(generation)
    ...
    def _ephemeral_change_key(context, redact_pii: bool) -> str:
        ...
        try:
            ...
            home_display = str(display_hbm_home())
        except Exception:
            home_display = ""
        key_tuple = (
    ...
    def _run_release_in_profile_scope(self, target, args: tuple, session_key: Optional[str]) -> None:
        ...
        root HBM AGENT dir even when the gateway was launched under a named profile. Its scope is
        entered unless the current one already is the owner's."""
        from agent.secret_scope import current_secret_scope, is_multiplex_active
        scope = nullcontext()
        if is_multiplex_active():
            from gateway.run import _profile_runtime_scope
            from hbm_constants import get_default_hbm_root, get_hbm_home, hbm_home_key
            owner = None
            store = getattr(self, "session_store", None)
            if session_key and store is not None:
                try:
                    ...
                    logger.warning("Could not resolve the owning profile for %s; releasing under the default profile",
                                   session_key, exc_info=True)
            owner_home = Path(owner) if owner else get_default_hbm_root()
            if current_secret_scope() is None or hbm_home_key(get_hbm_home()) != hbm_home_key(owner_home):
