...
# exit (bounded), then spawns ``hbm gateway restart``.
...
class GatewayShutdownMixin:
    ...
    def _scale_to_zero_has_live_background_work(self) -> bool:
        ...
        PERMANENT supervised watchers (_hbm_supervised_watcher, incl. the scale-to-zero watcher
        itself) are excluded, else this would be True forever and the gateway could never go dormant.
        """
        if any(
            not t.done() and not getattr(t, "_hbm_supervised_watcher", False)
    ...
    def _pause_failed_platform(self, platform, *, reason: str = "") -> None:
        ...
        logger.warning(
            ...
            "resume %s` to retry, or `hbm gateway restart` to restart the gateway.",
    ...
    async def _notify_interrupted_cron_jobs(self, job_ids) -> int:
        ...
        for job_id in job_ids:
            ...
            msg = (
                f"⚠️ Scheduled job '{job_name}' was cut short because HBM AGENT is {action}; "
                "no result this run. It will run again on schedule, or run it now with "
                f"`hbm cron run {job_name}` once HBM AGENT is back."
            )
    ...
    async def _notify_active_sessions_of_shutdown(self) -> None:
        ...
        msg = (
            "⚠️ HBM AGENT is shutting down — your current task will be interrupted. "
            "When it is back online, send any message and I'll try to pick up where we left off."
        )
        if self._restart_requested:
            msg = (
                "⚠️ HBM AGENT is restarting — your current task will be interrupted. "
    ...
    async def _finalize_session_off_loop(
        ...
        """Run hbm_cli.lifecycle.finalize_session off-loop, bounded; on timeout the worker is left alone.
    ...
    def _stuck_loop_counts_path(self) -> Path:
        ...
        return _hbm_home / self._STUCK_LOOP_FILE
    ...
    def _restart_watcher_env() -> dict:
        """Watcher env minus ``_HBM_GATEWAY`` (else the CLI's self-restart guard refuses; gateway stays down)."""
        ...
        watcher_env.pop("_HBM_GATEWAY", None)
        return watcher_env

    @staticmethod
    def _spawn_windows_restart_watcher(hbm_cmd: list, current_pid: int, restart_after_s: float) -> None:
        ...
        # it owns one hidden console, inherited by the `hbm gateway restart` child, so nothing flashes.
        # See #54220, #56747.
        ...
        watcher_argv = [
            ...
            str(current_pid), str(restart_after_s), *hbm_cmd, "gateway", "restart",
        ]
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        if not hbm_cmd:
            logger.error("Could not locate hbm binary for detached /restart")
            return
        ...
        if sys.platform == "win32":
            GatewayShutdownMixin._spawn_windows_restart_watcher(hbm_cmd, current_pid, restart_after_s)
            return
        cmd = " ".join(shlex.quote(part) for part in hbm_cmd)
        shell_cmd = (
    ...
    def _wedged_agent_count(self) -> int:
        ...
        timeout = _float_env("HBM_AGENT_TIMEOUT", 1800)
        if timeout <= 0:
            return 0
    ...
    def _describe_active_work(self) -> list:
        ...
        (``hbm update``, ``hbm gateway status``) can name it instead of printing a bare count.
    ...
    def _start_systemd_watchdog(self) -> bool:
        ...
        watchdog.ready("HBM AGENT Gateway running")
        return True
    ...
    def _stop_quiesce_and_close_session_dbs(self, timeout: float, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        def _close_shared() -> None:
            ...
            # (#90837).
            from hbm_state_registry import close_all
            closed = close_all()
            if closed:
    ...
    def _stop_persist_exit_state(self, ctx: "GatewayShutdownMixin._StopContext") -> None:
        ...
        from gateway.run import _hbm_home, _planned_restart_notification_path, _shutdown_gateway_health_export
        from utils import atomic_json_write
        ...
        if not ctx.timed_out:
            with suppress(Exception):
                (_hbm_home / ".clean_shutdown").touch()
        else:
            logger.info(
