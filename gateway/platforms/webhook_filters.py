...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.hbm to the active profile home."""
    ...
    if raw == "~/.hbm" or raw.startswith("~/.hbm/"):
        return hbm_home / raw[len("~/.hbm/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else hbm_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under HBM_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from hbm_constants import get_hbm_home
    scripts_root = (get_hbm_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.hbm" or raw_text.startswith("~/.hbm/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__hbm_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__hbm_ignore__") is True
