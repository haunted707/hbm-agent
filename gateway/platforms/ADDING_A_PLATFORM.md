...
There are two ways to add a platform to the HBM AGENT gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.hbm/plugins/` (or under `plugins/platforms/`
...
**zero changes to core HBM AGENT code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `hbm gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `hbm_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the HBM AGENT core.
...
"hbm-your-platform": {
    ...
    "tools": _HBM_CORE_TOOLS,
    "includes": []
},
```

And add it to the `hbm-gateway` composite:

```python
"hbm-gateway": {
    "includes": [..., "hbm-your-platform"]
}
```
...
## 12. Status Display (`hbm_cli/status.py`)
...
## 13. Gateway Setup Wizard (`hbm_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ hbm_cli/ toolsets.py \
  --include="*.py" -l | sort -u
