...
def _get_hbm_version() -> str:
    """Return the hbm-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    """``QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; HBM AGENT/<hbm_version>)``."""
    v = sys.version_info
    return (f"QQBotAdapter/{QQBOT_VERSION} (Python/{v.major}.{v.minor}.{v.micro}; "
            f"{platform.system().lower()}; HBM AGENT/{_get_hbm_version()})")
