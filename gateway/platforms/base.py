...
# HBM AGENT but not to sendAudio).
_AUDIO_MIME_TYPES = {
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (platform == "slack" and isinstance(raw_message, dict)
            and raw_message.get("_hbm_no_thread_response")):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    ``HBM_SESSION_PLATFORM`` contextvar before the post-handler auto-TTS block runs.
    ...
    audio_path = os.path.join(
        tempfile.gettempdir(), "hbm_voice", f"tts_reply_{uuid.uuid4().hex[:12]}.{ext}")
...
def resolve_proxy_url(
    ...
    *platform_env_var* is a per-adapter, per-profile-configurable setting (each proxy URL can
    ...
    process-env read — those are OS/system-level network settings, not a per-profile HBM AGENT concept."""
...
def _config_section(name: str) -> dict:
    ...
    try:
        from hbm_cli.config import load_config_readonly as _load_config
...
from hbm_constants import get_default_hbm_root, get_hbm_dir, get_hbm_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.hbm/.env manually.")
...
# ``hbm pairing approve`` command (hbm_cli/subcommands/pairing.py) that lets the owner fix it.
...
UNAUTHORIZED_ACTION_NOTICE = (
    ...
    "`hbm pairing approve {platform} <request-id>` on the host (`hbm pairing list` shows the id).")
...
IMAGE_CACHE_DIR = get_hbm_dir("cache/images", "image_cache")
...
def _cache_dir_accessors(kind: str, constant_name: str, new_subpath: str, old_name: str):
    ...
    get_hbm_dir (active profile) unless a test monkeypatched the module constant away from
    its import-time default, and creates the directory; ``cleanup(max_age_hours=24)`` deletes
    older files and returns the count."""
    def get_dir() -> Path:
        d = get_hbm_dir(new_subpath, old_name)
...
async def _cache_media_from_url(url: str, ext: str, retries: int, *, media_type: str, accept: str,
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; HbmAgent/1.0)", "Accept": accept}
...
AUDIO_CACHE_DIR = get_hbm_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_hbm_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_hbm_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_hbm_dir("cache/screenshots", "browser_screenshots")
...
_HBM_ROOT = get_default_hbm_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "HBM_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "HBM_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "HBM_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "HBM_MEDIA_DELIVERY_STRICT"
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    *(_HBM_HOME / "cache" / d for d in _MEDIA_DELIVERY_CACHE_SUBDIRS))
...
# Credential stores at the HBM_HOME root, denied per-file so skills/, logs/ and agent-written
...
def _profile_cache_roots() -> List[Path]:
    ...
    roots cover only the active HBM_HOME). Enumerated at check time so profiles created after
    startup count and are allowlisted BEFORE the ``/root`` denylist (HBM_HOME symlinked).

    ``HBM_HOME=/opt/data``) while the model emits a profile-scoped path silently fails delivery.
    ...
    when HBM_HOME is symlinked under a denied prefix and $HOME is not that prefix). See issue #31733.
    """
...
def _profile_dirs() -> List[Path]:
    ...
    try:
        return [p for p in (_HBM_ROOT / "profiles").iterdir() if p.is_dir()]
    except OSError:
        return []


def _credential_home_roots() -> List[Path]:
    """Every HBM AGENT home whose credential stores the denylist must cover: the ACTIVE home
    (the per-turn HBM_HOME override under ``gateway.multiplex_profiles``), the shared root
    ...
    return list(dict.fromkeys((get_hbm_home(), _HBM_ROOT, *_profile_dirs())))


def _kanban_root() -> Path:
    """Kanban is root-shared across profiles by design (``kanban_db.kanban_home``)."""
    return Path(os.environ.get("HBM_KANBAN_HOME", "").strip() or _HBM_ROOT).expanduser()
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    override = os.environ.get("HBM_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    Credential sub-dirs (``~/.ssh``, ``~/.hbm/.env``) stay blocked (more-specific entries)."""
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted HBM AGENT cache dirs (``/root/.hbm/...`` in
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted cache dirs (``/root/.hbm/...``), persistent ``/workspace`` and ``/root``."""
    if not candidate.is_absolute():
        return None
    # In-process gateways (Desktop, `hbm serve`) may not have bridged terminal.* config into
    ...
    # prefixes). /root/.hbm/* that missed a cache mount is the container's credential surface —
    # translating it via the home mount would dodge the host denylist.
    if "/root" not in mounted and not candidate.as_posix().startswith("/root/.hbm"):
        mounts.extend(
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    (``HBM_MEDIA_DELIVERY_STRICT=1``, public bots where prompt injection must not exfiltrate
    host secrets): MUST be under a HBM AGENT cache, an operator root (``HBM_MEDIA_ALLOW_DIRS``),
    ...
    for root in _media_delivery_allowed_roots():
        ...
        if resolved_root is not None and _path_is_within(resolved, resolved_root):
            return str(resolved)
    # Non-strict (default): anything not denylisted (/etc, /proc, ~/.ssh, HBM AGENT-root secrets).
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?hbm\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+hbm[.!?\s]*$", re.IGNORECASE))
...
class BasePlatformAdapter(ABC):
    ...
    # adapters (API server). Propagated to ``HBM_SESSION_ASYNC_DELIVERY`` so tools never promise
    # a delivery they can't keep.
    supports_async_delivery: bool = True
    # ``send()`` chunks natively via ``truncate_message()`` -> the router skips its truncation.
    splits_long_messages: bool = False
    # Prefix users can always TYPE for HBM AGENT commands ("!" where the client eats a leading "/").
    ...
    # ``hbm gateway migrate`` can tell "URL changes" from "this profile would be skipped" as new
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("HBM_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower() or "interrupt")
        self._busy_text_debounce_seconds: float = _float_env("HBM_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35)
        self._busy_text_hard_cap_seconds: float = _float_env("HBM_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0)
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        """Acquire a scoped lock for this adapter; True on success. A live cross-HBM_HOME
        ...
        remedy = (f" Stop that gateway first (hbm --profile {owner_profile} gateway stop)."
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        from hbm_cli.commands import (is_interrupt_then_dispatch, should_bypass_active_session)
    ...
    def _get_human_delay() -> float:
        """Random human-like pacing delay (s) from HBM_HUMAN_DELAY_MODE: "off" (default) |
        "natural" 800-2500ms | "custom" via HBM_HUMAN_DELAY_MIN_MS /
        HBM_HUMAN_DELAY_MAX_MS."""
        mode = os.getenv("HBM_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        lo, hi = 800, 2500
        if mode != "natural":  # custom mode tolerates malformed env vars
            lo = _or_default(lambda: int(os.getenv("HBM_HUMAN_DELAY_MIN_MS", str(lo))), lo)
            hi = _or_default(lambda: int(os.getenv("HBM_HUMAN_DELAY_MAX_MS", str(hi))), hi)
    ...
    async def _synthesize_auto_tts(self, text_content: str) -> Tuple[List[str], Optional[str]]:
        ...
        (logged, never raised). Path built platform-aware HERE: HBM_SESSION_PLATFORM is cleared
        post-handler."""
        paths: List[str] = []
        requested_path = None
        try:
    ...
    def _wants_auto_tts(self, event: MessageEvent, session_key: str, interrupt_event: asyncio.Event,
        ...
        generation = getattr(interrupt_event, "_hbm_run_generation", None)
        return bool(
    ...
    async def _fire_post_delivery_callback(self, session_key: str, interrupt_event: asyncio.Event) -> None:
        ...
        _post_cb = self.pop_post_delivery_callback(
            session_key, generation=getattr(interrupt_event, "_hbm_run_generation", None))
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            self._streaming_tts_completed_turns.discard(self._streaming_tts_turn_key(
                session_key, getattr(interrupt_event, "_hbm_run_generation", None),
                event=event) or "")
    ...
    def build_source(
        ...
        profile is stamped on ``source.profile`` for per-profile HBM_HOME isolation."""
