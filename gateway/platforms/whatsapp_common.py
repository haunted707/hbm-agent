...
class WhatsAppBehaviorMixin(OwnAccessPolicyMixin):
    ...
    DEFAULT_REPLY_PREFIX: str = "☤ *HBM AGENT*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    try:
        ...
        pass
    if hbm_home_bridge.exists():
        return hbm_home_bridge
    try:
        hbm_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, hbm_home_bridge, dirs_exist_ok=False)
        return hbm_home_bridge
    except Exception:
        return install_bridge
