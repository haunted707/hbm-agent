...
def _unscoped_profile_secrets() -> dict:
    ...
    if _UNSCOPED_PROFILE_SECRETS is None:
        try:
            ...
            _UNSCOPED_PROFILE_SECRETS = dict(build_profile_secret_scope(get_hbm_home()))
        except Exception:
            logger.warning(
...
def decode_json_list_literal(raw):
    """Decode a JSON-encoded allowlist written by ``hbm config set``.
...
def env_is_connected(*names: str) -> Callable[[Any], bool]:
    ...
    non-blank. Resolves ``hbm_cli.gateway.get_env_value`` at call time (scope-aware, .env-backed) so
