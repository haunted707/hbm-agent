...
from hbm_cli.config import atomic_config_write, cfg_get
...
_BUSY_MODE_BEHAVIOR = {
    "queue": ("queues for next turn", "Messages will be queued for the next turn while HBM AGENT is busy."),
    "steer": ("steers into current run (after next tool call)",
              "Messages will be steered into the current run (after the next tool call)."),
    "interrupt": ("interrupts current run", "Messages will interrupt the current run while HBM AGENT is busy."),
}
...
def _spawn_detached_update(hbm_cmd, output_path, exit_code_path) -> None:
    """Spawn ``hbm update --gateway`` detached so it survives the gateway restart it may trigger.
    ...
    this interpreter (not venv\\Scripts\\hbm.exe — that shim holds its own file open, and the
    ...
    if sys.platform == "win32":
        ...
        subprocess.Popen(
            [sys.executable, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             sys.executable, "-m", "hbm_cli.main", "update", "--gateway"],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **windows_detach_popen_kwargs())
        return
    hbm_cmd_str = " ".join(shlex.quote(part) for part in hbm_cmd)
    update_cmd = (
        f"PYTHONUNBUFFERED=1 {hbm_cmd_str} update --gateway"
...
class GatewaySlashCommandsMixin(
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """The prefix users can always type to reach HBM AGENT commands (adapter ``typed_command_prefix``,
    ...
    async def _handle_platform_command(self, event: MessageEvent) -> str:
        ...
        if action == "pause":
            ...
            return f"✓ {name} paused. Resume with `/platform resume {name}` or `hbm gateway restart` to reset."
        if not queued:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _hbm_home
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(atomic_json_write, _hbm_home / name, build(), indent=None)
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running HBM AGENT version."""
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        # (NOT `hbm skills diff <name>`, which diffs a bundled skill against its stock version).
        if sub == "diff" and len(out) > 3000:
            pending_id = args[1] if len(args) > 1 else "<id>"
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.hbm/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while HBM AGENT is working."""
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_hbm_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return ("No skill bundles installed.\nCreate one on the host with:\n"
                    "  `hbm bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        protect privacy; ``hbm debug share`` from the CLI does full uploads."""
        from hbm_cli.debug import (_GATEWAY_PRIVACY_NOTICE, _best_effort_sweep_expired_pastes,
        ...
        # ``get_hbm_home()`` and uploads them to a public paste. Losing the contextvar override
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update — spawn ``hbm update`` detached (``setsid``) so it survives the gateway
        restart it may trigger; marker files let this or the next gateway process notify the user."""
        import json
        from gateway.run import _hbm_home, _resolve_hbm_bin
        ...
        if src.platform not in self._UPDATE_ALLOWED_PLATFORMS:
            try:
                ...
                return t("gateway.update.platform_not_messaging")
        if is_managed():
            return f"✗ {format_managed_message('update HBM AGENT')}"
        if not (Path(__file__).parent.parent.resolve() / '.git').exists():
            return t("gateway.update.not_git_repo")
        hbm_cmd = _resolve_hbm_bin()
        if not hbm_cmd:
            return t("gateway.update.hbm_cmd_not_found")
        pending_path = _hbm_home / ".update_pending.json"
        output_path = _hbm_home / ".update_output.txt"
        exit_code_path = _hbm_home / ".update_exit_code"
        pending = {
        ...
        try:
            _spawn_detached_update(hbm_cmd, output_path, exit_code_path)
        except Exception as e:
