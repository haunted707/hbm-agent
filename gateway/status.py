"""Gateway runtime status helpers: PID/lock/marker files under ``{HBM_HOME}`` (one set per
...
from hbm_constants import _get_platform_default_hbm_home, get_hbm_home
...
def record_start_and_check_storm(
    ...
    try:
        path = get_hbm_home() / "gateway-starts.log"
...
def _get_process_hbm_home() -> Path:
    """Launch-home HBM_HOME for identity files (PID, lock, status, markers):
    ``get_hbm_home()`` honors the per-session ``_HBM_HOME_OVERRIDE`` and would misroute
    them."""
    val = os.environ.get("HBM_HOME", "").strip()
    return Path(val) if val else _get_platform_default_hbm_home()


def _canonical_hbm_home(path: Path | str) -> Path:
    """Stable absolute HBM_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_hbm_home(left: Path | str, right: Path | str) -> bool:
    """Compare HBM_HOME paths with the host platform's case semantics."""
    left_c = os.path.normcase(str(_canonical_hbm_home(left)))
    return left_c == os.path.normcase(str(_canonical_hbm_home(right)))


def recorded_gateway_home_conflicts(
    record: Optional[dict[str, Any]], *, expected_home: Optional[Path | str] = None
) -> bool:
    """True when a persisted gateway record names a DIFFERENT HBM_HOME (cross-profile kill guard:
    profile B's stop must never SIGTERM profile A). ``expected_home`` overrides the comparison base.
    Legacy records without ``hbm_home`` prove nothing -> False; a comparison failure fails
    closed -> True."""
    recorded_home = record.get("hbm_home") if isinstance(record, dict) else None
    if not isinstance(recorded_home, str) or not recorded_home.strip():
        return False
    try:
        base = expected_home if expected_home is not None else _get_process_hbm_home()
        return not _same_hbm_home(recorded_home, base)
    except Exception:
        return True


# Mirrors hbm_cli.profiles._PROFILE_ID_RE -- duplicated so gateway identity code
# stays import-light (hbm_constants + stdlib only).
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    try:
        canonical = _canonical_hbm_home(home)
    except Exception:
        return None
    if canonical.parent.name == "profiles" and _PROFILE_LABEL_RE.match(canonical.name):
        return canonical.name
    import hbm_constants
    default_homes = (hbm_constants.get_default_hbm_root, _get_platform_default_hbm_home)
    for default_home in default_homes:
        with contextlib.suppress(Exception):
            if _same_hbm_home(canonical, default_home()):
                return "default"
    return None
...
def _get_runtime_status_path() -> Path:
    return _get_process_hbm_home() / _RUNTIME_STATUS_FILE


def _get_lock_dir() -> Path:
    """Machine-local dir for token-scoped gateway locks; ``HBM_GATEWAY_LOCK_DIR`` overrides."""
    override = os.getenv("HBM_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "hbm" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """HBM AGENT gateway lifecycle subcommand from a command line, or None. No loose substring matches
    (``"gateway" in cmdline`` also matched ``gateway status`` / ``python -m tui_gateway``): needs a
    HBM AGENT entrypoint plus the ``gateway`` subcommand, or a gateway-dedicated entrypoint. Tokenizes
    ...
    if any(t == "gateway/run.py" or t.endswith("/gateway/run.py") for t in tokens):
        return "run"
    if any(b in ("hbm-gateway", "hbm-gateway.exe") for b in basenames):
        return "run"
    joined = " ".join(tokens)
    if "hbm_cli.main" not in joined and "hbm_cli/main.py" not in joined and not any(
        b in ("hbm-agent", "hbm-agent.exe") for b in basenames
    ):
        return None
    ...
    for i, token in enumerate(filtered):
        if token == "gateway":
            # Bare `hbm gateway` defaults to `run`.
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    HBM AGENT-owned records / cleanup scans only; ``looks_like_gateway_command_line`` stays strict."""
    return _gateway_command_subcommand(command) in {"run", "restart"}


def _looks_like_gateway_process(pid: int) -> bool:
    """True when the live PID still looks like the HBM AGENT gateway."""
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    ``hbm_cli.gateway._matches_current_profile``): a stale state file can record a PID recycled
    onto ANOTHER profile's live gateway. Named profiles carry ``-p``/``--profile <name>`` or
    ``HBM_HOME=`` on argv; the default gateway runs bare. Separators normalized."""
    command_lc = command.lower().replace("\\", "/")
    profile_name = _profile_name_for_home(profile_home)
    home_lc = str(profile_home).lower().replace("\\", "/")
    if profile_name is not None and profile_name != "default":
        return profile_flag_value(command_lc) == profile_name.lower() or f"hbm_home={home_lc}" in command_lc
    ...
    # or a conflicting explicit HBM_HOME= (its absence is not disqualifying -- HBM_HOME usually
    # arrives via the env).
    if profile_flag_value(command_lc) is not None:
        return False
    return not ("hbm_home=" in command_lc and f"hbm_home={home_lc}" not in command_lc)
...
def _build_pid_record() -> dict:
    return {
        ...
        "hbm_home": str(_canonical_hbm_home(_get_process_hbm_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    Lazy import keeps ``gateway.status`` free of ``hbm_cli`` at import time. Never raises.

    A gateway keeps serving the module versions it imported at startup, so stamping the identity into
    ``gateway_state.json`` lets `hbm update` (and the dashboard) prove whether a running gateway actually
    picked up new code after the restart phase — instead of assuming it did (#88654, #69754). Never raises;
    degrades to absent fields.
    """
    try:
        from hbm_cli.build_info import get_code_identity
...
def _pid_record_belongs_to_current_profile(record: Optional[dict[str, Any]]) -> bool:
    """True when the record's ``hbm_home`` matches the current process (legacy records: True);
    another HBM_HOME's record must be ignored or the default gateway assumes its identity."""
    if not isinstance(record, dict):
        return False
    record_home = record.get("hbm_home")
    return not record_home or _same_hbm_home(record_home, _get_process_hbm_home())
...
def multiplexer_liveness_for_profile(profile_dir: Path) -> Optional[tuple[int, dict[str, Any]]]:
    ...
    dashboard ladder reports it stopped while ``hbm -p X status`` says running — the two must agree.
    """
    name = _profile_name_for_home(Path(profile_dir))
    if not name:
        return None
    from hbm_cli.gateway import named_profile_served_by_running_multiplexer
    from hbm_cli.gateway_multiplex_served import live_default_gateway_pid
    ...
    if pid is None:
        return None
    return pid, read_runtime_status(get_default_hbm_root() / "gateway_state.json") or {}
...
def resolve_gateway_liveness(
    *, profile_dir: Optional[Path] = None, runtime: Any = _UNSET,
    ...
    # the multiplexer IS its gateway (mirrors `hbm -p X status` / `gateway list`). Unscoped, the
    # question is about the process's OWN home — which is a named profile inside a pooled
    # `hbm --profile X serve` (the Desktop's per-profile backend answers its REST without
    # `?profile=`), so it takes the same rung instead of reporting the served profile stopped.
    own_home = profile_dir if scoped else _get_process_hbm_home()
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # Active-profile context: the record's hbm_home must match this process so a stale record
...
def acquire_scoped_lock(
    ...
    # does not say WHICH profile). Omitted when not inferable; readers fall back to hbm_home.
    profile = _profile_label_for_home(_get_process_hbm_home())
    if profile:
...
# token. ``hbm.service`` + ``hbm- gateway.service``). See #5646.
...
def _get_takeover_marker_path(hbm_home: Optional[Path] = None) -> Path:
    """Takeover marker path; ``hbm_home`` is given only for a verified cross-home handoff."""
    home = _canonical_hbm_home(hbm_home or _get_process_hbm_home())
    return home / _TAKEOVER_MARKER_FILENAME


def _get_planned_stop_marker_path() -> Path:
    return _get_process_hbm_home() / _PLANNED_STOP_MARKER_FILENAME
...
def _pid_marker_names_self(target_pid: int, target_start_time: Any) -> bool:
    ...
    ``hbm gateway stop`` as an unexpected exit revived by the service manager."""
...
def _consume_pid_marker_for_self(path: Path, *, ttl_s: int) -> bool:
    ...
    # cross-HBM_HOME --replace while ignoring a marker accidentally written into another
    # profile's directory. Legacy markers have no target field: keep the same-replacer-home rule.
    # See #29092.
    our_home = _get_process_hbm_home()
    target_home = record.get("target_hbm_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_hbm_home(target_home, our_home):
            return False
    else:
        replacer_home = record.get("replacer_hbm_home")
        if replacer_home is not None and not _same_hbm_home(replacer_home, our_home):
            return False
...
def write_takeover_marker(
    ...
    try:
        marker_home = _canonical_hbm_home(target_home or _get_process_hbm_home())
        ...
        return _write_marker(_get_takeover_marker_path(marker_home), {
            ...
            "target_hbm_home": str(marker_home), "replacer_pid": os.getpid(),
            "replacer_hbm_home": str(_canonical_hbm_home(_get_process_hbm_home())),
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    if (
        ...
        return None
    target_home = _canonical_hbm_home(raw_home)
    ...
    pid_record_home = pid_record.get("hbm_home")
    if (
        ...
        or not _same_hbm_home(pid_record_home, target_home)
    ):
        return None
