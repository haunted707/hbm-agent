...
failed approvals, chmod 0600 data files, codes never logged. Storage: ~/.hbm/pairing/
"""
...
# path would freeze the HBM_HOME/profile context at gateway boot, ignoring later
# context-local overrides, so the gateway and ``hbm pairing`` CLI wrote different
...
# computing the path eagerly freezes it to whatever HBM_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# hbm_constants.set_hbm_home_override) is established afterward. A freshly-started, short-lived
# process (e.g. the ``hbm pairing`` CLI) re-imports this module later with the final environment already
...
def _default_pairing_dir() -> Path:
    return PAIRING_DIR if PAIRING_DIR is not None else get_hbm_dir("platforms/pairing", "pairing")
...
def _write_allowlist_env(env_var: str, ids: list) -> None:
    ...
    with contextlib.suppress(Exception):
        from hbm_cli.config import save_env_value, remove_env_value
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            # exec` runs as root and writes the file, then the gateway process — running as `hbm` after
        ...
        logger.warning(
            ...
            "If you ran `docker exec <container> hbm pairing approve ...` as root, "
            "re-run with `docker exec -u hbm <container> ...` and "
            "chown the existing file to the hbm user, or restart the "
...
class PairingStore:
    ...
    profile's HBM_HOME exactly as ``hbm -p <name> pairing ...`` does, so multiplex
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        self._dir = get_hbm_dir("platforms/pairing", "pairing", home=profile_home) if profile else _default_pairing_dir()
