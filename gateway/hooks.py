...
Hooks live in ~/.hbm/hooks/<name>/ with HOOK.yaml (name, description, events) and
...
def _resolve_hooks_dir() -> Path:
    ...
    else ``get_hbm_home()/hooks``. The import-time constant is the LAUNCH profile's; under
    ...
    return configured if configured != _HOOKS_DIR_AT_IMPORT else get_hbm_home() / "hooks"
...
def _load_hook_dir(hook_dir: Path) -> Optional[tuple]:
    ...
    module_name = f"hbm_hook_{hook_name}"
...
class ProfileHookRegistries:
    """``HookRegistry`` per served profile home, picked at emit time from the active HBM_HOME.
    ...
    ``get_hbm_home()`` there gives each profile its own ``hooks/`` and keeps the default
    profile's handlers from seeing other profiles' messages. Each home's registry is loaded on its
    first emit, i.e. inside that profile's scope (handler imports see its HBM_HOME); multiplexing off
