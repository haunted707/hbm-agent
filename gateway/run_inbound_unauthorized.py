...
def pairing_code_reply(platform_name: str, code: str, profile_arg: str = "") -> str:
    ...
    approve_cmd = f"hbm {profile_arg}pairing approve {platform_name} {code}"
    return (
...
def unauthorized_owner_hint(
    platform_name: str, user_id: str, user_name: str = "", *, hbm_home: str,
) -> str:
    ...
    allowlist = (
        f"add the ID to {env_var} in {hbm_home}/.env and restart the gateway"
    ...
    return (
        ...
        f"{hbm_home}/config.yaml so unknown senders receive a pairing code you can approve with "
        f"`hbm pairing approve {platform_name} <code>`."
    )
