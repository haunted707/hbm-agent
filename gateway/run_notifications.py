...
# (`hbm update` re-runs it in the terminal) and only a short tail is quoted in chat.
_UPDATE_FAILED_NOTICE = (
    "❌ HBM AGENT update failed; the previous version is still running. Run `hbm update` on the "
...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``hbm update --gateway`` and its watcher exchange under HBM_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_hbm_home / ".update_pending.json",
            claimed=_hbm_home / ".update_pending.claimed.json", output=_hbm_home / ".update_output.txt",
            exit_code=_hbm_home / ".update_exit_code",
            prompt=_hbm_home / ".update_prompt.json", response=_hbm_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``hbm update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ HBM AGENT update finished." if exit_code == 0 else _UPDATE_FAILED_NOTICE
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ HBM AGENT update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
    ...
    async def _send_update_notification(self) -> bool:
        ...
        try:
            ...
            if chat_id:
                ...
                if exit_code == 0:
                    msg = "✅ HBM AGENT update finished successfully."
                    if output:
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _hbm_home, _non_conversational_metadata
        notify_path = _hbm_home / ".restart_notify.json"
    ...
    def _free_tier_startup_line(self) -> Optional[str]:
        ...
        try:
            ...
            from hbm_cli.anon_auth import guest_carries_inference
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — HBM AGENT is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from hbm_constants import get_default_hbm_root, profile_cli_selector
        from hbm_state import _default_db_path, classify_persistence_error
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `hbm` follows
        # active_profile, which may be a different database (#105887).
        profile_arg = profile_cli_selector()
        if cause == "corrupt":
            db_path = _default_db_path()
            backups_dir = get_default_hbm_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `hbm {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   hbm {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) hbm {profile_arg}sessions recover "
                ...
                f"Run `hbm {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `hbm {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                "recovery tools or restore a backup unless `hbm doctor` confirms damage."
            )
        else:
            ...
            message = (
                ...
                f"empty. Cause: {failure.gloss}. Run `hbm {profile_arg}doctor --fix` on the "
                "gateway machine, then `hbm gateway restart`."
            )
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-HBM AGENT-Session-Id key, not a structured ``agent:...`` key.
    ...
    def _completion_event_scope(self, evt: dict):
        ...
        (``tools.async_delegation`` → ``get_hbm_home()/state.db``) resolve from the ambient scope.
        ...
        if get_hbm_home_override() == str(profile_home):
            return contextlib.nullcontext()
