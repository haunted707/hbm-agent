...
Defenses 1-2 (``_HBM_GATEWAY`` guard on ``hbm gateway stop|restart`` /
...
persisted to ``<HBM_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_hbm_home() / "gateway" / "restart_loop.json"
