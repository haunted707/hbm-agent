...
def _kbn():
    from hbm_cli import kanban_db_notify
...
def _warn_anchorless_thread_sub_once(sub: dict, platform: str) -> None:
    ...
    logger.warning(
        ...
        "profile route; it will not be delivered. Re-subscribe with `hbm kanban notify-subscribe ... "
...
class _Collector:
    ...
    def collect(self) -> list[dict]:
        ...
        # HBM_KANBAN_DB pins the board path.
        kb = self.kb
...
def _fmt_gave_up(ev, n) -> tuple:
    ...
    return (
        f"⛔ {n.head} is now blocked: {count}{last}. Fix the cause, then `hbm kanban unblock "
        f"{n.task_id}` (or `hbm kanban reassign {n.task_id}`). Logs: `hbm kanban log {n.task_id}`.",
        None, None,
    )
...
class _KanbanNotification:
    ...
    async def wake(self) -> None:
        ...
        if self.sub_profile and getattr(getattr(self.runner, "config", None), "multiplex_profiles", False):
            from hbm_cli.profiles import profile_exists
