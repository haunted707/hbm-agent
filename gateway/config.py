...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # write them where the docs and ``hbm config set platforms.webhook.port`` put them:
...
class GatewayConfig:
    ...
    sessions_dir: Path = field(default_factory=lambda: get_hbm_home() / "sessions")
    ...
    # route); HBM_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_hbm_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.hbm/config.yaml > legacy gateway.json > defaults."""
