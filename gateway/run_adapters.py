...
class GatewayAdapterLifecycleMixin:
    ...
    def _adapter_disconnect_timeout_secs(self) -> float:
        ...
        override = self._env_timeout_override("HBM_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT")
    ...
    def _platform_connect_timeout_secs(self, platform=None, *, initial: bool = False) -> float:
        ...
        override = self._env_timeout_override("HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT")
    ...
    async def _handoff_watcher(self, interval: float = 2.0, drain_timeout: float = 30.0) -> None:
        ...
        async def _tick(profile_name: Optional[str] = None) -> None:
            ...
            for row in pending:
                ...
                # ``set_hbm_home_override`` and ``set_secret_scope`` are ContextVar-based — ensure_future
    ...
    def _flag_reconnect_needs_attention(self, platform, info: dict, now: float) -> None:
        ...
        logger.warning(
            ...
            "credentials, missing intents, broken sidecar). Check `hbm status` / `/platform list`.",
    ...
    async def _start_secondary_profile_adapters(self) -> int:
        ...
        Each profile connects under its own HBM_HOME + secret scope; credential/listener collisions
        ...
        if not self._multiplex_on():
            ...
            # ``served_profiles`` would outlive it into this single-profile run and `hbm -p X ...`
    ...
    def _record_served_profiles(self, active: str, profile_homes) -> None:
        ...
        than "has a connected adapter") for `hbm status`; seed per-profile PairingStores."""
    ...
    def _note_unserved_secondary_platform(self, profile_name: str, platform: Platform) -> None:
        ...
        ``<profile>:<platform>`` status entry so ``hbm gateway status --profile X`` and the
    ...
    def _configure_profile_adapter(
        ...
        # See #76423.
        adapter._hbm_profile_name = profile_name
    ...
    def _make_default_profile_message_handler(self):
        ...
        from gateway.run import _async_profile_runtime_scope, get_hbm_home
        default_home = Path(get_hbm_home())
    ...
    async def _handle_gateway_platform_event(self, event: dict, source) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "gateway_platform_event hook dispatch failed", exc_info=True):
            from hbm_cli.lifecycle import has_hook, invoke_hook
    ...
    def _make_default_profile_platform_event_handler(self):
        ...
        from gateway.run import _profile_runtime_scope, get_hbm_home
        default_home = Path(get_hbm_home())
    ...
    def _adapter_credential_fingerprint(adapter: Any) -> Optional[str]:
        ...
        for obj, attr in candidates:
            ...
            if isinstance(val, str) and val.strip():
                import hashlib
                return hashlib.sha256(("hbm-mux:" + val.strip()).encode("utf-8")).hexdigest()[:16]
        return None
    ...
    def _make_adapter_auth_check(
        ...
        transport_home = Path(get_hbm_home()) if self._multiplex_on() and profile_name is None else None
