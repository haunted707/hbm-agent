"""Session-scoped context variables for the HBM AGENT gateway.

Replaces the old ``os.environ``-based ``HBM_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "HBM_SESSION_PLATFORM", "HBM_SESSION_SOURCE", "HBM_SESSION_CHAT_ID",
    "HBM_SESSION_CHAT_TYPE", "HBM_SESSION_CHAT_NAME", "HBM_SESSION_THREAD_ID",
    "HBM_SESSION_USER_ID", "HBM_SESSION_USER_ID_ALT", "HBM_SESSION_USER_NAME",
    "HBM_SESSION_SCOPE_ID", "HBM_SESSION_KEY", "HBM_SESSION_ID",
    "HBM_UI_SESSION_ID", "HBM_SESSION_MESSAGE_ID", "HBM_SESSION_PROFILE",
    "HBM_BROWSER_CONTROL_PRINCIPAL", "HBM_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "HBM_CRON_SESSION", "HBM_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("HBM_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("HBM_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("HBM_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("HBM_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("HBM_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``HBM_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``HBM_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds HBM_SESSION_PLATFORM, CLI/TUI/
# desktop bind HBM_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``HBM_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("HBM_PLATFORM") or get_session_env("HBM_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("HBM_SESSION_SOURCE", ""))
...
def async_delivery_supported() -> bool:
    ...
    (``HBM_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
