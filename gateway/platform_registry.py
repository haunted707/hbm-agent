...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``hbm setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``hbm gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved HBM_HOME and overlay the
        ...
        # `hbm` invocation, so the import happens only when a lookup asks for it.
