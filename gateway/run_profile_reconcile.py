...
verb (``hbm_cli/profiles.py`` create/delete fire it through the control socket) and by the supervised
...
class GatewayProfileReconcileMixin:
    ...
    async def _unserve_profile(self, name: str, home: "Path") -> None:
        ...
        with _log_suppressed(logging.DEBUG, "profile handle release failed", exc_info=True):
            from hbm_state_registry import close_all_under
...
def migrate_profile_identity_verb(runner):
    """Build the ``migrate-profile-identity`` control-verb handler for ``hbm profile rename``
