...
``validate_media_delivery_path`` reads ``HBM_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``HBM_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``HBM_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``hbm cron run``, ``hbm send``) calls
...
_FLAG_ENVS = (("strict", "HBM_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "HBM_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "HBM_MEDIA_ALLOW_DIRS"
_TRUST_RECENT_SECONDS_ENV = "HBM_MEDIA_TRUST_RECENT_SECONDS"
_TRUTHY = frozenset({"1", "true", "yes", "on"})


def _routed_gateway_cfg() -> Optional[Dict[str, Any]]:
    """``gateway`` section of the ROUTED profile's config when a HBM_HOME override is active
