...
class GatewayKanbanWatchersMixin:
    ...
    def _kanban_dispatcher_boot(self) -> Optional[tuple]:
        ...
        try:
            ...
            return None
        env_override = os.environ.get("HBM_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via HBM_KANBAN_DISPATCH_IN_GATEWAY env")
            return None
        try:
            cfg = _load_config()
        except Exception as exc:
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        loop exits and an external `hbm kanban daemon` is expected. Each
        ...
        while self._running:
            ...
            try:
                # Emergency stop (`hbm pause`): no auto-decompose or
