...
``<HBM_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_hbm_home() -> Path:
    """HBM_HOME for process-level identity files (ignore task overrides)."""
    from hbm_constants import get_hbm_home

    val = os.environ.get("HBM_HOME", "").strip()
    return Path(val) if val else get_hbm_home()


def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_hbm_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<HBM_HOME>/state/gateway.lifecycle.json``."""
...
def _report_unclean_exit(evidence: Dict[str, Any], home: Optional[Path]) -> None:
    ...
    if verdict not in ("ok", "absent"):
        logger.error(
            ...
            "missing until it is repaired. Run `hbm doctor`.",
            verdict,
        )
