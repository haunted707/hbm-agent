...
class GatewayGoalsMixin:
    ...
    def _goal_max_turns_from_config(self) -> int:
        ...
        top-level ``goals`` block is only reachable via hbm_cli.config.load_config()."""
        try:
            goals_cfg = (
    ...
    async def _warm_goals_session_db(self, label: str) -> None:
        ...
        try:
            from hbm_cli.goals import _get_session_db as _warm_goals_db
    ...
    async def _get_heartbeat_manager_for_event(self, event: "MessageEvent"):
        ...
        def _load():
            from hbm_cli.heartbeat import HeartbeatManager
    ...
    def _start_heartbeat_poller(self) -> None:
        ...
        from hbm_cli.heartbeat import POLL_SECONDS
    ...
    async def _defer_goal_status_notice_after_delivery(self, source: Any, message: str) -> None:
        ...
        if session_key and hasattr(adapter, "register_post_delivery_callback"):
            try:
                ...
                generation = getattr(active, "_hbm_run_generation", None) if active is not None else None
    ...
    async def _post_turn_goal_continuation(
        ...
        with suppress(Exception):
            from hbm_cli.goals import count_active_delegations, gather_background_processes as _gather_bg
    ...
    async def _post_turn_loop_completion(
        ...
        # contextvars: a bare executor hop drops the profile HBM_HOME override and secret scope,
    ...
    async def _loop_wakeup_fire_one(
        ...
        from hbm_cli.loops import LoopManager, goal_blocks_loop_tick
        ...
        # fired. The context-preserving executor keeps the profile HBM_HOME override under multiplex.
    ...
    async def _loop_wakeup_watcher(self, interval: float = 15.0) -> None:
        ...
        async def _scan_one_store(profile_name: Optional[str]) -> None:
            from hbm_cli.loops import list_active_loops
