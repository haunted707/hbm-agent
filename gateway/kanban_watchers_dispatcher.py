...
def _kbc():
    from hbm_cli import kanban_db_connect
    return kanban_db_connect


def _kbd():
    from hbm_cli import kanban_db_dispatch
...
class _KanbanDispatcher:
        ...
        try:
            ...
            if self.is_corrupt_board_db_error(exc):
                ...
                logger.error(
                    ...
                    "then run `hbm kanban init` if you need a fresh board.",
                    slug, fingerprint[0],
                )
                return None
    ...
    def auto_decompose_tick(self, auto_decompose_per_tick: int) -> int:
        ...
        """
        try:
            from hbm_cli import kanban_decompose as _decomp
        ...
        with _default_profile_secret_scope():
            for slug in self._board_slugs():
                ...
                prev_env = os.environ.get("HBM_KANBAN_BOARD")
                try:
                    os.environ["HBM_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("HBM_KANBAN_BOARD", None)
                    else:
                        os.environ["HBM_KANBAN_BOARD"] = prev_env
        return successes
...
def _default_profile_secret_scope():
    ...
    LLM reads ``auxiliary.*`` from ``get_hbm_home()``, so its credentials come
    ...
    if not is_multiplex_active():
        yield
        return
    token = set_secret_scope(build_profile_secret_scope(Path(get_hbm_home())))
    try:
        yield
    finally:
