...
SCALE_TO_ZERO_ENV = "HBM_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(hbm_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under HBM_HOME."""
    if hbm_home is None:
        from hbm_constants import get_hbm_home
        hbm_home = get_hbm_home()
    return Path(hbm_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
