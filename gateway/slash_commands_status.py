...
from hbm_cli.status_report import build_status_fields
...
HISTORY_UNREADABLE = ("⚠️ I can't read this conversation's history right now (your earlier messages "
                      "exist but cannot be loaded). Run `hbm doctor --fix` on the host, or use /new "
                      "to start fresh.")
...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Not a bare hop: ``SessionDB()`` resolves ``get_hbm_home()`` at call time, a
