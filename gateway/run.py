...
# hbm_bootstrap must be the very first import (UTF-8 stdio on Windows; no-op on POSIX).
try:
    import hbm_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass  # a partial ``hbm update`` can leave the bootstrap unregistered; only Windows UTF-8 stdio suffers
...
from hbm_cli.config import _is_ssh_remote_tilde_cwd, cfg_get
...
def _hygiene_compression_timeout_message(
    *, total_exhausted: bool, elapsed: float, idle_timeout: float, progress_observed: bool) -> str:
    ...
    model config, so the copy names /compress, /new and `hbm doctor`, never a config key or the
    ...
    if total_exhausted:
        return lead
    return lead + " If this keeps happening, run `hbm doctor` on the host."
...
async def run_codex_hygiene_compaction(
    ...
    * Evicting the cached live agent afterwards destroys the only real context: the next turn spawns an
    EMPTY thread and the model starts blank while HBM AGENT still mirrors a full history (abrupt amnesia — the
    user-facing damage documented on #73503).
    """
    mode = str(auto_mode or "native").lower()
    if mode not in {"native", "hbm", "off"}:
        mode = "native"
    if mode != "hbm":
    ...
    # copy_context carries profile secret scope / HBM_HOME override (executors don't propagate ContextVars).
...
def _ensure_windows_gateway_venv_imports() -> None:
    """Make detached Windows gateway runs see the HBM AGENT venv packages.
...
# slash command the chat user can run; raw provider text stays in the gateway log (`hbm logs`).
_PROVIDER_ERROR_REPLIES = (
    (_GATEWAY_AUTH_ERROR_RE, "⚠️ Sign-in to the AI model service failed. Use /login to sign in again, "
                             "or ask whoever runs this bot to run `hbm doctor` on the host."),
    ...
    (_GATEWAY_CONNECTION_ERROR_RE, "⚠️ The AI model service isn't reachable right now — the configured model "
                                   ...
                                   "if it persists, run `hbm doctor` on the host."))
...
def _gateway_provider_error_reply(text: str) -> str:
    ...
    return (
        ...
        "models. Details are in the gateway log (`hbm logs`).")
...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from hbm_cli.commands_platforms import _sanitize_telegram_name
...
def _coerce_gateway_timestamp(value: Any) -> Optional[float]:
    ...
    if isinstance(value, (int, float)):
        # Some platform events use milliseconds; HBM AGENT state rows use seconds.
...
def _startup_restore_drain_timeout_secs() -> float:
    ...
    """
    return _float_env("HBM_STARTUP_RESTORE_DRAIN_TIMEOUT", _STARTUP_RESTORE_DRAIN_TIMEOUT_SECS_DEFAULT)
...
def _startup_warmup_timeout_secs() -> float:
    ...
    return _float_env("HBM_STARTUP_WARMUP_TIMEOUT", _STARTUP_WARMUP_TIMEOUT_SECS_DEFAULT)
...
def _warm_turn_machinery_sync() -> int:
    ...
    from hbm_cli.config import load_config_readonly
...
def _build_gateway_agent_history(
    ...
    from hbm_time import get_timezone as _get_msg_tz
...
def _restart_notification_pending() -> bool:
    ...
    return (_hbm_home / ".restart_notify.json").exists()


def _planned_restart_notification_path() -> Path:
    return _hbm_home / ".restart_pending.json"
...
os.environ["_HBM_GATEWAY"] = "1"

_ensure_ssl_certs()

sys.path.insert(0, str(Path(__file__).parent.parent))

from hbm_constants import get_hbm_home, get_hbm_home_override
_hbm_home = get_hbm_home()

# Load ~/.hbm/.env first: user-managed env files must override stale shell exports on restart.
from hbm_cli.env_loader import load_hbm_dotenv
_env_path = _hbm_home / '.env'
load_hbm_dotenv(hbm_home=_hbm_home, project_env=Path(__file__).resolve().parents[1] / '.env')


def _reload_runtime_env_preserving_config_authority() -> None:
    """Reload .env per turn for rotated keys while config.yaml stays authoritative for budgets (else a
    stale HBM_MAX_ITERATIONS wins). Multiplex never reloads .env globally: secrets come from the
    ...
    if not is_multiplex_active():
        load_hbm_dotenv(
            hbm_home=_hbm_home, project_env=Path(__file__).resolve().parents[1] / '.env')
    _bridge_max_turns_from_config(_hbm_home)
...
def _bridge_max_turns_from_config(home: "Path") -> None:
    ...
    hbm_state reads the routed profile's ``sessions.*`` from its own config under scope."""
...
def _current_max_iterations() -> int:
    ...
    ``int()`` crash. A routed profile (HBM_HOME override, multiplexed turns) reads ITS
    ``agent.max_turns`` straight from config: the ``HBM_MAX_ITERATIONS`` bridge is one process-wide
    slot holding the launch profile's value, so every secondary would inherit the default's budget."""
    _reload_runtime_env_preserving_config_authority()
    from hbm_cli.config import resolve_turn_limit as _resolve_turn_limit
    override = get_hbm_home_override()
    if override:
    ...
    return _resolve_turn_limit(os.getenv("HBM_MAX_ITERATIONS"))
...
def _multiplex_profile_homes(config: object) -> list[tuple[str, "Path"]]:
    ...
    from hbm_cli.profiles import profiles_to_serve
...
def _cron_tick_profile_homes(config: object) -> list[tuple[str, "Path"]]:
    ...
    HBM_HOME). Adapter startup already skips ``active``."""
    from hbm_cli.profiles import get_active_profile_name, get_profile_dir
...
def _load_profile_secret_scope(profile_home: "Path") -> dict:
    ...
    from hbm_constants import set_hbm_home_override, reset_hbm_home_override
    # Caller already hydrated external sources off-loop (#99519).
    from agent.secret_scope import build_profile_secret_scope
    from hbm_cli.env_loader import hydrate_profile_secret_sources

    home_token = set_hbm_home_override(str(profile_home))
    try:
        hydrate_profile_secret_sources(Path(profile_home))
        return build_profile_secret_scope(Path(profile_home))
    finally:
        reset_hbm_home_override(home_token)
...
def _profile_runtime_scope(
    ...
    ``set_hbm_home_override`` is a contextvar (reaches the agent worker via ``copy_context()``);
...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    ``mcp_servers`` from ``get_hbm_home()``'s config, so an unscoped call only connects the launch
    ...
    actionable ``hbm mcp login`` warning instead of opening an authorize tab.
    """
...
def _platform_has_bot_credential(platform: "Platform", platform_config: "PlatformConfig") -> bool:
    ...
    # ~/.hbm/.env, so an env fallback would report "has credential" for every Matrix config on the box —
...
from hbm_cli.config_defaults import DEFAULT_CONFIG as _DEFAULT_CONFIG
os.environ["HBM_TURN_LEASE_TIMEOUT"] = str(_DEFAULT_CONFIG["agent"]["gateway_turn_lease_timeout"])
...
_AGENT_ENV_BRIDGE = {
    "gateway_timeout": "HBM_AGENT_TIMEOUT",
    "gateway_turn_lease_timeout": "HBM_TURN_LEASE_TIMEOUT",
    "gateway_timeout_warning": "HBM_AGENT_TIMEOUT_WARNING",
    "gateway_notify_interval": "HBM_AGENT_NOTIFY_INTERVAL",
    "session_stall_timeout": "HBM_SESSION_STALL_TIMEOUT",
    # Internal bridge only — config.yaml (agent.reconnect_attention_after) is the documented setting.
    "reconnect_attention_after": "HBM_RECONNECT_ATTENTION_AFTER_SECONDS",
    "restart_drain_timeout": "HBM_RESTART_DRAIN_TIMEOUT",
    "cron_drain_timeout": "HBM_CRON_DRAIN_TIMEOUT",
    "gateway_auto_continue_freshness": "HBM_AUTO_CONTINUE_FRESHNESS",
    "gateway_startup_restore_drain_timeout": "HBM_STARTUP_RESTORE_DRAIN_TIMEOUT",
    "gateway_startup_warmup_timeout": "HBM_STARTUP_WARMUP_TIMEOUT"}
# config-authoritative knobs for the session-search index (env stays the cross-process carrier).
_SESSIONS_ENV_BRIDGE = {"cjk_fts": "HBM_CJK_FTS", "search_slow_ms": "HBM_SEARCH_SLOW_MS"}
_DISPLAY_ENV_BRIDGE = {
    "busy_input_mode": "HBM_GATEWAY_BUSY_INPUT_MODE",
    "busy_text_mode": "HBM_GATEWAY_BUSY_TEXT_MODE",
    "busy_ack_enabled": "HBM_GATEWAY_BUSY_ACK_ENABLED"}
...
def _bridge_max_turns_to_env(agent_cfg: Any) -> None:
    ...
    if raw is not None:
        os.environ["HBM_MAX_ITERATIONS"] = str(raw)
    elif "HBM_MAX_ITERATIONS" in os.environ:
        del os.environ["HBM_MAX_ITERATIONS"]
...
def _bridge_auxiliary_config_to_env(_auxiliary_cfg: dict) -> None:
    ...
    try:
        from hbm_cli.plugins import get_plugin_auxiliary_tasks
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    # (e.g. HBM_MAX_ITERATIONS=60 written by an old `hbm setup` run) silently shadow the user's
    ...
    if (isinstance(_display_cfg, dict) and "busy_steer_ack_enabled" in _display_cfg
            and "HBM_GATEWAY_BUSY_STEER_ACK_ENABLED" not in os.environ):
        os.environ["HBM_GATEWAY_BUSY_STEER_ACK_ENABLED"] = str(_display_cfg["busy_steer_ack_enabled"])
    _tz_cfg = _cfg.get("timezone", "")
    if _tz_cfg and isinstance(_tz_cfg, str):
        os.environ["HBM_TIMEZONE"] = _tz_cfg.strip()
    _security_cfg = _cfg.get("security", {})
    if isinstance(_security_cfg, dict) and _security_cfg.get("redact_secrets") is not None:
        os.environ["HBM_REDACT_SECRETS"] = str(_security_cfg["redact_secrets"]).lower()
    # Media policy uses the shared bridge so standalone entrypoints (`hbm cron run`) match.
    ...
    if isinstance(_gateway_cfg, dict):
        ...
        if _trust_recent_seconds is not None:
            os.environ["HBM_MEDIA_TRUST_RECENT_SECONDS"] = str(_trust_recent_seconds)
        ...
        if ("platform_connect_timeout" in _gateway_cfg
                and not os.environ.get("HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()):
            os.environ["HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT"] = str(_gateway_cfg["platform_connect_timeout"])
...
def _load_bridge_config(config_path: Path) -> dict:
    ...
    from hbm_cli.config_effective import load_user_config_effective
    return load_user_config_effective(config_path)


_config_path = _hbm_home / 'config.yaml'
_cfg: dict = {}
if _config_path.exists():
    try:
        ...
        print(
            ...
            "your current config.yaml. Run `hbm doctor` to investigate.",
            file=sys.stderr)

# IPv4 preference must apply before any HTTP clients are created.
try:
    from hbm_constants import apply_ipv4_preference
...
try:
    from hbm_cli.config import print_config_warnings
    print_config_warnings()
except Exception as _bootstrap_exc:
    print(f"  Warning: config validation failed: {_bootstrap_exc}", file=sys.stderr)

try:
    from hbm_cli.config import warn_deprecated_cwd_env_vars
    warn_deprecated_cwd_env_vars()
except Exception as _bootstrap_exc:
    print(f"  Warning: deprecation check failed: {_bootstrap_exc}", file=sys.stderr)

os.environ["HBM_QUIET"] = "1"  # gateway runs quiet: no debug output, cwd used directly

# HBM_EXEC_ASK is set in start_gateway(), NOT at import: CLI tools importing this module must not
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from hbm_cli.runtime_provider import (
        resolve_runtime_provider, format_runtime_provider_error, _get_model_config)
    from hbm_cli.auth import AuthError, is_rate_limited_auth_error
...
def _resolve_gateway_model_context(
    ...
    def _read_config() -> None:
        ...
        try:
            from hbm_cli.config import get_compatible_custom_providers
    ...
    def _pin_still_applies() -> bool:
        ...
        from hbm_cli.route_identity import should_clear_context_pin
    ...
    def _custom_ctx() -> Optional[int]:
        from hbm_cli.config import get_custom_provider_context_length
...
def _resolve_runtime_agent_kwargs_for_provider(provider: str) -> dict:
    ...
    from hbm_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error
    try:
...
def _deep_merge_request_overrides(base: Optional[dict], override: Optional[dict]) -> dict:
    ...
    from hbm_cli.config import _deep_merge
...
def _try_resolve_fallback_provider() -> dict | None:
    ...
    from hbm_cli.runtime_provider import resolve_runtime_provider
    try:
        ...
        for entry in fb_list:
            try:
                from hbm_cli.fallback_config import effective_runtime_provider, resolve_entry_api_key
                runtime = resolve_runtime_provider(
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if slug == normalized and declared_name in disabled:
                    return (
                        ...
                        f"Enable it with: `hbm skills config`")

        # Check optional skills (shipped with repo but not installed)
        from hbm_constants import get_optional_skills_dir
        ...
        if optional_dir.exists():
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                return (
                    ...
                    f"Install it with: `hbm skills install {install_path}`")
    except Exception:
        pass
    return None
...
def _gateway_config_home() -> Path:
    """Return the HBM AGENT home that gateway config reads should use."""
    override = get_hbm_home_override()
    return Path(override) if override else _hbm_home
...
def _load_gateway_config(config_path: "Path | None" = None) -> dict:
    ...
    (``_hbm_home`` monkeypatches apply); multiplexers pass a path.
    """
...
def _checkpoint_agent_kwargs(config: dict | None) -> dict:
    ...
    if isinstance(cp_cfg, bool):
        ...
        cp_cfg = {}
    from hbm_cli.config import DEFAULT_CONFIG
...
def _resolve_hbm_bin() -> Optional[list[str]]:
    """HBM AGENT update/restart argv: the running interpreter's ``python -m hbm_cli.main``
    (exactly this install), else ``hbm`` on PATH, else None. The module argv must win: a
    PATH-first lookup lets an attacker-planted ``hbm`` shadow the running install when
    /update or /restart re-execs it (#111569)."""
    try:
        import importlib.util
        if importlib.util.find_spec("hbm_cli") is not None:
            return [sys.executable, "-m", "hbm_cli.main"]
    except Exception:
        pass
    import shutil
    hbm_bin = shutil.which("hbm-agent")
    if hbm_bin:
        return [hbm_bin]
    return None
...
_RECONNECT_ATTENTION_AFTER_SECONDS = _float_env("HBM_RECONNECT_ATTENTION_AFTER_SECONDS", 7200)
...
_BUILTIN_ADAPTERS: dict[Platform, tuple[str, str, str, str]] = {
    Platform.WHATSAPP_CLOUD: ("whatsapp_cloud", "WhatsAppCloudAdapter", "check_whatsapp_cloud_requirements",
                              "WhatsApp Cloud: aiohttp/httpx missing — reinstall hbm-agent"),
...
class GatewayRunner(
    ...
    def _init_startup_checks(self) -> None:
        ...
        try:
            from hbm_cli.config import load_config as _load_full_config
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            # WARNING (not DEBUG) so it lands in errors.log; else an NFS HBM_HOME silently loses /resume etc.
        ...
        # restart of the day (the ``hbm update`` one) looked hung and failed fleet verification.
    ...
    def _open_session_db_for_active_scope(self, raise_on_error: bool = False) -> Any:
        ...
        ``SessionDB()`` reads the context-local HBM_HOME; one handle cached per path. Construction
        ...
        resolves ``_default_db_path()`` at call time through the context-local HBM_HOME override
        ...
        #88235 broadcast.
        """
        from hbm_state import AsyncSessionDB, _default_db_path
        ...
        def _open():
            ...
            if borrowed is not None:
                ...
                wrapper.__dict__["_hbm_borrowed_handle"] = True
                return wrapper
    ...
    def close_all_session_db_handles(self) -> None:
        ...
        """
        def _close(db) -> None:
            if getattr(db, "__dict__", {}).get("_hbm_borrowed_handle"):
                return
            ...
            # instead (#90837).
            from hbm_state_registry import release_or_close
            try:
    ...
    def _warn_if_docker_media_delivery_is_risky(self) -> None:
        ...
        logger.warning(
            ...
            "output mount (for example '/home/user/.hbm/cache/documents:/output') is configured. "
            "This is fine if the model already emits host-visible paths, but MEDIA file delivery can fail "
            "for container-local paths like '/workspace/...' or '/output/...'.")

    _VOICE_MODE_PATH = _hbm_home / "gateway_voice_mode.json"
    ...
    def _update_runtime_status(self, gateway_state: Optional[str] = None, exit_reason: Optional[str] = None) -> None:
        # ``active_work`` names each unit only while draining — that is when an observer (``hbm
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10, thread_name_prefix="hbm-gateway")
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's HBM_HOME serves this source: ``source.profile``, then
        ``_profile_name_for_source`` (sources bypassing ``build_source``), then the active profile."""
        from gateway.profile_routing import ProfileRouteRejected
        from hbm_cli.profiles import get_active_profile_name, get_profile_dir, profile_exists
        ...
        try:
            ...
            if explicit_profile and not profile_exists(name):
                logger.warning(
                    ...
                    "falling back to global HBM_HOME",
            ...
            logger.warning(
                ...
                "falling back to global HBM_HOME: %s",
...
def _run_planned_stop_watcher(
    ...
    ``add_signal_handler``, so ``hbm gateway stop`` would never drain). Runs everywhere; on POSIX
    ...
    standard signal-driven shutdown path never runs when ``hbm gateway stop`` signals the gateway. The
...
def _housekeeping_paste_sweep() -> None:
    from hbm_cli.debug import _sweep_expired_pastes
...
def _housekeeping_auto_archive() -> None:
    ...
    from hbm_cli.config import load_config as _load_full_config
    from hbm_state_registry import acquire, release_or_close
    _sess_cfg = (_load_full_config().get("sessions") or {})
    if _sess_cfg.get("auto_archive", False):
        _adb = acquire()
        try:
...
def _housekeeping_deferred_fts_retry() -> None:
    ...
    from hbm_state_registry import borrow_live_shared_session_dbs
...
def _housekeeping_memory_trim() -> None:
    ...
    from hbm_cli.mem_trim import trim_memory
...
def _drain_restart_safe_cron_deliveries(adapters, loop, runner=None) -> None:
    ...
    for profile_name, profile_home in _handoff_watch_scopes(runner):
        ...
        if profile_adapters is None:
            continue
        with _profile_runtime_scope(profile_home or get_hbm_home()):
...
def _replace_target_belongs_to_other_profile(existing_pid: int) -> bool:
    ...
    by exact PID + start-time; live argv can never PROVE ownership (no HBM_HOME), it is only a
    ...
    # (no service supervisor — e.g. `hbm gateway run` on Windows) is at risk: its PID never joins the
    ...
    try:
        from gateway.status import (
            _get_pid_path, _get_process_hbm_home, _get_process_start_time, _pid_from_record,
            _read_pid_record, _record_looks_like_gateway, _read_process_cmdline, _same_hbm_home)
        our_home = _get_process_hbm_home()
        ...
        if _get_process_start_time(existing_pid) != recorded_start:
            return refuse("pid record start-time does not match the live process %s (stale/PID-reuse record).",
                          existing_pid)
        recorded_home = record.get("hbm_home")
        if not isinstance(recorded_home, str) or not recorded_home.strip():
            return refuse("pid record predates hbm_home stampings; ownership of PID %s unprovable.",
                          existing_pid)
        if not _same_hbm_home(recorded_home, our_home):
            return refuse("pid record belongs to a different HBM_HOME (%s, ours %s). Remove the stale PID "
                          "record or stop the owning profile explicitly.", recorded_home, our_home,
                          level=logging.ERROR)
        # Argv never proves ownership; an explicit contradicting --profile / HBM_HOME= still refuses.
        ...
        if live_cmdline and _looks_like_profile_conflict_from_cmdline(live_cmdline, our_home):
            return refuse("target PID %s command line explicitly advertises a different profile than "
                          "HBM_HOME %s.", existing_pid, our_home, level=logging.ERROR)
        return False
    except Exception:
...
def _looks_like_profile_conflict_from_cmdline(command: str, our_home) -> bool:
    ...
    def _env_home_value() -> Optional[str]:
        """HBM_HOME=<path> env-style assignment on the argv, token-exact."""
        prefix = "HBM_HOME="
    ...
    for flag in ("--profile", "-p"):
        ...
        if profile_name is None or profile_name == "default" or value != profile_name:
            return True
    home_value = _flag_value("--hbm-home") or _env_home_value()
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    """Handle a live gateway PID under this HBM_HOME: replace it (``--replace``) or refuse.
    ...
    if not replace:
        hbm_home = str(get_hbm_home())
        logger.error(
            "Another gateway instance is already running (PID %d, HBM_HOME=%s). "
            "Use 'hbm gateway restart' to replace it, or 'hbm gateway stop' first.",
            existing_pid, hbm_home)
        print(
            f"\n❌ Gateway already running (PID {existing_pid}).\n"
            f"   Use 'hbm gateway restart' to replace it,\n"
            f"   or 'hbm gateway stop' to kill it first.\n"
            f"   Or use 'hbm gateway run --replace' to auto-replace.\n")
        return False
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        from gateway.status import _get_process_hbm_home
        logger.error(
            "Refusing --replace: PID %d cannot be proven to belong "
            "to this profile's gateway (HBM_HOME %s). Remove the "
            "stale PID record or stop the owning profile explicitly.",
            existing_pid, _get_process_hbm_home())
        return False
    ...
    with suppress(Exception):
        (get_hbm_home() / "gateway.pid").unlink(missing_ok=True)
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    from hbm_logging import setup_logging, _safe_stderr
    setup_logging(hbm_home=_hbm_home, mode="gateway")

    def _security_audit() -> None:
        # Warn-on-load, never blocks: surfaces root / weak-SSH / unauthenticated-listener exposure.
        from hbm_cli.security_audit_startup import log_startup_security_warnings

        def _raw_cfg():
            from hbm_cli.config import read_raw_config
            return read_raw_config()

        log_startup_security_warnings(hbm_home=_hbm_home, config=_best_effort(_raw_cfg))
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _hbm_home / "logs" / "gateway-shutdown-diag.log", _shutdown_ctx["signal"], timeout_seconds=5.0)
...
async def _start_gateway_start_control_socket(runner):
    ...
    try:
        ...
        # becomes the authoritative gateway for its HBM_HOME, so from here on "does a socket answer?" is
        ...
        def _pause_for_update_handler() -> dict:
            try:
                from hbm_cli.gateway import _get_restart_drain_timeout
        ...
        def _rescan_profiles_handler() -> dict:
            """``hbm profile create/delete`` asks the multiplexer to reconcile ``profiles/`` now
    ...
    if not isinstance(cron_provider, InProcessCronScheduler):
        ...
        if not _has_api_server:
            logger.warning(
                ...
                "the gateway through its supervisor (`hbm gateway "
...
async def _start_gateway_shutdown_tail(
    ...
    def _stop_keepalive() -> None:
        from hbm_cli.nous_auth_keepalive import stop_nous_auth_keepalive
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duplicate-instance guard scoped to HBM_HOME; distinct-home multi-profile setups coexist.
    ...
    # Windows has no add_signal_handler, so `hbm gateway stop`'s SIGTERM would never drain; poll the
    # planned-stop marker (written BEFORE the kill) instead. Runs everywhere so masked-SIGTERM drains.
    # Windows fallback: asyncio.add_signal_handler raises NotImplementedError on Windows, so `hbm gateway
    ...
    # sessions are silently lost across restarts (issue #33778). The fix is a marker-polling thread: `hbm
...
def _guard_corrupt_user_config() -> None:
    ...
    named. Same policy and escape hatch (``HBM_IGNORE_USER_CONFIG=1``) as ``hbm_cli/main.py``."""
    from hbm_cli.config import InvalidUserConfigError, require_parseable_user_config
...
def main():
    ...
    # Advertise the harness to children (mirrors _advertise_agent_env in hbm_cli/main.py, inlined to
    # avoid its startup side effects). Value must equal registry id ``hbm-agent`` exactly.
    ...
    def _arm_watchdog() -> None:
        ...
        from hbm_startup_watchdog import arm_startup_watchdog
    ...
    import argparse
    parser = argparse.ArgumentParser(description="HBM AGENT Gateway - Multi-platform messaging")
...
def _exit_after_graceful_shutdown(exit_code: int) -> None:
    ...
    def _drain_logs() -> None:
        ...
        from hbm_logging import drain_log_queue
