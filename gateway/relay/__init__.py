"""Relay/connector support package for the HBM AGENT gateway.
...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_hbm_home())
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from hbm_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**HBM AGENT:**" and shadow the connector's
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``hbm gateway enroll``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.hbm/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'hbm'}"
