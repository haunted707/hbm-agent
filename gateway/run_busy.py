...
class GatewayBusySessionMixin:
    ...
    def _get_max_concurrent_sessions(self) -> Optional[int]:
        ...
        try:
            from hbm_cli.active_sessions import resolve_max_concurrent_sessions
    ...
    def _active_session_limit_message(self, session_key: str) -> Optional[str]:
        ...
        if active_count < max_sessions:
            return None
        from hbm_cli.active_sessions import active_session_limit_message
    ...
    _SECURITY_METADATA_KEYS = (
        "hbm_plugin_id", "hbm_plugin_injection", "gateway_session_key",
    ...
    def _busy_steer_ack_enabled(self, event: MessageEvent, session_key: str) -> bool:
        ...
        steer_ack_env = os.environ.get("HBM_GATEWAY_BUSY_STEER_ACK_ENABLED")
    ...
    def _compose_busy_ack_message(
        ...
        from gateway.run import (
            _AGENT_PENDING_SENTINEL, _hbm_home, _load_gateway_config, _platform_config_key
        )
        from gateway.display_config import resolve_display_setting
    ...
    async def _handle_active_session_busy_message(self, event: MessageEvent, session_key: str) -> bool:
        ...
        if os.environ.get("HBM_GATEWAY_BUSY_ACK_ENABLED", "true").lower() != "true":
    ...
    # busy_handler key (hbm_cli/commands.py CommandDef) → mid-run variant ``_busy_<key>_command``.
    ...
    async def _handle_pause_command(self, event: MessageEvent):
        ...
        if args.lower() in {"off", "resume", "stop", "disengage"}:
            ...
            return "HBM AGENT wasn't paused."
        state = estop.get_state()
        if state is not None and not args:
            suffix = f" (reason: {state.get('reason')})" if state.get("reason") else ""
            return f"⏸️ HBM AGENT is already paused{suffix}. Use `/pause off` to resume."
    ...
    async def _busy_goal_command(self, event: MessageEvent, quick_key: str, source):
        ...
        from hbm_cli.goal_command import is_goal_control
    ...
    def _is_stale_restart_redelivery(self, event: MessageEvent) -> bool:
        ...
        try:
            marker_path = _hbm_home / ".restart_last_processed.json"
    ...
    async def _handle_blueprint_command(self, event: MessageEvent):
        ...
        try:
            ...
            from hbm_cli.blueprint_cmd import BlueprintCommandResult
