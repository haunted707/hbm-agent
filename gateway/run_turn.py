...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        # Provider resolved but no model.default (`hbm auth add` without `hbm model`): use the
    ...
    def _resolve_turn_agent_config(self, user_message: str, model: str, runtime_kwargs: dict) -> dict:
        ...
        from hbm_cli.models import resolve_fast_mode_overrides
    ...
    async def _hmwa_acquire_turn_lease(self, _quick_key, run_generation, session_entry, _session_env_tokens):
        ...
        try:
            _lease_token = await _lease_registry.acquire(
                ...
                timeout=_float_env("HBM_TURN_LEASE_TIMEOUT", DEFAULT_LEASE_WAIT),
            )
    ...
    async def _hmwa_hygiene_settings(self, source, session_key):
        ...
        try:
            ...
            if hs.config_context_length is not None:
                try:
                    from hbm_cli.route_identity import should_clear_context_pin_async
    ...
    async def _hmwa_hygiene_detached_attempt(
        ...
        try:
            ...
            attempt.future = loop.run_in_executor(
                ...
                # secret scope / HBM_HOME override live in ContextVars, and a bare run_in_executor worker
    ...
    async def _hmwa_first_contact_notes(self, source, history, turn_sidecar_notes):
        ...
        from gateway.run import _hbm_home, _home_target_env_var, _load_gateway_config
        if history:
            return
        if not await self.async_session_store.has_any_sessions():
            ...
            try:
                ...
                if profile_build_mode(_onb_cfg) == "ask" and not is_seen(_onb_cfg, PROFILE_BUILD_FLAG):
                    ...
                    mark_seen(_hbm_home / "config.yaml", PROFILE_BUILD_FLAG)
                else:
        ...
        if not home_env:
            with suppress(Exception):
                ...
                if prof and prof != "default" and _lgc().get_home_channel(source.platform):
                    home_env = "set"
        if not home_env:
            # Slack routes every command through the parent `/hbm`; bare `/sethome` would fail.
            sethome_cmd = "/hbm sethome" if source.platform == Platform.SLACK else "/sethome"
            await self._deliver_platform_notice(
                source, f"📬 No home channel is set for {platform_name.title()}. "
                f"A home channel is where HBM AGENT delivers cron job results and cross-platform "
    ...
    def _hmwa_apply_message_timestamp(self, event, message_text):
        ...
        try:
            from hbm_time import get_timezone as _get_evt_tz
    ...
    # Chat-side next steps keyed by HTTP status; HBM AGENT commands only (/login is the gateway's own
    # sign-in, `hbm auth add <provider>` the host equivalent).
    _STATUS_HINTS = {
        401: (" Your sign-in to the AI model service has expired or the API key is wrong. "
              "Use /login here, or run `hbm auth add <provider>` on the host."),
    ...
    async def _hmwa_agent_error_reply(self, e, event, source, session_entry, session_key, prepared):
        ...
        return self._hmwa_add_failed_turn_notice(
            ...
            "Technical details are in the gateway log (`hbm logs`).",
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    ...
                    "configured. Use /login, or run `hbm setup` on the host.",
    ...
    async def _execute_mcp_reload(self, event: MessageEvent) -> str:
        ...
        if multiplex and not get_hbm_home_override():
        ...
        try:
            ...
            # a browser OAuth flow either: an expired token parks with a `hbm mcp login` hint.
    ...
    async def _run_agent_via_proxy(
        ...
        """Forward the message to a remote HBM AGENT API server instead of running a local AIAgent.
        ...
        # OpenAI chat format. The remote keeps continuity via X-HBM AGENT-Session-Id; send the current
        ...
        body = {"model": "hbm-agent", "messages": api_messages, "stream": True}
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        progress_mode, _tool_progress_explicit = resolve_tool_progress(
            user_config, platform_key, os.getenv("HBM_TOOL_PROGRESS_MODE"),
        )
        ...
        # "log" mode: tool calls go to ~/.hbm/logs/tool_calls.log instead of the chat. Gateway-only.
    ...
    async def _run_agent_write_tool_log(self, log_queue: Any) -> None:
        ...
        tool_logger = logging.getLogger(f"hbm.tool_calls.{id(log_queue)}")
    ...
    def _run_agent_start_turn_worker(self, turn_ctx: TurnContext, run_sync: Callable[[], Any]) -> "GatewayRunner._RunAgentWorker":
        ...
        *Inactivity* timeout (agent.gateway_timeout / HBM_AGENT_TIMEOUT, env wins; 0 = unlimited),
        ...
        _agent_timeout, _agent_warning = (
            v if v > 0 else None
            for v in (_float_env("HBM_AGENT_TIMEOUT", 1800), _float_env("HBM_AGENT_TIMEOUT_WARNING", 900))
        )
    ...
    def _run_agent_evict_on_fallback(self, turn_ctx: TurnContext) -> None:
        ...
        with suppress(Exception):
            from hbm_cli.model_normalize import _AGGREGATOR_PROVIDERS, normalize_model_for_provider
    ...
    async def _run_agent_drain_pending(
        ...
        if pending and pending.strip().startswith("/"):
            ...
            if _pending_cmd_word:
                with suppress(Exception):
                    from hbm_cli.commands import resolve_command as _rc_pending
    ...
    async def _run_agent_notify_long_running(
        ...
        Interval: agent.gateway_notify_interval / HBM_AGENT_NOTIFY_INTERVAL (default 180s; 0 or
        ...
        _NOTIFY_INTERVAL = _float_env("HBM_AGENT_NOTIFY_INTERVAL", 180)
