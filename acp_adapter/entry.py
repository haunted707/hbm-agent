"""CLI entry point for the hbm-agent ACP adapter.

Loads ``~/.hbm/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: hbm-agent acp / hbm-agent-acp
"""

# IMPORTANT: hbm_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See hbm_bootstrap.py for full rationale.
try:
    import hbm_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``hbm-agent update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing HBM AGENT modules.
...
def _load_env() -> None:
    """Load .env from HBM_HOME (default ``~/.hbm``)."""
    from hbm_cli.env_loader import load_hbm_dotenv

    hbm_home = get_hbm_home()
    loaded = load_hbm_dotenv(hbm_home=hbm_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", hbm_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="hbm-agent-acp", description="Run HBM AGENT as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print HBM AGENT version and exit")
...
def _run_check() -> None:
    ...
    from acp_adapter.server import HbmACPAgent  # noqa: F401

    print("HBM AGENT ACP check OK")


def _run_setup() -> None:
    from hbm_cli.main import main as hbm_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "hbm-agent", "model"]
        hbm_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting hbm-agent ACP adapter")
    ...
    import acp
    from .server import HbmACPAgent
    ...
    if os.environ.get("HBM_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
