"""ACP auth helpers — detect and advertise HBM AGENT authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "hbm-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active HBM AGENT runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for HBM AGENT.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no HBM AGENT credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate HBM AGENT using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure HBM AGENT provider", type="terminal", args=["--setup"],
        description=("Open HBM AGENT' interactive model/provider setup in a terminal. "
                     "Use this when HBM AGENT has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if HBM AGENT can resolve any runtime provider credentials."""
