"""ACP session manager — maps ACP sessions to HBM AGENT AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.hbm/state.db``) so they
...
from hbm_constants import get_hbm_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    when HBM AGENT runs in WSL so agents, tools, and persisted sessions agree; no-op elsewhere."""
...
def _expand_acp_enabled_toolsets(toolsets: List[str] | None = None,
    ...
    names = [n for n in (toolsets or ["hbm-acp"]) if n]
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed HBM AGENT."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by HBM AGENT AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        the runtime provider config. ``db``: SessionDB; default lazily opens ``~/.hbm/state.db``."""
    ...
    def _get_db(self):
        ...
        error in a minimal test env). ``HBM_HOME`` is resolved here, not via the import-time
        ...
        if self._db_instance is None:
            try:
                ...
                self._db_instance = acquire(get_hbm_home() / "state.db")
            except Exception:
    ...
    def _make_agent(self, *, session_id: str, cwd: str, model: str | None = None,
        ...
        kwargs = {
            ...
            "enabled_toolsets": _expand_acp_enabled_toolsets(["hbm-acp"], mcp_server_names=configured_mcp_servers),
        ...
        # picked up by HbmACPAgent._schedule_mcp_late_refresh.
        try:
