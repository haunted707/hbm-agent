...
class SlashCommandsMixin:
    """Slash-command surface for ``HbmACPAgent``; relies on ``_conn``, ``_send``, ``_schedule_soon``,
    ...
    _COMMANDS: dict[str, tuple[str, str, str | None]] = {
        ...
        "version": ("Show HBM AGENT version", "Show HBM AGENT version", None),
    }
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # and ``/model`` REBUILD the system prompt, so unpinned they'd bake the HBM AGENT install tree
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            ...
            toolsets = _expand_acp_enabled_toolsets(getattr(state.agent, "enabled_toolsets", None) or ["hbm-acp"])
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"HBM AGENT v{HBM_VERSION}"
