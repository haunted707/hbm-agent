"""ACP agent server — exposes HBM AGENT via the Agent Client Protocol."""
...
from acp_adapter.commands import HBM_VERSION, SlashCommandsMixin, _estimate_tokens
...
from tools.approval_context import reset_hbm_interactive_context, set_hbm_interactive_context
...
def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
    ...
    if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
        ...
        kind = "standalone"
    if kind == "standalone":
        return {"hbm": {"compactionSummary": True}}
    if kind == "merged":
        return {"hbm": {"containsCompactionSummary": True}}
    return None
...
class HbmACPAgent(SlashCommandsMixin, acp.Agent):
    """ACP Agent implementation wrapping HBM AGENT AIAgent."""
    ...
    def _switch_model(
        ...
        Resolution goes through ``hbm_cli.model_switch.switch_model`` seeded with the live
        ...
        from hbm_cli.config import get_compatible_custom_providers, load_config
        from hbm_cli.model_switch import switch_model
        from hbm_cli.models import parse_model_input
    ...
    def _provenance_meta(
        self, acp_session_id: str, current_hbm_session_id: str, previous_hbm_session_id: Optional[str] = None
    ) -> Optional[dict]:
        """Best-effort ``_meta.hbm.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(), acp_session_id, current_hbm_session_id,
                previous_hbm_session_id=previous_hbm_session_id,
            )
        except Exception:
    ...
    async def _send_session_info_update(
        self, session_id: str, *,
        current_hbm_session_id: Optional[str] = None, previous_hbm_session_id: Optional[str] = None,
    ) -> None:
        """Session metadata update; pass ``previous_hbm_session_id`` when the internal head
        ...
        update = SessionInfoUpdate(
            ...
            field_meta=self._provenance_meta(
                session_id, current_hbm_session_id or session_id, previous_hbm_session_id
            ),
        )
        await self._send(
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            agent.enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(agent, "enabled_toolsets", None) or ["hbm-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        try:
            from hbm_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from hbm_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="hbm-agent", version=HBM_VERSION),
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``HBM_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # HBM_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the HBM AGENT workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global HBM_CRON_SESSION.
            ...
            stack.callback(reset_hbm_interactive_context, set_hbm_interactive_context(True))
            # Tools tag side-effects with the ACP session (``kanban_create``); save/restore it.
            stack.callback(_restore_env, "HBM_SESSION_ID", os.environ.get("HBM_SESSION_ID"))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        """Run HBM AGENT on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        try:
            ...
            pre_turn_hbm_id = getattr(state.agent, "session_id", None)
        ...
        return await self._finish_turn(state, session_id, conn, result, pre_turn_hbm_id, cbs.streamed)
    ...
    async def _finish_turn(
        self, state: SessionState, session_id: str, conn: Any, result: dict, pre_turn_hbm_id: Any,
        ...
        post_turn_hbm_id = getattr(state.agent, "session_id", None)
        if conn and post_turn_hbm_id and pre_turn_hbm_id and post_turn_hbm_id != pre_turn_hbm_id:
            try:
                await self._send_session_info_update(
                    session_id, current_hbm_session_id=post_turn_hbm_id,
                    previous_hbm_session_id=pre_turn_hbm_id,
                )
            except Exception:
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when HBM AGENT has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
