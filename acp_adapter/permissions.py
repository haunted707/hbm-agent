"""ACP permission bridging for HBM AGENT dangerous-command approvals."""
...
# ACP permission option id -> HBM AGENT approval result. Ids are stable across the
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True, smart_denied: bool = False,
    ...
    """Return ACP options that match HBM AGENT approval semantics."""
    ...
    # DENY override — offering a scope HBM AGENT discards would re-prompt every write.
    # See #81887.
    once_only = smart_denied or not allow_session
    options = [PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once")]
    if not once_only:
        # ACP has no session-scoped kind: closest persistent hint, HBM AGENT semantics in the id.
...
def _map_outcome_to_hbm(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into HBM AGENT approval strings."""
    ...
    if outcome.option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_HBM.get(outcome.option_id, "deny")
...
def make_approval_callback(request_permission_fn: Callable, loop: asyncio.AbstractEventLoop,
    ...
    """Return a HBM AGENT approval callback (``command, description, **kw`` as used by
    ...
    def _callback(command: str, description: str, *, allow_permanent: bool = True,
        ...
        if response is None:
            return "deny"
        return _map_outcome_to_hbm(response.outcome, allowed_option_ids={option.option_id for option in options})
