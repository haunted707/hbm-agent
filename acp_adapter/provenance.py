...
Additive HBM AGENT extension under ACP ``_meta.hbm`` (unknown to other clients,
...
rotates the internal HBM AGENT head, ``build_session_provenance`` exposes the
...
def build_session_provenance(
    db: Any, acp_session_id: str, current_hbm_session_id: str, *, previous_hbm_session_id: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Build ``_meta.hbm.sessionProvenance`` for an ACP session.

    ``db`` must expose ``get_session``. ``current_hbm_session_id`` is the live
    internal id (``state.agent.session_id``); ``previous_hbm_session_id`` is
    the id before the most recent turn, supplied by ``prompt()`` to flag a
    rotation. Returns ``None`` if the session can't be read."""
    row = _get_row(db, current_hbm_session_id)
    if not row:
        return None
    ...
    root_id, compression_depth, cursor_parent = current_hbm_session_id, 0, parent_id
    seen = {current_hbm_session_id}
    ...
    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id, "currentHbmSessionId": current_hbm_session_id,
        "rootHbmSessionId": root_id, "parentHbmSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root", "compressionDepth": compression_depth,
    }
    if previous_hbm_session_id:
        provenance["previousHbmSessionId"] = previous_hbm_session_id
        if previous_hbm_session_id != current_hbm_session_id:
...
def session_provenance_meta(
    ...
    """Return a ready ``_meta`` payload: ``{"hbm": {"sessionProvenance": ...}}``."""
    prov = build_session_provenance(db, acp_session_id, current_hbm_session_id,
                                    previous_hbm_session_id=previous_hbm_session_id)
    return None if prov is None else {"hbm": {"sessionProvenance": prov}}
