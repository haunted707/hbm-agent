...
# HBM AGENT tool name -> ACP ToolKind (anything unlisted is "other").
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a hbm-agent tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    Some HBM AGENT tools append a human hint after the payload (``{...}\\n\\n[Hint: ...]``),
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured HBM AGENT tool result clearly failed.
...
def build_tool_title(tool_name: str, args: Args) -> str:
    ...
    every other HBM AGENT surface, so ACP clients never show a different summary than the CLI/TUI;
...
def build_tool_start(tool_call_id: str, tool_name: str, arguments: Args, *, edit_diff: Any = None) -> ToolCallStart:
    """Create a ToolCallStart event for the given hbm-agent tool invocation.
