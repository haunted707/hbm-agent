...
- **`optional-skills/`** — heavier/niche skills shipped but NOT active; installed via
  `hbm skills install official/<category>/<skill>` (adapter `tools/skills_hub_official.py`
...
`[linux, macos]`, ...), `metadata.hbm.tags`, `metadata.hbm.category`,
`metadata.hbm.related_skills`, `metadata.hbm.config` (config.yaml settings the skill needs —
stored under `skills.config.<key>`, prompted during setup, injected at load). Top-level `tags:` /
`category:` are accepted and mirrored from `metadata.hbm.*` by the loader.
...
2. **Prose references native HBM AGENT tools or the MCP servers the skill expects, in backticks**
...
4. **`author` credits the human first.** External contributor's real name + GitHub handle first,
   "HBM AGENT" second. A commit authored as "HBM AGENT" (they drafted with HBM AGENT) is replaced
...
in the `hbm-agent-dev` skill.

## Curator (skill lifecycle)

Background maintenance that tracks usage on agent-created skills and auto-archives stale ones;
archives go to `~/.hbm/skills/.archive/` and are restorable. Core `agent/curator.py` (review
loop, auto-transitions, LLM review prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots);
CLI `hbm_cli/curator.py` → `hbm curator status|run|pause|resume|pin|unpin|archive|restore|
prune|backup|rollback`; telemetry `tools/skill_usage.py` owns `~/.hbm/skills/.usage.json`
