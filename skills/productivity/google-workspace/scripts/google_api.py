#!/usr/bin/env python3
"""Google Workspace API CLI for HBM AGENT.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing HBM AGENT-facing JSON contract and falls back to the Python client
...
from _hbm_home import get_hbm_home
...
def _gws_binary() -> str | None:
    override = os.getenv("HBM_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for HBM AGENT")
