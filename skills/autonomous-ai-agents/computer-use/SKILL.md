...
version: 2.1.0
author: Francesco Bonacci (f-trycua), HBM AGENT
license: MIT
...
HBM AGENT drives [cua-driver](https://github.com/trycua/cua) under the hood.
This skill teaches the HBM AGENT `computer_use` **action vocabulary**, which is
NOT the driver's raw MCP vocabulary. Call the actions documented below and
never the driver's tools by name: `capture` is an HBM AGENT action that maps to
the driver's `get_window_state`; `element=N` is an HBM AGENT argument that the
...
`mode` decides what HBM AGENT hands back to you, not what the driver does.
...
rejects image tool results), HBM AGENT routes the screenshot through the
...
|---|---|
| `cua-driver not installed` | Run `hbm computer-use install`, or `hbm tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `hbm computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| "bare element_index is not accepted" / `snapshot_id_required` | The driver saw a raw index without its token. This is a wrapper defect, not something you fix by passing `snapshot_id` (HBM AGENT has no such argument). Re-capture once; if it repeats, tell the user to run `hbm update` and fall back to `coordinate=[x, y]` from the capture's bounds meanwhile |
| "tool 'capture' has no reviewed risk classification" / `Unknown tool` | Something called the driver's MCP vocabulary directly (`capture`, `screenshot`, `get_window_state`, `click` with raw args). Only the `computer_use(action=…)` vocabulary in this file exists on the HBM AGENT side |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `hbm computer-use doctor` |
...
| `hbm computer-use doctor` says "could not be started … Access is denied" (Windows) | The HBM AGENT venv interpreter can't execute a binary under `C:\Program Files\WindowsApps`; the tool itself may still work because the shell resolves another copy on PATH. Fix once: reinstall cua-driver with the upstream installer (lands under the user profile) or set `HBM_CUA_DRIVER_CMD` to a copy outside `WindowsApps`. The same denial spams `errors.log` for any other `WindowsApps` binary HBM AGENT spawns (e.g. `bws.exe`) |
| Anything else weird | **First action: ask the user to run `hbm computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
HBM AGENT intentionally keeps THIS skill focused on the HBM AGENT-side
...
links the pack into `~/.hbm/skills/cua-driver` (HBM AGENT is a detected
...
calling the HBM AGENT actions from this file — the wrapper does the translation.
