...
Edit with `hbm config edit` or `hbm config set section.key value`.
...
`hbm config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `hbm tools` (interactive) or `hbm tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_HBM_CORE_TOOLS` is the default bundle most platforms inherit).
