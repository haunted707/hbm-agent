...
Set via `hbm model` (picker) or `hbm setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `hbm auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `hbm auth add openai-codex` |
| qwen-oauth | OAuth | `hbm auth add qwen-oauth` |
| minimax-oauth | OAuth | `hbm auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`hbm auth`).
Fallback chain when the primary fails: `hbm fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`hbm_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   hbm config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
