...
HBM AGENT runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`hbm config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually HBM AGENT's env scrubber dropping
...
HBM AGENT-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every HBM AGENT tool and most Windows APIs. Prefer forward slashes in code
