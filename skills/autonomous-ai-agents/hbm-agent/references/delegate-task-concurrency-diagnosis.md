...
**three** code paths in HBM AGENT that cap a batch. If none of them fired, the
cap came from the **model itself** — not from HBM AGENT — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in HBM AGENT
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "HBM AGENT is
   refusing my value."
...
hbm config get delegation.max_concurrent_children

# 2. Did HBM AGENT's truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.hbm/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with hbm on path)
...
appears, **the cap is the model**, not HBM AGENT.
