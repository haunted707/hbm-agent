...
Config: `~/.hbm/config.yaml` (settings), `~/.hbm/.env` (API keys) — both under `$HBM_HOME` when it is set.
...
`_HBM_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_hbm_home()` for paths,
never hardcode `~/.hbm`. For custom/local-only tools, write a plugin in
`~/.hbm/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `hbm_cli/commands.py`
...
- Tests auto-redirect `HBM_HOME` to temp dirs — never touch real `~/.hbm/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/hbm_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_hbm_home()` from `hbm_constants` for all paths (profile-safe)
