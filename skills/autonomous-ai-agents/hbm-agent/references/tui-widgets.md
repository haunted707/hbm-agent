...
Author widget apps for the HBM AGENT TUI (`hbm --tui`): glanceable ambient
...
- The TUI must be in use (`hbm --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.hbm/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.hbm/logs/tui_gateway_crash.log` if a widget never appears.
