...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger HBM AGENT runs by POSTing events to a URL.
...
All management is via the `hbm webhook` CLI command:

### Create a subscription
```bash
hbm webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.hbm/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.hbm/scripts/` (path traversal is blocked).

```bash
hbm webhook subscribe todoist-hbm \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-hbm-label.py" \
...
```bash
hbm webhook remove <name>
```

### Test a subscription
```bash
hbm webhook test <name>
hbm webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
hbm webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
hbm webhook subscribe stripe-payments \
...
```bash
hbm webhook subscribe ci-builds \
  --events "pipeline" \
...
- Subscriptions persist to `~/.hbm/webhook_subscriptions.json`

## How It Works

1. `hbm webhook subscribe` writes to `~/.hbm/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status hbm-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.hbm/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `hbm webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `hbm webhook test <name>` to verify the route works.
