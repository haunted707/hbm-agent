...
the `cronjob` tool, the `hbm cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `hbm curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `hbm curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.hbm/skills/.usage.json` holds
...
Users drive it via `hbm kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `HBM_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `HBM_KANBAN_BOARD` pinned in env); tenant is a soft namespace
