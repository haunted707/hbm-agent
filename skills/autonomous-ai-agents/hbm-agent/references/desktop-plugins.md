...
Write plugins for the HBM AGENT desktop app: statusbar items, layout panes,
...
system (`~/.hbm/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$HBM_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$HBM_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.hbm-package.json` marker) — the ONLY root the renderer loads from — so
...
- The HBM AGENT desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$HBM_HOME/desktop-plugins/` (usually
  `~/.hbm/desktop-plugins/`).

## How to Run

1. Create `$HBM_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.hbm/...` by default, or `~/.hbm/profiles/<profile>/...` under a
...
The ONLY import surface is `@hbm/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from HBM AGENT
  ...
  (`hbm://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.hbm/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@hbm/plugin-sdk`, `react`, and
