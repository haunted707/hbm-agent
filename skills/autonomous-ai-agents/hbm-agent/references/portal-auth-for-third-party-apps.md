...
## Layer 1 — Is this thing an HBM AGENT plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the HBM AGENT process**. Its LLM calls go through HBM AGENT's already-configured provider. | Already uses Portal if user's HBM AGENT is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside HBM AGENT. That LLM call is already authenticated
via HBM AGENT's provider config. The plugin's own server auth (e.g.
...
What HBM AGENT has that *feels* like OAuth — `hbm login --provider nous`
opening a browser, user signs in, token lands in `~/.hbm/auth.json` — is a
**HBM AGENT-specific browser flow**. Under the hood it produces a credential
HBM AGENT uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read HBM AGENT's existing Portal credential out of `~/.hbm/auth.json`.
...
rides on the credential HBM AGENT already holds.

Where this could live in HBM AGENT:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for HBM AGENT itself.
...
1. First decide: HBM AGENT plugin (runs inside HBM AGENT) or separate app? If plugin,
   it already uses Portal via HBM AGENT's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   HBM AGENT itself") is fully supported. HBM AGENT treats agent
...
pitch in the Tool Gateway announcement is specifically about HBM AGENT's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when HBM AGENT is configured to use Portal as its provider. It is
