# Themes / Skins — Author an HBM AGENT Color Theme

Author an HBM AGENT **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`hbm_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.hbm/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for HBM AGENT itself.
...
- Write access to the HBM AGENT home dir — `~/.hbm` by default, or `$HBM_HOME`
  / the active profile's dir. Skins live in `<hbm-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `hbm config set`).
...
3. `write_file` it to `<hbm-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `hbm config set display.skin default` (or they can `/skin default`).
...
```
hbm skin set <key> <hex>      # e.g. hbm skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `hbm skin set background "#08201f"` changes
only the background; `hbm skin use <name>` / `hbm skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.hbm`** when a profile is active — resolve the real home
  from `$HBM_HOME` first, falling back to `~/.hbm`.
...
- **Never hand-edit `config.yaml` to activate.** Use `hbm config set
...
- **You apply it, not the user.** `hbm config set display.skin <name>` is
...
- `read_file` the written `<hbm-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `hbm config get display.skin` and confirm it reports `<name>`.
