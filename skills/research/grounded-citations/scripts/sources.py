...
Ledger path resolution (first wins):
  --ledger PATH
  $HBM_CITATION_LEDGER
  $HBM_HOME/cache/citations/ledger.json
"""
...
from _hbm_home import get_hbm_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("HBM_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_hbm_home() / "cache" / "citations" / "ledger.json"
