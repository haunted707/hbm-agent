# HBM AGENT Security Policy

This document describes HBM AGENT's trust model, names the one
...
security vulnerabilities. **HBM AGENT does not operate a bug
bounty program.**
...
- Environment details (`hbm --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted HBM AGENT access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in HBM AGENT's documentation
...
HBM AGENT supports two OS-level isolation postures. They address
...
HBM AGENT supports this in two ways:

- **HBM AGENT's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, HBM AGENT's in-process heuristics
...
HBM AGENT filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in HBM AGENT
itself. Bugs in HBM AGENT's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   HBM AGENT does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, HBM AGENT's own documentation, or reasonable
  operator expectations would predict — including cases where
  HBM AGENT has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  hbm-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not HBM AGENT's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in HBM AGENT. Bugs in HBM AGENT
...
- HBM AGENT includes supply-chain guards for MCP server
