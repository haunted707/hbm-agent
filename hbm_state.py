#!/usr/bin/env python3
"""SQLite state store for HBM AGENT: session metadata, message history, model
...
from hbm_constants import get_hbm_home, mkdir_under_hbm_home
from typing import Any, Callable, Dict, Iterator, List, Optional, Tuple, TypeVar, cast

from hbm_state_common import (
...
from hbm_state_holders import read_only_db_uri
from hbm_state_errors import (
...
from hbm_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from hbm_state_sessions import SessionSessionsMixin
from hbm_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from hbm_state_portability import SessionPortabilityMixin
from hbm_state_telegram import SessionTelegramTopicsMixin
from hbm_state_schema import SessionSchemaMixin
import hbm_state_holders as _state_holders
import hbm_state_lockguard as _lockguard
from hbm_state_dbfile import (
...
from hbm_state_messages import SessionMessagesMixin
from hbm_state_rewind import SessionRewindMixin
from hbm_state_wal import (
    _WAL_INCOMPAT_MARKERS, _on_disk_journal_mode, apply_database_pragmas, apply_wal_with_fallback,
)
from hbm_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from hbm_state_titles import SessionTitlesMixin
from hbm_state_usage import SessionUsageMixin
from hbm_state_maintenance import SessionMaintenanceMixin
from hbm_state_gateway import SessionGatewayMixin
from hbm_state_compression import SessionCompressionMixin
from hbm_state_search import SessionSearchMixin
...
class SessionResumeTooLargeError(ValueError):
    def __init__(
        ...
        super().__init__(
            ...
            "Start a fresh chat and use `hbm sessions export` to keep a copy, or raise the limit "
            "with `hbm config set sessions.max_resume_messages 0`."
        )
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_hbm_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_hbm_home()`` is resolved fresh (a runtime HBM_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_hbm_home() / "state.db"


# Live-DB guard knobs live HERE (not in hbm_state_guard): the hermetic conftest monkeypatches
# ``hbm_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-HBM_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``HBM_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real HBM AGENT root {root}). "
                "Tests must run against a temporary HBM_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "HBM_HOME. If this test genuinely needs the live database, mark it with "
...
def _secure_state_db_files(db_path: Path, *, create_main: bool = False) -> None:
    ...
    (gateway, desktop ``hbm serve``) keep using the deleted inodes.
    """
    if os.name == "nt":
        return
...
_NETWORK_DRIVE_ACTION = (
    "Move it to a local disk (`hbm doctor` shows where it is), then start HBM AGENT again."
)


def format_session_db_unavailable(
    prefix: str = "HBM AGENT can't open its session history right now",
    *,
    details: bool = False,
) -> str:
    """User-facing one-liner: ``<prefix>: <gloss>. <consequence> <action>[ network hint]``.

    The cause table lives in ``hbm_state_user_copy`` so CLI, gateway and TUI agree. Chat
    ...
    message names the network-drive suspicion, because ``hbm doctor --fix`` cannot repair a
    mount — only moving the file can."""
    cause = get_last_init_error()
    if not cause:
        return f"{prefix}. {_SESSION_DB_CONSEQUENCE} Run `hbm doctor` to check the storage location."
    from hbm_state_user_copy import describe_storage_failure
    ...
    if details:
        from hbm_state_user_copy import storage_failure_details
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to HBM_HOME/sessions/<id>.jsonl (state.db was replaced under a
    live process). Returns the path, or None if nothing to write."""
    sid = str(session_id or "").strip()
    if not sid or not messages:
        return None
    sessions_dir = get_hbm_home() / "sessions"
...
# connection per resolved path via hbm_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in hbm_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via hbm_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # SQLite's deterministic busy handler convoys under many hbm processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        self._wal_lock_guard: dict = {}  # hbm_state_lockguard.hold() record, see _open_writer
        ...
        # Opened via hbm_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via hbm_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        mkdir_under_hbm_home(self.db_path.parent)
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`hbm db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_invalid(self, already_locked: bool = False) -> None:
        ...
        msg = (
            f"state.db was empty or damaged ({zsize} bytes) and has been {where}; HBM AGENT started with a "
            "fresh, empty session database. To bring old sessions back, run "
            f"`hbm sessions recover --source {qpath or self.db_path} --inspect-only`, or restore a "
            "snapshot with `/snapshot list` then `/snapshot restore <id>` (terminal `hbm` chat only)."
        )
        logger.error(msg)
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if "locked" in err_msg or "busy" in err_msg:
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another HBM AGENT process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `hbm sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _settle_lost_generation_locked(self) -> bool:
        ...
        through hbm_state_registry.release_or_close, which swallows the error, so an interpreter
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see hbm_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``hbm_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from hbm_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                ...
                if quarantine_reason is not None:
                    logger.warning(
                        ...
                        "before restarting, then run `hbm sessions recover --source %s --inspect-only`.",
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('hbm_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('hbm_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('hbm_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('hbm_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('hbm_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('hbm_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('hbm_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('hbm_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('hbm_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('hbm_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('hbm_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('hbm_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('hbm_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('hbm_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('hbm_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('hbm_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('hbm_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('hbm_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('hbm_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('hbm_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('hbm_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('hbm_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('hbm_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('hbm_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('hbm_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('hbm_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('hbm_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('hbm_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('hbm_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('hbm_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('hbm_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('hbm_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('hbm_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('hbm_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('hbm_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('hbm_state_registry', 'release_or_close'),
    'report_startup_progress': ('hbm_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('hbm_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('hbm_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('hbm_state_wal', 'sqlite_source_id'),
    'workspace_key': ('hbm_state_sessions', 'workspace_key'),
}
