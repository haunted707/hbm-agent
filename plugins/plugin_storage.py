"""Per-plugin persistent storage: ``<hbm home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<hbm home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `hbm plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<hbm home>/plugin-data/<name>/``; resolves ``get_hbm_home()`` on
    every call so it follows the active profile — don't cache across profile switches."""
    from hbm_constants import get_hbm_home
    root = get_hbm_home() / "plugin-data" / _validate_name(name)
