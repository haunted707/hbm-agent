...
``api_mode="codex_responses"`` + the ``api.router.com`` host mandate in ``hbm_cli/providers.py``
...
class _CacheState:
    """Efforts cache + once-only flags for one HBM AGENT home (same names as the module slots)."""
...
def _state() -> Any:
    ...
    with _efforts_lock:
        return _state_by_home.setdefault(hbm_home_key(), _CacheState())


def _base_url() -> str:
    """Router base URL: profile ``.env`` first (scope-aware), plain os.environ as the fallback."""
    try:
        from hbm_cli.config import get_env_value_prefer_dotenv as prefer_dotenv
    except Exception:
        prefer_dotenv = None
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_hbm_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
...
router = RouterProfile(
    ...
    default_headers={"User-Agent": f"HBM AGENT-Agent/{_HBM_VERSION}"},
