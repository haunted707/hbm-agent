...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, HBM_HOME).
        ...
        try:
            ...
            hbm_home = Path(get_hbm_home()).resolve()
        except Exception:
            hbm_home = Path.home().joinpath(".hbm").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), hbm_home)):
...
def is_connected(config) -> bool:
    """Surface in ``hbm status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``hbm status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``hbm setup line`` wizard (writes ``~/.hbm/.env``); CLI helpers are lazy-imported."""
    from hbm_cli.config import get_env_value, save_env_value
    from hbm_cli.cli_output import print_header, print_info, prompt
