...
(enabled/policy/allow_from) falling back independently. Config ~/.hbm/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.hbm/feishu_comment_pairing.json."""
...
RULES_FILE = get_hbm_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_hbm_home() / "feishu_comment_pairing.json"
...
def _rules_file() -> Path:
    ...
    it, else live profile-scoped HBM_HOME — the multiplexed gateway serves every profile from
    one process, so the import-time constant would apply the launch profile's rules everywhere."""
    return RULES_FILE if RULES_FILE != _RULES_FILE_AT_IMPORT else get_hbm_home() / "feishu_comment_rules.json"


def _pairing_file() -> Path:
    return PAIRING_FILE if PAIRING_FILE != _PAIRING_FILE_AT_IMPORT else get_hbm_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("hbm_cli.env_loader", fromlist=["load_hbm_dotenv"]).load_hbm_dotenv()
    except Exception:
        pass
