...
_NATIVE_SLASH_COMMANDS: tuple = (
    ...
    ("reset", "Reset your HBM AGENT session", (), "/reset", "Session reset~"),
    ...
    ("status", "Show HBM AGENT session status", (), "/status", "Status sent~"),
    ("sethome", "Set this chat as the home channel", (), "/sethome", None),
    ("stop", "Stop the running HBM AGENT", (), "/stop", "Stop requested~"),
    ...
    ("reload-skills", "Re-scan ~/.hbm/skills/ for new or removed skills", (), "/reload-skills", None),
    ("voice", "Toggle voice reply mode",
     ...
     "/voice {mode}", None),
    ("update", "Update HBM AGENT to the latest version", (), "/update", "Update initiated~"),
    ("restart", "Gracefully restart the HBM AGENT gateway", (), "/restart", "Restart requested~"),
    ...
    ("thread", "Create a new thread and start an HBM AGENT session in it", (), None, None),
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning("Ignoring invalid HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r", raw)
    return 30.0
...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("HBM_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("HBM_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            env_key="HBM_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold", 2,
            env_key="HBM_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_hbm_home())
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("HBM_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        from hbm_constants import get_hbm_home
        directory = get_hbm_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs HBM AGENT work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit HBM AGENT/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:hbm|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields HBM AGENT manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "hbm_voice", f"ack_{_uuid.uuid4().hex[:12]}.mp3",
        )
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit HBM AGENT pairing grant."""
    ...
    def _register_thread_slash(self, tree, name: str, description: str) -> None:
        ...
        @discord.app_commands.describe(
            name="Thread name", message="Optional first message to send to HBM AGENT in the thread",
    ...
    def _register_slash_commands(self) -> None:
        ...
        def _auto_register(name: str, description: str, args_hint: str) -> None:
            ...
            try:
                ...
                pass
        try:
            from hbm_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
            try:
        ...
        try:
            from hbm_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill", description="Run an HBM AGENT skill", callback=_skill_handler,
            )
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        from hbm_cli.commands_platforms import discord_skill_commands_by_category
    ...
    async def _create_thread(
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by HBM AGENT: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Real semantic naming is done after the first agent turn, when HBM AGENT has an LLM-generated session
        ...
        thread_name = content[:80] if content else "HBM AGENT"
        if len(content) > 80:
    ...
    def _stamp_auto_thread_name(thread: Any, thread_name: str) -> Any:
        ...
        try:
            setattr(thread, "_hbm_auto_thread_initial_name", thread_name)
        except Exception:
            pass
        return thread
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by HBM AGENT: **{thread_name}**"
                    )
    ...
    async def rename_thread(
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="HBM AGENT semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        reason = "HBM AGENT session handoff"
        try:
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 HBM AGENT handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
    ...
    _EA_HEADER = (f"⚠️ **{EA_HEADER_TEXT}**\n\n"
                  "Do you want HBM AGENT to run this command?\n\n"
                  "**Requested command:**\n")
    _EA_CODE_OPEN = "```bash\n"
    _EA_CODE_CLOSE = "\n```\n"
    ...
    async def send_clarify(
        ...
        def _build(_channel):
            embed = discord.Embed(
                title="❓ HBM AGENT needs your input",
            ...
            content = self._self_contained_prompt_content(
                "❓ **HBM AGENT needs your input**", str(question or "").strip(), tail=f"\n\n{hint}",
            )
    ...
    async def send_update_prompt(
        ...
        """Yes/No prompt for the gateway ``/update`` watcher when ``hbm update --gateway`` needs input."""
        def _build(_channel):
    ...
    async def send_model_picker(
        ...
        def _build(_channel):
            try:
                from hbm_cli.providers import get_label
    ...
    async def _handle_message(
                ...
                if thread:
                    ...
                    try:
                        ...
                        await message.channel.send(
                            "⚠️ HBM AGENT could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_hbm_auto_thread_initial_name", None)
...
def _define_discord_view_classes() -> None:
    ...
    class _HbmView(discord.ui.View):
        """Shared plumbing for HBM AGENT component views: allowlist auth, single-use
    ...
    class ExecApprovalView(_HbmView):
    ...
    class SlashConfirmView(_HbmView):
    ...
    class UpdatePromptView(_HbmView):
        """Yes/No buttons for ``hbm update`` prompts; the answer is written to
        ...
        async def _respond(self, interaction: discord.Interaction, answer: str, color: discord.Color, label: str):
            ...
            try:
                from hbm_constants import get_hbm_home
                response_path = get_hbm_home() / ".update_response"
    ...
    class ModelPickerView(_HbmView):
        ...
        async def _expensive_warning_for(self, model_id: str):
            try:
                from hbm_cli.model_selection_guards import combined_selection_warning
    ...
    class ChoicePickerView(_HbmView):
    ...
    class ClarifyChoiceView(_HbmView):
...
# process (e.g. standalone ``hbm cron``); same forum/thread/multipart logic via Discord REST.
...
def _prompt_discord_bot_token(prompt) -> str:
    ...
    from hbm_cli.cli_output import print_error
    token = ""
...
def interactive_setup() -> None:
    ...
    from hbm_cli.config import get_env_value, remove_env_value, save_env_value
    from hbm_cli.cli_output import (
        prompt, prompt_yes_no, print_header, print_info, print_success,
    )
    from hbm_cli.setup_platforms import declines_reconfigure
    ...
    print()
    _info_lines(
        "📬 Home Channel: where HBM AGENT delivers cron job results,",
...
_YAML_WEBSOCKET_LIVENESS_KEYS = (
    ("websocket_liveness_interval_seconds", "liveness_interval_seconds", "HBM_DISCORD_LIVENESS_INTERVAL_SECONDS"),
    ("websocket_liveness_failure_threshold", "liveness_failure_threshold", "HBM_DISCORD_LIVENESS_FAILURE_THRESHOLD"),
...
def register(ctx) -> None:
    """Plugin entry point — called by the HBM AGENT plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `hbm setup` to install Discord support.",
