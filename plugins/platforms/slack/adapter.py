...
# User-Agent prefix (``HbmAgent/<version>``) for platform-partner attribution of API calls.
try:
    from hbm_cli import __version__ as _HBM_VERSION
except Exception:
    _HBM_VERSION = "unknown"
_HBM_SLACK_USER_AGENT_PREFIX = f"HbmAgent/{_HBM_VERSION}"
...
_MODEL_PICKER_PROVIDER_ACTION = "hbm_model_provider"
_MODEL_PICKER_MODEL_ACTION = "hbm_model_model"
_MODEL_PICKER_BACK_ACTION = "hbm_model_back"
_MODEL_PICKER_CANCEL_ACTION = "hbm_model_cancel"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    if not text.startswith("!"):
        return text
    try:
        from hbm_cli.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # URL); labels after dates (dates carry a label); bot mention after labels (``<@U…|hbm>``).
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if team_key not in warned and "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "from `hbm slack` produces a manifest with these already included.",
    ...
    def _register_bolt_handlers(self) -> None:
        ...
        # Catch-all no-op ack for any other subscribed event type that HBM AGENT has no listener for (e.g.
        ...
        async def handle_unhandled_event(event, body, logger):
            logger.debug(
                ...
                "events not handled by HBM AGENT can be removed from the Slack app manifest via "
                "`hbm slack manifest`)",
        ...
        # ALSO be declared in the app manifest (`hbm slack manifest`): Socket Mode won't
        # deliver undeclared commands at all.
        from hbm_cli.commands_platforms import slack_native_slashes
        _slash_names = [name for name, _d, _h in slack_native_slashes()]
        if _slash_names:
            _slash_pattern = re.compile(
                r"^/(?:" + "|".join(re.escape(n) for n in _slash_names) + r")$")
        else:  # pragma: no cover - registry always non-empty
            _slash_pattern = re.compile(r"^/hbm$")

        @self._app.command(_slash_pattern)
        async def handle_hbm_command(ack, command):
        ...
        self._app.action("hbm_feedback")(self._handle_feedback_action)
        # Clarify buttons (tools/clarify_gateway.py); indexed action IDs because
        # Block Kit requires unique IDs within an actions block.
        self._app.action(re.compile(r"^hbm_clarify_choice_\d+$"))(self._handle_clarify_action)
        self._app.action("hbm_clarify_other")(self._handle_clarify_action)
    ...
    def _new_web_client(token: str, proxy_url: Optional[str]) -> Any:
        client = AsyncWebClient(token=token, user_agent_prefix=_HBM_SLACK_USER_AGENT_PREFIX)
        _apply_slack_proxy(client, proxy_url)
        return client
    ...
    def _fatal_missing_env(self, env_name: str) -> None:
        ...
        logger.error(
            "[Slack] %s not set — this is a permanent config error; set %s via `hbm "
            "gateway setup` or in the active profile's ~/.hbm/.env file, then restart the "
            "gateway.", env_name, env_name)
        self._set_fatal_error(
            f"missing_{env_name.lower()}",
            f"{env_name} not configured. Use `hbm gateway setup` "
            "or add it to your active profile's ~/.hbm/.env file, then restart the gateway.",
            retryable=False)
    ...
    def _hint_allow_bots(self) -> None:
        ...
        if _allow_bots_cfg != "none":
            logger.info(
                ...
                "appropriate (run 'hbm slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: HBM AGENT handoff — *{(name or 'session').strip()[:80]}*"
    ...
    async def send_native_task_card_progress(
        self, chat_id: str, tasks: List[Dict[str, str]], *, title: str = "HBM AGENT is working",
    ...
    def _synthetic_reaction_event(
        ...
        from conflating it with the reacted-to message; ``_hbm_force_process`` skips the mention
        requirement (user auth and allowed_channels still apply); ``_hbm_reaction`` is
        ...
        if target_channel:
            ...
            synthetic["_hbm_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_hbm_no_thread_response"] = True
        return synthetic
    ...
    def _session_thread_ts(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
            return thread_ts
        if event.get("_hbm_no_thread_response"):
    ...
    async def _handle_slack_message_impl(self, event: dict, payload: Optional[dict] = None) -> None:
        ...
        force_process = bool(event.get("_hbm_force_process"))
    ...
    _EA_ACTION_IDS = {"once": "hbm_approve_once", "session": "hbm_approve_session",
                      "always": "hbm_approve_always", "deny": "hbm_deny"}
        ...
        def _build() -> Tuple[str, list]:
            ...
            blocks = [
                {"type": "section", "text": {"type": "mrkdwn", "text": f"*{_title}*\n\n{body}"}},
                {
                    "type": "actions",
                    "elements": [
                        self._button("Approve Once", "hbm_confirm_once", value, style="primary"),
                        self._button("Always Approve", "hbm_confirm_always", value),
                        self._button("Cancel", "hbm_confirm_cancel", value, style="danger")]}]
    ...
    async def send_clarify(
        ...
        """Clarify prompt as Block Kit buttons: one ``hbm_clarify_choice_<idx>`` per option
        (value ``clarify_id|idx``) plus "✏️ Other…" (``hbm_clarify_other``), which flips the
        ...
        def _build() -> Tuple[str, list]:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append(
                    self._button(
                        label[:75], f"hbm_clarify_choice_{idx}",
                        f"{clarify_id}|{idx}", emoji=True))
            elements.append(
                self._button("✏️ Other…", "hbm_clarify_other", f"{clarify_id}|other", emoji=True)
            )
    ...
    _APPROVAL_CHOICES: ClassVar[Dict[str, str]] = {
        "hbm_approve_once": "once", "hbm_approve_session": "session",
        "hbm_approve_always": "always", "hbm_deny": "deny"}
    ...
    _CONFIRM_CHOICES: ClassVar[Dict[str, str]] = {
        "hbm_confirm_once": "once", "hbm_confirm_always": "always",
        "hbm_confirm_cancel": "cancel"}
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "hbm_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        ``/hbm <subcommand> [args]``; other text after ``/hbm`` is a regular message."""
        ...
        # events only: free-form "/hbm <question>" replies must stay public.
    ...
    def _slash_command_text(command: dict) -> str:
        ...
        payload verbatim (internal/trailing spacing). ``/hbm`` (or a missing ``command``) maps
...
# in ``hbm_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in ``tools/send_message_tool.py``).
# ──────────────────────────────────────────────────────────────────────────
...
def _load_slack_bot_tokens(raw_token: str, *, quiet: bool) -> List[str]:
    ...
    try:
        ...
        tokens_file = get_hbm_home() / "slack_tokens.json"
...
def _write_slack_manifest_and_instruct() -> None:
    """Write the manifest under HBM_HOME and print paste instructions; non-fatal."""
    from hbm_cli.cli_output import print_info, print_success, print_warning
    try:
        from hbm_cli.slack_cli import _build_full_manifest
        from hbm_constants import get_hbm_home
        manifest = _build_full_manifest(
            bot_name="HBM AGENT", bot_description="Your HBM AGENT on Slack")
        target = _Path(get_hbm_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `hbm slack manifest --write` anytime to refresh after "
            "HBM AGENT adds new commands.")
    except Exception as e:
...
def interactive_setup() -> None:
    ...
    from hbm_cli.config import remove_env_value, save_env_value
    ...
    if declines_reconfigure("Slack", "Reconfigure Slack?", "SLACK_BOT_TOKEN"):
        ...
        if prompt_yes_no(
            ...
            "list? (recommended after `hbm update`)", True):
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `hbm setup` to install Slack support.",
