...
// `message.content.text` before HBM AGENT can see it. We rewrite the two inbound
...
const MARKER = "HBM AGENT patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // HBM AGENT to patch; keep the legacy v8 path below for older pinned installs.
    if (
