// HBM AGENT — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> HBM AGENT): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (HBM AGENT -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-HBM AGENT-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `hbm photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so HBM AGENT can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "hbm-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply HBM AGENT's
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local HBM AGENT allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of HBM AGENT, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-hbm-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when HBM AGENT supplied them so a known-good
