...
Small Node helper that bridges HBM AGENT to Photon's Spectrum SDK
(`spectrum-ts`).  HBM AGENT is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-HBM AGENT-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and HBM AGENT can receive inbound messages
...
The HBM AGENT plugin's `hbm photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. HBM AGENT is Python, so replies go through this sidecar
