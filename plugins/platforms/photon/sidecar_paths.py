...
``$HBM_HOME/photon/sidecar``. The mirror is refreshed by content compare; ``node_modules``
...
# Written by `hbm photon install-sidecar` on npm failure so check_requirements() can
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_hbm_home() / "photon" / "sidecar"
    try:
