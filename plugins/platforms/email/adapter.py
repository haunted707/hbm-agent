"""Email platform adapter for the HBM AGENT gateway: users talk to HBM AGENT by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _hbm_version = "0"
        imap.xatom("ID", f'("name" "hbm-agent" "version" "{_hbm_version}" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `hbm gateway setup` (env) or platforms.email in config.yaml."
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        for key, value in (("From", address), ("To", chat_id), ("Subject", "HBM AGENT"), ("Date", formatdate(localtime=True))):
            msg[key] = value
