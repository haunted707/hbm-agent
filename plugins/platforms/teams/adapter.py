...
``<hbm-venv>/bin/pip install microsoft-teams-apps aiohttp``) and credentials via env
...
async def _standalone_send(
    ...
    (``hbm cron``). ``TEAMS_SERVICE_URL`` is allowlisted and ``chat_id`` charset-checked
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    cwd-discovered ``.env`` at import, mutating process-global ``os.environ``. HBM AGENT owns dotenv loading.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "HBM AGENT"}))
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        headers = {"User-Agent": "Mozilla/5.0 (compatible; HbmAgent/1.0)"}
    ...
    async def _on_card_action(
        ...
        hbm_action = data.get("hbm_action", "")
        session_key = data.get("session_key", "")
        if not hbm_action or not session_key:
        ...
        choice = _APPROVAL_CHOICES.get(hbm_action)
        if not choice:
            return self._invoke_message("Unknown action.")
    ...
    async def _send_exec_approval_prompt(self, prompt: ExecApprovalPrompt) -> SendResult:
        ...
        for label, choice, style in prompt.actions:
            ...
            actions.append(ExecuteAction(
                title=label, verb="hbm_approve",
                data={**btn_data_base, "hbm_action": self._EA_CARD_ACTIONS[choice]}, **kw))
...
_SETUP_INTRO = (  # "" → blank line
    ...
    '  teams app create --name "HBM AGENT" --endpoint "https://<tunnel>/api/messages"', "",
...
def interactive_setup() -> None:
    ...
    from hbm_cli.cli_output import prompt, prompt_yes_no, print_info, print_success, print_warning
    ...
    print()
    print_success("Teams configuration saved to ~/.hbm/.env")
...
def _install_hint() -> str:
    ...
    ``venv_pip=True`` targets the real HBM AGENT venv, sidestepping PEP 668 on Ubuntu 24.04."""
    try:
