# A2A — Agent-to-Agent protocol for HBM AGENT
...
peer (another HBM AGENT, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
hbm gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, HBM AGENT serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.hbm/a2a_audit.jsonl`.
- Conversations persist to `~/.hbm/a2a_conversations/` — they survive context
