...
legacy single-user mode). Files under ``${HBM_HOME}``: ``google_chat_user_tokens/
...
from hbm_constants import display_hbm_home, get_hbm_home
...
# human-readable under ``ls ~/.hbm/google_chat_user_tokens/``.
...
def _token_rel(email: Optional[str]) -> str:
    """HBM_HOME-relative token file: per-user under the tokens dir, else the legacy path."""
...
def _user_tokens_dir() -> Path:
    return get_hbm_home() / "google_chat_user_tokens"


def _token_path(email: Optional[str] = None) -> Path:
    """Per-user token path for ``email``, or the legacy single-user path."""
    return get_hbm_home() / _token_rel(email)


def _client_secret_path() -> Path:
    return get_hbm_home() / "google_chat_user_client_secret.json"


def _pending_auth_path(email: Optional[str] = None) -> Path:
    if email:
        return get_hbm_home() / "google_chat_user_oauth_pending" / f"{_sanitize_email(email)}.json"
    return get_hbm_home() / "google_chat_user_oauth_pending.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `hbm setup` to install Google Chat support."
        )
        return None
    try:
...
def install_deps() -> bool:
    ...
    try:
        ...
        # lazy_deps honors HBM_LAZY_INSTALL_TARGET on sealed hosted images;
        ...
        print("Run `hbm setup` to repair the managed installation, then retry.")
        return False
    except Exception as exc:
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into HBM_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    print(f"Profile path: {display_hbm_home()}/{_token_rel(email)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for HBM AGENT (native attachment delivery)"
    )
