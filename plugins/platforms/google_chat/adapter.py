...
def ensure_google_chat_deps() -> bool:
    ...
    ``HBM_LAZY_INSTALL_TARGET`` instead of the read-only venv. Resets the
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # The profile scope this adapter was connected under (multiplex: HBM_HOME override + secret
        ...
        from hbm_constants import get_hbm_home as _get_hbm_home
        self._thread_count_store = _ThreadCountStore(_get_hbm_home() / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from hbm_constants import get_hbm_home as _get_hbm_home
        return _get_hbm_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a hbm MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "hbm_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'HBM AGENT is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "HBM AGENT is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. hbm-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``hbm setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.hbm/.env``."""
    from hbm_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.hbm/.env")
    print_info("Restart the gateway: hbm gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``hbm cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `hbm setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'HBM AGENT is thinking…' marker message "
