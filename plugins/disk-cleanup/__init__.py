"""disk-cleanup plugin — auto-cleanup of ephemeral HBM AGENT session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        with contextlib.suppress(ValueError):  # tokenise — catches `touch /tmp/hbm-x/test_foo.py`
...
All operations are scoped to HBM_HOME and /tmp/hbm-*.
...
def _cmd_track(argv: List[str]) -> str:
    ...
    return f"Not tracked (already present, missing, or outside HBM_HOME): {path_arg}"
...
def register(ctx) -> None:
    ...
    ctx.register_command("disk-cleanup", handler=_handle_slash,
                         description="Track and clean up ephemeral HBM AGENT session files.")
