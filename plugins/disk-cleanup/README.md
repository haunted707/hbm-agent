...
Auto-tracks and cleans up ephemeral files created during HBM AGENT
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HBM_HOME` and `/tmp/hbm-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HBM_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `HBM_HOME` or `/tmp/hbm-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HBM_HOME/disk-cleanup/` is itself excluded
- `$HBM_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
