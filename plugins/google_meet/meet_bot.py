...
Standalone subprocess spawned by ``process_manager.py``; configured via ``HBM_MEET_*`` env,
status + transcript written under ``$HBM_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``HBM_MEET_URL=... HBM_MEET_OUT_DIR=/tmp/x HBM_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__hbmMeetDrain()`` pulls new entries.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__hbmMeetInstalled) return;
  window.__hbmMeetInstalled = true;
  window.__hbmMeetQueue = [];
  ...
  window.__hbmMeetDrain = () => {
    const out = window.__hbmMeetQueue.slice();
    window.__hbmMeetQueue = [];
    return out;
  };
})();
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__hbmMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'hbm_meet_sink'}", str(pcm_path)]
...
def _setup_realtime(rt: dict, api_key: str, state: _BotState) -> None:
    ...
    if not api_key:
        state.set(error="realtime mode requested but no API key in HBM_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
        rt["enabled"] = False
        return
    try:
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``HBM_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("HBM_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("HBM_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("HBM_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("HBM_MEET_AUTH_STATE", "").strip(),
        guest_name=env("HBM_MEET_GUEST_NAME", "HBM AGENT"),
        duration_s=_parse_duration(env("HBM_MEET_DURATION", "")),
        realtime=env("HBM_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # HBM_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("HBM_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("HBM_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("HBM_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("HBM_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("HBM_MEET_LOBBY_TIMEOUT", "300")))
    ...
    while not stop_flag["stop"]:
        ...
        if not state.in_call and (now - last_admission_check) > 3.0:
            ...
            if _probe(page, _ADMISSION_PROBE_JS):
                ...
                return
        try:
            queued = page.evaluate("window.__hbmMeetDrain && window.__hbmMeetDrain()")
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — HBM_MEET_URL must be a "
                         "meet.google.com URL. got: %r\n" % cfg.url)
        return 2
    if cfg.out_dir is None:
        sys.stderr.write("google_meet bot: HBM_MEET_OUT_DIR is required\n")
        return 2
