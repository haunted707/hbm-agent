"""CLI commands for the google_meet plugin (``hbm meet <subcommand>``).
...
def _auth_state_path() -> Path:
    return Path(get_hbm_home()) / "workspace" / "meetings" / "auth.json"


# ``hbm meet <sub>`` in help order.
_SUBCOMMAND_HELP = (
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``hbm meet`` argparse tree (called at plugin load time)."""
    ...
    p["join"].add_argument("--guest-name", default="HBM AGENT")
...
_DISPATCH = {
    ...
    "stop": lambda a: _print_result(pm.stop(reason="hbm meet stop")),
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: hbm meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
...
def _cmd_setup() -> int:
    ...
    print("  google auth    : " + (f"ok ({auth_path})" if auth_path.is_file() else "not saved — run: hbm meet auth"))
    print()
    all_ok = system_ok and pw_ok and chromium_ok
    print("ready. Join a meeting:  hbm meet join https://meet.google.com/abc-defg-hij" if all_ok
    ...
    if not realtime:
        ...
        if system == "Linux":
            ...
            print("\n  NOTE: macOS does not auto-route audio. Open\n    System Settings → Sound → "
                  ...
                  "hbm will not switch your default input for you.")
    print("\ndone. verify with: hbm meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: hbm meet join <url>")
    return 0
...
if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="hbm meet")
    register_cli(parser)
