...
One active meeting at a time, recorded in ``$HBM_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_hbm_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "HBM AGENT", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "HBM_MEET_URL": url, "HBM_MEET_OUT_DIR": str(out),
           "HBM_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "HBM_MEET_HEADED"),
        (auth_state, "HBM_MEET_AUTH_STATE"),
        (duration, "HBM_MEET_DURATION"),
        (mode, "HBM_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "HBM_MEET_REALTIME_MODEL"),
        (realtime_voice, "HBM_MEET_REALTIME_VOICE"),
        (realtime_instructions, "HBM_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("HBM_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["HBM_MEET_REALTIME_KEY"] = realtime_api_key
