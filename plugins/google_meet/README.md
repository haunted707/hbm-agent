...
Let the hbm agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where hbm runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `hbm meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `hbm meet` CLI |
...
| `cli.py` | `hbm meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$HBM_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `hbm meet node {run,list,approve,remove,status,ping}` |
...
hbm meet install                                      # pip + Chromium
hbm meet setup                                        # preflight
hbm meet auth                                         # optional
hbm meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
hbm meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hbm/.env
hbm meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
hbm meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
hbm meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hbm/.env
hbm meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, hbm will **not** switch your system audio input automatically — the
...
hbm plugins enable google_meet
hbm meet node run --display-name my-mac --host 0.0.0.0 --port 18789
