...
class _Client:
    ...
    def _headers(self, path: str, json_body: bool = True) -> dict:
        ...
        return {"Authorization": f"Bearer {token}", "x-sdk-runtime": "hbm-plugin",
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._user_id, self._session_id, self._agent_id = "default", "", "hbm"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project: RETAINDB_PROJECT > config.yaml > hbm-<profile> > "default" (API auto-creates "default").
        ...
        if not project:
            profile_name = os.path.basename(str(kwargs.get("hbm_home", "")))
            project = f"hbm-{profile_name}" if profile_name not in {"", ".hbm"} else "default"
        ...
        self._agent_id = kwargs.get("agent_id", "hbm") or "hbm"
