...
``$HBM_HOME/plugins/<name>/``, project ``./.hbm/plugins/<name>/`` (opt-in via
HBM_ENABLE_PROJECT_PLUGINS), then ``hbm_agent.memory_providers`` entry points.
...
ENTRY_POINTS_GROUP = "hbm_agent.memory_providers"
# Per HBM AGENT home (plugin managers are per home too): pruning under one multiplexed profile must
...
def _registered_skills_for_active_home() -> dict[str, Path]:
    ...
    return _REGISTERED_MEMORY_PROVIDER_SKILLS.setdefault(hbm_home_key(), {})

# Synthetic parent package so user-installed providers don't collide with bundled ones.
_USER_NAMESPACE = "_hbm_user_memory"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.hbm/plugins/`` or None. Gated on HBM_ENABLE_PROJECT_PLUGINS like the
    PluginManager scan: a repo you merely ``cd`` into must not offer a memory backend."""
    try:
        from hbm_cli.plugins import _env_enabled

        if not _env_enabled("HBM_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".hbm" / "plugins"
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``hbm <provider>`` commands."""
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from hbm_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from hbm_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _hbm_user_memory.<name>.cli, usually before the
