...
Config file resolution: $HBM_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
from hbm_state_common import TITLE_SOURCE_DERIVED, TITLE_SOURCE_LLM
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for an HBM AGENT profile."""
...
def resolve_active_host() -> str:
    """Honcho host key: HBM_HONCHO_HOST (profile-scoped .env), else the active profile. The config's
    ...
    explicit = (get_secret("HBM_HONCHO_HOST", "") or "").strip()
    if explicit:
        return explicit
    try:
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $HBM_HOME/honcho.json -> default profile's honcho.json
    ...
    local_path = get_hbm_home() / "honcho.json"
    if local_path.exists():
        return local_path
    default_path = _get_default_hbm_home() / "honcho.json"
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat HBM AGENT spellings.
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_hbm_home() later — those resolvers read a ContextVar that
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "hbm", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), hbm_home=get_hbm_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active HBM AGENT profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, hbm_home=get_hbm_home(),
        )
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError("Honcho API key not found. Get your API key at https://app.honcho.dev, "
                         "then run 'hbm honcho setup' or set HONCHO_API_KEY. "
...
def _build_client(config: HonchoClientConfig) -> "Honcho":
    ...
    try:
        ...
        raise ImportError("honcho-ai is required for Honcho integration. Install it with: pip install honcho-ai  "
                          "(or run `hbm honcho setup` to configure).")
