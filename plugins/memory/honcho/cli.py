"""``hbm honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
from hbm_cli.config import cfg_get
from utils import read_json_or_empty

RULE = "─" * 40
REASONING_LEVELS = ("minimal", "low", "medium", "high", "max")
_RETRY_HINT = "  Re-run 'hbm honcho setup' to retry, or choose an API key instead.\n"
...
def _local_config_path() -> Path:
    ...
    return get_hbm_home() / "honcho.json"
...
def _write_config(cfg: dict, path: Path | None = None) -> None:
    ...
    with _refresh_lock, _config_refresh_lock(path):
        ...
        mkdir_under_hbm_home(path.parent)
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "hbm" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'hbm honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'hbm honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `hbm update`; no output, no exceptions."""
...
def cmd_enable(args) -> None:
    ...
    if not _resolve_api_key(cfg, block, env=False):
        ...
        setup = "hbm honcho setup" + (f" --target-profile {profile}" if profile != "default" else "")
...
def _resolve_effective_identity_mapping(cfg: dict, hbm_host: dict) -> tuple[bool, dict, str, bool, bool]:
...
def _apply_runtime_prefix(hbm_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        hbm_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(hbm_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(hbm_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {hbm_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {hbm_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(hbm_host, current_pin, current_aliases, current_prefix,
    ...
    _scrub_identity_mapping(hbm_host)
    hbm_host["pinUserPeer"] = pin
    if pin:
        return
    ...
    while entry := _prompt("    alias", default="").strip():
        ...
        if rid and peer:
            aliases[rid] = peer
    if aliases:
        hbm_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(hbm_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, hbm_host: dict, current_peer: str, new_host: bool) -> None:
    ...
    current_pin, current_aliases, current_prefix, aliases_from_root, prefix_from_root = (
        _resolve_effective_identity_mapping(cfg, hbm_host))
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Each gateway account (a Telegram user, a Discord user, ...)\n"
             ...
             "Running the HBM AGENT gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
            ("\n  No gateway platforms connected — nothing to map.", "Configure anyway? (y/N)"))
        print(notice)
        if not _yes(_prompt(question, default="n")):
            return

    peer_target = hbm_host.get("peerName") or current_peer or "user"
    ai_peer_label = hbm_host.get("aiPeer") or cfg.get("aiPeer") or "hbm"
    ...
    print("\n  This step covers the HUMAN mapping only. Each account using the\n"
          ...
          "  HBM AGENT profile brings its own AI peer to the gateway.\n"
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(hbm_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(hbm_host)  # each shape starts from a clean slate
        hbm_host["pinUserPeer"] = shape == "single"
        if shape == "single":
    ...
    _echo_identity_mapping(hbm_host)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if hbm_cli internals move."""
    try:
        from hbm_cli.auth import _can_open_graphical_browser, _is_remote_session
        return _is_remote_session(), _can_open_graphical_browser()
    except Exception:
        return False, True


def _apply_grant_to_host(cfg: dict, hbm_host: dict, cred) -> None:
    ...
    hbm_host["apiKey"] = cred.access_token
    hbm_host["oauth"] = cred.oauth_block()
    ...
    if cred.consent_peer_name:  # default the peer prompt to the consent name
        hbm_host["peerName"] = cred.consent_peer_name
    print("  Authorized — token saved. Let's finish configuring.\n")


def _setup_local_auth(cfg: dict, hbm_host: dict) -> None:
    ...
    if new_url := _prompt("Base URL", default=cfg.get("baseUrl") or "http://localhost:8000"):
        cfg["baseUrl"] = new_url
    current_host_key = hbm_host.get("apiKey", "")
...
def _setup_device_login(cfg: dict, hbm_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        ...
        print(" approved")
        _apply_grant_to_host(cfg, hbm_host, cred)
        return True
    return False


def _setup_browser_login(cfg: dict, hbm_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="hbm-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, hbm_host: dict, write_path: Path) -> bool:
    ...
    existing_oauth = OAuthCredential.from_host_block(hbm_host)
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(cfg, hbm_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(cfg, hbm_host, write_path)
    # A leftover grant on the host block would shadow the pasted key.
    stale_grant = existing_oauth is not None or is_oauth_access_token(hbm_host.get("apiKey"))
    current = ("" if stale_grant else hbm_host.get("apiKey", "")) or cfg.get("apiKey", "")
    ...
    if not key:
        print("\n  No API key configured. Get yours at https://app.honcho.dev\n"
              "  Run 'hbm honcho setup' again once you have a key.\n")
        return False
    hbm_host.pop("oauth", None)
    hbm_host["apiKey"] = key
    return True
...
def _choice_step(hbm_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        hbm_host[key] = new
    elif fallback is not None:
        hbm_host[key] = fallback


def _setup_tuning(cfg: dict, hbm_host: dict) -> None:
    ...
    _choice_step(hbm_host, "observationMode", _pref(hbm_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(hbm_host, cfg, "writeFrequency", "async")))
    hbm_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")

    _menu("Recall mode", *(f"{m:<7} -- {desc}" for m, desc in _MODES.items()))
    raw_recall = _pref(hbm_host, cfg, "recallMode", "hybrid")
    _choice_step(hbm_host, "recallMode", raw_recall if raw_recall in _MODES else "hybrid", "Recall mode", _MODES)

    hbm_host["recallSync"] = _yes(_prompt(
        "Wait for current-query recall (bounded by request timeout, default 5s)? (y/N)",
        default="y" if hbm_host.get("recallSync", cfg.get("recallSync", False)) else "n"))

    current_ctx_tokens = _pref(hbm_host, cfg, "contextTokens")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(hbm_host, cfg, "dialecticCadence") or "2"))
    if (val := _first_parsed([new_dialectic], int, None)) is None:
        hbm_host["dialecticCadence"] = 2
    elif val >= 1:
        hbm_host["dialecticCadence"] = val
    ...
    _choice_step(hbm_host, "dialecticReasoningLevel", _pref(hbm_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(hbm_host, "sessionStrategy", _pref(hbm_host, cfg, "sessionStrategy", "per-session"),
...
def _setup_wizard(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives HBM AGENT persistent cross-session memory.\n  Config: {write_path}")
    ...
    if is_local:
        _setup_local_auth(cfg, hbm_host)
    elif not _setup_cloud_auth(cfg, hbm_host, write_path):
        return

    # --- 3. Identity ---
    current_peer = hbm_host.get("peerName") or cfg.get("peerName", "")
    for key, label, default in (
        ("peerName", "Your name (user peer)", current_peer or os.getenv("USER", "user")),
        ("aiPeer", "AI peer name", _pref(hbm_host, cfg, "aiPeer", "hbm")),
        ("workspace", "Workspace ID", _pref(hbm_host, cfg, "workspace", "hbm")),
    ):
        if new := _prompt(label, default=default):
            hbm_host[key] = new

    _setup_identity_mapping(cfg, hbm_host, current_peer, new_host)
    print("\n  For a gateway with many users and agents, run\n"
          "  'hbm honcho peers map' to map accounts interactively.")

    _setup_tuning(cfg, hbm_host)
    hbm_host["enabled"] = True
    hbm_host.setdefault("saveMessages", True)
    _write_config(cfg)
    ...
    try:  # auto-enable Honcho as memory provider in config.yaml
        ...
        hbm_config = load_config()
        hbm_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(hbm_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}\n  Run: hbm config set memory.provider honcho")
  ...
  Other commands:
    hbm honcho status     -- show full config
    hbm honcho mode       -- change recall/observation mode
    hbm honcho tokens     -- tune context and dialectic budgets
    hbm honcho peer       -- update peer names
    hbm honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active HBM AGENT profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("hbm.work") readable.
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: hbm honcho setup\n")
        return
...
def _state_db_path() -> Path:
    ...
    if _profile_override and _profile_override not in {"default", "custom"}:
        try:
            from hbm_cli.profiles import get_profile_dir
            return get_profile_dir(_profile_override) / "state.db"
        except Exception:
            pass
    return get_hbm_home() / "state.db"
...
def _classify_workspace_peers(
    ...
    hbm_hosts = {hostk for _, hostk, _ in profile_rows}
    ...
    # Host blocks that are not HBM AGENT profiles: other apps sharing the config.
    for hostk, block in (cfg.get("hosts") or {}).items():
        if hostk in hbm_hosts or not isinstance(block, dict):
            continue
...
def cmd_peers_map(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    hbm_host = _host_block(cfg, host)
    pin, aliases, prefix, aliases_from_root, _ = _resolve_effective_identity_mapping(cfg, hbm_host)
    peer_name = hbm_host.get("peerName") or cfg.get("peerName") or ""

    if pin:
        print("\n  pinUserPeer is on: every gateway account resolves to peer")
        print(f"  '{peer_name or '(peerName not set)'}' and aliases have no effect.")
        print("  Turn the pin off with 'hbm honcho setup' to use per-account peers.")
    ...
    workspace = (
        ...
        or hbm_host.get("workspace") or cfg.get("workspace") or host
    )
...
Honcho peers
{RULE}
  User peer:   {_pref(hbm, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(hbm, cfg, 'aiPeer') or _host_key()}
    HBM AGENT's identity in Honcho. Seed with 'hbm honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(hbm, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(hbm, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: hbm honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(hbm, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(hbm, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(hbm, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. HBM AGENT asks Honcho's AI peer a question
  ...
  Set with: hbm honcho tokens [--context N] [--dialectic N]
""")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'hbm honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'hbm honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    hbm honcho identity --show        — show both peer representations
    hbm honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → HBM AGENT + Honcho."""
    ...
    print("\nHoncho migration: OpenClaw native memory → HBM AGENT\n" + "─" * 50)
    print("""
...
""")
    if has_key:
        ...
        print("""  Honcho is a cloud memory service that gives HBM AGENT persistent memory
  across sessions. You need an API key to use it.

  1. Get your API key at https://app.honcho.dev
  2. Run:  hbm honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'hbm honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'hbm honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  hbm honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'hbm'
  with Honcho configured and no prior session history.
  (HBM AGENT calls migrate_memory_files() on first session init.)

  If you want to migrate them now without starting a session:""")
        print("    hbm honcho migrate  — this step handles it interactively\n" * len(user_files), end="")
        if has_key:
            _offer("  Upload user memory files to Honcho now?", _migrate_upload, user_files)
        else:
            print("  Run 'hbm honcho setup' first, then re-run this step.")
    else:
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In HBM AGENT, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'hbm honcho setup' first, then seed manually:")
            print("\n".join(f"    hbm honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  hbm honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    HBM AGENT:   per-session by default — each run gets its own session
              Map a custom name:  hbm honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. hbm honcho setup              — configure API key (required)\n"
              "  2. hbm honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. hbm honcho status             — verify Honcho connection
  2. hbm                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. hbm honcho identity --show    — verify AI peer representation
  4. hbm honcho tokens             — tune context and dialectic budgets
  5. hbm honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to hbm memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to HBM AGENT Honcho", cmd_migrate, ()),
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":  # honcho setup goes through the unified memory-provider path
        print("\n  Honcho is configured via the memory provider system.\n  Running 'hbm memory setup'...\n")
        from hbm_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``hbm honcho`` argparse subcommand tree on the ``hbm honcho`` parser."""
