...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/hbm>
...
```bash
hbm memory setup honcho   # configure Honcho directly (works on a fresh install)
hbm memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
hbm config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hbm/.env
```

> `hbm honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `hbm memory setup honcho`.
...
Set `"recallSync": true` in `$HBM_HOME/honcho.json` (at the root or under
`hosts.hbm`), or enable **Current-query recall** in the memory settings or
`hbm honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `hbm model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HBM_HOME/honcho.json` | Profile-local (isolated HBM AGENT instances) |
| 2 | `~/.hbm/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active HBM AGENT profile: `hbm` (default) or `hbm_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `hbm honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 2 | `per-session` strategy | HBM AGENT session ID (`20260415_a3f2b1`) |
| 3 | Manual map (`sessions` config) | `"myproject-main"` |
| 4 | Explicit `/title` command (non-automatic title) | `"refactor-auth"` |
| 5 | `per-repo` strategy | Git root directory name (`hbm-agent`) |
...
Automatically generated HBM AGENT titles (`derived` or `llm`) are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions.
...
If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-hbm-agent`.
...
- **`per-session`** — HBM AGENT session ID (timestamp + hex). Every `hbm` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple HBM AGENT profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`hbm`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active HBM AGENT profile: `hbm` (default) or `hbm_<profile>` (e.g. `hbm -p coder` -> host key `hbm_coder`). Older `hbm.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `HBM_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `hbm-agent`) |
...
|---------|-------------|
| `hbm memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `hbm honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `hbm memory setup`) |
| `hbm honcho status` | Show resolved config for active profile |
| `hbm honcho enable` / `disable` | Toggle Honcho for active profile |
| `hbm honcho mode <mode>` | Change recall or observation mode |
| `hbm honcho peer --user <name>` | Update user peer name |
| `hbm honcho peer --ai <name>` | Update AI peer name |
| `hbm honcho tokens --context <N>` | Set context token budget |
| `hbm honcho tokens --dialectic <N>` | Set dialectic max chars |
| `hbm honcho map <name>` | Map current directory to a session name |
| `hbm honcho sync` | Create host blocks for all HBM AGENT profiles |
