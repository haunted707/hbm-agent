...
_DEFAULT_CONTAINER_TAG = "hbm"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=hbm"
...
def _load_supermemory_config(hbm_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $hbm_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if hbm_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(hbm_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, hbm_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(hbm_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "hbm"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes HBM AGENT writes into the "HBM AGENT" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "hbm", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, hbm_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(hbm_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._hbm_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `hbm memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, hbm_home):
        ...
        _save_supermemory_config(sanitized, hbm_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from hbm_constants import get_hbm_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_hbm_home())))}

    def post_setup(self, hbm_home: str, config: dict) -> None:
        from hbm_cli.config import save_config
        from hbm_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, hbm_home=hbm_home)
        ...
        status = _probe_supermemory_connection(api_key, hbm_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from hbm_constants import get_hbm_home
        self._hbm_home = kwargs.get("hbm_home") or str(get_hbm_home())
        self._session_id, self._turn_count, self._pending_turns = session_id, 0, []
        config = _load_supermemory_config(self._hbm_home)
    ...
    def _write_turns(self, mode: str, new_turn: Optional[Dict[str, str]] = None) -> None:
        ...
        with self._capture_lock:
            ...
            for sid in dict.fromkeys(t["session_id"] for t in turns):
                ...
                metadata = {"type": "conversation", "session_id": sid, "timestamp": now.isoformat()}  # no sm_capture_mode: HBM AGENT policy
