...
- Hosted: API key from [app.supermemory.ai/integrations?connect=hbm](http://app.supermemory.ai/integrations?connect=hbm)
...
Before running `hbm memory setup`, add the local endpoint to
...
Then run `hbm memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$HBM_HOME/supermemory.json`
...
| `container_tag` | `hbm` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `hbm-{identity}` → `hbm-coder`). |
...
`https://api.supermemory.ai`. HBM AGENT resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: hbm`, and document writes stamp
`metadata.sm_source: hbm`. This is a **functional routing key, not telemetry**:
it groups HBM AGENT-written memories into a dedicated "HBM AGENT" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per HBM AGENT profile:

```json
{
  "container_tag": "hbm-{identity}"
}
```

For a profile named `coder`, this resolves to `hbm-coder`. The default profile resolves to `hbm-default`. Without `{identity}`, all profiles share the same container.
