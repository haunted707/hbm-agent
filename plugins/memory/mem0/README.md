...
Behavioral settings live in `$HBM_HOME/mem0.json` (set them via `hbm memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.hbm/.env`.
...
| `user_id` | `hbm-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh HBM AGENT session and call `mem0_search` — it connects to your server.
...
```bash
hbm memory setup mem0 --mode oss \
...
```bash
hbm memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$HBM_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.hbm/mem0_qdrant"}}
  }
}
```
...
```bash
hbm memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$HBM_HOME/mem0.json`).
