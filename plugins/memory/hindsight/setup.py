"""`hbm memory setup` wizard for the Hindsight provider (``post_setup``)."""
...
def _select(title: str, items: list, values: list, current) -> str | None:
    ...
    from hbm_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
...
def _prompt_embedded_llm(llm_provider: str, provider_config: dict, env_writes: dict, hbm_env: Path) -> None:
    ...
    env_writes["HINDSIGHT_LLM_API_KEY"] = llm_key or _load_simple_env(hbm_env).get("HINDSIGHT_LLM_API_KEY", "")


def run_setup(provider, hbm_home: str, config: dict) -> None:
    ...
    if mode == "cloud":
        ...
        _prompt_embedded_llm(llm_provider, provider_config, env_writes, hbm_env)

    provider_config.setdefault("bank_id", "hbm")
    ...
    save_config(config)
    provider.save_config(provider_config, hbm_home)
    if env_writes:
        _write_env(hbm_env, env_writes)
    ...
    if _hs_templates.supported_for_mode(mode):
        from hbm_cli.memory_setup import _CANCELLED, _curses_select
        ...
        _hs_templates.run_template_step(
            ...
            bank_id=provider_config.get("bank_id", "hbm"),
    ...
    if mode == "local_embedded":
        ...
        with contextlib.suppress(Exception):
            materialized_config = json.loads(
                (Path(hbm_home) / "hindsight" / "config.json").read_text(encoding="utf-8-sig")
            )
        llm_api_key = (
            env_writes.get("HINDSIGHT_LLM_API_KEY", "")
            or _load_simple_env(hbm_env).get("HINDSIGHT_LLM_API_KEY", "")
