...
catalog, filter to the ``hbm`` integration, apply a manifest via
...
def fetch_hbm_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``hbm`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "hbm" in (e.get("integrations") or [])]
