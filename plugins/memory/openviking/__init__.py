...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-hbm/{_HBM_VERSION}"
...
_SYNC_TRACE_ENV = "HBM_OPENVIKING_SYNC_TRACE"
...
_RETRY_LATER = (
    "OpenViking memory is temporarily unavailable; HBM AGENT will retry on a later access or when the config changes."
)
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip HBM AGENT slash-skill scaffolding before sending content to OpenViking
...
# One entry per HBM AGENT home: a multiplexed gateway initializes a provider per profile and every
...
def _normalize_openviking_url(url: str) -> str:
    ...
    try:
        ...
        raise _OpenVikingEndpointError("OpenViking endpoint safety validation failed; HBM AGENT refused the connection.") from exc
    if blocked:
...
def _load_hbm_openviking_config() -> dict:
    try:
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        return _LOCAL_SERVER_OCCUPIED, (
            f"Port {host}:{port} is occupied by {_describe_local_port_listener(host, port)}. HBM AGENT did not start "
    ...
    log_path = get_hbm_home() / _OPENVIKING_SERVER_LOG_RELATIVE_PATH
    try:
        log_path.parent.mkdir(parents=True, exist_ok=True)
        # Strip PYTHONPATH: the Desktop backend puts the HBM AGENT venv on it, which
        # would shadow openviking-server's own site-packages (and on Windows lock
        # the HBM AGENT venv's .pyd files, breaking `hbm update`).
        # Do not let the server child inherit this process's PYTHONPATH. If inherited, openviking-server
        # would import aiohttp and friends from the HBM AGENT venv instead of its own (its venv's site-packages
        # are shadowed because PYTHONPATH precedes them) — and on Windows the loaded DLLs then lock the
        # HBM AGENT venv, aborting `hbm update` with access-denied on .pyd files. (#78153)
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._session_id, self._turn_count, self._hbm_home = "", 0, ""
        ...
        # increment. See hbm-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if get_secret("OPENVIKING_ENDPOINT", ""):
            return True
        provider_config = _load_hbm_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], hbm_home: str) -> None:
    ...
    def post_setup(self, hbm_home: str, config: dict) -> None:
        """Interactive setup that can reuse OpenViking's shared CLI config (see ``_setup``)."""
        _setup.run_setup(hbm_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_hbm_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        self._hbm_home = str(kwargs.get("hbm_home") or "").strip() or str(get_hbm_home())
        ...
        _active_providers_by_home[self._hbm_home] = self  # atexit safety net
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        so OPENVIKING_* values added to ``~/.hbm/.env`` after startup never reach the live client and
        tools keep running against stale auth until the user restarts hbm (#21130).
        """
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; HBM AGENT will retry on a later access (after cooldown) or when the config changes.",
                health_message,
            )
        else:
    ...
    def _extract_current_turn_messages(messages: Optional[List[Dict[str, Any]]], user_content: str, assistant_content: str) -> List[Dict[str, Any]]:
        """Slice the completed turn out of HBM AGENT's full canonical transcript: the last
    ...
    def _messages_to_openviking_batch(messages: List[Dict[str, Any]], *, assistant_peer_id: str = "") -> List[Dict[str, Any]]:
        """Convert HBM AGENT canonical messages into OpenViking batch payloads.
    ...
    def _state_path(self, kind: str, name: str) -> Optional[Path]:
        """Marker/lock file under HBM_HOME: ``pending`` -> pending_sessions/<sid>.json,
        ``lock`` -> runs/<run_id>.lock; an empty run id maps to the legacy recovery lock."""
        name = str(name or "").strip()
        if not self._hbm_home or (not name and kind != "lock"):
            return None
        if kind == "pending":
            return Path(self._hbm_home) / _PENDING_SESSIONS_RELATIVE_DIR / f"{quote(name, safe='')}.json"
        return Path(self._hbm_home) / _RUN_LOCKS_RELATIVE_DIR / (f"{quote(name, safe='')}.lock" if name else _LEGACY_RECOVERY_LOCK_FILENAME)
    ...
    def _pending_sessions(self) -> List[tuple[str, str]]:
        ...
        directory = Path(self._hbm_home) / _PENDING_SESSIONS_RELATIVE_DIR if self._hbm_home else None
    ...
    def shutdown(self) -> None:
        ...
        if _active_providers_by_home.get(self._hbm_home) is self:
            del _active_providers_by_home[self._hbm_home]
    ...
    def _tool_remember(self, args: dict) -> str:
        """Submit content through a dedicated session so it never touches the live HBM AGENT session."""
        ...
        session_id = f"hbm-remember-{uuid.uuid4().hex[:12]}"
        ...
        def failure(message: str, *, stage: str, message_status: str) -> str:
            return tool_error(
                ...
                recovery_note=(
                    ...
                    "HBM AGENT. Otherwise, do not resubmit automatically; report the uncertain state to the user."
                ),
            )
        try:
