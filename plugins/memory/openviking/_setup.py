"""Interactive ``hbm memory setup`` wizard for the OpenViking provider.

Pure UI flow: prompts, menus, and persistence of the chosen connection (HBM AGENT
``.env`` only, or mirrored to an ``ovcli.conf.<name>`` profile that HBM AGENT then
...
def _save_hbm_only_config(*, config: dict, provider_config: dict, env_path: Path, values: dict) -> None:
    ov = _ov()
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new HBM AGENT session to activate.\n")
...
def _run_create_profile_setup(*, prompt, select, cancelled, config: dict, provider_config: dict, env_path: Path) -> bool | object:
    ...
    save_choice = select("  Save OpenViking config",
                         [("Keep in HBM AGENT only", "write values only to HBM AGENT .env"),
    ...
    _save_hbm_only_config(config=config, provider_config=provider_config, env_path=env_path, values=values)
    _print_openviking_ready("Connection saved to HBM AGENT .env.")
    return True


def run_setup(hbm_home: str, config: dict) -> None:
    """Entry point for ``OpenVikingMemoryProvider.post_setup``."""
    from hbm_cli.config import save_config
    from hbm_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

    env_path = Path(hbm_home) / ".env"
