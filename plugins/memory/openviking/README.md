...
- OpenViking server running and reachable from HBM AGENT

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
HBM AGENT can identify older servers that expose the legacy status-only health
...
connection values into HBM AGENT, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.hbm/.env`; for a named profile use
...
OpenViking's server config is separate from HBM AGENT:
...
HBM AGENT-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, HBM AGENT lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
HBM AGENT sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
HBM AGENT also sends `User-Agent: openviking-memory-hbm/<version>` on
OpenViking requests. This standard harness identifier contains the HBM AGENT
...
does not ask for a peer ID. Without a configured peer, HBM AGENT sends neither
...
order remains environment, linked OpenViking config, then HBM AGENT YAML. To use
...
on the old implicit `hbm` peer now use user memory for new writes. Without
...
Set `agent: hbm` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `hbm-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live HBM AGENT
...
Use the same OpenViking profile and credentials as HBM AGENT for manual recovery.
...
HBM AGENT built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| HBM AGENT action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because HBM AGENT
...
file URIs, such as
`viking://user/default/peers/hbm/memories/preferences/mem_abc123.md` (any
