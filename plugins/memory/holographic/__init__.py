"""hbm-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $HBM_HOME/config.yaml under plugins.hbm-memory-store: db_path ($HBM_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from hbm_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "hbm-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hbm_home):
        """Write config to config.yaml under plugins.hbm-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *hbm_home* is the active profile already.
        from hbm_cli.config import save_config
        save_config({"plugins": {"hbm-memory-store": dict(values)}}, merge_existing=True)

    def get_config_schema(self):
        from hbm_constants import display_hbm_home
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_hbm_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _hbm_home = str(get_hbm_home())
        db_path = self._config.get("db_path", _hbm_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $HBM_HOME so paths resolve to the active profile
            db_path = db_path.replace("$HBM_HOME", _hbm_home).replace("${HBM_HOME}", _hbm_home)
