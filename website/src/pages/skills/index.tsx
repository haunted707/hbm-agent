        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            <div className={styles.installHint}>
              <code>{skill.installCmd || `hbm skills install ${skill.name}`}</code>
              <CopyButton
                text={skill.installCmd || `hbm skills install ${skill.name}`}
              />
            </div>
            {onPick ? (
              <button
...
export default function SkillsDashboard() {
  ...
  // app (HBM AGENT desktop's Bot Mode agent editor) as a skill PICKER. Site
  // chrome is hidden via a CSS class and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'hbm-skill-pick', name, identifier, installCmd, source }
      ...
      window.parent.postMessage(
        {
          ...
          installCmd: skill.installCmd || `hbm skills install ${skill.name}`,
          source: skill.source,
        },
        "*"
      );
  ...
  return (
    <Layout
      title="Skills Hub"
      description="Browse all skills and plugins available for HBM AGENT"
    >
