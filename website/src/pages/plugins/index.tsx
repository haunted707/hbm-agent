...
// hbm_cli/plugin_catalog.py and website/scripts/extract-plugins.py.
const CATEGORY_CONFIG: Record<string, { label: string; icon: string; blurb: string }> = {
  desktop: { label: "Desktop", icon: "\u{1F5A5}\u{FE0F}", blurb: "Panes, tabs and views for HBM AGENT Desktop" },
            ...
            {plugin.requiresHbm && (
              <div className={styles.metaRow}>
                ...
                <span className={styles.metaValue}>
                  <code>hbm {plugin.requiresHbm}</code>
                </span>
              </div>
            )}
            <div className={styles.metaRow}>
...
export default function PluginCatalogPage() {
  ...
  // (HBM AGENT desktop's Capabilities > Plugins tab) as a one-click catalog
  // picker. Site chrome is hidden via CSS and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'hbm-plugin-pick', name, repo, sha, subdir, tier,
  //     installCmd }
  ...
  const pickPlugin = useCallback((plugin: CatalogPlugin) => {
    ...
    window.parent.postMessage(
      {
        type: "hbm-plugin-pick",
        name: plugin.name,
        repo: plugin.repo,
        sha: plugin.sha,
        subdir: plugin.subdir || "",
        tier: plugin.tier,
        installCmd: plugin.installCommand || `hbm plugins install ${plugin.name}`,
      },
      "*"
    );
  }, []);
  ...
  return (
    <Layout
      ...
      description="Browse reviewed, SHA-pinned plugins for HBM AGENT"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>HBM AGENT</p>
          ...
          {!data && !loadError ? (
            ...
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>
                The plugin catalog is a curated, human-reviewed list of HBM AGENT
