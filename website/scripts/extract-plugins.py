...
- ``plugin-catalog.json`` — ``{"entries": [raw YAML mappings], "removed": [...]}`` in the loader's own
  schema; installed HBM AGENT clients fetch this for live catalog refresh
  (``hbm_cli.plugin_catalog.LIVE_CATALOG_URL``) so new entries and removals reach them without
...
def load_catalog_entries(catalog_dir: Path, stars: dict[str, int] | None = None) -> list[dict]:
    ...
    for path in sorted(catalog_dir.glob("*.yaml")):
        ...
        entries.append({
            ...
            "requiresHbm": str(raw.get("requires_hbm") or "").strip(),
            ...
            "installCommand": f"hbm plugins install {name}",
