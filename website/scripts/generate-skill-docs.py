...
def render_skill_page(
    ...
    hbm_meta = (fm.get("metadata") or {}).get("hbm") or {}
    tags = hbm_meta.get("tags") or []
    related = hbm_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `hbm skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with HBM AGENT"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "HBM AGENT ships with a large built-in skill library copied into `~/.hbm/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "HBM AGENT also syncs bundled skills on `hbm update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hbm/skills/` tree, it is still shipped with HBM AGENT; restore it with `hbm skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with hbm-agent — install via hbm skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with hbm-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "hbm skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "hbm skills install official/blockchain/solana",
        "hbm skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "hbm skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
