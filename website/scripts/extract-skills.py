...
2. The unified HBM AGENT Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                hbm_meta = metadata.get("hbm", {})
                if isinstance(hbm_meta, dict):
                    tags = hbm_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
