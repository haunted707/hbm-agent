...
| | |
|---|---|
| 来源 | 可选 — 通过 `hbm skills install official/finance/3-statement-model` 安装 |
...
以下许多段落提到"使用 S&P Kensho MCP / Daloopa MCP / FactSet MCP"。这些是原 Cowork 插件上下文中的商业金融数据 MCP。在 HBM AGENT 中：

- **如果已配置任何结构化金融数据 MCP**（HBM AGENT 支持 MCP — 参见 `native-mcp` skill），优先使用它获取时点可比数据、前例交易和申报文件。
- **否则**，回退至：
