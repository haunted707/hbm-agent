...
本 skill 专为 HBM AGENT 设计。它使用 HBM AGENT 工具、委派、调度和记忆来支撑完整的研究生命周期。

### 相关 Skill

将本 skill 与其他 HBM AGENT skill 组合用于特定阶段：
...
| **plan** | 阶段 0（设置）：执行前创建结构化计划。写入 `.hbm/plans/` | `skill_view("plan")` |
