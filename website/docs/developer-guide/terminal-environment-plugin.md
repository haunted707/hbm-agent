...
HBM AGENT runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.hbm/plugins/`, registering a backend the user selects
...
| `hbm setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `hbm status` / `hbm doctor` | `doctor_checks()` |
...
```python title="~/.hbm/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.hbm"    # where synced cache files land, or None
...
```yaml title="~/.hbm/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
hbm plugins enable acmebox
hbm config set terminal.backend acmebox
```
