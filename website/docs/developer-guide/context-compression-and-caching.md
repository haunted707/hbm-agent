...
HBM AGENT uses a dual compression system and Anthropic prompt caching to
...
Configure via `hbm plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

HBM AGENT has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.hbm/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|hbm|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `hbm`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
excluded because they already unlock the wider window), HBM AGENT raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$HBM_HOME`
...
```bash
hbm config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
hbm config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). HBM AGENT keeps the
...
`gpt-5.4-900k`). These are HBM AGENT-side aliases: the suffix is stripped before
...
thread context, so HBM AGENT's auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and HBM AGENT
  records the resulting compaction events (compression counters, session
  events). Set `hbm` to let HBM AGENT's compression threshold initiate
  app-server compaction, or `off` to disable HBM AGENT-initiated automatic
  compaction entirely (codex may still compact natively).

HBM AGENT's local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep HBM AGENT's summary compressor.
...
encrypted `compaction` output item. HBM AGENT captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. HBM AGENT
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   HBM AGENT can avoid; user-facing docs for `/model`, fallback providers, and
