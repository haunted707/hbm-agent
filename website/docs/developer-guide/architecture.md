...
sidebar_position: 1
title: "Architecture"
description: "HBM AGENT internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of HBM AGENT internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ hbm_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # HbmCLI facade — mixins in hbm_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── hbm_state.py           # SQLite session/state database facade (+ hbm_state_*.py siblings)
├── hbm_constants.py       # HBM_HOME, profile-aware paths
...
│
├── hbm_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `hbm` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # hbm skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # hbm tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # hbm gateway start/stop
│
...
```text
User input → HbmCLI.process_input()
...
Three discovery sources: `~/.hbm/plugins/` (user), `.hbm/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `hbm plugins` or `config.yaml`.
...
Exposes HBM AGENT as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`hbm -p <name>`) gets its own HBM_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
