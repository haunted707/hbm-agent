...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_hbm_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`HBM_HOME`, `HBM_PROFILE`, proxy settings,
...
reload helper: `hbm_cli.env_loader.load_hbm_dotenv` skips the
...
## The HBM_HOME override

`hbm_constants.py` holds a context-local override consulted by
`get_hbm_home()` before the `HBM_HOME` env var. Everything that resolves
...
automatically. `get_process_hbm_home()` exists for the few machine-level
assets that must not follow the override. `hbm_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `hbm_cli/profiles.py`
...
DB handles are resolved at call time through the active HBM_HOME override —
...
HBM_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `hbm_home_key()`. |
...
- `agent/secret_scope.py`, `hbm_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `hbm_cli/profiles.py`
