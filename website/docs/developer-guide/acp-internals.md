...
The ACP adapter wraps HBM AGENT's synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
hbm acp / hbm-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.hbm/.env
  -> configure stderr logging
  -> construct HbmACPAgent
...
- `allow_once` -> HBM AGENT `once`
- `allow_always` -> HBM AGENT `always`
- reject options -> HBM AGENT `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps HBM AGENT tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["hbm-acp"])
...
Instead it reuses HBM AGENT's runtime resolver:

- `acp_adapter/auth.py`
- `hbm_cli/runtime_provider.py`

So ACP advertises and uses the currently configured HBM AGENT provider/credentials. It also always advertises a terminal setup auth method (`hbm-setup`, args `--setup`) so first-run ACP clients can open HBM AGENT's interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.hbm/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `hbm-acp` toolset definition
- `hbm_cli/main.py` — `hbm acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `hbm-acp` script
