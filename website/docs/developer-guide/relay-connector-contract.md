...
description: "Wire contract between the HBM AGENT gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **HBM AGENT gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `hbm gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`hbm gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `hbm` CLI.
