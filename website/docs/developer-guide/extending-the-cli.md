...
description: "Build wrapper CLIs that extend the HBM AGENT TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

HBM AGENT exposes protected extension hooks on `HbmCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `hbm_cli/cli_tui_mixin.py` (where these hooks are defined; `HbmCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends HBM AGENT."""
...
Called after HBM AGENT registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
