...
description: "How HBM AGENT recovers state.db when the FTS index or the file itself is corrupt"
---
...
`hbm doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `hbm doctor --fix` while the gateway
is still up. Next steps:

```bash
hbm gateway stop
HBM_HOME="$HOME/.hbm" hbm sessions recover --source "$HOME/.hbm/state.db" --inspect-only
# if recoverable:
HBM_HOME="$HOME/.hbm" hbm sessions recover --source "$HOME/.hbm/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
hbm gateway stop
HBM_HOME="$HOME/.hbm" hbm sessions repair --check-only
HBM_HOME="$HOME/.hbm" hbm sessions repair
```
