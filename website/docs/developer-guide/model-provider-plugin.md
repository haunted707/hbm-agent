...
description: "How to build a model provider (inference backend) plugin for HBM AGENT"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that HBM AGENT can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$HBM_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with HBM AGENT
2. **User plugins** — `$HBM_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Installed plugins** — `$HBM_HOME/plugins/<name>/` (where `hbm plugins install owner/repo` clones) — imported only when `plugin.yaml` declares `kind: model-provider`; every other kind there belongs to the general PluginManager
4. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$HBM_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `hbm plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `hbm_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `hbm_cli/main.py` | Accepts `acme-inference` |
| `hbm model` picker | `hbm_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `hbm doctor` | `hbm_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `hbm setup` | `hbm_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `hbm_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `hbm model` picker |
...
An agent CLI driven over stdio is not an HTTP endpoint. Set `auth_type="external_process"`, describe how to launch the binary, and supply the client with `create_client`. No core edits are needed — `hbm -m <name>`, `/model`, credential resolution, runtime resolution and the auxiliary client (compression, vision) all key on `auth_type`, not on the provider name. `plugins/model-providers/copilot-acp/` is the in-tree example.
...
The client your `create_client` returns receives `command` and `args` in `client_kwargs`. If it is already complete and async-safe, declare `HBM_SKIP_TRANSPORT_WRAP = True` / `HBM_SKIP_ASYNC_WRAP = True` as class attributes so the auxiliary client does not re-dispatch it through an HTTP wire adapter.
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.hbm/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. HBM AGENT picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but HBM AGENT's CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `HBM_HOME` at a temp directory so you don't pollute your real config:

```bash
export HBM_HOME=/tmp/hbm-plugin-test
mkdir -p $HBM_HOME/plugins/model-providers/my-provider
cat > $HBM_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
hbm -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `hbm plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$HBM_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Model providers can ship as a pip package. Expose an entry point in the
`hbm_agent.plugins` group in your `pyproject.toml`:

```toml
[project.entry-points."hbm_agent.plugins"]
acme-inference = "acme_hbm_plugin:register"
```
...
- **Lowest precedence.** Entry-point plugins are discovered **before**
  ...
  bundled or `$HBM_HOME` profile of the same name always overrides a
...
See [Building an HBM AGENT Plugin](/developer-guide/plugins#distribute-via-pip) for the full entry-points setup.
...
- [Building an HBM AGENT Plugin](/developer-guide/plugins) — general plugin authoring
