...
| `rewind_session(session_id, n=1)` | Back up `n` user turns via soft-delete (keeps audit trail); thin wrapper over `SessionDB.rewind_user_turn` (`hbm_state_rewind.py`), the one rewind shared with CLI `/undo`/`/retry` and the TUI. Returns `{rewound_count, turns_undone, target_text}`. |
...
The canonical transcript store is SQLite via `SessionDB` (from `hbm_state`). The
...
Counts consecutive restarts via a JSON file (`{HBM_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `hbm update`, `hbm gateway restart`,
or `/restart`.
