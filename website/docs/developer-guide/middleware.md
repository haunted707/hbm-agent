...
# HBM AGENT Middleware

HBM AGENT middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing HBM AGENT's
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before HBM AGENT calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving HBM AGENT retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving HBM AGENT guardrails,
...
- `telemetry_schema_version`: currently `hbm.observer.v1`
- `middleware_schema_version`: currently `hbm.middleware.v1`
...
HBM AGENT stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, HBM AGENT runs
them as a nested chain in registration order. Middleware failures are fail-open:
HBM AGENT logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, HBM AGENT applies middleware in this order:
...
For each tool call, HBM AGENT applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal HBM AGENT pre-execution path against the effective arguments:
...
For isolated local testing, use one `HBM_HOME` for plugin enablement and the
agent run:

```bash
export HBM_HOME=/tmp/hbm-middleware-test
mkdir -p "$HBM_HOME"
hbm plugins enable <plugin-name>
hbm chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run hbm plugins enable <plugin-name>
uv run hbm chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["hbm_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/hbm-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape HBM AGENT expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, HBM AGENT treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, HBM AGENT preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. HBM AGENT does not convert downstream
