...
HBM AGENT gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by HBM AGENT's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `hbm.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `hbm.platform.up/degraded` with bounded `error_code` attributes |
...
`hbm.gateway.active_agents`, `hbm.gateway.background_work`, and
`hbm.gateway.background_delegations` are complementary. `active_agents`
...
```bash
hbm monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'hbm-agent[otlp]'`),
...
a plain `hbm gateway run`: the exporter lives in the gateway process, so
...
`hbm.gateway.up`, `hbm.platform.up`, and `hbm.platform.degraded`.
...
absent_over_time(hbm_gateway_up[5m])

# Locally owned bridge is explicitly down.
hbm_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
hbm_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
hbm_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
hbm_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(hbm_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `hbm.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
hbm.status = failed|unknown
hbm.delivery_outcome = failed|not_configured
hbm.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `hbm.job_key`;
...
HBM AGENT-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/hbm_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/hbm_otel_capture.jsonl --wait 8
...
`hbm.error_class = ...` list in this file's alert section and the enum's unit
...
The `hbm monitoring` CLI intentionally exposes `status` only. This first
release covers only HBM AGENT-owned service-health and operational-diagnostic
signals, including HBM AGENT-owned Relay transport health. Team Gateway's
