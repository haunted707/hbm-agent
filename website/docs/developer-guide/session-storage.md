...
HBM AGENT uses a SQLite database (`~/.hbm/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `hbm_state.py` (facade) plus the `hbm_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

## HBM AGENT home and profile isolation

`get_hbm_home()` is the authoritative filesystem resolver for state and
configuration. It uses a context-local override first, then the `HBM_HOME`
environment variable, and finally the platform default (`~/.hbm` on macOS
and Linux; `%LOCALAPPDATA%/hbm` on Windows). Consequently, the default
database is always `get_hbm_home() / "state.db"`, not a path that callers
should hard-code as `~/.hbm/state.db`.

Named profiles are isolated directories: a profile named `coder`, for example,
uses `<default HBM AGENT root>/profiles/coder/` and therefore has its own
...
profile must retain or pass that profile's `HBM_HOME`; falling back to the
...
of HBM AGENT. An explicit `--profile`/`-p` resolves that profile and writes the
resolved directory to `HBM_HOME`. Without an explicit selector, a
profile-specific `HBM_HOME` is preserved; otherwise the bootstrap can use
the default root's active-profile selection. `HOME` only determines the
platform default used when no context override or `HBM_HOME` is available.
Changing `HOME` is not a safe way to select a named profile. In particular, a
subprocess that drops `HBM_HOME` can fall back to the default profile even
when another profile is active, so subprocess spawners should pass
`HBM_HOME` explicitly.

Use `display_hbm_home()` only for user-facing text. It formats the resolved
home relative to the user's home directory when possible (for example,
`~/.hbm/profiles/coder`); it does not provide a separate resolution rule.

### Test isolation guard

Tests must use a temporary `HBM_HOME` or an explicit temporary database
path. The live-system guard raises before a test-context process opens a
production `state.db` under the real default HBM AGENT root or a real named
profile, preventing fixture data or SQLite side effects from reaching a live
installation.

`HBM_STATE_DB_GUARD_BYPASS=1` is a test-only escape hatch for a spawned
...
either bypass in normal HBM AGENT commands, development shells, or application
...
Each named profile stores its transcript in its own `$HBM_HOME/state.db`,
including when one `hbm serve` process serves several profiles. In-session
...
splice boundary, HBM AGENT excludes only that leading item when it exactly matches
...
`hbm sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `hbm_state_common.py` (applied by `hbm_state_schema.py`) for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `hbm_state_common.py` for the exact SQL.
...
Multiple hbm processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.hbm/state.db
...
Default path: `get_hbm_home() / "state.db"` — `~/.hbm/state.db` for the
default profile, `~/.hbm/profiles/<name>/state.db` for a named profile, or
wherever `HBM_HOME` points (see [HBM AGENT home and profile isolation](#hbm-home-and-profile-isolation)).
