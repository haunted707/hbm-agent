...
# HBM AGENT Observer Hooks

HBM AGENT observer hooks are the read-only telemetry contract for plugins that
...
They do not replace HBM AGENT's planner, model providers, memory, tool registry,
...
HBM AGENT also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "hbm.observer.v1"
```

Hook callbacks are fail-open. HBM AGENT catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so HBM AGENT only builds
...
The native NeMo Relay SDK integration maps HBM AGENT session, turn, LLM, and tool
