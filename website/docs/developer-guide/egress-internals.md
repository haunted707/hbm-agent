...
description: "How the iron-proxy egress firewall integrates with HBM AGENT — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`hbm egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
hbm_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `hbm egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

hbm_cli/subcommands/egress.py:_dispatch_egress
                                       Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `hbm proxy` subparser, which uses
                                       dest='proxy_command').

hbm_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _HBM_EGRESS_NODE_OPTIONS_APPEND
...
tests/hbm_cli/test_iron_proxy_cli.py          CLI handler unit tests (~20).  Argparse
                                       wiring, fail-loud paths, BWS refresh
                                       wire-up, dest='egress_command'
                                       regression guard.

tests/agent/test_iron_proxy_e2e.py          Live E2E (gated on HBM_RUN_E2E=1).
...
hbm egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.hbm/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.hbm/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

hbm egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.hbm/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `hbm egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `hbm egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `hbm egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, HBM_EGRESS_PROXY) and the per-mapping `HBM_PROXY_TOKEN_<NAME>` vars.
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/hbm-egress-ca.crt`).
...
- Bitwarden Secrets Manager integration: [`hbm secrets bitwarden`](https://hermes-agent.nousresearch.com/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`hbm egress`](https://hermes-agent.nousresearch.com/docs/reference/cli-commands#hbm-egress)
