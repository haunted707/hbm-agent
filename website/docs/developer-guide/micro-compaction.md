...
has to be thrown away or summarized. HBM AGENT has always done this in one batch:
...
HBM AGENT folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. HBM AGENT's normal session flush is
...
With `micro_compact` unset or `false` HBM AGENT behaves exactly as it always has:
...
Defaults to `$HBM_HOME/logs/agent.log`. It reports passes, outcome mix, net
