...
title: "Build an HBM AGENT Plugin"
description: "Step-by-step guide to building a complete HBM AGENT plugin with tools, hooks, data files, and skills"
---

# Build an HBM AGENT Plugin

This guide walks through building a complete HBM AGENT plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
HBM AGENT has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `HBM_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.hbm/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `hbm skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
hbm plugins install owner/repository --no-enable
hbm plugins list
hbm plugins enable <plugin-name>
```
...
HBM AGENT validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by HBM AGENT.
...
Portable `streamable-http` entries are routed through HBM AGENT's existing native
...
same full-trust posture as other installed HBM AGENT plugins.
...
records it as Published. HBM AGENT keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. HBM AGENT does not expose a
...
continue to work after a normal HBM AGENT upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  HBM AGENT inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older HBM AGENT releases can therefore load a plugin whose manifest contains
...
HBM AGENT enforces this contract with frozen external-plugin fixtures discovered
from an isolated `HBM_HOME`. Those tests load and invoke the plugin through
...
HBM AGENT's internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `hbm plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `hbm doctor` and after `hbm update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `HbmPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `hbm plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.hbm/plugins/calculator
cd ~/.hbm/plugins/calculator
```

### Validate with Plugin Doctor

`hbm plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by HBM AGENT itself. It reports invalid hook names, callbacks that do
...
```bash
hbm plugins doctor . --ci
```

Doctor uses a temporary `HBM_HOME`, restores plugin registration state after
...
This tells HBM AGENT: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `hbm_cli/plugin_capabilities.py` for the
...
`hbm_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`hbm_agent.plugins` entry point:

```toml
[project.entry-points."hbm_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."hbm_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

HBM AGENT reads these from installed metadata without importing your code, so
`hbm plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this HBM AGENT understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — HBM AGENT validates them, and `hbm plugins install` / `hbm plugins doctor` surface missing ones with a `pip install` hint, but HBM AGENT **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into HBM AGENT's shared venv is a conflict and supply-chain
...
4. **Accept `**kwargs`:** HBM AGENT may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `hbm my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but HBM AGENT continues fine
...
Use plugin-relative config keys for user-visible behavior. HBM AGENT resolves them
...
`<HBM_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `HBM_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
HBM_PLUGINS_DEBUG=1 hbm plugins list
```
...
The same logs are always written to `~/.hbm/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
hbm logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `hbm plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.hbm/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `hbm plugins update` / `remove` git-pull or delete it — your
...
# <hbm home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.hbm/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.hbm/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `hbm plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing HBM AGENT into installing arbitrary packages — only specs HBM AGENT itself ships are eligible |
...
auditable in `~/.hbm/logs/agent.log`. Plugins load after built-in
...
does not ship with HBM AGENT core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `hbm plugins list`
...
loader, the plugin is disabled and HBM AGENT continues. The gate exists
...
You normally never edit this key by hand. `hbm plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), HBM AGENT injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: hbm-agent"}
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $HBM_HOME/hook_outputs
```
...
- **The system prompt is HBM AGENT's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `hbm_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before HBM AGENT sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`hbm.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer HBM AGENT still loads on an older one.

### Register CLI commands

Plugins can add their own `hbm <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for hbm my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: hbm my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for hbm my-plugin."""
...
After registration, users can run `hbm my-plugin status`, `hbm my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `hbm name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `hbm chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `HBM_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `hbm <subcommand>` (e.g. `hbm kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "hbm kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive HBM AGENT from a hook.
...
HBM AGENT has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.hbm/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `hbm gateway status` without SDK instantiation.
...
HBM AGENT also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into HBM AGENT without any Python plugin. Declare them in `~/.hbm/config.yaml`:
...
HBM AGENT connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.hbm/hooks/<name>/`:

```yaml
# ~/.hbm/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.hbm/hooks/long-task-alert/handler.py
...
hbm skills search my-workflow --source myorg/skills-repo
hbm skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install hbm-plugin-calculator
# Plugin auto-discovered on next hbm startup
```
...
```python
# Wrong — will break if HBM AGENT passes extra context
def handler(args):
    ...
