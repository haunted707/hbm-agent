...
Plugins can launch and supervise fresh HBM AGENT child sessions without importing
...
active agent turn fails closed with `No active HBM AGENT parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until HBM AGENT can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; HBM AGENT's existing unsafe-tool block remains enforced.
