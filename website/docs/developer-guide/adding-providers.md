...
description: "How to add a new inference provider to HBM AGENT — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

HBM AGENT can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `hbm model` menu entries
...
1. `hbm_cli/auth.py` decides how credentials are found.
2. `hbm_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `hbm_cli/models.py` and `hbm_cli/main.py` make the provider show up in the CLI. (`hbm_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`hbm_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `hbm setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$HBM_HOME/plugins/model-providers/` both get picked up.
...
7. `hbm model` menu includes the provider
8. `hbm setup` wizard delegates to `main.py` automatically
...
User plugins at `$HBM_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `hbm model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `hbm_cli/auth.py`
- `_PROVIDER_LABELS` in `hbm_cli/models_catalog_static.py` (re-exported by `hbm_cli/models.py`)
- `_PROVIDER_ALIASES` in both `hbm_cli/auth.py` and `hbm_cli/models_catalog_static.py`
- CLI `--provider` choices in `hbm_cli/main.py`
...
## Step 2: Add auth metadata in `hbm_cli/auth.py`
...
- What env vars should HBM AGENT check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `hbm_cli/models.py`
...
## Step 4: Resolve runtime data in `hbm_cli/runtime_provider.py`
...
Be careful with API-key precedence. HBM AGENT already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hbm_cli/main.py`

A provider is not discoverable until it shows up in the interactive `hbm model` flow.

Update these in `hbm_cli/main.py`:
...
:::tip
`hbm_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `hbm model` and `hbm setup` automatically.
:::
...
When you add a native provider, double-check that HBM AGENT is only sending fields that provider actually understands.
...
- `tests/hbm_cli/test_runtime_provider_resolution.py`
- `tests/hbm_cli/test_cli_provider_resolution.py`
- `tests/hbm_cli/test_model_switch_custom_providers.py` (and adjacent `tests/hbm_cli/test_model_switch_*.py`)
- `tests/hbm_cli/test_setup_model_provider.py`
...
python -m pytest tests/hbm_cli/test_runtime_provider_resolution.py tests/hbm_cli/test_cli_provider_resolution.py tests/hbm_cli/test_setup_model_provider.py tests/agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m hbm_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `hbm_cli/auth.py`
- [ ] aliases added in `hbm_cli/auth.py` and `hbm_cli/models.py`
- [ ] model catalog added in `hbm_cli/models.py`
- [ ] runtime branch added in `hbm_cli/runtime_provider.py`
- [ ] CLI wiring added in `hbm_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `hbm model` but not `hbm setup`
