...
The Python core runs fine from any [git worktree](../user-guide/git-worktrees.md) — `cd` in and `hbm` just works. The two TypeScript surfaces do not: `ui-tui/` and `apps/desktop/` each need a populated `node_modules`, and a fresh `npm ci` per worktree is slow and duplicates gigabytes across every branch you have checked out.
...
| Variable | Meaning |
|----------|---------|
| `HBM_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `HBM_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `HBM_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by HBM AGENT itself — they're private to these helpers. The variables HBM AGENT *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `hbm --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_hbm_root)" || { echo "htui: not in an HBM AGENT checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$HBM_MAIN_CHECKOUT/.venv/bin/python" -m hbm_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `HBM_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_hbm_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `HBM_TUI_DIR` are mutually exclusive
`HBM_TUI_DIR` points HBM AGENT at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `hbm --tui --dev` exits with an error. Run `unset HBM_TUI_DIR` before `htui`.
:::
...
hgui() (
  ...
  root="$(_hbm_root)" || { print -u2 'hgui: not in an HBM AGENT checkout'; return 1; }
  deps="${HBM_GUI_DEPS_CHECKOUT:-$HBM_MAIN_CHECKOUT}"
  desktop="$root/apps/desktop"

  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _hbm_link_deps "$desktop" "$deps/apps/desktop" || return 1
    _hbm_link_deps "$root" "$deps" || return 1
  else
  ...
  export HBM_DESKTOP_HBM_ROOT="$root"
  export HBM_DESKTOP_PYTHON="$HBM_MAIN_CHECKOUT/.venv/bin/python"
  export HBM_DESKTOP_CWD="$root"
  export HBM_DESKTOP_DEV_SERVER="http://127.0.0.1:$vite_port"
  export HBM_DESKTOP_CDP_PORT="$cdp_port"
  export HBM_DESKTOP_USER_DATA_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/hbm-hgui/slot-$slot"
  # A userData override would otherwise also relocate the agent's home.
  export HBM_HOME="${HBM_HOME:-$HOME/.hbm}"
...
For example, after setting `HBM_MAIN_CHECKOUT` and sourcing the helpers:

```bash
# Terminal 1: main checkout
cd "$HBM_MAIN_CHECKOUT"
HGUI_SLOT=0 hgui

# Terminal 2: an existing worktree
cd /path/to/hbm-worktree
HGUI_SLOT=1 hgui
```
...
|----------|----------------|
| `HGUI_SLOT` | Helper-only slot number, `0`–`9`; not an HBM AGENT setting. |
| `HBM_DESKTOP_HBM_ROOT` | Runs the backend from this worktree, not the packaged/PATH runtime. |
| `HBM_DESKTOP_PYTHON` | Reuses the main checkout's Python environment. Adjust for an installation that uses `venv` rather than `.venv`. |
| `HBM_DESKTOP_CWD` | Roots new desktop work in the worktree. |
| `HBM_DESKTOP_DEV_SERVER` | Points Electron at this instance's Vite server. |
| `HBM_DESKTOP_CDP_PORT` | Gives each instance its own renderer debugging port. |
| `HBM_DESKTOP_USER_DATA_DIR` | Separates Electron's single-instance lock, browser storage, and desktop preferences. |
| `HBM_HOME` | Explicitly preserves the agent home despite the Electron user-data override. |
...
The default `HBM_HOME` is shared: sessions, configuration, credentials, and profiles remain the same. Avoid editing the same conversation from both instances. For destructive tests or incompatible database migrations, pass a separate temporary `HBM_HOME` and configure that sandbox independently.
:::

Quit the app normally or press Ctrl-C in its launching terminal. `concurrently -k` manages its own child commands, and Electron owns its backend shutdown. Do not add a global `killport`, `pkill electron`, or a sweep of all `serve`/`dashboard --port 0` processes: those can terminate another instance. Remove the old `_hbm_gui_cleanup` trap if replacing an earlier version of this helper.
...
```bash
# The enclosing worktree, verified as a real HBM AGENT checkout.
_hbm_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/hbm_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
- [TUI](../user-guide/tui.md) — `hbm --tui --dev` and the `HBM_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `HBM_*` variable HBM AGENT reads
