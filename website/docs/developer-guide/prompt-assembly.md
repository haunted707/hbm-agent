...
description: "How HBM AGENT builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

HBM AGENT deliberately separates:
...
2. **context** — caller-supplied `system_message`, project context files (`.hbm.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`), then the worktree-dependent git workspace snapshot, operator instructions and platform hints
...
the first host-info paragraph after the rendered `# HBM AGENT runtime environment` boundary, with a
...
```
# Layer 1: Agent Identity (from ~/.hbm/SOUL.md)
You are HBM AGENT, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance HBM AGENT
...
`SOUL.md` lives at `~/.hbm/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_hbm_home() / "SOUL.md"
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hbm_md(cwd_path)       # 1. .hbm.md / HBM.md (walks to git root)
    ...
    # SOUL.md from HBM_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.hbm.md`, `HBM.md` | CWD up to git root | HBM AGENT-native project config |
...
- **YAML frontmatter stripped** — `.hbm.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, HBM AGENT concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.hbm.md` / `HBM.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs HBM AGENT already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.hbm/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.hbm/MEMORY.md` and `~/.hbm/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.hbm.md`, `HBM.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking HBM AGENT.
- Ephemeral overlays such as `HBM_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how HBM AGENT assembles prompts for everyone, change Python and treat it as a code contribution
