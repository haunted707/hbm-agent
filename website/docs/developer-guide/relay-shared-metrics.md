...
HBM AGENT includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into HBM AGENT and does not require an HBM AGENT observability plugin. HBM AGENT remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
HBM AGENT execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `hbm-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the HBM AGENT `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `HBM_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `HBM_NEMO_RELAY_ATOF_*` and `HBM_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, HBM AGENT does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

HBM AGENT requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts HBM AGENT
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

HBM AGENT installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the HBM AGENT process so configured interceptors can
...
Collection remains off unless HBM AGENT policy enables it:
...
opt-in. Set `HBM_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, HBM AGENT does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, HBM AGENT reports the error and does not invoke
...
retain the same `session_id` and add `hbm.session.segment` plus
`hbm.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not an HBM AGENT
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that HBM AGENT process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, HBM AGENT logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the HBM AGENT
process and share the same policy boundary.

Run profiles in separate HBM AGENT processes when they require different trust
...
HBM AGENT core owns one Relay host and one isolated Relay session scope per HBM AGENT
...
marks do not require HBM AGENT plugin registration. Shared-metrics marks must
...
```text
HBM AGENT turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> HBM AGENT shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

HBM AGENT sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that HBM AGENT used for the logical call, such as
...
New calls use `hbm.model_route.count`. The previous
`hbm.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `hbm.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; HBM AGENT
version, OS family, architecture, and install method remain bounded package
resources. Concurrent HBM AGENT processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `hbm.task_run`, parented to
the owning HBM AGENT session. The start counter contains only bounded execution
...
provider attempts for the same HBM AGENT API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their HBM AGENT tool-call
...
cancellations. Active task ownership follows the task ID if HBM AGENT rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`hbm.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. HBM AGENT
...
tool or plugin names. HBM AGENT does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `hbm.tool_approval` marks and
...
Successful skill mutations emit `hbm.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `hbm.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. HBM AGENT derives reuse and patch-generation
...
```text
$HBM_HOME/telemetry/shared_metrics/metrics.sqlite3
$HBM_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the HBM AGENT version,
...
schema field name, its current scope is one `HBM_HOME`, so it is more
...
Deleting `$HBM_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `HBM_HOME`. To reset it, stop HBM AGENT
processes and remove `$HBM_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real HBM AGENT CLI turn against the deterministic local model server:
...
`$HBM_HOME/telemetry/shared_metrics/outbox/` to the HBM AGENT telemetry ingest
...
Removing `$HBM_HOME/telemetry/shared_metrics` still resets local identity,
