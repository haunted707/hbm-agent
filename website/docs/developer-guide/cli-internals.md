...
sidebar_position: 15
title: "CLI Internals"
description: "How hbm_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `hbm_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `hbm_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `hbm_cli.update_cmd._hbm_holder_subcommand` (top-level subcommand of any HBM AGENT argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `hbm_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.hbm/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

HBM AGENT supports profiles — fully isolated instances, each with its own `HBM_HOME` (config, API
keys, memory, sessions, skills, gateway). For single-profile commands (`hbm -p x <cmd>`),
`_apply_profile_override()` in `hbm_cli/main.py` sets `HBM_HOME` before any module imports, so
every `get_hbm_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["HBM_HOME"]` stays the launch
...
`Path.home() / ".hbm" / "profiles"`, not `get_hbm_home() / "profiles"`) so
`hbm -p coder profile list` sees all profiles regardless of which one is active — intentional.
