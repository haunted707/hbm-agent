...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to HBM AGENT — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in HBM AGENT tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying HBM AGENT core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build an HBM AGENT Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_HBM_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `hbm_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `hbm_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `hbm chat -q "Use the weather tool for London"`
