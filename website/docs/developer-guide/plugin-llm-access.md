...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept HBM AGENT already has applies. The plugin never sees a
...
hbm-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — HBM AGENT provider id (e.g. "openrouter")
...
using HBM AGENT built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different HBM AGENT provider
        # (must be one HBM AGENT already knows about — same names as
        # `hbm model` and config.yaml model.provider).
        ...
        # plugin sends — HBM AGENT does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.hbm/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through HBM AGENT's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing HBM AGENT subsystem:
