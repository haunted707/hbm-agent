...
HBM AGENT saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the HBM AGENT function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `hbm` CLI
