...
title: "Desktop Plugin SDK (@hbm/plugin-sdk)"
description: "Extend the native HBM AGENT Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [HBM AGENT Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `HbmPlugin`.
It imports one module — `@hbm/plugin-sdk` — and gets everything: the app's
...
`$HBM_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across HBM AGENT. This page is the **native
desktop app** (`hbm desktop`) SDK — the `@hbm/plugin-sdk` module and
`$HBM_HOME/desktop-plugins/`. The **web dashboard** (`hbm dashboard`) has
its own, unrelated plugin system on `window.__HBM_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build an HBM AGENT Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$HBM_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$HBM_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `HbmPlugin` contract, appear in **Capabilities → Plugins**,
...
[`hbm-example-plugins`](https://github.com/NousResearch/hermes-example-plugins)
repo.

## Quick start — your first plugin

Create `$HBM_HOME/desktop-plugins/hello/plugin.js` (that's `~/.hbm/...`
...
```javascript
// ~/.hbm/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@hbm/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, HBM AGENT's }),
      jsx('div', {
...
The only importable specifiers are `@hbm/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `HbmPlugin`:
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `hbm.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@hbm/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@hbm/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@hbm/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@hbm/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@hbm/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@hbm/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@hbm/plugin-sdk'
...
`window.hbm.send('get-price eth')` (or a declarative
`<button data-hbm-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@hbm/plugin-sdk'
...
for requests; `targetProfile` is the backend HBM AGENT profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from HBM AGENT
...
ctx.os.notify({
  ...
  // Body click → focus HBM AGENT + navigate. Same vocabulary as OS deep links:
  activate: 'hbm://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'hbm://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `hbm://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `hbm://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@hbm/plugin-sdk'
...
```javascript
import { queryClient } from '@hbm/plugin-sdk'
...
copies that half into `$HBM_HOME/desktop-plugins/<id>/` beside a
`.hbm-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.hbm/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`hbm plugins update`, or
...
a package into `~/.hbm/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.hbm/plugins` is not reachable as a filesystem —
...
the `hbm://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular HBM AGENT plugin and declare it in a
`manifest.json`:
...
hbm-agent codebase directly (`hbm_state`, `hbm_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.hbm/`)
...
(`hbm.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `HbmPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@hbm/plugin-sdk` alias.
2. It's still lint-fenced to `@hbm/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@hbm/plugin-sdk`, `react`,
...
| Plugin contract | `HbmPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `HbmOpenTarget`, `Contribution` |
...
### Agents: the `hbm-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`hbm-desktop-plugins`** skill — it carries the same contract as this page in
...
`$HBM_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `hbm logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@hbm/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.hbm/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.hbm/logs/errors.log` for
