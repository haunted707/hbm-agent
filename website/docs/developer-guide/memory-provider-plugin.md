...
description: "How to build a memory provider plugin for HBM AGENT"
---

# Building a Memory Provider Plugin

Memory provider plugins give HBM AGENT persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `hbm plugins`.
:::

## Installation Layouts

HBM AGENT discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with HBM AGENT. Closed to new providers — see [CONTRIBUTING](https://github.com/NousResearch/hermes-agent/blob/main/CONTRIBUTING.md). |
| User | `$HBM_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.hbm/plugins/<name>/` | Opt-in via `HBM_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `hbm_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
HBM AGENT, in `$HBM_HOME/plugins/<name>/` when installed by a user, or in
`./.hbm/plugins/<name>/` for a project-local one:
...
`hbm_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."hbm_agent.memory_providers"]
...
package layout. No copy under `$HBM_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files HBM AGENT reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `hbm <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hbm_home (str): Active HBM_HOME path. Use for storage.
        """
...
| Keyword | Meaning |
|---|---|
| `hbm_home` | Active profile's storage directory. |
...
| `session_title_source` | Stored title provenance, when available: `derived`, `llm`, or `user`. Automatic sources must not be mistaken for explicit identity overrides; missing provenance retains a provider's legacy behavior. Shared constants live in `hbm_state_common.py`. |
...
| `agent_workspace`, `agent_context` | Runtime agent scope (`hbm` and `primary` for the main agent). |
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `hbm memory setup` | **Yes** |
| `save_config(values, hbm_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the HBM AGENT
...
required checkpoint cannot be guaranteed there. The checkpoint-aware HBM AGENT
...
`get_config_schema()` returns a list of field descriptors used by `hbm memory setup`:
...
Every field in `get_config_schema()` is prompted during `hbm memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$HBM_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, hbm_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(hbm_home) / "my-provider.json"
...
**`sync_turn()` MUST be non-blocking.** If your backend has latency (API calls, LLM processing), run the work in a daemon thread — spawned with `agent.memory_provider.spawn_context_thread`, never a bare `threading.Thread`. Profile isolation (the active `HBM_HOME`, the per-turn secret scope) lives in `contextvars`, and a plain thread starts with an empty context: under multiplexed profiles it would silently write into the *default* profile's store, and `get_secret()` fails closed there.
...
The same applies to prefetch and writer threads. Small JSON config sidecars (`$HBM_HOME/<provider>.json`) are read with `utils.read_json_or_empty` and written with `utils.atomic_json_write`; anything under `config.yaml` goes through `hbm_cli.config.save_config(..., merge_existing=True)`.
...
the `messages` parameter; HBM AGENT will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hbm_home` kwarg from `initialize()`, not hardcoded `~/.hbm`:

```python
# CORRECT — profile-scoped
from hbm_constants import get_hbm_home
data_dir = get_hbm_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.hbm/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `hbm my-provider status`, `hbm my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `hbm <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `hbm --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: hbm my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the hbm my-provider argparse tree.
