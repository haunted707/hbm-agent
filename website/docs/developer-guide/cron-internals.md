...
description: "How HBM AGENT stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hbm_cli/cron.py` | CLI `hbm cron` subcommands |
...
Jobs are stored in `~/.hbm/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`hbm cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
The same store fields drive every topology: a standalone `hbm -p X gateway
...
In CLI mode, cron jobs only fire when `hbm cron` commands are run or during active CLI sessions.
...
```python
# ~/.hbm/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.hbm/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `hbm cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `hbm cron` CLI provides direct job management:

```bash
hbm cron list                    # Show all jobs
hbm cron create                  # Interactive job creation (alias: add)
hbm cron edit <job_id>           # Edit job configuration
hbm cron pause <job_id>          # Pause a running job
hbm cron resume <job_id>         # Resume a paused job
hbm cron run <job_id>            # Trigger immediate execution
hbm cron remove <job_id>         # Delete a job
```
