...
description: "Authoritative reference for HBM AGENT terminal commands and command families"
---
...
```bash
hbm [global-options] <command> [subcommand/options]
```
...
| `--profile <name>`, `-p <name>` | Select which HBM AGENT profile to use for this invocation. Overrides the sticky default set by `hbm profile use`. |
...
| `--ignore-user-config` | Ignore `~/.hbm/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `HBM_TUI=1`. Always wins over `display.interface`. |
...
| Command | Purpose |
|---------|---------|
| `hbm chat` | Interactive or one-shot chat with the agent. |
| `hbm model` | Interactively choose the default provider and model. |
| `hbm moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `hbm fallback` | Manage fallback providers tried when the primary model errors. |
| `hbm gateway` | Run or manage the messaging gateway service. |
| `hbm proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `hbm egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `hbm lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `hbm setup` | Interactive setup wizard for all or part of the configuration. |
| `hbm whatsapp` | Configure and pair the WhatsApp bridge. |
| `hbm whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `hbm whatsapp` (Baileys personal-account bridge). |
| `hbm slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
| `hbm auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Nous/Anthropic. |
| `hbm login` / `logout` | **Deprecated** — use `hbm auth` instead. |
| `hbm send` | Send a one-shot message to a configured messaging platform (Telegram, Discord, Slack, Signal, SMS, …). Useful from shell scripts, cron jobs, CI hooks, and monitoring daemons — no agent loop, no LLM. |
| `hbm peer` | Register peer HBM AGENT gateways on other machines and DM their agents' canonical Bot Chats (`hbm peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
| `hbm secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.hbm/.env`. |
| `hbm migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `hbm status` | Show agent, auth, and platform status. |
| `hbm cron` | Inspect and tick the cron scheduler. |
| `hbm pause` / `hbm resume` | Global emergency stop: no new cron fires (built-in ticker, managed-cron webhook, misfire catch-up), kanban dispatch or gateway turns start until resumed; in-flight work is never killed. |
| `hbm kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `hbm project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `hbm webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `hbm hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `hbm doctor` | Diagnose config and dependency issues. |
| `hbm security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `hbm approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `hbm dump` | Copy-pasteable setup summary for support/debugging. |
| `hbm prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `hbm debug` | Debug tools — upload logs and system info for support. |
| `hbm backup` | Back up HBM AGENT home directory to a zip file. |
| `hbm checkpoints` | Inspect / prune / clear `~/.hbm/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `hbm import` | Restore an HBM AGENT backup from a zip file. |
| `hbm logs` | View, tail, and filter agent/gateway/error log files. |
| `hbm config` | Show, edit, migrate, and query configuration files. |
| `hbm skin` | List, switch, and tweak display skins. |
| `hbm console` | Open the safe HBM AGENT command console. |
| `hbm pairing` | Approve or revoke messaging pairing codes. |
| `hbm skills` | Browse, install, publish, audit, and configure skills. |
| `hbm bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `hbm curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `hbm journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `hbm memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `hbm honcho`) register automatically when their provider is active. |
| `hbm acp` | Run HBM AGENT as an ACP server for editor integration. |
| `hbm mcp` | Manage MCP server configurations and run HBM AGENT as an MCP server. |
| `hbm plugins` | Manage HBM AGENT plugins (install, enable, disable, remove). |
| `hbm portal` | Nous Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `hbm tools` | Configure enabled tools per platform. |
| `hbm computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `hbm pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
| `hbm sessions` | Browse, export, prune, rename, and delete sessions. |
| `hbm insights` | Show token/cost/activity analytics. |
| `hbm claw` | OpenClaw migration helpers. |
| `hbm import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `hbm dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `hbm serve` | Start the HBM AGENT backend server (headless; powers the desktop app and remote backends). |
| `hbm desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `hbm profile` | Manage profiles — multiple isolated HBM AGENT instances. |
| `hbm completion` | Print shell completion scripts (bash/zsh/fish). |
| `hbm --version` | Show version information. |
| `hbm update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `HBM_HOME` snapshot. |
| `hbm uninstall` | Remove HBM AGENT from the system. |
...
| `--ignore-user-config` | Ignore `~/.hbm/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from HBM AGENT itself. |
...
```bash
hbm
hbm chat -q "Summarize the latest PRs"          # seeds an interactive session
hbm chat --oneshot -q "Summarize the latest PRs"  # answer and exit
hbm chat --provider openrouter --model anthropic/claude-sonnet-4.6
hbm chat --toolsets web,terminal,skills
hbm chat --quiet -q "Return only JSON"
hbm chat -q "Inspect this repository" --format stream-json
hbm chat --worktree -q "Review this repo and open a PR"
hbm chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
hbm chat --safe-mode -q "Is this bug mine or HBM AGENT's?"
```
...
```bash
hbm chat -q "Summarize this repository" --format stream-json
```
...
(`HBM_KANBAN_TASK` set) whose turn failed only because the provider was
...
### `hbm -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `hbm -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
hbm -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(hbm -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.hbm/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `HBM_INFERENCE_MODEL` | Override the model for this run |
...
```bash
hbm -z "…" --provider openrouter --model openai/gpt-5.5
# or:
HBM_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 hbm -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `hbm chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".
...
```bash
hbm -z "summarize this repo" --usage-file /tmp/usage.json
jq .estimated_cost_usd /tmp/usage.json
```

## `hbm model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active HBM AGENT chat session.
...
:::warning hbm model vs /model — know the difference
**`hbm model`** (run from your terminal, outside any HBM AGENT session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active HBM AGENT chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your HBM AGENT session first (`Ctrl+C` or `/quit`), then run `hbm model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `hbm model` from the terminal.
:::
...
| `migrate-legacy` | Remove legacy `hbm.service` units left over from pre-rename installs. Profile units (`hbm-gateway-<profile>.service`) and unrelated services are never touched. Flags: `--dry-run`, `-y`/`--yes`. |
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [HBM AGENT Relay](/user-guide/messaging/relay). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `HBM_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `hbm update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `HBM_GATEWAY_NO_SUPERVISE=1`. |
...
`hbm gateway restart`, or service-restart update exits with status `75`
...
:::tip WSL users
Use `hbm gateway run` instead of `hbm gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s hbm 'hbm gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hbm-gateway-start-fails) for details.
:::
...
```bash
hbm setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure] [--portal]
```

**Easiest path:** `hbm setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `hbm setup` on an existing install now does this by default. |
...
For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `hbm setup --portal` above.
...
```bash
hbm slack manifest              # print manifest to stdout
hbm slack manifest --write      # write to ~/.hbm/slack-manifest.json
hbm slack manifest --long-description-file AGENTS.md --write
hbm slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$HBM_HOME/slack-manifest.json`. |
| `--name NAME` | `HBM AGENT` | Bot display name in Slack. |
...
Run `hbm slack manifest --write` again after `hbm update` to pick
up any new commands.
...
```bash
hbm send --to <target> "message text"
hbm send --to <target> --file <path>
echo "message" | hbm send --to <target>
hbm send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.hbm/.env` + `~/.hbm/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `hbm send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `hbm send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
hbm send --to telegram "MEDIA:/tmp/screenshot.png"
hbm send --to telegram "Build chart for today MEDIA:/tmp/chart.png"   # with caption
hbm send --to discord:#ops "MEDIA:/tmp/report.pdf"
```
...
```bash
hbm send --to telegram "[[as_document]] MEDIA:/tmp/screenshot.png"
```
...
```bash
hbm peer add <name> --url http://host:port --key <API_SERVER_KEY>
hbm peer list
hbm peer dm <peer>[/<agent>] "message"
hbm peer run <peer>[/<agent>] --idempotency-key <key> "message"
hbm peer status <peer>[/<agent>] <run_id>
hbm peer stop <peer>[/<agent>] <run_id>
hbm peer remove <name>
```

Bot-to-bot DMs across machines. Register another HBM AGENT gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`hbm peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`hbm -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `HBM_PEER_<NAME>_KEY` in `~/.hbm/.env`. |
...
includes the peer roster and the `hbm peer dm` pattern, so agents discover
...
```bash
hbm secrets bitwarden <subcommand>
hbm secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.hbm/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `hbm claw migrate` (one-shot import of OpenClaw configuration into HBM AGENT) — `hbm migrate` is the top-level config-rewrite command.


## `hbm proxy`

```bash
hbm proxy <subcommand>
```
...
```bash
hbm security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the HBM AGENT venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.hbm/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the HBM AGENT Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `hbm login` / `hbm logout` *(Deprecated)*

:::caution
`hbm login` has been removed. Use `hbm auth` to manage OAuth credentials, `hbm model` to select a provider, or `hbm setup` for full interactive setup.
:::
...
```bash
hbm auth                                              # Interactive wizard
hbm auth list                                         # Show all pools
hbm auth list openrouter                              # Show specific provider
hbm auth add openrouter --api-key sk-or-v1-xxx        # Add API key
hbm auth add openrouter --type oauth                  # Browser login (OpenRouter PKCE) mints a key for you
hbm auth add anthropic --type oauth                   # Add OAuth credential
hbm auth add openai-codex --type oauth --priority 0   # Add an account and try it first
hbm auth remove openrouter 2                          # Remove by index
hbm auth priority openrouter backup-key 0             # Move a credential to the front of fill_first order
hbm auth reset openrouter                             # Clear cooldowns
hbm auth reset openrouter 2                           # Clear the cooldown on one credential
hbm auth refresh openai-codex work                    # Refresh one OAuth credential and clear its cooldown
hbm auth status anthropic                             # Show auth status for a provider
hbm auth logout anthropic                             # Log out and clear stored auth state
hbm auth spotify                                      # Authenticate HBM AGENT with Spotify via PKCE
```
...
```bash
hbm cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
`$HBM_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
```bash
hbm kanban [--board <slug>] <action> [options]
```

Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.hbm/kanban.db` for back-compat; additional boards live at `~/.hbm/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.

**Global flags (apply to every action below):**

| Flag | Purpose |
|------|---------|
| `--board <slug>` | Operate on a specific board. Defaults to the current board (set via `hbm kanban boards switch`, the `HBM_KANBAN_BOARD` env var, or `default`). |

**This is the human / scripting surface.** Agent workers spawned by the dispatcher drive the board through a dedicated `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_create`, `kanban_link`, `kanban_comment`, `kanban_heartbeat`; orchestrator profiles also get `kanban_list` and `kanban_unblock`) instead of shelling to `hbm kanban`. Workers have `HBM_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.hbm/kanban/current`). |
...
hbm kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
hbm kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
hbm kanban boards switch atm10-server
hbm kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
hbm kanban boards rm atm10-server
hbm kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `HBM_KANBAN_BOARD` env var → `~/.hbm/kanban/current` file → `default`.
...
```bash
hbm egress install                  # download the pinned iron-proxy binary
hbm egress install --force          # re-download even if already installed

hbm egress setup                    # interactive wizard: CA, mappings, config
hbm egress setup --tunnel-port N    # override the tunnel listener port (default 9090)
hbm egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
hbm egress setup --no-bitwarden     # explicitly switch back to env-based credentials
hbm egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

hbm egress start                    # spawn the managed proxy daemon
hbm egress stop                     # SIGTERM (then SIGKILL after 5s grace)
hbm egress restart                  # stop (if running) then start — needed for secret changes
hbm egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

hbm egress status                   # binary + config + pid + listening + mappings
hbm egress status --show-tokens     # print proxy tokens in full (default: redacted)

hbm egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
hbm egress config                   # print the path to proxy.yaml for inspection
```
...
hbm egress setup && hbm egress start
hbm config set terminal.backend docker   # if not already

# Switching credential source after the fact
hbm egress setup --from-bitwarden       # env → bitwarden
hbm egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
hbm egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.hbm/config.yaml proxy.extra_allowed_hosts: [api.example.com]
hbm egress setup
hbm egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
hbm egress status                     # current state in one view
cat ~/.hbm/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.hbm/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.hbm/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
```bash
hbm project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
hbm webhook <subscribe|list|remove|test>
```
...
### `hbm webhook subscribe`

```bash
hbm webhook subscribe <name> [options]
```
...
| `--script` | Filter/transform script under `~/.hbm/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |
...
Subscriptions persist to `~/.hbm/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart. Re-running `subscribe` for an existing name keeps its secret and profile binding unless you pass `--secret` / `--route-profile`.
...
Outputs a compact, plain-text summary of your entire HBM AGENT setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | HBM AGENT version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, HBM_HOME path |
...
:::tip
`hbm dump` is specifically designed for sharing. For interactive diagnostics, use `hbm doctor`. For a visual overview, use `hbm status`.
:::

## `hbm debug`

```bash
hbm debug share [options]
```
...
The report includes system info (OS, Python version, HBM AGENT version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
```bash
hbm debug share              # Upload debug report, print URL
hbm debug share --lines 500  # Include more log lines
hbm debug share --expire 30  # Keep paste for 30 days
hbm debug share --nous       # Upload a private diagnostics bundle for Nous support
hbm debug share --local      # Print report to terminal (no upload)
```
...
Create a zip archive of your HBM AGENT configuration, skills, sessions, and data. The backup excludes the hbm-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/hbm-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |
| `-k`, `--keep <N>` | After a full backup, delete older `hbm-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |

The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when HBM AGENT is running (WAL-mode safe).

**What's excluded from the zip:**

- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite's WAL / shared-memory / journal sidecars. The `*.db` file already got a consistent snapshot via `sqlite3.backup()`; shipping the live sidecars alongside it would let a restore see a half-committed state.
- `checkpoints/` — per-session trajectory caches. Hash-keyed and regenerated per session; wouldn't port cleanly to another install anyway.
- `models/`, `runtimes/`, `node/` at the root of `~/.hbm` (and of each `profiles/<name>/`) — regenerable runtime downloads, often tens of GB. Deeper directories with the same names (a skill's `models/`) are kept.
...
- The `hbm-agent` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
hbm backup                           # Full backup to ~/hbm-backup-*.zip
hbm backup -o /tmp/hbm.zip        # Full backup to specific path
hbm backup --quick                   # Quick state-only snapshot
hbm backup --quick --label "pre-upgrade"  # Quick snapshot with label
```

## `hbm checkpoints`

```bash
hbm checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.hbm/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `hbm checkpoints` is equivalent. |
...
```bash
hbm checkpoints                                  # status overview
hbm checkpoints prune --retention-days 3         # aggressive cleanup
hbm checkpoints prune --max-size-mb 200          # tighten size cap once
hbm checkpoints clear-legacy -f                  # drop v1 archive dirs
hbm checkpoints clear -f                         # wipe everything
```
...
```bash
hbm import <zipfile> [options]
```

Restore a previously created HBM AGENT backup into your HBM AGENT home directory. All files in the archive overwrite existing files in your HBM AGENT home; `--force` only skips the confirmation prompt that fires when the target already has an HBM AGENT installation.
...
```
  ⚠ Session data replaced by older backup contents:
    ...
    Recover from a newer backup or snapshot: hbm snapshot list
```

### Examples
```bash
hbm import ~/hbm-backup-20260423.zip           # Prompts before overwriting existing config
hbm import ~/hbm-backup-20260423.zip --force   # Overwrite without prompting
```

## `hbm logs`

```bash
hbm logs [log_name] [options]
```

View, tail, and filter HBM AGENT log files. All logs are stored in `~/.hbm/logs/` (or `<profile>/logs/` for non-default profiles).
...
hbm logs gateway -n 100

# Show only warnings and errors from the last hour
hbm logs --level WARNING --since 1h

# Filter by a specific session
hbm logs --session abc123

# Follow errors.log, starting from 30 minutes ago
hbm logs errors --since 30m -f
...
hbm logs --level WARNING --since 2h --session tg-12345
```
...
HBM AGENT uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hbm logs list` subcommand shows all log files including rotated ones.


## `hbm prompt-size`

```bash
hbm prompt-size [--platform <name>] [--json]
```
...
- **Prompt tiers** — stable / context / volatile, matching how HBM AGENT layers
...
hbm prompt-size --platform telegram

# Machine-readable output for scripts
hbm prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`hbm tools`) or
uninstall skills you don't need (`hbm skills`). Context files (AGENTS.md,
...
`hbm config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `hbm config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `hbm config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
hbm pairing <list|approve|revoke|clear-pending>
```
...
```bash
hbm skills browse
hbm skills browse --source official
hbm skills search react --source skills-sh
hbm skills search https://mintlify.com/docs --source well-known
hbm skills inspect official/security/1password
hbm skills inspect skills-sh/vercel-labs/json-render/json-render-react
hbm skills install official/migration/openclaw-migration
hbm skills install skills-sh/anthropics/skills/pdf --force
hbm skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
hbm skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
hbm skills check
hbm skills update
hbm skills config
hbm skills reset google-workspace
hbm skills reset google-workspace --restore --yes
hbm skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
hbm skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
hbm skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point HBM AGENT at a site exposing `/.well-known/skills/index.json`.
...
```bash
hbm bundles <subcommand>
```

Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.hbm/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.hbm/skill-bundles/` and report added/removed bundles |

Examples:

```bash
hbm bundles create backend-dev \
...
hbm bundles list
hbm bundles show backend-dev
hbm bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.hbm/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.hbm/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `hbm update`. Use `hbm curator run --dry-run` to preview before that happens.
...
```bash
hbm moa list
hbm moa configure [name]
hbm moa delete <name>
```

`hbm moa configure` reuses HBM AGENT's provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.

## `hbm fallback`

```bash
hbm fallback <subcommand>
```
...
| `add` | Pick a provider + model (same picker as `hbm model`) and append to the chain |
...
```bash
hbm hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.hbm/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.hbm/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `hbm <provider>` command for provider-specific management (e.g. `hbm honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `hbm --help` to see what's currently wired in.
:::
...
Starts HBM AGENT as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run HBM AGENT as an MCP server.
...
| `install <name>` | Install a catalog entry (e.g. `hbm mcp install n8n`). |
| `serve [-v\|--verbose]` | Run HBM AGENT as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with HBM AGENT](../guides/use-mcp-with-hbm.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hbm-as-an-mcp-server).

## `hbm plugins`

```bash
hbm plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `hbm plugins` with no subcommand opens a composite interactive screen with two sections:
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the HBM AGENT plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance in a `.hbm-catalog.json` sidecar. Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist. |
| `search [term] [--json]` | Search the HBM AGENT plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
See [Plugins](../user-guide/features/plugins.md) and [Build an HBM AGENT Plugin](../developer-guide/plugins/index.md).
...
`hbm computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`hbm tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, HBM AGENT checks its version and runtime
...
installer. HBM AGENT never replaces a custom binary selected through
`HBM_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`hbm computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended HBM AGENT integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects HBM AGENT and links Cua's
skill pack into the HBM AGENT skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode HBM AGENT passes Cua's canonical
...
`hbm update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next HBM AGENT update.

## `hbm pets`

```bash
hbm pets <list|install|select|show|off|scale|remove|doctor>
```

[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and HBM AGENT shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
```bash
hbm insights [--days N] [--source platform]
```
...
```bash
hbm claw migrate [options]
```

Migrate your OpenClaw setup to HBM AGENT. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hbm`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing HBM AGENT files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hbm/` (by default a single restore-point archive is written to `~/.hbm/backups/pre-migration-*.zip` before apply; restorable with `hbm import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into HBM AGENT equivalents or **archived** for manual review.
...
hbm claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
hbm claw migrate --preset full

# Full migration including API keys
hbm claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
hbm claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
hbm claw migrate --source /home/user/old-openclaw
```

## `hbm import-agent`

```bash
hbm import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into HBM AGENT. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.hbm/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Start the HBM AGENT **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](/user-guide/desktop) and remote clients connect to. It is the same server `hbm dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `hbm serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `hbm dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.

**Port conflicts:** if the requested port (default `9119`) is already held by another process (e.g. a second `hbm serve` or the gateway), the command prints a machine-readable sentinel line `BACKEND_PORT_IN_USE port=<port>` to stdout, a human hint naming the likely holder, and exits with code **75** (`EX_TEMPFAIL`) instead of a generic error — so scripts and the desktop app can tell "port occupied" apart from "backend broken". Pass `--port 0` to bind a free ephemeral port (the successful boot announces the chosen port via `HBM_BACKEND_READY port=<port>`).

## `hbm dashboard`

```bash
hbm dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`hbm serve`](#hbm-serve) above.) Requires `cd ~/.hbm/hbm-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.hbm/hbm-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
| `--stop` | — | Stop running `hbm dashboard` processes and exit. |
| `--status` | — | List running `hbm dashboard` processes and exit. |

### `hbm dashboard register`

Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `HBM_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hbm/.env`, and prints how to engage the login gate. Requires being logged in (`hbm setup`).

| Option | Description |
|--------|-------------|
| `--name` | Human-readable label for the dashboard (default: auto-generated). |
| `--redirect-uri` | Public HTTPS OAuth redirect URI (e.g. `https://hbm.example.com/auth/callback`). Omit for localhost-only use. |
| `--portal-url` | Override the Nous Portal base URL for registration (default: the portal you logged into). Also settable via `HBM_DASHBOARD_PORTAL_URL`. |
...
hbm dashboard --port 8080 --no-open
...
Manage profiles — multiple isolated HBM AGENT instances, each with its own config, sessions, skills, and home directory.
...
hbm profile create work --clone --sync-imports   # also carry over the import-agent sync manifest
hbm profile use work
hbm profile alias work --name h-work
hbm profile export work -o work-backup.tar.gz
hbm profile import work-backup.tar.gz --name restored
hbm profile install github.com/user/my-distro --alias
hbm profile update work
hbm -p work chat -q "Hello from work profile"
```
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of HBM AGENT commands, subcommands, and profile names.
...
```bash
hbm update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```

Pulls the latest `hbm-agent` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`hbm update` pulls the configured update branch (default: `main`). If your checkout is on another branch, HBM AGENT may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `HBM_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `hbm config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, HBM AGENT attempts to restart all running gateway profiles automatically so they pick up the new code. Use `hbm gateway restart` when you want to restart a gateway without applying an update.
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.hbm/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
- **npm lockfile churn.** Before stashing or switching branches, HBM AGENT makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `hbm update`.
- **Pairing data snapshot.** Even when `--backup` is off, `hbm update` takes a lightweight snapshot of `~/.hbm/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hbm backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `hbm.service` warning.** If HBM AGENT detects a pre-rename `hbm.service` systemd unit (instead of the current `hbm-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `hbm --version` | Print version information. |
| `hbm update` | Pull latest changes and reinstall dependencies. |

| `hbm uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove HBM AGENT, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
