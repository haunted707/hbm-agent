...
HBM AGENT fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `hbm-agent` release.

When the manifest is unreachable (offline, network blocked, hosting failure), HBM AGENT silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.
...
- **`version`** — integer schema version. Future schemas bump this; HBM AGENT refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. HBM AGENT ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
- **`description`** — OpenRouter-only. Drives picker badge text (`"recommended"`, `"free"`, `"default"`, or empty). Nous Portal doesn't use this.
- **`default`** — exactly one entry per provider may carry `"default": true`. That model is the **silent default**: what HBM AGENT lands on when the user never selected a model (GUI onboarding confirm card, `provider` configured with no `model`, empty `model.default`). Read cache-only at runtime (`get_default_model_from_cache`) so hot resolution paths never hit the network; when no cached manifest exists, HBM AGENT falls back to the in-repo `PREFERRED_SILENT_DEFAULT_MODEL` constant, which must match the labeled entry. This lets maintainers rotate the silent default without shipping a release. It is deliberately a capable low-cost model, never the priciest flagship.
...
| When | What happens |
|---|---|
| `/model` or `hbm model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.hbm/cache/model_catalog.json`.
...
The exclusion is matched case-insensitively against every key a provider can surface under — the HBM AGENT id and models.dev id (built-in mapped providers), the overlay pid and resolved HBM AGENT slug (overlay providers), and the canonical slug (canonical providers) — so a single entry like `copilot` hides the provider regardless of which section emits it. It is honored by every `/model` picker surface: the gateway interactive/text pickers, the TUI picker, and the interactive `hbm model` CLI picker. An empty list (or omitting the key) has no effect.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in hbm_cli/models.py).
