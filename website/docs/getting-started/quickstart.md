---
sidebar_position: 1
title: "HBM AGENT Quickstart"
description: "Your first conversation with HBM AGENT — from install to chatting in under 5 minutes"
---

# HBM AGENT Quickstart

This guide gets you from zero to a working HBM AGENT setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [HBM AGENT Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="HBM AGENT Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up HBM AGENT for a team, bot, or always-on workflow
...
|---|---|---|
| I just want HBM AGENT working on my machine | `hbm setup` | Run a real chat and verify it responds |
| I already know my provider | `hbm model` | Save the config, then start chatting |
| I want a bot or always-on setup | `hbm gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `hbm model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `hbm model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if HBM AGENT cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install HBM AGENT

> **Note:** HBM AGENT is distributed from its own repository — it is **not**
> installed by the upstream Hermes Agent one-liners (`hermes-agent.nousresearch.com/install.sh` / `.ps1`).
> Clone this repository and run the local installer (`./scripts/install.sh`).

### With the HBM AGENT Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the HBM AGENT Desktop installer](https://hermes-agent.nousresearch.com/) from our website and run it.

### Without HBM AGENT Desktop:
For a command-line only install without HBM AGENT Desktop, run:
...
The single most important setup step. Use `hbm model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `hbm setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `hbm update`. Re-enable anything later with `hbm tools`, seed skills with `hbm skills opt-in --sync`, or tune settings with `hbm setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `hbm model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `hbm model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `hbm model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hbm_cli/models.py` may change between releases) | `hbm model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `hbm model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `hbm model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `hbm model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `hbm model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `hbm model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

HBM AGENT separates secrets from normal config:

- **Secrets and tokens** → `~/.hbm/.env`
- **Non-secret settings** → `~/.hbm/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
hbm config set model anthropic/claude-opus-4.6
...
```bash
hbm            # classic CLI
hbm --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
HBM AGENT ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `hbm` vs `hbm --tui`.
:::
...
- HBM AGENT replies without error
...
```bash
hbm --continue    # Resume the most recent session
hbm -c            # Short form
```
...
```bash
hbm gateway setup    # Interactive platform configuration
```
...
- `hbm tools` — tune tool access per platform
- `hbm skills` — browse and install reusable workflows
...
```bash
hbm config set terminal.backend docker    # Docker isolation
hbm config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `hbm egress setup && hbm egress start`; `hbm setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the HBM AGENT install directory (the curl installer placed it at
# ~/.hbm/hbm-agent on Linux/macOS or %LOCALAPPDATA%\hbm\hbm-agent on Windows):
cd ~/.hbm/hbm-agent
...
Skills are on-demand instruction documents that teach HBM AGENT how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

HBM AGENT ships with a catalog of bundled skills already installed in `~/.hbm/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
hbm skills browse                      # list everything available
hbm skills search kubernetes           # find skills by keyword
hbm skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `hbm skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let HBM AGENT ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.hbm/hbm-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| HBM AGENT opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `hbm model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `hbm gateway setup` and check `hbm gateway status` |
| `hbm --continue` can't find old session | Switched profiles or session never saved | Check `hbm sessions list` and confirm you're in the right profile |
...
| `hbm doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
|---------|-------------|
| `hbm` | Start chatting |
| `hbm model` | Choose your LLM provider and model |
| `hbm tools` | Configure which tools are enabled per platform |
| `hbm setup` | Full setup wizard (configures everything at once) |
| `hbm doctor` | Diagnose issues |
| `hbm update` | Update to latest version |
| `hbm gateway` | Start the messaging gateway |
| `hbm --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-hbm-to-another-machine)** — `hbm backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
