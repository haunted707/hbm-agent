...
description: "Run HBM AGENT directly on an Android phone with Termux"
---

# HBM AGENT on Android with Termux
...
HBM AGENT can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop HBM AGENT from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed HBM AGENT installs are marked with install method `apt`. HBM AGENT therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade hbm-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. HBM AGENT runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
- links `hbm` into `$PREFIX/bin` so it stays on your Termux PATH
...
HBM AGENT requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `hbm` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/hbm" "$PREFIX/bin/hbm"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `hbm` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.hbm/.env`.
...
### `hbm doctor` says ripgrep or Node is missing
