...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  HBM AGENT  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to HBM AGENT instantly when PRs are opened or updated.
:::
...
- **HBM AGENT installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  hbm gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.hbm/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure HBM AGENT can access GitHub. Start a chat:
...
Still in the chat, ask HBM AGENT to review a real PR:
...
A skill gives HBM AGENT consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.hbm/skills/code-review
```

Create `~/.hbm/skills/code-review/SKILL.md`:
...
Verify it loaded — start `hbm` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach HBM AGENT your team's standards:
...
2. Teach HBM AGENT your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
hbm gateway status    # Is the gateway running?
hbm cron list         # Is the job enabled?
```
