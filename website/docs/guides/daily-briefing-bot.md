...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `hbm setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **HBM AGENT spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo hbm gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.hbm/cron/output/` and you can read them anytime.
:::
...
HBM AGENT will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by HBM AGENT
```
...
Before creating cron jobs, ensure HBM AGENT has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell HBM AGENT what you want:
...
HBM AGENT will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell HBM AGENT to delegate each topic to a sub-agent:
...
HBM AGENT will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
