...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `hbm send`."
---

# Pipe Script Output to Messaging Platforms

`hbm send` is a small, scriptable CLI that pushes a message to any
messaging platform HBM AGENT is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | hbm send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `hbm
...
hbm send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
hbm send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
hbm send --list

# Filter by platform
hbm send --list telegram
```
...
Any platform HBM AGENT ships adapters for works as a target:
...
`hbm send` resolves the message body in this order:

1. **Positional argument** — `hbm send --to telegram "hi"`
2. **`--file PATH`** — `hbm send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | hbm send --to telegram`

When stdin is a TTY (no pipe), HBM AGENT does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  hbm send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `hbm send` reuses your HBM AGENT config, the same script works on
any host where HBM AGENT is installed — no need to export bot tokens into
...
`hbm send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  hbm send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | hbm send \
...
```bash
./train.py --epochs 200 && \
  hbm send --to telegram "training done" || \
  hbm send --to telegram "training failed (exit $?)"
```
...
hbm send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(hbm send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `hbm send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `hbm send` calls
the platform's REST endpoint directly using credentials from
`~/.hbm/.env` and `~/.hbm/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `hbm gateway start` and retry.
...
The listing is built from `~/.hbm/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`hbm gateway
...
| Approach | Multi-platform | Reuses HBM AGENT creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `hbm send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`hbm send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `hbm send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `hbm send` shares with cron delivery.
