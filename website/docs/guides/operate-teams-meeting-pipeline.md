...
```bash
hbm teams-pipeline token-health
hbm teams-pipeline token-health --force-refresh
```
...
```bash
hbm teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
hbm teams-pipeline maintain-subscriptions
hbm teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: HBM AGENT cron (recommended if you already run the HBM AGENT gateway)

HBM AGENT ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hbm/scripts/`. First create the script:

```bash
mkdir -p ~/.hbm/scripts
cat > ~/.hbm/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec hbm teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.hbm/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
hbm cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/hbm-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=HBM AGENT Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=hbm
EnvironmentFile=/etc/hbm/env
ExecStart=/usr/local/bin/hbm teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/hbm-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run HBM AGENT Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now hbm-teams-pipeline-maintain.timer
systemctl list-timers hbm-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/hbm teams-pipeline maintain-subscriptions >> /var/log/hbm/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.hbm/.env` at the top of a wrapper script that crontab calls.
...
```bash
hbm teams-pipeline subscriptions   # should show expirationDateTime advanced
hbm teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
hbm teams-pipeline list
hbm teams-pipeline list --status failed
hbm teams-pipeline show <job-id>
```

### Replay a stored job

```bash
hbm teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
hbm teams-pipeline fetch --meeting-id <meeting-id>
hbm teams-pipeline fetch --join-web-url "<join-url>"
hbm teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `hbm teams-pipeline maintain-subscriptions --dry-run`
- inspect `hbm teams-pipeline list --status failed`
...
- run `hbm teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `hbm teams-pipeline run`
...
- [ ] `hbm teams-pipeline validate` returns an OK snapshot
- [ ] `hbm teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (HBM AGENT cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
