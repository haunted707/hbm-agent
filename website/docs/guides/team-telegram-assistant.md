...
This tutorial walks you through setting up a Telegram bot powered by HBM AGENT that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **HBM AGENT installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.hbm/.env`

:::tip
A $5/month VPS is plenty for running the gateway. HBM AGENT itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team HBM AGENT Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_hbm_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by HBM AGENT. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting HBM AGENT Gateway...
...
# View live logs
journalctl --user -u hbm-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `hbm gateway install` to update the plist.
:::
...
```bash
hbm gateway stop && hbm gateway start
```
...
hbm pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.hbm/.env`:
...
Control how much detail the bot shows when using tools. In `~/.hbm/config.yaml`:
...
Customize how the bot communicates by editing `~/.hbm/SOUL.md`:

For a full guide, see [Use SOUL.md with HBM AGENT](/guides/use-soul-with-hbm).
...
```bash
# From the CLI
hbm cron list          # View all scheduled jobs
hbm cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hbm-gateway` (Linux) or `~/.hbm/logs/gateway.log` (macOS) |
| Cron job output | `~/.hbm/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.hbm/cron/jobs.json` |
| Pairing data | `~/.hbm/pairing/` |
| Session history | `~/.hbm/sessions/` |
