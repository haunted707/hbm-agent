...
Copy-paste blueprints for common automation patterns. Each blueprint uses HBM AGENT's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`hbm webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `hbm webhook subscribe`) |
...
```bash
hbm webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (hbm_cli/commands.py, hbm_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (hbm_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
hbm webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
hbm webhook subscribe alert-triage \
...
```python title="~/.hbm/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "HBM AGENT-Monitor/1.0"})
...
hbm cron create "every 30m" \
  ...
  --script ~/.hbm/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
hbm cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to HBM AGENT development." \
...
```bash
hbm webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
hbm cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the hbm-agent codebase.
