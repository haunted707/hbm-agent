---
sidebar_position: 6
title: "Use MCP with HBM AGENT"
description: "A practical guide to connecting MCP servers to HBM AGENT, filtering their tools, and using them safely in real workflows"
---

# Use MCP with HBM AGENT

This guide shows how to actually use MCP with HBM AGENT in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native HBM AGENT tool
- you want HBM AGENT to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect HBM AGENT to internal APIs, databases, or company systems without modifying HBM AGENT core

Do not use MCP when:
- a built-in HBM AGENT tool already solves the job well
...
- HBM AGENT remains the agent
- MCP servers contribute tools
- HBM AGENT discovers those tools at startup or reload time
...
If you installed HBM AGENT with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- HBM AGENT banner/status should show MCP integration when configured
- ask HBM AGENT what tools it has available
...
## WSL2: bridge HBM AGENT in WSL to Windows Chrome

This is the practical setup when:

- HBM AGENT runs inside WSL2
...
In this setup, HBM AGENT does **not** connect to Chrome directly. Instead:

- HBM AGENT runs in WSL
- HBM AGENT starts a local stdio MCP server
...
```text
HBM AGENT (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- HBM AGENT stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on HBM AGENT core browser transport
...
```bash
hbm mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
hbm mcp test chrome-devtools-win
```

Then start a fresh HBM AGENT session or run:
...
Once loaded, HBM AGENT can use the MCP-prefixed browser tools directly. For example:
...
If HBM AGENT runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start HBM AGENT from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start HBM AGENT from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in HBM AGENT:
...
2. HBM AGENT-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So HBM AGENT will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want HBM AGENT to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want HBM AGENT to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. HBM AGENT remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
hbm mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
hbm mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `hbm mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want HBM AGENT to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize HBM AGENT to merge, publish, deploy, or spawn runtimes.
...
Now HBM AGENT can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing HBM AGENT core.
...
Because HBM AGENT now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
