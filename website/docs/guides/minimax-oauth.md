...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in HBM AGENT — no API key required"
---

# MiniMax OAuth

HBM AGENT supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and HBM AGENT automatically refreshes your session.
...
# → HBM AGENT opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.hbm/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.hbm/.env
```
...
```bash
hbm auth add minimax-oauth --no-browser
```

HBM AGENT will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

HBM AGENT implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. HBM AGENT generates a PKCE verifier / challenge pair and a random state value.
...
4. HBM AGENT polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.hbm/auth.json` under the `minimax-oauth` key.
...
```bash
hbm config set model.default MiniMax-M2.7
hbm config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.hbm/config.yaml` will contain entries similar to:
...
```bash
hbm --provider minimax-oauth    # canonical
hbm --provider minimax-portal   # alias
hbm --provider minimax-global   # alias
hbm --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `hbm setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
hbm --provider minimax-oauth
```
...
HBM AGENT refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, HBM AGENT marks the session as requiring re-login.
...
**Fix:** run `hbm auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, HBM AGENT raises a timeout error.

**Fix:** re-run `hbm auth add minimax-oauth` (or `hbm model`). The flow starts fresh.

### State mismatch (possible CSRF)

HBM AGENT detected that the `state` value returned by the authorization server does not match what it sent.
...
If `hbm` cannot open a browser window, use `--no-browser`:

```bash
hbm auth add minimax-oauth --no-browser
```

HBM AGENT prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `hbm model` and select MiniMax (OAuth), or run `hbm auth add minimax-oauth`.
...
- [hbm doctor](../reference/cli-commands.md)
