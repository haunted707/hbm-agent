...
description: "Diagnose and fix common HBM AGENT cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`hbm gateway` for foreground, or `hbm gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `hbm cron tick`.
...
```bash
date
hbm cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.hbm/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.hbm/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.hbm/.env` |
...
| `local` | Write access to `~/.hbm/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `hbm cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (HBM AGENT Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `hbm skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `hbm skills list`.
...
2. `~/.hbm/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `hbm cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the HBM AGENT config directory). Verify:
```bash
ls ~/.hbm/scripts/your-script.py   # Must exist
hbm cron edit <job_id> --script ~/.hbm/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `hbm skills install <skill-name>`.
...
Jobs are stored in `~/.hbm/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.hbm/cron/jobs.json
chmod 600 ~/.hbm/cron/jobs.json   # Your user should own it
```
...
```bash
hbm cron list                    # Show all jobs, states, next_run times
hbm cron run <job_id>            # Schedule for next tick (for testing)
hbm cron edit <job_id>           # Fix configuration issues
hbm logs                         # View recent HBM AGENT logs
hbm skills list                  # Verify installed skills
```
...
1. Run the job with `hbm cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.hbm/logs/agent.log` for scheduler messages and `~/.hbm/logs/errors.log` for warnings
