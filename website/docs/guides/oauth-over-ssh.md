...
description: "How to complete browser-based OAuth (Spotify, MCP servers) when HBM AGENT runs on a remote machine, container, or behind a jump box"
---

# OAuth over SSH / Remote Hosts

Some HBM AGENT providers — **Spotify** and **remote MCP servers** (Linear, Sentry, Atlassian, Asana, Figma, …) — use a *loopback redirect* OAuth flow. The auth server redirects your browser to `http://127.0.0.1:<port>/callback` so a tiny HTTP listener started by HBM AGENT can grab the authorization code.

This works perfectly when HBM AGENT and your browser are on the same machine. It breaks the moment they aren't: your laptop's browser tries to reach `127.0.0.1` on **your laptop**, but the listener is bound to `127.0.0.1` on **the remote server**.
...
**xAI Grok OAuth (`xai-oauth`) uses OAuth device code**, not a loopback callback — open the printed verification URL in any browser and HBM AGENT polls until approval. No SSH tunnel is required. See [xAI Grok OAuth](./xai-grok-oauth.md).
...
hbm auth spotify --no-browser
# → HBM AGENT prints an authorize URL. Open it in a browser on your laptop.
# → Your browser redirects to 127.0.0.1:43827/callback, the tunnel forwards
#   the request to the remote listener, login completes.
```

HBM AGENT prints the exact port it bound to on the `Waiting for callback on ...` line — copy it from there. Spotify defaults to port `43827`.

## Which Providers Need This

| Provider | Loopback port | Tunnel needed? |
|----------|---------------|----------------|
| Spotify | `43827` (default) | Yes, when HBM AGENT is remote |
| MCP servers (`auth: oauth`) | auto-picked per server | Yes, when HBM AGENT is remote (or paste redirect URL) |
...
| `openrouter` (`hbm auth add openrouter --type oauth`) | OS-assigned, local only | No — over SSH HBM AGENT switches to OpenRouter's headless flow and asks you to paste the code shown in the browser |
...
Remote MCP servers (Linear, Sentry, Atlassian, Asana, Figma, etc.) use the same loopback redirect flow. HBM AGENT auto-picks a free port per server and prints the authorize URL when the OAuth flow kicks off — either at startup (when a new server appears in `mcp_servers:`) or when you run `hbm mcp login <server>`.

You have two ways to complete it from a remote host:

**Option 1 — paste the redirect URL back (no setup, works anywhere).** On an interactive terminal, HBM AGENT prompts you to paste the redirect URL alongside running the local listener. After approving in your browser, the redirect to `http://127.0.0.1:<port>/callback` will show a connection error — that's expected. Copy the **full URL from the browser's address bar** and paste it at the HBM AGENT prompt:
...
**Option 2 — SSH port forward (same as Spotify).** HBM AGENT prints the exact port it bound to in the SSH-session hint. Open a separate terminal on your laptop:
...
**Pitfall — the 30s config-reload race.** If you edit `~/.hbm/config.yaml` to add an OAuth MCP server from inside a running HBM AGENT session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough time to complete an interactive OAuth flow, and the reload will give up. Use `hbm mcp login <server>` from a fresh terminal instead — it has no such cap and waits the full 5 min for you to paste back.
...
HBM AGENT detects the SSH session, skips the browser auto-open, and prints an authorize URL plus a `Waiting for callback on http://127.0.0.1:<port>/callback` line.

### 3. Open the URL in your local browser

Copy the authorize URL from the remote terminal and paste it into the browser on your laptop. Approve the consent screen. The auth server redirects to `http://127.0.0.1:<port>/callback`. Your browser hits the tunnel, the request is forwarded to the remote listener, and HBM AGENT prints `Login successful!`.

You can tear down the tunnel (Ctrl+C in the first terminal) once you see the success line.

## Step-by-step: through a jump box

If you reach HBM AGENT through a bastion / jump host, use SSH's built-in `-J` (ProxyJump):
...
The tunnel is a property of the underlying SSH connection. If you're running HBM AGENT inside `tmux` over a mosh session, the mosh roaming doesn't carry the `-L` forwarding. Open a *separate* plain SSH session **only** for the `-L` tunnel — that's the connection that has to stay alive during the auth flow. Your interactive mosh/tmux session can keep running HBM AGENT normally.
...
Something on your laptop is already using that port. Either the previous tunnel didn't shut down cleanly, or a local HBM AGENT is also listening on it. Find and kill the offender:
...
The redirect never made it back to the remote listener. Check the tunnel is still alive (`ssh -N` doesn't show output, so look at the terminal you started it from), confirm you used the port from the latest `Waiting for callback on ...` line (HBM AGENT may auto-bump if the preferred port is busy), restart the tunnel if needed, and re-run the auth command.

### Tokens land in the wrong `~/.hbm`

The tokens are written under the Linux user that ran `hbm auth add ...`. If your gateway / systemd service runs as a different user (e.g. `root` or a dedicated `hbm` user), authenticate as **that** user so the tokens land in their `~/.hbm/auth.json`. `sudo -u hbm -i` or equivalent.
