---
sidebar_position: 3
title: "HBM AGENT Desktop"
description: "The native HBM AGENT desktop app — a polished experience for chatting with HBM AGENT, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# HBM AGENT Desktop

The HBM AGENT desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same HBM AGENT core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hbm` in a terminal, everything you set up there is already here, and anything you do here shows up there.
...
HBM AGENT has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`hbm`) and **[TUI](./tui.md)** (`hbm --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`hbm dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [HBM AGENT Desktop product page](https://hermes-agent.nousresearch.com/desktop), or follow the [installation instructions for HBM AGENT Desktop](../getting-started/installation.md).

If you already have HBM AGENT installed, simply run
...
- **The same conversation history** as every other HBM AGENT surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against an HBM AGENT instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting HBM AGENT Desktop to a remote backend](./features/web-dashboard.md#connecting-hbm-desktop-to-a-remote-backend).
...
HBM AGENT Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional HBM AGENT sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `hbm desktop --cwd <path>` (or the `HBM_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask HBM AGENT to open PR**. You can also create and switch branches from here.
...
When `hbm gui` runs inside WSL2 with `/dev/dxg` present and Mesa's `d3d12_dri.so` installed, the launcher sets `GALLIUM_DRIVER=d3d12` for Electron so rendering uses the Windows GPU instead of the llvmpipe software rasterizer; an explicit `GALLIUM_DRIVER`, `MESA_LOADER_DRIVER_OVERRIDE`, `LIBGL_ALWAYS_SOFTWARE`, or `LIBGL_DRIVERS_PATH` in your environment is left untouched (for example `GALLIUM_DRIVER=llvmpipe hbm gui` keeps software rendering).
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `hbm model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader HBM AGENT management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md). The Skills tab lists your installed skills with enable/disable toggles, and below them the full built-in optional-skills catalog that ships with HBM AGENT — each entry has a one-click **Install** button that flips the row into the installed list once it finishes.
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#hbm-cron).
- **Profiles** — switch between [HBM AGENT profiles](./profiles.md) (isolated config/skills/sessions).
- **Messaging** — set up gateway channels. Telegram has a **Quick setup** card: click **Create with QR**, scan the code (or open the link) in Telegram, and HBM AGENT creates the bot, detects your user ID for the allowlist, saves the credentials, and restarts the gateway for you. Any credential save, clear, or enable toggle keeps a **Restart now** banner on the page until the gateway has actually restarted; if a restart fails, the banner stays so you can retry or restart manually.
...
roster where every [HBM AGENT profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by HBM AGENT cron). The roster lives in the left
...
each other's Bot Chats directly (`hbm -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update HBM AGENT, and more.
...
The desktop app and the HBM AGENT backend it talks to update on separate clocks — the app package on your machine, the backend wherever it runs. When more than one update target exists (a remote gateway, or several registered gateways), the update affordances (**Update now** on the About panel, the ⌘K **Update HBM AGENT** row, and the update-ready toast) update **everything**: the connected backend first, then every other eligible registered gateway (HBM AGENT Cloud entries are platform-managed and skipped), and the desktop app itself last, since applying the client update relaunches the app. Single-machine installs keep the one-button experience.
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the HBM AGENT, your config, and your chats stay. (Same as `hbm uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `hbm uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `hbm uninstall --full`.)
...
You can do the same from the terminal — `hbm uninstall --gui` for the GUI alone, or `hbm uninstall` / `hbm uninstall --full` for the agent too.

:::note
Running `hbm uninstall --gui` from a **source checkout** (a `hbm desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `hbm desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `hbm desktop`

To launch via the CLI, simply run `hbm desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.

On Linux, launches refresh `$XDG_DATA_HOME/applications/hbm.desktop` (by default `~/.local/share/applications/hbm.desktop`) so HBM AGENT appears in the application menu. To preserve a hand-edited entry, disable refreshes:

```bash
hbm config set desktop.manage_launcher_entry false
```

A missing entry is still created; the flag only stops `hbm desktop` from rewriting an entry that already exists.

When you start HBM AGENT from the application grid or menu (the launcher sets `DESKTOP_STARTUP_ID`), the entry is written only after the window is on screen. If the app exits before a window appears, nothing is written that time; the next terminal launch, updater relaunch, or grid launch that shows a window installs it. Some GNOME Shell versions lose track of an app whose `.desktop` file changes while it is still starting (they keep it in that state until the startup notification completes or times out, not until the process exits), and that can crash the whole session later; waiting for the window avoids it. Terminal launches and the updater's relaunch still write the entry immediately.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `hbm update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `HBM_DESKTOP_CWD`)           |
| `--hbm-root PATH` | Override the HBM AGENT source root the app uses (sets `HBM_DESKTOP_HBM_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `hbm` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the HBM AGENT runtime into `HBM_HOME` (`~/.hbm`, or `%LOCALAPPDATA%\hbm` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `HBM_DESKTOP_HBM_ROOT`, then a completed managed install, then a probed `hbm` on `PATH` (unless `--ignore-existing` / `HBM_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `HBM_DESKTOP_HBM` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `hbm serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `hbm --tui`. The desktop app is **self-contained**: it runs its own `hbm serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at an HBM AGENT backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `hbm serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **HBM AGENT Cloud** — sign in once to HBM AGENT Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every HBM AGENT gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), HBM AGENT Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another HBM AGENT gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many HBM AGENT Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the HBM AGENT Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `hbm update` to every eligible gateway at once — HBM AGENT Cloud entries are skipped (the platform updates them), and each instance reports its own result.


:::info The remote backend is a running `hbm serve` process
"Remote backend" means a **`hbm serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `hbm serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.hbm/.env` (the secrets file, mode 0600):
...
hbm serve --host 0.0.0.0 --port 9119
```

Keep that `hbm serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `hbm serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `HBM_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.hbm/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/hbm` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `HBM_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `HBM_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `HBM_DASHBOARD_BASIC_AUTH_USERNAME` / `HBM_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.hbm/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `HBM_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting HBM AGENT Desktop to a remote backend](./features/web-dashboard.md#connecting-hbm-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & HBM AGENT Desktop](../reference/environment-variables.md#web-dashboard--hbm-desktop).
...
`$HBM_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
HBM AGENT: **one row per plugin**, with two switch columns.
...
- **Desktop column** — the half loaded into this app. It is app-level: the
  ...
  `~/.hbm/desktop-plugins/`; the desktop half of a unified agent+desktop
...
- **Open logs** — opens `HBM_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `hbm debug share --nous`; secrets are always redacted, the bundle is
...
Boot logs land in `HBM_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.hbm/hbm-agent/.hbm-bootstrap-complete"
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.hbm/hbm-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```

**Other native downloads (e.g. the `get-windows` prebuilt on Windows) that need a mirror:** put the npm keys in `$HBM_HOME/npmrc` (`%LOCALAPPDATA%\hbm\npmrc` on Windows, `~/.hbm/npmrc` elsewhere) — for example `node_get_windows_binary_host_mirror=https://<mirror>/sindresorhus/get-windows/releases/download/`. Every `npm ci`/`npm run` the updater spawns (desktop, web and TUI builds) points `NPM_CONFIG_USERCONFIG` at that file when it exists, so the config survives `hbm update`; the repo-root `.npmrc` is git-tracked and gets autostashed on every update, and `~/.npmrc` may be missed because the desktop hand-off inherits the GUI's environment. An `NPM_CONFIG_USERCONFIG` you set yourself is never overridden.
...
```bash
HBM_DESKTOP_HBM_ROOT=/path/to/clone npm run dev
HBM_HOME=/tmp/throwaway npm run dev
...
(Desktop, then Downloads, then Documents, ...) as HBM AGENT touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with HBM AGENT's stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **HBM AGENT.app** if you use Desktop.
3. Fully quit and relaunch them once.

`hbm doctor` reports whether the current terminal context already has the
grant, and `hbm setup` shows this tip on macOS when it doesn't.
...
HBM AGENT. Grants are stable from then on.
...
certificate once and tell HBM AGENT to use it. The one-shot command does
...
```bash
hbm desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `HBM AGENT Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `hbm config set desktop.macos_signing_identity "HBM AGENT Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `HBM AGENT Local Signing`). The command is idempotent —
...
- [TUI](./tui.md) — the modern terminal UI used by `hbm --tui` and the dashboard chat tab
