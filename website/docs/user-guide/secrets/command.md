...
secrets:
  command:
    enabled: true
    command: "cat /run/user/1000/hbm-secrets.env"
    # or any vault CLI that dumps KEY=VALUE lines:
    # command: "pass show hbm/env"
    # command: "secret-tool lookup service hbm-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach HBM AGENT's output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (HBM AGENT discards its stderr on purpose) |
