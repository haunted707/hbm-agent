...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, HBM AGENT supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::note
[Nous Portal](/integrations/nous-portal) decides routing centrally per model and does not accept caller-supplied provider preferences; HBM AGENT never sends the `provider` object to Portal, so `provider_routing` is simply ignored there.
:::

## Configuration

Add a `provider_routing` section to your `~/.hbm/config.yaml`:
...
own pins. Edit `config.yaml` directly for these keys: model ids contain dots, which `hbm config set`
...
- **CLI mode** — configured in `~/.hbm/config.yaml`, loaded at startup
