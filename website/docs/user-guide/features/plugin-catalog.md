...
description: "Browse and install reviewed, SHA-pinned HBM AGENT plugins from the curated catalog"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of HBM AGENT plugins you
can install by name with a single command:

```bash
hbm plugins install <name>
```
...
directory of the hbm-agent repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `hbm plugins install` |
...
| `requires_hbm` | Minimum HBM AGENT version, e.g. `>=0.19` (optional) |
...
The catalog name and the plugin's own manifest name can differ; `hbm
...
```bash
hbm plugins install touchdesigner
hbm plugins enable td
```
...
skill, so one install gives HBM AGENT code, dependency, container and IaC scanning
plus the workflow for using it; the catalog name and manifest name match:

```bash
hbm plugins install snyk
hbm plugins enable snyk
```

### Updating a catalog install

`hbm plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `hbm plugins list` shows catalog
...
`~/.hbm/cache/`, so new entries and removals reach installed clients without
updating HBM AGENT. Offline, the copy shipped with your checkout is used. Removals
from the in-tree list and the live list are always both enforced.

### Custom git URLs are different

`hbm plugins install <git-url>` still works for any repository, but it
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with HBM AGENT
- [Build an HBM AGENT Plugin](/developer-guide/plugins) — write your own
