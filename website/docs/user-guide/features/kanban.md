...
description: "Durable SQLite-backed task board for coordinating multiple HBM AGENT profiles"
---
...
HBM AGENT Kanban is a durable task board, shared across all your HBM AGENT profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.hbm/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
The board has two front doors, both backed by the same `~/.hbm/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `hbm kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `hbm kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), reaps workers that outlived their finished run (a worker still alive after its own `kanban_complete`/`kanban_block` is terminated once its run has been closed for two minutes, leaving it time to finish its final turn — matched by PID *and* spawn-time fingerprint, so a recycled PID is never signalled; recorded as a `terminal_worker_reaped` event), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `HBM_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
hbm kanban boards list

# Create a new board.
hbm kanban boards create atm10-server \
    --name "ATM10 Server" \
...
hbm kanban --board atm10-server list
hbm kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
hbm kanban boards switch atm10-server
hbm kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
hbm kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `HBM_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.hbm/kanban/current` — the slug persisted by `hbm kanban
   boards switch`.
4. `default`.
...
`hbm dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<hbm-home>/kanban/attachments/<task_id>/` for the default board, or
  `<hbm-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `HBM_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
hbm kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `hbm kanban show t_abcd`.
...
Override the config flag at runtime via `HBM_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `hbm gateway
...
until one comes up — `hbm kanban create` warns about this at creation
time.

Running `hbm kanban daemon` as a separate process is **deprecated**;
...
Mounting one `kanban.db` in several HBM AGENT homes (containers, fleet hosts) shares
...
hbm kanban create "nightly ops review" \
    --assignee ops \
...
```bash
hbm kanban complete t_abc t_def t_hij --result "batch wrap"
hbm kanban archive  t_abc t_def t_hij
hbm kanban unblock  t_abc t_def
hbm kanban block    t_abc "need input" --ids t_def t_hij
```
...
```bash
hbm -p planner tools enable kanban                      # CLI / TUI / Desktop chats
hbm -p planner tools enable kanban --platform telegram  # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `hbm tools` and the dashboard.
...
**Workers do not shell out to `hbm kanban`.** When the dispatcher spawns a worker it sets `HBM_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `hbm kanban` CLI.
...
kanban_show()                                     # no args — uses HBM_KANBAN_TASK
...
### Why tools instead of shelling to `hbm kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `hbm kanban complete` *inside* the container, where `hbm` isn't installed and `~/.hbm/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.hbm/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `hbm chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HBM_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/hbm_cli/test_kanban_db.py -q"],
...
2. `cd $HBM_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, HBM AGENT injects up to two
...
guard is active only for the dispatcher-spawned worker itself (`HBM_KANBAN_TASK` is
...
and can be disabled with `HBM_KANBAN_STOP_NUDGE=0`.
...
**Worker session names.** A worker's session is titled after its card (`Fix the swap modal`, or `Kanban task <id>` when the board row can't be read) at spawn, so `hbm sessions` and session search show the card, not a model's guess. Workers never make the auxiliary `title_generation` model call that names interactive sessions; a manual `/title` in a worker session still wins.
...
```bash
hbm kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

hbm kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `hbm skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
hbm kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
hbm kanban set-model t_abcd claude-opus-4.6 --provider anthropic
hbm kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.hbm/profiles/<name>/`, and the dispatcher injects the profile-scoped `HBM_HOME` when it spawns `hbm -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.hbm/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.hbm/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.hbm/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `hbm kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
hbm kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `hbm kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `hbm kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `hbm profile create --description "..."`, `hbm profile describe <name> --text "..."`, `hbm profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.hbm/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `hbm profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.hbm/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `hbm dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.hbm/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `hbm -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard HBM AGENT dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
hbm kanban init                                     # create kanban.db + print daemon hint
hbm kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
hbm kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
hbm kanban show <id> [--json]
hbm kanban assign <id> <profile>                    # or 'none' to unassign
hbm kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
hbm kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
hbm kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
hbm kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
hbm kanban diagnostics [--json]                     # board health snapshot (alias: diag)
hbm kanban link <parent_id> <child_id>
hbm kanban unlink <parent_id> <child_id>
hbm kanban claim <id> [--ttl SECONDS]
hbm kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
hbm kanban complete <id>... [--result "..."] [--force]
hbm kanban block <id> "<reason>" [--ids <id>...]
hbm kanban unblock <id>...
hbm kanban archive <id>...

hbm kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
hbm kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
hbm kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

hbm kanban tail <id>                                # follow a single task's event stream
hbm kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
hbm kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
hbm kanban runs <id> [--json]                       # attempt history (one row per run)
hbm kanban assignees [--json]                       # profiles on disk + per-assignee task counts
hbm kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
hbm kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `hbm gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
hbm kanban stats [--json]                           # per-status + per-assignee counts
hbm kanban log <id> [--tail BYTES]                  # worker log from ~/.hbm/kanban/logs/
hbm kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
hbm kanban notify-list [<id>] [--json]
hbm kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
hbm kanban context <id>                             # what a worker sees
hbm kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
hbm kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
hbm kanban create "nightly backup audit" \
...
To see why a ready card is not spawning, run `hbm kanban dispatch --dry-run` — the output lists `Guarded (<reason>): <task id>` per held card (and `respawn_guarded`, `rate_limited`, `skipped_locked`, `memory_pressure` with `--json`). The gateway's and the standalone daemon's "dispatcher stuck" warning also names what the last tick held back, e.g. `Last tick held back: active_pr=1`.
...
```bash
hbm kanban swarm "Design a multi-region failover plan" \
...
Every `hbm kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `hbm chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hbm_cli.kanban.run_slash()` entry point that reuses the `hbm kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `hbm kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.hbm/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Dispatcher workers creating tasks through `kanban_create` or `hbm kanban create`
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`hbm kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `hbm kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
_metadata_: `{"changed_files": ["hbm_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
hbm kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
```
hotspot: hbm_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
hbm kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
hbm kanban notify-list
hbm kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
hbm kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
hbm kanban runs t_abcd
...
**Bulk close caveat.** `hbm kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.

**Live-claim guard on complete.** A `running` task whose worker holds a live claim is only completed by that worker (`kanban_complete` from inside the run) or by an explicit operator override: `hbm kanban complete <id> --force` and the dashboard's "mark done" action. A claim-less `hbm kanban complete <id>` or an orchestrator session's `kanban_complete` is refused with a pointer to `--force` / `hbm kanban reclaim`, so a second session can no longer close a live worker's run underneath it. Completing `ready`, `blocked` or `review` cards without a claim is unchanged.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`hbm kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `hbm kanban heartbeat $TASK` to signal liveness during long operations. |
...
`hbm kanban tail <id>` shows these for a single task. `hbm kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.hbm/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
