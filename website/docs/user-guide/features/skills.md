...
All skills live in **`~/.hbm/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point HBM AGENT at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `hbm update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:
...
```bash
hbm profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
hbm skills opt-out            # stop future seeding — nothing on disk is touched
hbm skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
hbm skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `hbm update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `hbm skills opt-in`) to re-enable.

:::note Safe by default
`hbm skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version HBM AGENT installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
(Plan mode works the same way but is a built-in command now: `/plan [request]` tells HBM AGENT to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hbm/plans/` relative to the active workspace/backend working directory.)

You can also interact with skills through natural conversation:

```bash
hbm chat --toolsets skills -q "What skills do you have?"
hbm chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, HBM AGENT-tool framing, no invented
commands).
...
/home/user/.hbm/cache/chart-q4-2025.png
...
When a missing value is encountered, HBM AGENT asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `hbm setup` or `~/.hbm/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `hbm config migrate` prompts for unconfigured settings, and `hbm config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.hbm/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. HBM AGENT scans the
...
policy above), HBM AGENT can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of HBM AGENT — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell HBM AGENT to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.hbm/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.hbm/skills/` (or `skills.create_dir` when configured — see below). Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the HBM AGENT process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, HBM AGENT ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.hbm/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
By default the agent writes new skills to the profile-local `~/.hbm/skills/`. If you want agent-created skills to land somewhere else — a shared "brain" directory, a git-tracked repo, or a fleet-wide skills volume — set `create_dir` under the `skills` section:
...
Paths support `~` expansion and `${VAR}` substitution; relative paths resolve against your HBM AGENT home. Setting `create_dir` to the local skills dir is the same as leaving it unset.


## Project-Local Skills

Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch HBM AGENT inside a git checkout, it looks for skills in:

```text
<project-root>/.hbm/skills/    # HBM AGENT-native location
...
Skills are procedure documents the agent follows, so HBM AGENT does **not** auto-load them from arbitrary cloned repos. The first time you run HBM AGENT in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `hbm skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
hbm skills trust             # trust the current repo
hbm skills trust ~/myproject # or explicitly
hbm skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.hbm/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.hbm/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.hbm/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.hbm/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
Bundles live in **`~/.hbm/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `hbm bundles list`.
...
hbm bundles create research

# Overwrite an existing bundle
hbm bundles create backend-dev --skill ... --force

# Delete a bundle
hbm bundles delete backend-dev

# Re-scan ~/.hbm/skill-bundles/ and report changes
hbm bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.hbm/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.hbm/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.hbm/pending/skills/` and are
...
```bash
hbm skills browse                              # Browse all hub skills (official first)
hbm skills browse --source official            # Browse only official optional skills
hbm skills search kubernetes                   # Search all sources
hbm skills search react --source skills-sh     # Search the skills.sh directory
hbm skills search https://mintlify.com/docs --source well-known
hbm skills inspect openai/skills/k8s           # Preview before installing
hbm skills install openai/skills/k8s           # Install with security scan
hbm skills install official/security/1password
hbm skills install skills-sh/vercel-labs/json-render/json-render-react --force
hbm skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
hbm skills list --source hub                   # List hub-installed skills
hbm skills check                               # Check installed hub skills for upstream updates
hbm skills update                              # Reinstall hub skills with upstream changes when needed
hbm skills audit                               # Re-scan all hub skills for security
hbm skills uninstall k8s                       # Remove a hub skill
hbm skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
hbm skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
hbm skills publish skills/my-skill --to github --repo owner/repo
hbm skills snapshot export setup.json          # Export skill config
hbm skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with HBM AGENT. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `hbm skills search <query> --source skills-sh`. HBM AGENT resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
HBM AGENT currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the HBM AGENT repository itself and install with built-in trust.
...
This is Vercel's public skills directory. HBM AGENT can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
hbm skills search https://mintlify.com/docs --source well-known
hbm skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
hbm skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

HBM AGENT can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
hbm skills install openai/skills/k8s
hbm skills tap add myorg/skills-repo
```
...
categorization, no HBM AGENT-side changes required.
...
- HBM AGENT source id: `clawhub`

#### 6. LobeHub (`lobehub`)

HBM AGENT can search and convert agent entries from LobeHub's public catalog into installable HBM AGENT skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- HBM AGENT source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

HBM AGENT integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with HBM AGENT's browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- HBM AGENT source id: `browse-sh`
- Trust level: `community`

```bash
hbm skills search airbnb --source browse-sh
hbm skills inspect browse-sh/airbnb.com/search-listings-ddgioa
hbm skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). HBM AGENT also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- HBM AGENT source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. HBM AGENT does not enumerate or copy unrelated files from the host.

```bash
hbm skills install https://sharethis.chat/SKILL.md
hbm skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
hbm skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `hbm skills update` re-fetches from the same URL automatically when you want to refresh.
...
`hbm skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with HBM AGENT | Always trusted |
...
```bash
hbm skills check          # Report which installed hub skills changed upstream
hbm skills update         # Reinstall only the skills with updates available
hbm skills update react   # Update one specific installed hub skill
hbm skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Checks skip network requests for missing or non-directory installs (`orphaned`) and unsafe or unresolvable recorded paths (`invalid_install`). Missing-directory entries can be removed with `hbm skills uninstall <name>`; invalid paths require inspecting and repairing the active profile's `skills/.hub/lock.json` before retrying. No entries are removed automatically.
...
Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `hbm skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other HBM AGENT users add with `hbm skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.hbm.tags`, `version`, `author`, `platforms`, `metadata.hbm.config`).
...
HBM AGENT discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any HBM AGENT user can subscribe and install:

```bash
hbm skills tap add my-org/hbm-skills
hbm skills search deploy
hbm skills install my-org/hbm-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.hbm/skills/.hub/taps.json`:
...
The `hbm skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `hbm skills tap list` shows the effective path per tap.
...
```bash
hbm skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.hbm/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`hbm skills reset`)

HBM AGENT ships with a set of bundled skills in `skills/` inside the repo. On install and on every `hbm update`, a sync pass copies those into `~/.hbm/skills/` and records a manifest at `~/.hbm/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, HBM AGENT recomputes the hash of your local copy and compares it to the origin hash:
...
Generated runtime caches inside a skill (`__pycache__/`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, and a `.pyc` sitting next to its `.py`) are not part of the hash, so running a skill's helper script never marks it user-modified or hides it from `hbm skills list-modified` / `diff`.

The protection is good, but it has one sharp edge. If you edit a bundled skill and then later want to abandon your changes and go back to the bundled version by just copy-pasting from `~/.hbm/hbm-agent/skills/`, the manifest still holds the *old* origin hash from whenever the last successful sync ran. Your fresh copy-paste contents (current bundled hash) won't match that stale origin hash, so sync keeps flagging it as user-modified.

`hbm skills reset` is the escape hatch:
...
hbm skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HBM_HOME`, so `hbm -p coder skills reset <name>` only affects that profile.
:::
