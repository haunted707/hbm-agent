...
description: "Customize HBM AGENT's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

HBM AGENT's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `HBM_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who HBM AGENT is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

HBM AGENT now seeds a default `SOUL.md` automatically in:

```text
~/.hbm/SOUL.md
```

More precisely, it uses the current instance's `HBM_HOME`, so if you run HBM AGENT with a custom home directory, it will use:
...
- HBM AGENT creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- HBM AGENT loads `SOUL.md` only from `HBM_HOME`
- HBM AGENT does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, HBM AGENT falls back to a built-in default identity
...
If HBM AGENT loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HBM_HOME`, the personality belongs to the HBM AGENT instance itself.

That also makes it easier to teach users:
- "Edit `~/.hbm/SOUL.md` to change HBM AGENT's default personality."
...
- how HBM AGENT should handle uncertainty, disagreement, or ambiguity
...
## What HBM AGENT injects into the prompt
...
HBM AGENT ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain HBM AGENT, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives HBM AGENT its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.hbm/config.yaml` under `agent.personalities` (a top-level `personalities:` block works too; `agent.personalities` wins if the same name appears in both).
...
1. Keep a thoughtful global `SOUL.md` in `~/.hbm/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-hbm)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how HBM AGENT speaks
- `display.skin` and `/skin` affect how HBM AGENT looks in the terminal
