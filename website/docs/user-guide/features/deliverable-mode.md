...
When HBM AGENT runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.hbm/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.hbm/config.yaml` (switchable per session
via `/personality`).
...
If you use HBM AGENT's kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.hbm/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. HBM AGENT's deliverable
