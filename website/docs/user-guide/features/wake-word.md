...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey HBM AGENT's wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey HBM AGENT")

The wake word turns HBM AGENT into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, HBM AGENT listens in the background for a spoken
trigger phrase. Say it, and HBM AGENT starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** HBM AGENT backend (for example a
...
HBM AGENT supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey hbm"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey hbm"** — a model for it ships with HBM AGENT, so
...
```bash
cd ~/.hbm/hbm-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `hbm` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.hbm/config.yaml`,
...
wake_word:
  ...
  phrase: "hey hbm"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_hbm         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey hbm" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey hbm" utterances are missed.
...
default) to let HBM AGENT choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which HBM AGENT installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three HBM AGENT surfaces, and `surface` picks which one
...
| `cli` | Only the classic `hbm` CLI. |
| `tui` | Only `hbm --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when HBM AGENT surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. HBM AGENT does not silently fail over to another open surface.
...
"Hey HBM AGENT" works out of the box — the bundled openWakeWord model
(`model: hey_hbm`) is the default. To wake on something else, the easiest
...
command (`hbm -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.hbm/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("hbm" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey HBM AGENT" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey hbm"
  porcupine:
    keyword: ~/.hbm/wakewords/hey_hbm.ppn
```

Set your access key in `~/.hbm/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `hbm tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `hbm-agent[wake]`).
...
but the phrase never fires. HBM AGENT detects this (`/wake status` shows
"mic delivers only silence"; the desktop's folded voice menu carries the same
hint on its trigger).
Fix: System Settings → Privacy & Security → Microphone → enable the HBM AGENT
backend (it may appear as your terminal, `python`, or HBM AGENT), then toggle the
...
```bash
hbm config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
hbm config set wake_word.input_device null
```
