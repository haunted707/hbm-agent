...
Type `@` followed by a reference to inject content directly into your message. HBM AGENT expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- HBM AGENT env: `$HBM_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$HBM_HOME/skills/.hub/`
