...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .hbm.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

HBM AGENT automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the HBM AGENT instance and is loaded from `HBM_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.hbm.md** / **HBM.md** | Project instructions (highest priority) | Walks to git root |
...
| **SOUL.md** | Global personality and tone customization for this HBM AGENT instance | `HBM_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.hbm.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
- `~/.hbm/SOUL.md`
- or `$HBM_HOME/SOUL.md` if you run HBM AGENT with a custom home directory

Important details:

- HBM AGENT seeds a default `SOUL.md` automatically if one does not exist yet
- HBM AGENT loads `SOUL.md` only from `HBM_HOME`
- HBM AGENT does not probe the working directory for `SOUL.md`
...
HBM AGENT is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.hbm.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using HBM AGENT.
...
1. **Scan working directory** — checks for `.hbm.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
