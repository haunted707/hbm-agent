...
description: "Plugins shipped with HBM AGENT that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

HBM AGENT ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.hbm/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build an HBM AGENT Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.hbm/plugins/<name>/`
3. **Project** — `./.hbm/plugins/<name>/` (requires `HBM_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `hbm_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `hbm memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `hbm plugins list` and the interactive `hbm plugins` UI), but none load until you explicitly enable them:

```bash
hbm plugins enable disk-cleanup
```

Or via `~/.hbm/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer HBM AGENT. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
hbm plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `hbm plugins enable <name>`.
...
| `hbm-achievements` | dashboard tab | Steam-style collectible badges generated from your real HBM AGENT session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `hbm memory` and `hbm plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HBM_HOME` or `/tmp/hbm-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under HBM_HOME | always | Never |
...
**State** — everything lives at `$HBM_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `HBM_HOME` or `/tmp/hbm-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `hbm plugins enable disk-cleanup` (or check the box in `hbm plugins`).

**Disabling again:** `hbm plugins disable disk-cleanup`.
...
**Enabling:** `hbm plugins enable security-guidance` (or check the box in `hbm plugins`).

**Disabling again:** `hbm plugins disable security-guidance`.
...
Traces HBM AGENT turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of HBM AGENT's canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `hbm logs`.
...
```bash
hbm tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart HBM AGENT and the next turn ships a trace.
...
Then put the credentials in `~/.hbm/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "HBM AGENT turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `HBM_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the HBM AGENT session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `hbm chat` session lives under one Langfuse session.

**Verify:**

```bash
hbm plugins list                 # observability/langfuse should show "enabled"
hbm chat -q "hello"              # check the Langfuse UI for a "HBM AGENT turn" trace
```
...
| `HBM_LANGFUSE_RELEASE` | — | Release/version tag |
| `HBM_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `HBM_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `HBM_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

HBM AGENT-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — HBM AGENT-prefixed wins when both are set.
...
**Disabling:** `hbm plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled HBM AGENT plugin. Do not run `hbm plugins enable observability/nemo_relay`; HBM AGENT core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `HBM_NEMO_RELAY_PLUGINS_TOML` to that file before starting HBM AGENT. The policy is process-wide for every profile hosted by that HBM AGENT process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `HBM_NEMO_RELAY_ATOF_*` and `HBM_NEMO_RELAY_ATIF_*` settings no longer activate exporters — a `.env` that still carries them (and no `HBM_NEMO_RELAY_PLUGINS_TOML`) exports **nothing**, and the gateway logs one warning saying so. `hbm doctor` reports these stale settings when no replacement `plugins.toml` is selected.

**Automatic migration.** `hbm update` (and `hbm migrate relay`, or `hbm migrate relay --all-profiles` for every profile home) converts the legacy variables into `<hbm home>/relay-plugins.toml`, sets `HBM_NEMO_RELAY_PLUGINS_TOML` in that profile's `.env`, and comments the legacy lines out (nothing is deleted). Under a multiplexed gateway every profile home gets its own file. The generated file is validated through Relay before it is written; this is the shape it produces (note the `type = "file"` sink discriminator — a sink without it is rejected):
...
type = "file"
output_directory = "/home/you/.hbm/telemetry/nemo-relay/atof"
filename = "hbm-atof.jsonl"
mode = "append"

[components.config.atif]
enabled = true
agent_name = "HBM AGENT"
model_name = "unknown"
output_directory = "/home/you/.hbm/telemetry/nemo-relay/atif"
...
Then add `HBM_NEMO_RELAY_PLUGINS_TOML=/home/you/.hbm/relay-plugins.toml` to `.env` and restart the gateway.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `hbm.session.segment` plus `hbm.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.hbm/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
hbm plugins enable google_meet
hbm meet setup   # preflight: playwright, chromium, auth file
hbm meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `hbm plugins disable google_meet`. Any saved transcripts stay in `~/.hbm/workspace/meetings/` until you remove them.

### hbm-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real HBM AGENT session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with HBM AGENT feature changes.

**How it works:**

- Scans your entire `~/.hbm/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$HBM_HOME/plugins/hbm-achievements/state.json`
...
| Secret | Hidden until HBM AGENT detects the first related signal in your history |

**API** — routes mount under `/api/plugins/hbm-achievements/`:
...
**State files** — live under `$HBM_HOME/plugins/hbm-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across HBM AGENT updates. |
...
**Enabling:** Nothing to enable — `hbm-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `hbm dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/hbm-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.hbm/plugins/hbm-achievements/` that ships no dashboard. The plugin's state files under `$HBM_HOME/plugins/hbm-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other HBM AGENT plugin — see [Build an HBM AGENT Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.hbm/plugins/<name>/`
- Manifest source is reported as `bundled` in `hbm plugins list`
