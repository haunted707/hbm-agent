...
The `execute_code` tool lets the agent write Python scripts that call HBM AGENT tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with HBM AGENT over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from hbm_tools import ...`
2. HBM AGENT generates a `hbm_tools.py` stub module with RPC functions
3. HBM AGENT opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to HBM AGENT
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from hbm_tools import web_search, web_extract
...
```python
from hbm_tools import search_files, read_file
import json
...
```python
from hbm_tools import search_files, read_file, patch
...
```python
from hbm_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.hbm/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to HBM AGENT's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (HBM AGENT's own python) |
...
| **Stdout** | 50 KB | Shown head-and-tail inline; the full output is saved to `~/.hbm/cache/exec/` and the path is included in the result |
...
**Remote backends** (Docker, SSH, Modal) run a remote session kernel with the same contract. If the kernel cannot be spawned on the backend, HBM AGENT falls back to running each call as a standalone script and says so in the result.

**Large output.** Stdout over 50 KB is shown head-and-tail inline, and the full text is saved under `~/.hbm/cache/exec/` with the path included in the result, so the agent can page through it with `read_file` instead of re-running the script.
...
### `HBM_*` variables in the child

The child process receives only a small, fixed set of operational `HBM_*`
...
(plus `HBM_RPC_DIR` / `HBM_RPC_SOCKET` / `TZ` / `HOME`, which HBM AGENT
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `HBM_`
through to the child. That broad prefix was removed for security hardening: it
could leak `HBM_*`-named configuration that doesn't match a secret substring
(for example `HBM_BASE_URL`, `HBM_KANBAN_DB`, or a `HBM_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `HBM_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (HBM AGENT-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `HBM_*`
variables, HBM AGENT emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`hbm logs --level
DEBUG`, or check `~/.hbm/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted HBM_* var(s)` if a script behaves
as though a `HBM_*` variable is missing.

HBM AGENT always writes the script and the auto-generated `hbm_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call HBM AGENT tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.

## Platform Support

Code execution is available on **Linux, macOS, and Windows**. On Linux and macOS the RPC channel uses a Unix domain socket; on Windows, where `AF_UNIX` is unreliable, HBM AGENT automatically falls back to a loopback TCP socket for the sandbox RPC transport. Remote terminal backends (Docker/SSH/Modal/etc.) use a file-based RPC transport instead and additionally require Python 3 inside the backend.
