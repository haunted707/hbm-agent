...
HBM AGENT can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended HBM AGENT integration. It
...
**Fresh installs already have the driver.** The HBM AGENT installer
...
- **`hbm tools`** → pick `🖱️  Computer Use` — installs the driver
...
on macOS/Linux, `install.ps1` on Windows. Use `hbm computer-use
status` to verify the install.

Already have cua-driver? HBM AGENT reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `hbm update`, and the first
`computer_use` call of a session, HBM AGENT checks the local version and
...
selected with `HBM_CUA_DRIVER_CMD` stays
under your control, so HBM AGENT reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. HBM AGENT autodetection is a
planned cua-driver follow-up, so currently point HBM AGENT at that directory or
...
interface. The built-in toolset provides HBM AGENT actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `hbm computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the HBM AGENT host identity. |
...
```
hbm -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.hbm/config.yaml`.

## Permission modes and logged-in browser profiles

HBM AGENT maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| HBM AGENT session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal HBM AGENT approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit HBM AGENT risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.hbm/cua-manifest.yaml
```
...
for the format). HBM AGENT launches a private runtime with
...
identity instead of resetting with every HBM AGENT build), and HBM AGENT verifies
...
resetting or closing the HBM AGENT session, cancellation cleanup, or process exit
closes that transport session. HBM AGENT also stops private runtimes that it
launched for bounded or unrestricted access. One HBM AGENT
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the HBM AGENT host identity.
...
## `hbm computer-use doctor` — your first triage stop

`hbm computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ hbm computer-use doctor
...
HBM AGENT run declares a public cua-driver **session name** (something like
`hbm-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. HBM AGENT disables it automatically where it is a known failure
...
HBM AGENT keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the HBM AGENT-side `computer_use` workflow and action vocabulary. For
...
The command links the pack into `~/.hbm/skills/cua-driver` (HBM AGENT is one
...
These are **platform deep dives, not duplicates of the HBM AGENT skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **HBM AGENT
...
image capture also saves a bounded, shareable copy under HBM AGENT's image cache
...
HBM AGENT applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.hbm/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. HBM AGENT applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  HBM AGENT) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the HBM AGENT itself at High integrity (launch from an
  elevated terminal); otherwise target non-elevated windows.
- **Windows: `hbm computer-use doctor` fails with "Access is denied"
  while the tool works.** A cua-driver installed under
  `C:\Program Files\WindowsApps` cannot be executed by the HBM AGENT venv
  ...
  `HBM_CUA_DRIVER_CMD` to a copy outside `WindowsApps`.
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. HBM AGENT warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive HBM AGENT from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. HBM AGENT passes the
...
```
HBM_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
HBM_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **HBM AGENT disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) HBM AGENT sets
...
When it's on, `hbm computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point HBM AGENT at a binary you built from source instead
of the published release. HBM AGENT resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`HBM_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for HBM AGENT testing (see notes).
...
### Option B — point HBM AGENT straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`HBM_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
HBM_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
HBM_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm HBM AGENT is using your build

- `hbm computer-use status` prints the resolved binary path and
  version.
- `hbm computer-use doctor` confirms the binary is reachable and
...
- **HBM AGENT spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  HBM AGENT YOLO, HBM AGENT instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no HBM AGENT
  change needed either way.
- **Local builds skip the version check.** HBM AGENT warns when the
...
**First action when anything's off: run `hbm computer-use doctor`.**
...
Run `hbm computer-use install` to fetch the cua-driver binary, or
run `hbm tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `hbm computer-use doctor` will
...
- **HBM AGENT-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  HBM AGENT `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. HBM AGENT autodetection is a
  planned follow-up; currently point HBM AGENT at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (HBM AGENT among them)
