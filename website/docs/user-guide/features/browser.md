...
HBM AGENT includes a full browser automation toolset with multiple backend options:
...
- **Lightpanda local engine** via [Lightpanda](https://lightpanda.io) — a headless browser built from scratch in Zig for machines; instant start up, 16x lower memory and 9x faster than Chrome. Works in Browser Use mode (HBM AGENT spawns it, no Chromium or Node needed) and with the built-in tools (automatic Chrome fallback for actions it doesn't support yet)
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `hbm setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `hbm model` or `hbm tools`.
:::
...
When a cloud provider is configured, HBM AGENT auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`hbm setup tools → Browser Automation`
...
When enabled, HBM AGENT copies your default browser's **active** profile — the one
...
`~/.hbm/browser-profile/<browser>/`, then launches your **real browser
...
When you turn the toggle back off, HBM AGENT deletes the snapshot store
(`~/.hbm/browser-profile/`) on the next browser use, so the copied
...
an exclusive (deny-all) lock, so HBM AGENT cannot copy them while the browser is
...
stays locked, HBM AGENT stops the launch and asks you to close the browser and retry.
...
Set `browser.real_profile_autoclose: true` to let HBM AGENT **offer to close the
browser for you** when it's holding the profile. Even with this on, HBM AGENT never
closes it automatically — when the profile is locked it always stops and the
agent asks you first; only on your approval does it run `hbm browser
...
(e.g. a background/tray instance relaunched), HBM AGENT stays blocked and tells you
...
- **Assume nothing can be prompted for.** A cron, webhook, API or
  `hbm chat -q` session has nobody to answer a prompt, so a site that is not
...
be taken at all, so a scheduled tick needs it closed beforehand — HBM AGENT never
...
Then fully restart HBM AGENT so the new config is picked up.

:::warning Nested path matters
HBM AGENT reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — HBM AGENT ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, HBM AGENT silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active HBM AGENT profile, so different HBM AGENT profiles get different browser profiles (profile isolation).

##### What HBM AGENT does not do
- It does not force persistence on the Camofox server. HBM AGENT only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), HBM AGENT cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start HBM AGENT and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted HBM AGENT after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

HBM AGENT derives the stable `userId` from the profile-scoped directory `~/.hbm/browser_auth/camofox/` (or the equivalent under `$HBM_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding HBM AGENT profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure HBM AGENT to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` HBM AGENT uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, HBM AGENT calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- HBM AGENT skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- HBM AGENT does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, HBM AGENT issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, HBM AGENT adopts the most recently created one in that group.
3. Otherwise, HBM AGENT adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, HBM AGENT falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — HBM AGENT does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want HBM AGENT to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, HBM AGENT generates a per-task `session_key` (`task_<id>`) — HBM AGENT will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and HBM AGENT can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while HBM AGENT runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. HBM AGENT automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
Lightpanda is a **local engine** (a browser source, like "Local Browser"), not a cloud provider. Install the binary and put it on your `PATH` (see the [Lightpanda installation guide](https://lightpanda.io/docs/run-locally/installation/one-liner)), then pick **Lightpanda** in `hbm tools` → Browser Automation, or set:
...
- **Browser Use mode (the default).** HBM AGENT launches `lightpanda serve --host 127.0.0.1 --port <free>` itself — one process per `browser_exec` session name (or per task) — and points the Browser Use CLI at it. No Chromium, Playwright or Node.js is needed. The process is reaped after `browser.inactivity_timeout`, on exit, and by the orphan sweep if HBM AGENT crashes. All of these processes share one on-disk HTTP cache at `$HBM_HOME/cache/browser-use/lightpanda/http-cache`, so repeat visits skip re-downloading assets. HBM AGENT passes the cache flag only when the installed Lightpanda supports it (0.3.x+); older binaries simply run without a cache. To clear it, stop your Lightpanda sessions first, then delete that directory. Lightpanda has no graphical renderer, so `capture_screenshot()` is unavailable and the tool description tells the model to work text-first; it also holds one page per session, so the model is told to call `new_tab()` once and `goto_url()` afterwards (tracked upstream in [lightpanda-io/browser#1962](https://github.com/lightpanda-io/browser/issues/1962)).
- **Built-in browser tools** (`/browser use off`). HBM AGENT drives Lightpanda through `agent-browser --engine lightpanda` over CDP, the same way it drives local Chrome, with **automatic Chrome fallback**: Lightpanda handles the actions it supports (navigate, snapshot, click, type, scroll, back, press, eval) and HBM AGENT transparently retries on Chrome for anything it doesn't. Screenshots and `browser_vision` are routed straight to Chrome.

**When the engine is ignored.** `browser.engine` is the lowest-precedence browser setting: a cloud provider (including the Nous subscription browser — and on never-configured setups, any `BROWSERBASE_API_KEY` / `BROWSER_USE_API_KEY` in `~/.hbm/.env` auto-selects one), Camofox, a `browser.cdp_url` / `/browser connect` override, or `browser.use_real_profile` all take precedence. Picking Lightpanda in `hbm tools` writes `cloud_provider: local` for you; `/browser status` and `hbm doctor` report when the engine is configured but shadowed, and by what.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach HBM AGENT browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start HBM AGENT from the terminal (`hbm` or `hbm chat`) and issue `/browser connect` there.
:::
...
brave-browser \
  ...
  --user-data-dir=$HOME/.hbm/chrome-debug \
  --no-first-run \
...
Then launch the HBM AGENT CLI and run `/browser connect`.
...
If HBM AGENT runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects HBM AGENT itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let HBM AGENT talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through HBM AGENT MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, HBM AGENT can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Browser Use mode (default) "lightpanda" makes HBM AGENT spawn `lightpanda serve`;
...
# Extra Chromium launch flags (comma- or newline-separated). HBM AGENT auto-injects
...
# HBM AGENT doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `hbm config set toolsets '["hbm-cli", "browser"]'`.
:::
...
You can also run `hbm config set browser.snapshot_threshold 30000`. The setting applies to both explicit `browser_snapshot` calls and the automatic snapshot returned after navigation, including the Camofox backend (minimum 1000). Restart the current HBM AGENT session after changing it so the browser config cache reloads.
...
```
Type "hbm agent" into the search field @e3
```
...
Screenshots are stored in `~/.hbm/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.hbm/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, HBM AGENT automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages are truncated at `browser.snapshot_threshold` (default 15,000 characters, matching `web_extract`; no LLM summarization); the complete snapshot is saved to `~/.hbm/cache/web/` and the output points at it for `read_file` paging
