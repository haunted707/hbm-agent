...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of HBM AGENT's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple HBM AGENT instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
hbm memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `hbm` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Automatically generated HBM AGENT titles (`derived` or `llm`), including lineage titles for Desktop branches, are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions without a manual mapping.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — HBM AGENT syncs them back at session init.
...
The `hbm honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `hbm memory setup honcho` (or run `hbm memory setup` and pick it from the list); the `hbm honcho` subcommand then appears on the next invocation.

```bash
hbm memory setup honcho    # Configure Honcho directly (works before activation)
hbm honcho status          # Connection status, config, and key settings
hbm honcho setup           # Redirects to `hbm memory setup` (post-activation alias)
hbm honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
hbm honcho peer            # Show or update peer names + dialectic reasoning level
hbm honcho mode            # Show or set recall mode (hybrid/context/tools)
hbm honcho tokens          # Show or set token budget for context and dialectic
hbm honcho identity        # Seed or show the AI peer's Honcho identity
hbm honcho sync            # Sync Honcho config to all existing profiles
hbm honcho peers           # Show peer identities across all profiles
hbm honcho sessions        # List known Honcho session mappings
hbm honcho map             # Map current directory to a Honcho session name
hbm honcho enable          # Enable Honcho for the active profile
hbm honcho disable         # Disable Honcho for the active profile
hbm honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `hbm honcho`

If you previously used the standalone `hbm honcho setup`:
...
No re-login or re-setup needed. Run `hbm memory setup` and select "honcho" — the wizard detects your existing config.
