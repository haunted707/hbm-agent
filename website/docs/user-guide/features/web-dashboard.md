...
# HBM AGENT Web Dashboard

The web dashboard is a browser-based UI for managing your HBM AGENT installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `hbm setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
hbm dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
hbm dashboard --host 0.0.0.0

# Start without opening browser
hbm dashboard --no-open
```
...
child with the selected profile's `HBM_HOME`, so the conversation runs
...
processes (manage them via `hbm -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `hbm-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.hbm/hbm-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.hbm/hbm-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `hbm dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `hbm dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.hbm` is nearly full
...
- Node.js (same requirement as `hbm --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.hbm/hbm-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.hbm/.env
ExecStart=/path/to/venv/bin/python -m hbm_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.hbm/.env` containing:

```bash
HBM_DASHBOARD_BASIC_AUTH_USERNAME=admin
HBM_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
HBM_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `hbm config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated HBM AGENT instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `hbm profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.hbm/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `hbm skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `hbm mcp` reads from.
...
`hbm mcp catalog` / `hbm mcp install` use.
...
Connect HBM AGENT to any messaging platform from the browser — full parity with
`hbm setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.hbm/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page. On a system-scope install (`hbm gateway install --system`) the dashboard runs the restart under `sudo -n`, so the dashboard user needs passwordless sudo; without it the request fails immediately instead of reporting a restart that the CLI then refuses.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and HBM AGENT versions, CPU core count + utilization, memory, disk usage of the HBM AGENT home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The HBM AGENT version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `hbm update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `hbm portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `hbm curator`.
...
This re-reads `~/.hbm/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`HBM_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.hbm`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/hbm/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 24h cache (the check goes through the GitHub API, never `git fetch`) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — HBM AGENT engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
|-------|-----------|----------|
| `hbm dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `hbm dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `hbm dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `hbm dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, HBM AGENT doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `hbm dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
- **CLI — `hbm dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `hbm setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HBM_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hbm/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  hbm dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes HBM_DASHBOARD_OAUTH_CLIENT_ID to ~/.hbm/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `HBM_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `HBM_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `hbm dashboard register` |

Per the HBM AGENT convention (`~/.hbm/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: HBM_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `hbm dashboard register` (Nous Portal) or install a
...
From a logged-in HBM AGENT install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `hbm dashboard register` uses your existing Nous login to provision an OAuth client and writes `HBM_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hbm/.env` for you:

```bash
hbm setup            # if you're not already logged into Nous Portal
hbm dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HBM_DASHBOARD_OAUTH_CLIENT_ID to ~/.hbm/.env
```
...
```bash
hbm dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://hbm.example.com/auth/callback` and set `HBM_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `HBM_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `HBM_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `HBM_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `HBM_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `HBM_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.hbm/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.hbm/.env <<EOF
HBM_DASHBOARD_BASIC_AUTH_USERNAME=admin
HBM_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
HBM_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.hbm/.env
```
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/hbm/   # required
      client_id: hbm-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `HBM_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `HBM_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `HBM_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-hbm.json` — it defines a `hbm` realm, a **public PKCE client** (`hbm-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "hbm-dashboard",
      "name": "HBM AGENT Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-hbm.json:/opt/keycloak/data/import/realm-hbm.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/hbm/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/hbm`). The admin console is at
...
```bash
export HBM_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/hbm"
export HBM_DASHBOARD_OIDC_CLIENT_ID="hbm-dashboard"
export HBM_DASHBOARD_PUBLIC_URL="http://localhost:9119"
hbm dashboard --host 0.0.0.0 --port 9119 --no-open
```

`HBM_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → hbm-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `HBM_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/hbm"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, HBM AGENT fails closed at startup. This prevents the local SPA
...
HBM AGENT always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
hbm dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `HBM_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `HBM_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `hbm_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `hbm_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `hbm_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `hbm_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$HBM_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.hbm/plugins/dashboard-auth-myidp/__init__.py
from hbm_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `HBM_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
HBM_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  hbm dashboard --host 0.0.0.0
...
HBM AGENT Desktop can drive an HBM AGENT backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [HBM AGENT Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`hbm dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
```bash
# 1. Set the dashboard login credentials in ~/.hbm/.env (secrets file, 0600).
cat >> ~/.hbm/.env <<'EOF'
HBM_DASHBOARD_BASIC_AUTH_USERNAME=admin
HBM_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
hbm dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `HBM_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.hbm/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.hbm/.env`, so the credentials are in the environment at boot.
...
### In HBM AGENT Desktop

**Settings → Gateways → Remote gateway:**

- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/hbm` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `HBM_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `HBM_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `HBM_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `HBM_DASHBOARD_BASIC_AUTH_USERNAME` / `HBM_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `HBM_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `hbm_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `hbm update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `hbm dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **HBM AGENT Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **HBM AGENT Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
