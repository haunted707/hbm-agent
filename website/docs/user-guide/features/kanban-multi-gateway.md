...
HBM AGENT supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.hbm/config.yaml`:
...
Or set the env var: `HBM_KANBAN_DISPATCH_IN_GATEWAY=false`
