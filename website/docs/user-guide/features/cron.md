...
Schedule tasks to run automatically with natural language or cron expressions. HBM AGENT exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to HBM AGENT itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `hbm model`.

- **Per-job pin** — set by *you* via the dashboard, `hbm cron create/edit --model … --provider …`, or by editing `~/.hbm/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`hbm config set cron.model <name>`) and switching your chat model with `hbm model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `hbm model`. HBM AGENT **snapshots** the provider and model at creation, and that snapshot is the job's effective pin: if you later switch the global default (`hbm model`, `/model`, `hbm config set model.default …`), the job **keeps running on the model and provider it was created under** and logs one INFO line per run noting the difference. A global model change never stops a scheduled job, and an unattended job never silently inherits a switch to a paid provider/model (#44585). To move a job to the new default, **resnap** it (`hbm cron resnap <job_id>`, or `--all` for every unpinned job) so it adopts the current default while staying unpinned, pin it (`hbm cron edit <job_id> --provider <provider> --model <model>`), or set `cron.model` to move the whole fleet at once. Jobs created before snapshots existed keep following the live global default.
...
`hbm setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `hbm cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. HBM AGENT disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
hbm cron create "every 2h" "Check server status"
hbm cron create "every 1h" "Summarize new feed items" --skill blogwatcher
hbm cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask HBM AGENT normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

HBM AGENT will use the unified `cronjob` tool internally.
...
Or: `hbm config set cron.preflight false`
...
```bash
hbm cron edit <job_id> --provider <provider> --model <model>   # one job
hbm config set cron.model <model>                               # every unpinned job
```

`hbm config set model.default …` and the Desktop model picker list the unpinned jobs that will
...
```bash
hbm cron resnap <job_id>   # one job
hbm cron resnap --all      # every unpinned agent job
```
...
A cron job can load one or more skills before it runs the prompt. Each skill loads exactly as it does from `/skill-name` in a chat session, including the `[Skill config ...]` block with its resolved `metadata.hbm.config` values from `config.yaml`.
...
hbm cron create "every 1d at 09:00" \
...
```bash
hbm cron edit <job_id> --schedule "every 4h"
hbm cron edit <job_id> --prompt "Use the revised task"
hbm cron edit <job_id> --skill blogwatcher --skill maps
hbm cron edit <job_id> --add-skill maps
hbm cron edit <job_id> --remove-skill blogwatcher
hbm cron edit <job_id> --clear-skills
```
...
```bash
hbm cron list
hbm cron pause <job_id_or_name>
hbm cron resume <job_id_or_name>
hbm cron run <job_id_or_name>
hbm cron remove <job_id_or_name>
hbm cron edit <job_id_or_name> [...flags]
hbm cron status
hbm cron tick
```
...
### Pausing everything: `hbm pause`

`hbm pause [--reason ...]` is the global emergency stop (`hbm resume` lifts it). While it is engaged no scheduled cron fire starts, whichever door it arrives through: the built-in ticker skips its dispatch, the managed-cron (hosted scheduler) fire webhook answers `503` with `Retry-After: 60` so the scheduler redelivers the fire after you resume, and the [misfire catch-up](#misfire-catch-up) sweep stays idle instead of force-firing everything that was held back. Runs already in flight are never killed, and nothing is lost: due work catches up on the first tick or sweep after `hbm resume`. Explicit manual runs (`hbm cron run`, the dashboard's Trigger button) are an operator override and still execute while paused.
...
```bash
hbm cron create "every 1h" "Post the digest" --paused --paused-reason "Awaiting review"
hbm cron resume <job_id>
```
...
sudo hbm gateway install --system   # Linux: boot-time system service for servers
...
On each tick HBM AGENT:

1. loads jobs from `~/.hbm/cron/jobs.json`
...
A file lock at `~/.hbm/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
HBM AGENT records each claimed cron attempt in the profile-local
`~/.hbm/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, HBM AGENT marks an abandoned
...
Inspect recent attempts with `hbm cron runs [job-id] --limit 20` (alias:
...
that the retained ledger records as completed, HBM AGENT skips that replay and
...
in a row and suggesting you fix, pause (`hbm cron pause <job>`), or remove
it. Any successful run resets the streak, and `hbm cron list` shows the
...
```bash
hbm cron incidents                 # list incidents (newest activity first)
hbm cron incidents --state alerted # filter: detected | alerted | resolved | closed
hbm cron incidents ack <id>        # acknowledge — stop re-pinging
```
...
### Fleet health check: `hbm cron doctor`

`hbm cron doctor` is a read-only health check over every active job. It
...
- script missing, not a file, or resolving outside `HBM_HOME/scripts`,
...
`hbm cron incidents` (durable failure records) and `hbm cron runs`
...
| `"local"` | Save to local files only (`~/.hbm/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `hbm cron list` shows it in yellow as
`delivery_failed: <reason>`, `hbm cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `hbm profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
and in `hbm cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.hbm/cron/output/`), but no message is sent to the delivery target.
...
Or set the `HBM_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `HBM_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$HBM_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other HBM AGENT-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to HBM AGENT directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

HBM AGENT will write the check script to `~/.hbm/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.hbm/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.hbm/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.hbm/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.hbm/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, HBM AGENT reads Job 1's most recent output from `~/.hbm/cron/output/{job1_id}/*.md`
...
From the CLI: `hbm cron create "every 6h" "Scan for news" --continuity`, and `hbm cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
HBM AGENT home. Its `## Error` section includes the chained traceback, with credential patterns
...
- `hbm cron list` — a red `⚠ Missed scheduled fire:` line under the job
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `hbm gateway restart`).
...
`hbm cron list` as `⚠ late` / `⚠ catch-up after missed fire`.
...
Or run `hbm config set cron.catch_up_missed false`. With this opt-out, a recurring
...
Runtimes that can't receive detached results (one-shot `hbm -z`, `hbm
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `hbm tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `hbm tools` cron-platform config wins; otherwise HBM AGENT falls back to the built-in defaults. If the cron-platform toolset config cannot be read at all (for example a malformed `platform_toolsets` block in `config.yaml`), the run fails with a recorded error instead of quietly running with every tool — check `hbm cron list` / `hbm cron doctor`. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.
...
If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether HBM AGENT should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.hbm/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.hbm/scripts/.feed-changed.last"
...
:::tip
HBM AGENT's own `~/.hbm/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
...
Jobs are stored in `~/.hbm/cron/jobs.json`. Output from job runs is saved to `~/.hbm/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `hbm update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `hbm cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `HBM_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, HBM AGENT resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
