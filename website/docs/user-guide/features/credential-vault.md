...
fills the password through HBM AGENT. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): HBM AGENT generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `hbm vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, HBM AGENT
...
in, HBM AGENT picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). HBM AGENT hands the master password to the manager's
...
Prefer not to use a detected manager? `hbm vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `hbm vault add`), bound to the checkout site,
...
- **CLI**: `hbm vault list`, `hbm vault add`, `hbm vault rm <handle>`,
  `hbm vault sources`.

Items live encrypted under `~/.hbm/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `hbm chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through HBM AGENT: not in
