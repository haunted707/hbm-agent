...
chat completions — use your HBM AGENT-managed provider subscription as their
...
| Use case | "Use HBM AGENT as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. HBM AGENT stores
the refresh token in `~/.hbm/auth.json` — the same place all HBM AGENT
...
Model:      HBM AGENT-4-70B    # or HBM AGENT-4.3-36B, HBM AGENT-4-405B
```
...
interface in `hbm_cli/proxy/adapters/`.
...
If you see `not logged in`, run `hbm portal`. If you see
...
`hbm_cli/proxy/adapters/<provider>.py` and registering it in
