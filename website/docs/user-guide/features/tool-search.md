...
**Tool Search** is HBM AGENT's opt-in progressive-disclosure layer for that
...
:::info Built-in HBM AGENT tools never defer
The tools that make up HBM AGENT's core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_HBM_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, HBM AGENT **unwraps the bridge** and
...
- **No provider-native validation for deferred schemas.** `tool_describe`
  ...
  only the generic `tool_call.arguments` object. HBM AGENT therefore coerces and
  validates the underlying arguments locally before dispatch; the concrete
  tool or MCP server remains responsible for schemas HBM AGENT cannot safely
...
- **No JS sandbox.** HBM AGENT uses the simpler "structured tools" mode
