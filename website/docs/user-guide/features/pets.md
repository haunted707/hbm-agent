...
HBM AGENT can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<HBM_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), HBM AGENT renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `hbm pets list [query] [--limit N]` |
| List installed pets | `hbm pets list --installed` |
| Install a pet | `hbm pets install <slug> [--select] [--force]` |
| Set the active pet | `hbm pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `hbm pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `hbm pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `hbm pets off` |
| Remove an installed pet | `hbm pets remove <slug>` |
| Diagnose setup | `hbm pets doctor` |
...
Beyond installing pre-made pets from the gallery, HBM AGENT can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `hbm pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `hbm tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `HBM_PET_IMAGE_PROVIDER` env var (e.g. `HBM_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while HBM AGENT is
...
Run `hbm pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; HBM AGENT uses its own
  profile-scoped `<HBM_HOME>/pets/` instead — install through `hbm pets`.

## See also

- The [`hbm-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hbm-agent.md)
