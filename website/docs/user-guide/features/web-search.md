...
HBM AGENT includes two model-callable web tools backed by multiple providers:

- **`web_search`** — search the web and return ranked results
- **`web_extract`** — fetch and extract readable content from one or more URLs

Both are configured through a single backend selection. Providers are chosen via `hbm tools` or set directly in `config.yaml`.
...
| **xAI (Grok)** | `XAI_API_KEY` or `hbm auth add xai-oauth` | ✔ | — | Paid (SuperGrok or per-token) |

Brave Search, DDGS, and xAI are **search-only** — pair any of them with Firecrawl/Tavily/Perplexity/Keenable/Exa/Parallel when you also need `web_extract`. DDGS uses the [`ddgs` Python package](https://pypi.org/project/ddgs/) under the hood; if it isn't already installed, run `pip install ddgs` (or let HBM AGENT lazy-install it on first use). xAI runs Grok's server-side `web_search` tool on the Responses API — results are LLM-generated rather than index-backed, so titles, descriptions, and URL choice are all model output (see the [trust-model caveat](#xai-grok) below).
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, web search and extract are available through the **[Tool Gateway](tool-gateway.md)** via managed Firecrawl — no API key needed. New installs can run `hbm setup --portal` to log in and turn on all gateway tools at once; existing installs can flip just web via `hbm tools`.
:::
...
| `web_extract` — same URL, same format, same provider | Full text stored under `~/.hbm/cache/web/` | Shared across CLI, gateway, cron, and subagent processes |
...
### Quick setup via `hbm tools`

Run `hbm tools`, navigate to **Web Search & Extract**, and pick a provider. The wizard prompts for the required URL or API key and writes it to your config.
...
SearXNG is a privacy-respecting, open-source metasearch engine that aggregates results from 70+ search engines. **No API key required** — just point HBM AGENT at a running SearXNG instance.
...
To enable JSON responses for HBM AGENT, add the following override:
...
Then select SearXNG as the search backend in `~/.hbm/config.yaml`:
...
Or set via `hbm tools` → Web Search & Extract → SearXNG.
...
With this config, HBM AGENT uses SearXNG for all search queries and Firecrawl for URL extraction — combining free search with high-quality extraction.

---

### Tavily

AI-optimised search and extract. Select Tavily in `hbm tools` (or set `web.backend: tavily`) to use it **keyless** with no account (rate-limited). Set an API key when you want higher limits.
...
If no backend has **ever** been selected (no `web.backend` / per-capability key written by you or `hbm tools`), HBM AGENT picks the first available one based on which credentials are set:
...
Run `hbm setup` to see which web backend is detected:
...
source ~/.hbm/hbm-agent/.venv/bin/activate
...
```bash
hbm skills install official/research/searxng-search
```
