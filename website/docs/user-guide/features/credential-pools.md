...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, HBM AGENT automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, HBM AGENT auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
hbm auth add openrouter --api-key sk-or-v1-your-second-key

# ...or let a browser login mint one (OpenRouter OAuth PKCE; stored as a plain API key)
hbm auth add openrouter --type oauth

# Add a second Anthropic key
hbm auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
hbm auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  hbm_pkce          oauth   id=ab12cd34 priority=0 hbm_pkce ←
...
`hbm auth remove <provider> <target>` when a label is ambiguous, and `priority=` is
the order the pool tries credentials in under the `fill_first` strategy.

## Interactive Management

Run `hbm auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `hbm auth` | Interactive pool management wizard |
| `hbm auth list` | Show all pools and credentials |
| `hbm auth list <provider>` | Show a specific provider's pool |
| `hbm auth add <provider>` | Add a credential (prompts for type and key) |
| `hbm auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `hbm auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `hbm auth add <provider> --priority 0` | Add a credential and place it first in the `fill_first` order |
| `hbm auth priority <provider> <target> <n>` | Move a credential to priority `n` (0 = tried first); the rest are renumbered |
| `hbm auth remove <provider> <index>` | Remove credential by 1-based index |
| `hbm auth reset <provider>` | Clear all cooldowns/exhaustion status |
| `hbm auth reset <provider> <target>` | Clear the cooldown on one credential by index, id, or label |
| `hbm auth refresh <provider> [target]` | Refresh one OAuth credential's tokens and return it to rotation (proves the grant is alive; the next request re-checks quota) |
...
cooldowns are preserved. Reauthenticate with `hbm auth add nous --type oauth`
...
Configure via `hbm auth` → "Set rotation strategy" or in `config.yaml`:
...
| Strategy | Behavior |
|----------|----------|
| `fill_first` (default) | Use the first healthy key until it's exhausted, then move to the next; order is each credential's `priority` (`hbm auth priority` changes it) |
...
When you set up a custom endpoint via `hbm model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via hbm model:
hbm auth list
# Shows:
...
hbm auth add Together.ai --api-key sk-together-second-key
```
...
HBM AGENT automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| HBM AGENT PKCE OAuth | `~/.hbm/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `hbm auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `hbm auth add`) are never auto-pruned.
...
Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. HBM AGENT can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and HBM AGENT-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`hbm_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`hbm_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.hbm/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to HBM AGENT's credential store, so its token remains persistable.

An `env:` row is re-hydrated from the environment on every load, and the variable name does not have to be one HBM AGENT declares for the provider: numbered siblings (`OPENROUTER_API_KEY_2`, see [Auto-Discovery](#auto-discovery)) appear here automatically, and a hand-written row pointing at any other variable is filled the same way, without the secret ever being written to `auth.json`.
