...
title: "Plugins"
description: "Extend HBM AGENT with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

HBM AGENT has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in HBM AGENT
core tools that live in `tools/` and `toolsets.py`.

**→ [Build an HBM AGENT Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.hbm/plugins/` with a `plugin.yaml` and Python code:

```
~/.hbm/plugins/my-plugin/
...
Start HBM AGENT — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.hbm/plugins/hello-world/plugin.yaml`**
...
**`~/.hbm/plugins/hello-world/__init__.py`**

```python
"""Minimal HBM AGENT plugin — registers a tool and a hook."""
...
Drop both files into `~/.hbm/plugins/hello-world/`, restart HBM AGENT, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but HBM AGENT does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.hbm/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HBM_ENABLE_PROJECT_PLUGINS=true` before starting HBM AGENT.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `hbm <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `hbm plugins install` |
| Distribute via pip | `[project.entry-points."hbm_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with HBM AGENT — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.hbm/plugins/` | Personal plugins |
| Project | `.hbm/plugins/` | Project-specific plugins (requires `HBM_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `hbm_agent.plugins` entry_points | Distributed packages |
| Nix | `services.hbm-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, HBM AGENT also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.hbm/plugins/model-providers/<name>/` override bundled model providers of the same name (last-writer-wins in `register_provider()`), so you can replace a built-in provider profile without any repo edits. Memory providers resolve the other way round: for `~/.hbm/plugins/memory/<name>/` the **bundled** provider wins on a name collision (bundled, then user, then project, then entry points; first seen wins), so a user memory provider needs its own unique name.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `hbm plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.hbm/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
hbm plugins                    # interactive toggle (space to check/uncheck)
hbm plugins enable <name>      # add to allow-list
hbm plugins disable <name>     # remove from allow-list + add to disabled
```

After `hbm plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
hbm plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

HBM AGENT checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `hbm plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`hbm plugins install <source> --force --ref <new-commit>`. The
profile-local install metadata contains no config values, environment values,
secrets, or capability grants.

The same pin is available in HBM AGENT Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `hbm plugins list` prints
...
`hbm plugins install` clones non-interactively (it never prompts for a
username or password), so a private repo needs a credential HBM AGENT can find on
...
before. The same resolution applies to `hbm plugins update`, catalog MCP
installs from git, and profile distributions fetched from a git URL.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of HBM AGENT's built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.hbm/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.hbm/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
HBM AGENT tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. HBM AGENT runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, HBM AGENT never
materializes the prompt on another surface.

HBM AGENT still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of HBM AGENT that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.hbm/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 27 lifecycle events currently accepted by `hbm_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
HBM AGENT has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.hbm/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and HBM AGENT also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build an HBM AGENT Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build an HBM AGENT Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build an HBM AGENT Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `hbm <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `HBM_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. HBM AGENT auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `hbm skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.hbm/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `hbm plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
hbm plugins                               # unified interactive UI
hbm plugins list                          # table: enabled / disabled / not enabled
hbm plugins search <term>                 # search the HBM AGENT plugin catalog
hbm plugins install <name>                # install a catalog entry (repo @ reviewed pinned SHA)
hbm plugins install user/repo             # install from Git, then prompt Enable? [y/N]
hbm plugins install user/repo --enable    # install AND enable (no prompt)
hbm plugins install user/repo --no-enable # install but leave disabled (no prompt)
hbm plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
hbm plugins remove my-plugin              # uninstall
hbm plugins enable my-plugin              # add to allow-list
hbm plugins disable my-plugin             # remove from allow-list + add to disabled
hbm plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

HBM AGENT Desktop registers the `hbm://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
hbm://plugin/install?repo=owner/repo            # main install link
hbm://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
hbm://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens HBM AGENT and shows a **confirmation dialog** — the repo id,
...
Plugins → Install from Git**. Legacy `hbm://plugin-agent/…` and
`hbm://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `hbm-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="hbm://plugin/install?repo=owner/repo&enable=1">Install in HBM AGENT</a>
```

MCP servers have the equivalent link form — see
[Add to HBM AGENT link](/reference/mcp-config-reference#add-to-hbm-link).
...
When a plugin declares capabilities, `hbm plugins install` (and
`hbm plugins enable`) shows the list with one-line risk descriptions and
...
granted, `hbm plugins update` surfaces the additions and asks again. New
...
`hbm plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
hbm plugins capabilities             # all plugins with declared/granted capabilities
hbm plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and HBM AGENT has not reviewed the plugin's code. Only install
...
### Discovering plugins — the HBM AGENT plugin catalog

`hbm plugins search <term>` searches the **HBM AGENT plugin catalog** — the
curated, SHA-pinned catalog maintained in the hbm-agent repository
(`plugin-catalog/`). Matching covers entry names, descriptions, and declared
tools:

```bash
hbm plugins search telegram    # search the catalog
hbm plugins browse             # browse every entry
hbm plugins info <name>        # full details for one entry
```
...
recorded so `hbm plugins update` can re-pin when the catalog moves:

```bash
hbm plugins install <catalog-name>
```
...
A **plugin pack** is a declarative, shareable YAML file (`hbm-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: hbm-telegram-business       # bare plugin-catalog name…
...
```bash
hbm plugins pack show ./hbm-pack.yaml     # dry-run review
hbm plugins pack install ./hbm-pack.yaml  # review → confirm → install
hbm plugins pack export > hbm-pack.yaml   # snapshot the current install
hbm plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `hbm plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `hbm plugins install`.
...
provenance (installed via `hbm plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`hbm skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `hbm plugins install` and `hbm plugins update` runs a static
...
On `hbm plugins update`, a dangerous verdict on the updated tree
...
Running `hbm plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `hbm plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- HBM AGENT reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- HBM AGENT rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless HBM AGENT can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. HBM AGENT drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. HBM AGENT checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
