...
sidebar_position: 1
title: "CLI Interface"
description: "Master the HBM AGENT terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

HBM AGENT's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `hbm setup --portal` — and you're ready to `hbm chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
HBM AGENT also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `hbm --tui` — see the [TUI](tui.md) guide.
:::
...
hbm chat --query-file prompt.txt
hbm chat --query-file - < prompt.txt

# With a specific model
hbm chat --model "anthropic/claude-sonnet-4"

# With a specific provider
hbm chat --provider nous        # Use Nous Portal
hbm chat --provider openrouter  # Force OpenRouter
...
hbm chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
hbm --continue             # Resume the most recent CLI session (-c)
hbm --resume <session_id>  # Resume a specific session by ID (-r)
hbm --resume latest        # Resume the most recent session (same as -c)
hbm --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
hbm chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
hbm -w                         # Interactive mode in worktree
hbm -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`hbm -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
hbm worktree list              # audit: age, size, verdict, reason per tree
hbm worktree list --json       # machine-readable audit (trees, external trees, branches)
hbm worktree prune             # remove safe trees + delete merged branches
hbm worktree prune --dry-run   # show the plan without changing anything
hbm worktree prune --older-than 7   # only reap trees idle for 7+ days
hbm worktree prune --trees-only     # leave local branches alone
hbm worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running hbm session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.hbm/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `hbm -w` for days — no longer accumulate merged scratch trees
...
The `hbm plugins` commands manage native HBM AGENT plugins and portable Agent
...
hbm plugins disable <plugin-name>
hbm plugins update <plugin-name>
hbm plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. HBM AGENT currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the HBM AGENT CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The HBM AGENT CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HBM_YOLO_MODE` is on (either `hbm --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. HBM AGENT refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`hbm -c` or `hbm --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend HBM AGENT to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.hbm/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. HBM AGENT supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for HBM AGENT replies |
...
```bash
hbm -s hbm-agent-dev,github-auth
hbm chat -s github-pr-workflow -s github-auth
```

HBM AGENT loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
`--ignore-rules` (equivalently `HBM_IGNORE_RULES=1`) skips auto-load together with AGENTS.md, SOUL.md, `.cursorrules` and memory injection; explicit `-s` skills still load. The setting is profile-scoped: each profile's `config.yaml` controls its own list.

## Skill Slash Commands

Every installed skill in `~/.hbm/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.hbm/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain HBM AGENT..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, HBM AGENT also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. HBM AGENT recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches HBM AGENT — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while HBM AGENT is working, HBM AGENT prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend HBM AGENT to the background — just like any terminal process. The shell prints a confirmation:

```
HBM AGENT has been suspended. Run `fg` to bring HBM AGENT back.
```
...
Resume this session with:
  hbm --resume 20260225_143052_a1b2c3
...
```bash
hbm --continue                          # Resume the most recent CLI session
hbm -c                                  # Short form
hbm -c "my project"                     # Resume a named session (latest in lineage)
hbm --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
hbm --resume "refactoring auth"         # Resume by title
hbm --resume latest                     # Resume the most recent session (same as -c)
hbm --resume latest --in ./my-project   # Latest session for ./my-project's workspace
hbm -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `hbm sessions rename <id> <title>` from the command line. Use `hbm sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in HBM AGENT's SQLite state database under `~/.hbm/state.db`. The database keeps:
...
HBM AGENT immediately confirms the task and gives you back the prompt:
