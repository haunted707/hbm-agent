...
`~/.hbm/config.yaml`, `~/.hbm/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/hbm`:

```text
/etc/hbm/
├── config.yaml     # managed config layer (wins over ~/.hbm/config.yaml)
└── .env            # managed env layer (wins over ~/.hbm/.env + shell)
```
...
The location can be relocated with the `HBM_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `HBM_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by HBM AGENT.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export HBM_MANAGED_DIR=/opt/org/hbm-policy
```

:::warning
A user who can set `HBM_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `hbm doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/hbm/config.yaml` (managed) | `/etc/hbm/.env` (managed) |
| 2 | `~/.hbm/config.yaml` (user) | `~/.hbm/.env` (user) |
...
```bash
hbm config        # shows a header naming the managed source + the pinned keys
hbm doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, HBM AGENT refuses and names the source:

```bash
$ hbm config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/hbm/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `hbm config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/hbm

# Pin some config values for every user on this machine
sudo tee /etc/hbm/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/hbm/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/hbm
sudo chmod 0644 /etc/hbm/config.yaml /etc/hbm/.env
```

Changes take effect on the next HBM AGENT start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`hbm doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs HBM AGENT as `root`), managed scope is advisory.
