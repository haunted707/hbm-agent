...
If you only run one HBM AGENT, you don't need this page — see
...
[Connecting Desktop to Many HBM AGENT Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more HBM AGENTs that should all
...
(`ai.hbm.gateway-<name>.plist`), a systemd user service
(`hbm-gateway-<name>.service`), a systemd **system** service when installed with
`sudo hbm gateway install --system` (runs as the invoking user via `User=`), a
Windows Scheduled Task, or an s6/Docker service — and the Desktop app spawns its own
per-profile `hbm serve` backend. This guide adds the patterns for managing them
collectively.
...
hbm profile create coder
hbm profile create personal-bot
hbm profile create research
...
```bash
hbm config set gateway.multiplex_profiles true
hbm gateway restart
```

Equivalently, in the default profile's `~/.hbm/config.yaml`:
...
You do **not** run `hbm gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `hbm gateway run`, `start`,
...
loop. `hbm -p coder gateway stop` refuses the same way (exit 78) when coder has no
gateway of its own — there is nothing to stop but the multiplexer, which
`hbm gateway stop` on the default profile takes down for every served profile.
...
The setup flows follow the same rule: `hbm -p coder setup gateway`, `hbm -p coder setup`,
`hbm -p coder gateway setup` and `hbm -p coder import` configure the profile's bots but
...
- **WhatsApp (bridge) and Relay are shared ingress owned by the default profile.**
  ...
  is not being served; `hbm gateway status --profile work` shows
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.hbm/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `hbm webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`hbm webhook ls` shows the
...
A standalone `hbm -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ hbm -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: hbm gateway status
...
`hbm gateway status` and `hbm status` on the default profile list the same
...
home). `hbm status` on the default profile reports the multiplexer and lists
the profiles it serves (`Serves: coder, research`); `hbm -p coder status`,
`hbm -p coder gateway status` and `hbm -p coder cron status` all report
...
`HINDSIGHT_BANK_ID`, `HBM_HONCHO_HOST`) are read from the routed profile's
...
`HBM_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`HBM_HOME`, without the default profile's secrets in their environment, and
...
| Dashboard actions (`hbm -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `HBM_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
...
| The launch (default) profile's own credentials in a `hbm serve` / dashboard process that also serves another profile | Its `.env` + secret sources over the process env **frozen the moment the first other profile is served**; not re-read afterwards | A credential rotated only in the process env (`systemctl set-environment`, a refreshed `op run` wrapper that did not re-exec) is not picked up until the process restarts — put rotating keys in `.env` or a secret source, or restart after rotating |
| Cron `.env` tuning (`HBM_CRON_TIMEOUT`, `HBM_MODEL` fallback, `HBM_CRON_MAX_PARALLEL`, prefill file), worker / Bot Chat child env | The profile's own `.env`; children never inherit the default profile's `.env` settings or bridged `TERMINAL_*` policy | Cron defaults / model refusal, exactly as a standalone `hbm -p <name> gateway run` |
...
archived or deleted instead — `hbm profile delete <name>`, or move the
...
multiplexer started as `hbm -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`hbm profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `hbm -p <name> gateway
...
adapters the same way, and `hbm profile rename` unroutes the old name before
...
`~/.local/bin/hbm-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: hbm-gateways {start|stop|restart|status|list}"
}
...
```bash
hbm-gateways start      # start every configured profile
hbm-gateways stop       # stop every configured profile
hbm-gateways restart    # restart all
hbm-gateways status     # status across all
hbm-gateways list       # delegates to `hbm gateway list`
```

:::tip
The `default` profile is targeted with `hbm gateway <action>` (no `-p`),
not `hbm -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `hbm -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.hbm.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/hbm-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.hbm.gateway.plist` /
`hbm-gateway.service`.
...
```bash
# Default profile
tail -f ~/.hbm/logs/gateway.log
tail -f ~/.hbm/logs/gateway.error.log

# Named profile
tail -f ~/.hbm/profiles/<name>/logs/gateway.log
tail -f ~/.hbm/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.hbm/logs/gateway.log ~/.hbm/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
hbm logs -f                  # follow default profile
hbm -p coder logs -f         # follow one profile
hbm logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
hbm profile list             # profiles + model + gateway state
hbm-gateways status          # full status across every profile
launchctl list | grep hbm    # macOS — PIDs and labels
systemctl --user list-units 'hbm-gateway-*'   # Linux — units
```
...
The default profile uses `~/.hbm/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
hbm config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.hbm/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=hbm --why="gateways running" \
  sleep infinity &
...
`hbm-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
```bash
hbm gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
hbm gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
hbm gateway migrate --standalone            # roll back to per-profile gateways
```

### What `hbm update` does
...
service) and `gateway.multiplex_profiles` is off, `hbm update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `hbm gateway migrate --multiplex --yes`) and prints what it did. This
...
| `HBM_HOME` outside `<default home>/profiles/` | a unit pinning `HBM_HOME=/opt/hbm/profiles/emma` |

In that case `hbm update` prints the boundary it found plus
`hbm gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `hbm gateway migrate --multiplex --dry-run`
...
```bash
hbm config set gateway.auto_multiplex_migration false
```

`hbm update` then leaves per-profile gateways exactly as they are, with no
...
other setting, so a value pinned in the managed scope (`/etc/hbm/config.yaml`)
wins over the profile's own file. It governs the **automatic** path only:
`hbm gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `hbm gateway migrate --multiplex` with
...
`hbm profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `hbm profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.hbm/gateway_migration.json` for rollback.
...
above). `hbm profile create` confirms this when the live multiplexer picked the
profile up; it prints the `hbm gateway restart` reminder only when it could not
reach the multiplexer (for example, a gateway started from an older build).

### Rollback

```bash
hbm gateway migrate --standalone
```
...
window, the next `hbm gateway migrate --multiplex` sees the flag on, the
...
with `hbm config set gateway.multiplex_profiles false && hbm gateway restart`
...
flag, stop the per-profile tasks, `hbm gateway restart`). The dashboard's
System page offers the same migration as a button when the preflight finds an
eligible install.

## Updating the code

`hbm update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `hbm gateway start` after a previous `hbm gateway stop`. The
...
```bash
ps -ef | grep "hbm_cli.*-p <profile>"
cat ~/.hbm/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.hbm.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.hbm.gateway-<profile>.plist

# Linux
systemctl --user restart hbm-gateway-<profile>.service
```

### Health check

```bash
hbm doctor                  # default profile
hbm -p <profile> doctor     # one profile
```
