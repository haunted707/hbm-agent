...
HBM AGENT automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.hbm/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
HBM AGENT stores session history so it can resume conversations, but it does not
...
HBM AGENT explicitly injects for that turn.
...
For example, if a user sends an image and asks HBM AGENT to make a meme from it,
HBM AGENT may inspect that image once with vision and run an image-processing
...
`hbm sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `hbm sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`hbm` or `hbm chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.hbm/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `hbm -c` again in the *same* terminal, HBM AGENT resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
hbm -c "my project"   # → resumes "my project #3"
```
...
hbm --resume 20250305_091523_a1b2c3d4
hbm -r 20250305_091523_a1b2c3d4

# Resume by title
hbm --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
hbm --resume latest

# Or with the chat subcommand
hbm chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `hbm sessions list`.
...
hbm --resume latest --in ./my-project

# Works with the TUI too
hbm --tui --resume latest --in ./my-project
```
...
```bash
hbm --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`hbm sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
hbm sessions list --workspace my-project
hbm sessions list --workspace ~/code/hbm-agent
```

### Conversation Recap on Resume

When you resume a session, HBM AGENT displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming an HBM AGENT session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.hbm/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `hbm -r "<title>"` from the shell) and pick up where the platform left off.
...
HBM AGENT automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `hbm sessions list` or `hbm sessions browse`.
...
```bash
hbm sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), HBM AGENT creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`hbm -c "my project"`), it automatically picks the most recent session in the lineage.
...
HBM AGENT provides a full set of session management commands via `hbm sessions`:
...
# Filter by platform
hbm sessions list --source telegram

# Show more sessions
hbm sessions list --limit 50
```
...
`hbm sessions export` is one surface for every export format, selected with `--format`:
...
hbm sessions export backup.jsonl

# Export sessions from a specific platform
hbm sessions export telegram-history.jsonl --source telegram

# Export a single session
hbm sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Point at a directory (existing, or ending in /) and the file is named for you:
# ~/exports/hbm_session_20250305_091523_a1b2c3d4.jsonl
hbm sessions export ~/exports/ --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
hbm sessions export backup.jsonl --redact
```

Exported files contain one JSON object per line with full session metadata and all messages.

Each record also carries a `timings` block derived from the message timestamps, so a reader of an export attached to a bug report can tell a single long model gap from many small tool round-trips without reconstructing it by hand. It holds only ids, roles, counts and durations — `wall_clock_ms`, `largest_gap_ms`, `role_counts`, `tool_calls_emitted` and per-message `intervals` — never prompt text, tool arguments or results, so it survives `--redact` unchanged. HBM AGENT does not persist a model/tool stopwatch, so `complete` is always `false`; when a session has no timestamped messages, `available` is `false` and `unavailable_reason` says why. The block is rebuilt on every export and ignored (and not counted toward size limits) on import.
...
hbm sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
hbm sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
hbm sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
hbm sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `hbm-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
hbm sessions export --format trace

# One session to a local trace file
hbm sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
hbm sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.hbm/session-exports`).

```bash
# Export one session to Markdown
hbm sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
hbm sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
hbm sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
hbm sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
hbm sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
hbm sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
hbm sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
hbm sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
hbm sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
hbm sessions pin 20250305_091523_a1b2c3d4
hbm sessions pin 20250305 20250306

# Remove the pin
hbm sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
hbm sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
hbm sessions pinned --json > pinned-sessions.json
```
...
hbm sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
hbm sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
hbm sessions prune --newer-than 5h

# Explicit window with absolute timestamps
hbm sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
hbm sessions prune --source telegram
hbm sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
hbm sessions prune --newer-than 5h --title "smoke test"   # title substring
hbm sessions prune --older-than 30 --max-messages 3        # tiny sessions
hbm sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
hbm sessions prune --model gpt-5 --older-than 1w           # by model (substring)
hbm sessions prune --provider openrouter --older-than 60   # by billing provider
hbm sessions prune --branch feature/old-experiment         # by git branch
hbm sessions prune --user 12345678 --chat-type group       # by messaging origin
hbm sessions prune --max-tokens 500 --older-than 7         # by token usage
hbm sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
hbm sessions prune --newer-than 5h --dry-run

# Skip confirmation
hbm sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `hbm sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `hbm sessions prune` keeps the 90-day cutoff. Every
...
`hbm sessions archive` takes the same filters as `prune` but soft-hides
...
hbm sessions archive --newer-than 5h

# Archive by title substring, preview first
hbm sessions archive --title "dry run" --dry-run
hbm sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `hbm sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`hbm sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`hbm insights`](/reference/cli-commands#hbm-insights).
...
`hbm sessions repair-routing` finds message-bearing session rows with no
...
hbm sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
hbm sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.hbm/state.db ~/.hbm/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into HBM AGENT and
continue it here. HBM AGENT reads Claude Code's session logs
...
hbm sessions import --from claude
hbm sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
When HBM AGENT cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, HBM AGENT uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to HBM AGENT in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.hbm/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.hbm/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.hbm/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.hbm/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.hbm/sessions/sessions.json` is a **legacy mirror** of it,
...
`hbm sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.hbm/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `hbm sessions list`, the cause is
`state.db` not receiving them — run `hbm sessions repair` and watch for a
...
`*.jsonl` files in `~/.hbm/sessions/`. They are no longer written or
read by HBM AGENT. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every HBM AGENT process in the same `HBM_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.hbm/config.yaml`:
...
`in its tip segment`). `hbm sessions export` still works for such sessions.
...
hbm sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `hbm sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
