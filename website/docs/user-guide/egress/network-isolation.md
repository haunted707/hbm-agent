...
When running HBM AGENT inside Docker, the default `network_mode: host` gives the
...
The HBM AGENT [SECURITY.md](https://github.com/NousResearch/hermes-agent/blob/main/SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   HBM_UID=$(id -u) HBM_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=hbm,hbm-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://hbm-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](https://github.com/NousResearch/hermes-agent/blob/main/SECURITY.md) — HBM AGENT trust model and vulnerability reporting
- [Docker](/user-guide/docker) — running HBM AGENT in a container
