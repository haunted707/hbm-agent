...
sidebar_position: 1
title: "Telegram"
description: "Set up HBM AGENT as a Telegram bot"
---
...
The **Messaging → Telegram** page in the [dashboard](../features/web-dashboard.md) and the [desktop app](../desktop.md) has a **Create with QR** button. Scan the code (or open the link) in Telegram; HBM AGENT creates the bot for you, detects your Telegram user ID, writes `TELEGRAM_BOT_TOKEN` and `TELEGRAM_ALLOWED_USERS` into your profile's `.env`, and restarts the gateway. If you prefer to create the bot yourself, follow the manual steps below.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "HBM AGENT") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_hbm_bot`)
...
Enable `status_indicator` and HBM AGENT sets that short description to **Online**
...
`priority_mode` controls how your list combines with HBM AGENT's built-in priority list:

- `prepend`: put your commands first, then HBM AGENT defaults
- `append`: keep HBM AGENT defaults first, then your commands
...
Telegram allows up to 100 BotCommands, but large command payloads can fail. HBM AGENT defaults to 60 for reliability and clamps configured values to `1..100`; use `/commands` for the full command list.

### Inline command picker: search every command (no cap)

The `/` menu is capped, but Telegram's **inline mode** is not. Once enabled, type `@yourbotname` followed by a search term in any chat to get a live, searchable picker over **every** HBM AGENT command and installed skill — results are computed per keystroke and paginated, so nothing is ever trimmed:
...
HBM AGENT uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
By default, HBM AGENT connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
If the fallback IP discovery path is unhealthy on your host, set `HBM_TELEGRAM_DISABLE_FALLBACK_IPS=true` to keep cold connect on the plain `api.telegram.org` path. You can also bound DNS-over-HTTPS fallback discovery with `HBM_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT` in seconds; the default is `5`.
...
You can also set it manually in `~/.hbm/.env`:
...
Voice messages you send on Telegram are automatically transcribed by HBM AGENT's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running HBM AGENT — no API key required
...
If you'd rather have the **agent itself** handle audio — for diarization, a custom transcription tool, or just archiving the recording — set `stt.enabled: false` in `~/.hbm/config.yaml`:
...
With STT disabled, the gateway still downloads the voice/audio attachment into HBM AGENT's audio cache, but **does not transcribe it**. The agent receives the message with a marker like:

```
[The user sent a voice message: /home/<user>/.hbm/cache/audio/<hash>.ogg]
```
...
Telegram's **public** Bot API caps `getFile` downloads at **20 MB**, so any voice note, audio file, video, or document larger than that is silently rejected by HBM AGENT with a "too large" reply. The documented way around this is to run a **local** [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) daemon — the same server software Telegram uses, but running on your network. A local server raises the file ceiling to **2 GB** and HBM AGENT auto-lifts its own internal cap when it sees a custom `base_url` configured.
...
### Step 4: Point HBM AGENT at the local server

Add the URLs under `platforms.telegram.extra` in `~/.hbm/config.yaml`:
  ...
  telegram:
    extra:
      ...
      local_mode: true        # see Step 5 below — only set this if the bot's data
                              # directory is readable by the HBM AGENT process
```
...
When `base_url` is set, HBM AGENT:
...
```bash
hbm gateway restart
grep -E "Using custom Telegram base_url|Using Telegram local_mode" ~/.hbm/logs/gateway.log | tail
```
...
2. **With `--local`** (set via `TELEGRAM_LOCAL=1` above): files are written to the server's filesystem and the `getFile` response returns an **absolute path** instead of an HTTP URL. The 20MB ceiling is lifted. HBM AGENT must then read the bytes **from disk**, not over HTTP.

To make the disk-read path work, set `local_mode: true` in the config above **and** make sure the HBM AGENT process can read the path the server returns. Two scenarios:

- **Same machine** — telegram-bot-api and HBM AGENT run on the same host. Bind-mount the data volume to a directory that HBM AGENT can read (e.g., `/var/lib/telegram-bot-api`), and make sure the file ownership matches. The container drops privileges to its internal `telegram-bot-api` user (uid varies by image); the simplest fix is to add `user: "<UID>:<GID>"` to the compose service so files are owned by a uid HBM AGENT already runs as.
- **Different machines** — the bot server runs on one host (e.g., a NAS, a separate VM) and HBM AGENT on another. The server's data directory must be shared with the HBM AGENT machine at the **same absolute path** the server reports (typically `/var/lib/telegram-bot-api`). NFS works well for this; CIFS/SMB with `uid=` mount remapping is friendlier if you don't want to deal with uid mismatches at the filesystem level.

If `local_mode: true` is set but HBM AGENT can't `stat` the returned file path (permissions or wrong mount), python-telegram-bot silently falls back to an HTTP `getFile` against the local server — which in `--local` mode responds with `404 Not Found`. The symptom shows up in `gateway.log` as:
...
If you see that, the cap-lift is working but the file-share isn't. Verify `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/` from the HBM AGENT host as the user the gateway runs as, and confirm a single file is `cat`-able without a permission error.
...
```bash
tail -f ~/.hbm/logs/gateway.log | grep -iE "telegram|cache"
```

You should see a `[Telegram] Cached user voice at /home/<user>/.hbm/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.

## Group Chat Usage

HBM AGENT works in Telegram group chats with a few considerations:
...
- In groups with multiple HBM AGENT bots, `telegram.exclusive_bot_mentions` keeps routing deterministic. When a message explicitly mentions one or more Telegram bot usernames, only the mentioned bot profiles process it; other HBM AGENT bots ignore it before reply and wake-word fallbacks run. This is enabled by default.
- Renaming the bot's `@username` in BotFather is picked up automatically — HBM AGENT follows the new handle for mention routing without a gateway restart. Collectible (Fragment) usernames that don't end in `bot` are supported too.
- Use `telegram.ignored_threads` to keep HBM AGENT silent in specific Telegram forum topics, even when the group would otherwise allow free responses or mention-triggered replies
- If `telegram.require_mention` is left unset or false, HBM AGENT keeps the previous open-group behavior and responds to normal group messages it can see

### Multiple HBM AGENT bots in one group

If you run several HBM AGENT profiles in the same Telegram group, create one Telegram bot token per profile and start one gateway per profile. Do not reuse the same bot token in multiple running gateways; Telegram will reject concurrent polling for the same token.
...
With this setup, a group message like `@research_bot @ops_bot summarize this` is processed by `research_bot` and `ops_bot` only. Other HBM AGENT bots in the group stay silent, even if the message is a reply to one of their earlier messages or would otherwise match a shared wake word.
...
Group conversation text and media captions keep every mention when the message names other participants too (`@research_bot , @ops_bot are you both listening?` reaches `research_bot` verbatim); when this bot is the only one addressed, its own handle is still stripped so short answers such as `@hbm_bot 2` keep working. Group turns also carry the bot's own Telegram username in the per-channel context so the model can tell which retained mentions are for it. Slash commands still use the normal command-trigger cleanup.
...
# named profiles
hbm -p research gateway start
hbm -p research gateway status
hbm -p research gateway stop
```

For a small fixed fleet, use a shell loop or script that calls `hbm gateway <action>` for the default profile and `hbm -p <profile> gateway <action>` for each named profile. This is more reliable than assuming a single process-level command controls every named profile on every service manager.
...
1. **Telegram delivery:** turn off BotFather privacy mode, promote the bot to
   admin, or mention the bot directly. HBM AGENT cannot respond to group messages
...
3. **HBM AGENT authorization:** make sure the sender is listed in
...
5. **Multi-bot routing:** if a group contains several bots, make sure each
   HBM AGENT profile uses a unique bot token and keep `exclusive_bot_mentions`
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with HBM AGENT.
...
1. Open your private chat with the HBM AGENT bot in Telegram
2. Tap the bot's name at the top to open chat info
3. Enable **Topics** (the toggle to turn the chat into a forum)

Without this, HBM AGENT will log `The chat is not a forum` on startup and skip topic creation. This is a Telegram client-side setting — the bot cannot enable it programmatically.
:::

Add topics under `platforms.telegram.extra.dm_topics` in `~/.hbm/config.yaml`:
...
1. On gateway startup, HBM AGENT calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
A ChatGPT-style multi-session DM — one bot, many parallel conversations. Unlike the operator-curated `extra.dm_topics` above, this mode is **user-driven**: no config, no pre-declared topic names. The end user flips it on with `/topic`, then taps the Telegram **+** button to create as many topics as they want, each one a fully independent HBM AGENT session.
...
| Topic names | Chosen by operator | Chosen by user; auto-renamed to match HBM AGENT session title |
...
When the user first runs `/topic`, HBM AGENT calls `getMe` to verify both flags. If either is off, HBM AGENT sends a screenshot of the BotFather Threads Settings page and explains what to toggle — no activation happens until prerequisites are met.
...
4. HBM AGENT responds inside that topic — the topic is now a standalone session
...
When HBM AGENT generates a session title for a topic (via the auto-title pipeline, after the first exchange), the Telegram topic itself is renamed to match — e.g. "New Topic" becomes "Database migration plan". The rename is best-effort: failures are logged but don't break the session.
...
When this flag is on, HBM AGENT still generates an internal session title (used by `hbm sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.

### `/new` inside a topic

Resets the current topic's session (new session ID, fresh history) without touching other topics. HBM AGENT replies with a reminder that for parallel work, creating another topic (via **All Messages**) is usually what you want.
...
This binds the current topic to an existing HBM AGENT session instead of starting fresh. Useful for continuing a conversation that started before topic mode was enabled. Restrictions:

- The target session must belong to the same Telegram user
- The target session must not already be bound to another topic

HBM AGENT confirms with the session title and replays the last assistant message for context.

To discover session IDs, send `/topic` (no argument) in the root DM — HBM AGENT lists the user's unlinked Telegram sessions.
...
Send `/topic off` in the root DM. HBM AGENT flips the row off for **this profile's** namespace, clears that profile's `(thread_id → session_id)` bindings for the chat, and the root DM reverts to a normal HBM AGENT chat. Existing topics in Telegram aren't deleted — they just stop being gated as independent sessions. Re-run `/topic` later to turn it back on.
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.hbm/config.yaml`:
...
1. When a message arrives in a mapped group topic, HBM AGENT looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | HBM AGENT creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
- **Bot API 9.4 (Feb 2026):** Private Chat Topics — bots can create forum topics in 1-on-1 DM chats via `createForumTopic`. HBM AGENT uses this for two distinct features: operator-curated [Private Chat Topics](#private-chat-topics-bot-api-94) (config-driven, fixed topic list) and user-driven [Multi-session DM mode](#multi-session-dm-mode-topic) (activated by `/topic`, unlimited user-created topics).
- **Privacy policy:** Telegram now requires bots to have a privacy policy. Set one via BotFather with `/setprivacy_policy`, or Telegram may auto-generate a placeholder. This is particularly important if your bot is public-facing.
- **Bot API 9.5 (Mar 2026): Native streaming via `sendMessageDraft`.** HBM AGENT supports Telegram's native streaming-draft API as an opt-in transport for private chats. The default remains the legacy `editMessageText` path because draft previews can visibly collapse and re-render on some Telegram clients.

### Streaming transport (`gateway.streaming.transport`)

When streaming is enabled (`gateway.streaming.enabled: true`), HBM AGENT picks one of four transports:
...
**MarkdownV2 fallback.** When the rich path is unavailable for a message, HBM AGENT converts markdown to MarkdownV2. Since MarkdownV2 has no native table syntax, pipe tables are normalized:
...
When enabled, HBM AGENT attaches Telegram's `LinkPreviewOptions(is_disabled=True)` to every outgoing message and falls back to the legacy `disable_web_page_preview` parameter on older `python-telegram-bot` versions.
...
When you send `/model` with no arguments in a Telegram chat, HBM AGENT shows an interactive inline keyboard for switching models:
...
Or add it to `~/.hbm/.env`:

```bash
HTTPS_PROXY=http://proxy.example.com:8080
```

The proxy applies to both the primary transport and all fallback IP transports. No additional HBM AGENT configuration is needed — if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that HBM AGENT uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `hbm gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.hbm/.env`. |
...
Configure the response timeout via `agent.clarify_timeout` in `~/.hbm/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.
...
```bash
HBM_TELEGRAM_NOTIFICATIONS=all
```
