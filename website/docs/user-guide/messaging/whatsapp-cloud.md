...
description: "Set up HBM AGENT as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

HBM AGENT can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The HBM AGENT gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not an HBM AGENT limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. HBM AGENT degrades gracefully if absent.
...
2. **Add** → name (e.g. `hbm-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.hbm/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing HBM AGENT to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the HBM AGENT gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.hbm/.env`.
3. Start the HBM AGENT gateway: `hbm gateway`.
...
## Allowlist (HBM AGENT-side)

In addition to Meta's recipient whitelist, HBM AGENT has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.hbm/.env`:
...
The `hbm whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.hbm/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, HBM AGENT uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
HBM AGENT acknowledges inbound messages immediately:
...
HBM AGENT TTS produces MP3. Two paths:
...
HBM AGENT warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in HBM AGENT. If you need it, please [open an issue](https://github.com/NousResearch/hermes-agent/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  HBM AGENT's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  HBM AGENT doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.hbm/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `hbm gateway` is up.
- **App Secret not set** — without it, HBM AGENT refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in HBM AGENT.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `hbm tools list` and verify the platform is using `hbm-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `hbm_cli/platforms.py` for the platform → default toolset mapping.

### STT (voice note transcription) returns empty / "could not transcribe"

The default `stt.provider: local` requires `pip install faster-whisper`.  If you're a Nous subscriber, you can route STT through the managed gateway instead — select **Nous Subscription** for speech-to-text in `hbm tools`, or set it directly:

```bash
hbm config set stt.provider nous
hbm gateway restart
```
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that HBM AGENT will accept as authentic.
...
| | Baileys (`hbm whatsapp`) | Cloud API (`hbm whatsapp-cloud`) |
|---|---|---|
...
Most users running HBM AGENT for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
