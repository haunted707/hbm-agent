...
sidebar_position: 12
title: "Google Chat"
description: "Set up HBM AGENT as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect HBM AGENT to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your HBM AGENT
...
> Run `hbm gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only HBM AGENT can read (e.g.,
`~/.hbm/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a hbm restart)
...
- Principal: `hbm-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — HBM AGENT calls
...
- **App name**: whatever you want users to see ("HBM AGENT" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/hbm-chat-events`.
...
`ADDED_TO_SPACE` event that HBM AGENT uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure HBM AGENT

Add the Google Chat section to `~/.hbm/.env`:
...
On Docker / hosted images `/opt/hbm/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `HBM_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "HBM AGENT is thinking…" marker, then
...
`~/.hbm/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "HBM AGENT is thinking…").
...
Thread support: when a user replies inside a thread, HBM AGENT detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate HBM AGENT session.
...
3. Download the JSON. Move it onto the host that runs HBM AGENT.
4. Register the client with HBM AGENT (run under the profile you want it scoped to):
...
hbm -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's HBM AGENT home (e.g.
`~/.hbm/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.hbm/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.hbm/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, HBM AGENT isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `hbm gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: HBM AGENT will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.hbm/google_chat_user_tokens/<sanitized_email>.json` (filesystem
