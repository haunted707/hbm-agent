...
description: "Chat with HBM AGENT from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with HBM AGENT from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with HBM AGENT](/guides/use-voice-mode-with-hbm).
...
saved by `hbm gateway setup` can enable a credential-based platform without a
...
```mermaid
flowchart TB
    subgraph Gateway["HBM AGENT Gateway"]
...
For group chats, hooks, and automation flows, HBM AGENT supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. HBM AGENT keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; HBM AGENT does not hide failures just because the text resembles a silence token.
...
```bash
hbm gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
hbm gateway              # Run in foreground
hbm gateway setup        # Configure messaging platforms interactively
hbm gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo hbm gateway install --system   # Linux only: install a boot-time system service
hbm gateway start        # Start the default service
hbm gateway stop         # Stop the default service
hbm gateway status       # Check default service status
hbm gateway status --system         # Linux only: inspect the system service explicitly
```

### Stack dump on demand (`SIGUSR2`)

On Linux and macOS, `kill -USR2 <gateway pid>` appends a dump of every thread's
stack to `~/.hbm/logs/gateway_faulthandler.log` and the gateway keeps
...
```bash
hbm gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. HBM AGENT sends heartbeats
...
| `/update` | Update HBM AGENT to the latest version |
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.hbm/config.yaml`:
...
hbm pairing approve telegram XKGH5N7P

# Other pairing commands:
hbm pairing list          # View pending + approved users
hbm pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, HBM AGENT appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.hbm/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.hbm/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `HBM_HOME`:

- `~/.hbm/status_phrases.yaml` or any `*.yaml` in `~/.hbm/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to HBM_HOME
...
Off by default. When enabled, HBM AGENT prepends a human-readable timestamp
...
HBM AGENT confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.hbm/config.yaml`:
...
```bash
HBM_BACKGROUND_NOTIFICATIONS=result
```
...
hbm gateway start                 # Start the service
hbm gateway stop                  # Stop the service
hbm gateway status                # Check status
journalctl --user -u hbm-gateway -f  # View logs
...
sudo hbm gateway start --system
sudo hbm gateway status --system
journalctl -u hbm-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit HBM AGENT installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit hbm-gateway` (or `sudo systemctl edit hbm-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `hbm update`. When `hbm update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart hbm-gateway` command (it never blocks on an interactive password prompt).
...
```bash
hbm gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `hbm update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo hbm update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/hbm-gateway`:

```
hbm ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed hbm-gateway*, /usr/bin/systemctl --no-ask-password start hbm-gateway*, /usr/bin/systemctl --no-ask-password restart hbm-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. HBM AGENT will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple HBM AGENT installations on the same machine (with different `HBM_HOME` directories), each gets its own systemd service name. The default `~/.hbm` uses `hbm-gateway`; other installations use `hbm-gateway-<hash>`. The `hbm gateway` commands automatically target the correct service for your current `HBM_HOME`.
:::

### macOS (launchd)

```bash
hbm gateway install               # Install as launchd agent
hbm gateway start                 # Start the service
hbm gateway stop                  # Stop the service
hbm gateway status                # Check status
tail -f ~/.hbm/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.hbm.gateway.plist`. It includes three environment variables:
...
- **HBM_HOME** — scopes the gateway to your HBM AGENT installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `hbm gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HBM_HOME` directory gets its own launchd label. The default `~/.hbm` uses `ai.hbm.gateway`; other installations use `ai.hbm.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `hbm-cli` | Full access |
| Telegram | `hbm-telegram` | Full tools including terminal |
| Discord | `hbm-discord` | Full tools including terminal |
| WhatsApp | `hbm-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `hbm-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `hbm-slack` | Full tools including terminal |
| Google Chat | `hbm-google_chat` | Full tools including terminal |
| Signal | `hbm-signal` | Full tools including terminal |
| SMS | `hbm-sms` | Full tools including terminal |
| Email | `hbm-email` | Full tools including terminal |
| Home Assistant | `hbm-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `hbm-mattermost` | Full tools including terminal |
| Matrix | `hbm-matrix` | Full tools including terminal |
| DingTalk | `hbm-dingtalk` | Full tools including terminal |
| Feishu/Lark | `hbm-feishu` | Full tools including terminal |
| WeCom | `hbm-wecom` | Full tools including terminal |
| WeCom Callback | `hbm-wecom-callback` | Full tools including terminal |
| Weixin | `hbm-weixin` | Full tools including terminal |
| BlueBubbles | `hbm-bluebubbles` | Full tools including terminal |
| QQBot | `hbm-qqbot` | Full tools including terminal |
| Yuanbao | `hbm-yuanbao` | Full tools including terminal |
| Microsoft Teams | `hbm-teams` | Full tools including terminal |
| API Server | `hbm-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `hbm-webhook` | Full tools including terminal |
| Raft | `hbm-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.hbm/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.hbm/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
