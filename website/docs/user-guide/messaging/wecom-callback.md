...
Connect HBM AGENT to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
HBM AGENT supports two WeCom integration modes:
...
> Run `hbm gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. HBM AGENT decrypts the message, queues it for the agent
...
(Use `hbm gateway start` only after `hbm gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in HBM AGENT and the token the
...
in `~/.hbm/.env` values around `=` will also break signature checks. After
fixing, restart `hbm gateway run`.
...
Check `hbm gateway run` logs for the bound host/port. If the adapter bound to
