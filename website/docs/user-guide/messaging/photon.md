...
Connect HBM AGENT to **iMessage** through [Photon][photon], a managed
...
your first iMessage from HBM AGENT — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, HBM AGENT runs it in a
...
hbm photon setup --phone +15551234567
```
...
2. **Finds or creates** the `HBM AGENT` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.hbm/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.hbm/.env`
...
(device token, dashboard project id) lives in `~/.hbm/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other HBM AGENT
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, HBM AGENT replies with a pairing code. Approve it with:

```bash
hbm pairing approve photon <CODE>
```

Use `hbm pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.hbm/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.hbm/.env`):
...
By default HBM AGENT responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `HBM AGENT` and
`@HBM AGENT` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and HBM AGENT will reply.
...
assigned iMessage line HBM AGENT uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run hbm photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `hbm photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `hbm photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `hbm photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** HBM AGENT sends images, voice
  ...
  media.
- **Native polls are supported.** HBM AGENT sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to HBM AGENT, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for HBM AGENT-sent messages
...
- **Message effects are supported.** HBM AGENT sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `hbm send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<hbm-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when HBM AGENT is set
...
| `PHOTON_MENTION_PATTERNS` | HBM AGENT wake words  | JSON list / comma / newline regex patterns for group mentions |
