---
sidebar_position: 30
title: "HBM AGENT Relay"
description: "Connect HBM AGENT to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# HBM AGENT Relay (Connector)
...
HBM AGENT Relay is not a chat platform itself — it is a **connector system** that
...
secret. `hbm gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.hbm/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.hbm/.env`:
...
`hbm gateway enroll` refuses to run in managed/hosted installs — there the
...
`hbm gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.hbm/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
token. Re-login with `hbm auth add nous` (or `hbm setup`) and retry.
...
startup. Restart it (`hbm gateway restart`).
