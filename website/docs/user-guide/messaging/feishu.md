...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up HBM AGENT as a Feishu or Lark bot"
---

# Feishu / Lark Setup

HBM AGENT integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; HBM AGENT opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | HBM AGENT responds to every message. |
| Group chats | HBM AGENT responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. HBM AGENT will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `hbm gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when HBM AGENT runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run HBM AGENT behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, HBM AGENT starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how HBM AGENT responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | HBM AGENT responds to @mentions from any user in any group. |
| `allowlist` | HBM AGENT only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | HBM AGENT ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let HBM AGENT read all group traffic without requiring an @mention:
...
HBM AGENT auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default HBM AGENT ignores messages sent by other bots. Enable bot-to-bot messaging when you want HBM AGENT to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions HBM AGENT. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `hbm update --gateway` needs confirmation, the adapter records the selected answer in HBM AGENT's `.update_response` file and replaces the card inline with a resolved state.
...
:::warning
Without a published card callback, Feishu will still successfully *send* interactive cards (sending only requires `im:message:send` permission), but clicking any button returns error 200340. The card appears to work — the error only surfaces when a user interacts with it, and the click never reaches HBM AGENT (nothing is logged), because Feishu rejects it before delivering the callback.
:::

Error codes 200672 / 200673 indicate the callback *did* reach HBM AGENT and Feishu rejected the response; if you see them, please file an issue with the matching `gateway.log` lines.

## Document Comment Intelligent Reply

Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, HBM AGENT reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.hbm/feishu_comment_rules.json` (pairing grants in `~/.hbm/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
You can invite the HBM AGENT Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, HBM AGENT can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends HBM AGENT the meeting invitation event.
- HBM AGENT extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, HBM AGENT drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so HBM AGENT can reply to the inviter.
...
|---------|---------|---------|
| Quiet period | `HBM_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `HBM_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `HBM_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `HBM_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.hbm/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `HBM_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `HBM_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `HBM_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `HBM_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `HBM_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `HBM_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `hbm gateway setup` |
| `Another local HBM AGENT gateway is already using this Feishu app_id` | Only one HBM AGENT instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `hbm-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
