...
sidebar_position: 4
title: "Slack"
description: "Set up HBM AGENT as a Slack bot using Socket Mode"
---

# Slack Setup

Connect HBM AGENT to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your HBM AGENT instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. HBM AGENT uses the modern
...
The fastest path is to paste a manifest HBM AGENT generates for you. It
...
### Option A: From an HBM AGENT-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   hbm slack manifest --agent-view --write
   ```
   This writes `~/.hbm/slack-manifest.json` and prints paste-in
...
4. Enter an app name (e.g., "HBM AGENT") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, HBM AGENT can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `hbm-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not an HBM AGENT configuration issue.
:::
...
HBM AGENT uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
hbm gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
hbm gateway              # Foreground
hbm gateway install      # Install as a user service
sudo hbm gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; HBM AGENT now
...
Every HBM AGENT command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
HBM AGENT command with its description.

Under the hood: HBM AGENT ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/hermes-agent/blob/main/hbm_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing HBM AGENT
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so HBM AGENT can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. HBM AGENT only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When HBM AGENT adds new commands (e.g. after `hbm update`), regenerate
the manifest and update your Slack app:

```bash
hbm slack manifest --write
```
...
3. Paste the new contents of `~/.hbm/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/hbm <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/hbm bg run the tests` — HBM AGENT routes it the same way as `/bg
run the tests`. Free-form questions also work: `/hbm what's the
...
Slack never delivers them to HBM AGENT.

As a workaround, HBM AGENT recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
HBM AGENT treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@HBM AGENT !stop`) and with
...
HBM AGENT falls back to a text prompt, the prompt instructs you to reply
...
fails, HBM AGENT retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@HBM AGENT /cmd` — reply as normal visible
messages instead.)
...
```bash
hbm slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how HBM AGENT behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@HBM AGENT what time is it?`). In channels, HBM AGENT replies in a thread attached to that message. |
| **Threads** | If you @mention HBM AGENT inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.hbm/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions HBM AGENT, and "all" accepts every
      # other bot. HBM AGENT always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.native_task_cards` | `false` | When `true`, renders live tool calls as Slack-native plan/task cards. Cards work with Slack's built-in default `tool_progress: off`; an explicitly configured `display.tool_progress: off` (global or `display.platforms.slack`; `/verbose` writes the same key) disables cards too. Cards need a thread: when the card lane is active and the chat has no thread to anchor on (a top-level DM with `reply_in_thread: false`), HBM AGENT shows no tool progress instead of text bubbles, unless you explicitly set `tool_progress: new`/`all`, which falls back to editable text progress there. Recoverable native API failures fall back to one continuously edited text update. |
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions HBM AGENT, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default HBM AGENT sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"HBM AGENT is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), HBM AGENT uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — HBM AGENT seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and HBM AGENT falls back
...
- HBM AGENT checks thread eligibility before attempting publication, so a
...
- On a supported threaded destination, if the native stream fails for a
  recoverable reason (API error, rate limit), HBM AGENT falls back to a single continuously edited text message so
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to HBM AGENT in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before HBM AGENT will respond.
...
By default HBM AGENT ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several HBM AGENT instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- HBM AGENT always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer HBM AGENTs are filtered consistently across workspaces.
...
`client_msg_id` — the same signature HBM AGENT uses to recognise app posts — so it
is dropped as bot traffic. This blocks a common pattern: a custom front-end (an
internal dashboard, a mobile shell, a kiosk) that sends messages to HBM AGENT *as*
the logged-in user.
...
manifest — regenerate with `hbm slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where HBM AGENT will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @HBM AGENT`).
...
Delivery works even when the cron process isn't co-located with the gateway — HBM AGENT falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
HBM AGENT can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, HBM AGENT also loads tokens from an **OAuth token file** at:

```
~/.hbm/slack_tokens.json
```
...
- When a message arrives, HBM AGENT uses the correct workspace-specific client to respond.
...
HBM AGENT supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @HBM AGENT` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @HBM AGENT` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. HBM AGENT now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @HBM AGENT`)
...
- Tokens should be stored in `~/.hbm/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your HBM AGENT config directory
