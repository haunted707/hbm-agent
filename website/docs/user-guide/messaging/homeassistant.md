...
description: Control your smart home with HBM AGENT via Home Assistant integration.
...
HBM AGENT integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "HBM AGENT"
5. Copy the token
...
HBM AGENT registers four tools for smart home control:
...
Configure which events the agent sees in `~/.hbm/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "HBM AGENT".
...
The adapter reads credentials from `~/.hbm/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active HBM AGENT profile
...
running HBM AGENT — `curl -H "Authorization: Bearer <token>" <url>/api/` should
