...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to HBM AGENT via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With HBM AGENT's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["hbm-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to HBM AGENT's API server just like it would connect to OpenAI. HBM AGENT handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

:::important Runtime location
The API server is a **HBM AGENT runtime**, not a pure LLM proxy. For each request, HBM AGENT creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.

For example, if a laptop points Open WebUI or another OpenAI-compatible client at an HBM AGENT API server on a remote machine, `pwd`, file tools, browser tools, local MCP tools, and other workspace tools run on the remote API-server host, not on the laptop.
:::

Open WebUI talks to HBM AGENT server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
```bash
hbm config set API_SERVER_ENABLED true
hbm config set API_SERVER_KEY your-secret-key
```

`hbm config set` auto-routes the flag to `config.yaml` and the secret to `~/.hbm/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
hbm gateway stop && hbm gateway
```

### 2. Start HBM AGENT gateway
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **hbm-agent** for the default profile). Start chatting!
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in HBM AGENT
7. Click the **checkmark** to verify the connection
8. **Save**

Your agent model should now appear in the model dropdown (named after your profile, or **hbm-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and HBM AGENT responds accordingly. Each request includes the full conversation history.
...
2. Edit your hbm-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and HBM AGENT can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, HBM AGENT also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. HBM AGENT creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
Your agent has access to the same tools and capabilities as that API-server HBM AGENT instance. If the API server is remote, those tools are remote too.

If you need tools to run against your **local** workspace today, run HBM AGENT locally and point it at a pure LLM provider or pure OpenAI-compatible model proxy (for example vLLM, LiteLLM, Ollama, llama.cpp, OpenAI, OpenRouter, etc.). A future split-runtime mode for "remote brain, local hands" is being tracked in [#18715](https://github.com/NousResearch/hermes-agent/issues/18715); it is not the behavior of the current API server.
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | HBM AGENT's API URL (include `/v1`) |
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `hbm-agent`
...
- **Empty Ollama backend shadowing the picker**: If you omitted `ENABLE_OLLAMA_API=false`, Open WebUI shows an empty Ollama section above your HBM AGENT models. Restart the container with `-e ENABLE_OLLAMA_API=false` or disable Ollama in **Admin Settings → Connections**.
...
HBM AGENT may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in HBM AGENT.
...
To run separate HBM AGENT instances per user — each with their own config, memory, and skills — use [profiles](/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated HBM AGENT.
...
```bash
hbm -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
