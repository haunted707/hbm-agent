---
title: "HBM AGENT Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "HBM AGENT Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# HBM AGENT Skill Authoring
...
| Path | `skills/software-development\hbm-agent-skill-authoring` |
| Version | `2.0.0` |
| Author | HBM AGENT |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `hbm-agent`, `conventions`, `skill-md` |
...
# Authoring HBM AGENT-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.hbm/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the hbm-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with hbm-agent
...
- Don't use for: personal skills in `~/.hbm/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `hbm skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), HBM AGENT
license: MIT
...
- Credit the **human first**, then "HBM AGENT" as secondary collaborator: `Ben Barclay (benbarclay), HBM AGENT`.
- Never `author: HBM AGENT` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), HBM AGENT`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.hbm/skills/`.
...
|---|---|
| HBM AGENT tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.hbm/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: HBM AGENT` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.hbm.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through HBM AGENT tools
