...
Ledger location: `$HBM_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `HBM_CITATION_LEDGER`.

## How to Run

```bash
S=~/.hbm/skills/research/grounded-citations/scripts/sources.py
...
`hbm skills install official/social-media/reddit-reading` or
`hbm skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `HBM_CITATION_LEDGER`) if their
