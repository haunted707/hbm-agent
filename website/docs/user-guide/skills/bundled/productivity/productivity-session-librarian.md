...
| List/filter by metadata (age, source, cost, tokens, workspace) | `hbm sessions list` / `stats` via terminal |
| Rename | `hbm sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `hbm sessions archive <filters>` |
| Delete (destructive) | `hbm sessions delete` / `hbm sessions prune <filters>` |
| Export before deleting anything valuable | `hbm sessions export --session-id <id> --format md` |
...
`hbm sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `hbm sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `hbm sessions list`
