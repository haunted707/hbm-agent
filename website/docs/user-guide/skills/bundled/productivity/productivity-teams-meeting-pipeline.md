...
Everything operator-facing is a `hbm teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${HBM_HOME:-~/.hbm}/.env`:
...
```bash
hbm teams-pipeline validate              # config snapshot — run first after any change
hbm teams-pipeline token-health          # Graph token status
hbm teams-pipeline token-health --force-refresh   # force a fresh token acquisition
hbm teams-pipeline list                  # recent meeting jobs
hbm teams-pipeline list --status failed  # only failed jobs
hbm teams-pipeline show <job-id>         # full detail of one job
hbm teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
hbm teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
hbm teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
hbm teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
hbm teams-pipeline fetch --join-web-url "<url>" --organizer-user-id <id>   # organizer-scoped lookup (required for /meet/ short URLs)
```

### Subscription management

```bash
hbm teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

hbm teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
hbm teams-pipeline delete-subscription <sub-id>
hbm teams-pipeline maintain-subscriptions            # renew near-expiry ones
hbm teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `hbm teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `hbm cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
