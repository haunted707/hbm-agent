...
4. Store the token in `${HBM_HOME:-~/.hbm}/.env` (or via `hbm setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for HBM AGENT. Pipe through `python -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${HBM_HOME:-~/.hbm}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
