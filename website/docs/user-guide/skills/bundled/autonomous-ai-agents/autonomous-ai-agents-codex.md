...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`hbm-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hbm-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the HBM AGENT terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For HBM AGENT itself, `model.provider: openai-codex` uses HBM AGENT-managed Codex
OAuth from `~/.hbm/auth.json` after `hbm auth add openai-codex`. For the
...
## HBM AGENT Gateway Caveat

When invoking the Codex CLI from an HBM AGENT gateway/service context (for example,
