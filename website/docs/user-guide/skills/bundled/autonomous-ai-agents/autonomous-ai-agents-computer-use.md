...
This wrapper skill teaches the HBM AGENT `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. HBM AGENT autodetection is a
planned cua-driver follow-up, so currently point HBM AGENT at the resulting
...
HBM AGENT autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point HBM AGENT at that
