...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/software-development/code-wiki` |
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.hbm/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.hbm/wikis/$REPO_NAME"
...
1. **User input** — [`cli.py:HbmCLI.run_session`](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/software-development\code-wiki/<link>)
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "hbm-agent code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.hbm/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.hbm/wikis/`. Only write into the repo when the user explicitly requests it.
