---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in HBM AGENT"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in HBM AGENT"
---
...
Set up Actual Computer (actual.inc) inference in HBM AGENT.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hbm skills install official/devops/actual-setup` |
| Path | `optional-skills/devops\actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + HBM AGENT |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as an HBM AGENT inference
...
- User has an `ac_` key and wants HBM AGENT routed through their Actual cluster.
...
- HBM AGENT has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current HBM AGENT — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.hbm/.env`.
...
3. Select provider + model:
   ```bash
   hbm config set model.provider actual
   hbm config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   hbm chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point HBM AGENT at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.hbm/.env`, then:
   ```bash
   hbm config set model.provider actual
   hbm config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   hbm chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by HBM AGENT since the first-class provider).**
   ...
   request still 400s this way on an old HBM AGENT, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** HBM AGENT's default toolset
   ...
   servers emit a bare `data: [DONE]` — HBM AGENT reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current HBM AGENT the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
hbm chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
hbm chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
