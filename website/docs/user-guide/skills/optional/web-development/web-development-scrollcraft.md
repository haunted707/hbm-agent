...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/web-development/scrollcraft` |
| Path | `optional-skills/web-development/scrollcraft` |
| Version | `1.0.0` |
| Author | nateherkai (upstream scroll-craft), ported by HBM AGENT |
| License | MIT |
...
- `scripts/shoot.mjs` needs Playwright (`npm install playwright` or
  `playwright-core` plus a Chrome install). HBM AGENT's `browser_exec` tool is the
