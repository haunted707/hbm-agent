...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/mcp/fastmcp` |
...
| Related skills | [`hbm-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hbm-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into HBM AGENT or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to HBM AGENT. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hbm/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hbm/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is HBM AGENT integration, either:

- configure the server in `~/.hbm/config.yaml` using the `native-mcp` skill, or
...
### HBM AGENT cannot see the deployed server

The server-building part may be correct while the HBM AGENT config is not. Load the `native-mcp` skill and configure the server in `~/.hbm/config.yaml`, then restart HBM AGENT.
