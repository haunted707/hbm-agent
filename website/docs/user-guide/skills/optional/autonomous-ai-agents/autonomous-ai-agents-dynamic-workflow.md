...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/autonomous-ai-agents/dynamic-workflow` |
| Path | `optional-skills/autonomous-ai-agents/dynamic-workflow` |
| Version | `2.0.0` |
| Author | Teknium + HBM AGENT |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `orchestration`, `fan-out`, `subagents`, `delegation`, `verification`, `migration`, `audit`, `research`, `campaign` |
| Related skills | [`hbm-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hbm-agent), [`simplify-code`](/docs/user-guide/skills/bundled/software-development/software-development-simplify-code) |
...
for serial chains. For a refactor or fix campaign on hbm-agent itself, load
`hbm-agent` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `HBM_HOME`,
