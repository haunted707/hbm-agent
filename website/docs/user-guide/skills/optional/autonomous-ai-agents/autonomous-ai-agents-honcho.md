---
title: "Honcho — Configure and troubleshoot Honcho memory for HBM AGENT"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for HBM AGENT"
---
...
Configure and troubleshoot Honcho memory for HBM AGENT.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hbm skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for HBM AGENT

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every HBM AGENT profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hbm#running-honcho-locally-with-hbm

### Verify

```bash
hbm honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this HBM AGENT profile's AI peer
...
Honcho models conversations as interactions between **peers**. HBM AGENT creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this HBM AGENT instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each HBM AGENT run |
| `global` | Single session across all directories |

Manual override: `hbm honcho map my-project-name`
...
Each HBM AGENT profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block hbm.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `hbm.coder` host block in `honcho.json`
...
```bash
hbm honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for HBM AGENT when Honcho memory is active.
...
Config file: `$HBM_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `hbm honcho setup`. Ensure `memory.provider: honcho` is in `~/.hbm/config.yaml`.

### Memory not persisting across sessions
Check `hbm honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `hbm profile create <name> --clone`. For existing profiles: `hbm honcho sync`.
...
| Command | Description |
|---------|-------------|
| `hbm honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `hbm honcho status` | Show resolved config, connection test, peer info for active profile |
| `hbm honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `hbm honcho disable` | Disable Honcho for the active profile |
| `hbm honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `hbm honcho peers` | Show peer identities across all profiles |
| `hbm honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `hbm honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `hbm honcho sessions` | List known directory-to-session-name mappings |
| `hbm honcho map <name>` | Map current working directory to a Honcho session name |
| `hbm honcho identity` | Seed AI peer identity or show both peer representations |
| `hbm honcho sync` | Create host blocks for all HBM AGENT profiles that don't have one yet |
| `hbm honcho migrate` | Step-by-step migration guide from OpenClaw native memory to HBM AGENT + Honcho |
| `hbm memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `hbm memory status` | Show active memory provider and config |
| `hbm memory off` | Disable external memory provider |
