...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), HBM AGENT |
| License | MIT |
...
The script reads `${HBM_HOME:-~/.hbm}/.env` for two optional defaults:
...
Helper script: `~/.hbm/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.hbm/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${HBM_HOME:-~/.hbm}/.env`.
...
```bash
python ~/.hbm/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.hbm/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
