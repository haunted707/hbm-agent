...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by HBM AGENT |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during HBM AGENT terminal calls (desktop app flow only)
...
### Service Account (recommended for HBM AGENT)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HBM_HOME:-~/.hbm}/.env` (the skill will prompt for this on first load).
...
## HBM AGENT Execution Pattern (desktop app flow)

HBM AGENT terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/hbm-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hbm-op.sock"
