...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/creative/ip-as-logo` |
| Path | `optional-skills/creative/ip-as-logo` |
| Version | `1.0.0` |
| Author | s1dashu (https://github.com/s1dashu, upstream s1dashu/ip-as-logo-skill), ported by HBM AGENT |
| License | MIT |
...
> **HBM AGENT adaptation notes** (the rest of this document is the upstream
...
- An image-generation backend configured for the built-in `image_generate` tool; check with `hbm tools` and enable one there if the tool reports no backend.
...
8. Determine the available image-generation path before promising output. In HBM AGENT this is the `image_generate` tool; if it reports no configured backend, ask the user to enable one (`hbm tools`) instead of fabricating results.
