...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the HBM AGENT MCP catalog. The
...
Two halves, in this order: the editor side must be up before HBM AGENT connects.
...
### One-time, HBM AGENT side

    hbm mcp install unreal-engine
...
`~/.hbm/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for HBM AGENT — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. HBM AGENT connects
...
2. Start the HBM AGENT session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through HBM AGENT
they appear as:

| HBM AGENT tool | Purpose |
|---|---|
...
then happens at `hbm mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — HBM AGENT runs batched calls
...
- **Start order matters.** Editor + server up first, then the HBM AGENT
...
- **Timeouts on long operations.** HBM AGENT's per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.hbm/config.yaml` for
