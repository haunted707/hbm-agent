...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/creative/pretext` |
...
This skill exists so HBM AGENT can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
