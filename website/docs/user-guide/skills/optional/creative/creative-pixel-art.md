...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.hbm/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.hbm/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — HBM AGENT installs package this)
