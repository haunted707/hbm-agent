...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/creative/hyperframes` |
...
```bash
bash "$(dirname "$(find ~/.hbm/skills -path '*/hyperframes/SKILL.md' 2>/dev/null | head -1)")/scripts/setup.sh"
```
