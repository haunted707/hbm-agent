...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in an HBM AGENT Kanban pipeline that
...
3. **Generates** a setup script that creates HBM AGENT profiles, project workspace, and the initial kanban task
...
For mapping role → which HBM AGENT skills + toolsets it loads, see
...
3. Creates each HBM AGENT profile via `hbm profile create --clone`
...
7. Fires the initial `hbm kanban create` task assigned to the director
...
```bash
hbm kanban watch --tenant <project-tenant>     # live events
hbm kanban list  --tenant <project-tenant>     # board snapshot
hbm dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `hbm kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${HBM_HOME:-~/.hbm}/.env` or the user's secret store.
