...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/productivity/canvas` |
...
4. Name the token (e.g., "HBM AGENT"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${HBM_HOME:-~/.hbm}/.env`:
...
```bash
CANVAS="python $HBM_HOME/skills/productivity/canvas/scripts/canvas_api.py"
