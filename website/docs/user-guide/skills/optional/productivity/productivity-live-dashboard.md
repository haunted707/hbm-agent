...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/productivity/live-dashboard` |
...
- Optional: the `desktop_preview` tool (HBM AGENT desktop app sessions). When it is in the toolset, dashboards render in the in-app preview pane; otherwise the user is given the file path.
...
Write two files under the HBM AGENT home directory's `dashboards/<slug>/` (the same directory that holds `config.yaml`; never assume a fixed location):
...
- Hardcoding the HBM AGENT home path — resolve it from the running install (the directory holding `config.yaml`) and write absolute paths into cron prompts.
