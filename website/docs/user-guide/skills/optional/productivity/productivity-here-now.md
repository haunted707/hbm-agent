...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${HBM_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HBM_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${HBM_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client hbm
```
...
```bash
PUBLISH="${HBM_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client hbm
```
...
```bash
DRIVE="${HBM_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `hbm`)    |
