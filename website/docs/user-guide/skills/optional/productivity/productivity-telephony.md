...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/productivity/telephony` |
...
This optional skill gives HBM AGENT practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${HBM_HOME:-~/.hbm}/.env`
...
It does **not** turn HBM AGENT into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to HBM AGENT memory
...
### 1) "I want HBM AGENT to own a real phone number"
Use **Twilio**.
...
- pairs well with HBM AGENT `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.hbm/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
hbm skills search telephony
hbm skills install official/productivity/telephony
```
...
Then save credentials into HBM AGENT:
...
3. Buy it and save it into `${HBM_HOME:-~/.hbm}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is HBM AGENT calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing HBM AGENT's existing `text_to_speech` support.

Use this when:
- you want the call to use HBM AGENT's configured TTS voice rather than Twilio `<Say>`
...
Recommended HBM AGENT TTS -> Twilio Play workflow:

1. Generate the audio with HBM AGENT `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask HBM AGENT to create the message audio with `text_to_speech`
...
Important note:
- HBM AGENT TTS is great for prerecorded outbound messages
...
- HBM AGENT STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to HBM AGENT memory.
...
- Do not store arbitrary third-party phone numbers in HBM AGENT memory.
...
3. persist that number to `${HBM_HOME:-~/.hbm}/.env`
