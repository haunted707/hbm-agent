---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into HBM AGENT"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into HBM AGENT"
---
...
Import an OpenClaw setup (memories, skills) into HBM AGENT.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hbm skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | HBM AGENT (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `HBM AGENT`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> HBM AGENT Migration

Use this skill when a user wants to move their OpenClaw setup into HBM AGENT with minimal manual cleanup.
...
```bash
hbm claw migrate              # Full interactive migration
...
hbm claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `hbm setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hbm.py` to:

- import `SOUL.md` into the HBM AGENT home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into HBM AGENT memory entries
- merge OpenClaw command approval patterns into HBM AGENT `command_allowlist`
- migrate HBM AGENT-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to HBM AGENT working-directory configuration
- copy OpenClaw skills into `~/.hbm/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen HBM AGENT workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hbm/tts/`
- archive non-secret docs that do not have a direct HBM AGENT destination
...
- `scripts/openclaw_to_hbm.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hbm/skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py`

Do not guess a shorter path like `~/.hbm/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.hbm/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of HBM AGENT-compatible secrets, currently:
...
HBM AGENT CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in HBM AGENT. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into an HBM AGENT workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing HBM AGENT skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.hbm/skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.hbm/skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.hbm/skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.hbm/skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.hbm/skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.hbm/skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of HBM AGENT unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty HBM AGENT targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean HBM AGENT destination. Unsupported auth blobs must still be reported as skipped.
...
- HBM AGENT persona state imported
- HBM AGENT memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.hbm/skills/openclaw-imports/`
