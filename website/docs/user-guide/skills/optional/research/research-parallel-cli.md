...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not an HBM AGENT core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with HBM AGENT native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer HBM AGENT native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer HBM AGENT native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended HBM AGENT workflow:
...
## Recommended HBM AGENT usage patterns
...
- Do not silently choose Parallel when HBM AGENT native tools are already sufficient.
