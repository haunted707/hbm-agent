...
| | |
|---|---|
| Source | Optional — install with `hbm skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), HBM AGENT |
| License | MIT |
...
import upstream classes into HBM AGENT itself.
...
```bash
mkdir -p ~/.hbm/cache/darwinian-evolver && cd ~/.hbm/cache/darwinian-evolver
...
```bash
cd ~/.hbm/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.hbm/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.hbm/skills/research/darwinian-evolver
DE_DIR=~/.hbm/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside HBM AGENT core.
   Custom driver scripts under `~/.hbm/skills/...` are user-side and fine.
