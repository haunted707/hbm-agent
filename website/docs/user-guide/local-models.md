...
HBM AGENT can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; HBM AGENT does the rest.
...
2. Click **Install runtime**. HBM AGENT downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with HBM AGENT, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How HBM AGENT picks what to download
...
Models ship in several quality grades (quantizations). HBM AGENT picks the
...
guarantees. Below 4-bit the quality loss is too severe, so HBM AGENT never
...
Local models live or die by memory placement, so HBM AGENT manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, HBM AGENT deliberately places the
...
- **Memory fit includes the launch configuration**, not just the model file:
  ...
  count. For multi-token prediction (MTP), HBM AGENT uses smaller batches when
...
- **Conversation compression follows a growth check.** If a larger window
  ...
  HBM AGENT compresses instead of claiming a window the server did not receive.
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — HBM AGENT
...
If a llama-server is already running on your machine, HBM AGENT detects it
...
[Run HBM AGENT Locally with Ollama](/guides/local-ollama-setup) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with HBM AGENT.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; HBM AGENT updates it with
                     # each release after re-validation
```

Models and runtime builds live under the HBM AGENT home directory
