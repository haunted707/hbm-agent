...
# Connecting Desktop to Many HBM AGENT Instances

Register every HBM AGENT backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and HBM AGENT Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another HBM AGENT gateway…"**) deep-links straight to
...
list of HBM AGENT gateways. Its intro says it plainly: *"Manage this device and
every HBM AGENT gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The HBM AGENT runtime managed by this app." | automatic |
| **Remote gateway** | "An HBM AGENT gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "An HBM AGENT install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **HBM AGENT Cloud** | "A hosted instance discovered through your HBM AGENT Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the HBM AGENT Cloud sign-in/discovery flow at
  the top of the Gateways page — the **HBM AGENT Cloud** kind in the add-connection
...
The HBM AGENT Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **HBM AGENT Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **HBM AGENT Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `hbm serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated HBM AGENTs
...
connection is registered) dispatches `hbm update` to every eligible
...
- **HBM AGENT Cloud** instances are skipped with a *"Managed by HBM AGENT Cloud"*
...
panel, ⌘K **Update HBM AGENT**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `hbm serve` is running on the remote host, the
...
- **"Update HBM AGENT Desktop to chat with agents on other connections"** — the
