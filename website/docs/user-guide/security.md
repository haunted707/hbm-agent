...
HBM AGENT is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, HBM AGENT checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.hbm/config.yaml`:
...
| `timeout` | `300` | Seconds HBM AGENT waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `HBM_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `hbm --yolo` or `hbm chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `HBM_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `HBM_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, HBM AGENT shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
the job uses KeepAlive or controls HBM AGENT.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that HBM AGENT refuses to run them **regardless** of:
...
Configure the timeout in `~/.hbm/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.hbm/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.hbm/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` hbm/gateway | Self-termination prevention |
...
The `HBM_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.hbm/config.yaml`:
...
unattended ones: a cron job, `hbm chat -q` run or webhook session under
...
re-save it with `hbm config edit`. Other malformed values are ignored with
a warning; they never become per-character approvals. Loading does not rewrite
your configuration file.

:::tip
Use `hbm config edit` to review or remove patterns from your permanent allowlist.
:::

:::caution
The list is read when HBM AGENT starts. A pattern you remove while a session is
already running stays approved in that session until it next writes the file
(the next time you answer `always` to a prompt) or you restart HBM AGENT. If you
removed it for safety reasons, restart.
:::

### Mining Approval History (`hbm approvals suggest`)
...
```bash
hbm approvals suggest            # dry run — prints a numbered proposal
hbm approvals suggest --apply 1,3  # merge picks into command_allowlist
hbm approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.hbm/state.db`) for
...
Before `write_file` or `patch` touches disk, HBM AGENT checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `HBM_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| HBM AGENT secret stores | `.env`, `.anthropic_oauth.json`, `auth/google_oauth.json`, Bitwarden cache (`cache/bws_cache.json`, `cache/bws_cache.enc.json`), `vault/`, `browser-profile/`, `mcp-tokens/`, `pairing/` under HBM_HOME (active profile and global root). Control files (`auth.json`, `config.yaml`, `webhook_subscriptions.json`) are read-denied but stay writable. |
...
Sensitive paths inside the safe root are still blocked — pointing `HBM_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

Safe-root violations return `Write denied: '…' is outside HBM_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### HBM_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/NousResearch/hermes-agent) (`HBM_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.hbm/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.hbm/cron/jobs.json`, profile skills, or other HBM AGENT state outside that prefix

To allow both a workspace and HBM AGENT home:

```bash
export HBM_WRITE_SAFE_ROOT=/path/to/project:/home/you/.hbm
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [HBM_WRITE_SAFE_ROOT](../reference/environment-variables.md#hbm_write_safe_root).

### Cron and other HBM AGENT state

Do not ask the agent to `patch` `~/.hbm/cron/jobs.json` directly. Use the `cronjob` tool, [`hbm cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other HBM AGENT control files when write safety blocks direct edits.
...
When running the messaging gateway, HBM AGENT controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.hbm/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.hbm/.env to allow open access,
...
For more flexible authorization, HBM AGENT includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `hbm pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.hbm/config.yaml`:
...
hbm pairing approve telegram ABC12DEF

# Revoke a user's access
hbm pairing revoke telegram 123456789

# Clear all pending codes
hbm pairing clear-pending
```

:::tip Docker users: run pairing commands as the `hbm` user
The official Docker image runs the gateway as the unprivileged `hbm` user
...
**Storage:** Pairing data is stored in `~/.hbm/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, HBM AGENT applies strict security hardening to every container.
...
Container resources are configurable in `~/.hbm/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.hbm/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `HBM_HOME`. Skills declare these in frontmatter:
...
When loaded, HBM AGENT checks if these files exist in the active profile's `HBM_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit HBM AGENT infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- HBM AGENT infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
HBM AGENT integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run HBM AGENT under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.hbm/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.hbm/logs/` for unauthorized access attempts
10. **Keep updated** — run `hbm update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.hbm/.env`:
...
TERMINAL_SSH_USER=hbm
TERMINAL_SSH_KEY=~/.ssh/hbm_agent_key
```
...
HBM AGENT ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hbm_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `hbm doctor` for the full remediation.
- **`hbm doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. HBM AGENT installs these **lazily** on first use rather than eagerly under `hbm-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `hbm tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `hbm tools`.
