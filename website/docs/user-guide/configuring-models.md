...
HBM AGENT uses two kinds of model slots:
...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `hbm setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `hbm portal info`.

- Portal subscribers also get **10% off token-billed providers**.
:::

:::note `model:` schema — empty string vs. mapping
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `hbm setup` or `hbm model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md). If you ever see an empty string in `config.yaml`, run `hbm model` (or click **Change** in the dashboard) and HBM AGENT will write the dict form for you.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models HBM AGENT recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and HBM AGENT writes it to `~/.hbm/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.

### Mid-session switches and context warnings

When you switch models **inside an active session** (Herm TUI model picker, `hbm` CLI, or `/model` on Telegram/Discord), HBM AGENT estimates whether your **next message** will run **preflight context compression** against the new model's window. If the session is already near or above that model's compression threshold (see [Context Compression](./configuration.md#context-compression)), the switch reply includes a warning — the same `warning_message` path used for expensive-model notices. The switch still applies immediately; compression runs on the **first user message after the switch**, before the model answers.
...
Because of that one-time re-read cost, HBM AGENT asks for **explicit confirmation** before applying a mid-session switch when the live session already holds a large context (default: **100,000 tokens**, measured from the latest provider-billed prompt size). The confirmation renders through the same selection-guard prompt as the expensive-model and data-training warnings wherever a live session is switching: the CLI and TUI `/model` command and picker, and a typed gateway `/model` in a chat with an active agent. Tune or disable it in `config.yaml`:
...
```bash
hbm config set security.allow_data_training_tiers_noninteractive true
```

HBM AGENT still prints the full data-policy warning and the acknowledgement key on every unattended startup, so worker logs retain an audit trail. This setting does not approve expensive-model or provider-routing warnings, and it does not replace the interactive confirmation prompt. Revoke it with `hbm config unset security.allow_data_training_tiers_noninteractive`.
...
Every auxiliary task defaults to `auto` — meaning HBM AGENT tries your main model for that job too. If that route is unavailable or hits a capacity-style failure, `auto` follows any task-specific `auxiliary.<task>.fallback_chain`, then the main `fallback_providers` / `fallback_model` chain. It never guesses a provider you did not configure: with a main provider selected and no fallback declared, the side task is skipped with a warning rather than billed to another account you happen to be logged into. (HBM AGENT's built-in discovery chain only runs when no main provider is selected at all.) Override a specific task when you want a cheaper or faster model for a side-job.

### Common override patterns

| Task | When to override |
|---|---|
| **Title Gen** | When title latency or cost matters more than matching the main model. Pin a known-good flash model, or set `auxiliary.title_generation.prefer_fast_model: true` to let HBM AGENT choose the provider's fast tier. |
...
| **Skills Hub** | `hbm skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`hbm kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`hbm profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
When you save via the dashboard, HBM AGENT writes to `~/.hbm/config.yaml`:
...
`provider: auto` with `model: ''` tells HBM AGENT to use the main model for that task, while still honoring fallback policy if the main route cannot serve the auxiliary call.
...
When `fallback_chain` is absent, `auto` uses the top-level `fallback_providers` chain. If that is also absent and the main provider cannot serve the call, the task is skipped with a warning — HBM AGENT does not fall through to other logged-in providers.

## Per-provider request options

Provider entries (`providers.<name>` in the `providers:` dict, or items in the legacy `custom_providers` list) accept knobs that shape how HBM AGENT talks to the endpoint:
...
Header values routinely carry credentials — HBM AGENT never logs them. `extra_headers` applies to OpenAI-compatible routes; the `anthropic_messages` and `bedrock_converse` API modes do not use it.
...
With discovery off, the model picker (`hbm model`, `/model`) shows the configured list instead of a live probe.
...
HBM AGENT matches this declaration to the exact provider route and runtime model
...
omitted, HBM AGENT keeps its normal provider and model capability detection.

:::note Legacy format
Older configs used a top-level `custom_providers:` list (with `base_url` instead of `api`). It still works and is auto-migrated to the `providers:` dict on `hbm update` (config v12).
:::
...
`auto` is for when the Portal serves the same model from more than one upstream. A session starts on chat, HBM AGENT reads which upstream answered the first call, and switches that session to native only when the upstream is one where native is known to be clean (the switch happens between calls, so no in-flight response and no warm cache is lost). Today no upstream is cleared, so `auto` behaves exactly like `chat`; it exists so the flip can be made from a measurement rather than a config change.

## When does it take effect?

- **CLI** (`hbm chat`): next `hbm chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`hbm gateway restart`) if you want to force all sessions to pick up the change.
...
HBM AGENT lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `hbm setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but HBM AGENT switched providers on me
...
Define your own short names for models you reach for often, then use `/model <alias>` in a running session or `hbm chat --model <alias>` at startup. There are two equivalent formats — pick whichever fits your workflow.
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `hbm config set` writes scalars and now also parses inline list/mapping literals, though this short alias form still can't carry a custom `base_url`:

```bash
hbm config set model.aliases.fav anthropic/claude-opus-4.6
hbm config set model.aliases.grok x-ai/grok-4
```

> `hbm config set` also accepts inline **list/mapping literals** (JSON/YAML flow style). Quote them so your shell passes them through intact:
>
> ```bash
> hbm config set platform_toolsets.line '["clarify", "file", "web"]'
> hbm config set display.tool_progress_overrides '{"terminal": "off"}'
> ```

Both paths feed the same loader (`hbm_cli/model_switch.py`). Entries declared in `model_aliases:` take precedence over `model.aliases:` entries with the same name.
...
### `hbm model` subcommand

```bash
hbm model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`hbm model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.default` in `~/.hbm/config.yaml`. After a new model is saved, a reasoning-effort step follows (`minimal` … `ultra`, **Disable reasoning**, or **Skip** to keep the current value) and writes `agent.reasoning_effort`; the step is skipped for models the catalog marks as having no reasoning control. The provider list also has a **Reasoning effort for the current model...** row to change only the effort.
...
To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `hbm config get model --json` and `hbm status`.

### Direct config edit

Edit `~/.hbm/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-HBM AGENT-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-HBM AGENT-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-HBM AGENT-Session-Token: $TOKEN" \
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__HBM_SESSION_TOKEN__`) if you're scripting against a running dashboard.
