...
Run multiple independent HBM AGENTs on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate HBM AGENT home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up HBM AGENT state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same HBM AGENT home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
```bash
hbm profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own HBM AGENT profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `hbm setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
hbm profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `hbm profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies **everything** — config, API keys, personality, all memories, skills, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. When cloning from the default profile, the local-model runtime trees (`models/`, `runtimes/`, `node/` — downloaded weights and managed binaries, re-fetched on demand) are skipped too, as `hbm backup` already does. **Cron jobs are not cloned** either: they are scheduled work bound to the source profile and its delivery channel, and a clone that inherited them would run every job twice (two gateways, same job ids). The new profile starts with an empty `cron/`. For a full backup including history and cron jobs, use `hbm profile export` or `hbm backup` instead.

:::note OAuth logins are shared, not copied
Anthropic (Claude Pro/Max), OpenAI Codex, and xAI OAuth logins use **single-use refresh tokens** — a copy of one is not a second credential, it is the same credential with two owners, and the first profile to refresh it revokes it for every other copy. `--clone-all` (and the dashboard's credential mirroring) therefore drops those OAuth rows from the clone. The new profile keeps reading the login from the root `~/.hbm/auth.json`, and a token refresh performed inside any profile is written back to root, so all profiles stay signed in. Static API keys are copied as usual. To give a profile its own separate OAuth login, run `hbm -p <name> auth add <provider>` inside it.
:::

### Clone from a specific profile

```bash
hbm profile create work --clone-from coder
```
...
```bash
hbm profile create work-backup --clone-from coder --clone-all
```
...
parks the duplicate adapter (and `hbm gateway migrate --multiplex` refuses with one
duplicate-credential blocker per platform). Configure the new profile's own bots with
`hbm -p <name> setup` or the dashboard Messaging page.

```bash
hbm profile create twin --clone --clone-channels   # keep the source's bots anyway
```
...
without a clone flag. `hbm profile list` prints the same warning for any existing
...
The alias works with every hbm subcommand — it's just `hbm -p <name>` under the hood.
...
```bash
hbm -p coder chat
hbm --profile=coder doctor
hbm chat -p coder -q "hello"    # works in any position
```

### Sticky default (`hbm profile use`)

```bash
hbm profile use coder
hbm chat                   # now targets coder
hbm tools                  # configures coder's tools
hbm profile use default    # switch back
```

Sets a default so plain `hbm` commands target that profile. Like `kubectl config use-context`.
...
- **`hbm profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives HBM AGENT its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory HBM AGENT was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.hbm/profiles/coder/.env

# Edit assistant's tokens
nano ~/.hbm/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates hbm-gateway-coder systemd/launchd service
assistant gateway install     # creates hbm-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `hbm profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `hbm -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.hbm/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `hbm profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`hbm update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
hbm profile list           # show all profiles with status
hbm profile show coder     # detailed info for one profile
hbm profile rename coder dev-bot   # rename (updates alias + service)
hbm profile migrate-identity coder dev-bot   # retry a rename's identity migration
hbm profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
hbm profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.hbm` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
hbm profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `hbm profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.hbm/profile.yaml`; remove that line to
...
```bash
hbm profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `hbm profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.hbm`). To remove everything, use `hbm uninstall`.
:::
...
Profiles use the `HBM_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HBM_HOME=~/.hbm/profiles/coder` before launching hbm. Since 119+ files in the codebase resolve paths via `get_hbm_home()`, HBM AGENT state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `HBM_HOME`.
...
isolated by `HBM_HOME`, not by changing `HOME`. Container backends still use
`{HBM_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `HBM_HOME` is the profile boundary. It controls HBM AGENT config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other HBM AGENT
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, HBM AGENT keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode HBM AGENT launches tool
subprocesses with `HOME={HBM_HOME}/home`; you then need to initialize or link
...
HBM AGENT also exposes `HBM_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.hbm` itself. No migration needed — existing installs work identically.
...
hbm profile export coder
hbm profile import ./coder.tar.gz --name coder
```
...
hbm profile install github.com/you/research-bot --alias

# Update later when the author ships a new version (keeps your memories + .env)
hbm profile update research-bot
```
