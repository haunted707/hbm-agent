...
title: "Git Worktrees"
description: "Run multiple HBM AGENTs safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

HBM AGENT is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with HBM AGENT so each session has a clean, isolated working directory.

## Why Use Worktrees with HBM AGENT?

HBM AGENT treats the **current working directory** as the project root:

- CLI: the directory where you run `hbm` or `hbm chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.hbm/config.yaml`
...
HBM AGENT creates `.worktrees/my-experiment/` inside the repo (branch
`hbm/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `hbm-<id>`
...
git worktree add ../repo-feature feature/hbm-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/hbm-experiment` checked out in that directory

Now you can `cd` into the new worktree and run HBM AGENT there:

```bash
cd ../repo-feature

# Start HBM AGENT in the worktree
hbm
```
...
git worktree add ../repo-experiment-a feature/hbm-a
git worktree add ../repo-experiment-b feature/hbm-b
```
...
Each HBM AGENT process:

- Works on its own branch (`feature/hbm-a` vs `feature/hbm-b`).
...
- HBM AGENT checkpoint data under `~/.hbm/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per HBM AGENT experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/hbm-checkpoints-docs`, `feature/hbm-refactor-tests`.
- **Commit frequently**
...
- **Avoid running HBM AGENT from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `hbm -w` (Automatic Worktree Mode)

HBM AGENT has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `hbm/hbm-<hash>`).
...
```bash
hbm -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `hbm -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each HBM AGENT session its own clean checkout.
