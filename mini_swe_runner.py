#!/usr/bin/env python3
"""SWE Runner with HBM AGENT Trajectory Format

Runs tool-calling agent tasks in HBM AGENT-Agent's execution environments (local,
docker, modal) and writes trajectories in HBM AGENT format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a HBM AGENT execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a HBM AGENT execution environment, emitting HBM AGENT trajectories."""
    ...
    def _convert_to_hbm_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the HBM AGENT trajectory format used by batch_runner.py."""
        system_msg = HBM_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + HBM_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_hbm_format(messages, task), "completed": completed, "api_calls": api_call_count,
...
def main(
    ...
    """
    Run SWE tasks with HBM AGENT trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with HBM AGENT Trajectory Format")
    print("=" * 60)
