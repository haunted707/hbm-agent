...
def child(out: Path) -> None:
    ...
    runner.session_store = SessionStore(Path(os.environ["HBM_HOME"]) / "sessions", runner.config)
    ...
    cli = HbmCLI(model="fixture", provider="openai-compat", api_key="fixture", base_url=base_url)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="context-display-") as home:
        ...
        if pid == 0:
            os.environ.clear()
            os.environ.update(HOME=home, HBM_HOME=home + "/.hbm", PATH="/usr/bin:/bin", TERM="dumb", NO_COLOR="1")
            try:
                child(out)
            finally:
                sys.stdout.flush()
