#!/usr/bin/env python3
"""Fan-out resource benchmark for hbm-agent.
...
#   turn 1: call write_file on <its worktree>/hbm_cli/bench_<i>.py
...
def _snap(pid: int, db_path: str) -> dict:
    ...
    return {
        ...
        "tcp": int(tcp or 0), "pyright": kids.count("pyright"), "kernels": kids.count("hbm_kernel_runner"),
...
def main() -> None:
    ...
    home = tempfile.mkdtemp(prefix="hbm_bench_home_")
    os.environ["HBM_HOME"] = home
    os.environ["TERMINAL_ENV"] = "local"
    os.environ.pop("OPENROUTER_API_KEY", None)
    sys.path.insert(0, a.repo)
    os.chdir(a.repo)
    pyright = shutil.which("pyright-langserver", path=os.path.expanduser("~/.hbm/lsp/bin") + os.pathsep + os.environ.get("PATH", ""))
    ...
    wts = []
    base = tempfile.mkdtemp(prefix="hbm_bench_wt_")
    for w in range(a.worktrees):
        d = os.path.join(base, f"wt{w}")
        os.makedirs(os.path.join(d, "hbm_cli"))
        subprocess.run(["git", "init", "-q", d], check=True)
        open(os.path.join(d, "pyproject.toml"), "w", encoding="utf-8").write("[project]\nname='b'\n")
        open(os.path.join(d, "hbm_cli", "__init__.py"), "w", encoding="utf-8").write("")
        wts.append(d)
    ...
    tasks = [{"goal": json.dumps({"file": os.path.join(wts[i % len(wts)], "hbm_cli", f"bench_{i}.py")}),
