...
test.beforeAll(async () => {
  ...
  for (const name of ['default', 'alpha', 'beta']) {
    const home = name === 'default' ? sandbox.hbmHome : path.join(sandbox.hbmHome, 'profiles', name)
  ...
  fs.mkdirSync(bin)
  fs.writeFileSync(path.join(bin, 'hbm'), `#!/bin/sh\ncd ${repo}\nprintf '%s\\n' "$$ $HBM_HOME $*" >> ${evidence}/children.log\nexec ${python} -m hbm_cli.main "$@"\n`, { mode: 0o755 })
  env = buildAppEnv(sandbox, { HOME: sandbox.root, HBM_DESKTOP_PYTHON: python,
    HBM_DESKTOP_HBM: path.join(bin, 'hbm'), PATH: `${bin}:${process.env.PATH}`,
    PYTHONPATH: repo, HBM_SINGLE_QUERY_LINGER_SECONDS: '1' })
...
test('ordinary cron pins an unowned named target under a custom root', async () => {
  ...
  const page = fixture.page
  const composer = await openBot('HBM AGENT')
  ...
  const discovery = 'from pathlib import Path; import json,os; from tools.bot_live_delivery import find_canonical_owner; h=Path(os.environ["HBM_HOME"]); print(json.dumps({"default":find_canonical_owner(h),"alpha":find_canonical_owner(h/"profiles"/"alpha")}))'
  ...
  try {
    ...
    expect(launched).toContain(path.join(fixture.sandbox.hbmHome, 'profiles', 'alpha'))
    expect(held.default.lease_id).toBe(before.default.lease_id)
    expect(fs.existsSync(path.join(fixture.sandbox.hbmHome, 'cron', 'bot_chat_pending'))).toBe(false)
    ...
    const rows = probe('import sqlite3,json,os; from pathlib import Path; h=Path(os.environ["HBM_HOME"]); print(json.dumps({n:sqlite3.connect(h/"state.db" if n=="default" else h/"profiles"/n/"state.db").execute("select session_id,role,content from messages").fetchall() for n in ["default","alpha"]}))')
    ...
    const missing = probe('import json,os; from pathlib import Path; from cron.scheduler_delivery import _deliver_to_bot_chat; h=Path(os.environ["HBM_HOME"]); p=h/"profiles"/"beta"; p.rename(h/"profiles"/"beta-removed"); result=_deliver_to_bot_chat({"id":"removed","execution_id":"missing"},"MUST_NOT_RUN","beta"); print(json.dumps({"result":result,"recreated":p.exists(),"wrong_root":(Path.home()/".hbm").exists()}))')
