...
test('cron output waits for a CLI-only owner and arrives after owner release', async () => {
  ...
  const child = spawn(python, ['-m', 'hbm_cli.main', '-p', 'beta', 'chat', '--in', '~', '-c', 'Bot Chat', '--create-if-missing', '-Q', '-q', 'CLI_OWNER_HOLD'], { cwd: repo, env, stdio: ['ignore', output, output] })
  const cronEnv = { ...env, HBM_HOME: fixture.sandbox.hbmHome }
  try {
    ...
    const setup = process.env.BOT_DM_EXCEPTION ? 'from pathlib import Path; from cron.bot_chat_delivery import defer; from hbm_constants import get_hbm_home; defer("e"*64,{"id":"exception-head"},"EXCEPTION_MUST_NOT_RUN","beta",get_hbm_home()/"profiles"/"beta"); ' : ''
    ...
    if (process.env.BOT_DM_CORRUPT) fs.writeFileSync(path.join(fixture.sandbox.hbmHome, 'cron', 'bot_chat_pending', 'broken.json'), '{')
...
test('named Bot Chat receives a nested one-shot message_agent delivery once', async () => {
  ...
  const child = spawn(python, ['-m', 'hbm_cli.main', '-p', 'beta', 'chat', '--in', '~', '-c', 'Bot Chat', '--create-if-missing', '-Q', '-q', 'E2E_DM(alpha)[nested-one-shot-sentinel]'], { cwd: repo, env, stdio: ['ignore', output, output] })
  try {
