...
Real Electron, production Python backend and tool execution, disposable HOME/HBM_HOME,
...
(cd apps/desktop && DISPLAY=:0 XAUTHORITY=/run/user/1000/xauth_cnpsqU \
  ...
  HBM_DESKTOP_CDP_PORT=off npx playwright test e2e/probe-dm-delivery.spec.ts --reporter=list)
...
fixture's generated hbm shim pins every child to this checkout, not an installed launcher.
...
custom HBM AGENT root, and holds the real quiet CLI child at loopback inference.
...
creates a second `.hbm` root under HOME.
...
actual Desktop label is HBM AGENT. No production failure is claimed for that attempt.
