...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, HBM_HOME=home, PATH="/usr/bin:/bin", HBM_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["hbm-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["hbm-cli"], quiet_mode=True)
