#!/usr/bin/env python3
"""Local A/B harness for `hbm auth add openrouter --type oauth` against a FAKE OpenRouter.

Runs the REAL entry point (``hbm_cli.auth_commands.auth_add_command``) with a temp HBM_HOME.
...
Scenarios (each prints PASS/FAIL, exit code = number of failures):
  ...
  api_key_path   `hbm auth add openrouter --api-key` still works with no --type (regression)

Usage: HBM_PYTHON=<venv python> python3 evals/openrouter_pkce_ab/harness.py [--json OUT]
...
def run(scenario: str, fake: FakeOpenRouter, home: str) -> dict:
    ...
    for m in [m for m in sys.modules if m.startswith(("hbm_cli", "agent", "hbm_constants"))]:
        del sys.modules[m]
    ...
    try:  # BEFORE (origin/main) has no auth_openrouter sibling; the flow itself must then fail.
        ...
        import hbm_cli.auth_device_flow as dfl
