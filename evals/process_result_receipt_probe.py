...
from hbm_cli.oneshot import _linger_for_background_completions
...
env = {key: value for key, value in os.environ.items()
       if not key.startswith(('HBM_', 'OPENAI_', 'ANTHROPIC_', 'TERMINAL_'))
       and not key.endswith(('_API_KEY', '_TOKEN', '_SECRET'))}
env.update(HOME=str(out), HBM_HOME=str(out / 'profile'), PYTHONPATH=str(repo),
           TERMINAL_CWD=str(out), PYTHONDONTWRITEBYTECODE='1')

def run(code, *args, profile=None):
    child_env = dict(env)
    if profile:
        child_env['HBM_HOME'] = str(out / profile)
