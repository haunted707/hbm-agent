...
for key in list(os.environ):
    if key.startswith("HBM_") or key.endswith(("_API_KEY", "_TOKEN")):
        os.environ.pop(key, None)
os.environ.update(HOME=str(home), HBM_HOME=str(home / ".hbm"), SESSION_IDLE_MINUTES="1", SESSION_RESET_HOUR="0")
...
Path(os.environ["HBM_HOME"]).mkdir(exist_ok=True)
Path(os.environ["HBM_HOME"], "config.yaml").write_text(yaml.safe_dump({"session_reset": {"mode": "both", "idle_minutes": 1}, "gateway": {"session_reset": {"mode": "daily", "at_hour": 0}}}), encoding="utf-8")
...
spec = importlib.util.spec_from_file_location("migration_probe", repo / "optional-skills/migration/openclaw-migration/scripts/openclaw_to_hbm.py")
