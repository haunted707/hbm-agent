...
Run with the HBM AGENT venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by HBM AGENT children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    from hbm_cli.model_switch_providers import (
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "hbm_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="hbm-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".hbm"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "HBM_HOME": str(state),
