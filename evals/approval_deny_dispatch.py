...
with tempfile.TemporaryDirectory(prefix="hbm-deny-dispatch-") as directory:
    home = Path(directory)
    os.environ.clear()
    # The terminal's own NOPASSWD probe must also resolve to our inert fixture.
    os.environ.update(PATH=directory + ":/usr/bin:/bin", HOME=directory, HBM_HOME=directory,
                      HBM_INTERACTIVE="1", TERMINAL_ENV="local")
