...
ctx keys: workspace, hbm_home, events (desktop emit records),
...
def _grep_home(ctx, needle):
    """Binary-safe grep of the entire HBM_HOME tree (sqlite included)."""
    nb = needle.encode()
    for root, _dirs, files in os.walk(ctx["hbm_home"]):
        for fn in files:
            try:
