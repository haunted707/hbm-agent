...
async def run(repo: str, out: str):
    ...
    for m in list(sys.modules):
        if m.startswith(("gateway", "plugins", "tools", "hbm", "agent")):
            del sys.modules[m]
    ...
    r1 = await adapter.send_native_task_card_progress(
        ...
        metadata=metadata, fallback_text="HBM AGENT is working\n- terminal - ls - running")
...
def main():
    ...
    a = ap.parse_args()
    os.environ.setdefault("HBM_HOME", os.path.join(os.path.dirname(a.out), "hbm-home-probe"))
    os.makedirs(os.environ["HBM_HOME"], exist_ok=True)
