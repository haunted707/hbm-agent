"""Run the read-tool eval through the REAL HBM AGENT AIAgent.

For each task: fresh temp HBM_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    hbm_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".hbm"
    ...
    try:
        ...
        shutil.rmtree(hbm_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.hbm/.env; set +a  — then relaunch."
        )
