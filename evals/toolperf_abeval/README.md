...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  HBM AGENT home, same model, same tasks, same reps.
...
1. Create a dedicated HBM AGENT home with credentials for the models under test:
   ...
   integration at it; no HBM AGENT observability plugin needs to be enabled.
