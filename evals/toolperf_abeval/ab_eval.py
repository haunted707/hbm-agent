...
measured in the production session DB) through `hbm chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    HBM_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured HBM AGENT home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "HBM_HOME": str(HOME),
                "HBM_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "hbm_cli.main", "chat", "--query", q,
