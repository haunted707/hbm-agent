"""Drive the real ``hbm fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from hbm_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="hbm_test_fallback_") as home:
        hh = Path(home) / ".hbm"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "HBM_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "hbm_cli.main", "fallback", "add"],
