...
Run from the repo with a clean environment and a temporary HBM_HOME:
...
from hbm_cli import heartbeat  # noqa: E402
...
async def main(base_poller):
    assert os.environ.get("HBM_HOME"), "Use a temporary HBM_HOME"
    ...
    with patch.object(heartbeat, "time", SimpleNamespace(time=lambda: clock.now)):
        ...
        if not base_poller:
            ...
            runner.session_store = SessionStore(
                sessions_dir=Path(os.environ["HBM_HOME"]) / "sessions", config=runner.config)
