...
def backend(port):
    from hbm_cli.web_server import app, start_server
...
''')
    ...
    (home / 'user').mkdir()
    env.update(HBM_HOME=str(home), HBM_DASHBOARD_SESSION_TOKEN='lease-probe',
    ...
    def control(port, action, sid):
        req = urllib.request.Request(f'http://127.0.0.1:{port}/lease-probe/{action}/{sid}', data=b'', method='POST', headers={'X-HBM AGENT-Session-Token': 'lease-probe'})
