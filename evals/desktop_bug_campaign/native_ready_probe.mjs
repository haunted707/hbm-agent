...
const home = mkdtempSync(join(tmpdir(), 'hbm-native-ready-'))
const token = randomUUID()
const env = {
  PATH: process.env.PATH, HOME: home, HBM_HOME: home,
  LANG: 'C.UTF-8', PYTHONUNBUFFERED: '1', HBM_NONINTERACTIVE: '1',
  HBM_DASHBOARD_SESSION_TOKEN: token, HBM_SERVE_HEADLESS: '1',
  HBM_PARENT_PID: String(process.pid),
}
const child = spawn(process.argv[2] || join(repo, '.venv/bin/python'),
  ['-m', 'hbm_cli.main', 'serve', '--host', '127.0.0.1', '--port', '0', '--isolated'],
...
try {
  ...
  for (let i = 0; i < 20; i++) {
    for (const path of ['/api/status', '/api/profiles']) {
      const response = await fetch(`http://127.0.0.1:${port}${path}`, { headers: { 'X-HBM AGENT-Session-Token': token } })
