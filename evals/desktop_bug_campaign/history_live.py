...
env.update(HOME=str(home), HBM_HOME=str(home), HBM_DASHBOARD_SESSION_TOKEN='history-fixture-token', PYTHONPATH=repo, HBM_NONINTERACTIVE='1')
cmd=[sys.executable, '-m', 'hbm_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(args.port), '--isolated']
...
try:
    for _ in range(120):
        try:
            req=urllib.request.Request(f'http://127.0.0.1:{args.port}/api/status',headers={'X-HBM AGENT-Token':'history-fixture-token'})
