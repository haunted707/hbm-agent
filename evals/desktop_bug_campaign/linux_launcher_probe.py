...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "HBM_HOME": str(root / "home/.hbm"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "HBM_NONINTERACTIVE": "1"}
    Path(env["HBM_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from hbm_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'hbm')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,hbm_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,hbm_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=HBM AGENT\n", b"Name=HBM AGENT custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["HBM_HOME"]) / "config.yaml"
