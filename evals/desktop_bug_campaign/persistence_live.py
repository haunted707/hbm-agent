...
def main():
    ...
    env = {k: v for k, v in os.environ.items() if not (k.startswith('HBM_') or 'API_KEY' in k or 'TOKEN' in k or k in ('PYTHONPATH', 'PYTEST_PLUGINS'))}
    env.update(HBM_HOME=str(home), HOME=str(home / 'os-home'), HBM_DASHBOARD_SESSION_TOKEN='persistence-fixture-token', HBM_IGNORE_RULES='1', PYTHONPATH=str(args.repo), OPENAI_API_KEY='local-fixture')
    log = (args.out / 'serve.log').open('w')
    cmd = [sys.executable, '-m', 'hbm_cli.main', 'serve', '--isolated', '--port', str(args.port)]
    if observe:
