...
env = {k: v for k, v in os.environ.items() if not (k.startswith('HBM_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), HBM_HOME=str(home), HBM_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', HBM_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'hbm_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
