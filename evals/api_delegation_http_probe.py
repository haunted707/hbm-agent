...
async def probe():
    ...
    db = SessionDB(db_path=Path(os.environ["HBM_HOME"]) / "state.db")
    ...
    def create_agent(**kwargs):
        ...
        def run(**turn):
            sid = get_session_env("HBM_SESSION_CHAT_ID", "")
            args = [sid]
    ...
    async with TestClient(TestServer(app)) as client:
        for stream in (False, True):
            for explicit in (False, True):
                ...
                if explicit:
                    headers["X-HBM AGENT-Session-Id"] = "parent"
                ...
                records.append({"stream": stream, "explicit": explicit, "status": response.status, "header": response.headers.get("X-HBM AGENT-Session-Id"), "runtime": captured[-1] if captured else None, "body": body[:120]})
        ...
        response = await client.post("/v1/chat/completions", headers={"Authorization": "Bearer fixture-api-key", "X-HBM AGENT-Session-Id": "parent"}, json={"messages": [{"role": "user", "content": "read result"}]})
        await response.read()
        ...
        for name, body, extra_headers in (
            ...
            ("declared_key", {}, {"X-HBM AGENT-Session-Key": "fixture-key"}),
        ):
            count = len(captured)
