...
os.environ.update(HOME=str(home), HBM_HOME=str(home/'hbm'), XDG_CONFIG_HOME=str(home/'config'), CODEX_HOME=str(home/'codex'))
for k in list(os.environ):
    if any(x in k for x in ('API_KEY','TOKEN','NOUS_','SECRET')): os.environ.pop(k, None)
(home/'hbm').mkdir()
(home/'hbm'/'config.yaml').write_text('nous:\n  keepalive_interval_seconds: 0\nmemory:\n  memory_enabled: false\n', encoding='utf-8')
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        data={'id':'local-probe','object':'chat.completion','created':int(time.time()),'model':'hbm-test','choices':[{'index':0,'message':{'role':'assistant','content':'local-only'},'finish_reason':'stop'}],'usage':{'prompt_tokens':1,'completion_tokens':1,'total_tokens':2}}
...
os.environ['HBM_SHARED_AUTH_DIR']=str(home/'shared')
...
def store(token):
    exp=claims(token)['exp']; state={'portal_base_url':'https://portal.nousresearch.com','inference_base_url':'https://inference-api.nousresearch.com/v1','client_id':'hbm-cli','token_type':'Bearer','scope':'inference:invoke','access_token':token,'refresh_token':'fixture-refresh-never-send','expires_at':datetime.fromtimestamp(exp,timezone.utc).isoformat(),'expires_in':3600,'agent_key':token,'agent_key_expires_at':datetime.fromtimestamp(exp,timezone.utc).isoformat()}
    (home/'hbm'/'auth.json').write_text(json.dumps({'version':1,'active_provider':'nous','providers':{'nous':state}}), encoding='utf-8')
results=[]
for case, own_sub, store_sub, ttl in [('same-account','account-A','account-A',30),('explicit-account','account-A','account-B',30),('far-from-expiry','account-A','account-B',3000)]:
    ...
    agent=AIAgent(api_key=own,base_url=url,provider='nous',model='hbm-test',quiet_mode=True,skip_context_files=True,skip_memory=True,enabled_toolsets=[])
    ...
    reply=client.chat.completions.create(model='hbm-test',messages=messages)
...
def worker(_):
    a=AIAgent(api_key=expired,base_url=url,provider='nous',model='hbm-test',quiet_mode=True,skip_context_files=True,skip_memory=True,enabled_toolsets=[])
    ...
    try:
        c.chat.completions.create(model='hbm-test',messages=messages)
        status=200
