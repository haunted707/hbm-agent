...
Runs N independent HBM AGENT ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated hbm-agent #104284 / #104421 and NousResearch/api#227 (2026-09-05/06,
...
Usage:
  ...
  providers: nous (Portal creds from HBM_HOME), openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real HBM AGENT request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="hbm-agent checkout to import from")
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what HBM AGENT would pick for the provider/model)")
...
os.environ.setdefault("HBM_HOME", os.path.expanduser("~/.hbm"))
import anthropic
