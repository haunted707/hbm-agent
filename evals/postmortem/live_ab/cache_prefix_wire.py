...
message prefix between call N and N+1. If HBM AGENT strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--hbm-home DIR]   (default HBM_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("HBM_HOME", os.path.expanduser("~/.hbm"))
if "--hbm-home" in sys.argv:
    os.environ["HBM_HOME"] = sys.argv[sys.argv.index("--hbm-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.hbm/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
