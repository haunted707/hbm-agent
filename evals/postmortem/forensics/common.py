"""Shared loaders for the post-mortem forensics: point at ANY HBM AGENT ``state.db`` (a copy, never the live
...
the recomputed dollars match what THAT HBM AGENT recorded, not an invoice.
...
class Run:
    ...
    def parser(cls, description: str = "") -> argparse.ArgumentParser:
        ...
        ap.add_argument("--db", required=True, help="path to a COPY of ~/.hbm/state.db")
