...
- **`forensics/`** reads a *copy* of an HBM AGENT `state.db` (plus rotated `agent.log*` and git) and
...
- A HBM AGENT checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.hbm/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while HBM AGENT is idle) and, optionally, the rotated `~/.hbm/logs/agent.log*`.
- For `--live` probes: real credentials in `HBM_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.hbm/logs/agent.log*"
...
HBM AGENT recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp HBM_HOME), main vs a branch or integration checkout:
...
Forensic lanes: five parallel HBM AGENT subagents (2026-09-04), rewritten here to take `--db`/`--root`
