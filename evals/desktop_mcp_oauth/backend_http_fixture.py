...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["HBM_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(HbmTokenStorage("positive", hbm_home=owner).get_tokens())
        ...
        override = set_hbm_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from hbm_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="hbm-oauth-http-") as temp:
            home = Path(temp)
            (home / ".hbm").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "HBM_HOME": str(home / ".hbm"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
