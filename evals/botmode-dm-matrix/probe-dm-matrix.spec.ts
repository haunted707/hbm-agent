...
test.beforeAll(async () => {
  ...
  env = buildAppEnv(sandbox, { HOME: sandbox.root, HBM_DESKTOP_PYTHON: python,
    ...
    PYTHONPATH: repo, HBM_SINGLE_QUERY_LINGER_SECONDS: '30' })
  for (const name of ['alpha', 'beta', 'gamma']) {
    const h = path.join(sandbox.hbmHome, 'profiles', name)
    execFileSync(python, ['-c', 'import sys; from pathlib import Path; from hbm_state import SessionDB; d=SessionDB(db_path=Path(sys.argv[1])/"state.db"); d.create_session("matrix-"+sys.argv[2],"cli",cwd=sys.argv[3]); d.set_session_title("matrix-"+sys.argv[2],"Bot Chat"); d.close()', h, name, sandbox.root], { env, cwd: repo })
  }
...
async function openBot(name: string) {
  ...
  const row = page.getByRole('button', { name: new RegExp(`^${name === 'default' ? 'hbm' : name}\\b`, 'i') }).filter({ visible: true }).first()
...
function dbMessages(name: string) {
  const home = name === 'default' ? fixture.sandbox.hbmHome : path.join(fixture.sandbox.hbmHome, 'profiles', name)
...
test('incoming live sender card is rendered before any reload', async () => {
  ...
  const child = spawn(python, ['-m', 'hbm_cli.main', '-p', 'beta', 'chat', '--in', '~', '-c', 'Bot Chat', '--create-if-missing', '-Q', '-q', 'E2E_DM(alpha)[matrix-live-sender-card]'], { cwd: repo, env, stdio: ['ignore', output, output] })
  try {
