...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), HBM_HOME=str(home / "hbm"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("hbm-gateway*", "hbm-serve*")] + [unit + ".service"]
...
unit = f"hbm-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
