#!/usr/bin/env python3
"""Live test harness for HBM AGENT's Tool Search feature.
...
# Force-isolate the test environment BEFORE any hbm imports.
ORIGINAL_HOME = os.environ.get("HBM_HOME")
ORIGINAL_AUTH = Path.home() / ".hbm" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.hbm/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.hbm/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="hbm_ts_live_"))
    hbm_home = home_dir / ".hbm"
    hbm_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, hbm_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".hbm" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, hbm_home / ".env")
        ...
        load_hbm_dotenv(hbm_home=str(Path.home() / ".hbm"))
    ...
    (hbm_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return hbm_home
...
def reset_module_state():
    """Drop cached modules so the new HBM_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "hbm_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original HBM_HOME
    if ORIGINAL_HOME is not None:
        os.environ["HBM_HOME"] = ORIGINAL_HOME
    else:
