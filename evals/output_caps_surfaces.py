...
def exercise_surfaces(agent, captures, url, home, config):
    ...
    env.update(HOME=str(home), HBM_HOME=str(home), HBM_MAX_TOKENS="13", TERM="xterm", NO_COLOR="1")
    start = len(captures)
    proc = subprocess.Popen([sys.executable, "-m", "hbm_cli.main", "chat", "--provider", "fixture-local", "-m", "fixture", "-q", "Return fixture"], stdin=slave, stdout=slave, stderr=slave, cwd=os.getcwd(), env=env)
    os.close(slave)
    output = bytearray()
