...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='hbm-acp-empty-'))
    hbm = home / '.hbm'
    hbm.mkdir()
    (hbm / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), HBM_HOME=str(hbm), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               HBM_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(hbm / 'state.db') as db:
