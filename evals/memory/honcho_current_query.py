...
Run with isolated HOME/HBM_HOME and honcho-ai==2.2.0 installed (or on PYTHONPATH):
...
def prepared(sync):
    ...
    with tempfile.TemporaryDirectory() as home:
        p.save_config({"baseUrl": f"http://127.0.0.1:{server.server_port}", "workspace": "probe",
                       ...
                       "hosts": {"hbm": {"recallSync": sync}}}, home)
        cfg = HonchoClientConfig.from_global_config(host="hbm", config_path=Path(home) / "honcho.json")
