...
if len(sys.argv) == 2:
    with tempfile.TemporaryDirectory(prefix="kanban-transport-") as home:
        env = {"HOME": home, "HBM_HOME": home + "/hbm", "PATH": os.environ["PATH"], "PYTHONDONTWRITEBYTECODE": "1"}
...
os.environ.update({"HBM_KANBAN_DB": str(db), "HBM_KANBAN_BOARD": "default", "HBM_KANBAN_TASK": own, "HBM_KANBAN_RUN_ID": str(task.current_run_id), "HBM_KANBAN_CLAIM_LOCK": task.claim_lock})
ch = home / "codex"
ch.mkdir()
(ch / "config.toml").write_text(
    ...
    '[mcp_servers.hbm-tools.env]\nPYTHONPATH=' + json.dumps(str(repo)) + '\nHBM_HOME=' + json.dumps(str(hh)) + '\n'
)
report = {}
with CodexAppServerClient(codex_bin=shutil.which("codex") or "codex", codex_home=str(ch)) as c:
    try:
        ...
        raise
    script = "import os,sys,json;sys.path.insert(0," + repr(str(repo)) + ");from tools import kanban_tools as kt;print(json.dumps({'marker':os.getenv('HBM_DELEGATED_CHILD_CONTEXT'),'db':os.getenv('HBM_KANBAN_DB'),'mutation':json.loads(kt._handle_complete({'task_id':" + repr(own) + ",'summary':'must refuse'}))}))"
    ...
    for name, tid in (("foreign", foreign), ("own", own)):
        report[name] = c.request("mcpServer/tool/call", {"threadId": thread_id, "server": "hbm-tools", "tool": "kanban_complete", "arguments": {"task_id": tid, "summary": "supervised parent handoff"}}, timeout=50)
