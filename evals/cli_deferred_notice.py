...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="hbm_test_notice_") as home:
        hh = Path(home) / ".hbm"
        hh.mkdir()
        ...
        from hbm_cli.banner import VERSION
        ...
        bootstrap = ("import hbm_cli.main as m; import hbm_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "hbm_cli/banner.py") in text,
