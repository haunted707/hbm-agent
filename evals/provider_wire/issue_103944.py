...
os.environ.clear()
os.environ.update(
    HOME=home, HBM_HOME=home + "/.hbm", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
assert Path(os.environ["HBM_HOME"]).is_relative_to(Path(os.environ["HOME"]))
...
Path(os.environ["HBM_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["HBM_HOME"]) / "config.yaml").write_text(json.dumps(config))
from run_agent import AIAgent
from providers import get_provider_profile
from hbm_cli.config import get_compatible_custom_providers, load_config
from hbm_cli.config_providers import get_custom_provider_context_length
