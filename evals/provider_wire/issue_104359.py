...
home = tempfile.mkdtemp(prefix="hbm-104359-")
os.environ.clear()
os.environ.update(
    HOME=home,
    HBM_HOME=home + "/.hbm",
...
Path(os.environ["HBM_HOME"], "config.yaml").write_text(yaml.safe_dump(config), encoding="utf-8")
...
db = SessionDB(Path(os.environ["HBM_HOME"]) / "state.db")
