...
Each probe runs in a FRESH subprocess with an isolated HBM_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "HBM_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "HBM_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("HBM_SESSION", "HBM_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "hbm_cli.main", "gateway.run", "tools.registry", "hbm_state", "tui_gateway.server", "hbm_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "hbm-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `hbm --version`, `hbm --help`, `hbm doctor --help`, `hbm config get model` (no network)
CLI = {"version": ["hbm_cli/main.py", "--version"], "help": ["hbm_cli/main.py", "--help"], "config_get": ["hbm_cli/main.py", "config", "get", "model"], "tools_list": ["hbm_cli/main.py", "tools", "--help"], "skills_help": ["hbm_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_hbm_default_ms", lambda: resolve_toolset("hbm-default"), 200)
...
try:
    import hbm_state, tempfile, uuid
    from pathlib import Path
    db=hbm_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(hbm_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
