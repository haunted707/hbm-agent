...
def run_cli(repo, mode):
    with tempfile.TemporaryDirectory(prefix="hbm-device-wire-") as directory, oauth_fixture(mode) as (base, wire):
        home = Path(directory)
        ...
        env = {key: value for key, value in os.environ.items()
               if not key.startswith("HBM_") and not any(part in key for part in ("API_KEY", "TOKEN", "SECRET"))}
        env.update(HOME=str(home), HBM_HOME=str(home), PYTHONPATH=str(repo), PYTHONDONTWRITEBYTECODE="1")
        command = ["reauth", "fixture"] if mode == "preregistered" else ["login", "fixture", "--flow", "device"]
        argv = [sys.executable, "-m", "hbm_cli.main", "mcp", *command]
        if mode == "persistence":
...
''', "mcp", *command]
        ...
        if token_path.exists() and not previous:
            ...
            refreshed = subprocess.run([sys.executable, "-m", "hbm_cli.main", "mcp", "test", "fixture"],
