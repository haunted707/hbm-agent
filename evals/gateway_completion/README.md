...
Run with a fresh HOME and HBM_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" HBM_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `HBM_EVAL_REPO` selects another checkout for A/B.
