 ...
 *   The artifact published on the website right now -- HBM AGENT-Setup.exe has
 ...
 * @typedef {InstallMethod | 'hbm-update' | 'open-app-update' | 'hbm-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; hbm-desktop-app-update starts the app
 *   via `hbm-agent desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://hbm.nousresearch.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds HBM AGENT.exe AND
      ...
      // Website HBM AGENT-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `hbm-agent desktop`.
      { method: 'hbm-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published HBM AGENT-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `hbm-agent desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'hbm-desktop-app-update';
}
