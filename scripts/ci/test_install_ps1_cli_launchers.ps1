# Behavioral test for install.ps1's hbm-agent launcher staging (PR #92092,
...
#
# The test lifts the real Install-HbmCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $HbmHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-HbmCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-HbmCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'hbm-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'hbm-agent'
    ...
    $missingThrew = $false
    try {
        Install-HbmCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing hbm-agent.exe fails the launcher stage'
    ...
    $hbmV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'hbm-agent.exe'), $hbmV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-HbmCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $hbmV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'hbm-agent.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'hbm-agent-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'hbm-agent.exe'), $hbmV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'hbm-agent-acp.exe'), $acp)
    Install-HbmCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $hbmV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'hbm-agent.exe'))) `
        'installer refreshes an existing HBM AGENT launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'hbm-agent-acp.exe'))) `
    ...
    Install-HbmCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'hbm-agent.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'hbm-agent.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'hbm-agent.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'hbm-agent.exe')) -and $cmdBody.Contains('%*')) `
