...
# tree generates llms.txt — the index every LLM (HBM AGENT included, via the
# hbm-agent skill) reads to learn what HBM AGENT can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/hbm_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"hbm_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (HBM AGENT-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``hbm-agent serve`` backend, the Docker image)
