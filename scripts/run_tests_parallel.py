...
Environment:
    HBM_TEST_WORKERS  Override worker count (default: os.cpu_count())
    HBM_TEST_PATHS    Override discovery roots (colon-sep; on Windows
...
#                        nousresearch/hbm-agent:test image, via
#                        ``HBM_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or HBM_TEST_FILE_TIMEOUT.
#
...
# Set to 0 to disable (env: HBM_TEST_FILE_RETRIES).
...
def _split_pathspec(value: str) -> List[str]:
    ...
    ``HBM_TEST_PATHS``) into individual paths.
...
def _effective_file_timeout(
    ...
    but a handful of large-collection files (e.g. ``tests/test_hbm_state.py``,
...
def _run_one_file_once(
    ...
    env = os.environ.copy()
    temproot = tempfile.mkdtemp(prefix="hbm-pytest-tmproot-")
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/hbm-agent/hbm-agent/tests/acp_adapter/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("HBM_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $HBM_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("HBM_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
        help=(
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("HBM_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: HBM_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--file-retries",
        type=int,
        default=int(
            os.environ.get("HBM_TEST_FILE_RETRIES", _DEFAULT_FILE_RETRIES)
        ),
        help=(
            ...
            f"Default: {_DEFAULT_FILE_RETRIES}, env: HBM_TEST_FILE_RETRIES."
        ),
    )
    parser.add_argument(
        "--slice",
    ...
    # Parse --slice (or HBM_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("HBM_TEST_SLICE")
