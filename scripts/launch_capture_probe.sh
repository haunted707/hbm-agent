...
run_py() {
  ...
  if [ "$with_var" = yes ]; then
    PYTHONPATH="$CAP_DIR${PYTHONPATH:+:$PYTHONPATH}" \
      HBM_E2E_CAPTURE_LAUNCH="$WORK/spec.json" python3 "$@"
  else
...
run_py yes -c '
import subprocess
r = subprocess.run(["npm", "exec", "--", "electron", "."], cwd="/tmp", env={"HBM_DESKTOP_CWD": "/tmp", "PATH": "/usr/bin"})
...
assert spec["env"]["HBM_DESKTOP_CWD"] == "/tmp", "env= kwarg not captured"
...
run_py yes -c '
import subprocess
exe = "/x/apps/desktop/release/linux-unpacked/HBM AGENT"
...
run_py yes -c '
import subprocess
r = subprocess.run(["C:\\x\\apps\\desktop\\release\\win-unpacked\\HBM AGENT.exe"], env={})
