...
WITHOUT installing hbm-agent.

NOTE: this script intentionally duplicates the schema rules instead of
importing ``hbm_cli`` — the whole point is the no-install requirement for
cheap cross-repo CI use. The runtime twin of this schema lives in
``hbm_cli/plugin_catalog.py``; if the contract changes there, update the
...
# One comparator clause of a requires_hbm spec, e.g. ">=0.19" or "!=1.2.3".
...
def _check_requires_hbm(spec: object, errors: list[str]) -> None:
    if not isinstance(spec, str):
        errors.append(f"requires_hbm must be a string, got {type(spec).__name__}")
        return
    if spec.strip() == "":
    ...
    for clause in spec.split(","):
        if not _COMPARATOR_RE.match(clause.strip()):
            errors.append(
                f"requires_hbm clause {clause.strip()!r} is not a valid "
...
def validate_entry(data: object) -> tuple[list[str], list[str]]:
    ...
    if "requires_hbm" in data:
        _check_requires_hbm(data["requires_hbm"], errors)
