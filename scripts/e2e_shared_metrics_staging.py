...
HBM_HOME so the operator's own telemetry state is untouched.
...
def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="hbm-telemetry-e2e-"))
    os.environ["HBM_HOME"] = str(scratch)
    ...
    from hbm_cli.observability.shared_metrics_send_config import (
        resolve_send_config,
    )
    ...
    from hbm_cli.observability.shared_metrics_sender import (
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "hbm.shared_metrics.v2",
    ...
    print(f"scratch HBM_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://hbm-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
