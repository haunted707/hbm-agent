 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.hbm/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.hbm', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (HBM_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.HBM_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.hbm', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.HBM_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.hbm', 'document_cache');
const AUDIO_CACHE_DIR = process.env.HBM_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.hbm', 'audio_cache');
...
const DEFAULT_REPLY_PREFIX = '☤ *HBM AGENT*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls HBM AGENT itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['HBM AGENT', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into HBM AGENT and trigger
      ...
      // Ignore HBM AGENT' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `hbm_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `hbm_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
