"""Run a real HBM AGENT CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_hbm_executable(hbm_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "hbm",
        Path(".venv") / "Scripts" / "hbm-agent.exe",
    ):
        candidate = hbm_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("hbm-agent")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "HBM AGENT executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="HBM AGENT source checkout containing .venv/bin/hbm-agent",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated HBM_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["hbm.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['hbm.client.active']}"
        )
    [model] = by_name["hbm.model_route.count"]
    expected_model = {
        "name": "hbm.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['hbm.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["hbm.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['hbm.task_run.started']}"
        )
    [terminal] = by_name["hbm.task_run.finished"]
    ...
    [tool] = by_name["hbm.tool_call.count"]
    ...
    lifecycle = by_name["hbm.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["hbm.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The HBM AGENT development environment requires jsonschema"
        ) from exc
    ...
    if metrics["hbm.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['hbm.client.active']}"
        )
    [model] = metrics["hbm.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['hbm.model_route.count']}"
        )
    [terminal] = metrics["hbm.task_run.finished"]
    ...
    [tool] = metrics["hbm.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["hbm.skill.lifecycle.count"]
    ...
    loads = metrics["hbm.skill.load.count"]
    if {
        (
...
def main() -> int:
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="hbm-relay-shared-metrics-"))
    home = root / "hbm-home"
    ...
    (root / "hbm.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "hbm.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"HBM AGENT exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("HBM AGENT model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("HBM AGENT did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("HBM AGENT did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "hbm.shared_metrics.v2.schema.json",
    )

    print("HBM AGENT -> NeMo Relay shared-metrics smoke test passed")
