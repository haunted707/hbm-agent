...
Run after changing any ``desktop=`` value or alias in ``hbm_cli/commands.py``;
``tests/hbm_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from hbm_cli.commands import desktop_surface_registry
