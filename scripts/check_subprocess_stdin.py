...
When HBM AGENT runs in TUI mode, the gateway child process communicates with
...
# ``get_hbm_home() / "plugins"`` (user) and ``./.hbm/plugins/`` (project,
# gated behind ``HBM_ENABLE_PROJECT_PLUGINS``) — see
# ``hbm_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``hbm_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import hbm_constants (this script
    ...
    # plugins in get_hbm_home()/plugins/ and project plugins in
    # ./.hbm/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_hbm_home() / "plugins"]
    if os.environ.get("HBM_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".hbm" / "plugins")
