...
# changing PATH, installing software, or touching the user's HBM AGENT home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("hbm-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$HbmHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -HbmHome $HbmHome -InstallDir $InstallDir
