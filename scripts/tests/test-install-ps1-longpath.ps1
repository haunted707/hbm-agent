...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[hbm] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[hbm`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[hbm`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        HbmHome = $(if ($paths) { $paths.hbm_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\hbm\hbm-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-hbm'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-hbm') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
