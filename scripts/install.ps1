# ============================================================================
# HBM AGENT Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$HbmHome = $(if ($env:HBM_HOME) { $env:HBM_HOME } else { "$env:LOCALAPPDATA\hbm" }),
    [string]$InstallDir = $(if ($env:HBM_HOME) { "$env:HBM_HOME\hbm-agent" } else { "$env:LOCALAPPDATA\hbm\hbm-agent" }),
    ...
    # builds apps/desktop into a launchable HBM AGENT.exe.
    #
    # Why opt-in:
    #   * HBM AGENT-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched HBM AGENT.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live HBM AGENT.exe
    ...
    #     `hbm-agent desktop` already builds on demand.
...
# HBM_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[hbm] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'HbmInstall.LongPath').Type) {
            Add-Type -Namespace 'HbmInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [HbmInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -HbmHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('HbmHome')) {
    $HbmHome = ConvertTo-LongPath $HbmHome
} else {
    $HbmHome = ConvertTo-LongPath $(
        if ($env:HBM_HOME) { $env:HBM_HOME } else { "$env:LOCALAPPDATA\hbm" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:HBM_HOME) { "$env:HBM_HOME\hbm-agent" } else { "$env:LOCALAPPDATA\hbm\hbm-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: HbmHome=$HbmHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * HBM AGENT Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $HbmHome)) {
        New-Item -ItemType Directory -Force -Path $HbmHome | Out-Null
    }
    $envFile = Join-Path $HbmHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $HbmHome "node"
...
function Install-Uv {
    ...
    Write-Info "Installing managed uv into $HbmHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $HbmHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $HbmHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $HbmHome "bin"
        ...
        # (hbm_cli/managed_uv.py looks only at $HbmHome\bin\uv.exe).
...
# Put the HBM AGENT-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone hbm-setup.exe run, a user typing `npm`) silently
...
# Upgrade the HBM AGENT-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "HBM AGENT-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("hbm-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and HBM_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by HBM AGENT belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # hbm_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".hbm-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first HBM AGENT-managed interpreter
    ...
    #
    # Under HBM AGENT-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve HBM AGENT-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun HBM AGENT setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which HBM AGENT uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\hbm\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    HBM AGENT needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the HBM AGENT copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\hbm\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HBM_GIT_BASH_PATH`` (User scope) so HBM AGENT can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying an HBM AGENT-managed PortableGit install instead..."
    }

    # Download PortableGit into $HbmHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $HbmHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent HBM AGENT features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  HBM AGENT needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HBM_GIT_BASH_PATH`` (User env scope) so HBM AGENT can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $HbmHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $HbmHome\git\usr\bin\bash.exe
    $candidates += "$HbmHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$HbmHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("HBM_GIT_BASH_PATH", $candidate, "User")
            $env:HBM_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set HBM_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- HBM AGENT may not find Git Bash."
    Write-Info "If needed, set HBM_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the HBM AGENT-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (HBM AGENT requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy HBM AGENT requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using an HBM AGENT-managed Node.js installation instead..."

    # Prefer an HBM AGENT-managed Node from a previous run over a too-old system one.
    $managedNode = "$HbmHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$HbmHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$HbmHome\node"
        Write-Success "Node.js $version found (HBM AGENT-managed)"
        ...
        Update-ManagedNpm "$HbmHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing HBM AGENT-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $HbmHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $HbmHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\hbm-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$HbmHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$HbmHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$HbmHome\node") {
                    try {
                        Rename-Item "$HbmHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "HBM AGENT-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$HbmHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $HbmHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\hbm-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `hbm-agent
                    # update` path (#4735).
                    ...
                    $stashName = "hbm-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. hbm-setup.exe bakes its build-time commit
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if HBM AGENT behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running hbm processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $zipPath = "$env:TEMP\hbm-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\hbm-agent-extract"
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under HBM AGENT-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "HBM AGENT-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running hbm process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running hbm processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Hbm_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (hbm-agent.exe) plus its child tree.
            & taskkill /F /T /IM hbm-agent.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM hbm-agent.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m hbm_cli.main gateway run` straight out of
            # venv\\Scripts\\, so its image name is python/pythonw, not hbm-agent.exe.
            ...
            # HBM AGENT children re-exec through .hbm-runtime, so killing only
            ...
            # user (see hbm_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-HBM AGENT " +
...
function Restore-VenvBackup {
    ...
    # venv so HBM AGENT (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `hbm-agent.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install hbm-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and hbm-agent.exe broken with
    ...
    # here, not 30 seconds later when the user runs `hbm-agent`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close HBM AGENT processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close HBM AGENT processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves HBM AGENT and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register hbm-agent.exe in dist-info/RECORD but fail to
        ...
        # `hbm-agent` command while hbm-agent.exe / hbm-agent-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m hbm_cli.main <command>"
                    } else {
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\hbm_cli\web_server.py" 2>&1 | Out-Null
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: hbm_cli/web_server.py"
        }
    }
...
function Install-HbmCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the hbm-agent launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring hbm-agent.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "hbm-agent.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the hbm-agent command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # hbm_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("hbm-agent", "hbm-agent-acp")) {
    ...
    $requiredExe = Join-Path $Destination "hbm-agent.exe"
    $requiredCmd = Join-Path $Destination "hbm-agent.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the hbm-agent command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up hbm-agent command..."
    ...
    if ($NoVenv) {
        ...
        # Install-HbmCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $hbmBin = "$HbmHome\bin"
        Install-HbmCommandLaunchers -Root $InstallDir -Destination $hbmBin | Out-Null
    }
    ...
    #   hbm-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The hbm-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept hbm via $hbmBin)"
        }
    }
    
    if ($currentPath -notlike "*$hbmBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$hbmBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $hbmBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HBM_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hbm instead
    # of the Unix default ~/.hbm
    $currentHbmHome = [Environment]::GetEnvironmentVariable("HBM_HOME", "User")
    if (-not $currentHbmHome -or $currentHbmHome -ne $HbmHome) {
        [Environment]::SetEnvironmentVariable("HBM_HOME", $HbmHome, "User")
        Write-Success "Set HBM_HOME=$HbmHome"
    }
    $env:HBM_HOME = $HbmHome
    
    # Update current session
    $env:Path = "$hbmBin;$env:Path"
    
    Write-Success "hbm-agent command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.hbm-bootstrap-complete which tells the HBM AGENT
    ...
    $markerPath = Join-Path $InstallDir ".hbm-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the HBM_HOME directory structure ($HbmHome, default %LOCALAPPDATA%\hbm)
    New-Item -ItemType Directory -Force -Path "$HbmHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HbmHome\skills" | Out-Null
    ...
    # Create config.yaml
    $configPath = "$HbmHome\config.yaml"
    ...
    # (the default PS5 behaviour), and HBM AGENT's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$HbmHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in hbm_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $HbmHome"
    
    # Seed bundled skills into $HbmHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $HbmHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $HbmHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$HbmHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $HbmHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (HBM AGENT-Setup.exe runs each -Stage NAME
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\hbm-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\hbm-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\hbm-npm-tui-$(Get-Random).log"
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $HbmHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only HBM AGENT's managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside HBM AGENT's managed bin dir,
...
# Provision it at install time so enabling the tool later -- via `hbm-agent
...
# paths still lazy-install via install_cua_driver() (hbm_cli/tools_config)
...
# the final `electron` -> `HBM AGENT` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable HBM AGENT.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_hbm_desktop command
    # resolves apps/desktop/release/win-unpacked/HBM AGENT.exe directly,
    ...
    # enforces the supported Node lines and prepends the HBM AGENT-managed Node to
    ...
    # The --dir mode produces an unpacked HBM AGENT.exe in
    ...
    # are NOT trying to work around). The HBM AGENT icon + product name are
    # stamped onto HBM AGENT.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\hbm-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\HBM AGENT.exe",
        "$desktopDir\release\win-arm64-unpacked\HBM AGENT.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no HBM AGENT.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The HBM AGENT icon + identity are stamped onto HBM AGENT.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, hbm-agent#38216).
    ...
    #    HBM AGENT.exe. We deliberately do NOT point them at `hbm-agent desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'HBM AGENT.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'HBM AGENT.lnk')
        )
        ...
        # with the HBM AGENT icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.hbm/.env are importable.  Two problems this solves:
    #
...
function Start-GatewayIfConfigured {
    ...
    $hbmCmd = "$InstallDir\venv\Scripts\hbm-agent.exe"
    if (-not (Test-Path $hbmCmd)) {
        $hbmCmd = "hbm-agent"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$HbmHome\whatsapp\session\creds.json"
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$HbmHome\logs\gateway.log"
            Start-Process -FilePath $hbmCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$HbmHome\logs\gateway-error.log" `
                -WindowStyle Hidden
...
function Write-Completion {
    ...
    Write-Host "$HbmHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$HbmHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HbmHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HbmHome\hbm-agent\"
    Write-Host ""
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning HBM AGENT repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.hbm-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (HBM AGENT-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding HBM AGENT to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
