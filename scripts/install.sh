...
# Python-driven tool session (e.g. HBM AGENT terminal tool). A pre-set PYTHONPATH
...
HBM_HOME="${HBM_HOME:-$HOME/.hbm}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${HBM_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$HBM_INSTALL_DIR"
...
#   code at /usr/local/lib/hbm-agent, command at /usr/local/bin/hbm-agent,
#   data still at /root/.hbm (HBM_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> HBM AGENT.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.hbm/hbm-agent"
            echo "                   default (root, Linux): /usr/local/lib/hbm-agent"
            echo "  --hbm-home PATH  Data directory (default: ~/.hbm, or \$HBM_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, HBM AGENT installs the code under"
            echo "  /usr/local/lib/hbm-agent and links the command into"
            echo "  /usr/local/bin/hbm-agent (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HBM_HOME"
            echo "  (default /root/.hbm).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HBM_HOME/hbm-agent are preserved in-place."
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (HBM AGENT-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download HBM AGENT","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hbm-agent command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `hbm-agent` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HBM_HOME/hbm-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HBM_HOME/hbm-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/hbm-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HBM_HOME/hbm-agent — then preserve it)
#
# Always no-op when the user set --dir or $HBM_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in HBM_HOME.
    if is_termux; then
        INSTALL_DIR="$HBM_HOME/hbm-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HBM_HOME/hbm-agent/.git" ]; then
            INSTALL_DIR="$HBM_HOME/hbm-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/hbm-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/hbm-agent"
        ...
        # /usr/local/bin/hbm-agent wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/hbm-agent"
        log_info "  Data:    $HBM_HOME (unchanged)"
...
# Point an HBM AGENT-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $HBM_HOME/node/bin — which is NOT on
...
# HBM AGENT's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no HBM AGENT-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$HBM_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$HBM_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$HBM_HOME/node/etc/npmrc"
}

get_hbm_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/hbm-agent" ]; then
        echo "$link_dir/hbm-agent"
    else
        echo "hbm-agent"
    fi
}
...
install_uv() {
    ...
    log_info "Installing managed uv into $HBM_HOME/bin ..."
    mkdir -p "$HBM_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/hbm-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/hbm-uv-installer.$$.sh")"
    ...
    # directly into $HBM_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$HBM_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # HBM AGENT currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; HBM AGENT requires Python >=3.11,<3.14"
...
# replaced with the HBM AGENT-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing HBM AGENT-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing HBM AGENT-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer an HBM AGENT-managed Node from a previous run over a too-old system one.
    if [ -x "$HBM_HOME/node/bin/node" ] && [ -x "$HBM_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$HBM_HOME/node/bin/node" --version)"; then
        export PATH="$HBM_HOME/node/bin:$PATH"
        log_success "Node.js $("$HBM_HOME/node/bin/node" --version) found (HBM AGENT-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing HBM AGENT-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (HBM AGENT requires Node 22.22+, 24.11+, or 26+) — installing HBM AGENT-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.hbm/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.hbm/node/..."
    ...
    # Place into ~/.hbm/node/ and symlink binaries into the same bin dir
    # the hbm-agent command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$HBM_HOME/node"
    mkdir -p "$HBM_HOME"
    mv "$extracted_dir" "$HBM_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$HBM_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$HBM_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$HBM_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$HBM_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.hbm/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. HBM AGENT install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "HBM AGENT itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                local stash_name
                stash_name="hbm-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if HBM AGENT behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up hbm-agent command..."

    if [ "$USE_VENV" = true ]; then
        HBM_BIN="$INSTALL_DIR/venv/bin/python"
        HBM_ENTRYPOINT="$INSTALL_DIR/hbm-agent"
    else
        HBM_BIN="$(which hbm-agent 2>/dev/null || echo "")"
        if [ -z "$HBM_BIN" ]; then
            log_warn "hbm-agent not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$HBM_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$HBM_ENTRYPOINT" ]; }; then
        log_warn "HBM AGENT launcher prerequisites not found"
    ...
    # Create a user-facing shim for the hbm-agent command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $HBM_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$HBM_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/hbm-agent"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/hbm-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HBM_BIN" "$HBM_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/hbm-agent" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/hbm-agent"
    log_success "Installed hbm-agent launcher → $command_link_display_dir/hbm-agent"

    # Also expose `hbm-agent-run`. The `hbm-agent-run` console script declared in
    ...
    rm -f "$command_link_dir/hbm-agent-run"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/hbm-agent-run" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HBM_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/hbm-agent-run" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HBM_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/hbm-agent-run"
    log_success "Installed hbm-agent-run launcher → $command_link_display_dir/hbm-agent-run"

    # Also expose `hbm-agent-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `hbm-agent-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report HBM AGENT as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/hbm-agent-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/hbm-agent-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HBM_BIN" "$HBM_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/hbm-agent-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/hbm-agent-acp"
    log_success "Installed hbm-agent-acp launcher → $command_link_display_dir/hbm-agent-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "hbm-agent command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v hbm' \
                >/dev/null 2>&1; then
        ...
        log_info "hbm-agent not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# HBM AGENT — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# HBM AGENT — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# HBM AGENT — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so hbm works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.hbm directory structure (config at top level, code in subdir)
    mkdir -p "$HBM_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.hbm/.env (top level, easy to find)
    if [ ! -f "$HBM_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HBM_HOME/.env"
            log_success "Created ~/.hbm/.env from template"
        else
            touch "$HBM_HOME/.env"
            log_success "Created ~/.hbm/.env"
        fi
    else
        log_info "~/.hbm/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$HBM_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.hbm/config.yaml (top level, easy to find)
    if [ ! -f "$HBM_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HBM_HOME/config.yaml"
            log_success "Created ~/.hbm/config.yaml from template"
        fi
    else
        log_info "~/.hbm/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in hbm_cli/default_soul.py — the
    ...
    if [ ! -f "$HBM_HOME/SOUL.md" ]; then
        cat > "$HBM_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.hbm/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.hbm/"
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.hbm/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HBM_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$HBM_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.hbm/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# HBM AGENT browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "HBM AGENT will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# HBM AGENT browser tools — explicit browser override."
...
install_browser_use_cli() {
    ...
    if [ -x "$HBM_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside HBM AGENT's managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$HBM_HOME/bin" \
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        python -m hbm_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$HBM_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for HBM AGENT to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$HBM_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HBM_CMD gateway install 2>/dev/null; then
            ...
            nohup $HBM_CMD gateway > "$HBM_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.hbm-bootstrap-complete, which tells the HBM AGENT
    ...
    local marker_path="$INSTALL_DIR/.hbm-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $HBM_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $HBM_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $HBM_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'hbm-agent' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'hbm-agent' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'hbm-agent' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$HBM_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$HBM_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$HBM_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$HBM_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`HBM AGENT` rename
...
# _purge_electron_build_cache() used by `hbm-agent desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the HBM AGENT-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `hbm-agent desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/HBM AGENT" ]; then
            app="$desktop_dir/release/linux-unpacked/HBM AGENT"
        elif [ -x "$desktop_dir/release/linux-unpacked/hbm" ]; then
            app="$desktop_dir/release/linux-unpacked/hbm"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/HBM AGENT.app" \
            "$desktop_dir/release/mac/HBM AGENT.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `hbm-agent desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading HBM_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if HBM_HOME="$HBM_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from hbm_cli.main import _desktop_macos_relaunchable_fixup
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the HBM AGENT-managed Node
            # provisioned during prerequisites/node-deps (at $HBM_HOME/node/bin)
