...
VERSION_FILE = REPO_ROOT / "hbm_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@hbm.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.hbm": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level HBM_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (hbm-agent journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "hbm.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.hbm/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows hbm-shim resolution + prefer --update on recovery; #52378)
    ...
    "274096618+hbm-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "hbm@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on hbm-agent mcp add)
    ...
    "hbm@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyahbm.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (HBM_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (HBM_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: hbm-eval community link)
    ...
    "hbmagent26@gmail.com": "hbmagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+hbmagent26@users.noreply.github.com": "hbmagent26",
    ...
    "vigo@hbm": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local HBM AGENT runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level HBM_HOME state files)
    ...
    "zhchl@hbm-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live HBM AGENT
    ...
    # Keep the bootstrap installer (HBM AGENT-Setup.dmg CFBundleShortVersionString)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "hbm@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hermes-agent",
    ...
    lines.append(f"# HBM AGENT v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for HBM AGENT. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="HBM AGENT Release Tool")
    ...
    print(f"{'='*60}")
    print("  HBM AGENT Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"HBM AGENT v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"HBM AGENT v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'HBM AGENT v{new_version} ({calver_date})' "
