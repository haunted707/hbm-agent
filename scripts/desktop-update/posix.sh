...
# the HBM_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (HBM_HOME/hbm-agent)
...
HBM_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
HBM_HOME="${HBM_HOME:-${TMPDIR:-/tmp}}"
MARKER="$HBM_HOME/.hbm-update-in-progress"
LOG_DIR="$HBM_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$HBM_HOME/.hbm-update-result.json"
STATUS="${TMPDIR:-/tmp}/hbm-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"HBM AGENT update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "HBM AGENT update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="HBM AGENT update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "HBM AGENT update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
find_browser() {
  ...
  # email included — inside this window that is titled "HBM AGENT" (#88410).
...
start_ui() {
  ...
  UI_PROFILE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hbm-update-ui-XXXXXXXX")" || {
    UI_PROFILE_DIR=""
...
stop_ui() { # error/manual outcomes keep the window up briefly so a watching
  ...
  if [ "${1:-}" = "leave-window" ]; then
    sleep "${HBM_UPDATE_SHIM_GRACE_SECONDS:-15}"
  fi
...
linux_gate() {
  ...
  # _desktop_linux_userns_sandbox_available() probe in hbm_cli/main.py).
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen HBM AGENT to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/HBM AGENT.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/HBM AGENT.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall HBM AGENT (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE HBM AGENT also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen HBM AGENT to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
# (`venv/bin/hbm-agent` has a `python3` shebang), so no Python-side heal —
...
tcc_pick_update_invoke() { # sets UPDATE_INVOKE; safety net past a failed heal
  ...
  # launchd gateway proves `venv/bin/python -m hbm_cli.main` works when
  # every alias entrypoint is bricked — drive the update the same way.
  local bin="$1"
  UPDATE_INVOKE=("$bin/hbm-agent")
  if ! tcc_probe_python "$bin/python3" && tcc_probe_python "$bin/python"; then
    UPDATE_INVOKE=("$bin/python" -m hbm_cli.main)
  fi
}
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${HBM_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${HBM_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch HBM AGENT while the
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the HBM AGENT window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close HBM AGENT fully and try again."
...
tcc_pick_update_invoke "$INSTALL_ROOT/venv/bin"
if [ "${UPDATE_INVOKE[0]}" != "$HBM_BIN" ]; then
...
# "close all HBM AGENT windows" sentinel.
KEEP_STASH=""
if "${UPDATE_INVOKE[@]}" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed hbm predates --keep-stash; running without it"
fi
...
if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  ...
  # Exit 2 ("close all HBM AGENT windows") is not retryable.
  #
