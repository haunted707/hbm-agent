...
# (%HBM_HOME%\hbm-setup.exe). That binary has no self-update path --
# copy_self_to_hbm_home deliberately no-ops during --update -- so every
...
#     -InstallRoot <path>   repo checkout (HBM_HOME\hbm-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] HBM AGENT.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .hbm-update-in-progress in place (tests)
#
...
# .hbm-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim HBM_HOME\.hbm-update-in-progress with OUR pid as
...
# pass our foreground right on to the new HBM AGENT.exe pid.
try {
    Add-Type -Namespace HbmHandoff -Name Win32 -MemberDefinition @'
...
$HbmHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $HbmHome ".hbm-update-in-progress"
$LogDir = Join-Path $HbmHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $HbmHome ".hbm-update-result.json"
$script:Ui = $null
$script:UiStage = "HBM AGENT will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale hbm-update-ui-* leftovers from interrupted
    ...
    # update marker (.hbm-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "hbm-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-HbmStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("hbm-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [HbmHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    try {
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [HbmHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen HBM AGENT themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old HBM AGENT.exe before writing the replacement,
    ...
    # DO NOT spawn HBM AGENT.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [HbmHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [HbmHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [HbmHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh HBM AGENT process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [HbmHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
if ($env:HBM_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:HBM_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:HBM_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:HBM_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
...
if ($env:HBM_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:HBM_UPDATE_PROGRESS_LOG
}
...
public static class HbmUpdateJob {
...
function Invoke-HbmStep([string]$Exe, [string[]]$HbmArgs, [string]$Tag) {
    ...
    # .hbm-update-result.json, clearing .hbm-update-in-progress,
    ...
    # HBM AGENT" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($HbmArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [HbmUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [HbmUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; HBM_SELFTEST_FAIL=1 exercises the
# error state, HBM_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:HBM_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:HBM_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:HBM_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-HbmStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no HBM AGENT install to
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:HBM_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:HBM_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:HBM_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:HBM_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "hbm-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "hbm-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "hbm-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "hbm-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "hbm-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "hbm-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "hbm-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "hbm-step-logstall-$stamp.update.log"
...
'@
    ...
    $flood = Invoke-HbmStep $powershell @(
    ...
    $stall = Invoke-HbmStep $powershell @(
    ...
    try {
        $logstall = Invoke-HbmStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-HbmStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-HbmStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:HBM_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from HBM_HOME; pin the process directory to the
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-HbmStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for HBM AGENT to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the HBM AGENT window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close HBM AGENT fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing HBM AGENT files"
    $shim = Join-Path $InstallRoot "venv\Scripts\hbm.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the HBM AGENT install open (venv\Scripts\hbm.exe locked after 20s). Nothing was changed. Close other HBM AGENT windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the hbm.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\hbm.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_hbm_exe must first rename the running hbm.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that hbm.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second HBM AGENT window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a HBM AGENT.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m hbm_cli.main update` puts the
    ...
    $updateArgs = @("-m", "hbm_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all HBM AGENT windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m hbm_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed hbm predates --keep-stash; running without it"
        }
    } catch {
    ...
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-HbmUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import hbm_cli.main; from hbm_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-HbmStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated HBM AGENT runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    #   3. only then the terminal UI state — done means "HBM AGENT is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. HBM AGENT was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen HBM AGENT."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening HBM AGENT"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen HBM AGENT to finish (it could not restart itself)."
