...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # HBM AGENT derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            hbm-agent = prev.hbm-agent.overrideAttrs (_old: {
              HBM_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $HBM_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$HBM_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        hbm-agent = prev.hbm-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "hbm-agent-env" {
    hbm-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "hbm-agent-editable-env" {
    hbm-agent = dependency-groups;
  };
}
