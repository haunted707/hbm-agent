...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      hbmNpmLib = self'.packages.default.passthru.hbmNpmLib;
      ...
      hbmAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "hbm" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../hbm-agent} $out/bin/hbm-agent
          '')
        ...
        shellHook = ''
          ${hbmAgentDevShellHook}
          ${hbmNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export HBM_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "HBM AGENT dev shell in $HBM_PYTHON_SRC_ROOT"
          echo "Ready. Run 'hbm' or 'sandbox hbm' to start."
        '';
      };
    };
}
