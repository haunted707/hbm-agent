# nix/packages.nix — HBM AGENT package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./hbm-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.hbmTui;
        web = full.hbmWeb;
        desktop = full.hbmDesktop;

        update-npm-lockfile = full.hbmNpmLib.updateNpmLockfile;
      };
    };
}
