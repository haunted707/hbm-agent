...
#
# `services.hbm-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  hbmHome, programs.hbm-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "hbm-config-attrs";
    description = "HBM AGENT YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $HBM_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "HBM AGENT";
      ...
      package = mkOption {
        ...
        description = "The hbm-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The HBM AGENT configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `hbm config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $HBM_HOME/.env. HBM AGENT reads that file at each start, with
          load_hbm_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."hbm/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that HBM AGENT refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          HBM AGENT reads SOUL.md and memories/ from HBM_HOME, so put those
          files in `hbmHomeFiles`.
      ...
      hbmHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into HBM_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          HBM AGENT reads SOUL.md and the memory files from HBM_HOME and not
          from the working directory. Declare those files here, or HBM AGENT
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the hbm plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. HBM AGENT discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          hbm_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as hbm (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in hbm-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `hbm gateway`.";
      };
      ...
      # ── The backend: `hbm serve` or `hbm dashboard` ──────────────
      # `hbm serve` and `hbm dashboard` are the same entry point,
      # hbm_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that HBM AGENT Desktop
      ...
      # controls an external gateway, with `hbm gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that HBM AGENT
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            HBM_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. HBM AGENT Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."hbm/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `hbm` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.hbm-agent.installPackage was removed. HBM AGENT now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.hbm-agent.enable = ${lib.boolToString (value != false)};  # the hbm CLI, and HBM_HOME for your shells
        programs.hbm-agent.desktop.enable = true;  # the desktop application

      `services.hbm-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "hbm-config.yaml" (
    ...
    pkgs.writeShellScript "hbm-env-merge" ''
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "hbm-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # HBM_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and HBM AGENT make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${hbmHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. HBM AGENT
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${hbmHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${hbmHome}/config.yaml
            ${run}chmod ${modes.config} ${hbmHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "hbm-managed" managedSystem} ${hbmHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${hbmHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${hbmHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${hbmHome}/auth.json"
        else
          ''
            if [ ! -e ${hbmHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${hbmHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree hbmHome cfg.hbmHomeFiles}
      ...
      ${run}find ${hbmHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "hbm-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${hbmHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/hbm-agent"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps hbm as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "hbm-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "hbm-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "hbm-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          HBM_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export HBM_DASHBOARD_SESSION_TOKEN

          if [ -z "$HBM_DASHBOARD_SESSION_TOKEN" ]; then
            echo "hbm-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "hbm-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "hbm-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "hbm-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "hbm-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "hbm-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "hbm-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "HBM AGENT web dashboard and desktop backend"
    else
      "HBM AGENT backend for HBM AGENT Desktop";

  # The environment that each HBM AGENT process needs, from either module.
  #
  # managedSystem gives the value of HBM_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give HBM AGENT an identity and a memory, use
          ${optionPath}.hbmHomeFiles instead. Those files go to
          HBM_HOME. HBM AGENT reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of HBM_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
