# nix/nixosModules.nix — the NixOS module for hbm-agent
#
...
#
# Container mode: hbm runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $HBM_HOME/.env
# and read by hbm at startup — no container recreation needed for env changes.
#
# Tool resolution: the hbm wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.hbm-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      hbm-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      hbmHome = "${cfg.stateDir}/.hbm";
      ...
      # HBM_HOME via addToSystemPackages, so they can save settings through the
      ...
      # hbm user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "hbm-container-entrypoint" ''
        set -eu

        HBM_UID="''${HBM_UID:?HBM_UID must be set}"
        HBM_GID="''${HBM_GID:?HBM_GID must be set}"

        # ── Group: ensure a group with GID=$HBM_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$HBM_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="hbm"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$HBM_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$HBM_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$HBM_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$HBM_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="hbm"
          TARGET_HOME="/home/hbm"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$HBM_UID" -g "$HBM_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$HBM_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$HBM_UID:$HBM_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure HBM_HOME is owned by the target user.
        ...
        if [ -n "''${HBM_HOME:-}" ] && [ -d "$HBM_HOME" ]; then
          find "$HBM_HOME" \! -user "$HBM_UID" -exec chown "$HBM_UID:$HBM_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/hbm-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/hbm-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/hbm ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/hbm
          chmod 0440 /etc/sudoers.d/hbm
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$HBM_UID" --regid="$HBM_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $HBM_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "hbm-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        hbm_bin=${containerDataDir}/current-package/bin/hbm-agent
      '';

      # Default: /var/lib/hbm/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the hbm group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit hbmHome; };
    ...
    in
    {
      options.services.hbm-agent =
        common.sharedOptions {
          defaultPackage = hbm-agent;
          defaultPackageText = lib.literalExpression "hbm-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/hbm";
              description = "State directory. Contains .hbm/ subdir (HBM_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the hbm CLI to environment.systemPackages and export
                HBM_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.hbm symlink to the service
                  stateDir. These users are automatically added to the hbm group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.hbm-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the hbm CLI to system PATH and export HBM_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.hbm/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.HBM_HOME = hbmHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.hbm-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.hbm-agent.workingDirectory;
                optionPath = "services.hbm-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.hbm-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the hbm user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.hbm-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed hbm
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure hbm is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${hbmHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${hbmHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."hbm-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${hbmHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${hbmHome} ${cfg.workingDirectory}
                  ...
                  find ${hbmHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${hbmHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${hbmHome}/$_subdir"
                    chmod 2770 "${hbmHome}/$_subdir"
                    find "${hbmHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg hbmHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${hbmHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${hbmHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${hbmHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.hbm";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${hbmHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "hbm-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.hbm -> stateDir/.hbm for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.hbm-agent = {
              ...
              # $HBM_HOME/.env by the activation script. load_hbm_dotenv()
              ...
              preStart = lib.mkIf lingerEnabled ''
                ...
                if [ ! -S "/run/user/$(id -u)/bus" ]; then
                  echo "hbm-agent: no user bus at /run/user/$(id -u)/bus after 10s;" \
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.hbm-agent = {
              description = "HBM AGENT Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
                "network-online.target"
              ]
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  HBM_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  HBM_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env HBM_UID="$HBM_UID" \
                    --env HBM_GID="$HBM_GID" \
                    --env HBM_HOME=${containerDataDir}/.hbm \
                    --env HBM_MANAGED=true \
