# nix/homeManagerModules.nix — the Home Manager module for hbm-agent
#
# This module is the user-level equivalent of nixosModules.default. HBM AGENT is
...
#
# `services.hbm-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.hbm-agent.enable and
#                                        home.sessionVariables
#   added     programs.hbm-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.hbm")  -> hbmHome, set directly
#
# To use the module:
#   imports = [ hbm-agent.homeManagerModules.default ];
#   programs.hbm-agent = {
#     enable = true;          # the hbm CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.hbm-agent;
      ...
      # not reach it, and the application would open ~/.hbm while the
      # services use hbmHome. Thus the launcher carries the value itself.
      #
      # HBM_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        HBM_HOME = cfg.hbmHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) HBM_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        HBM_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("HBM_DESKTOP_REMOTE_URL is set
      # but HBM_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          HBM_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export HBM_DESKTOP_REMOTE_TOKEN
        else
          echo "hbm-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "hbm-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.hbm-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". HBM AGENT needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.hbm-agent` stays the authority for the state and the
      # configuration. This module reads hbmHome and the backend address
      # from it, and never the reverse.
      options.programs.hbm-agent = {
        enable = lib.mkEnableOption ''
          the HBM AGENT command line application.

          This adds `hbm` to home.packages, and exports HBM_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.hbm-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.hbm-agent.package";
          description = ''
            The hbm-agent package to install.

            The default follows `services.hbm-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the HBM AGENT Desktop application (Electron).

            This adds `hbm-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same HBM AGENT
            runtime that `package` gives, and reads the HBM_HOME of
            `services.hbm-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own HBM AGENT runtime with
            the usual distribution. This module gives it the Nix package
            instead, with HBM_DESKTOP_HBM. It installs no second copy
            of HBM AGENT, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.hbmDesktop;
            defaultText = lib.literalExpression "config.programs.hbm-agent.package.hbmDesktop";
            description = ''
              The hbm-desktop package to use.

              The default follows `package`, and thus also
              `services.hbm-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own HBM AGENT runtime, and this module cannot make
      ...
      options.services.hbm-agent =
        ...
        // {
          hbmHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.hbm";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.hbm"'';
            description = ''
              The value of HBM_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.hbm` to it.
              This module sets HBM_HOME directly. Thus an existing
              ~/.hbm continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.hbm-work";
          };

          # `installPackage` moved to `programs.hbm-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `hbm` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.hbm-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.HBM_HOME = cfg.hbmHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # HBM_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put HBM_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `hbm` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.hbm-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
                ...
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) hbmHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.hbm-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.hbm-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.hbm-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "hbm-agent";
              };
            })
            ...
            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.hbm-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "hbm-backend";
              };
            })
