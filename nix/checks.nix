...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      hbm-agent = self'.packages.default;
      hbmVenv = hbm-agent.hbmVenv;
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.hbm-agent = programs;
              services.hbm-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.hbm-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.hbm-agent);
      ...
      configKeys = pkgs.runCommand "hbm-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${hbmVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "hbm-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "hbm-build-package" { } ''
          echo "PASS: package built at ${hbm-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "hbm-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.HBM_TEST = "1";
                environmentFiles = [ "/run/secrets/hbm-env" ];
                hbmHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "hbm" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "hbm" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.hbmAgentSetup.data;
            ...
            failures =
              ...
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/hbm-check/.hbm" (envOf n)) names
              ) "gateway and backend must share one HBM_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.HBM_HOME or null != "/home/hbm-check/.hbm"
              ) "programs.hbm-agent.enable must export HBM_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "hbm-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/hbm-check/.hbm/SOUL.md" activation
              ) "hbmHomeFiles must install into HBM_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads HBM_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "HBM_MANAGED=home-manager" (envOf n)) names
              ) "processes must report HBM_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "hbm-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "hbm-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            cases = [
              ...
              {
                name = "hbmHomeFiles needs no directory";
                ok = accepts { hbmHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "hbm-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one HBM_HOME ───────────────
        # `programs.enable` exports HBM_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.hbm
        # while the services use the HBM_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/hbm-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                hbmHome = "/home/hbm-check/.hbm-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "hbm-desktop") cfg.home.packages;
            ...
            # second HBM AGENT runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "hbm-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.hbm-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.hbm-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one hbm-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "HBM_HOME" != "/home/hbm-check/.hbm-work"
              ) "the launcher must carry HBM_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "HBM_HOME")})"
              ++ lib.optional (
                setValue "HBM_MANAGED" != "home-manager"
              ) "the launcher must report HBM_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "HBM_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "HBM_DESKTOP_HBM" != "${lib.head agentPackages}/bin/hbm-agent"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "HBM_DESKTOP_HBM")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "HBM_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "HBM_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "HBM_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "HBM_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "HBM_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "hbm-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares HBM_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set HBM_HOME" wrapper
              ) "the launcher must carry HBM_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "HBM_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "HBM_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? hbm-backend || cfg.launchd.agents ? hbm-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "hbm-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `hbm`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.hbm-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.hbm-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "hbm-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "hbm-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "hbm" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."hbm-agent-setup".text;
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "/var/lib/hbm/.hbm/SOUL.md" activation
              ) "hbmHomeFiles must install into HBM_HOME";
          ...
          in
          pkgs.runCommand "hbm-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs hbm, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.hbm-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              ++ lib.optional (!lib.hasInfix "hbm-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec hbm, so that it keeps the MainPID"
          ...
          in
          pkgs.runCommand "hbm-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        cron-worker-user-scope =
          let
            ...
            managed = configOf { };
            gateway = managed.systemd.services.hbm-agent;
            ...
            unmanaged = (configOf { createUser = false; }).systemd.services.hbm-agent;
          ...
          in
          pkgs.runCommand "hbm-cron-worker-user-scope" { } (
            if failures != [ ] then
        ...
        # $HBM_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          ...
          in
          pkgs.runCommand "hbm-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'HBM_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--hbm-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "hbm-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "hbm-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${hbm-agent}/bin/hbm-agent || (echo "FAIL: hbm binary missing"; exit 1)
          test -x ${hbm-agent}/bin/hbm-agent || (echo "FAIL: hbm-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${hbm-agent}/bin/hbm-agent --version 2>&1 | grep -qi "hbm" || (echo "FAIL: version check"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hbm-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking hbm --help ==="
          ${hbm-agent}/bin/hbm-agent --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${hbm-agent}/bin/hbm-agent --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hbm-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${hbm-agent}/share/hbm-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${hbm-agent}/share/hbm-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HBM_BUNDLED_SKILLS" ${hbm-agent}/bin/hbm-agent || \
            (echo "FAIL: HBM_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: HBM_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${hbm-agent}/share/hbm-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${hbm-agent}/share/hbm-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "HBM_OPTIONAL_SKILLS" ${hbm-agent}/bin/hbm-agent || \
            (echo "FAIL: HBM_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, HBM_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "hbm-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${hbm-agent}/share/hbm-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${hbm-agent}/share/hbm-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HBM_BUNDLED_PLUGINS" ${hbm-agent}/bin/hbm-agent || \
            (echo "FAIL: HBM_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: HBM_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "hbm-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${hbm-agent}/share/hbm-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${hbm-agent}/share/hbm-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${hbm-agent}/share/hbm-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HBM_BUNDLED_LOCALES" ${hbm-agent}/bin/hbm-agent || \
            (echo "FAIL: HBM_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: HBM_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + HBM_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HBM_BUNDLED_LOCALES=${hbm-agent}/share/hbm-agent/locales \
            ${hbmVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with HBM_BUNDLED_LOCALES set"; exit 1)
        ...
        # HBM_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "hbm-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${hbm-agent}/share/hbm-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${hbm-agent}/share/hbm-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "HBM_OPTIONAL_MCPS" ${hbm-agent}/bin/hbm-agent || \
            (echo "FAIL: HBM_OPTIONAL_MCPS not in wrapper"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "hbm-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${hbm-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${hbm-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HBM_TUI_DIR" ${hbm-agent}/bin/hbm-agent || \
            (echo "FAIL: HBM_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: HBM_TUI_DIR set in wrapper"
        ...
        # Verify HBM_NODE is set in wrapper and points to Node 26+
        # (HBM AGENT pins its toolchain to Node 26 everywhere)
        hbm-node = pkgs.runCommand "hbm-node-version" { } ''
          set -e
          echo "=== Checking HBM_NODE in wrapper ==="
          grep -q "HBM_NODE" ${hbm-agent}/bin/hbm-agent || \
            (echo "FAIL: HBM_NODE not set in wrapper"; exit 1)
          echo "PASS: HBM_NODE present in wrapper"

          HBM_NODE=$(sed -n "s/^export HBM_NODE='\(.*\)'/\1/p" ${hbm-agent}/bin/hbm-agent)
          test -x "$HBM_NODE" || (echo "FAIL: HBM_NODE=$HBM_NODE not executable"; exit 1)
          echo "PASS: HBM_NODE executable at $HBM_NODE"

          NODE_MAJOR=$("$HBM_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, HBM AGENT requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All HBM_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify HBM_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "hbm-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(HBM_MANAGED=true "$@" 2>&1 || true)
        ...
        extra-python-packages = let
          ...
          hbmWithExtra = hbm-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "hbm-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hbmWithExtra}/bin/hbm-agent || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hbmWithExtra}/bin/hbm-agent || \
          ...
          if grep -q "PYTHONPATH" ${hbm-agent}/bin/hbm-agent; then
        ...
        extra-dependency-groups = let
          hbmWithGroups = hbm-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "hbm-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${hbmWithGroups}"
          echo "venv: ${hbmWithGroups.hbmVenv}"
        ...
        messaging-variant = pkgs.runCommand "hbm-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.hbmVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "hbm-config-roundtrip" {
          ...
          merge_and_load() {
            local hbm_home="$1"
            export HBM_HOME="$hbm_home"
            ${configMergeScript} ${nixSettings} "$hbm_home/config.yaml"
            ${hbmVenv}/bin/python3 -c '
import json, sys
