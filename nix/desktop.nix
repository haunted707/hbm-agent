# nix/desktop.nix — HBM AGENT Desktop (Electron) app build + wrapper
#
# `hbmAgent` is the fully-built `.#default` package — it ships the
# `hbm` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `HBM_DESKTOP_HBM` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing HBM AGENT CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # HBM_HOME and HBM_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "hbm-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "hbm-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = hbmNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "hbm-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe hbmNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/hbm-desktop $out/bin
    cp -r ${renderer}/* $out/share/hbm-desktop/
    ...
    substituteInPlace $out/share/hbm-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/hbm-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # HBM_DESKTOP_HBM to the absolute path of the nix-built `hbm`
    # binary so the desktop's resolver step 4 ("existing HBM AGENT CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/hbm-desktop \
      --add-flags "$out/share/hbm-desktop" \
      --set HBM_DESKTOP_HBM "${lib.getExe hbmAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/hbm.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../hbm_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/hbm-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/hbm.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/hbm.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for HBM AGENT";
    homepage = "https://github.com/NousResearch/hermes-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "hbm-desktop";
  };
}
