# nix/web.nix — HBM AGENT Web Dashboard (Vite/React) frontend build
{ hbmNpmLib, ... }:
hbmNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @hbm/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../hbm_cli/web_dist for the
