"""Timezone-aware clock for HBM AGENT.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``HBM_TIMEZONE`` env var, then ``timezone`` in ``~/.hbm/config.yaml``, else server-local
...
# HBM_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``HBM_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("HBM_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without hbm_cli importable.
        try:
