"""
HBM AGENT MCP Server — expose messaging conversations as MCP tools (`hbm mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the HBM AGENT-specific
channels_list. Client config: {"mcpServers": {"hbm": {"command": "hbm-agent", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("hbm.mcp_serve")
...
def _hbm_home() -> Path:
    try:
        ...
        return Path(os.environ.get("HBM_HOME", Path.home() / ".hbm"))


def _get_sessions_dir() -> Path:
    return _hbm_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_hbm_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_hbm_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the HBM AGENT analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the HBM AGENT MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("hbm", instructions=(
        "HBM AGENT messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the HBM AGENT MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
