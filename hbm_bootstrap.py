"""Windows UTF-8 bootstrap for HBM AGENT entry points (no-op on POSIX).
...
point (``hbm-agent``, ``hbm-agent-acp``, ``hbm-agent-run``, ``gateway.run``, ``batch_runner``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``hbm_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``hbm_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing HBM AGENT modules.

    HBM AGENT ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the HBM AGENT root is *relocated* to the front, not merely inserted when
    ...
    """
    root = src_root or os.environ.get("HBM_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``HBM_LAZY_INSTALL_TARGET``) on ``sys.path``.
    ...
    """
    if not os.environ.get("HBM_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points only need ``import hbm_bootstrap`` first.
