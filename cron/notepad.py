...
(``hbm cron notepad <job_id> set ...``) via the terminal tool; no model tool is added.
"""
...
# ticks (set_hbm_home_override) cannot leak one profile's notepad rows into the import-time home
...
def _current_notepad_file() -> Path:
    return NOTEPAD_FILE or (get_hbm_home().resolve() / "cron" / "notepad.db")


def _connect() -> sqlite3.Connection:
    # Late imports: a scheduler daemon that outlives an on-disk upgrade already has the OLD
    # ``hbm_cli.sqlite_util`` / ``cron.jobs`` cached, so new names must be resolved at call time,
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`hbm cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `hbm cron notepad {job_id} delete "
