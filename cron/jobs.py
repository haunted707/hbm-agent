"""Cron job storage: ~/.hbm/cron/jobs.json; output in
~/.hbm/cron/output/{job_id}/{timestamp}.md"""
...
from hbm_time import get_timezone
...
# Cron is per-profile by design: anchor at get_hbm_home() (active profile home), NOT
# get_default_hbm_root() — the shared root would funnel every profile's jobs into one jobs.json
# and run them under the ticker's HBM_HOME, leaking config/credentials/skills across profiles.
# Each profile owns its own cron store under its own HBM_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same HBM_HOME — so a job authored in profile `coder` lives in
# `~/.hbm/profiles/coder/cron/jobs.json` and executes with `coder`'s `.env`, `config.yaml`, and skills.
# Do NOT change this to the default root: that re-breaks per-profile isolation. See also the dynamic
# `_get_hbm_home()` / `_get_lock_paths()` resolution in cron/scheduler.py. See #4707.
HBM_DIR = get_hbm_home().resolve()
...
CRON_DIR = HBM_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat: touched every ticker loop so `hbm cron status` can tell the ticker THREAD is alive,
# not just the gateway PROCESS; success = last tick that completed WITHOUT raising.
# The gateway process and the (separate) ``hbm cron status`` process share it so status can tell whether
...
# threshold in `hbm cron status` (hbm_cli/cron.py), so they never drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    get_hbm_home(), so re-pointing HBM_HOME after import uses ITS OWN store rather than the
...
# Fallback stale-recovery window for a one-shot's running-claim when HBM_CRON_TIMEOUT=0
...
def _oneshot_run_claim_ttl_seconds() -> float:
    """One-shot running-claim TTL from ``HBM_CRON_TIMEOUT``: unset/invalid → 600s → 1800s;
...
def _secure_dir(path: Path):
    ...
    container/HBM_HOME_MODE rules as the rest of HBM_HOME (#10757)."""
    from hbm_cli.config import _secure_dir as _shared_secure_dir
    _shared_secure_dir(path)


def _secure_file(path: Path):
    """Owner-only (0600) via the shared helper (managed/container skip included)."""
    from hbm_cli.config import _secure_file as _shared_secure_file
...
def _is_named_profile_path(path: Path) -> bool:
    """True if *path* is under ``<hbm_home>/profiles/<name>/`` (default/custom homes are not).
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # Naive timestamps become aware in the CONFIGURED HBM AGENT timezone (not server-local):
            # the due-check compares against hbm_time.now().
            ...
            if dt.tzinfo is None:
                dt = dt.replace(tzinfo=_hbm_now().tzinfo)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
...
def _ensure_aware(dt: datetime) -> datetime:
    """Aware datetime in the configured HBM AGENT timezone. Legacy naive values are read as
    ...
    if kind == "cron":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall hbm-agent or run 'pip install croniter' in your runtime env.",
                expr)
            return None
...
# --- Ticker heartbeat (liveness signal for `hbm cron status`) ---
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    tick* marker. We track two distinct signals so `hbm cron status` can tell a thread that is merely
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    critical under multiplex_profiles where ``hbm cron status`` must report per-profile liveness
    (#69377).
    """
...
def _stage_jobs_payload(jobs_file: Path, jobs: List[Dict[str, Any]]) -> str:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"jobs": jobs, "updated_at": _hbm_now().isoformat()},
    ...
    *, provider: Any, model: Any, base_url: Any, no_agent: Any,
    ...
    if normalized_provider is None:
        with contextlib.suppress(Exception):
            ...
            # Delegate all rate-limit / 5xx retry to hbm's outer conversation loop, which honors
...
def trigger_job(job_id: str, extra_prompt: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if is_terminal_job(job):
        ...
        raise ValueError(
            ...
            f"Create a new occurrence with 'hbm cron resume {name} "
            "--run-now' or '--at <ISO-8601>'.")
    manual_run_at = _hbm_now().isoformat()
    return update_job(job["id"], {
        "enabled": True,
        "state": "scheduled",
        "paused_at": None,
...
def _claim_owner_is_dead(claim: Dict[str, Any]) -> bool:
    ...
    ``_machine_id()`` stamps ``host:pid[:token]``; a foreign host, an explicit HBM_MACHINE_ID,
...
def _claim_is_live(claim: Any, now: datetime, ttl_seconds: float) -> bool:
    ...
    after the TTL (a killed ``hbm cron run`` otherwise blocks the next manual run for the
...
def note_fire_forward_failure(job_id: str, detail: str) -> bool:
    ...
    def apply(jobs, _i, job):
        job["last_fire_error"] = {
            "at": _hbm_now().isoformat(), "detail": str(detail or "")[:500]}
        save_jobs(jobs)
        return True
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    written = _write_oneshot_diagnostic(
        ...
        f"- removed at: {_hbm_now().isoformat()}\n\n"
...
def _machine_id() -> str:
    ...
    fresh-claim check): ``HBM_MACHINE_ID`` if set, else hostname:pid."""
    explicit = os.getenv("HBM_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _oneshot_dispatch_limit_reached(job: Dict[str, Any], scan: _DueScan) -> bool:
    ...
    if job.get("last_run_at") is not None:
        ...
        logger.warning(
            ...
            "'hbm cron resume <job> --run-now' (#93524).",
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if kind == "once":
        ...
        # same HBM_HOME must not re-dispatch it while in flight, and advancing next_run_at by a
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    raw_jobs = load_jobs()
    scan = _DueScan(raw_jobs, _hbm_now())
...
# the disk.
# Unlike the quick-snapshot store (`hbm_cli.backup`, capped at 20) it had no retention, so a
...
def save_job_output(job_id: str, output: str):
    ...
    output_file = job_output_dir / f"{_hbm_now().strftime('%Y-%m-%d_%H-%M-%S')}.md"
