...
A file lock (~/.hbm/cron/.tick.lock) keeps overlapping processes to one tick at a time.
"""
...
# `hbm update`) otherwise fail with ModuleNotFoundError for hbm_time et al.
...
from hbm_time import now as _hbm_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "No backup provider succeeded either."
    return (
        "No backup provider is configured — add one with `hbm fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `hbm cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    open). Raised, not returned, so ``record_ticker_error`` sees it and ``hbm cron status``
    isn't green.
    """
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # underneath it (interrupted `hbm update`, manual git pull) serves MIXED modules and every
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy: avoid heavy hbm_cli import at module load; shares MCP-membership with gateway/CLI
    from hbm_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str]:
    ...
    try:
        from hbm_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
        return sorted(_get_platform_tools(cfg or {}, "cron"))
    except Exception as exc:
        raise RuntimeError(
            "Cron toolset resolution failed, so this run was refused rather than given every "
            f"tool. Check `platform_toolsets.cron` in config.yaml (`hbm cron doctor`): {exc}"
        ) from exc
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    from hbm_constants import parse_reasoning_effort, resolve_reasoning_config
...
def get_running_job_details() -> list[dict]:
    ...
    runs). The drain wait publishes this so ``hbm update`` can say WHICH job it is waiting on."""
    now = time.time()
    with _running_lock:
        return [
...
def _inflight_min_allowance_minutes() -> float:
    ...
    with contextlib.suppress(Exception):
        ...
        if _cfg_val is not None:
            ...
            if val > 0:
                return val
    raw = cron_env_setting("HBM_CRON_INFLIGHT_MAX_MINUTES").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid HBM_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _hbm_now().isoformat()}
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_hbm_home() / "cron" / "inflight_forced_releases.jsonl"
...
def mark_running_jobs_interrupted(
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_hbm_home())
                for job_id in (
...
def _cron_inactivity_seconds() -> float:
    """Parse HBM_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600. Shared by the
    ...
    raw = cron_env_setting("HBM_CRON_TIMEOUT").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid HBM_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Per-fire usage audit log; resolves via _get_hbm_home() so profile-scoped paths work.
def _usage_audit_path() -> Path:
    return _get_hbm_home() / "cron" / "usage_audit.jsonl"
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    A cron tick can fire while the gateway is tearing down — SIGTERM from ``hbm update`` / ``hbm
...
_hbm_home: Path | None = None


def _get_hbm_home() -> Path:
    """HBM AGENT home at call time (honouring the test override). Cron is per-profile: never freeze
    ...
    resolving the active HBM_HOME at call time means a profile's jobs are stored AND executed under that
    profile's home (its .env, config.yaml, scripts, skills).
    """
    return _hbm_home or get_hbm_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hbm_home = _get_hbm_home()
    lock_dir = hbm_home / "cron"
...
def _run_no_agent_job(
    ...
    try:
        ...
        load_hbm_dotenv(hbm_home=_get_hbm_home())
    except Exception:
    ...
    now_iso = _hbm_now().strftime("%Y-%m-%d %H:%M:%S")
...
def _apply_monitor_gate(
    ...
    _mon_now = _hbm_now().strftime("%Y-%m-%d %H:%M:%S")
...
def _snapshot_pin(job: dict, axis: str, current: str, job_id: str) -> str:
    ...
    if snapshot and current and snapshot.lower() != current.lower():
        logger.info(
            ...
            "`hbm cron resnap %s` adopts the new default (stays unpinned), "
            "`hbm cron edit %s --%s <value>` or cron.%s in config.yaml pins it.",
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    creation snapshot > HBM_MODEL > config ``model:``. Re-read every tick (no cache) so
    ``hbm cron edit --model`` applies next tick."""
    model = job.get("model") or cron_env_setting("HBM_MODEL") or ""
    _cron_default_provider = ""
    _cfg: dict = {}
    _model_cfg: Any = {}
    try:
        from hbm_cli.config_effective import load_user_config_effective
        _cfg_path = str(_get_hbm_home() / "config.yaml")
            ...
            if not job.get("model"):
                if _cron_default_model:
                    ...
                    _, _global_model = resolve_cron_model_drift_defaults(
                        _cfg, environ={"HBM_MODEL": cron_env_setting("HBM_MODEL")})
    ...
    if not (isinstance(model, str) and model.strip()):
        raise RuntimeError(
            ...
            f"HBM_MODEL={cron_env_setting('HBM_MODEL')!r}, "
            "config.yaml model.default missing or empty). "
            f"Set a per-job model via "
            f"`hbm cron edit {job_id} --model <name>` or set a "
            "default with `hbm model <name>`."
        )
...
def _load_prefill_messages(cfg: dict, job_id: str) -> Optional[list]:
    ...
    prefill_file = (
        cron_env_setting("HBM_PREFILL_MESSAGES_FILE")
...
def _blocked_config_result(job_id: str, job_name: str, _pf_reason: str) -> tuple:
    ...
    blocked_doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {_hbm_now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"**Status:** BLOCKED (configuration)\n\n"
        "The pre-run configuration check found a problem, so the agent did not run "
        "(nothing was charged).\n\n"
        f"**Reason:** {_pf_reason}\n\n"
        "HBM AGENT tries again at the next scheduled time and clears this state on the first healthy "
        "run; this alert is not repeated. Check with `hbm cron doctor`. Set `cron.preflight: "
...
def _resolve_job_runtime(job: dict, job_id: str, jc: _CronJobConfig) -> tuple[dict, str]:
    ...
    try:
        # Do NOT pass HBM_INFERENCE_PROVIDER as `requested`: it would override persisted config
...
def _open_cron_session_db(job: dict):
    """Open the SQLite session store under its own timeout (HBM_CRON_TIMEOUT only watches
    ...
    # handle (#96290). Bounded with its own timeout (separate from HBM_CRON_TIMEOUT, which only watches
...
def _run_agent_with_watchdog(
    ...
    watchdog: default 600s, override HBM_CRON_TIMEOUT, 0 = unlimited."""
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if final_response.strip() and turn_exit_reason:
        ...
        _explainer_variants = []
        try:
            from hbm_state_errors import PERSISTENCE_ERROR_CAUSES as _causes
        except Exception:
...
def _finalize_cron_session(session_db, agent, job_id: str, job_name: str, cron_session_id: str) -> None:
    ...
    try:
        ...
        with contextlib.suppress((Exception, KeyboardInterrupt)):
            ...
            # HBM_SESSION_KEY is not in the CLI environment, so the key resolves empty here. Since #64240
    ...
    try:
        ...
        _cron_title = f"{_title_base} · {_hbm_now().strftime('%b %d %H:%M')}"
    ...
    # vocabulary in hbm_state's session_lifecycle_statuses docstring — keep the tuple below in sync when
...
def _prepare_job_prompt(
    ...
    # user never chose. no_agent jobs are exempt. Escape hatch: HBM_IGNORE_USER_CONFIG=1.
    if not job.get("no_agent"):
...
_CRON_DELIVERY_VARS = (
    "HBM_CRON_AUTO_DELIVER_PLATFORM",
    "HBM_CRON_AUTO_DELIVER_CHAT_ID",
    "HBM_CRON_AUTO_DELIVER_THREAD_ID")
...
class _CronRunScope:
    ...
    HBM_SESSION_* are deliberately NOT seeded from job["origin"]: it is delivery metadata, not
    a sender, and terminal/tts/skills/send_message tools would act as if the origin user were
    driving the agent. Delivery reads job["origin"] / HBM_CRON_AUTO_DELIVER_* directly.
    """
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._ctx_tokens = set_session_vars(
            ...
            # We clear the HBM_SESSION_* routing keys just below, so an async delegation's completion
            ...
            # HBM_SESSION_KEY, which risks routing a cron subagent's output into an unrelated user chat.)
        ...
        self._cron_session_var = _VAR_MAP["HBM_CRON_SESSION"]
    ...
    def enter(self) -> None:
        ...
        # HBM_KANBAN_TASK in env, and an unrelated job could close the worker's task. Must be a
...
def _reload_dotenv_and_publish_delivery_target(job: dict) -> None:
    ...
    from hbm_cli.env_loader import load_hbm_dotenv, reset_secret_source_cache
    from gateway.session_context import _VAR_MAP

    reset_secret_source_cache(_get_hbm_home())
    load_hbm_dotenv(hbm_home=_get_hbm_home())

    delivery_target = _resolve_delivery_target(job)
    if delivery_target:
        _VAR_MAP["HBM_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
        _VAR_MAP["HBM_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
        _VAR_MAP["HBM_CRON_AUTO_DELIVER_THREAD_ID"].set(
...
def run_job(
    ...
    _cron_session_id = f"cron_{job_id}_{_hbm_now().strftime('%Y%m%d_%H%M%S')}"
...
def run_one_job(
    ...
    external_owner = os.environ.get("_HBM_CRON_EXTERNAL_WORKER") == execution_id
    ...
    profile_home = _get_hbm_home().resolve()
    with _running_lock:
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(build_profile_secret_scope(_get_hbm_home()))
        ...
        _terminal_scope_token = install_profile_terminal_scope(_get_hbm_home())
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    execution_id = str(job["execution_id"])
    job_id = str(job["id"])
    handoff_dir = _get_hbm_home() / "cron" / "external-workers"
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as payload_file:
            json.dump(
                {
                    "job": job,
                    "profile_home": str(_get_hbm_home().resolve()),
    ...
    try:
        worker_env = strip_launch_profile_env(build_subprocess_env(
            ...
            extra={"HBM_HOME": str(profile_home)},
        ))
    finally:
        reset_secret_scope(secret_token)
    worker_env = systemd_user_bus_env(worker_env)
    # Unattended worker: the gateway sets HBM_EXEC_ASK at startup (interactive launches set
...
def _run_external_worker_payload(payload_path: Path, ack_path: Path) -> bool:
    ...
    home_token = set_hbm_home_override(profile_home)
    ...
    try:
        with use_cron_store(profile_home):
            ...
            try:
                ...
                return False
            old_external_execution = os.environ.get("_HBM_CRON_EXTERNAL_WORKER")
            os.environ["_HBM_CRON_EXTERNAL_WORKER"] = execution_id
            try:
                return run_one_job(job, adapters=None, loop=None, verbose=False)
            finally:
                if old_external_execution is None:
                    os.environ.pop("_HBM_CRON_EXTERNAL_WORKER", None)
                else:
                    os.environ["_HBM_CRON_EXTERNAL_WORKER"] = old_external_execution
    finally:
        reset_secret_scope(secret_token)
...
def _worktree_maintenance_repos() -> List[str]:
    """Repos whose ``.worktrees/`` to keep pruned: the hbm checkout plus job workdir repo roots,
    filtered to those that actually have a ``.worktrees/`` dir."""
    repos: set = set()

    # HBM AGENT source checkout (git installs only; wheel installs have no .git).
    with contextlib.suppress(Exception):
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    git. Same conservative pruner as ``hbm -w`` startup (dirty/unpushed/locked trees untouched).
...
def _maybe_reap_dead_owners() -> None:
    ...
    # recovery previously ran only at scheduler STARTUP. A one-shot `hbm cron run` that claimed a job and
...
def _resolve_max_parallel_workers() -> Optional[int]:
    """Max workers: env > config.yaml > unbounded (HBM_CRON_MAX_PARALLEL=1 restores serial)."""
    try:
        _env_par = cron_env_setting("HBM_CRON_MAX_PARALLEL").strip()
        if _env_par:
            return int(_env_par) or None
    except (ValueError, TypeError):
        logger.warning("Invalid HBM_CRON_MAX_PARALLEL value; defaulting to unbounded")
    with contextlib.suppress(Exception):
        _ucfg = load_config() or {}
...
def tick(
    ...
    try:
        # `hbm pause` ESTOP: skip dispatch, never touch in-flight runs; check_paused logs once.
        ...
        if not due_jobs:
            ...
            if verbose:
                ...
                # due.
                logger.info("%s - No jobs due", _hbm_now().strftime('%H:%M:%S'))
            _sweep_mcp_orphans()
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _hbm_now().strftime('%H:%M:%S'), len(due_jobs))
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            from hbm_logging import setup_logging

            setup_logging(hbm_home=_get_hbm_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
