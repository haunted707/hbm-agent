...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"HBM AGENT — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from hbm_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str, *, deferred: Optional[dict] = None) -> Optional[str]:
    ...
    try:
        source_home = get_hbm_home().resolve()
    ...
    # The running install first (same trust order as gateway.run._resolve_hbm_bin): the
    # scheduler lives in the long-running gateway, so a PATH-first lookup would hand delivery
    # to whatever `hbm` PATH names — another install, or a planted one — instead of this one.
    try:
        import importlib.util as _ilu
        found = _ilu.find_spec("hbm_cli") is not None
    except Exception:
        found = False
    if found:
        argv = [sys.executable, "-m", "hbm_cli.main"]
    else:
        hbm_bin = shutil.which("hbm-agent")
        if not hbm_bin:
            return ("HBM AGENT could not deliver this result to Bot Chat: the `hbm` command was not found. "
                    "The result is saved; run `hbm cron runs` to see it, or `hbm doctor` if this keeps happening")
        argv = [hbm_bin]
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="hbm-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
        ...
        if result.returncode != 0:
            ...
            return (
                f"HBM AGENT could not deliver this result to Bot Chat (profile '{profile_label}'). "
                "The result is saved; run `hbm cron runs` to see it, or `hbm doctor` if this keeps happening"
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`hbm cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_HBM_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `hbm cron list` shows it.
    ...
    # at boot; standalone runs (`hbm cron run`) did not, silently dropping files. Idempotent.
