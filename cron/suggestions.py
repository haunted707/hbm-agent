...
# Per-profile by design (anchored on get_hbm_home(), see cron/jobs.py). Optional test override;
# production resolves the path at CALL time so multiplexed profile ticks (set_hbm_home_override)
...
# get_hbm_home() (profile home), not the shared default root. Same pattern as cron/executions.py.
...
def _current_suggestions_file() -> Path:
    return SUGGESTIONS_FILE or (get_hbm_home().resolve() / "cron" / "suggestions.json")
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    _ensure_dir()
    payload = {"suggestions": suggestions, "updated_at": _hbm_now().isoformat()}
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _hbm_now().isoformat()
                _save_raw(suggestions)
                return True
        return False
