...
schedule via the `cronjob` tool; users via `hbm cron list|add|edit|pause|resume|run|remove` or
...
- **Inactivity watchdog** on cron agent sessions (`_cron_inactivity_seconds()`): default 600s idle,
  `HBM_CRON_TIMEOUT` overrides, `0` = unlimited. It is idle time, not wall-clock — a stalled
...
- Per-home tick lock `<home>/cron/.tick.lock` prevents duplicate ticks across processes for
  that profile's store; never a `~/.hbm/...` literal.
...
- The cron ticker runs in the desktop-spawned backend when `HBM_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `hbm kanban
...
- **CLI:** `hbm_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Toolset:** `tools/kanban_tools.py` — `kanban_show, kanban_complete, kanban_request_review,
  ...
  `kanban` (`hbm tools enable kanban --platform <p>`; default-off, in `CONFIGURABLE_TOOLSETS`) get
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/hbm-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `HBM_KANBAN_BOARD` pinned in their env and
...
- **Notifications leave through the task's owning profile.** `hbm_cli/kanban_db_notify.py`
  ...
  Dispatched workers get `HBM_KANBAN_BOARD` and the assignee's `HBM_HOME` pinned in a
...
- **Descendant fence is a path, not a flag.** A delegated child's Kanban marker
  ...
  `kanban_path_is_fenced(path)` denies mutations only on the dispatcher-pinned `HBM_KANBAN_DB`
  or under that root, so a child working against a scratch `HBM_HOME` keeps a writable board.

## Tests

`tests/cron/`, `tests/hbm_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
