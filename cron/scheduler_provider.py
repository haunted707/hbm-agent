...
def _guarded_store_write(action, description, *args, **kwargs):
    ...
    markers are diagnostics for ``hbm cron status`` — losing one write to a broken store
...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``hbm profile list`` (#47368). Filtering on directory existence leaves a deleted
...
def _profile_cron_scope(home):
    ...
    # unsuccessful (#32612).
    home_token = set_hbm_home_override(str(home))
    try:
...
def fire_overdue_jobs(
    ...
    """
    # `hbm pause` ESTOP: skip the sweep entirely. No state to unwind — the
    # next housekeeping pass after `hbm resume` catches overdue work up
    ...
    from cron.jobs import (
        ONESHOT_GRACE_SECONDS, _ensure_aware, _hbm_now, is_job_runnable, load_jobs,
    )
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # the process-global HBM_HOME (the default profile) is ticked. Heartbeats and recovery are also
        # scoped per profile so `hbm cron status` reflects liveness for every profile independently.
        ...
        try:
            ...
            # Heartbeat before the first sleep so `hbm cron status` sees a live ticker
            # immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `hbm cron status` (separate process) shows WHY.
                _guarded_store_write(
