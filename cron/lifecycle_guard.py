...
A cron job that restarts/stops the gateway from inside the gateway (``hbm gateway restart``,
``launchctl kickstart ai.hbm.gateway``, ``systemctl restart hbm-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `hbm gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `hbm` from being a path component or word tail (`/docs/hbm gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])hbm\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a hbm-gateway label so unrelated hbm services stay
    ...
    # -l ai.hbm.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a hbm-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.hbm.gateway-... \` / `  -- ...`,
...
# Branch A2: `hbm -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `hbm -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the hbm-gateway label anchor
...
_HBM_GATEWAY_LABEL_RE = re.compile(r"(?i)\bhbm[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``HBM_PROFILE_NAME``/``HBM_PROFILE`` env first, then
    ``hbm_cli.profiles.get_active_profile_name`` (from ``HBM_HOME``); ``None`` if neither."""
    for env_name in ("HBM_PROFILE_NAME", "HBM_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _HBM_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``hbm``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: hbm gateway
    ...
    # Profile-flag form (#78028): `hbm -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `hbm` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<HBM_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_hbm_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_hbm_home() falls back to Path.home(), which raises when neither HBM_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.hbm/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `hbm gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `hbm gateway restart` from a shell outside "
