"""Profile-scoped reads of the ``HBM_*`` tuning settings cron honours from ``.env``.

A standalone ``hbm -p X gateway run`` loads X's ``.env`` into ``os.environ``, so a bare
``os.getenv("HBM_CRON_TIMEOUT")`` is X's value. Under ``gateway.multiplex_profiles`` the same
...
def cron_env_setting(name: str, default: str = "") -> str:
    ...
    if scope is None:
        scope = load_env_file(get_hbm_home() / ".env")
