...
Every notice says WHAT happened and WHAT TO DO, and names the exact ``hbm cron`` command plus
...
def cron_output_dir_display(job_id: str) -> str:
    ...
    return f"{display_hbm_home()}/cron/output/{job_id}/"
...
_PROVIDER_FAILURE_ACTION: dict[str, str] = {
    "billing": (
        ...
        "`hbm cron edit {job_id} --provider <name>`."
    ),
    "auth": (
        "Sign in again with /login (or `hbm auth add <provider>` in a terminal), or pin a "
        "working provider with `hbm cron edit {job_id} --provider <name>`, then "
        "`hbm cron run {job_id}` to retry."
    ),
    "model_not_found": "Pick another model with `hbm cron edit {job_id} --model <name>`.",
    "context_overflow": "Shorten the job's prompt with `hbm cron edit {job_id} --prompt <text>`.",
}
...
_PROVIDER_FAILURE_ACTION["content_policy_blocked"] = (
    "Reword the job's prompt with `hbm cron edit {job_id} --prompt <text>`, or pick another "
    "model with `hbm cron edit {job_id} --model <name>`."
)
_DEFAULT_FAILURE_ACTION = "Run it again with `hbm cron run {job_id}`, or edit it with `hbm cron edit {job_id}`."
...
def provider_failure_notice(
    ...
    if reason in _TRANSIENT_REASONS:
        action = (
            ...
            f"`hbm cron run {job_id}` tries now."
        )
    else:
    ...
    return (
        ...
        f"Run log: `hbm cron runs {job_id}`."
    )
...
def generic_failure_notice(job_name: str, job_id: str, cleaned_error: str) -> str:
    ...
    return (
        ...
        f"See the full run with `hbm cron runs {job_id}` (output saved under "
        f"{cron_output_dir_display(job_id)}); run it again with `hbm cron run {job_id}`, "
        f"edit it with `hbm cron edit {job_id}`, or pause it with `hbm cron pause {job_id}`."
    )
...
def script_timeout_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"Check the script's output under {cron_output_dir_display(job_id)} or `hbm cron runs {job_id}`, "
        f"then run it again with `hbm cron run {job_id}`."
    )
...
def inactivity_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"{cron_output_dir_display(job_id)} (`hbm cron runs {job_id}`), then run it again with "
        f"`hbm cron run {job_id}`."
    )
...
def blocked_config_notice(job_name: str, reason: str) -> str:
    ...
    if reason and reason[-1] not in ".!?":
        reason += "."
    return (
        f"⛔ Cron '{job_name}' did not run: {reason} Nothing was charged. HBM AGENT will try again at "
