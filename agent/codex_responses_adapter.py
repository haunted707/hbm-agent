...
# long"). Codex message ids can run 400+ chars; HBM AGENT ``msg_...`` ids stay under the cap.
...
def _replay_reasoning_items(
    ...
    matching issuer. ``id`` (store=False lookups 404) and the HBM AGENT provenance fields are stripped."""
...
def _chat_messages_to_responses_input(
    ...
    decision was wrong — xAI explicitly relies on HBM AGENT threading encrypted reasoning back across turns for
    ...
    HBM AGENT' local history is never truncated by native compaction, so the full conversation is still on the
    wire.
    """
