...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into HBM AGENT accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and hbm-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior HBM AGENT deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → HBM AGENT UI bridge -------------------------------------
# The app-server bypasses the HBM AGENT tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a HBM AGENT tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping HBM AGENT' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.hbm-tools.* prefix is stripped (users see HBM AGENT tools).
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic HBM AGENT tool name for a codex item (mirrors CodexEventProjector)."""
    item_type = item.get("type") or ""
    if item_type == "mcpToolCall":
        server, tool = item.get("server") or "mcp", item.get("tool") or "unknown"
        return tool if server == HBM_TOOLS_MCP_SERVER_NAME else f"mcp.{server}.{tool}"
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: HBM AGENT' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, HBM_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # HBM AGENT' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
