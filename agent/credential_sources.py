"""Unified removal contract for every credential source HBM AGENT reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``hbm auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.hbm/.env; hint when the shell exports it."""
    from hbm_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like hbm_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to HBM AGENT.",
            f"  The pool entry is now suppressed — HBM AGENT will ignore "
            f"{env_var} until you run `hbm auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_hbm_pkce(provider: str, removed) -> RemovalResult:
    """~/.hbm/.anthropic_oauth.json is ours — delete it outright."""
    from hbm_constants import get_hbm_home

    result = RemovalResult()
    oauth_file = get_hbm_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared HBM AGENT Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `hbm model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `hbm auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `hbm auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `hbm auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="hbm_pkce",
        remove_fn=_remove_hbm_pkce,
        description="~/.hbm/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `hbm auth add qwen-oauth` to re-enable if needed.",
        ),
