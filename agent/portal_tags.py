...
Every HBM AGENT request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=hbm-agent", "client=hbm-client-v<__version__>"]``.
The version is read live from ``hbm_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("hbm_affinity_scope", default=None)
...
def hbm_client_tag() -> str:
    """``client=hbm-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if hbm_cli is unimportable)."""
    try:
        from hbm_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=hbm-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=hbm-agent", hbm_client_tag()]
