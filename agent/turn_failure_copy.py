...
_NEXT_STEPS_LOOP = (
    ...
    "If it happens again, run `hbm doctor` and share the error details."
)
...
_NONRETRYABLE_COPY: Dict[str, str] = {
    FailoverReason.model_not_found.value: (
        ...
        "(or `hbm model` in a terminal).{prefix_hint}"
    ),
    FailoverReason.format_error.value: (
        "{label} rejected this request as malformed, so the model didn't answer. Start a clean "
        "session with /new or switch models with /model; if it keeps happening, run `hbm doctor`."
    ),
    FailoverReason.ssl_cert_verification.value: (
        "HBM AGENT couldn't verify {label}'s security certificate, so the connection was refused. "
...
_AUTH_COPY: Dict[str, str] = {
    "oauth": (
        ...
        "`hbm portal` for Nous, `hbm auth add <provider> --type oauth` for other accounts."
    ),
    "api_key": (
        "{label} rejected your API key, so the model can't be reached. Update it in "
        "Settings → Providers, or run `hbm setup` in a terminal."
    ),
}
...
_FAILURE_CODE_COPY: Dict[str, str] = {
    "context_overflow": (
        "This conversation has grown too long for {model} to read, and HBM AGENT couldn't shrink "
    ...
    "truncated": (
        ...
        "HBM AGENT didn't run the incomplete action. Nothing was changed. Send `continue`, ask for "
    ...
    "loop_error": (
        "HBM AGENT hit repeated errors and stopped this turn so it wouldn't keep retrying. "
        + _NEXT_STEPS_LOOP + "\n\nDetails: {detail}"
    ),
    "interpreter_shutdown": (
        "HBM AGENT was shutting down and stopped this turn. Your conversation is saved — reopen "
...
_ONE_OFF_COPY: Dict[str, str] = {
    "payload_too_large": (
        ...
        "HBM AGENT couldn't shrink it enough automatically. Start a new session with /new (your "
    ...
    "local_processing_error": (
        "HBM AGENT hit an internal error while handling the model's reply and stopped this turn. "
        + _NEXT_STEPS_LOOP + "\n\nDetails: {detail}"
    ),
    "reasoning_only": (
    ...
    "nous_rate_limit": (
        ...
        "a backup provider with `hbm fallback add`."
    ),
}
...
def site_copy(code: str, **fields: Any) -> str:
    ...
    fields.setdefault("home", display_hbm_home())
...
def exhausted_copy(reason: str, *, label: str, attempts: int, summary: str) -> str:
    ...
    return (
        ...
        f"add a backup provider with `hbm fallback add`.\n\nProvider said: {summary}"
    )
...
def nonretryable_copy(
    ...
    body = template.format(label=label, model=model, home=display_hbm_home(), prefix_hint=prefix_hint)
    return f"{body}\n\nProvider said: {summary}"
