...
def _mint(command: str, label: str) -> tuple[str, Optional[float]]:
    ...
    provider is being minted: it gets that profile's own env (secrets + HBM_HOME), never the multiplexer's
    ...
    if stdout.lstrip().startswith("{"):
        ...
        if isinstance(payload, dict):
            ...
            # lifetime; honour it or the token 401s once past. Lazy import: hbm_cli.auth imports agent.*.
