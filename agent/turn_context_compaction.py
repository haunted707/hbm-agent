...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; HBM AGENT only
    initiates compaction in "hbm" mode."""
    return (
        # See #36801.
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping HBM AGENT preflight compression for codex app-server "
            "(mode=%s); HBM AGENT will not start thread compaction here.",
