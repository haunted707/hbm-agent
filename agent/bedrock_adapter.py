...
def scoped_aws_session_kwargs() -> Dict[str, str]:
    ...
    Under a HBM_HOME override the process env holds the LAUNCH profile's ``AWS_*`` (or nothing), so
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install HBM AGENT with Bedrock support: pip install -e '.[bedrock]'"
        )
    try:
...
def _cached_client(cache: Dict[str, Any], service: str, region: str):
    ...
    if get_hbm_home_override() is None:
        ...
        return cache[region]
    key = (hbm_home_key(), service, region)
...
def invalidate_runtime_client(region: str) -> bool:
    ...
    if get_hbm_home_override() is not None:
        return _bedrock_clients_by_home.pop((hbm_home_key(), "bedrock-runtime", region), None) is not None
...
def discover_bedrock_models(region: str, provider_filter: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    if get_hbm_home_override() is not None:
        cache_key = f"{hbm_home_key()}|{cache_key}"
