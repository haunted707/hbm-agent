...
def _constants_path(getter_name: str) -> Path:
    """Call ``hbm_constants.<getter_name>()`` (local import avoids cycles); ``~/.hbm`` on any failure."""
    try:
        import hbm_constants

        return getattr(hbm_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.hbm"))


def _hbm_home_path() -> Path:
    """Active HBM_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_hbm_home")


def _hbm_root_path() -> Path:
    """HBM AGENT root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_hbm_root")


def _hbm_dirs() -> list[Path]:
    """Resolved active HBM_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (HBM_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_hbm_home_path(), _hbm_root_path()))))
...
# they are a routine local form (see hbm_cli/windows_ssh_runtime.py) and
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    # Secret material under HBM_HOME, on both the active profile and the global
    ...
    paths = [
        ...
        *(str(base / f) for f in hbm_files for base in (_hbm_home_path(), _hbm_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved HBM_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("HBM_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# HBM_HOME / root subpaths that the agent's generic file tools must not
...
_HBM_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing", "vault", "browser-profile")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _hbm_dirs():
        for sub in _HBM_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside HBM_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under HBM_HOME / <root>. The agent never
...
# Directory-prefix read denies under HBM_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the HBM AGENT MCP token directory and cannot be read directly.",
     "is a HBM AGENT MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the HBM AGENT real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the HBM AGENT real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
    # vault.key + vault.json.enc sit side by side; key + ciphertext = plaintext, so the whole dir is one credential.
    ("vault",
     "is the HBM AGENT credential vault directory and cannot be read directly (secrets are filled server-side by browser_vault_fill).",
     "is inside the HBM AGENT credential vault (encrypted secrets + local key) and cannot be read directly (browser_vault_fill resolves them server-side)."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied HBM AGENT path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under HBM_HOME and the global root (exact files, plus anything
    ...
    hbm_dirs = _hbm_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in hbm_dirs):
        reason = (
            "is an internal HBM AGENT cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in hbm_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a HBM AGENT credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in hbm_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied HBM AGENT read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from HBM_HOME: ``~/.hbm`` -> ``"default"``,
    ``~/.hbm/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _hbm_home_path().resolve().relative_to(_hbm_root_path().resolve() / "profiles").parts
...
#   <HBM_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.hbm/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative HBM AGENT state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.hbm`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .hbm / <thing>; inner_idx = the .hbm part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".hbm"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host HBM AGENT process never reads — the "
        "authoritative file is likely {inner_path!r} under the real HBM_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.hbm/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host HBM AGENT "
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside HBM AGENT scope, or is inside
    ...
    try:
        ...
        root_real = _hbm_root_path().resolve()
