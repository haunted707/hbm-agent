...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in hbm_state.py; kept local so importing
# hbm_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for HBM AGENT tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but HBM AGENT "
        ...
        "again. A known-good starting point is 65,536 tokens. In HBM AGENT config, set "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `hbm auth reset anthropic`. Until that "
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from hbm_cli.moa_config import decode_moa_turn
...
def _run_conversation_turn(
    ...
    # Adopt ~/.hbm/.env credential/base-url edits made since the last turn — a
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('hbm_constants', 'PARTIAL_STREAM_STUB_ID'),
