...
Owns one Codex thread per HBM AGENT session: drives ``turn/start``, consumes
...
# HBM AGENT' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_HBM_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per HBM AGENT session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        self._permission_profile = permission_profile or _HBM_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("HBM_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="hbm", client_title="HBM AGENT", client_version=_get_hbm_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via HBM AGENT' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own hbm-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == HBM_TOOLS_MCP_SERVER_NAME else "decline"
...
# HBM AGENT approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a HBM AGENT approval choice onto codex's approval decision wire value."""
...
def _get_hbm_version() -> str:
    """Best-effort HBM AGENT version string for codex's userAgent line."""
    try:
