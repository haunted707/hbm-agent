...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 HBM AGENT secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 HBM AGENT secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = hbm_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # Native shell children remain unowned. Only HBM AGENT' managed MCP tool
        ...
        owned_task = os.environ.get("HBM_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "HBM_KANBAN_DB", "HBM_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.{HBM_TOOLS_MCP_SERVER_NAME}.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.{HBM_TOOLS_MCP_SERVER_NAME}.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            kanban_db = spawn_env.get("HBM_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("HBM_HOME", os.path.expanduser("~/.hbm")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("HBM_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "hbm", client_title: str = "HBM AGENT",
