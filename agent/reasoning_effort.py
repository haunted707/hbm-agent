...
HBM AGENT' internal effort ladder (``VALID_REASONING_EFFORTS`` plus ``none``) is wider than any
...
# disable can be clamped when a provider publishes it as a level. ``ultra`` is HBM AGENT-internal
...
def is_astra_model(model: Optional[str]) -> bool:
    """``gpt-6-astra`` or its HBM AGENT-side ``-900k`` picker alias, with or without a ``vendor/`` prefix.
...
def clamp_reasoning_config(reasoning_config: Optional[dict], supported: Sequence[str] = OPENAI_COMPAT_WIRE_EFFORTS) -> Optional[dict]:
    ...
    The entry clamp for an OpenAI-compatible chat-completions request builder: HBM AGENT-internal
