...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    ...
    try:
        from hbm_cli.anon_auth import route_is_welcome_host
        if route_is_welcome_host(getattr(agent, "base_url", "")):
            ...
            _plines(agent, "   Your session ended and HBM AGENT couldn't start a new one.",
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: hbm auth add nous",
        "     • Check credits / billing: https://portal.nousresearch.com",
        f"     • Verify stored credentials: {display_hbm_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `hbm doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_hbm_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for HBM AGENT-managed OAuth/setup tokens",
        ...
        "     • Legacy cleanup: hbm config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: hbm config set ANTHROPIC_API_KEY \"\"",
    )
...
def recover_after_classification(
    ...
    if (
        ...
        try:
            from hbm_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `hbm auth` to re-authenticate.",
            )
        elif provider == "xai-oauth":
            _vlines(
                agent,
                "   💡 xAI OAuth token was rejected (HTTP 401). To fix:",
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `hbm model`.",
            )
        else:  # nous
            _vlines(
                agent,
    ...
    _vlines(
        ...
        "      • Is the key valid? Run: hbm setup",
...
def _welcome_tier_guidance(classified: Any, *, model: Any, in_chat: bool, door: bool = True) -> str:
    ...
    if not refusal and not route:
        return ""
    from hbm_cli.anon_auth import welcome_refusal_copy, welcome_route_refusal_copy
    if refusal:
...
def _welcome_outage_copy(base_url: Any, classified: Any) -> str:
    ...
    try:
        from hbm_cli.anon_auth import FREE_TIER_OUTAGE_COPY, route_is_welcome_host
...
def _missing_vendor_prefix_suggestion(api_error: Exception, provider: Any, model: Any) -> Optional[str]:
    ...
    if getattr(api_error, "status_code", None) != 404:
        return None
    try:
        from hbm_cli.model_normalize import suggest_prefixed_model_id
...
def nonretryable_client_error_result(
    ...
    if _welcome_hint:
        ...
        _vlines(agent, f"   💡 Fix: pick another model (/model), or check `{display_hbm_home()}/logs/agent.log`.")
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "      To route future blocks to another provider automatically: hbm fallback add",
        )
    ...
    if classified.reason == FailoverReason.ssl_cert_verification:
        _vlines(
            agent,
            "   💡 HBM AGENT couldn't verify the provider's security certificate. This fails the same",
            ...
            "      • Missing/stale system CA store? Refresh it (in HBM AGENT's venv: `uv pip install",
