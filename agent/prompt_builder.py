...
from hbm_constants import (
    get_hbm_home, get_skills_dir, is_wsl, reset_hbm_home_override, set_hbm_home_override,
)
...
def _find_hbm_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.hbm.md`` / ``HBM.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".hbm.md", "HBM.md") if (directory / n).is_file()), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are HBM AGENT, built by Nous Research. Be direct: match the length of your reply to the weight of the ask "
...
HBM_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the hbm-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on HBM AGENT (by Nous Research). When the user needs help with HBM AGENT itself — configuring, "
    ...
    "authoritative reference and always holds the latest, most up-to-date information. The `hbm-agent` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='hbm-agent') "
    "before configuring, modifying, or troubleshooting HBM AGENT so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
HBM_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on HBM AGENT (by Nous Research). When the user needs help with HBM AGENT itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.hbm/kanban.db`. Your task id is in "
    "`$HBM_KANBAN_TASK`; your workspace is `$HBM_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $HBM_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $HBM_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${HBM_KANBAN_BRANCH:-wt/$HBM_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$HBM_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`hbm profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `hbm kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The hbm-agent runtime already executes a batch of tool
...
# to hbm-agent's Python prompt-assembly architecture.
...
# Text after the "[" that opens one of HBM AGENT' own control frames (the steer marker above, the compaction
...
# apps/desktop/src/plugins/hbm-bots/group-round-prompt.ts byte-equivalent to this list.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: HBM AGENT delivers their message as a standalone user message right after "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating HBM AGENT "
         ...
         "rather than anything inside HBM AGENT. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the HBM AGENT terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the HBM AGENT desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-hbm-send=\"prompt\" on any clickable element (or "
        "window.hbm.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
        "file, not with prose."
    ),
    "sms": (
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.hbm tree just to run `uname`,
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    cache_key = (hbm_home_key(), env_type, _tenv_read("TERMINAL_CWD", ""))
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where HBM AGENT itself is running. The host OS, "
            f"home, and cwd of the HBM AGENT process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where HBM AGENT itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: HBM_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("HBM_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# HBM AGENT runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End HBM AGENT runtime environment -->"
...
def _skills_prompt_snapshot_path() -> Path:
    return get_hbm_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("HBM_PLATFORM") or os.environ.get("HBM_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("HBM_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the HBM_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_hbm_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_hbm_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from HBM_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the HBM_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the HBM_HOME
    ...
    """
    try:
        from hbm_cli.config import ensure_hbm_home
        ensure_hbm_home()
    except Exception as e:
        logger.debug("Could not ensure HBM_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_hbm_home()) / "SOUL.md"
...
def _hbm_md_candidates(cwd_path: Path) -> list[tuple[str, Path, str]]:
    """.hbm.md / HBM.md — nearest match walking up to the git root."""
    path = _find_hbm_md(cwd_path)
    if path is None:
        return []
...
def _project_context_suppressed(cwd: Optional[str], cwd_path: Path, allow_install_tree_fallback: bool) -> bool:
    """A FALLBACK-picked cwd inside the HBM AGENT install tree must not gain system-prompt authority (the desktop
    default would load this repo's contributor AGENTS.md). An explicitly configured cwd is honored verbatim —
    the HBM AGENT tree is a legitimate workspace when the user deliberately points a session at it — and
    CLI-style surfaces pass allow_install_tree_fallback=True because their launch dir IS the user's shell cwd
    (developing HBM AGENT in-tree). See #64590."""
    from agent.runtime_cwd import _is_install_tree
    return cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path)


def _load_hbm_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.hbm.md / HBM.md — nearest match walking up to the git root."""
    for label, path, content in _hbm_md_candidates(cwd_path):
        if content:
            return _context_section(_strip_yaml_frontmatter(content), label, ".hbm.md", path, context_length)
    return ""
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .hbm.md/HBM.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from HBM_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    ...
    if _project_context_suppressed(cwd, cwd_path, allow_install_tree_fallback):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the HBM AGENT "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_hbm_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
