...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        for raw_schema in schemas:
            ...
            if tool_name in _HBM_CORE_TOOLS:
                logger.warning(
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        def _collect(provider: MemoryProvider) -> None:
            for raw_schema in provider.get_tool_schemas():
                ...
                elif schema["name"] not in _HBM_CORE_TOOLS and schema["name"] not in seen:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        """Initialize all providers, injecting ``hbm_home`` so they resolve profile-scoped paths."""
        if "hbm_home" not in kwargs:
            from hbm_constants import get_hbm_home
            kwargs["hbm_home"] = str(get_hbm_home())
