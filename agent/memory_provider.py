...
def ctx_bound(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    is a ContextVar-scoped HBM_HOME override plus the per-turn secret scope; a worker started
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include ``hbm_home`` (profile-scoped storage; never hardcode
        ``~/.hbm``) and ``platform``; may include ``agent_context`` ("primary" |
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        """Setup fields for ``hbm memory setup`` ([] if none): ``key``, ``description``,
    ...
    def backup_paths(self) -> List[str]:
        """Absolute paths of provider state OUTSIDE HBM_HOME for ``hbm backup``/``import``
