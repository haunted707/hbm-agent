...
``hbm pause`` writes a sentinel at ``$HBM_HOME/ESTOP``; ``hbm resume``
...
(fail safe, e.g. ``touch ~/.hbm/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.hbm).
from agent.file_safety import _hbm_home_path as _hbm_home, _hbm_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `hbm pause`."""
    return _hbm_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (HBM_HOME=~/.hbm/profiles/<n>) must still honor an operator's ~/.hbm/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ HBM AGENT is paused{tag}. New work is on hold; run `hbm resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `hbm resume` (%s)",
