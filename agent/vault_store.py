...
- Follows the repo's "default frictionless, 0600 files OK" policy: the key
  file and vault file are created 0600 under ``<HBM_HOME>/vault/``.
...
# browser fill targets (agent/vault_login_classifier.py); the Desktop Add dialog and `hbm vault add`
...
class VaultStore:
    """Encrypted, profile-scoped vault under ``<HBM_HOME>/vault/``."""

    def __init__(self, base_dir: Optional[Path] = None):
        self._base = Path(base_dir) if base_dir is not None else (
            Path(get_hbm_home()) / "vault"
        )
    ...
    def _ensure_dir(self) -> None:
        ...
        # group-share mode and HBM_UID/GID ownership) rather than a
        # bespoke chmod — same requirement as the browser-profile snapshot
        # dir (f1d05c review).
        try:
            from hbm_cli.config import _secure_dir
    ...
    def _locked(self):
        ...
        `hbm vault add` and a TUI slash worker all write the same ``vault.json.enc``; two unlocked
