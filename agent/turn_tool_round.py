...
def run_tool_round(
    ...
    HBM AGENT; a failed canonical append ends the turn rather than running tools from
    process-only state."""
    ...
    # block if a destructive tool restarts HBM AGENT.
    try:
    ...
    # the gateway inactivity timeout (HBM_AGENT_TIMEOUT).
    ...
    # follow-up API call can exceed the gateway inactivity timeout (HBM_AGENT_TIMEOUT, default 1800s) and
