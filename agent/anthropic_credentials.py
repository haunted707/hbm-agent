...
``ANTHROPIC_API_KEY``, HBM AGENT-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.hbm/.anthropic_oauth.json`` (HBM AGENT PKCE) and
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by HBM AGENT so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".hbm-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "hbm_pkce": lambda: _get_hbm_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from hbm_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("HBM_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: HBM AGENT historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``hbm models``) never mutate
...
# ── HBM AGENT-native PKCE OAuth flow (~/.hbm/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_hbm_oauth_file() -> Path:
    return get_hbm_home() / ".anthropic_oauth.json"


def _root_hbm_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        root = get_default_hbm_root()
        return None if root.resolve(strict=False) == get_hbm_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_hbm_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run HBM AGENT-native OAuth PKCE flow and return credential state."""
    import webbrowser
    ...
    print("\n".join([
        "", "Authorize HBM AGENT with your Claude Pro/Max subscription.", "",
    ...
    try:
        from hbm_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_hbm_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read HBM AGENT-managed OAuth credentials from ~/.hbm/.anthropic_oauth.json."""
    data = _load_json_if_exists(_get_hbm_oauth_file(), "HBM AGENT OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_hbm_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int], *, target: Optional[Path] = None
) -> None:
    """Commit refreshed hbm_pkce tokens to ~/.hbm/.anthropic_oauth.json (``CredentialPersistError`` on failure).
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to the ROOT singleton
    instead of forking a copy under its own HBM_HOME; without this write-through the next ``load_pool()``
    re-seeds the stale (consumed) pair from the file over the rotated pool entry."""
    _commit_private_json(
        target if target is not None else _get_hbm_oauth_file(),
        {"accessToken": access_token, "refreshToken": refresh_token, "expiresAt": expires_at_ms},
        "HBM AGENT OAuth credentials",
    )
