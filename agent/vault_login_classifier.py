...
INSPECTION_STAMP_ATTR = "data-hbm-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-hbm-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-hbm-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-hbm-vault-slot]").forEach((n) => n.removeAttribute("data-hbm-vault-slot"));
