...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: ``HBM_VERIFY_ON_STOP`` env var, then ``agent.verify_on_stop``
    ...
    """
    env = os.environ.get("HBM_VERIFY_ON_STOP")
    if env is not None:
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether ``hbm verify`` has a runtime recipe here: a saved
    ``.hbm/environment.json`` or a statically detected recipe with a start
    ...
    *, session_id: str | None, changed_paths: Iterable[str], attempts: int=0, max_attempts: int=2,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `hbm-verify-` filename prefix, run it "
