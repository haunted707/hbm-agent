...
Installs go to a HBM AGENT-owned staging dir, ``<HBM_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``hbm lsp status`` reports it).
...
# ``bin`` in ``<HBM_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `hbm lsp status` reports its presence.
...
def hbm_lsp_bin_dir() -> Path:
    """Return the HBM AGENT-owned bin staging dir for LSP servers."""
    from hbm_constants import get_hbm_home

    p = get_hbm_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(hbm_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = hbm_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $HBM_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that HBM AGENT installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = hbm_lsp_bin_dir().parent  # <HBM_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = hbm_lsp_bin_dir().parent / "python-packages"
    pip_target.mkdir(parents=True, exist_ok=True)
    try:
        logger.info("[install] pip install --target %s %s", pip_target, pkg)
        from hbm_cli.tools_config import _pip_install
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``hbm lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "hbm_lsp_bin_dir"]
