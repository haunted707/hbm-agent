...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<HBM_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_hbm_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <HBM_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = hbm_lsp_session_dir()
    inner = (
        ...
        f"-HostName HBM AGENT -HostProfileId hbm -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
...
def hbm_lsp_session_dir() -> str:
    """Return (and create) the dir for PSES session/log scratch files."""
    from hbm_constants import get_hbm_home
    d = os.path.join(str(get_hbm_home()), "lsp", "pses")
