"""Language Server Protocol (LSP) integration for HBM AGENT.
...
# Routed multiplex profiles (HBM_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``hbm chat`` exit leaks pyright processes for a few seconds
