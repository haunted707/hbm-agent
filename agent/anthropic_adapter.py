"""Anthropic Messages API adapter: client construction + the Messages call for HBM AGENT's
...
# HBM AGENT effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain HBM AGENT tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://hermes-agent.nousresearch.com", "X-Title": "HBM AGENT",
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    hbm's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ANTHROPIC_AUTH_TOKEN in the environment (both loaded from ~/.hbm/.env) and then sends dual
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to hbm's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    HBM AGENT tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("HBM AGENT", "Claude Code"), ("HBM AGENT", "Claude Code"),
    ("hbm-agent", "claude-code"), ("Nous Research", "Anthropic"),
)
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning HBM AGENT shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that HBM AGENT surfaces in its CLI. We request "summarized" so the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_hbm_home': ('hbm_constants', 'get_hbm_home'),
    ...
    'read_hbm_oauth_credentials': ('agent.anthropic_credentials', 'read_hbm_oauth_credentials'),
    ...
    'run_hbm_oauth_login_pure': ('agent.anthropic_credentials', 'run_hbm_oauth_login_pure'),
