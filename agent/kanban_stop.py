...
HBM AGENT treats that as a clean exit → ``rc=0`` → dispatcher ``protocol_violation``.
...
def kanban_stop_nudge_enabled() -> bool:
    """On when ``HBM_KANBAN_TASK`` is set for the dispatcher-owned worker, unless
    ``HBM_KANBAN_STOP_NUDGE`` disables it. In-process delegate_task children and cron runs
    inherit the env var but own no board task and carry no kanban toolset."""
    if (os.environ.get("HBM_KANBAN_STOP_NUDGE") or "").strip().lower() in {"0", "false", "no", "off"}:
        return False
    return bool((os.environ.get("HBM_KANBAN_TASK") or "").strip()) and is_dispatcher_owned_worker_context()
...
def build_kanban_stop_nudge(
    *,
    ...
    tid = (task_id or os.environ.get("HBM_KANBAN_TASK") or "").strip() or "this task"
    return (
        "[System: You are a HBM AGENT kanban worker. A plain-text reply is NOT a "
