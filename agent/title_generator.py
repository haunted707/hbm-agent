...
# HBM AGENT' own machine-authored openers: a compaction handoff or resumed session must not be titled after them.
_MACHINE_PREFIXES = (
...
def _title_config() -> dict:
    """``auxiliary.title_generation`` (lazy read-only import: no hbm_cli cycle, no migration writes)."""
...
def auto_title_session(
    ...
    trigger is the post-``hbm update`` window where lazy imports read NEW source against OLD modules."""
    try:
        ...
        logger.warning("Auto-title failed (harmless; if this started after an update, restart the running HBM AGENT process): %s", e)
...
def _is_real_user_turn(message: Any) -> bool:
    """A question a person actually asked (HBM AGENT persists machinery under ``role="user"``)."""
...
def _kanban_task_title() -> Optional[str]:
    ...
    if not task_id:
        return None
    try:
        from hbm_cli import kanban_db, kanban_db_connect
