"""OpenAI-shape bridge shared by HBM AGENT' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so HBM AGENT' tool schemas travel INTO the
...
read/edit/execute tools forwards only HBM AGENT' agent-level tools, since re-offering overlapping ones
makes HBM AGENT redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``hbm_projected_messages``)
    HBM AGENT reads off the ``create`` result; a plain list would drop them on the stream path."""
