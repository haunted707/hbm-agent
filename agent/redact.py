...
# `export HBM_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in hbm_cli/main.py, gateway/run.py, and cli.py) or `HBM_REDACT_SECRETS=false` in
# ~/.hbm/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("HBM_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a HBM_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in hbm_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    try:
        ...
        raw = scope.get("HBM_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``hbm config`` / ``status`` / ``dump``):
...
# a shell rc/profile, HBM AGENT' own ``config.yaml`` where ``hbm mcp add --env`` writes
...
_HBM_HOME_PREFIXES = ("$HBM_HOME/", "${HBM_HOME}/")
# ``$HOME/.hbm/config.yaml`` keeps the ``.hbm`` segment, so stripping the prefix is
...
def _is_under_hbm_home(path: str) -> bool:
    """True when an absolute ``config.yaml`` path sits under the active HBM AGENT home or root.

    The default home's basename is an installation detail — ``.hbm`` on POSIX, ``hbm``
    under ``AppData/Local`` on Windows — and a resolved path never spells ``$HBM_HOME``,
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.hbm`` directory, ``$HBM_HOME``, or the resolved HBM AGENT home (never arbitrary
    YAML). The resolved-home arm is what covers native Windows, where the home directory
    is ``%LOCALAPPDATA%\\hbm`` and carries no ``.hbm`` segment."""
    path = arg.strip("\"'").replace("\\", "/")
    hbm_home = False
    for prefix in _HBM_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    # ``config.yaml`` plus the ``config.yaml.good.<stamp>`` / ``.corrupt.<stamp>`` copies HBM AGENT
    ...
    if parts[-1] != "config.yaml" and not parts[-1].startswith(("config.yaml.good.", "config.yaml.corrupt.")):
        return False
    return hbm_home or ".hbm" in parts[:-1] or _is_under_hbm_home(path)
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, HBM AGENT ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
