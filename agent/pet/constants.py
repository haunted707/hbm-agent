...
inferred from the atlas shape so HBM AGENT renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (HBM AGENT names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy HBM AGENT/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical HBM AGENT names -> accepted row-name aliases in descending preference.
