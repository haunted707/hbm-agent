...
A HBM AGENT process may itself be a Kanban dispatcher worker with HBM_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("hbm_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("hbm_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "HBM_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "HBM_KANBAN_TASK", "HBM_KANBAN_RUN_ID", "HBM_KANBAN_CLAIM_LOCK",
    "HBM_KANBAN_GOAL_MODE", "HBM_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``HBM_KANBAN_*`` identity gate should use."""
...
def _fenced_kanban_root() -> str:
    ...
    try:
        from hbm_cli.kanban_db import kanban_home
...
def scrub_kanban_env(env: Mapping[str, str] | MutableMapping[str, str]) -> dict[str, str]:
    ...
    against a temp ``HBM_HOME`` got a silently read-only board there. An inherited
    path-valued marker is kept (a grandchild that moved HBM_HOME must not re-fence
...
def kanban_path_is_fenced(path: "os.PathLike[str] | str") -> bool:
    ...
    descendant only when *path* is the dispatcher-pinned ``HBM_KANBAN_DB`` or lies under the
    ...
    pinned = os.environ.get("HBM_KANBAN_DB", "").strip()
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("HBM_KANBAN_TASK")
            or (env and (env.get("HBM_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
