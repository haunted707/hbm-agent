...
thread refreshes) → disk cache (~/.hbm/models_dev_cache.json, any age) →
...
from hbm_constants import openrouter_variant_base
...
# HBM AGENT provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # opencode-free is Zen-hosted (hbm_cli/models.py) and models.dev's "opencode" catalog lists
    ...
    # HBM AGENT provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → HBM AGENT ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_hbm_ids(mdev_id: str) -> List[str]:
    """Return the HBM AGENT provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for hbm_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(hbm_id)
...
def _hbm_path(name: str) -> Path:
    from hbm_constants import get_hbm_home
    return get_hbm_home() / name


def _get_cache_path() -> Path:
    return _hbm_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _hbm_path("models_dev_cache.etag")
...
def _save_etag(etag: str) -> None:
    def write() -> None:
        ...
        mkdir_under_hbm_home(etag_path.parent)
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a HBM AGENT provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the HBM AGENT
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by HBM AGENT OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (HBM AGENT → models.dev id) and reverse (caller passed a models.dev id, config keyed by HBM AGENT id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_hbm_ids(provider_key)]
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by HBM AGENT or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by HBM AGENT or models.dev provider ID (exact match, then case-insensitive), or
