...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("hbm-verify-", "hbm-ad-hoc-")
...
def _db_path() -> Path:
    return get_hbm_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "hbm verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``hbm verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="hbm verify", kind="verify",
