...
# applied by hbm_cli/commands_platforms.py, not here.
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    try:
        ...
        resolved_platform = os.getenv("HBM_PLATFORM") or get_session_env("HBM_SESSION_PLATFORM")
    except Exception:
        resolved_platform = os.getenv("HBM_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Effective HBM AGENT home the scan is scoped to (profiles carry their own
    ...
    ``set_hbm_home_override``), but the module-level scan only tracked
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    """Append a ``[Skill config: ...]`` block with resolved ``metadata.hbm.config``
    ...
    try:
        ...
        if not resolved:
            return
        parts.append("")
        parts.append(f"[Skill config (from {display_hbm_home()}/config.yaml):")
...
def _scan_skill_md(skill_md: Path, disabled: set, seen_names: set, commands: Dict[str, Dict[str, Any]], resolve_command) -> None:
    ...
    if resolve_command(cmd_name) is not None:
        logger.warning("Skill %r generates slash command '/%s' which collides with a core HBM AGENT command; "
...
def build_preloaded_skills_prompt(
    ...
    this path bypasses the scan-time filter, and ``hbm -s <skill>`` must not
...
def build_auto_load_prompt(
    ...
    *home_override* makes home resolution EXPLICIT (same seam as ``build_skills_system_prompt``): the config,
    ...
    that lost the HBM_HOME ContextVar cannot pin the launch profile's skills into another profile's prompt.
    """
    from hbm_constants import reset_hbm_home_override, set_hbm_home_override
    home_token = set_hbm_home_override(str(home_override)) if home_override is not None else None
    try:
