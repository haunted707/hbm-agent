"""Upload a HBM AGENT session transcript to Hugging Face as an agent trace, re-emitted in the **Claude Code
...
DEFAULT_DATASET_NAME = "hbm-traces"
_HBM_VERSION = "hbm-agent"
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.hbm/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `hbm trace upload`)."
)
...
# --- Conversion: HBM AGENT OpenAI-format messages -> Claude Code JSONL ---
...
def build_trace_jsonl(messages: List[Dict[str, Any]], *, session_id: str, model: str = "", cwd: str = "", redact: bool = True) -> str:
    ...
    for msg in messages:
        ...
        entry = {  # key order is the wire order
            ...
            "sessionId": session_id, "version": _HBM_VERSION, "gitBranch": git_branch, "uuid": turn_uuid,
...
def upload_session_trace(
    ...
    """CLI/gateway entry point: load, convert, upload to ``{user}/hbm-traces``. Status string, never raises."""
    if not session_id:
