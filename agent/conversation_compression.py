...
# Defaults for the in-agent progress-aware wrap; mirror hbm_cli.config.DEFAULT_CONFIG["compression"] keys.
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    Only the exact old ``hbm_state.SessionDB`` class (hot-reload skew) may fail open; proxies, lookalikes,
...
def _rebind_session_context(session_id: str) -> None:
    ...
    with contextlib.suppress(Exception):
        from hbm_logging import set_session_context
...
def _lower_threshold_to_aux_context(
    ...
    if threshold_suggestion_viable:
        ...
        msg += (
            ...
            f"HBM AGENT's small-context floor and output reservation would recompute the trigger to "
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    "⚠ No auxiliary LLM provider configured: HBM AGENT has no helper model for summarising "
                    "long chats, so older messages will be cut without a summary. Run `hbm setup` to add one."
                )
            agent._compression_warning = msg
            agent._emit_status(msg)
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by HBM AGENT "
        ...
        if _lock_lookup_error is not None:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "in-memory module after an update; restart the process (or `hbm update`) to resync.",
                    _lock_sid,
                )
...
def _parent_deliberately_ended(session_db: Any, session_id: str) -> bool:
    ...
    if not callable(reader):
        return False
    try:
        from hbm_state_common import is_automatic_end_reason
...
def _carry_session_state_to_child(agent: Any, old_session_id: str, old_title: Any) -> None:
    ...
    with _swallow('Could not migrate goal on compression: %s'):
        ...
        from hbm_cli.goals import migrate_goal_to_session
        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
    with _swallow('Could not migrate heartbeat on compression: %s'):
        from hbm_cli.heartbeat import migrate_heartbeat_to_session
...
def _publish_rotated_compaction(
    ...
    # publish also COALESCEs from the parent row for threads lacking HBM_HOME.
    ...
    agent._session_db.publish_compression_child(
        ...
        source=agent.platform or os.environ.get("HBM_SESSION_SOURCE", "cli"), model=agent.model,
...
def _warn_summary_or_aux_fallback(agent: Any) -> None:
    ...
    if summary_error:
        ...
        if _aux_fail_model and getattr(agent, "_last_aux_fallback_warning_key", None) != _aux_key:
            ...
            agent._emit_warning(
                f"ℹ Configured compression model '{_aux_fail_model}' failed, so HBM AGENT summarised "
...
def compress_context(
    ...
    # compression.codex_app_server_auto). Memory handoff is HBM AGENT-only: no native
...
def _compress_context_via_codex_app_server(
    ...
    HBM AGENT' transcript is left unchanged."""
    ...
    if auto_mode not in {"native", "hbm", "off"}:
        auto_mode = "native"
    skip_reason = None
    if not force and auto_mode != "hbm":
...
def _shrink_data_url(url: str, *, max_dimension: int, resize_fn: Any) -> tuple:
    ...
    try:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="hbm_shrink_", suffix=_IMAGE_SUFFIX_BY_MIME.get(mime, ".jpg"), delete=False
        )
        try:
            tmp.write(raw)
            tmp.close()
