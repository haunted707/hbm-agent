...
``## label`` section, so a .hbm.md whose YAML frontmatter the builder strips can read a few chars larger
...
_STATUS_DISPLAY = {
    ...
    "suppressed": ("○", "not loaded — cwd fell back to the HBM AGENT install tree"),
}
...
def list_context_file_sources(
    ...
    """One dict per context file HBM AGENT considered, in the builder's priority order.
    ...
    if not skip_soul:
        home = Path(home_override) if home_override is not None else _pb.get_hbm_home()
