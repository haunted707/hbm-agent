...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from hbm_cli.moa_config import coerce_privacy_filter
...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    # hbm_home_key() in the key: the resolved api_key/base_url are per-profile, and under a
    # multiplex gateway two profiles can share (provider, model) with different accounts.
    from hbm_constants import hbm_home_key
    cache_key = (hbm_home_key(), provider, model)
    now = time.monotonic()
...
def aggregate_moa_context(
    *, user_prompt: str, api_messages: list[dict[str, Any]], reference_models: list[dict[str, Any]],
    ...
    synth_prompt = (
        ...
        "HBM AGENT. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal HBM AGENT loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
