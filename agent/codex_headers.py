...
def codex_cloudflare_headers(access_token: str, *, base_url: str = CODEX_AUX_BASE_URL) -> Dict[str, str]:
    ...
    endpoint gets HBM AGENT' originator and version, custom endpoints keep the
    ...
    if is_official_codex_base_url(base_url):
        ...
        headers = {"User-Agent": f"HbmAgent/{__version__}", "originator": "hbm-agent"}
    else:
        headers = {"User-Agent": "codex_cli_rs/0.0.0 (HBM AGENT)", "originator": "codex_cli_rs"}
