...
_OBSERVABLE_METRIC_NAMES = (
    "hbm.gateway.up", "hbm.gateway.state", "hbm.gateway.active_agents", "hbm.gateway.busy",
    "hbm.gateway.drainable", "hbm.gateway.restart_requested", "hbm.gateway.background_work",
    "hbm.gateway.background_delegations", "hbm.platform.up", "hbm.platform.degraded",
    "hbm.cron.scheduler.heartbeat_age_seconds", "hbm.cron.scheduler.last_success_age_seconds",
    "hbm.cron.scheduler.catch_up_occurrences", "hbm.cron.jobs.enabled", "hbm.cron.jobs.running",
    "hbm.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="hbm-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("hbm.gateway.background_work", _read_background_work_count),
            ("hbm.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("hbm.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("hbm.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'hbm-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="hbm-gateway-health-export", daemon=True)
            thread.start()
