...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "hbm.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("hbm.gateway.up", int(bool(gateway_running))),
        metric("hbm.gateway.active_agents", active_agents),
        metric("hbm.gateway.busy", int(bool(busy))),
        metric("hbm.gateway.drainable", int(bool(drainable))),
        metric("hbm.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("hbm.gateway.state", 1, **{"hbm.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"hbm.platform": str(platform), "hbm.platform.state": state}
        metrics.append(metric("hbm.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("hbm.platform.degraded", int(is_degraded), **pattrs, **{"hbm.error_code": error_code}))
        if is_degraded:
