...
a cache problem must never block HBM AGENT startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_hbm_dotenv()``, else ``$HBM_HOME``/``~/.hbm``."""
    if home_path is None:
        from hbm_constants import get_hbm_home

        home_path = get_hbm_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<hbm_home>/cache/<basename>``::
