...
Pulls API keys from BSM at startup so they need not live in ``~/.hbm/.env``.
``bws`` is auto-installed into ``<hbm_home>/bin/bws`` (one pinned version,
...
# <hbm_home>/cache/bws_cache.json holds only secret VALUES (never the access
...
_ENCRYPTED_CACHE_INFO = b"hbm-bws-encrypted-cache-v1"
...
def _hbm_bin_dir() -> Path:
    """Where HBM AGENT stores its managed binaries. Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<hbm_home>/bin/bws`` first, then PATH, then optional auto-install."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="hbm-bws-") as tmpdir:
        tmp = Path(tmpdir)
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `hbm secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `hbm secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `hbm secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_hbm_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `hbm secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `hbm secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `hbm secrets bitwarden setup` to install."
        )
        return result
