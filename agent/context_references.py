...
from hbm_cli._subprocess_compat import IS_WINDOWS, harden_git_argv, noninteractive_git_env, windows_hide_flags
...
_SENSITIVE_HBM_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    home, hbm_home = Path(os.path.expanduser("~")).resolve(), get_hbm_home().resolve()
    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES} | {hbm_home / ".env"}
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS] + [hbm_home / rel for rel in _SENSITIVE_HBM_DIRS]
    if path in blocked_exact:
        raise ValueError("path is a sensitive credential file and cannot be attached")
    if any(_is_under(path, blocked_dir) for blocked_dir in blocked_dirs):
        raise ValueError("path is a sensitive credential or internal HBM AGENT path and cannot be attached")
