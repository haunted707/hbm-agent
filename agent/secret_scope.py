...
def serves_routed_profile() -> bool:
    ...
    multiplexing, else when a HBM_HOME override names another home (dashboard/desktop backend,
    ...
    if is_multiplex_active():
        return True
    from hbm_constants import get_hbm_home_override, get_process_hbm_home, hbm_home_key
    override = get_hbm_home_override()
    return override is not None and hbm_home_key(override) != hbm_home_key(get_process_hbm_home())
...
class UnscopedSecretError(RuntimeError):
    ...
    def __init__(self, secret_name: str = "", developer_detail: str = ""):
        ...
        super().__init__(
            f"HBM AGENT could not read {what} (an internal profile-scoping bug on the multiplexed "
            "gateway, not your configuration). Run `hbm gateway restart`; if it keeps happening, "
            "report it with `hbm debug share`."
        )
...
_GLOBAL_ENV_EXACT = frozenset({
    # HBM AGENT runtime / deployment
    "HBM_HOME", "HBM_PROFILE", "HBM_GATEWAY_LOCK_DIR",
    "HBM_MAX_ITERATIONS", "HBM_API_TIMEOUT",
    "HBM_REDACT_SECRETS", "HBM_NOUS_TIMEOUT_SECONDS",
    "_HBM_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "HBM_KANBAN_DB", "HBM_KANBAN_WORKSPACES_ROOT", "HBM_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "HBM_KANBAN_",
    "HBM_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset HBM AGENT writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``hbm_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(hbm_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(hbm_home) / ".env")
    try:
        from hbm_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(hbm_home))
    except Exception:
        external_secrets = {}
    ...
    if bridged is not None and _is_process_home(hbm_home):
        secrets.setdefault("GATEWAY_ALLOW_ALL_USERS", bridged)
    return secrets


def _is_process_home(hbm_home: Path) -> bool:
    from hbm_constants import get_process_hbm_home
    try:
        return Path(hbm_home).resolve() == get_process_hbm_home().resolve()
    except OSError:
        return False
