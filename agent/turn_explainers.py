...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "turn_lease": (
        "the turn was stopped because another HBM AGENT process "
    ...
    "locked": (
        ...
        "(another HBM AGENT process was writing to the state "
    ...
    # logger.error at hbm_state.py::_raise_if_db_replaced — never in the chat reply.
    "replaced": (
        "the session database file was replaced while HBM AGENT was running, so this "
        "message was not saved (a copy is kept in {home}/sessions/). Stop HBM AGENT "
        "(`hbm gateway stop`), run `hbm doctor` — not `hbm doctor --fix`, which "
    ...
    "deleted_wal": (
        "the session database was changed or replaced while HBM AGENT was running, so this "
        "message was not saved (a copy is kept in {home}/sessions/). Stop HBM AGENT "
        "(`hbm gateway stop`), run `hbm doctor`, then start it again and send your "
    ...
    "corrupt": (
        ...
        "1. Run `hbm {profile_arg}doctor --fix`\n"
        "2. Stop the gateway, then recover with:\n"
        "   hbm {profile_arg}sessions recover --source {db_path} --inspect-only\n"
        "   (if it reports recoverable) hbm {profile_arg}sessions recover "
    ...
    "fts_index": (
        ...
        "recovery tools or restore a backup. Run `hbm {profile_arg}doctor --fix` "
        "(or restart HBM AGENT, which repairs the index on open), then "
        "send your message again."
    ),
    "disk": (
        "HBM AGENT couldn't save this conversation to disk, so it stopped rather than lose "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    "HBM AGENT couldn't save this conversation, so it stopped rather than lose your messages. "
    "Possible causes: the drive is out of room, or another HBM AGENT process is holding the "
    "database. Close other HBM AGENT windows, run `hbm doctor` to check storage, then send "
    "your message again."
)
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``hbm_cli.config``). Any failure → True (safe default: on)."""
    try:
        ...
        if cached is not None:
            return cached
        try:
            from hbm_cli.config import load_config as _load_config
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        *, task_id: Optional[str] = None,
        ...
        if landed:
            ...
            # HBM AGENT-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``HBM_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="HBM_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``HBM_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="HBM_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                # the process default, and a bare `hbm` follows active_profile (#105887).
                ...
                body = body.replace(
                    "{backups_dir}", str(get_default_hbm_root() / "backups")
                )
