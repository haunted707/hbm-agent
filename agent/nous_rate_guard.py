...
retries x 3 HBM AGENT retries), all counted against RPH.
"""
...
def _state_path() -> str:
    ...
    try:
        ...
        base = os.path.join(os.path.expanduser("~"), ".hbm")
...
def is_long_welcome_rate_limit(error_context: Any) -> bool:
    ...
    (``welcome_refusal`` from ``hbm_cli.anon_auth.parse_welcome_refusal``). Capacity refusals
