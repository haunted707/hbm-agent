...
try:
    import hbm_cli as _hbm_cli

    _HBM_VERSION = str(_hbm_cli.__version__)
except Exception:
    _HBM_VERSION = "0.0.0"
_API_CLIENT = f"hbm-agent/{_HBM_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "HBM AGENT typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.hbm/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.hbm/.env (get one at https://aistudio.google.com/app/apikey), or run `hbm setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with HBM AGENT retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No HBM_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
