...
YAML files in ``<HBM_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``HBM_BUNDLES_DIR`` override (tests) or ``<HBM_HOME>/skill-bundles``."""
    override = os.environ.get("HBM_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_hbm_home() / "skill-bundles"
...
# File-level CRUD — used by `hbm bundles`.
