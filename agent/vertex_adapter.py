...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed HBM AGENT at Vertex.

    Stricter than :func:`has_vertex_credentials`: an ambient ``GOOGLE_APPLICATION_CREDENTIALS``
    must NOT gate the model picker open (unknowing spend). Only HBM AGENT-scoped signals count.
    """
