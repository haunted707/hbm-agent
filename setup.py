...
HBM AGENT (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``HBM_NIX_BUILD=1`` on the
HBM AGENT package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("HBM_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for hbm-agent is not supported.\n"
    "HBM AGENT is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the HBM AGENT Nix derivation sets HBM_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``hbm_state``, ``toolsets``...)
