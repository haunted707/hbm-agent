"""Migrate legacy ``HBM_NEMO_RELAY_ATIF_*`` / ``ATOF_*`` exporter vars into a Relay ``plugins.toml``.

The Relay cutover (Aug 2026) stopped honouring the legacy exporter variables: a profile that still
carries them and no ``HBM_NEMO_RELAY_PLUGINS_TOML`` logs one warning and initialises NO exporters,
...
the file is exactly what Relay validates), points ``HBM_NEMO_RELAY_PLUGINS_TOML`` at it, and
comments the legacy lines out. It runs from ``hbm-agent update`` for every profile home and from
``hbm-agent relay migrate`` for the active one.
"""
...
from hbm_cli.relay_plugin_cutover import (
...
def relay_plugin_payload_from_legacy_env(env: Mapping[str, Any]) -> dict[str, Any]:
    ...
    atif = atof = None
    if _truthy(env.get("HBM_NEMO_RELAY_ATIF_ENABLED")):
        kwargs: dict[str, Any] = {
            "enabled": True,
            "agent_name": _s(env, "HBM_NEMO_RELAY_ATIF_AGENT_NAME", "HBM AGENT"),
            "model_name": _s(env, "HBM_NEMO_RELAY_ATIF_MODEL_NAME", "unknown"),
            "filename_template": _s(env, "HBM_NEMO_RELAY_ATIF_FILENAME_TEMPLATE", "hbm-atif-{session_id}.json"),
        }
        if _s(env, "HBM_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"):
            kwargs["output_directory"] = _s(env, "HBM_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY")
        if _s(env, "HBM_NEMO_RELAY_ATIF_AGENT_VERSION"):
            kwargs["agent_version"] = _s(env, "HBM_NEMO_RELAY_ATIF_AGENT_VERSION")
        atif = obs.AtifConfig(**kwargs)
    if _truthy(env.get("HBM_NEMO_RELAY_ATOF_ENABLED")):
        mode = _s(env, "HBM_NEMO_RELAY_ATOF_MODE", "append").lower()
        sink = obs.AtofFileSinkConfig(
            output_directory=_s(env, "HBM_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY") or None,
            filename=_s(env, "HBM_NEMO_RELAY_ATOF_FILENAME", "hbm-atof.jsonl"),
...
def migrate_profile_relay_env(home: Path, *, validate: bool = True) -> RelayMigrationResult:
    ...
    from hbm_cli.config import _env_line_defines_key, _quote_env_value, _read_env_lines, _write_env_lines
    ...
    if env.get(RELAY_PLUGINS_CONFIG_ENV, "").strip():
        ...
        return result
    if not (_truthy(env.get("HBM_NEMO_RELAY_ATIF_ENABLED")) or _truthy(env.get("HBM_NEMO_RELAY_ATOF_ENABLED"))):
    ...
    header = (
        "# NeMo Relay plugin configuration for HBM AGENT (selected via "
        f"{RELAY_PLUGINS_CONFIG_ENV} in .env).\n"
        "# Generated by `hbm-agent update` from the legacy HBM_NEMO_RELAY_ATIF_*/ATOF_* variables,\n"
...
def migrate_all_profile_relay_envs(*, validate: bool = True) -> list[RelayMigrationResult]:
    ...
    from hbm_cli.profiles import _get_default_hbm_home, _iter_named_profile_dirs
    homes = [_get_default_hbm_home(), *_iter_named_profile_dirs()]
...
def print_relay_migration_report(results: list[RelayMigrationResult]) -> None:
    """Loud, actionable notice for `hbm-agent update` / `hbm-agent relay migrate`."""
    ...
    if migrated:
        ...
        print("   The legacy HBM_NEMO_RELAY_ATIF_*/ATOF_* variables stopped producing traces after the")
...
def run_relay_migration_after_update() -> None:
    """`hbm-agent update` hook: migrate every profile home, print the notice. Best-effort by contract."""
    print_relay_migration_report(migrate_all_profile_relay_envs())


RELAY_MIGRATE_COMMAND = "hbm-agent migrate relay"


def cmd_migrate_relay(args) -> None:
    """``hbm-agent migrate relay [--all-profiles] [--no-validate]``."""
    ...
    if getattr(args, "all_profiles", False):
        ...
        results = [migrate_profile_relay_env(get_hbm_home(), validate=validate)]
