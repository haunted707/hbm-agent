...
comments the legacy lines out. It runs from ``hbm update`` for every profile home and from
...
def migrate_profile_relay_env(home: Path, *, validate: bool = True) -> RelayMigrationResult:
    ...
    header = (
        ...
        "# Generated by `hbm update` from the legacy HBM_NEMO_RELAY_ATIF_*/ATOF_* variables,\n"
...
def print_relay_migration_report(results: list[RelayMigrationResult]) -> None:
    """Loud, actionable notice for `hbm update` / `hbm-agent relay migrate`."""
...
def run_relay_migration_after_update() -> None:
    """`hbm update` hook: migrate every profile home, print the notice. Best-effort by contract."""
    print_relay_migration_report(migrate_all_profile_relay_envs())


RELAY_MIGRATE_COMMAND = "hbm migrate relay"


def cmd_migrate_relay(args) -> None:
    """``hbm migrate relay [--all-profiles] [--no-validate]``."""
