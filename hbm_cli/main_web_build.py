...
# to the checkout (NOT in HBM_HOME): __pycache__ is per-checkout state shared
# by every profile.
...
def _record_bytecode_fingerprint() -> None:
    ...
    from hbm_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    """
    from hbm_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Vite outputs to ``hbm_cli/web_dist/`` (vite.config.ts outDir), NOT ``web/dist/``."""
    return _web_project_root(web_dir) / "hbm_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Path of the web UI build stamp under $HBM_HOME."""
    from hbm_constants import get_hbm_home
    return get_hbm_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    half-state with the ``hbm-agent`` launcher present but ``hbm_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    """``PYTHON=`` env for node-gyp on NixOS (bare PATH lookup fails outside nix-shell): the hbm-agent
...
def _run_npm_install_deterministic(
    ...
    run_env["PATH"] = with_hbm_node_path(run_env)["PATH"]
...
def _report_web_build_failure(step: str, result: subprocess.CompletedProcess, *, fatal: bool) -> bool:
    ...
    _console_print(f"  {'✗' if fatal else '⚠'} Web UI {step} failed" + ("" if fatal else " (hbm-agent web will not be available)"))
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    soft warning (used by ``hbm-agent web``). Returns True when the build succeeded
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_hbm_node_path())
