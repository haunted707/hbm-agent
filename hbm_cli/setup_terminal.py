...
setup.py names are resolved through the module object so test patches on ``hbm_cli.setup.<name>``
...
from hbm_cli import nous_subscription
...
def _pip_install_vercel(package):
    """uv when HBM AGENT has one ($HBM_HOME/bin is never on PATH, so which() misses it and
...
def _setup_backend_vercel(config: dict) -> None:
    ...
    _setup._info("Cloud microVM sandboxes with snapshot-backed filesystem persistence.",
                 "Requires the optional SDK: pip install 'hbm-agent[vercel]'")
    _ensure_sdk("vercel", "pip install 'hbm-agent[vercel]'", show_stderr=True, install=_pip_install_vercel)
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where HBM AGENT runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.hbm/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
...
import hbm_cli.setup as _setup  # noqa: E402  (bottom: hbm_cli.setup imports this module)
