"""Unified provider-credential lifecycle across every store HBM AGENT reads.
...
def _scrub_config_yaml_mirrors(old_value: str, new_value: str | None) -> List[str]:
    ...
    from hbm_cli.config import get_config_path, read_user_config_raw, require_readable_config_before_write
...
def purge_env_credential_references(
    ...
    if clear_models_cache and providers:
        ...
        _for_each_provider(providers, "hbm_cli.models.clear_provider_models_cache")
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    _for_each_provider(providers, "hbm_cli.auth.unsuppress_credential_source", f"env:{env_var}")
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    from hbm_cli.config import load_env, remove_env_value
