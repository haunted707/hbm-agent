...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "HbmSetupBot",
    ...
    print(f"\n  Contacting HBM AGENT Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the HBM AGENT Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "HbmSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``hbm_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``hbm_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"hbm_{generate_username_slug()}_bot"
