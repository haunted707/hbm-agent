"""Bounded product contract for the first HBM AGENT shared-metrics slice."""
...
SCHEMA_KEY = "hbm.metrics.schema_version"
SCHEMA_VERSION = "hbm.metrics.event.v2"
MODEL_CALL_SCOPE = "hbm.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "hbm.task_run"
TOOL_CALL_SCOPE = "hbm.tool_call"
CLIENT_ACTIVE_MARK = "hbm.client.active"
TOOL_APPROVAL_MARK = "hbm.tool_approval"
SKILL_LIFECYCLE_MARK = "hbm.skill.lifecycle"
SKILL_LOAD_MARK = "hbm.skill.load"
SUBSCRIBER_NAME = "hbm.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "hbm.client.active"
LEGACY_MODEL_CALL_METRIC = "hbm.model_call.count"
MODEL_ROUTE_METRIC = "hbm.model_route.count"
TASK_STARTED_METRIC = "hbm.task_run.started"
TASK_FINISHED_METRIC = "hbm.task_run.finished"
TOOL_CALL_METRIC = "hbm.tool_call.count"
TOOL_APPROVAL_METRIC = "hbm.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "hbm.skill.lifecycle.count"
SKILL_LOAD_METRIC = "hbm.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "hbm_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted HBM AGENT installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    hbm_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(hbm_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "hbm_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("hbm_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its HBM AGENT logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "hbm.call_role"
    )
    call_role = (metadata or {}).get("hbm.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from hbm_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map HBM AGENT terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "hbm-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map HBM AGENT registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal HBM AGENT tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to HBM AGENT."""
