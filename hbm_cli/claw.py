...
from hbm_cli.config import get_hbm_home, get_config_path, load_config, save_config
from hbm_constants import get_optional_skills_dir
from hbm_cli.setup import (Colors, color, print_header, print_info, print_success, print_error,
                              prompt_yes_no)
...
_SCRIPT_REL = Path("migration", "openclaw-migration", "scripts", "openclaw_to_hbm.py")
...
_OPENCLAW_SCRIPT_INSTALLED = get_hbm_home() / "skills" / _SCRIPT_REL
...
def _print_banner(title: str) -> None:
    ...
    print()
    rule = "─" * 57
    for line in (f"┌{rule}┐", f"│          ☤ HBM AGENT — {title:<35s}│", f"└{rule}┘"):
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    if running and _warn_running(
        ...
        ("Messaging platforms (Telegram, Discord, Slack) only allow one "
         ...
         "HBM AGENT may try to use the same token, causing disconnects.",
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Warn if a HBM AGENT gateway has connected platforms (token conflicts, e.g. Telegram 409)."""
    ...
    if connected and _warn_running(
        auto_yes, "HBM AGENT gateway is running with active connections: " + ", ".join(connected), [],
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_hbm.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_hbm", script_path)
...
def _cmd_migrate(args):
    """Run the OpenClaw → HBM AGENT migration: preflight, preview, confirm, back up, apply."""
    ...
    opts.hbm_home = get_hbm_home()
    print()
    print_header("Migration Settings")
    _info(f"Source:      {opts.source_dir}", f"Target:      {opts.hbm_home}",
...
def _load_migrator(script_path: Path, opts: SimpleNamespace) -> Optional[Callable[[bool], dict]]:
    ...
    return lambda execute: mod.Migrator(
        source_root=opts.source_dir.resolve(), target_root=opts.hbm_home.resolve(),
...
def _preview_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> bool:
    ...
    if conflicts > 0 and not opts.overwrite:
        _error_block(
            ...
            "Each conflict is an item whose target already exists in ~/.hbm/. "
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    if not opts.no_backup:
        try:
            from hbm_cli.backup import create_pre_migration_backup
            from hbm_cli.sizefmt import format_bytes as _format_size
            backup_archive = create_pre_migration_backup(hbm_home=opts.hbm_home)
            if backup_archive:
                print()
            ...
            return _error_block(
                ...
                "Re-run with --no-backup to skip, or free up disk space under the HBM AGENT home.",
