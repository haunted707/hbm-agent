...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("HBM_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the HBM AGENT telemetry policy applied; falls back to the current
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports HBM AGENT' runtime contract (30s cache keyed on the
...
def _pip_install(args: List[str], *, timeout: int = 300, capture_output: bool = True):
    ...
    # Managed uv first: $HBM_HOME/bin is never on PATH, so a bare which() misses the uv HBM AGENT
...
def install_cua_driver(upgrade: bool = False, require_confirmed_update: bool = False,
    ...
    override = os.environ.get("HBM_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        return _fail(f"    HBM_CUA_DRIVER_CMD does not resolve to an executable: {override}",
    ...
    # A driver failing HBM AGENT' runtime contract (version floor, missing manifest verbs) is repaired
    # regardless of mode. HBM AGENT' minimum requirement IS the confirmation an upgrade is needed, so
    ...
    if repair_existing:
        _print_warning(f"    Found cua-driver {contract.get('version') or 'unknown version'}, but "
                       "HBM AGENT cannot use its current runtime contract: "
                       f"{contract.get('reason') or 'required runtime features are missing'}.")
        if override:
            return _fail("    Update the binary selected by HBM_CUA_DRIVER_CMD, or unset the "
...
def _print_cua_platform_notes(is_windows: bool, is_linux: bool, *, fresh_install: bool) -> None:
    ...
    if is_windows:
        ...
        if fresh_install:
            _print_info("    Both must allow the terminal / HBM AGENT process.")
