...
Mixin split out of ``cli.py``; bound onto ``HbmCLI`` via the MRO. cli.py-internal symbols are
...
from hbm_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname, file_signature

from hbm_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from hbm_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   HBM AGENT needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from hbm_cli.model_switch import is_nous_hbm_non_agentic
        if is_nous_hbm_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research HBM AGENT 3 & 4 models are NOT agentic and are not "
                "designed for use with HBM AGENT.[/]")
            self._console_print(
    ...
    def show_help(self, arg: str = ""):
        ...
        from hbm_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from hbm_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        (tests/hbm_cli/test_bang_shell_mode.py). Returns False when the text is not a bang command or
        bang mode is disabled for this context (gateway/cron), so the caller routes normally.
        """
        from cli import _rich_text_from_ansi
        from hbm_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_hbm_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_hbm_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from hbm_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from hbm_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from hbm_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.hbm/skills/ and queue a note for the next user turn.
