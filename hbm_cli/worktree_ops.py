...
def _setup_worktree(repo_root: str = None, sync_base: bool = True,
    ...
    *sync_base* branches from the fetched remote tip (``_resolve_worktree_base``), else local
    HEAD. *name* replaces the random ``hbm-<id>``; named trees lack the ``hbm-`` prefix so
    ...
    try:
        _git(["worktree", "lock", "--reason", f"hbm pid={os.getpid()}", str(wt_path)], repo_root)
...
def _worktree_merge_cache_path() -> Path:
    ...
    return get_hbm_home() / "cache" / "worktree_merge_verdicts.json"
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    ...
    for line in listing.splitlines():
        if line.startswith("worktree "):
            ...
            m = re.search(r"hbm pid=(\d+)", reason)
            if not m:
...
def _prune_candidates(worktrees_dir: Path, max_age_hours: int, now: float) -> list:
    ...
    Kanban trees (``t_<hex>``) belong to the kanban gc. ``hbm-*`` trees age on *max_age_hours*,
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        if not entry.is_dir() or kanban_re.match(entry.name):
            continue
        tier_hours = max_age_hours if entry.name.startswith("hbm-") else max_age_hours * 3
        try:
...
def _classify_prune_candidates(repo_root: str, candidates: list) -> list:
    ...
    def _classify(item):
        ...
        # Live lock = running hbm; a dead lock is unlocked in phase 3.
    ...
    try:
        if workers > 1:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=workers, thread_name_prefix="hbm-wt-prune"
            ) as pool:
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    try:
        from hbm_cli.worktree_gc import worktrees_summary
...
def _prune_orphaned_branches(repo_root: str, protect: Optional[set] = None) -> None:
    """Delete local ``hbm/hbm-*`` and ``pr-*`` branches with no worktree, except *protect*."""
    try:
