...
Three layers make every long-lived HBM AGENT process positively identifiable, so reapers (``hbm-agent
update``, Desktop startup sweeps) never guess lineage from PPID archaeology or cmdline matching:
1. spawn tags (``HBM_SPAWN`` env stamped by the spawner); 2. a ``(pid, create_time)`` ledger;
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    if project_root is None:
        try:
            from hbm_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``HBM_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``HBM_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_hbm_root()) / LEDGER_FILENAME
    except Exception:
        from hbm_cli.config import get_hbm_home

        return Path(get_hbm_home()) / LEDGER_FILENAME
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    """Spawner ``(pid, create_time)`` from the Electron app's HBM_PARENT_PID (+ optional
    ...
    try:
        spawner_pid = int(os.environ.get("HBM_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
    marker = os.environ.get("HBM_PARENT_START_MARKER", "")
...
def register_child(pid: int, purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    helpers: arbitrary ``npx``/binary servers never import HBM AGENT code). Records the child's
