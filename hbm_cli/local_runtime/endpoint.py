...
def _state_endpoint() -> dict | None:
    from hbm_cli.local_runtime.recovery import is_modern, read_state, recorded_process
...
def resolve_llamacpp_endpoint(config: dict | None = None,
    ...
    from hbm_cli.local_runtime.detect import detect_server
...
def _boot_in_flight(config: dict | None) -> bool:
    ...
    with suppress(Exception):
        ...
        if not ((config or {}).get("local_runtime") or {}).get("enabled"):
            return False
        from hbm_cli.local_runtime.binaries import manifest_verified, runtimes_root
