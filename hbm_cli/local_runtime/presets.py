...
from hbm_cli.local_runtime.context_policy import (
...
from hbm_cli.local_runtime.gguf import model_id_from_stem, read_gguf_header
...
def preset_for_model(gguf: Path, budget: HardwareBudget,
    ...
    from hbm_cli.local_runtime.catalog import entry_for_model
    from hbm_cli.local_runtime.growth import load_window_overrides
...
def generate_presets(models_dir: Path, budget: HardwareBudget, preset_path: Path,
    ...
    from hbm_cli.local_runtime.bootstrap import staged_in
    ...
    for gguf in staged_in(models_dir):
        ...
        sections.append("# hbm-decision: " + json.dumps({
...
def read_preset_decisions(preset_path: Path | None = None) -> dict[str, PresetEntry]:
    ...
    try:
        ...
        for line in text.splitlines():
            if line.startswith("# hbm-decision: "):
                fact = json.loads(line.removeprefix("# hbm-decision: "))
