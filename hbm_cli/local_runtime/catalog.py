...
from hbm_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile, PhysicsRefusal
from hbm_cli.local_runtime.gguf import model_id_from_stem
...
_CATALOG_URL = ("https://raw.githubusercontent.com/NousResearch/hermes-agent"
                "/main/hbm_cli/local_runtime/catalog.json")
_SCHEMA_VERSION = 1
...
def _packaged_catalog() -> "tuple[CatalogEntry, ...]":
    ...
    raw = files("hbm_cli.local_runtime").joinpath("catalog.json").read_text(encoding="utf-8")
...
def refresh_catalog(force: bool = False) -> bool:
    ...
    try:
        req = urllib.request.Request(_CATALOG_URL, headers={"User-Agent": "hbm-local-runtime"})
