...
def parse_bang_command(text: str) -> str:
    ...
    (history expansion), not to HBM AGENT.
    """
...
def bang_shell_enabled() -> bool:
    ...
    return not (env_var_enabled("HBM_GATEWAY_SESSION") or env_var_enabled("HBM_CRON_SESSION")
                or (os.getenv("HBM_SESSION_PLATFORM") or "").strip())
...
def _bang_env() -> dict:
    """Environment for a bang command with HBM AGENT-managed secrets filtered.
...
def run_bang_command(command: str, *, cwd: Optional[str] = None, timeout: int = DEFAULT_TIMEOUT, writer=None) -> int:
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the HBM AGENT session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
