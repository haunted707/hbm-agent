...
Tokens live in ~/.hbm/auth.json, NOT ~/.codex/: HBM AGENT keeps its own Codex OAuth session
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from hbm_cli.auth import _POOL_STATUS_FIELDS
...
def _codex_base_url() -> str:
    return os.getenv("HBM_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from hbm_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from HBM AGENT auth store (~/.hbm/auth.json)."""
    from hbm_cli.auth import _load_provider_state, _nonempty_str
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into HBM AGENT auth, if available."""
    from hbm_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
...
def refresh_codex_oauth_pure(
    ...
    """Refresh Codex OAuth tokens without mutating HBM AGENT auth state."""
    from hbm_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    """
    from hbm_cli.auth import _provider_state_transaction, _save_codex_tokens, refresh_codex_oauth_pure
    ...
    with _provider_state_transaction("openai-codex", lock_timeout) as (_store, state, _source):
        ...
        try:
            ...
            # (or another HBM AGENT process) rotates the shared token this frozen copy fails with a
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from hbm_cli.auth import _codex_access_token_is_expiring
...
def resolve_codex_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    """Resolve runtime credentials from HBM AGENT's own Codex token store.
    ...
    refresh_timeout_seconds = env_float("HBM_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    if _should_refresh(access_token):
        # Re-read under lock to avoid racing with other HBM AGENT processes
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout):
            ...
            access_token = _stripped(tokens.get("access_token"))
    return _codex_runtime_result(
        access_token, source="hbm-auth-store", last_refresh=data.get("last_refresh"))
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from hbm_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    ...
    from hbm_cli.auth import _nonempty_str, read_credential_pool
...
def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.hbm/auth.json."""
    from hbm_cli.auth import (
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("HBM AGENT will create its own session to avoid conflicts with Codex CLI / VS Code.")
            if _prompt_yes_no(
                ...
                print("HBM AGENT will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — HBM AGENT gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(HBM AGENT creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from hbm_cli.auth import _utc_now_z
