...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``hbm.exe`` quarantine, ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "hbm_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from hbm_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from hbm_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__HBM_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        "import importlib, json, sys\n"
        # Importing hbm_cli.main runs the startup dotenv load, which pulls external secret
        ...
        # only our own packages count. Roots come from hbm_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('hbm_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        ...
        logger.debug("Could not snapshot active HBM AGENT Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    try:
        ...
        logger.debug("HBM AGENT Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} HBM AGENT Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not HBM AGENT extras / ``LAZY_DEPS``), so the core reinstall can strip them;
    ...
    try:
        from hbm_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    from hbm_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _npm_lockfile_changed(hbm_root: Path) -> bool:
    from hbm_cli.update_cmd import _m
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(hbm_root)
...
def _npm_lock_cache_file(hbm_root: Path) -> Path:
...
def _record_npm_lockfile_hash(hbm_root: Path) -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(hbm_root).write_text(digest, encoding="utf-8")
    except OSError:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run hbm-agent
...
def _update_node_dependencies() -> list[str]:
    ...
    shared_hbm_root = get_default_hbm_root()
    ...
    if not _m()._npm_lockfile_changed(shared_hbm_root):
    ...
    nixos_env = with_hbm_node_path(_m()._nixos_build_env())
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_hbm_root)
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".hbm-bootstrap-complete", _m()._update_marker_path())
...
# canonical case: ``hbm_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``hbm.exe`` shim
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `hbm-agent` launch will complete it")
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "hbm_cli.main", "desktop", "--build-only"]
    # Capture the loud build output into update.log; retry once on failure (still-settling
    # rebuild window), then surface the tail. Put HBM AGENT-managed Node on PATH: the desktop
    # updater chain loses shell PATH customizations, so a bare-PATH child hits `node: not found`.
    from hbm_constants import with_hbm_node_path
    build_env = with_hbm_node_path()
    for _attempt in range(2):
        ...
        if build_result.returncode == 0:
            break
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `hbm-agent desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo hbm`` dies mid-update with ``venv/bin/hbm-agent`` already
    ...
    """
    from hbm_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave HBM AGENT broken.")
    print("  This usually happens after running hbm or pip with sudo. Offending paths:")
...
def _sync_python_dependencies_after_pull(
    ...
    if _m()._refresh_active_lazy_features(install_prefix, env=lazy_env, features=active_lazy_features):
        ...
        print(
            "  ⚠ Lazy-refresh recovery incomplete — run `hbm-agent` again "
