"""Default configuration data for HBM AGENT: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from hbm_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hbm-cli"],
    # journal_mode: SQLite journal mode for every HBM AGENT DB. "wal" default; use "delete" on
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `hbm-agent
        ...
        # HBM AGENT-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping HBM AGENT (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env HBM_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # HBM_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else HBM_HOME/cache/terminal (auto-pruned
        ...
        # HBM_HOME/home; "real" = force real HOME; "profile" = force HBM_HOME/home when it
        ...
        # ["/home/user/.hbm/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled HBM AGENT image,
        # which drops to `hbm-agent` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a HBM AGENT-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by HBM AGENT' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.hbm/browser-profile/ so credentials
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.hbm/checkpoints/ size (MB); the oldest checkpoint per project
    ...
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .hbm.md,
    ...
    # Same bound for single-query mode (``hbm -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # HBM AGENT' summarizer cannot shrink it. native = codex decides; hbm = HBM AGENT' threshold
    ...
    "display": {
        ...
        # `path`/`paths` = HBM_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/hbm`.
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: HBM_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than HBM_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env HBM_DASHBOARD_OAUTH_CLIENT_ID / HBM_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env HBM_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # HBM_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env HBM_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/hbm) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.hbm/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → HBM AGENT turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to HBM AGENT (any model / provider you have
    ...
    # "Hey HBM AGENT" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_hbm" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.hbm/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.hbm/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <hbm_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.hbm/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # HBM_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.hbm/skills/ and <root>/.agents/skills/ load as the
        ...
        # skip; HBM_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${HBM_SKILL_DIR} / ${HBM_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.hbm/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.hbm/.curator_backups/blobs/); powers `hbm-agent
    ...
    # peerName, sessions, enabled); hbm-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "☤ *HBM AGENT*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (HBM_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export HBM_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. HBM_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # approvals persist in ~/.hbm/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # HBM_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .hbm config) always need human approval, even under yolo. Extra patterns are fnmatch
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $HBM_HOME/plugins/ <name>/),
        ...
        # Env override: HBM_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: HBM_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: HBM_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: HBM_CRON_MEDIA_SEND_TIMEOUT.
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across HBM AGENT homes (#110995): profile names
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with hbm-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core HBM AGENT tools
    ...
    "logging": {  # File logging to ~/.hbm/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: HBM AGENT
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "hbm-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to HBM_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to HBM_STARTUP_WATCHDOG / HBM_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.hbm/sessions/sessions.json mirror of the routing index
        ...
        # (HBM_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # HBM_GATEWAY_MAX_STARTS / HBM_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.hbm/.env, auth.json). True:
        # files must be under the HBM AGENT cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to HBM_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the HBM AGENT cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to HBM_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to HBM_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # HBM_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.hbm/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        # (native/fts5_cjk/build.sh → ~/.hbm/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to HBM_CJK_FTS.
        "cjk_fts": True,
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <HBM_HOME>/lsp/bin/ on
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.hbm/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.hbm/cache/,
            ...
            # Auto-download bws into ~/.hbm/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; HBM AGENT sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; HBM AGENT also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.hbm/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # HBM AGENT Desktop (Electron) launch options; only affect `hbm-agent desktop`.
        ...
        # Bridged to HBM_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to HBM_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "local_runtime": {
        # Off = detection-only (HBM AGENT still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by HBM AGENT releases after validation.
        "tag": "b10964",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials HBM AGENT mints short-lived "
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the HBM AGENT auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey HBM AGENT' wake word engine (optional; "
    ...
    "HBM_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "HBM_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "HBM_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use HBM AGENT, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @hbm:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. HBM_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #hbm)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: hbm-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'hbm-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote HBM AGENT API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote HBM AGENT API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote HBM AGENT API server (proxy mode). Must "
    ...
    # HBM_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. HBM_TOOL_PROGRESS is unsupported.
    "HBM_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "HBM_EPHEMERAL_SYSTEM_PROMPT": _setting(
