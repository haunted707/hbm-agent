"""Dependency-light venv recovery that runs BEFORE hbm_cli.main's imports.

Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``hbm_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``hbm_cli.main`` to become importable again (force-reinstall of
...
def _should_skip_external_secret_sources() -> bool:
    ...
    Profile flags are stripped before ``hbm_cli.main`` loads dotenv, so ``argv[1]`` is
...
def _find_uv_binary() -> str | None:
    ...
    fallback refuses to touch them; the only sanctioned installer is then uv itself, which HBM AGENT
    vendors (``~/.hbm/bin/uv.exe``) or the user has on PATH.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    for sub in ((".hbm", "bin"), (".local", "bin"), (".cargo", "bin")):
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``hbm_cli.main`` with recovery armed and
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    try:
        ...
        try:
            ...
            print("  The next launch will retry; hbm will keep working from "
