"""Migrate HBM AGENT MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
from agent.transports.hbm_tools_mcp_server import HBM_TOOLS_MCP_SERVER_NAME
...
MIGRATION_MARKER = (
    "# managed by hbm-agent — `hbm-agent codex-runtime migrate` regenerates this section")
MIGRATION_END_MARKER = "# end hbm-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in HBM AGENT config.")
...
# HBM AGENT MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown HBM AGENT keys.
_KNOWN_HBM_KEYS = {
...
# (hbm-agent key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, hbm_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one HBM AGENT MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. HBM AGENT' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(hbm_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if hbm_cfg.get("command"):
        if hbm_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(hbm_cfg["command"])
        if hbm_cfg.get("args"):
            out["args"] = [str(a) for a in hbm_cfg["args"]]
        if hbm_cfg.get("env"):
            out["env"] = _str_map(hbm_cfg["env"])
        if hbm_cfg.get("cwd"):
            out["cwd"] = str(hbm_cfg["cwd"])
    elif hbm_cfg.get("url"):
        out["url"] = str(hbm_cfg["url"])
        if hbm_cfg.get("headers"):
            out["http_headers"] = _str_map(hbm_cfg["headers"])
        if hbm_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for hbm_key, codex_key, note in _TIMEOUT_KEYS:
        if hbm_key in hbm_cfg:
            try:
                out[codex_key] = float(hbm_cfg[hbm_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if hbm_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in hbm_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_HBM_KEYS:
            skipped.append(f"{key} (unknown HBM AGENT key)")
    return out, skipped
...
# env-var passthrough (HBM_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by HBM AGENT)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="hbm-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a HBM_HOME pointing there burned into
...
def _build_hbm_tools_mcp_entry() -> dict:
    """Codex stdio entry launching HBM AGENT' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    HBM_HOME passes through only IF SET, read from os.environ (not get_hbm_home()): when
    unset the codex subprocess must inherit its launcher's runtime HBM_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched HBM_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched hbm finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # HBM_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_hbm_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever HBM_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's HBM_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("HBM_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    hbm_home = os.environ.get("HBM_HOME") or ""
    if hbm_home and not _looks_like_test_tempdir(hbm_home):
        env["HBM_HOME"] = hbm_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["HBM_QUIET"] = "1"
    env["HBM_REDACT_SECRETS"] = env.get("HBM_REDACT_SECRETS", "true")
    return {
...
def migrate(
    hbm_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_hbm_tools: bool = True) -> MigrationReport:
    """Translate HBM AGENT mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_hbm_tools`` registers HBM AGENT' own tool surface
    (agent/transports/hbm_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    hbm_servers = (hbm_config or {}).get("mcp_servers") or {}
    if not isinstance(hbm_servers, dict):
        report.errors.append("mcp_servers in HBM AGENT config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in hbm_servers.items():
        name = str(raw_name)
    ...
    if expose_hbm_tools:
        translated[HBM_TOOLS_MCP_SERVER_NAME] = _build_hbm_tools_mcp_entry()
        if HBM_TOOLS_MCP_SERVER_NAME not in report.migrated:
            report.migrated.append(HBM_TOOLS_MCP_SERVER_NAME)
