...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hbm-agent -c.",
    ...
    "Ctrl+Z suspends HBM AGENT to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "hbm-agent -c resumes your most recent CLI session. hbm-agent -c \"project name\" resumes by title.",
    "hbm-agent -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hbm-agent -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hbm-agent chat -t web,terminal enables only specific toolsets for a focused session.",
    "hbm-agent chat -s github-pr-workflow preloads a skill at launch.",
    "hbm-agent chat -q \"query\" runs a single non-interactive query and exits.",
    "hbm-agent chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "hbm-agent chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hbm-agent --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hbm-agent chat --source telegram tags the session for filtering in hbm-agent sessions list.",
    "hbm-agent -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "hbm-agent doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hbm-agent dump outputs a compact setup summary — great for bug reports.",
    "hbm-agent config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hbm-agent config edit opens config.yaml in your default editor.",
    "hbm-agent config check scans for missing or stale configuration options.",
    "hbm-agent sessions browse opens an interactive session picker with search.",
    "hbm-agent sessions stats shows session counts by platform and database size.",
    "hbm-agent sessions prune --older-than 30 cleans up old sessions.",
    "hbm-agent skills search react --source skills-sh searches the skills.sh public directory.",
    "hbm-agent skills check scans installed hub skills for upstream updates.",
    "hbm-agent skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hbm-agent skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hbm-agent mcp add github --command npx adds MCP servers from the command line.",
    "hbm-agent mcp serve runs HBM AGENT itself as an MCP server for other agents.",
    "hbm-agent auth add lets you add multiple API keys for credential pool rotation.",
    "hbm-agent completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hbm-agent logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hbm-agent backup creates a zip backup of your entire HBM AGENT home directory.",
    "hbm-agent profile create coder creates an isolated profile that becomes its own command.",
    "hbm-agent profile create work --clone copies your current config and keys to a new profile.",
    "hbm-agent update syncs new bundled skills to ALL profiles automatically.",
    "hbm-agent gateway install sets up HBM AGENT as a system service (systemd/launchd).",
    "hbm-agent memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hbm-agent webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hbm-agent tools disables unused tools, hbm-agent skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "hbm-agent models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hbm-agent -w).",
    ...
    "HBM_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call HBM AGENT tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'hbm-agent profile create coder' creates the 'coder' command.",
    "hbm-agent profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, HBM AGENT prints a resume command with session ID and stats.",
    "hbm-agent sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hbm-agent -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "hbm-agent skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .hbm/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hbm-agent cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "HBM AGENT runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "hbm-agent gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to HBM AGENT via a local macOS server.",
    ...
    "Context files (.hbm.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "HBM AGENT loads project context from .hbm.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "hbm-agent mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "hbm-agent mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "hbm-agent mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — hbm-agent tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.hbm/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hbm-agent chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hbm-agent plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hbm-agent acp runs HBM AGENT as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "HBM_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "hbm-agent auth add nous or hbm-agent auth add openai-codex sets up OAuth-based providers.",
    ...
    "hbm-agent status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.hbm/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make HBM AGENT your own.",
    ...
    "Before a gateway session reset, HBM AGENT auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from hbm_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.hbm/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope HBM AGENT state via HBM_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "HBM_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in HBM_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.hbm/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hbm-agent profile create ls' would be rejected.",
    "hbm-agent profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "hbm-agent claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "HBM_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hbm-agent tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.hbm/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.hbm/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — HBM AGENT auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of HBM AGENT config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while HBM AGENT is working.',
    ...
    '/reload reloads ~/.hbm/.env into the running session — pick up new API keys without restarting.',
    ...
    "HBM_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.hbm/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.hbm/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'hbm-agent curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hbm-agent curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hbm-agent curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'hbm-agent auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    # --- TUI & Dashboard ---
    'HBM_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "HBM_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'hbm-agent dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hbm-agent dashboard embeds the full HBM AGENT TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.hbm/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.hbm/dashboard-plugins/ — no npm build required.',
    ...
    'HBM_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'HBM_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'HBM_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'HBM_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.hbm/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.hbm/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hbm-agent -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hbm-agent chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hbm-agent chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hbm-agent chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "hbm-agent chat --source tool tags programmatic chats so they don't clutter hbm-agent sessions list.",
    'hbm-agent dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hbm-agent sessions rename <ID> "new title" renames any past session; hbm-agent sessions delete <ID> removes one.',
    'hbm-agent import restores a full HBM AGENT backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'hbm-agent fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hbm-agent pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hbm-agent setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hbm-agent status --deep runs the full health sweep across every component; plain hbm-agent status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'HBM_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HBM_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hbm/plugins/ — trust-gated by design.',
    "HBM_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'HBM_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'HBM_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'HBM_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'HBM_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'HBM_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'HBM_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'HBM_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'HBM_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'HBM_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'HBM_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes HBM AGENT block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from hbm-agent sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.hbm/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — HBM AGENT is
