...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hbm -c.",
    ...
    # --- CLI Flags ---
    "hbm -c resumes your most recent CLI session. hbm -c \"project name\" resumes by title.",
    "hbm -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hbm -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hbm chat -t web,terminal enables only specific toolsets for a focused session.",
    "hbm chat -s github-pr-workflow preloads a skill at launch.",
    "hbm chat -q \"query\" runs a single non-interactive query and exits.",
    "hbm chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "hbm chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hbm-agent --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hbm chat --source telegram tags the session for filtering in hbm sessions list.",
    "hbm -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "hbm doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hbm dump outputs a compact setup summary — great for bug reports.",
    "hbm config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hbm config edit opens config.yaml in your default editor.",
    "hbm config check scans for missing or stale configuration options.",
    "hbm sessions browse opens an interactive session picker with search.",
    "hbm sessions stats shows session counts by platform and database size.",
    "hbm sessions prune --older-than 30 cleans up old sessions.",
    "hbm skills search react --source skills-sh searches the skills.sh public directory.",
    "hbm skills check scans installed hub skills for upstream updates.",
    "hbm skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hbm skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hbm mcp add github --command npx adds MCP servers from the command line.",
    "hbm mcp serve runs HBM AGENT itself as an MCP server for other agents.",
    "hbm auth add lets you add multiple API keys for credential pool rotation.",
    "hbm completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hbm logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hbm backup creates a zip backup of your entire HBM AGENT home directory.",
    "hbm profile create coder creates an isolated profile that becomes its own command.",
    "hbm profile create work --clone copies your current config and keys to a new profile.",
    "hbm update syncs new bundled skills to ALL profiles automatically.",
    "hbm gateway install sets up HBM AGENT as a system service (systemd/launchd).",
    "hbm memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hbm webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hbm tools disables unused tools, hbm skills config trims skills down.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hbm -w).",
    ...
    "Profile names become shell commands — 'hbm profile create coder' creates the 'coder' command.",
    "hbm profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "hbm sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hbm -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "hbm skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hbm cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "hbm gateway install sets it up as a system service that starts on boot.",
    ...
    # --- MCP ---
    "hbm mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "hbm mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "hbm mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — hbm tools can toggle them per platform.",
    ...
    "hbm chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hbm plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hbm acp runs HBM AGENT as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "hbm auth add nous or hbm auth add openai-codex sets up OAuth-based providers.",
    ...
    "hbm status --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hbm profile create ls' would be rejected.",
    "hbm profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "hbm claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hbm tools.",
    ...
    # --- Curator ---
    'hbm curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hbm curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hbm curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'hbm auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    'hbm dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hbm dashboard embeds the full HBM AGENT TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'hbm -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hbm chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hbm chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hbm chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "hbm chat --source tool tags programmatic chats so they don't clutter hbm sessions list.",
    'hbm dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hbm sessions rename <ID> "new title" renames any past session; hbm sessions delete <ID> removes one.',
    'hbm import restores a full HBM AGENT backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'hbm fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hbm pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hbm setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hbm status --deep runs the full health sweep across every component; plain hbm status is the quick view.',
    ...
    '--source tool chats are excluded from hbm sessions list by default — set --source explicitly to see them.',
