...
def format_nous_portal_entitlement_message(
    ...
    if account_info.paid_service_access is None:
        detail = f"HBM AGENT could not verify your Nous Portal paid access, so {capability} is unavailable."
...
def nous_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from hbm_cli.auth import get_provider_auth_state, _decode_jwt_claims
...
def _fresh_account_info(state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> NousPortalAccountInfo:
    ...
    try:
        from hbm_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    try:
        from hbm_cli.auth import _decode_jwt_claims
    except Exception:
        return None
