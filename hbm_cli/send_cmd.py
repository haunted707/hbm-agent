...
def _load_hbm_env() -> None:
    """Populate the credential environment from ``~/.hbm/.env`` AND bridge top-level ``config.yaml``
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform HBM AGENT "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.hbm/.env + ~/.hbm/config.yaml) — no LLM, "
