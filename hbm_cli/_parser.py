"""Top-level argparse construction for the hbm CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``hbm_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``HBM_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/hbm_cli/
...
def _cfg_path() -> str:
    """``~/.hbm/config.yaml`` spelled for the active profile, for help text."""
    from hbm_constants import display_hbm_home

    return f"{display_hbm_home()}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``hbm-agent --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``hbm_cli.relaunch``."""
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via HBM_INFERENCE_MODEL env var."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to HBM_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with HBM AGENT")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also HBM_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from HBM AGENT itself.")
...
class HbmArgumentParser(argparse.ArgumentParser):
...
def build_top_level_parser():
    ...
    """
    parser = HbmArgumentParser(
        prog="hbm-agent", description="HBM AGENT - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``hbm [...] <command>`` instead of the brace list of
    # every subcommand name; ``hbm-agent --help`` still lists each command with its help row.
