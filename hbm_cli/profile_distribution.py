"""Profile distributions — shareable, packaged HBM AGENT profiles via git.
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            hbm_requires=_str(data, "hbm_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("hbm_requires", self.hbm_requires),
...
def check_hbm_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires HBM AGENT {op}{target}, but you have {current_version}.")
    ...
    *workdir* (``.git`` removed); a local directory is used in place."""
    ...
    if _looks_like_git_url(src_str):
        ...
        missing = (
            ...
            "This repository is not a HBM AGENT profile distribution."
        )
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from hbm_cli.profiles import _canon_valid, get_profile_dir
    from hbm_cli import __version__ as hbm_version
    staged, provenance = _stage_source(source, workdir)
    _reject_distribution_symlinks(staged)
    manifest = read_manifest(staged)
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root — this source is not a HBM AGENT distribution."
        )
    check_hbm_requires(manifest.hbm_requires, hbm_version)  # fail fast
    canon = _canon_valid(override_name or manifest.name)
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' — that is the built-in "
            "root profile (~/.hbm).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from hbm_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from hbm_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="hbm_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `hbm-agent profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from hbm_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`hbm-agent profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="hbm_dist_update_") as tmp:
