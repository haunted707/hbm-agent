...
Split out of ``hbm_cli/auth.py`` and re-exported there; origin helpers are imported lazily
inside each function so ``hbm_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from hbm_cli.auth_constants import _decode_jwt_claims
...
def _is_pkce_row(row: Dict[str, Any]) -> bool:
    return str(row.get("source") or "").endswith("hbm_pkce")
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``hbm-agent profile create
    ...
    """
    from hbm_cli.auth import _same_path, _save_auth_store
    ...
    try:
        ...
        root_auth_path = get_default_hbm_root() / "auth.json"
...
# the heal for the life of ONE process, so every fresh `hbm-agent` invocation and every new worker
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``hbm-agent auth list`` / ``hbm-agent auth status`` print them so the user sees the consolidation.
    """
    from hbm_cli.auth import _oauth_heal_notices
...
def heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    row with no root counterpart (an independent ``hbm-agent -p <p> auth add`` grant, or the only
...
class _HealPass:
    ...
    def heal_pool_rows(self) -> None:
        ...
        for row in self.p_rows:
            ...
            # (the ``hbm-agent auth`` PKCE shape); a profile hbm_pkce-family row is its copy.
    ...
    def heal_profile_singleton(self, profile_singleton: Optional[Path]) -> None:
        if profile_singleton is None or not profile_singleton.exists():
            return
        from hbm_cli.auth import _is_same_auth_store
    ...
    def sync_root_singleton_with_pkce_row(self) -> None:
        """Keep root's singleton and its ``hbm_pkce``-seeded pool row in step.
        ...
        pkce_idx = next(
            (i for i, r in enumerate(self.r_rows)
             if _is_oauth_pool_payload(r) and r.get("source") == "hbm_pkce"), None)
        if pkce_idx is None:
            return
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # ~/.hbm/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".hbm" / "auth.json"):
            return None
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            if summary["adopted"] and root_singleton is not None and singleton_row is not None:
                from agent.anthropic_credentials import _write_hbm_oauth_credentials
                _write_hbm_oauth_credentials(
