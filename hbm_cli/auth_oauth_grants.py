...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    Called after any path that copies credential files between profiles (``hbm profile create
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``hbm auth list`` / ``hbm auth status`` print them so the user sees the consolidation.
    """
...
def heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    row with no root counterpart (an independent ``hbm -p <p> auth add`` grant, or the only
...
class _HealPass:
    ...
    def heal_pool_rows(self) -> None:
        ...
        for row in self.p_rows:
            ...
            # (the ``hbm auth`` PKCE shape); a profile hbm_pkce-family row is its copy.
