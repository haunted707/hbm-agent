...
def _reload_config_modules() -> None:
    ...
    for mod_name in (
        "hbm_cli.config_defaults", "hbm_cli.config", "hbm_cli.tools_config",
        "hbm_cli.config_migrations", "hbm_cli._subprocess_compat", "hbm_cli.dashboard_procs"):
...
def _run_config_check_fresh() -> tuple:
    ...
    from hbm_cli.update_cmd import _reload_config_modules
    _reload_config_modules()
    from hbm_cli.config import check_config_version
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    sibling (active skipped): scope via the context-local HBM_HOME override (never ``os.environ``)
    ...
    with _best_effort('Sibling profile enumeration failed: %s'):
        from hbm_constants import (
            get_process_hbm_home, reset_hbm_home_override, set_hbm_home_override)
        from hbm_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
        active_home = get_process_hbm_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_hbm_home_override(entry)
            try:
...
def _restore_snapshot_safety_nets(pre_update_snapshot_id) -> None:
    ...
    *pre_update_snapshot_id*) and every sibling profile (against ITS OWN snapshot)."""
    ...
    with _best_effort('Sibling cron auto-restore check failed: %s'):
        from hbm_cli.backup import restore_cron_jobs_all_profiles
    ...
    with _best_effort('Sibling config auto-restore check failed: %s'):
        from hbm_cli.backup import restore_config_model_settings_all_profiles
...
def _ask_configure_new_options(*, assume_yes: bool, gateway_mode: bool) -> str:
    ...
    from hbm_cli.update_cmd import _gateway_prompt
    if assume_yes:
...
def _check_and_apply_config_migration(
    *, assume_yes: bool = False, gateway_mode: bool = False, pre_update_snapshot_id: str | None = None
    ...
    # Evict EVERY cached HBM AGENT module before touching migration code: the updater is the
    # pre-pull process, and a migration step's call-time import (``_migrate_to_45`` →
    # ``hbm_cli.tools_config._configurable_keys``) resolves against whatever old module
    ...
    try:
        ...
        from hbm_cli.config import get_missing_env_vars, get_missing_config_fields
    ...
    # migration per sibling home via the context-local HBM_HOME override (never os.environ).
