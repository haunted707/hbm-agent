"""hbm webhook — manage dynamic webhook subscriptions from the CLI."""
...
def _setup_hint() -> str:
  ...
  Then start the gateway: hbm gateway run
"""


def webhook_command(args):
    """Entry point for 'hbm webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)
    if not sub:
        print("Usage: hbm webhook {subscribe|list|remove|test}")
        print("Run 'hbm webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    if cron_job:
        ...
        try:
            ...
            return
        if job is None:
            print(f"Error: no cron job matches '{cron_job}'. List jobs with: hbm cron list")
            return
    ...
    print("  The gateway must be running to receive events (hbm gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: hbm webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from hbm webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (hbm gateway run)")
