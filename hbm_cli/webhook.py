"""hbm-agent webhook — manage dynamic webhook subscriptions from the CLI."""
...
def _subscriptions_path() -> Path:
    ...
    return get_hbm_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: hbm-agent gateway run
"""


def webhook_command(args):
    """Entry point for 'hbm-agent webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)
    if not sub:
        print("Usage: hbm-agent webhook {subscribe|list|remove|test}")
        print("Run 'hbm-agent webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    if profile_arg is None:
        ...
        from hbm_cli.profiles import normalize_profile_name, profile_exists, validate_profile_name
        try:
    ...
    if cron_job:
        ...
        try:
            ...
            return
        if job is None:
            print(f"Error: no cron job matches '{cron_job}'. List jobs with: hbm-agent cron list")
            return
    ...
    print("  The gateway must be running to receive events (hbm-agent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: hbm-agent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from hbm-agent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (hbm-agent gateway run)")
