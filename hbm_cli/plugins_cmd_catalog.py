"""``hbm-agent plugins`` catalog surface: resolution, provenance sidecar, search/browse/info/validate,
catalog-aware update, plus the dashboard/TUI-facing catalog payload helpers.

Sibling of :mod:`hbm_cli.plugins_cmd` (the installer core, enable/disable state and console helpers
...
from hbm_cli.plugin_catalog import (
...
CATALOG_SIDECAR = ".hbm-catalog.json"
...
def looks_like_catalog_name(identifier: str) -> bool:
    ...
    from hbm_cli.plugins_cmd import _URL_SCHEMES
...
def raise_if_removed(*candidates: str) -> None:
    ...
    from hbm_cli.plugins_cmd import PluginOperationError
    for candidate in candidates:
        ...
        if removed is not None:
            ...
            raise PluginOperationError(
                f"Plugin '{removed.name}' was removed from the HBM AGENT plugin catalog and is blocked from "
...
def resolve_catalog_name(identifier: str, console) -> PluginCatalogEntry:
    ...
    from hbm_cli.plugins_cmd import _fail
    entry = get_live_catalog_entry(identifier)
    if entry is None:
        _fail(console, (
            f"[red]Error:[/red] '{identifier}' is not in the HBM AGENT plugin catalog and is not a Git URL or "
            "owner/repo shorthand. Browse entries with `hbm-agent plugins search`."))
...
def write_catalog_sidecar(target: Path, entry: PluginCatalogEntry) -> None:
    """``.hbm-catalog.json`` inside the install dir — how ``update``/``list``/dashboards know the plugin
...
def install_catalog_entry(entry: PluginCatalogEntry, *, force: bool, ref: Optional[str] = None,
    ...
    from hbm_cli.plugins_cmd import _install_plugin_core
...
def repin_catalog_plugin(target: Path, sidecar: dict) -> tuple[str, bool]:
    ...
    from hbm_cli.plugins_cmd import PluginOperationError, _get_enabled_set, _save_enabled_set
    ...
    if entry is None:
        raise PluginOperationError(
            ...
            "See `hbm-agent plugins info` and the removed blocklist.")
...
def cmd_update_catalog(name: str, target: Path, sidecar: dict, console) -> None:
    from hbm_cli.plugins_cmd import PluginOperationError, _fail
...
def _render_entries(entries: List[PluginCatalogEntry], console) -> None:
    from hbm_cli.plugins_cmd import _table
    table = _table(((("Name", "bold")), ("Category", None), ("Tier", None), ("Description", None),
                    ("Pinned", "dim"), ("Capabilities", "dim")), title="HBM AGENT Plugin Catalog (curated)")
    ...
    console.print(table)
    console.print()
    console.print("[dim]Details:[/dim] hbm-agent plugins info <name>    [dim]Install:[/dim] hbm-agent plugins install <name>")
...
def cmd_info(name: str) -> None:
    ...
    from hbm_cli.plugins_cmd import _console, cmd_show
    ...
    rows = [("Repo", entry.repo), ("Subdir", entry.subdir), ("Pinned SHA", entry.sha),
            ("Maintainer", entry.maintainer), ("Requires", f"hbm {entry.requires_hbm}" if entry.requires_hbm else ""),
    ...
    if removed is not None:
        ...
        console.print()
    console.print(f"[dim]Install:[/dim]     hbm-agent plugins install {entry.name}")
    console.print()
...
def cmd_validate(path: str, as_json: bool = False) -> None:
    ...
    from hbm_cli.plugin_validate import validate_plugin_dir
    from hbm_cli.plugins_cmd import _console
...
def installed_catalog_state(installed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    equals the manifest name (``hbm-plugin-x`` vs ``x``), so installs are matched through the
