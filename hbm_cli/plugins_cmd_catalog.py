"""``hbm plugins`` catalog surface: resolution, provenance sidecar, search/browse/info/validate,
...
def resolve_catalog_name(identifier: str, console) -> PluginCatalogEntry:
    ...
    if entry is None:
        _fail(console, (
            ...
            "owner/repo shorthand. Browse entries with `hbm plugins search`."))
...
def repin_catalog_plugin(target: Path, sidecar: dict) -> tuple[str, bool]:
    ...
    if entry is None:
        raise PluginOperationError(
            ...
            "See `hbm plugins info` and the removed blocklist.")
...
def _render_entries(entries: List[PluginCatalogEntry], console) -> None:
    ...
    console.print(table)
    console.print()
    console.print("[dim]Details:[/dim] hbm plugins info <name>    [dim]Install:[/dim] hbm plugins install <name>")
...
def cmd_info(name: str) -> None:
    ...
    if removed is not None:
        ...
        console.print()
    console.print(f"[dim]Install:[/dim]     hbm plugins install {entry.name}")
    console.print()
