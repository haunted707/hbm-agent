...
Custom-provider lookup lives in :mod:`hbm_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`hbm_cli.runtime_provider_backends` — both re-exported here so
``hbm_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from hbm_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
...
from hbm_cli import config as _config_mod
from hbm_cli import models as _models  # attribute access keeps ``hbm_cli.models.<name>`` patches effective
from hbm_constants import OPENROUTER_BASE_URL
from hbm_cli.providers import determine_api_mode, is_actual_route, is_official_openai_host, nous_api_mode
from utils import base_url_host_matches, base_url_hostname, env_int


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``hbm_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in hbm_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider == "xai":
        ...
        # the host_mandated_api_mode clause in hbm_cli/providers.py so the runtime resolver stays in
...
def _nous_min_key_ttl() -> int:
    return max(60, env_int("HBM_NOUS_MIN_KEY_TTL_SECONDS", 1800))


def _resolve_nous_creds() -> Dict[str, Any]:
    return resolve_nous_runtime_credentials(timeout_seconds=float(get_secret_str("HBM_NOUS_TIMEOUT_SECONDS", "15")))
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``HBM_INFERENCE_PROVIDER``, else
    ...
    return get_secret_str("HBM_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from hbm_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from hbm_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider in _POOL_ENTRY_SIMPLE_MODES:
        ...
        if provider == "openai-codex":
            ...
            # HBM_CODEX_BASE_URL override must apply consistently to every
            # credential source, including pooled OAuth credentials.
            override_url = get_secret_str("HBM_CODEX_BASE_URL", "").strip().rstrip("/")
            if override_url:
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    ...
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "hbm-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "hbm-auth-store",
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('hbm_cli.providers', 'custom_provider_aliases'),
