"""``hbm-agent kanban …`` — dispatch (``kanban_command``), task-verb handlers, ``run_slash`` for ``/kanban``.
...
from hbm_cli import kanban_swarm as ks
from hbm_cli.kanban_output import (
...
from hbm_cli.kanban_boards import _dispatch_boards
from hbm_cli.kanban_ops import (
    _cmd_daemon, _kanban_config, _cmd_dispatch, _cmd_gc, _cmd_repair, _cmd_tail, _cmd_watch,
)
from hbm_cli.kanban_parser import build_parser  # noqa: F401  (re-exported: hbm_cli.main, run_slash)
...
def _check_dispatcher_presence(hbm_home: Optional[Path] = None) -> tuple[bool, str]:
    """``(running, message)`` for the "will anything dispatch this?" warning: True when a gateway is
    alive for this HBM_HOME with ``kanban.dispatch_in_gateway`` on, else False + human guidance.
    Fails OPEN (probe/config errors -> ``(True, "")``) — a missed warning beats crying wolf.
    ``hbm_home`` scopes the probe to a profile dir (dashboard backend); CLI callers pass None.

    The dashboard plugin API passes it because the dashboard backend process can be running under a
    different HBM_HOME than the profile the request targets, which otherwise produced a "no gateway is
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(profile_dir=hbm_home, use_cache=False)
    except Exception:
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``hbm-agent kanban …``; returns a shell-style exit code."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: hbm-agent kanban <action> [options]\n"
                  "Run 'hbm-agent kanban --help' for the full list of actions.", file=sys.stderr)
        return 0
    ...
    # `--board <slug>` pins HBM_KANBAN_BOARD for the duration of this call so it inherits the
    ...
    with board_scope:
        ...
        # "no such table: tasks" on first use from a fresh HBM_HOME.
        try:
            kb.init_db()
...
def _profile_author() -> str:
    ...
    for env in ("HBM_PROFILE_NAME", "HBM_PROFILE"):
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.hbm/profiles/.\n"
              "Create one with `hbm-agent -p <name> setup` before assigning tasks.")
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if _json_out(args, data):
        return 0
    if not data:
        print("(no assignees — create a profile with `hbm-agent -p <name> setup`)")
        return 0
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            creator_task_id=(os.environ.get("HBM_KANBAN_TASK")
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(f"Board: {kb.get_current_board()} ({other_count} other board{'s' if other_count != 1 else ''} — "
              f"`hbm-agent kanban boards list`)\n")
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    from hbm_cli import kanban_diagnostics as kd
...
def _cmd_link(args: argparse.Namespace) -> int:
    ...
    if gated:
        print(
            ...
            f"completes; use `hbm-agent kanban unlink {args.parent_id} {args.child_id}` "
            f"to run it now."
        )
    return 0
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    ...
    raw = os.environ.get("HBM_KANBAN_RUN_ID")
    if os.environ.get("HBM_KANBAN_TASK") != task_id or not raw:
        return None
    try:
        return int(raw)
    except ValueError:
        ...
        def op(tid):
            ...
            try:
                ...
                fail_msg[tid] = (f"cannot complete {tid}: a live worker is running it. Wait for the "
                                 f"worker, `hbm-agent kanban reclaim {tid}` to release it, or re-run with "
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from hbm_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from hbm_cli import kanban_decompose as decomp
