...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/hbm-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/hbm-update-backups/orphan-{branch}-"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_hbm_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from hbm_constants import get_hbm_home
        (get_hbm_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from hbm_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        "\nℹ Your fork is not tracking the official HBM AGENT repository.\n"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from hbm_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_hbm_root(), Path(get_hbm_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from hbm_cli.update_cmd import _locate_real_git
