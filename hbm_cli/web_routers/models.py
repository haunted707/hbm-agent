...
from hbm_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
from hbm_cli.web_routers._common import _CONFIG_MUTATION_LOCK, config_write_scope, http_failure
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        ...
        from hbm_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    with http_failure("PUT /api/model/moa failed", 500, detail="Failed to save MoA config"):
        from hbm_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.hbm/config.yaml`` — applies to **new** sessions only; a running chat
