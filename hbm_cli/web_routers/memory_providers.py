...
from hbm_cli.web_server_dashboard import _invalidate_plugins_hub_cache
from hbm_cli.web_server_memory import (
...
from hbm_cli.web_models import MemoryProviderConfigUpdate, MemoryProviderSetupRequest
from hbm_cli.web_routers._common import _CONFIG_MUTATION_LOCK, scoped_to_thread
...
_dependency_importable = late("_dependency_importable", "hbm_cli.web_server_memory")
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_hbm_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # read-only and installs must go to HBM_LAZY_INSTALL_TARGET, which
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        ...
        if _BaseMemoryProvider is None or type(provider).save_config is not _BaseMemoryProvider.save_config:
            provider.save_config(values, str(get_hbm_home()))
            return
    with _CONFIG_MUTATION_LOCK:  # RMW span vs. the dashboard's config autosave
        cfg = load_config()
