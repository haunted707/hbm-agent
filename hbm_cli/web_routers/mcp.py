...
from hbm_cli.web_server_mcp import _mcp_oauth_flows, _mcp_server_summary, _normalize_mcp_server_create
from hbm_cli.web_models import MCPCatalogInstall, MCPEnabledToggle, MCPServerCreate, MCPServersReplace
from hbm_cli.web_routers._common import (
    _profile_cli_args, _profile_scope, _spawn_hbm_action, config_write_scope, http_failure,
...
_run_dashboard_mcp_oauth = late("_run_dashboard_mcp_oauth", "hbm_cli.web_server_mcp")
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from hbm_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from hbm_cli.mcp_config import _get_mcp_servers, _save_bearer_auth_token, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    from hbm_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from hbm_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from hbm_cli.mcp_config import _get_mcp_servers, _oauth_tokens_present, _probe_single_server
    ...
    try:  # probe blocks on a dedicated MCP event loop — keep it off the FastAPI loop
        ...
        from hbm_cli.mcp_config import redact_mcp_probe_text
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    def _home() -> str:
        return str(get_hbm_home().expanduser().resolve(strict=False))
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(f.server_name == name and f.hbm_home == flow_home for f in live):
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to HBM AGENT and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to HBM AGENT for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to HBM AGENT.</p>")
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from hbm_cli import mcp_catalog
    from hbm_cli.config import validate_env_var_name_for_write
    ...
    if entry.install is not None:
        ...
        try:
            _spawn_hbm_action(_profile_cli_args(effective_profile) + ["mcp", "install", name], action)
        except HTTPException:
            raise
        except Exception as exc:
