...
Extracted from ``hbm_cli.web_server``; app state and helpers are late-bound through
:mod:`hbm_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from hbm_cli.web_deps import LateState, late
from hbm_cli.web_server_gateway import _display_system_platform
...
from hbm_cli.config import get_config_path, get_env_path
from hbm_constants import get_process_hbm_home, profile_name_for_home
from hbm_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from hbm_cli.web_routers._common import scoped_to_thread
from pathlib import Path
from typing import Any, Dict, Optional

_log = logging.getLogger("hbm_cli.web_server")
router = APIRouter()
# Mounted separately by web_server so /api/logs keeps its original route-table position.
logs_router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "hbm_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "hbm_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "hbm_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "hbm_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "hbm_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "hbm_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "hbm_cli.web_server_lifecycle")
_spawn_hbm_action = late("_spawn_hbm_action", "hbm_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "hbm_cli.config")
get_hbm_home = late("get_hbm_home", "hbm_cli.config")
...
_open_session_db_for_profile = late("_open_session_db_for_profile", "hbm_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another HBM AGENT process uses it."""
    from hbm_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from hbm_cli.web_server import _SSH_OWNER_NONCE
...
# Profile segment mirrors hbm_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level HBM_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``hbm-agent --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_hbm_home())
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from hbm_cli.dashboard_auth import (
...
def _nous_session_validity() -> str:
    ...
    try:
        from hbm_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from hbm_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_hbm_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from hbm_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from hbm_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of hbm_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_hbm": not _dashboard_local_update_managed_externally(),
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_hbm_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (HBM AGENT Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "hbm_home": str(get_hbm_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond hbm home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "hbm_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_hbm_home()))
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``hbm <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_hbm_action(argv, name)
    except Exception as exc:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from hbm_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from hbm_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    from hbm_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_hbm_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from hbm_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
