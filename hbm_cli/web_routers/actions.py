"""Gateway restart/drain, HBM AGENT update and background-action status dashboard routes.
...
from hbm_cli.config import format_docker_update_message, recommended_update_command_for_method
from hbm_cli.web_deps import LateState, late
from hbm_cli.web_server_gateway import _ACTION_LOG_FILES
...
detect_install_method = late("detect_install_method", "hbm_cli.config")
get_hbm_home = late("get_hbm_home", "hbm_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "hbm_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "hbm_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "hbm_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "hbm_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import hbm_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== hbm-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "HBM AGENT updates are managed outside this dashboard in containerized environments."
...
async def gateway_migrate_plan():
    ...
    from hbm_cli.gateway_migrate import build_migration_plan
...
async def gateway_migrate():
    ...
    with http_failure("Failed to spawn gateway migrate", 500, "Failed to start gateway migration"):
        proc = _spawn_hbm_action(["gateway", "migrate", "--multiplex", "--yes"], "gateway-migrate")
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("hbm-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "hbm-update", "error": error, "message": message,
...
async def update_hbm():
    ...
    existing = _ACTION_PROCS.get("hbm-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "hbm-update", "already_running": True}
        action_id = _ACTION_IDS.get("hbm-update")
        if action_id:
...
_NON_APPLYABLE_MESSAGES = {
    ...
    "apt": lambda: "HBM AGENT is managed by Termux APT; run `pkg upgrade hbm-agent`.",
}


@router.get("/api/hbm/update/check")
async def check_hbm_update(force: bool = False):
    """Report whether a HBM AGENT update is available, without applying it.
    ...
    try:
        from hbm_cli.banner import check_for_updates, upstream_commits_behind

        if force:
            with contextlib.suppress(OSError):
                (get_hbm_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "hbm-update":
        ...
        if durable_update_action_id:
            marker = f"=== hbm-update completed {durable_update_action_id} ==="
...
@status_router.get("/api/hbm/update/receipt")
