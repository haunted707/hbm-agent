...
from hbm_cli import config as config_mod, web_deps
from hbm_cli.web_routers._common import _CONFIG_MUTATION_LOCK
from hbm_cli.local_runtime import (
...
def _assign_default(job: Dict[str, Any], model_id: str) -> None:
    ...
    web_deps.late("_apply_model_assignment_sync", "hbm_cli.web_server_config")("main", "llamacpp", model_id, "", "", "")
...
def _terminate_state_pid() -> None:
    ...
    from hbm_cli.local_runtime.recovery import stop_recorded_orphan

    if not stop_recorded_orphan():
        raise HTTPException(status_code=409, detail=(
            "Another HBM AGENT process owns this server, or its ownership could not be verified"))
...
async def local_models_sideload(body: SideloadBody):
    ...
    impossible) and bounce the router. The original stays put; delete-from-HBM AGENT removes only our link."""
    src = Path(body.path)
