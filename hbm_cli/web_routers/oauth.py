...
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "hbm_cli.web_server_oauth")
...
_nous_plain_poller = late("_nous_plain_poller", "hbm_cli.web_server_oauth")
_nous_promotion_poller = late("_nous_promotion_poller", "hbm_cli.web_server_oauth")
_minimax_poller = late("_minimax_poller", "hbm_cli.web_server_oauth")
_xai_device_poller = late("_xai_device_poller", "hbm_cli.web_server_oauth")
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before HBM AGENT "
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from hbm_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from hbm_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from hbm_cli.auth import _save_codex_tokens
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        if status_fn is not None:
            return status_fn()
        from hbm_cli import auth as hauth
...
async def _start_nous_device_code(profile: Optional[str]) -> Dict[str, Any]:
    ...
    from hbm_cli.auth import PROVIDER_REGISTRY, _request_device_code
    from hbm_cli.web_server_profiles import _config_profile_scope, _profile_scope
    pconfig = PROVIDER_REGISTRY["nous"]
    portal_base_url = (
        os.getenv("HBM_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL") or pconfig.portal_base_url
    ).rstrip("/")
...
async def _start_xai_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from hbm_cli.auth import _xai_oauth_request_device_code
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside HBM AGENT — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    try:
        from hbm_cli.provider_catalog import provider_catalog
...
def _clear_anthropic_auth() -> bool:
    """Clear only the HBM AGENT-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_hbm_oauth_file
        oauth_file = _get_hbm_oauth_file()
...
async def disconnect_oauth_provider(provider_id: str, request: Request, profile: Optional[str] = None):
    ...
    def _run():
        ...
        try:
            from hbm_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
