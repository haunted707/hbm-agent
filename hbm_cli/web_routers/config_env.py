...
from hbm_cli.web_routers._common import http_failure, scoped_to_thread
...
from hbm_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, redact_key, _deep_merge
from hbm_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, CustomEndpointUpdate
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "hbm_cli.web_server_messaging")
...
async def update_config(
    ...
    def _run():
        ...
        # different HBM_HOME than this process's gateway sessions.
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level HBM_HOME (mirrors ``/api/config``).
    """
