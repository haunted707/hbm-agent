...
Shared helpers are reached via the late-binding seam in :mod:`hbm_cli.web_deps`
...
from hbm_cli.web_deps import late
from hbm_cli.config import get_process_hbm_home
from hbm_cli.web_server_config import (
    _apply_main_model_assignment, _normalize_main_model_assignment, _validated_main_model_selection,
)
from hbm_cli.web_server_gateway import _strip_session_list_rows
from hbm_cli.web_routers._common import _CONFIG_MUTATION_LOCK
from hbm_cli.web_server_profiles import (
...
from hbm_cli.web_models import (
    ...
    SessionPrScanBody)
from hbm_cli.web_server_profiles import _hbm_home_scope
...
_cron_profile_home = late("_cron_profile_home", "hbm_cli.web_server_cron")
...
def _profile_setup_command(name: str) -> str:
    ...
    return "hbm-agent setup" if name == "default" else f"{name} setup"


def _write_profile_model(profile_dir: Path, provider: str, model: str, validate_in: Optional[Path] = None) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (HBM_HOME-scoped)
    ...
    with _hbm_home_scope(validate_in or profile_dir):
        provider, model = _normalize_main_model_assignment(provider, model)
        result = _validated_main_model_selection(load_config(), provider, model)
    with _hbm_home_scope(profile_dir), _CONFIG_MUTATION_LOCK:  # RMW span
        cfg = load_config()
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    from hbm_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map hbm_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _set_model() -> bool:
        ...
        try:
            _write_profile_model(path, provider, model, validate_in=get_process_hbm_home())
    ...
    def _spawn_install(ident: str):
        return _spawn_hbm_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``hbm-agent profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``hbm-agent profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the HBM_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``hbm-agent profile
...
# ── Export / Import ── wraps hbm_cli.profiles.export_profile / import_profile. Paths are
