...
def _profile_setup_command(name: str) -> str:
    ...
    return "hbm setup" if name == "default" else f"{name} setup"
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``hbm profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``hbm profile use``); does not retarget the
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``hbm profile
