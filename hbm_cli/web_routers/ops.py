...
from hbm_cli.config import redact_key
from hbm_cli.web_deps import late
from hbm_cli.web_server_files import _path_is_under
from hbm_cli.web_server_gateway import _restart_gateway_after
from hbm_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from hbm_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
from hbm_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("hbm_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "hbm_cli.web_server_memory")
...
save_config = late("save_config", "hbm_cli.config")


def _spawn_action(argv: List[str], name: str, *, log_msg: str, prefix: str) -> dict:
    """Spawn a dashboard-profile ``hbm <argv>`` action; spawn failure -> 500."""
...
# --- Webhooks: same JSON store as the CLI (hbm_cli.webhook); the adapter
...
async def start_gateway(profile: Optional[str] = None):
    ...
    with http_failure("Failed to spawn gateway start", 500, "Failed to start gateway"):
        proc = _spawn_hbm_action(_gateway_subcommand(profile, "start"), "gateway-start")
...
async def stop_gateway(profile: Optional[str] = None):
    ...
    with http_failure("Failed to spawn gateway stop", 500, "Failed to stop gateway"):
        proc = _spawn_hbm_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
...
async def run_backup(body: BackupRequest):
    ...
    if not output:
        ...
        archive = _dashboard_backup_dir() / f"hbm-backup-{stamp}-{secrets.token_hex(4)}.zip"
        try:
...
async def list_hooks():
    ...
    def _run():
        ...
        with contextlib.suppress(Exception):
            from hbm_cli.plugins import VALID_HOOKS
...
async def create_hook(body: HookCreate):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from hbm_cli.plugins import VALID_HOOKS as valid_hooks
...
async def list_checkpoints():
    ...
    cp_dir = get_hbm_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
