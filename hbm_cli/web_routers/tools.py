...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # install `hbm tools` runs — otherwise the toggle "saves" but the tool
...
async def select_toolset_provider(
    ...
    ``hbm tools``, so both write identical keys).
...
async def run_toolset_post_setup(
    ...
    """Spawn ``hbm tools post-setup <key>`` (long-running installs) as a
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``hbm computer-use permissions grant`` (macOS-only: launches
