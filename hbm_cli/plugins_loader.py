...
dependency/config-schema warnings. Mixed into :class:`hbm_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``hbm_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from hbm_constants import get_hbm_home, reset_hbm_home_override, set_hbm_home_override
from registration_lifecycle import replacement_coordinator
from hbm_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from hbm_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from hbm_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from hbm_cli.plugins import LoadedPlugin

logger = logging.getLogger("hbm_cli.plugins")

_NS_PARENT = "hbm_plugins"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable HBM AGENT home."""
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        """
        from hbm_cli.plugins import PluginContext, _PLUGINS_DEBUG
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        See #64165.
        python_dependencies is a declaration seam ONLY: HBM AGENT validates and prints the requirements with an
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. HBM AGENT does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from hbm_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_hbm gate: skip cleanly (no import, no traceback) on a version mismatch.
        from hbm_cli.plugins_manifest import requires_hbm_error
        reason = requires_hbm_error(manifest)
        if reason:
            loaded.error = reason
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (hbm_cli.plugin_compat).
        from hbm_cli.plugin_compat import disable_reason
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.enabled = True
                from hbm_cli.plugins_ledger import _hook_source_of
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from hbm_cli.plugins import PluginContext
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from hbm_cli.agent_plugins import load_agent_plugin
            package = load_agent_plugin(
                Path(manifest.path), get_hbm_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``hbm_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``hbm_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another HBM AGENT home, or an earlier force
