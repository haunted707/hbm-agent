...
from hbm_cli._subprocess_compat import noninteractive_git_env
from hbm_cli.cli_output import line_input
from hbm_cli.config import cfg_get
from hbm_cli.plugin_capabilities import _child_dict
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if manifest_file is None:
        if not _has_portable_manifest(plugin_dir):
            return {}
        try:
            from hbm_cli.agent_plugins import read_agent_plugin_manifest
...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.hbm/.env``."""
    env_specs = [
...
def _print_python_dependencies(manifest: dict, console) -> None:
    """Print declared ``python_dependencies`` with an install hint — HBM AGENT never auto-installs
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_hbm_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_hbm_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_hbm_home()}/.env later)[/dim]")
...
def _install_metadata_path() -> Path:
    return get_hbm_home() / "plugins" / ".install-metadata.json"
...
def _check_manifest_version(manifest: dict, plugin_name: str) -> None:
    ...
    if mv_int > _SUPPORTED_MANIFEST_VERSION:
        ...
        raise PluginOperationError(
            ...
            f"Run {recommended_update_command()} to update HBM AGENT.",
        ) from None
...
def _ensure_tree_readable(root: Path, plugins_dir: Path) -> None:
    """Refuse to ship a tree HBM AGENT cannot read back. A clone can land unreadable (Windows ACL
...
def cmd_install(
    ...
    a ``.hbm-catalog.json`` sidecar; URLs/shorthand are flagged as custom (unreviewed) sources. Every
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    """
    from hbm_cli import plugins_cmd_catalog as catalog
    console = _console()
    entry = None
    if catalog.looks_like_catalog_name(identifier):
        ...
        console.print("[yellow]Warning:[/yellow] custom (unreviewed) source — not from the HBM AGENT catalog.")
    if allow_removed:
        console.print(
    ...
    if not _looks_like_plugin_dir(target):
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid HBM AGENT plugin.")
...
def cmd_update(name: str) -> None:
    ...
    if declared_caps:
        from hbm_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from hbm_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
    ...
    def _refuse_legacy_relay(plugin: str) -> None:
        if plugin in LEGACY_RELAY_PLUGIN_KEYS:
            _fail(console, (
                ...
                f"by HBM AGENT core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."))
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from hbm_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from hbm_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from hbm_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
...
def _discover_all_plugins() -> list:
    ...
    from hbm_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from hbm_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from hbm_cli.curses_ui import _addnstr, flush_stdin
...
def dashboard_install_plugin(
    ...
    if catalog_name:
        ...
        if entry is None:
            return {"ok": False, "error": f"'{catalog_name}' is not in the HBM AGENT plugin catalog."}
        identifier = entry.install_identifier
    else:
        warnings.append("Custom (unreviewed) source — not from the HBM AGENT catalog.")
    try:
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.hbm/plugins/<name>`` if it exists."""
    try:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.hbm/plugins/<name>``."""
    from hbm_cli import plugins_cmd_catalog as catalog
...
def _run_plugin_git(
    ...
    if auth_url:
        from hbm_cli.git_credentials import with_git_auth
...
def _autostash_dirty_tree(git_exe: str, target: Path) -> tuple[bool, str]:
    ...
    push = _run_plugin_git(
        git_exe, target, "stash", "push", "--include-untracked", "-m", "hbm-plugin-update-autostash")
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    marketplace now succeeds when its checkout has local changes"), and the same autostash approach ``hbm-agent
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.hbm/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from hbm_cli.plugin_dev import doctor_plugin
...
def _catalog():
    from hbm_cli import plugins_cmd_catalog
    return plugins_cmd_catalog


def _action_pack(args):
    from hbm_cli.plugin_packs import pack_command
    pack_command(args)
