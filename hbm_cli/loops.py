...
``hbm_cli/goals.py``); CLI, gateway, and TUI all drive it through :class:`LoopManager`.
"""
...
def _get_session_db() -> Optional[Any]:
    ...
    """
    try:
        from hbm_cli.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from hbm_cli.goals import _warn_dropped_write
