"""HBM AGENT — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m hbm_cli.main web [--port 8080]``.
"""
...
from hbm_cli.install_identity import get_install_id as _shared_get_install_id
from hbm_cli.pty_session import run_reaper
...
from hbm_cli import __version__
...
try:
    ...
    # running `hbm-agent dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["HBM_WEB_DIST"]) if "HBM_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)


from hbm_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``hbm-agent dashboard`` backend, not a gateway, so without
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            ...
            if initial_profile_homes:
                ...
                from hbm_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `hbm-agent update` otherwise 500s
    ...
    # Import hbm_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `hbm-agent update` replaced the code
    ...
    # Desktop-spawned backends (HBM_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hbm-agent
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("HBM_DESKTOP") == "1":
        ...
        try:
            from hbm_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from hbm_cli.config import load_config
            from hbm_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    # when HBM_GUEST_ONBOARDING=1, records the answer for setup.status / free_tier.status and
    ...
    from hbm_cli.free_tier_bootstrap import start_background_bootstrap
    ...
    try:
        ...
        try:
            from hbm_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="HBM AGENT", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from hbm_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402

app.include_router(_memory_oauth_router)

# Session token for sensitive endpoints. The desktop shell mints it via
# HBM_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
# the process and is injected into the SPA HTML so only the web UI can use it.
def _resolve_session_token() -> str:
    return os.environ.get("HBM_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-HBM AGENT-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".hbm-ssh-runtime-{nonce}")
...
from hbm_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from hbm_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 hbm-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``HBM_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("HBM_DESKTOP") == "1"
        and bool(os.environ.get("HBM_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from hbm_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from hbm_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from hbm_cli.dashboard_auth.token_auth import token_auth_middleware
...
from hbm_cli import web_server_gateway as _gateway_mod  # noqa: E402
from hbm_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from hbm_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from hbm_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from hbm_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT HBM AGENT home (not the profile HBM_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``hbm-agent gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_hbm_action(subcommand, "gateway-restart")
...
from hbm_cli.web_routers import (  # noqa: E402
...
from hbm_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    if host in _LOOPBACK_HOST_VALUES:
        ...
        fix_hint = (
            ...
            "(and unset HBM_DASHBOARD_PUBLIC_URL) to restore the "
    ...
    fix_hint += (
        ...
        "  • OAuth: run `hbm-agent dashboard register` (Nous Portal) or "
    ...
    try:
        from hbm_cli.config import load_config as _load_cfg
        from hbm_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`hbm-agent plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 hbm-0day
    ...
    if app.state.auth_required:
        ...
        from hbm_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from hbm_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from hbm_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from hbm_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `hbm-agent serve` (no HBM_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from hbm_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # ACTUAL port — what lets `hbm-agent update` relaunch a manually-started serve
    # on its real endpoint (#63206).
    def _register_identity() -> None:
        from hbm_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    ready_token = "HBM_BACKEND_READY" if headless else "HBM_DASHBOARD_READY"
    ...
    if headless:
        ...
        print(f"  HBM AGENT backend listening on {host}:{actual_port}", flush=True)
    else:
        print(f"  HBM AGENT Web UI → http://{host}:{actual_port}")
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from hbm_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``HBM_SERVE_HEADLESS``).
    ...
    from hbm_cli.resource_limits import apply_nofile_soft_limit
    ...
    try:
        from hbm_cli.nous_auth_keepalive import start_nous_auth_keepalive
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('hbm_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('hbm_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('hbm_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('hbm_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('hbm_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('hbm_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('hbm_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('hbm_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('hbm_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('hbm_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('hbm_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('hbm_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('hbm_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('hbm_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('hbm_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('hbm_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('hbm_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('hbm_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('hbm_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('hbm_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('hbm_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('hbm_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('hbm_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('hbm_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('hbm_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('hbm_cli.web_models', 'HookCreate'),
    'HookDelete': ('hbm_cli.web_models', 'HookDelete'),
    'ImportRequest': ('hbm_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('hbm_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('hbm_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('hbm_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('hbm_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('hbm_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('hbm_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('hbm_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('hbm_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('hbm_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('hbm_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('hbm_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('hbm_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('hbm_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('hbm_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('hbm_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('hbm_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('hbm_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('hbm_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('hbm_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('hbm_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('hbm_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('hbm_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('hbm_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('hbm_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('hbm_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('hbm_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('hbm_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('hbm_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('hbm_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('hbm_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('hbm_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('hbm_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('hbm_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('hbm_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('hbm_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('hbm_cli.web_models', 'SessionImport'),
    'SessionPrune': ('hbm_cli.web_models', 'SessionPrune'),
    'SessionRename': ('hbm_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('hbm_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('hbm_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('hbm_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('hbm_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('hbm_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('hbm_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('hbm_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('hbm_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('hbm_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('hbm_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('hbm_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('hbm_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('hbm_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('hbm_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('hbm_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('hbm_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('hbm_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('hbm_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('hbm_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('hbm_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('hbm_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('hbm_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('hbm_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('hbm_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('hbm_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('hbm_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('hbm_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('hbm_cli.web_routers.mcp', 'auth_mcp_server'),
    'build_cron_model_impact': ('hbm_cli.config', 'build_cron_model_impact'),
    'bulk_delete_sessions_endpoint': ('hbm_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('hbm_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('hbm_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('hbm_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('hbm_cli.config', 'cfg_get'),
    'check_config_version': ('hbm_cli.config', 'check_config_version'),
    'check_hbm_update': ('hbm_cli.web_routers.actions', 'check_hbm_update'),
    'clear_model_endpoint_credentials': ('hbm_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('hbm_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('hbm_cli.config', 'coerce_provider_id'),
    'console_ws': ('hbm_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('hbm_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('hbm_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('hbm_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('hbm_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('hbm_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('hbm_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('hbm_cli.web_routers.ops', 'create_webhook'),
    'cron_fire_webhook': ('hbm_cli.web_routers.cron', 'cron_fire_webhook'),
    'custom_endpoint_key_env': ('hbm_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('hbm_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('hbm_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('hbm_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('hbm_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('hbm_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('hbm_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('hbm_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('hbm_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('hbm_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('hbm_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('hbm_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('hbm_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('hbm_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('hbm_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('hbm_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('hbm_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('hbm_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('hbm_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('hbm_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('hbm_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('hbm_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('hbm_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('hbm_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('hbm_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('hbm_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('hbm_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('hbm_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('hbm_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('hbm_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('hbm_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('hbm_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('hbm_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('hbm_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('hbm_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('hbm_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('hbm_cli.config', 'get_config_path'),
    'get_config_raw': ('hbm_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('hbm_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('hbm_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('hbm_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('hbm_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('hbm_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('hbm_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('hbm_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('hbm_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('hbm_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('hbm_cli.config', 'get_env_path'),
    'get_env_vars': ('hbm_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('hbm_cli.web_routers.status', 'get_health'),
    'get_hbm_home': ('hbm_cli.config', 'get_hbm_home'),
    'get_learning_graph': ('hbm_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('hbm_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('hbm_cli.web_routers.status', 'get_logs'),
    'get_media': ('hbm_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('hbm_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('hbm_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('hbm_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('hbm_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('hbm_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('hbm_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('hbm_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('hbm_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('hbm_cli.web_routers.status', 'get_portal_status'),
    'get_process_hbm_home': ('hbm_cli.config', 'get_process_hbm_home'),
    'get_profile_setup_command': ('hbm_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('hbm_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('hbm_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('hbm_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('hbm_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('hbm_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('hbm_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('hbm_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('hbm_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('hbm_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('hbm_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('hbm_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('hbm_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('hbm_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('hbm_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('hbm_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('hbm_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('hbm_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('hbm_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('hbm_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('hbm_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('hbm_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('hbm_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('hbm_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('hbm_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('hbm_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('hbm_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('hbm_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('hbm_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('hbm_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('hbm_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('hbm_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('hbm_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('hbm_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('hbm_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('hbm_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('hbm_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('hbm_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('hbm_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('hbm_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('hbm_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('hbm_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('hbm_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('hbm_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('hbm_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('hbm_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('hbm_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('hbm_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('hbm_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('hbm_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('hbm_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('hbm_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('hbm_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('hbm_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('hbm_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('hbm_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('hbm_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('hbm_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('hbm_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('hbm_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('hbm_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('hbm_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('hbm_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('hbm_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('hbm_cli.config', 'load_env'),
    'mcp_oauth_callback': ('hbm_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('hbm_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('hbm_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('hbm_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('hbm_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('hbm_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('hbm_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('hbm_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('hbm_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('hbm_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('hbm_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('hbm_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('hbm_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('hbm_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('hbm_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('hbm_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('hbm_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('hbm_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('hbm_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('hbm_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('hbm_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('hbm_cli.config', 'remove_env_value'),
    'remove_env_var': ('hbm_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('hbm_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('hbm_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('hbm_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('hbm_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('hbm_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('hbm_cli.web_routers.ops', 'reset_memory'),
    'resolve_cron_model_drift_defaults': ('hbm_cli.config', 'resolve_cron_model_drift_defaults'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('hbm_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('hbm_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('hbm_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('hbm_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('hbm_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('hbm_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('hbm_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('hbm_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('hbm_cli.doctor', 'run_doctor'),
    'run_dump': ('hbm_cli.dump', 'run_dump'),
    'run_import': ('hbm_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('hbm_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('hbm_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('hbm_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('hbm_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('hbm_cli.config', 'save_config'),
    'save_env_value': ('hbm_cli.config', 'save_env_value'),
    'save_toolset_env': ('hbm_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('hbm_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('hbm_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('hbm_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('hbm_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('hbm_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('hbm_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('hbm_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('hbm_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('hbm_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('hbm_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('hbm_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('hbm_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('hbm_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('hbm_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('hbm_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('hbm_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('hbm_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('hbm_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('hbm_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('hbm_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('hbm_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('hbm_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('hbm_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('hbm_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('hbm_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('hbm_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('hbm_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('hbm_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('hbm_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('hbm_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('hbm_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('hbm_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('hbm_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('hbm_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('hbm_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('hbm_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('hbm_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('hbm_cli.web_routers.cron', 'update_cron_job'),
    'update_hbm': ('hbm_cli.web_routers.actions', 'update_hbm'),
    'update_learning_node': ('hbm_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('hbm_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('hbm_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('hbm_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('hbm_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('hbm_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('hbm_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('hbm_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('hbm_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('hbm_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('hbm_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('hbm_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('hbm_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('hbm_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('hbm_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('hbm_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('hbm_cli.config', 'write_platform_config_field'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from hbm_cli.plugin_compat import warn_once
