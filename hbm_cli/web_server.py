...
try:
    ...
    # running `hbm dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``hbm dashboard`` backend, not a gateway, so without
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `hbm update` otherwise 500s
    ...
    # refuse with "restart required" after `hbm update` replaced the code
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``hbm gateway restart``, reusing an in-flight or recent restart.
...
def _no_auth_provider_message(host: str) -> str:
    ...
    fix_hint += (
        ...
        "  • OAuth: run `hbm dashboard register` (Nous Portal) or "
    ...
    try:
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`hbm plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _on_server_started(
    ...
    *,
    ...
    # No-op for standalone `hbm serve` (no HBM_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    # ACTUAL port — what lets `hbm update` relaunch a manually-started serve
