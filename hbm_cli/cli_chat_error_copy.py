...
_REASON_COPY: dict[str, str] = {
    "auth": "Your {provider} key was rejected. Run `hbm model` to re-enter it.",
    "auth_permanent": "Your {provider} key was rejected. Run `hbm model` to re-enter it.",
...
_UNKNOWN_COPY = "The model request failed. Run /model to switch or `hbm doctor` to check the setup."
...
def agent_init_failure_message(error: BaseException) -> str:
    ...
    return (
        ...
        "Your message was not sent. Run `hbm doctor` to check the setup, "
