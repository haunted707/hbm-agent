...
_REASON_COPY: dict[str, str] = {
    "auth": "Your {provider} key was rejected. Run `hbm-agent model` to re-enter it.",
    "auth_permanent": "Your {provider} key was rejected. Run `hbm-agent model` to re-enter it.",
...
_UNKNOWN_COPY = "The model request failed. Run /model to switch or `hbm-agent doctor` to check the setup."
...
def agent_init_failure_message(error: BaseException) -> str:
    ...
    return (
        f"HBM AGENT couldn't start the model connection: {_short(str(error)) or type(error).__name__}. "
        "Your message was not sent. Run `hbm-agent doctor` to check the setup, "
