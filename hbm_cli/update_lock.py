"""Cross-process mutual exclusion for in-flight HBM AGENT updates.
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "HBM AGENT is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* HBM AGENT home (never the context-local profile override): the Rust
    updater resolves ``$HBM_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_hbm_home() / MARKER_NAME
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another HBM AGENT update is already running (started {elapsed} ago, "
