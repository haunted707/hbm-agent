...
``~/.hbm/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
...
def _cache_path() -> Path:
    ...
    return get_hbm_home() / "cache" / "model_catalog.json"
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _HBM_USER_AGENT})
...
def refresh_catalogs() -> bool:
    ...
    try:
        from hbm_cli.models import fetch_nous_recommended_models, fetch_openrouter_models
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model HBM AGENT silently lands on when the
