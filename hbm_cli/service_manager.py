...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged hbm user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every HBM AGENT runtime call
    inside the container drops to hbm via ``s6-setuidgid``, that silent failure made the entire
...
class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``hbm_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import hbm_cli
        importlib.import_module(f"hbm_cli.{self._backend}")
        return getattr(hbm_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in hbm_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in hbm_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``hbm_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's HBM_HOME blindly: derive the shared profile root and map the suffix to the root
    default profile or ``<root>/profiles/<profile>``.
    """
    profile = _profile_from_service(name)
    validate_profile_name(profile)
    hbm_home = Path(os.environ.get("HBM_HOME", "/opt/data"))
    root = hbm_home.parent.parent if hbm_home.parent.name == "profiles" else hbm_home
...
# UID/GID of the in-image ``hbm-agent`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_HBM_UID = 10000
_HBM_GID = 10000


def _chown_hbm(path: Path) -> None:
    try:
        os.chown(path, _HBM_UID, _HBM_GID)
    except PermissionError:
        # Already running as hbm → the dir is hbm-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create hbm-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    hbm user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources HBM_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to hbm.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root HBM_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``HBM_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``hbm_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        lines.append("export HBM_S6_SUPERVISED_CHILD=1")
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            f': "${{HBM_HOME:=/opt/data}}"\n'
            f'log_dir="$HBM_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock AS HBM when starting as root. Never
            # chown/unlink hbm-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded hbm-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid hbm mkdir -p "$log_dir"\n'
            f'  s6-setuidgid hbm rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid hbm s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed hbm-owned supervise/ BEFORE publishing so the hbm-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is hbm-owned (see ``_seed_supervise_skeleton``)
