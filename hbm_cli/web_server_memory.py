...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    values: Dict[str, Any] = {}
    for path in (hbm_home / f"{name}.json", hbm_home / name / "config.json"):
    ...
    # Holographic stores under plugins.hbm-memory-store.
    plugins_cfg = cfg.get("plugins")
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("hbm-memory-store")
