...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible HBM AGENT defaults, tweakable in the Slack
    UI after pasting.
    """
    from hbm_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with HBM AGENT in threads and DMs."}
        ...
        features["agent_view"] = {"agent_description": "Chat with HBM AGENT in Slack Messages."}
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your HBM AGENT on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``hbm_cli/main.py``)."""
    name = getattr(args, "name", None) or "HBM AGENT"
    description = getattr(args, "description", None) or "Your HBM AGENT on Slack"
    ...
    if isinstance(write_target, bool) and write_target:  # bare --write → default location
        ...
        target = Path(get_hbm_home()) / "slack-manifest.json"
    else:
    ...
    print(
        "\nNext steps:\n"
        "  1. Open https://api.slack.com/apps and pick your HBM AGENT app\n"
