...
maps. Split out of ``hbm_cli.models``.
"""
...
# Mirrors the xAI-direct IDs from $HBM_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append HBM AGENT-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $HBM_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # (see hbm_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# Offline/fresh-install fallback for the model HBM AGENT silently lands on when the user never picked
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: HBM AGENT' default
