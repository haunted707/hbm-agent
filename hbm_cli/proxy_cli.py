...
from hbm_cli.config import load_config, load_env, save_config
...
def _setup_mint_tokens(console: Console, args: argparse.Namespace):
    ...
    if args.from_bitwarden:
        ...
        # Operators commonly keep provider keys only in ~/.hbm/.env (loaded when the agent
        # runs, NOT exported into an interactive shell); backfill so discovery sees them.
        loaded = _load_env_file_into_environ()
        if loaded:
            console.print(f"  [dim]Loaded {loaded} provider key name(s) from ~/.hbm/.env for discovery.[/dim]")
    ...
    if rotate and existing:
        ...
        if sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running HBM AGENT sandbox.  They will start 401-ing against upstreams until restarted."
            )
...
def _load_env_file_into_environ() -> int:
    """Backfill known provider keys from ``~/.hbm/.env`` into ``os.environ``; returns the count.
