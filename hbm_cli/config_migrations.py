...
in :func:`run_migrations`. Every write goes through ``hbm_cli.config._persist_migration`` so a
...
def _cfg():
    """Return the live ``hbm_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
    from hbm_cli import config
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$HBM_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
        user_plugins_dir = _c.get_hbm_home() / "plugins"
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        curator_dir = _c.get_hbm_home() / "logs" / "curator"
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from hbm_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_41(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from tools.bot_mode_probe import _PROTOCOL_HEADING, _hbm_root, _roster, strip_legacy_protocol

    cleaned: List[str] = []
    for name, profile_dir in _roster(_hbm_root(get_hbm_home())):
...
def _migrate_to_45(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from hbm_cli.tools_config import _configurable_keys, _get_plugin_toolset_keys
    ...
    for platform, toolsets in saved.items():
        ...
        if not toolset_allowed_for_platform("connections", platform):
            continue
        # A composite like [hbm-cli] already inherits every core tool at read time.
