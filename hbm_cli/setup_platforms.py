...
def _profile_name_from_hbm_home(hbm_home) -> str | None:
    """Return the active profile name when HBM_HOME is a profile dir."""
    return hbm_home.name if hbm_home.parent.name == "profiles" else None


def _setup_telegram_auto_result():
    """Attempt automatic Telegram bot creation via managed QR onboarding."""
    from hbm_cli.setup import get_hbm_home
    try:
        from hbm_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    profile_name: str | None = None
    with contextlib.suppress(Exception):
        profile_name = _profile_name_from_hbm_home(Path(get_hbm_home()))
...
def declines_reconfigure(label: str, question: str, *env_vars: str) -> bool:
    ...
    """
    from hbm_cli.setup import get_env_value, print_info, prompt_yes_no
...
def save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from hbm_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from hbm_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from hbm_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from hbm_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from hbm_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from hbm_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _obtain_telegram_token():
    ...
    from hbm_cli.setup import _info, print_error, prompt
...
def _setup_telegram():
    ...
    from hbm_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
    ...
    _info(None, "📬 Home Channel: where HBM AGENT delivers cron job results,",
...
def _setup_bluebubbles():
    ...
    from hbm_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
    print_header("BlueBubbles (iMessage)")
    if declines_reconfigure("BlueBubbles", "Reconfigure BlueBubbles?", "BLUEBUBBLES_SERVER_URL"):
        return
    _info("Connects HBM AGENT to iMessage via BlueBubbles — a free, open-source",
...
def _setup_webhooks():
    ...
    from hbm_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
...
def _warn_missing_home_channels() -> None:
    ...
    from hbm_cli.setup import get_env_value, _info, print_warning
    missing_home = [
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from hbm_cli.setup import print_error, prompt_yes_no
...
def setup_gateway(config: dict):
    ...
    from hbm_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from hbm_cli.gateway import _all_platforms, _platform_status, _configure_platform
    print_header("Messaging Platforms")
    _info("Connect to messaging platforms to chat with HBM AGENT from anywhere.",
