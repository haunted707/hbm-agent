...
def _is_hbm_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for HBM AGENT ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current HBM AGENT gateway shapes get ``--external-
...
def _prepare_child_command(command: Sequence[str], environ: Mapping[str, str] | None = None) -> list[str]:
    ...
    if EXTERNAL_SUPERVISOR_FLAG not in argv and xpc_service and xpc_service != "0" and _is_hbm_gateway_run_argv(argv):
