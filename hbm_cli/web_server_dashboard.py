...
from hbm_cli.config import cfg_get, get_process_hbm_home
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    ``hbm_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
    from hbm_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    target palette instead of flashing the bundle's default HBM AGENT Teal until
...
def mount_spa(application: FastAPI):
    ...
    proxy (``X-Forwarded-Prefix: /hbm``) the served index.html is rewritten so absolute
    asset URLs and the runtime ``__HBM_BASE_PATH__`` honour that prefix without a rebuild.
        ...
        async def no_frontend(full_path: str):
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__HBM_SESSION_TOKEN__={json.dumps(_server()._SESSION_TOKEN)};"
                    "window.__HBM_AUTH_REQUIRED__=false;"
    ...
    # A missing WEB_DIST is deliberately NOT a mount-time terminal state (#82614): a long-lived `hbm-agent
    ...
    def _serve_index(prefix: str = ""):
        ...
        cookie auth; ``__HBM_AUTH_REQUIRED__`` tells it which scheme to use for /api/pty
        ...
        token_js = "" if gated else f'window.__HBM_SESSION_TOKEN__="{_server()._SESSION_TOKEN}";'
        bootstrap_script = (
            f"<script>{token_js}"
            f"window.__HBM_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__HBM_BASE_PATH__="{prefix}";'
            f"window.__HBM_AUTH_REQUIRED__={'true' if gated else 'false'};"
            f"</script>"
        )
        if prefix:
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "HBM AGENT Teal",         "description": "Classic dark teal — the canonical HBM AGENT look"},
    {"name": "default-large", "label": "HBM AGENT Teal (Large)", "description": "HBM AGENT Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.hbm/, the same trust level as config.yaml. Empty asset values are dropped so a
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_hbm_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``HBM_HOME``.
    """
    themes_dir = get_process_hbm_home() / "dashboard-themes"
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``HBM_HOME=<root>/profiles/<name>``) the
    launch home has no ``plugins/`` — user plugins live in the hbm-agent root — so the default
    ...
    """
    from hbm_cli.plugins import get_bundled_plugins_dir
    ...
    # context-local HBM_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``HBM_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the hbm-agent root (``~/.hbm/plugins``). Scan the default root as well (``get_default_hbm_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``HBM_HOME`` unchanged when it
    # *is* the root), mirroring how ``hbm_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any) authoritative over same-named root
    # plugins.
    user_plugin_roots = [get_process_hbm_home() / "plugins"]
    root_plugins = get_default_hbm_root() / "plugins"
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``hbm_cli/plugins.py`` and the documented user
    # contract.
    if env_var_enabled("HBM_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".hbm" / "plugins", "project"))
    return search_dirs
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``hbm_cli.plugins``."""
    plugins = []
    seen_names: set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from hbm_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from hbm_cli.web_server import _get_dashboard_plugins
    from hbm_cli.config import get_hbm_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from hbm_cli.plugins_cmd_catalog import removed_annotation
    ...
    plugins_root_resolved = (get_hbm_home() / "plugins").resolve()
...
def _plugin_api_mount_skip_reason(plugin: Dict[str, Any], enabled_set: set, disabled_set: set) -> Optional[str]:
    ...
    disable; project plugins (``./.hbm/plugins/``) ship with the CWD and are
...
def _mount_plugin_api_routes():
    ...
    (``./.hbm/plugins/``) ship with the CWD and are therefore attacker-controlled in any threat model
    ...
    """
    from hbm_cli.web_server import _get_dashboard_plugins, app
    try:
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"hbm_dashboard_plugin_{plugin['name']}"
