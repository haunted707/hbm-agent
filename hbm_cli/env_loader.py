"""Helpers for loading HBM AGENT .env files consistently across entrypoints."""
...
# Once-per-process guards: load_hbm_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `hbm-agent
...
# HBM_HOME paths already pulled external secrets for: load_hbm_dotenv() runs at import time from
...
# Behavioral routing keys a parent HBM AGENT process injects into child env that silently redirect a profile
...
def get_secret_source_values(hbm_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``hbm_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(hbm_home).resolve()), {}))


def hydrate_profile_secret_sources(hbm_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(hbm_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_hbm_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
...
def reset_secret_source_cache(hbm_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``hbm_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    if hbm_home is None:
        ...
        return
    home_key = str(Path(hbm_home).resolve())
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from hbm_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hbm_dotenv(
    *,
    hbm_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load HBM AGENT env files: ``~/.hbm/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(hbm_home) if hbm_home else get_process_hbm_home()
    ...
    if is_multiplex_active() and get_hbm_home_override() is not None:
        ...
        if load_external_secrets:
            from hbm_cli import _early_recovery
    ...
    # ~/.hbm/.env win on every reload and flip the backend mid-session in long-lived processes.
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_hbm_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE HBM AGENT reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and hbm_logging reuse one parse instead of 3-4.
    if home_path == _process_hbm_home():
        try:
...
def _process_hbm_home() -> Path:
    """The HBM_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_hbm_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``hbm_constants.get_process_hbm_home()`` is the override-immune
