...
from hbm_cli.auth_constants import httpx
...
UPGRADE_UNAVAILABLE = "The free tier is not available right now; run `hbm-agent auth add nous` to sign in."
UPGRADE_REASON_COPY = {
...
UPGRADE_NO_DEFAULT_TERMINAL = "No default model is set yet; run `hbm-agent model` to pick one."
...
LOGIN_DM_ONLY = "Sign in from a direct message with HBM AGENT."
LOGIN_BUSY_ELSEWHERE = "Another sign-in is already running on this HBM AGENT. Try again in a few minutes."
LOGIN_NOT_ALLOWED = "Only an operator of this HBM AGENT can sign it in."
...
# the in-chat form (never a raw exception, a URL or a ``hbm-agent`` verb) and ``.copy_terminal`` the
...
class Failed(SignInState):
    ...
    def retryable(self) -> bool:
        from hbm_cli import anon_auth as _core
...
def run_sign_in(
    ...
    *client_factory* is the HTTP client seam, ``client_factory(timeout_seconds, verify)``.
    """
    from hbm_cli import anon_auth as _core
    from hbm_cli.auth import PROVIDER_REGISTRY, _resolve_verify
    from hbm_cli.auth_device_flow import _request_device_code
    from hbm_cli.auth_nous import _nous_http_client
