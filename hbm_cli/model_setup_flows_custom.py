...
Imports of hbm_cli.main / auth / config / models stay lazy (main.py import cycle;
...
def _report_context_length_detection(model_name: str, base_url: str, api_key: str) -> None:
    ...
    try:
        ...
        from hbm_cli.banner import _format_context_length
...
def _probe_custom_endpoint(effective_key: str, effective_url: str) -> tuple[dict, str]:
    ...
    from hbm_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(f"Warning: could not verify this endpoint via {probe.get('probed_url')}. HBM AGENT will still save it.")
...
def _model_flow_custom(config):
    ...
    from hbm_cli.main_provider_setup import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from hbm_cli.auth import _save_model_choice, deactivate_provider
    from hbm_cli.config import custom_endpoint_key_env, get_env_value, save_env_value
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `hbm-agent model` to set a model.")
...
def _discover_named_custom_models(provider_info: dict, api_key: str, configured_models: list, explicit_catalog: bool):
    ...
    from hbm_cli.models import fetch_api_models, _get_ollama_native_headers
    ...
    if live_models:
        with contextlib.suppress(Exception):
            from hbm_cli.model_switch_providers import _entry_credentials, _save_discovered_models_to_config
...
def _model_flow_named_custom(config, provider_info):
    ...
    from hbm_cli.main_provider_setup import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from hbm_cli.auth import _save_model_choice
    from hbm_cli.config import load_config, save_config
    from hbm_cli.model_switch import _entry_models_discovered, _models_config_is_allowlist
