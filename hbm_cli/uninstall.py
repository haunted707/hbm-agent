...
def _strip_hbm_path_lines(content: str) -> str:
    """Drop the ``# HBM AGENT`` marker (+ its PATH line) and any hbm PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# HBM AGENT' in line or '# hbm-agent' in line:
            skip_next = True
            continue
        if skip_next and ('hbm' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'hbm' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove HBM AGENT PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_hbm_path_lines(content)
...
def remove_wrapper_script():
    """Remove the hbm-agent wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a hbm_cli / hbm-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('hbm_cli' in content or 'hbm-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("hbm-agent", "hbm-agent-acp", "hbm-agent-run"))
...
def remove_node_symlinks(hbm_home: Path) -> list:
    ...
    node_dir = (hbm_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from hbm_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from hbm_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from hbm_cli import gateway_windows as gw
...
# HBM_HOME / HBM_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\hbm\git\{cmd,bin,usr\bin}, ...\hbm\node), PortableGit + Node copies
...
def _hbm_path_markers(hbm_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying HBM AGENT-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(hbm_home).rstrip("\\/")
    subs = ("hbm-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(hbm_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip HBM AGENT-owned entries from User-scope PATH in the registry (see ``_hbm_path_markers``)."""
    markers = tuple(m.lower() for m in _hbm_path_markers(hbm_home, include_managed_bin=include_managed_bin))
...
def remove_hbm_env_vars_windows() -> list[str]:
    """Delete HBM_HOME and HBM_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("HBM_HOME", "HBM_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(hbm_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``hbm_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (hbm_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``hbm-agent`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from hbm_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from hbm_constants import get_default_hbm_root
        bin_dir = get_default_hbm_root() / "bin"
    except Exception as e:
...
def _is_default_hbm_home(hbm_home: Path) -> bool:
    """Return True when ``hbm_home`` points at the default (non-profile) root."""
    try:
        from hbm_constants import get_default_hbm_root
        return hbm_home.resolve() == get_default_hbm_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    # 1. Gateway service, via `python -m hbm_cli.main` (the `hbm-agent` wrapper may be half-gone).
    hbm_invocation = [sys.executable, "-m", "hbm_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                hbm_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's HBM_HOME directory.
...
def run_gui_uninstall(args):
    ...
    summary = gui_install_summary(hbm_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ☤ HBM AGENT Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No HBM AGENT Chat GUI installation was found.")
        print(f"  Checked: {hbm_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The HBM AGENT stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(hbm_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The HBM AGENT at {hbm_home / 'hbm-agent'}")
        print(f"  • Your config, sessions, and secrets under {hbm_home}")
        print()
...
def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.hbm/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, hbm_home=hbm_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie HBM_HOMEs and systemd units behind.
    is_default_profile = _is_default_hbm_home(hbm_home)
    ...
    print()
    _print_box("│            ☤ HBM AGENT Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {hbm_home / 'config.yaml'}")
    print(f"  Secrets: {hbm_home / '.env'}")
    print(f"  Data:    {hbm_home / 'cron/'}, {hbm_home / 'sessions/'}, {hbm_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, HBM_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL HBM AGENT data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the HBM AGENT code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, hbm_home=hbm_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, hbm_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • HBM AGENT PATH entries from shell configs / Windows User PATH")
    print("  • HBM AGENT wrapper scripts and HBM AGENT-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep HBM AGENT config/data: {hbm_home}")
    else:
        print(f"  • HBM AGENT config/data: {hbm_home}")
        profiles = _discover_named_profiles() if _is_default_hbm_home(hbm_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$HBM_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: hbm_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; hbm\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_hbm_home(hbm_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(hbm_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No HBM AGENT-owned PATH entries in User environment"),
        (windows, "Removing HBM_HOME / HBM_GIT_BASH_PATH User env vars...",
         remove_hbm_env_vars_windows, "Removed User env var: {}", "No HBM AGENT-set User env vars to remove"),
        (True, "Removing hbm-agent command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows hbm-agent launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows hbm-agent launchers found"),
        (True, "Removing HBM AGENT-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(hbm_home), "Removed {}", "No HBM AGENT-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE HBM_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from hbm_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(hbm_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(hbm_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.hbm/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(hbm_home)
    else:
        log_info(f"Keeping configuration and data in {hbm_home}")
    ...
    print()
    print("Thank you for using HBM AGENT! ☤")
    print()
...
def main(argv=None) -> int:
    """``python -m hbm_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``hbm_constants`` + ``hbm_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m hbm_cli.uninstall")
    parser.add_argument(
