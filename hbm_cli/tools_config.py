"""Unified tool configuration for HBM AGENT."""
...
from hbm_cli.cli_output import print_info as _print_info
from hbm_cli.colors import Colors, color
from hbm_cli.config import cfg_get, load_config, save_config, get_env_value
from hbm_cli.nous_subscription import (
    NousSubscriptionFeatures, apply_nous_managed_defaults, get_nous_subscription_features)
from hbm_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from hbm_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS, toolset_allowed_for_platform as _toolset_allowed_for_platform)
# Re-exports: keep ``hbm_cli.tools_config.X`` callers and test patch targets resolving.
from hbm_cli.tools_config_cua import (  # noqa: F401
...
from hbm_cli.tools_config_post_setup import (  # noqa: F401
...
from hbm_cli.tools_config_providers import (  # noqa: F401
...
from hbm_cli.tools_config_mcp import (  # noqa: F401
...
# OFF by default for new installs (still in _HBM_CORE_TOOLS; the checklist won't pre-select them). x_search
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from hbm_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from hbm_cli.plugins import get_plugin_toolset_keys_nowait
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``hbm-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"hbm-{platform}"
...
_LANGFUSE_PUBLIC = ("HBM_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("HBM_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "HBM AGENT routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by HBM AGENT, text-only (no screenshots)",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; HBM_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("HBM_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from hbm_cli.plugins import get_portable_mcp_server_names_nowait
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (hbm-acp, hbm-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``hbm-cli`` would re-enable disabled toolsets). A mixed list (``[hbm-cli, spotify]``) still expands the
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``hbm-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[hbm-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[hbm-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("hbm-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``hbm`` for ``hbm-cli`` → no
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (hbm-cli, ...) resolve to ALL tools and would silently override the user's
...
def _prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from hbm_cli.curses_ui import curses_radiolist
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str], platform: str = "cli", *, force_fresh: bool = True) -> Set[str]:
    ...
    from hbm_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from hbm_cli.setup import setup_telemetry
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        return
    print(color("☤ HBM AGENT Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print(color(f"  Tool configuration saved to {display_hbm_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'hbm-agent' or gateway restart.", Colors.DIM))
    print()
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('hbm_cli.nous_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    ...
    'format_nous_portal_entitlement_message': ('hbm_cli.nous_account', 'format_nous_portal_entitlement_message'),
