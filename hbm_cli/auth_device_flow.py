...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(
        ...
        f"HBM AGENT is waiting for the OAuth callback on {redirect_uri}\n"
...
def _resolve_verify(
    *, insecure: Optional[bool] = None, ca_bundle: Optional[str] = None,
    ...
    from hbm_cli.auth import _default_verify
    ...
    effective_ca = (
        ca_bundle or tls_state.get("ca_bundle") or os.getenv("HBM_CA_BUNDLE")
...
def _poll_for_token(
    ...
    def _error(_response, error_payload) -> Exception:
        ...
        from hbm_cli.auth_error_copy import device_flow_error
    ...
    *resolve* attempts a refresh, so a resolved token should be valid — but double-check the
    ...
    try:
        ...
        if isinstance(api_key, str) and api_key and not is_expiring(api_key, 60):
            print(f"Existing {display_name} credentials found in HBM AGENT auth store.")
