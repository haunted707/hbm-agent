"""Audit log for dashboard-auth events: ``$HBM_HOME/logs/dashboard-auth.log``, one JSON object
per line. Token-like fields are stripped before serialisation so refresh tokens / JWTs never
reach disk. Minimal import surface (no ``hbm_constants`` at import time) so early-loading
...
def _resolve_log_path() -> Path:
    ...
    return get_hbm_home() / "logs" / "dashboard-auth.log"
