...
def _platform_default_toolset(platform: object) -> str:
    ...
    return info.default_toolset if info is not None else f"hbm-{platform}"
...
def _platform_default_is_valid(
    ...
    # hbm-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    Reports: a toolset name ``is_valid_toolset`` rejects (suggesting ``hbm-<platform>`` when that
