"""On-demand supply-chain audit for HBM AGENT installs.
...
def _discover_plugins(hbm_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.hbm/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, hbm_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = hbm_home or Path(get_hbm_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    components = _discover_components(hbm_home=home, **skips)
    ...
    try:
        findings = run_audit(hbm_home=home, components=components)
