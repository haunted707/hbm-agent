"""Agent construction + session-resume display for ``HbmCLI``: credential resolution,
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``hbm-agent chat -q``.
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``HbmCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not callable(api_key) and not (isinstance(api_key, str) and api_key):
            if _keyless_custom_base(base_url):
                ...
                print("   Run 'hbm-agent model' to choose a provider, or "
                      "'hbm-agent setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: hbm-agent setup")
            return False
        ...
        # is just the provider slug/display name (`hbm-agent chat --model <provider-name>`
        ...
        # Still empty (e.g. `hbm-agent auth add` without `hbm-agent model`): fall back to the
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        startup, TTY). Runs the same flow as ``hbm-agent model`` so onboarding has a single
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'hbm-agent model' or 'hbm-agent setup' any time.")
            return False
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'hbm-agent model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠️  Provider setup failed: {exc}")
            _cprint("  Run 'hbm-agent model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'hbm-agent model' to retry.")
        return False
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(hbm-agent chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (hbm-agent sessions list)."
            if _quiet_mode:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if not self._ensure_runtime_credentials():
            return False
        from hbm_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        try:
            ...
            console = ChatConsole()
            from hbm_cli.cli_chat_error_copy import agent_init_failure_message
            console.print(f"[bold red]{_escape(agent_init_failure_message(e))}[/]")
            from hbm_constants import partial_update_hint
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (hbm-agent sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
    ...
    def _display_resumed_history(self):
        ...
        role_styles = {
            ...
            "assistant": ("  ◆ HBM AGENT: ", f"dim bold {_assistant_label_c}", "dim", " " * 12),
            "assistant_last": ("  ◆ HBM AGENT: ", f"bold {_assistant_label_c}", "", " " * 12),  # full, non-dim
        }
        lines = Text()
        if skipped:
