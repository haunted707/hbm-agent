...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``hbm chat -q``.
...
class CLIAgentSetupMixin:
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not callable(api_key) and not (isinstance(api_key, str) and api_key):
            if _keyless_custom_base(base_url):
                ...
                print("   Run 'hbm model' to choose a provider, or "
                      "'hbm setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: hbm setup")
            return False
        ...
        # is just the provider slug/display name (`hbm chat --model <provider-name>`
        ...
        # Still empty (e.g. `hbm auth add` without `hbm model`): fall back to the
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        startup, TTY). Runs the same flow as ``hbm model`` so onboarding has a single
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'hbm model' or 'hbm setup' any time.")
            return False
        try:
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'hbm model' to retry.")
        return False
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(hbm chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (hbm sessions list)."
            if _quiet_mode:
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (hbm sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
