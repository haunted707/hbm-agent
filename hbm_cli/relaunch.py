...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched hbm."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hbm_bin() -> Optional[str]:
    """HBM AGENT entry point: ``sys.argv[0]`` if a real executable, else ``which hbm``, else ``None``
    (caller falls back to ``python -m hbm_cli.main``).
    ...
    lose the venv. Falling through to ``sys.executable -m hbm_cli.main`` keeps the venv.
    """
    argv0 = sys.argv[0]
    ...
    def _is_unsafe_python_launcher(p: str) -> bool:
        ...
        from hbm_cli.linux_desktop_entry import _needs_interpreter
    ...
    if not argv0.startswith("-") and os.path.isfile(argv0):
        ...
        if os.access(abs_path, os.X_OK) and not _is_unsafe_python_launcher(abs_path):
            return abs_path
    path_bin = shutil.which("hbm-agent")
...
def build_relaunch_argv(
    ...
    """Construct an argv list for replacing the current process with hbm."""
    bin_path = resolve_hbm_bin()
    argv = [bin_path] if bin_path else [sys.executable, "-m", "hbm_cli.main"]
...
def relaunch(
    ...
    """Replace the current process with a fresh hbm invocation.
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``hbm-agent`` not on
            # PATH yet (install hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nHbm relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run hbm.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
