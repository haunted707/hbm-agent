...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_hbm_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from hbm_cli.auth_qwen import _mark_qwen_oauth_active
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:hbm_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> PooledCredential:
    ...
    # Codex-style auto-import: a shared Nous credential at <hbm-root>/shared/nous_auth.json
    # (written by any previous login) makes `hbm-agent --profile <name> auth add nous --type oauth`
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source HBM AGENT reads from (env vars, external OAuth files, auth.json blocks,
