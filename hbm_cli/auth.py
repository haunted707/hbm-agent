"""Multi-provider authentication system for HBM AGENT.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.hbm/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``hbm_cli.auth.<name>`` stays the public/patchable surface."""
...
from hbm_cli.config import (
    get_hbm_home, get_config_path, read_raw_config, require_readable_config_before_write)
from hbm_constants import OPENROUTER_BASE_URL, hbm_home_key, secure_parent_dir
...
from hbm_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from hbm_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from hbm_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from hbm_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from hbm_cli.auth_nous import (  # noqa: F401  re-exported
...
from hbm_cli.auth_minimax import (  # noqa: F401  re-exported
...
from hbm_cli.auth_xai import (  # noqa: F401  re-exported
...
from hbm_cli.auth_codex import (  # noqa: F401  re-exported
...
from hbm_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from hbm_cli.auth_openrouter import _openrouter_pkce_login  # noqa: F401  re-exported
from hbm_cli.auth_qwen import (  # noqa: F401  re-exported
...
from hbm_cli.auth_constants import (  # noqa: F401  re-exported
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.hbm/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from hbm_cli.config import get_env_value_prefer_dotenv
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$HBM_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from hbm_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.hbm/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $HBM_HOME/.env (e.g. HBM_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
# ── Auth Store — persistence layer for ~/.hbm/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_hbm_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch HBM_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".hbm" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set HBM_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_hbm_root()
    except Exception:
        return None
    return None if _same_path(get_hbm_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".hbm" / "auth.json"
        try:
...
def _save_private_json(target: Path, data: Any, *, fsync_dir: bool = False, **dump_kwargs: Any) -> None:
    ...
    mkdir_under_hbm_home(target.parent)
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT HBM AGENT flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in HBM AGENT.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "hbm_pkce", "manual"})
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit HBM AGENT flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when HBM AGENT-scoped routing config is present.
    ...
    HBM AGENT-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    HBM AGENT-initiated flow, or HBM AGENT-scoped routing config for keyless cloud-SDK providers. Ambient
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from hbm_cli.config import get_active_config_parse_failure
...
def _config_model_provider() -> Tuple[Any, Optional[str]]:
    ...
    try:
        ...
        if base_url:
            from hbm_cli.runtime_provider import _config_base_url_trustworthy_for_bare_custom
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.hbm/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.hbm/.env isn't silently switched without knowing why.
...
# Keyed by hbm_home_key(): the resolution itself is profile-scoped (_auth_file_path reads the
# per-turn HBM_HOME override a multiplex gateway sets), so a single slot would hand profile A's
...
def _nous_portal_base_url(state: Dict[str, Any]) -> str:
    """HBM_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted operator override and wins
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        if not state:
            raise _nous_err("HBM AGENT is not logged into Nous Portal.", relogin=True)
        ...
        with _nous_shared_store_lock(timeout_seconds=lock_timeout):
            from hbm_cli.anon_auth import is_guest_state, refresh_guest_state
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("hbm_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
...
def _provider_env_base_url(pconfig: ProviderConfig) -> str:
    if pconfig.id == "actual":
        from hbm_cli.providers import normalize_provider
...
def _provider_is_keyless(provider_id: str) -> bool:
    """HbmOverlay keyless flag — the same source the provider catalog and GUI contract tests use."""
    try:
        from hbm_cli.providers import HBM_OVERLAYS
        return bool(getattr(HBM_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    HBM AGENT can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from hbm_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: HBM_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / HBM_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``hbm_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from hbm_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _update_config_for_provider(
    ...
    *default_model*, when given, is written as ``model.default`` in the same step so the gateway
    ...
    from hbm_cli.config import clear_model_endpoint_credentials
...
def logout_command(args) -> None:
    ...
    if not target:
        ...
        return
    if target == "nous":
        from hbm_cli.anon_auth import FREE_TIER_NOT_SIGNED_IN, is_guest_state
    ...
    if target == "nous":
        ...
        from hbm_cli.auth_nous import _clear_shared_nous_state
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('hbm_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('hbm_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('hbm_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('hbm_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('hbm_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NOUS_INFERENCE_INVOKE_SCOPE': ('hbm_cli.auth_constants', 'NOUS_INFERENCE_INVOKE_SCOPE'),
    'NOUS_SHARED_STORE_FILENAME': ('hbm_cli.auth_nous', 'NOUS_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('hbm_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('hbm_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('hbm_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('hbm_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('hbm_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('hbm_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('hbm_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('hbm_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('hbm_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nous_oauth_pure': ('hbm_cli.auth_nous', 'refresh_nous_oauth_pure'),
}
