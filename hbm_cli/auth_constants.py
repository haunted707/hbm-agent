...
Pure leaf: imports nothing from ``hbm_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because hbm_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("hbm_cli.auth.httpx.Client")
...
DEFAULT_NOUS_CLIENT_ID = "hbm-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from hbm_cli import __version__ as _HBM_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _HBM_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"hbm-cli/{_HBM_CLI_VERSION}"
