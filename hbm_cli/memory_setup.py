...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    if provider_name == "hindsight":
        ...
        cfg = read_json_or_empty(get_hbm_home() / "hindsight" / "config.json")
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    # install_specs routes to the durable target on sealed hosted images (HBM_LAZY_INSTALL_TARGET)
...
def _post_setup_hook(provider, config: dict) -> bool:
    ...
    if hasattr(provider, "post_setup"):
        provider.post_setup(str(get_hbm_home()), config)
        return True
    return False
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.hbm/plugins/ and try again.\n")
        return
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, str(get_hbm_home()))
        except Exception as e:
...
def _write_env_vars(
    env_writes: dict, hbm_home: str | os.PathLike[str] | None = None) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    ``save_env_value`` applies the shared gate (name regex, ``LD_PRELOAD``/``PYTHONPATH``/``HBM_HOME``
    denylist, CR/LF stripping, atomic 0o600 writes). ``ValueError`` is reported and skipped so one
    bad key doesn't sink the batch; filesystem errors propagate. ``hbm_home`` is applied via the
    ...
    token = set_hbm_home_override(hbm_home) if hbm_home is not None else None
    try:
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        if url and not is_set:
                            line += f"  → {url}"
                        print(line)
                print("  Note: systemd/gateway services do not inherit ~/.hbm/.env —")
                print("        set any variables above in the service environment.")
        else:
            print("\n  Plugin:    NOT installed ✗")
            print(f"  Install the '{provider_name}' memory plugin to ~/.hbm/plugins/")
