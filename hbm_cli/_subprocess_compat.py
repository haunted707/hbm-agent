...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_HBM_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    HBM AGENT's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    HBM AGENT shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_hbm(text: str) -> bool:
    r"""True when *text* names HBM AGENT at a path-segment / token boundary.

    A bare ``"hbm" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\shbma\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("hbm", ".hbm"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_hbm(pid: int) -> bool:
    """Best-effort check that *pid* currently runs HBM AGENT code."""
    try:
        import psutil
        ...
        return _text_names_hbm(f"{command} {executable}")
    except Exception:
        return False


def pid_is_hbm(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a HBM AGENT process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_hbm(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_hbm`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``hbm-agent update`` on slow-WMI
