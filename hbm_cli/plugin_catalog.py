"""Plugin catalog — curated, Nous-approved HBM AGENT plugins shipped with the repo.
...
an installed HBM AGENT sees new entries and removals without updating. Any fetch failure falls back to the
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_hbm": self.requires_hbm,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_hbm=str(data.get("requires_hbm") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_hbm_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``HBM_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_hbm_home(cache.parent)
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_hbm:
        bits.append(f"Requires HBM AGENT {entry.requires_hbm}.")
    return " ".join(bits)
