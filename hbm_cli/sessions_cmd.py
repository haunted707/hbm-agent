...
``_DB_HANDLERS`` (everything else, sharing one ``SessionDB``). ``get_hbm_home`` is resolved through
``hbm_cli.main`` at call time so monkeypatches keep working. Picker: :mod:`hbm_cli.sessions_cmd_browse`.
"""
...
from hbm_cli.cli_output import print_truncated
from hbm_cli.sessions_cmd_browse import _relative_time, _session_browse_picker


def get_hbm_home():
    from hbm_cli import main
    return main.get_hbm_home()


def _sessions_dir() -> Path:
    return get_hbm_home() / "sessions"
...
def _not_found(session_id) -> int:
    print(f"No session '{session_id}'. Run: hbm-agent sessions list to find the id.")
    return 1


def _print_dry_run_preview(candidates, filters) -> None:
    from hbm_cli.session_filters import describe_filters
...
def _export_dir(output) -> Path:
    """``--output`` dir for multi-file exports; ``~/.hbm/session-exports`` when empty or ``-``."""
    return Path(output).expanduser() if output and output != "-" else get_hbm_home() / "session-exports"
...
def _cmd_repair(args):
    from hbm_state import DEFAULT_DB_PATH as db_path, SessionDB
    ...
    print(
        ...
        f"    hbm-agent sessions recover --source {source_hint} \\\n"
        "        --inspect-only\n"
...
def _cmd_recover(args):
    ...
    import sqlite3
    from hbm_cli.session_recovery import (
...
def _cmd_import(args):
    from hbm_cli.foreign_sessions import run_sessions_import
...
def _cmd_list(db, args):
    from hbm_state_sessions import workspace_key as _ws_key
...
def _cmd_export(db, args):
    from hbm_cli.session_filters import build_prune_filters
    filters = None
    ...
    def _redact(data):
        if not args.redact or data is None:
            return data
        from hbm_cli.session_export_md import redact_session_data
...
def _render_only(args, sessions):
    ...
    from hbm_cli.session_export import export_record_count, render_sessions_export
...
def _render_html(args, sessions):
    ...
    from hbm_cli.session_export_html import generate_html_export, generate_multi_session_html_export
...
def _export_flat(kind, args, collect):
    ...
    if sessions is not None:
        from hbm_cli.session_export import default_save_filename
        name = (default_save_filename(sessions[0].get("id", ""), args.format) if len(sessions) == 1
                else f"hbm_sessions.{args.format}")
...
def _export_markdown(db, args, filters, redact):
    ...
    from hbm_cli.session_export_md import append_manifest_entry, write_session_markdown
    if args.output == "-":
...
def _export_markdown_single(db, args, export_one, output_dir, lineage_is_logical):
    ...
    from hbm_cli.session_export_md import verify_export_file
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from hbm_cli.session_filters import format_epoch, parse_duration_seconds
...
def _note_pinned_skipped(db, filters, action):
    ...
    if action == "prune":
        verb = "deleted"
        optin = "Pass --include-pinned to delete them anyway, or unpin first with `hbm-agent sessions unpin <id>`."
    else:
        verb, optin = "archived", "Unpin first with `hbm-agent sessions unpin <id>` to include them."
...
def _cmd_prune_or_archive(db, args, action):
    ...
    from hbm_cli.session_filters import build_prune_filters, describe_filters, format_epoch
    ...
    if skipped_open:
        print(f"Note: {skipped_open} open session{'' if skipped_open == 1 else 's'} also match these filters but "
              "will be skipped because prune only deletes ended sessions. Use `hbm-agent sessions delete <id>` "
...
def _cmd_pinned(db, args):
    ...
    if getattr(args, "json", False):
        ...
        return
    if not pinned_rows:
        print("No pinned sessions. Pin one with: hbm-agent sessions pin <session_id>")
        return
...
def _cmd_optimize_storage(db, args):
    ...
    if result.get("vacuumed") is False:
        print("  (VACUUM was skipped or failed — run `hbm-agent sessions optimize` later to reclaim freed space.)")
...
def _print_empty_store(action: str, args) -> None:
    ...
    if action == "stats":
        ...
        print("[]" if getattr(args, "json", False) else "No pinned sessions. Pin one with: hbm-agent sessions pin <session_id>")
    else:
        print("No sessions found.")
...
def cmd_sessions(args, sessions_parser=None):
    ...
    from hbm_state import SessionDB, _default_db_path
    try:
        ...
        print("Could not open your session history database. "
              "Run: hbm-agent sessions repair to fix it (a backup is made first).")
        print(f"Details: {e}")
        return 1
    try:
        ...
        try:
            ...
            from hbm_state_repair import _schema_not_built
            ...
            print(f"Error: session database needs migration — run any writing hbm-agent command first ({e})")
            return 1
    finally:
        db.close()
