...
def _not_found(session_id) -> int:
    print(f"No session '{session_id}'. Run: hbm sessions list to find the id.")
    return 1
...
def _cmd_repair(args):
    ...
    print(
        ...
        f"    hbm sessions recover --source {source_hint} \\\n"
        "        --inspect-only\n"
...
def _note_pinned_skipped(db, filters, action):
    ...
    if action == "prune":
        verb = "deleted"
        optin = "Pass --include-pinned to delete them anyway, or unpin first with `hbm sessions unpin <id>`."
    else:
        verb, optin = "archived", "Unpin first with `hbm sessions unpin <id>` to include them."
...
def _cmd_prune_or_archive(db, args, action):
    ...
    if skipped_open:
        print(f"Note: {skipped_open} open session{'' if skipped_open == 1 else 's'} also match these filters but "
              "will be skipped because prune only deletes ended sessions. Use `hbm sessions delete <id>` "
...
def _cmd_pinned(db, args):
    ...
    if getattr(args, "json", False):
        ...
        return
    if not pinned_rows:
        print("No pinned sessions. Pin one with: hbm sessions pin <session_id>")
        return
...
def _cmd_optimize_storage(db, args):
    ...
    if result.get("vacuumed") is False:
        print("  (VACUUM was skipped or failed — run `hbm sessions optimize` later to reclaim freed space.)")
...
def _print_empty_store(action: str, args) -> None:
    ...
    if action == "stats":
        ...
        print("[]" if getattr(args, "json", False) else "No pinned sessions. Pin one with: hbm sessions pin <session_id>")
    else:
        print("No sessions found.")
...
def cmd_sessions(args, sessions_parser=None):
    ...
    try:
        ...
        print("Could not open your session history database. "
              "Run: hbm sessions repair to fix it (a backup is made first).")
        print(f"Details: {e}")
        return 1
    try:
