...
write ``HBM_DASHBOARD_OAUTH_CLIENT_ID`` (+ portal/public URL when warranted) into ``.env``
...
def _print_post_register_hint(
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    from hbm_cli.config import get_env_path
    print(f"\n  Wrote to {get_env_path()}:\n    HBM_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
    if wrote_portal_url:
        print("    HBM_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
...
def _public_url_from_redirect(redirect_uri: Optional[str]) -> str:
    ...
    ``/auth/callback`` to HBM_DASHBOARD_PUBLIC_URL, so the raw URI would double the path."""
    try:
...
def cmd_dashboard_register(args) -> None:
    ...
    from hbm_cli.auth import AuthError, resolve_nous_access_token
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("HBM_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("HBM_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:  # client_id is load-bearing: fatal on failure
        save_env_value("HBM_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write HBM_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}\n"
              f"  Set it manually:  HBM_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    # Explicit portal → always persist (the user asked); inferred → only if unset AND non-default.
    existing_portal = _env_value("HBM_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("HBM_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("HBM_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("HBM_DASHBOARD_PUBLIC_URL", public_url))
