...
_TASK_NAME_DEFAULT = "Hbm_Gateway"
_TASK_DESCRIPTION = "HBM AGENT Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("HBM_GATEWAY_DETACHED", "1"), ("HBM_SUPERVISED_CHILD", "1"))
...
def _hbm_home() -> Path:
    ...
    return Path(get_hbm_home())


def _preserve_hbm_home_path(path: str | Path) -> str:
    r"""Render HBM AGENT-owned paths under the configured HBM_HOME spelling.

    ``%LOCALAPPDATA%\hbm`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under HBM_HOME.
    """
    candidate = Path(path)
    try:
        home = _hbm_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current HBM AGENT profile."""
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "hbm_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"HBM_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "HBM_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "HBM_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from hbm_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<HBM_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _hbm_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at HBM_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m hbm_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "hbm_cli.main"]
    if profile_arg:
...
def _launcher_settings(home: Path | None = None) -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, hbm_home, profile_arg) for generated launchers.
    ``home`` targets another profile's HBM_HOME (per-profile cold-start, #110959)."""
    from hbm_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    hbm_home = str(home if home is not None else _hbm_home())
    return (
        _preserve_hbm_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        hbm_home,
        _profile_arg(hbm_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_hbm_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_hbm_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, hbm_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "HBM_HOME={hbm_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_hbm_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, hbm_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('HBM_HOME')}) = {q(hbm_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_hbm_home_path(venv_dir))}",
...
def _build_gateway_argv(home: Path | None = None) -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from hbm_cli.gateway import PROJECT_ROOT

    python_path, working_dir, hbm_home, profile_arg = _launcher_settings(home)
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"HBM_HOME": hbm_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_hbm_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_hbm_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (HBM_HOME,
    ...
    try:
        hbm_home = str(_hbm_home().resolve())
    except Exception:
        hbm_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "HBM_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if hbm_home:
        env_overlay["HBM_HOME"] = hbm_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("HBM_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("HBM_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from hbm_cli.setup import prompt_yes_no
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated HBM AGENT gateway install prompt.")
            if start_now:
...
def _start_attestation_path(home: Path | None = None) -> Path:
    ...
    return (home if home is not None else _hbm_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _write_start_attestation(pids: list[int], via: str, home: Path | None = None) -> None:
    ...
    try:
        ...
        from hbm_cli.process_identity import _process_create_time
...
def _attested_pid_exited_cleanly(pid: int, create_time: float | None = None, home: Path | None = None) -> bool:
    ...
    try:
        ...
        sentinel = get_lifecycle_sentinel_path(home if home is not None else _hbm_home())
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_hbm_home()}\\logs\\gateway.log\n    type {_hbm_home()}\\logs\\gateway-stdio.log")
