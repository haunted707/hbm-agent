#!/usr/bin/env python3
"""Skills Hub CLI — Unified interface for the HBM AGENT Skills Hub."""
...
# serves everything from "hbm-index", so that entry MUST exceed the whole catalog or browse
...
_BROWSE_LIMITS = {
    "hbm-index": 1000000, "official": 200, "skills-sh": 200, "well-known": 50,
    "github": 200, "clawhub": 500, "lobehub": 500, "browse-sh": 500}
# Programmatic (TUI gateway) browse keeps its own, lower caps.
_BROWSE_API_LIMITS = {
    "hbm-index": 5000, "official": 100, "skills-sh": 100, "well-known": 25,
...
def _existing_categories() -> List[str]:
    """Sorted category buckets under ``~/.hbm/skills/`` (children without their own SKILL.md)."""
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print("[bold]Category[/] "
                f"[dim](optional — press Enter to install flat at {display_hbm_home()}/skills/<name>/)[/]")
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    # hbm-index AND the external APIs at once would double-count every skill.
...
def _scan_block_message(result, identifier: str) -> str:
    ...
    policy = ("HBM AGENT never installs unverified skills with high-risk findings, even with --force."
...
def _announce_blueprint(c: Console, skill_name: str) -> None:
    """Offer an installed skill's ``metadata.hbm.blueprint`` via /suggestions — never
...
_INSTALL_PANELS = {
    "official": (
        ...
        "It ships with hbm-agent but is not activated by default.\n"
...
def _confirm_install(c: Console, bundle, category: str) -> bool:
    ...
    files_at = f"Files will be at: [cyan]{display_hbm_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]"
...
def do_list(source_filter: str = "all", enabled_only: bool = False,
    ...
    profile's config — ``-p`` swaps HBM_HOME at process start, so no profile flag here."""
    from tools.skills_hub import HubLockFile, ensure_hub_dirs
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            _print_error(c, "GitHub authentication required.\n"
                            f"Set GITHUB_TOKEN in {display_hbm_home()}/.env "
...
def _github_publish(skill_path: Path, skill_name: str, target_repo: str, auth) -> tuple:
    ...
    try:
        resp = call("post", f"{target_repo}/pulls", json={
            ...
            "body": f"Submitting the `{skill_name}` skill via HBM AGENT Skills Hub.\n\n"
                    f"This skill was scanned by the HBM AGENT Skills Guard before submission.",
