"""Profile management for multiple isolated HBM AGENT instances."""
...
from hbm_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
...
# stay visible; containers persist HOME state here). See hbm_constants.get_subprocess_home().
...
# (``~/.hbm``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
# ``hbm_cli.backup._EXCLUDED_ROOT_DIRS`` so the two lists cannot drift.
_CLONE_ALL_DEFAULT_EXCLUDE_ROOT: frozenset[str] = frozenset({
    "hbm-agent", ".worktrees", "profiles", "bin", "node_modules",
...
_PLACEHOLDER_ENV = (
    "# Per-profile secrets for this HBM AGENT profile.\n"
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_hbm_home().resolve():
...
# Allow-list for ``export_profile("default")``: when HBM_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known HBM AGENT profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``hbm_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"hbm-agent", "hbm-agent-acp", "hbm-agent-run", "default", "test", "tmp", "root", "sudo"})

# HBM AGENT subcommands that cannot be used as profile names/aliases
_HBM_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the hbm-agent root (NOT the current HBM_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_hbm_home() / "profiles"


def _get_default_hbm_home() -> Path:
    """Default (pre-profile) HBM_HOME: ``~/.hbm``, or HBM_HOME itself in
    Docker/custom deployments (e.g. ``/opt/data``)."""
    from hbm_constants import get_default_hbm_root
    return get_default_hbm_root()


def _get_active_profile_path() -> Path:
    return _get_default_hbm_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    """
    if name == "default":
        return  # special alias for ~/.hbm
    if not _PROFILE_ID_RE.match(name):
        raise _invalid_profile_name_error(name)
    if name in _RESERVED_NAMES:
        raise ValueError(
            f"Profile name {name!r} is reserved — it collides with either "
            f"the HBM AGENT installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HBM_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_hbm_home()
    ...
    # profiles/hbm may still exist and must keep resolving.
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _HBM_SUBCOMMANDS:
        return f"'{canon}' conflicts with a hbm subcommand"
    try:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    try:
        if sys.platform == "win32":
            ...
            hbm_exe = shutil.which("hbm-agent") or "hbm-agent"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(hbm_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    with contextlib.suppress(Exception):
        ...
        from hbm_cli.config import check_config_version, migrate_config
        token = set_hbm_home_override(str(profile_dir))
        try:
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    with contextlib.suppress(Exception):  # creation must not fail over this; `hbm-agent model` sets it later
        ...
        from hbm_constants import get_hbm_home
        from hbm_cli.config import read_user_config_raw
        source = get_hbm_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating HBM_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from hbm_cli.gateway import named_profile_served_by_running_multiplexer
...
# Deliberately tiny and separate from ``config.yaml`` (user-facing HBM AGENT config, ~5000
...
def list_profiles() -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_hbm_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """``(profile_name, hbm_home)`` pairs a gateway should serve — the single chokepoint
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_hbm_home())]
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from hbm_cli.auth import strip_cloned_single_use_oauth_grants
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path],
    ...
    for relpath in _CLONE_SUBDIR_FILES:
        _clone_file(source_dir, profile_dir, relpath)
    if sync_imports:
        from hbm_cli.agent_import_sync import SYNC_MANIFEST_NAME  # lazy: keeps yaml/utils off the hot startup path
...
def create_profile(
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.hbm).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and clone_channels:
        from hbm_cli.profile_channels import clone_channels_refusal
    ...
    try:
        ...
        if source_dir is not None and not clone_channels:
            from hbm_cli.profile_channels import strip_channel_settings
...
def _notify_multiplexer(canon: str) -> None:
    from hbm_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
...
def _live_default_multiplexer() -> bool:
    ...
    try:
        from hbm_cli.gateway_multiplex_served import recorded_served_profiles
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches HBM_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HBM_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_hbm_home() / ".env"
...
_HBM_ARGV_MARKERS = ("hbm_cli.main", "hbm-gateway", "tui_gateway")
...
# `findOnPath('hbm-agent')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "hbm".
...
# argv[1] is matched against exact names, not ``startswith("hbm")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("hbm-notes.py").
_HBM_CONSOLE_SCRIPT_NAMES = frozenset({"hbm-agent", "hbm-agent-acp", "hbm-agent-run"})


def _is_hbm_argv(argv: list) -> bool:
    """True for a HBM AGENT process: entrypoint marker in argv, executable named ``hbm*``,
    or a python interpreter directly exec'ing a known ``hbm-agent`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _HBM_ARGV_MARKERS) or exe_name.startswith("hbm"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _HBM_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running HBM AGENT *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_hbm_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by HBM_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("HBM_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    shutdown_mcp_servers(scope=hbm_home_key(profile_dir))
    ...
    with contextlib.suppress(Exception):
        from hbm_state_registry import close_all_under as _close_session_dbs_under
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``HBM_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # HBM_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("HBM_HOME")
    try:
        os.environ["HBM_HOME"] = str(profile_dir)
        from hbm_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("HBM_HOME", None)
        if old_home is not None:
            os.environ["HBM_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's hbm_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded HBM_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from HBM_HOME: ``"default"`` when unset or ``~/.hbm``, the
    name under ``~/.hbm/profiles/<name>``, ``"custom"`` for any other path."""
    from hbm_constants import get_hbm_home
    resolved = get_hbm_home().resolve()
    if resolved == _get_default_hbm_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when HBM_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_hbm_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point HBM_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/hbm-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".hbm-profile-exports", Path(tempfile.gettempdir()) / f"hbm-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    HBM_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-HBM AGENT file the user may have lying alongside HBM_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``hbm-agent
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.hbm (dir name ".hbm"), so both paths stage a filtered
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="hbm_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"hbm_{old_name}"
    legacy_old_host = f"hbm.{old_name}"
    new_host = f"hbm_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_hbm_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``hbm_<old>`` or legacy ``hbm.<old>``
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    # ``mkdir_under_hbm_home`` does not refuse it) and the periodic reconcile re-adopts the
    ...
    from hbm_constants import hbm_home_key
    from tools.mcp_tool_lifecycle import shutdown_mcp_servers
    shutdown_mcp_servers(scope=hbm_home_key(old_dir))
    ...
    from hbm_cli.profile_identity import _migrate_profile_identity
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HBM_HOME path string. Called early in the CLI entry
    point, before hbm modules are imported, to set HBM_HOME.

    When HBM_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match HBM AGENT-owned PYTHONPATH
