...
from hbm_cli.config import get_env_path, get_hbm_home, get_project_root
from hbm_cli.env_loader import load_hbm_dotenv
from hbm_constants import display_hbm_home

PROJECT_ROOT = get_project_root()
HBM_HOME = get_hbm_home()
_DHH = display_hbm_home()  # user-facing display path (e.g. ~/.hbm or ~/.hbm/profiles/coder)

# Load environment variables from ~/.hbm/.env so API key checks work
_env_path = get_env_path()
load_hbm_dotenv(hbm_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from hbm_cli.colors import Colors, color
from hbm_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from hbm_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from hbm_cli.doctor_tools import _safe_which

from hbm_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from hbm_cli.doctor_platform import (
    _check_certificates,
...
from hbm_cli.doctor_tools import (
    _check_git_and_rg,
...
from hbm_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from hbm_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("Nous Portal auth", get_nous_auth_status_local())
        # Native OAuth is HBM AGENT' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
...
def run_doctor(args):
    ...
    os.environ.setdefault("HBM_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from hbm_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('hbm_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('hbm_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('hbm_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('hbm_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('hbm_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('hbm_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('hbm_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('hbm_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('hbm_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('hbm_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('hbm_cli.vercel_auth', 'describe_vercel_auth'),
    'detect_install_method': ('hbm_cli.config', 'detect_install_method'),
    'is_nix_install_method': ('hbm_cli.config', 'is_nix_install_method'),
    'managed_scope_check': ('hbm_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('hbm_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('hbm_cli.doctor_config', 'report_deprecated_config_and_env'),
}
