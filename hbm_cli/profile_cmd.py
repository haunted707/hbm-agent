...
def _render_distribution_plan(plan) -> None:
    ...
    from hbm_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hbm_requires:
        print(f"  Requires: HBM AGENT {mf.hbm_requires}")
...
def _shared_credential_warnings(profiles) -> list:
    ...
    from hbm_cli.profile_channels import shared_channel_credentials, shared_credential_warning
...
def _profile_use(args):
    from hbm_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.hbm)" if name == "default" else f"Switched to: {name}")
...
def _print_channel_clone_notice(name: str, source_label: str, clone_channels: bool, clone_flag: str) -> None:
    from hbm_cli.profile_channels import (
...
def _profile_delete(args):
    from hbm_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_hbm_home),
    everything else to its named directory."""
    from hbm_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from hbm_constants import get_hbm_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from hbm_cli import profile_describer as _pd
    if all_flag:
...
def _profile_rename(args):
    from hbm_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_migrate_identity(args):
    ...
    from hbm_cli.profile_identity import migrate_profile_identity
    try:
...
def _profile_export(args):
    from hbm_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from hbm_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="hbm_dist_preview_") as tmp:
...
def _profile_update(args):
    from hbm_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from hbm_cli.profiles import get_profile_dir, normalize_profile_name
    try:
...
_INFO_FIELDS = (
    ...
    ("author", "Author:       "),
    ("license", "License:      "),
    ("hbm_requires", "Requires:     HBM AGENT "),
    ("source", "Source:       "),
...
def _profile_info(args):
    from hbm_cli.profile_distribution import describe_distribution, DistributionError
    try:
