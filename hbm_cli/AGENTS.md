# hbm_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `HbmCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`hbm_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`HbmCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.hbm/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`hbm_cli/curses_ui.py`; example
`hbm_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`hbm_cli/commands.py`)
...
`hbm_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`hbm_cli/config.py`)
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' hbm_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`hbm_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.hbm/skins/*.yaml` → built-ins → default; missing values inherit
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `hbm-gateway*` unit /
  `ai.hbm.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.hbm/logs/update_receipts/`
...
other service domain / UNIX user / HBM_HOME outside `profiles/` — notices for the explicit command,
...
profile's `HBM_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
...
## Nous free tier (`hbm_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `hbm-agent` verb) and `.copy_terminal`, so no caller
