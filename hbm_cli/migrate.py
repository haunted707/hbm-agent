"""CLI handlers for ``hbm migrate ...``.

Currently exposes only ``hbm migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``hbm migrate <subtype>``."""
    if getattr(args, "migrate_type", None) == "xai":
        return cmd_migrate_xai(args)

    print("usage: hbm migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `hbm migrate xai --apply` to rewrite "
    ...
    print()
    print(color("Run `hbm doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
