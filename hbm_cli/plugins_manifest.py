...
Split out of :mod:`hbm_cli.plugins`; validation warns and never fails a load.
"""
...
from hbm_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``hbm``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "hbm", "depends",
    "requires_hbm",
}

# Highest manifest schema version this HBM AGENT understands.
...
def _plugins_debug() -> bool:
    from hbm_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this HBM AGENT "
...
class PluginManifest:
    ...
    # recorded with an error and skipped before import — see ``requires_hbm_error``.
    requires_hbm: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — HBM AGENT never auto-installs these (isolation design for the install seam
...
# ── requires_hbm version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_hbm_version() -> str:
    """Installed ``hbm-agent`` distribution version, else ``hbm_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("hbm-agent")
    except Exception:
...
def requires_hbm_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_hbm`` rejects the running version."""
    if not manifest.requires_hbm:
        return None
    current = running_hbm_version()
    if version_satisfies(manifest.requires_hbm, current):
        return None
    return f"requires hbm {manifest.requires_hbm}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_hbm=str(data.get("requires_hbm") or "").strip(),
