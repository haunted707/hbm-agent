...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "hbm_cli.gateway",
        "hbm_cli.auth",  # provider auth state → copilot_auth → subprocess
        "hbm_cli.copilot_auth",
        "hbm_cli.runtime_provider",
        "hbm_cli.skin_engine",  # resolve_skin() config + skin engine init
        "hbm_cli.inventory",  # provider catalogs + models.dev cache
...
def _eager_reconcile_own_session_db() -> None:
    ...
    :func:`hbm_cli.web_server_sessions._open_session_db_at_path`. Never
    raises: an unfixable store still gets the per-poll read-probe heal.
    """
    try:
        from hbm_cli.web_server_sessions import _open_session_db_for_profile
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    """Publish the port through an atomic ready file when ``HBM_DESKTOP_READY_FILE`` is set.
    ...
    """
    target = os.environ.get("HBM_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``HBM_PARENT_PID`` is the Electron PID, not necessarily our PPID (the
    Windows ``hbm.exe`` launcher adds shims), so never compare getppid().
...
def _start_parent_death_watchdog() -> None:
    ...
    """
    raw_pid = os.environ.get("HBM_PARENT_PID")
    # Empty inherited values mean "absent", not "marker present but blank".
    start_marker = os.environ.get("HBM_PARENT_START_MARKER") or None
    nonce = os.environ.get("HBM_PARENT_NONCE") or None
    ...
    if has_marker and (not _valid_parent_start_marker(start_marker or "") or not nonce or nonce != nonce.strip()):
        ...
        _log.warning(
            "Parent-death watchdog disabled: unusable HBM_PARENT_START_MARKER=%r / nonce; "
    ...
    try:
        poll = max(0.5, float(os.environ.get("HBM_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'hbm-agent serve' / 'hbm-agent dashboard' backend or the HBM AGENT gateway. "
