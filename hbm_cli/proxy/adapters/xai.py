...
class XAIGrokAdapter(UpstreamAdapter):
    ...
    auth_hint = "hbm auth add xai-oauth --type oauth"
    ...
    def get_credential(self) -> UpstreamCredential:
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    "No xAI OAuth credentials found. Run `hbm auth add xai-oauth --type oauth` first."
                )
            entry = pool.select()
            if entry is None:
                raise RuntimeError(
                    "No available xAI OAuth credentials found. Run "
                    "`hbm auth reset xai-oauth` or re-authenticate with `hbm auth add xai-oauth --type oauth`."
                )
            self._pool = pool
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `hbm auth add xai-oauth --type oauth`."
            )
        base_url = str(
