...
from hbm_cli.active_sessions import active_session_registry_snapshot, session_already_owned_message
...
def discover_attach_url(session_id: str, *, registry_home: str | Path | None = None) -> str | None:
    ...
    """
    home = Path(registry_home if registry_home is not None else get_hbm_home()).resolve()
...
def configure_tui_attachment(env: dict[str, str], session_id: str | None, *,
    ...
    if not session_id or env.get("HBM_TUI_GATEWAY_URL", "").strip():
        return
    url = discover_attach_url(session_id, registry_home=registry_home)
    if url is not None:
        env["HBM_TUI_GATEWAY_URL"] = url
