...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.hbm/.env contains provider auth/base URL settings."""
    from hbm_cli.doctor import _PROVIDER_ENV_HINTS
...
# TERMINAL_CWD do not false-positive). HBM_TOOL_PROGRESS is silently ignored since the v12 config floor
# removed its only consumer; HBM_TOOL_PROGRESS_MODE is still read by the gateway as a back-compat fallback.
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ("HBM_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("HBM_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(raw_config: dict | None, env_map: dict | None) -> list[tuple[str, str]]:
    """Return actionable findings for the removed HBM AGENT Relay plugin."""
    from hbm_cli.relay_plugin_cutover import (LEGACY_RELAY_EXPORT_ENV_VARS, RELAY_PLUGINS_CONFIG_ENV,
...
def managed_scope_check() -> None:
    """Report the active managed scope (resolved dir + pinned key counts); silent when none. A HBM_MANAGED_DIR
    ...
    if os.environ.get("HBM_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via HBM_MANAGED_DIR={managed_dir}")
...
def _check_env_file(should_fix: bool, f: Finding) -> None:
    """Managed scope plus ~/.hbm/.env presence and provider credentials."""
    from hbm_cli.doctor import HBM_HOME, PROJECT_ROOT, _DHH
    managed_scope_check()
...
def _known_provider_ids(cfg: dict) -> tuple[set, list, object, object, object]:
    ...
    with warn_on_error(""):
        from hbm_cli.auth import PROVIDER_REGISTRY, resolve_provider as resolve_auth
        known = set(PROVIDER_REGISTRY.keys()) | {"openrouter", "custom", "auto", "moa"}
    with warn_on_error(""):
        from hbm_cli.config import get_compatible_custom_providers
        from hbm_cli.providers import custom_provider_aliases as aliases, normalize_provider as normalize, resolve_provider_full as resolve_full
        with warn_on_error(""):
...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    if runtime_provider and runtime_provider not in ("auto", "custom"):
        from hbm_cli.doctor import _DHH
        with warn_on_error(""):
...
def _check_config_file(should_fix: bool, f: Finding) -> None:
    ...
    if config_path.exists():
        ...
        if example_config.exists():
            ...
            from hbm_cli.config import DEFAULT_CONFIG, save_config
...
def _drift_max_iterations_ghost(f: Finding, should_fix: bool, config_path) -> None:
    """A stale HBM_MAX_ITERATIONS in .env shadows agent.max_turns in config.yaml.

    The setup wizard used to dual-write the budget. The gateway bridge derives HBM_MAX_ITERATIONS from
    ...
    """
    from hbm_cli.doctor import _DHH
    # Detect stale HBM_MAX_ITERATIONS ghost in .env shadowing agent.max_turns in config.yaml (issue
    # #17534). The setup wizard used to dual-write the iteration budget to both stores; users who later edit
    # only config.yaml are left with a .env ghost. The gateway bridge normally derives HBM_MAX_ITERATIONS
    ...
    from hbm_cli.config import load_env, read_user_config_raw, remove_env_value
    raw_config = read_user_config_raw(config_path)
    ...
    if not should_fix:
        ...
        check_warn("Could not remove HBM_MAX_ITERATIONS from .env")
        f.manual_issues.append(f"Manually delete the HBM_MAX_ITERATIONS line from {_DHH}/.env — config.yaml agent.max_turns is authoritative.")
...
def _drift_deprecations(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from hbm_cli.config import load_env, read_user_config_raw
...
def _check_config_drift(should_fix: bool, f: Finding) -> None:
    """Config version, stale root keys, HBM_MAX_ITERATIONS ghost, deprecations, structure.
...
def _check_xai_retirement(should_fix: bool, f: Finding) -> None:
    from hbm_cli.config import load_config
    from hbm_cli.xai_retirement import MIGRATION_GUIDE_URL, find_retired_xai_refs, format_issue
...
def _check_plugin_compat(should_fix: bool, f: Finding) -> None:
    from hbm_cli.plugin_compat import ALLOW_KEY, COMPAT_REMOVAL, compat_report, removal_in_effect
