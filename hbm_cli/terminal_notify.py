...
Inside the running CLI, ``HbmCLI._ring_bell`` sends ``notification_sequence()`` through the
...
def warp_osc777(event: str, detail: str, session_id: str = "") -> str:
    ...
    try:
        ...
        advertised = 1
    cwd = os.getcwd()
    payload = {"v": min(advertised, _WARP_PROTOCOL_VERSION), "agent": "hbm", "event": event,
