...
cycle) and tests patch ``hbm_cli.config.load_config`` etc. at call time.
"""
...
def _ask(prompt: str, *, secret: bool = False, raw: bool = False, cancel_msg: str | None = "\nCancelled.",
    ...
    if secret:
        from hbm_cli.secret_prompt import masked_secret_prompt as fn
    else:
...
def _commit_model_config(cfg: dict) -> None:
    ...
    from hbm_cli.auth import deactivate_provider
