...
_USER_AGENT = "hbm-agent/debug-share"
...
def _pending_file() -> Path:
    return get_hbm_home() / "pastes" / "pending.json"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from hbm_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_hbm_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by HBM AGENT Desktop on the machine running the app, not by this "
...
_NOUS_BUNDLE_FORMAT = "hbm-debug-share/1"
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/HBM AGENT version, provider, which API keys are
