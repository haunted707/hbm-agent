...
def run_oneshot(
    ...
    Model/provider fall back to ``HBM_INFERENCE_MODEL`` and config.yaml. ``usage_file`` gets a
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB — oneshot bypasses ``HbmCLI._init_agent()``, so it must wire the
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    env_model = os.getenv("HBM_INFERENCE_MODEL", "").strip()
    ...
    try:
        from hbm_cli import model_switch as _ms
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("HBM_INFERENCE_PROVIDER", "").strip().lower() or "auto"
...
def _apply_stored_session_runtime(
    ...
    if explicit_model:
        return choice
    from hbm_cli.cli_model_switch_mixin import stored_session_route
...
def _run_agent(
    ...
    try:
        agent = AIAgent(
            ...
            # HBM_INTERACTIVE (never set), hook approval via HBM_ACCEPT_HOOKS=1, dangerous
            # commands via HBM_YOLO_MODE=1, skill secret capture degrades gracefully.
