...
01-hbm-setup (the stage2 hook) has chowned the volume and seeded $HBM_HOME, but
...
def reconcile_profile_gateways(
    *, hbm_home: Path, scandir: Path, dry_run: bool = False,
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        hbm_home, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(hbm_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(hbm_home)
        _register_service(scandir, "default", start=default_should_start)
    actions.append(_slot_action("default", hbm_home, default_prior_state, default_should_start))
    ...
    if not dry_run:
        _write_reconcile_log(hbm_home, actions)
    return actions


def _maybe_migrate_legacy_gateway_run_state(
    hbm_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
    ...
    state_file = hbm_home / "gateway_state.json"
    if state_file.exists():
        return None
    if os.environ.get("HBM_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the hbm args.
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "hbm-agent":  # the wrapper re-execs `hbm-agent <subcommand>`
        args = args[1:]
    return args
...
def _write_reconcile_log(hbm_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $HBM_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted HBM_HOME both
    ...
    actions = reconcile_profile_gateways(hbm_home=hbm_home, scandir=scandir)
    for a in actions:
