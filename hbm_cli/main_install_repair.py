...
Split out of ``hbm_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
# Install-scoped breadcrumbs live next to the venv (not under $HBM_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``hbm-agent acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``hbm-agent update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    from hbm_cli.main import PROJECT_ROOT
    print(
        "⚠ A previous `hbm-agent update` was interrupted mid-install — "
        "finishing dependency installation now...")

    # Windows: a ``hbm.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    self_locked = _windows_running_hbm_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from hbm.exe; applying package-only first aid, "
    ...
    try:
        from hbm_cli import _install_repair as _ir
        ...
        manual = (
            "  HBM AGENT is still running from the launcher that needs "
            "replacing. Close other HBM AGENT windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\hbm.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``hbm.exe`` never matches.
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _hbm_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_hbm_launcher_locked() -> bool:
    """True when a venv ``hbm*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "HBM_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``hbm-agent update`` so the sync and its tail happen
    ...
    user's own console.
    ``venv\\Scripts\\hbm.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    cmd = [str(python_exe), "-m", "hbm_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from hbm_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `hbm-agent update` starts, so an update running
...
def _venv_scripts_dir() -> Path | None:
    ...
    from hbm_cli.main import PROJECT_ROOT
    from hbm_constants import project_venv_dir, venv_bin_dir
...
def _hbm_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    names = set(_load_console_script_names()) or {"hbm-agent", "hbm-agent-acp", "hbm-agent-run"}
    # Not a [project.scripts] entry point, but older update/install paths still
    # rewrite and quarantine it.
    names.add("hbm-gateway")
...
def _quarantine_running_hbm_exe(
    scripts_dir: Path, *, max_attempts: int = 4, failed_out: list[str] | None = None
) -> list[tuple[Path, Path]]:
    """Pre-empt the Windows file lock on the running ``hbm.exe``.
    ...
    transient; HBM AGENT Desktop backend child: until closed) — retry with backoff, then warn
    ...
    for shim in _hbm_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close HBM AGENT Desktop, exit other `hbm-agent` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `hbm-agent update`.")
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older HBM AGENT versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _hbm_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_hbm_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``hbm-agent``
...
class ShimQuarantineError(RuntimeError):
    """A live ``hbm*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    """Run an editable install, quarantining the running ``hbm.exe`` first.

    Every editable install rewrites the entry-point shims; on Windows the live ``hbm.exe``
    can be neither deleted nor overwritten, so without quarantine ``hbm-agent`` drops off PATH.
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_hbm_exe(scripts_dir, failed_out=failed)
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `hbm-agent` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``hbm.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``hbm.exe`` missing while ``hbm.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``hbm.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``hbm.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `hbm-agent update`:")
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    See #76105.
    """
    from hbm_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``hbm*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves hbm.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``hbm.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``hbm-agent`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``hbm-agent-acp.exe`` present but ``hbm-agent.exe`` missing, so
    ``hbm-agent`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `hbm-agent update --force` after "
            "closing other hbm processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m hbm_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``hbm-agent update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `hbm-agent update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `hbm-agent update --force` after closing other hbm processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `hbm-agent update --force` after closing other hbm processes.",
...
def _is_termux_env(env: dict[str, str] | None = None) -> bool:
    from hbm_cli.main import _is_termux_startup_environment
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from hbm_constants import find_node_executable
