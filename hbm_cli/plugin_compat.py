...
The decomposition (PR #102117) moved most of HBM AGENT's internals into ``<stem>_<topic>`` sibling modules.
...
class HbmPluginCompatWarning(FutureWarning):
...
def report_file_path() -> Path:
    ...
    return get_hbm_home() / REPORT_FILE
...
def warn_once(facade: str, name: str, target_module: str, target_name: str) -> None:
    """Per-name record that a moved name was resolved through its old path: a ``HbmPluginCompatWarning``
    ...
    msg = (f"hbm-agent plugin compat: `{facade}.{name}` moved to `{new}`. The old path is kept only for external "
           f"plugins and is removed on {COMPAT_REMOVAL}; update your import.")
    _log.warning(msg)
    warnings.warn(msg, HbmPluginCompatWarning, stacklevel=3)
...
def quiet_for_interactive() -> None:
    ...
    Appends (does not override) so an explicit ``-W error::...HbmPluginCompatWarning`` still wins."""
    if not any(a == "error" and c is not None and issubclass(HbmPluginCompatWarning, c)
               for a, _m, c, _mod, _l in warnings.filters):
        warnings.filterwarnings("ignore", category=HbmPluginCompatWarning, append=True)
