"""Provider-specific runtime builders for :mod:`hbm_cli.runtime_provider`: Azure Foundry, the
...
``hbm_cli.runtime_provider.*`` (``_get_model_config``, ``load_config``, ``has_usable_secret``,
...
def _resolve_bedrock_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: Optional[str]) -> Dict[str, Any]:
    ...
    from hbm_cli.config import load_config  # direct (not the origin delegate), as before
    rp = _rp()
