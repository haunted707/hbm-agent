...
Split out of ``hbm_cli.models``. OpenRouter and
...
``hbm_cli.models`` — tests reset them there — and are read/written by attribute name.
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_hbm_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._HBM_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``hbm_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from hbm_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from hbm_cli.models_profile_cache import profile_slot_set
