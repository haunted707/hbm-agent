...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from hbm_cli.main import _resolve_last_session
    target = (
    ...
    print(f"\nResume this session with:\n  hbm-agent --tui --resume {target}")
    if title:
        print(f'  hbm-agent --tui -c "{title}"')
    print(f"\nSession:        {target}")
    if title:
        print(f"Title:          {title}")
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @hbm/ink is missing or node_modules is behind package-lock.json.
    ...
    if not (ws_root / "node_modules" / "@hbm" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/hbm-ink/src")
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/hbm-ink/package.json",
    "packages/hbm-ink/index.js",
    "packages/hbm-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``HBM_TUI_FORCE_BUILD=1`` forces a rebuild."""
    force = (os.environ.get("HBM_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``HBM_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    from hbm_cli.main import PROJECT_ROOT
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("HBM_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HBM_HOME": hbm_home},
    ...
    extras += [Path(hbm_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _find_bundled_tui(hbm_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if hbm_cli_dir is None:
        hbm_cli_dir = Path(__file__).parent
    bundled = hbm_cli_dir / "tui_dist" / "entry.js"
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("HBM_QUIET"):
...
def _npm_lifecycle_env(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    # the rebuild fails). ``$HBM_HOME`` lives outside the git tree and the
    # update hand-off already carries ``HBM_HOME`` down to every npm child.
    # An explicit ``NPM_CONFIG_USERCONFIG`` wins (#106373).
    from hbm_constants import get_hbm_home
    npmrc = get_hbm_home() / "npmrc"
    if npmrc.is_file():
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``HBM_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$HBM_HOME/node`` tree a bare which() misses."""
    if bin == "node":
        env_node = os.environ.get("HBM_NODE")
    ...
    if not path and bin == "node":
        with contextlib.suppress(Exception):
            from hbm_cli.dep_ensure import ensure_dependency
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        ...
        return subprocess.run(
            ...
            env=_npm_lifecycle_env(with_hbm_node_path()))
    ...
    if result.returncode != 0:
        ...
        from hbm_cli.npm_engine import maybe_repair_npm_engine
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (HBM_TUI_DIR prebuilt or esbuild)."""
    from hbm_cli.main import _is_termux_startup_environment
    _ensure_tui_node()

    # Footgun: --dev against a prebuilt bundle that has no source/node_modules.
    ext_dir = os.environ.get("HBM_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with HBM_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset HBM_TUI_DIR (e.g. `unset HBM_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr)
        sys.exit(1)
    ...
    # hbm_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   hbm_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        # --dev runs src/entry.tsx directly, but @hbm/ink resolves through
        # packages/hbm-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "hbm-ink", "TUI dev prebuild failed.")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from hbm_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from hbm_cli.main import PROJECT_ROOT
    src_root = str(env.get("HBM_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["HBM_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("HBM_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["HBM_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("HBM_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["HBM_CWD"]) / python_path
...
def _launch_tui(
    ...
    import tempfile
    # TUI child is a hbm process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    from hbm_cli.shared_session_attach import configure_tui_attachment
    try:
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="hbm-tui-active-session-", suffix=".json")
    os.close(active_session_fd)
    env["HBM_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        wt_info = _setup_tui_worktree()
        env["HBM_CWD"] = wt_info["path"]
    ...
    for key, value in (
        ("HBM_MODEL", model), ("HBM_INFERENCE_MODEL", model),
        ("HBM_TUI_PROVIDER", provider), ("HBM_INFERENCE_PROVIDER", provider),
        ("HBM_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("HBM_TUI_SKILLS", skills_value),
        ("HBM_TUI_QUERY", query), ("HBM_TUI_IMAGE", image),
        ("HBM_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("HBM_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("HBM_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("HBM_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("HBM_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # HBM_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `hbm-agent --tui` try to resume a non-existent session; only forward the id
    # argparse resolved for this invocation.
    env.pop("HBM_TUI_RESUME", None)
    if resume_session_id:
        env["HBM_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    ...
    """
    if os.environ.get("HBM_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from hbm_cli.kanban_db import get_current_board
        os.environ["HBM_KANBAN_BOARD"] = get_current_board()


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.hbm/skills/`` with the bundled library (idempotent, milliseconds when synced).
