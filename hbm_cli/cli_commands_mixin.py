...
class CLICommandsMixin:
    ...
    def _handle_export_command(self, command: str):
        ...
        try:
            ...
            _pr(f"  ✓ Exported '{name}' to {result}",
                "  Share it: the other user runs /import or `hbm profile import <archive>`.")
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            tip = f'Tip: type your next message, or run hbm chat -q --image {example} "What do you see?"'
            _cp(_dim_line(tip))
    ...
    def _handoff_wait(self, platform_name: str, session_title: str) -> bool:
        ...
        try:  # pending timed out: CAS-clear so the user can retry
            ...
            pass
        return self._handoff_keep(
            "  Timed out waiting for the gateway. Is `hbm gateway` running?",
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            return _cp("  Tip:   Use /history or `hbm sessions list` to find sessions.")
    ...
    def _resolve_resume_target(self, target: str):
        ...
        if not session_meta:
            return _cp(f"  Session not found: {target}",
                       "  Use /sessions or `hbm sessions list` to see available sessions.")
        try:
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        ``new`` moves this session into the tree (as ``hbm -w``: kept on exit only with
    ...
    def _worktree_new(self, repo_root: str, rest: str) -> None:
        ...
        # Retarget the session's terminal/file tools at the new tree (as `hbm -w` does).
        try:
            os.chdir(wt_info["path"])
        except OSError as e:
            print(f"  ⚠ Created worktree but could not enter it: {e}")
        os.environ["TERMINAL_CWD"] = wt_info["path"]
        # Same keep-if-unpushed cleanup as `hbm -w`. Only one tree is "active" per process;
    ...
    def _handle_curator_command(self, cmd: str):
        """Handle /curator — delegates to hbm_cli.curator so the CLI and the `hbm curator`
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        """In-session ``/bundles`` — show installed skill bundles (``hbm bundles list`` rendered
        ...
        if not bundles:
            return _cp("  No skill bundles installed.",
                       _dim_line('Create one with: hbm bundles create <name> --skill <s1> --skill <s2>'),
        ...
        _cp("\n" + _dim_line("Invoke a bundle with /<slug>. Manage with `hbm bundles`."))
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        Session-scoped and in-process — use `hbm cron` for durable schedules."""
    ...
    def _heartbeat_set(self, mgr, arg: str) -> None:
        ...
        _cp(f"  ♥ Heartbeat set (every {format_interval(state.interval_seconds)}): {state.prompt}",
            _dim_line("Fires as a normal turn whenever the session is idle and the interval has "
                      ...
                      "HBM AGENT process runs — use `hbm cron` for durable schedules."))
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — exit the session and relaunch as ``hbm update``. Returns True when
        ...
        raw = self._prompt_text_input_modal(
            ...
            detail="This will exit the current session and run `hbm update`.", choices=choices)
