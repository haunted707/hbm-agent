...
resolved through the module object so test patches on ``hbm_cli.setup.<name>`` take effect."""
...
def _print_setup_summary(config: dict, hbm_home):
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {hbm_home}/cron/, sessions/, logs/")):
