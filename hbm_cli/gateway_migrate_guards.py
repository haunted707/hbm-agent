...
if TYPE_CHECKING:
    from hbm_cli.gateway_migrate import MigrationPlan, ProfileGateway
...
def _system_unit_uid(unit_path: Path) -> Optional[int]:
    ...
    from hbm_cli.gateway import _read_systemd_user_from_unit
...
def gateway_identity(home: Path, pid: Optional[int], services: list[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    ...
    HBM_HOME an installed unit pins, which is where the gateway really runs; ``home`` otherwise.
    """
    from hbm_cli.gateway import _hbm_home_pinned_by_unit, get_systemd_unit_path
    from hbm_cli.gateway_migrate import _home_env
    ...
    for kind, system in services:
        ...
        pinned = _hbm_home_pinned_by_unit(unit_path)
...
def _guard_home_tree(plan: MigrationPlan, profile: ProfileGateway) -> Optional[str]:
    ...
    if runtime_home.is_relative_to(profiles_root):
        return None
    return (f"Profile '{profile.name}' runs with HBM_HOME={runtime_home}, outside {profiles_root}: "
