...
import subprocess
import sys
from hbm_cli.doctor_platform import _system_package_install_cmd
from hbm_cli.doctor_report import Finding, _fail_and_issue, check_bool, check_info, check_ok, check_warn, doctor_check
from hbm_cli.vercel_auth import describe_vercel_auth
from hbm_constants import agent_browser_runnable, is_termux as _is_termux
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    return (item.get("name") == "kanban" and not os.environ.get("HBM_KANBAN_TASK")
            and bool(tools) and all(str(tool).startswith("kanban_") for tool in tools))


def _doctor_tool_availability_detail(toolset: str) -> str:
    """Optional explanatory suffix for toolsets whose doctor status needs context."""
    if toolset == "kanban" and not os.environ.get("HBM_KANBAN_TASK"):
...
def _apply_doctor_tool_availability_overrides(available: list[str], unavailable: list[dict]) -> tuple[list[str], list[dict]]:
    ...
    from hbm_cli.doctor_state import _honcho_is_configured_for_doctor
...
def _check_vercel_backend(issues: list[str]) -> None:
    ...
    _require(importlib.util.find_spec("vercel") is not None, ("vercel SDK", "(installed)"),
             ("vercel SDK not installed", "(pip install 'hbm-agent[vercel]')"),
             "Install the Vercel optional dependency: pip install 'hbm-agent[vercel]'", issues)
...
def _check_terminal_backend(should_fix: bool, f: Finding) -> None:
    ...
    try:
        from hbm_constants import is_container as _is_container
...
def _check_agent_browser(should_fix: bool) -> bool:
    ...
    Mirrors ``tools.browser_tool_install._find_agent_browser``'s own cascade (lazy npx or a global/HBM AGENT-managed
    ...
    try:
        ...
        # (or a global/HBM AGENT-managed install) at first use.
...
def _check_npm_audit(should_fix: bool, f: Finding) -> None:
    ...
    HBM_HOME mirror rather than the (possibly read-only) Docker install tree, hence the shared resolver.
    """
    from hbm_cli.doctor import PROJECT_ROOT
    ...
    if npm_bin:
        try:
            ...
            # See #38772. The WhatsApp bridge may live under a writable HBM_HOME mirror instead of the
