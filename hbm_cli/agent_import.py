...
# Entry delimiter of the HBM AGENT memory store (memories/MEMORY.md) and the openclaw script.
ENTRY_DELIMITER = "\n§\n"
...
def _translate_mcp_server(name: str, srv: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    """Map one Claude/Codex MCP server entry to HBM AGENT shape; returns (server, stripped secret paths)."""
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record("slash-commands", commands_dir, None, "skipped",
                        "Claude slash commands have no direct HBM AGENT equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → HBM_HOME/skills/<category>/<name>."""
        ...
        if not skill_dirs:
            ...
            return
        from hbm_cli.agent_import_sync import skill_tree_digest
...
def import_agent_command(args) -> None:
    ...
    if getattr(args, "sync", False):
        from hbm_cli.agent_import_sync import sync_imported_agents
    ...
    print(color("│          ☤ HBM AGENT — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
    ...
    def run_import(execute: bool, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return AgentImporter(agent, source_dir.resolve(), hbm_home.resolve(),
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    from hbm_cli.setup import Colors, color, print_header, print_info
