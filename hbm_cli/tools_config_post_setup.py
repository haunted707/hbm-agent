"""Post-setup install hooks and installed-state predicates for `hbm tools` provider rows."""
...
def _post_setup_spotify() -> None:
    # Full `hbm auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.hbm/.env)
    ...
    try:
        ...
        _info_lines("Run manually: hbm auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: hbm auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        ...
        _info_lines("Run manually: hbm plugins enable observability/langfuse")
    _info_lines("Restart HBM AGENT for tracing to take effect.", "Verify: hbm plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        ...
        _info_lines("Run later: hbm auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    idx = prompt_choice(
        ...
        choices=["Sign in with xAI Grok OAuth (SuperGrok / Premium+) — browser login",
                 ...
                 "Skip — configure later via `hbm auth add xai-oauth`"])
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            _print_success("    Logged in — xAI will use these OAuth credentials")
        else:
            _print_warning("    xAI Grok OAuth login did not complete. Run later: hbm auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: hbm auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``hbm tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
    backend setup without re-implementing install logic. Exit code: 0 ok, 2 unknown key."""
    key = getattr(args, "post_setup_key", None)
    if not key:
        _print_error("Usage: hbm tools post-setup <key>")
        return 2
...
# Python deps installed via ``hbm tools`` aren't in the managed runtime's locked ``all`` sync, so a
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``hbm tools`` Python dependencies present in this runtime."""
    return [
