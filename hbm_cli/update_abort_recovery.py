...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``hbm-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``hbm-serve.service`` can exist in BOTH the user and system manager (two processes)."""
    rows: list[dict] = []
...
def _run_fresh_recovery_process(
    ...
    """Spawn ``hbm_cli.update_restart_recovery --stdin`` detached from this process; None when it
    could not run (no systemd-run in gateway mode, OSError, timeout) — the caller fails closed."""
    command = [sys.executable, "-m", "hbm_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
    env["HBM_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_HBM_GATEWAY", "HBM_GATEWAY", "HBM_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _recover_gateway_restart_after_abort(
    ...
    part of the identity, not decoration: ``hbm-serve.service`` can exist in both the user and the system
    ...
    """
    from hbm_cli.update_cmd import _gateway_recovery_partition
