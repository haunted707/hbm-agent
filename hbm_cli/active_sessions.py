...
def active_session_limit_message(
    ...
    return (
        f"HBM AGENT is at the active session limit ({active_count}/{max_sessions})."
...
def format_refusal_stderr(message: str) -> str:
    ...
    return f"hbm-refusal-reason: {reason}\n{message}" if reason else str(message)
...
def session_already_owned_message(session_id: str, entry: dict[str, Any]) -> str:
    ...
    return (
        "This chat is open in another HBM AGENT window/terminal. Use it there, or start a new chat here.\n"
        f"Details: session {session_id} opened by {surface}{age}."
    )


def _registry_home(registry_home: str | Path | None = None) -> Path:
    return Path(registry_home) if registry_home is not None else Path(get_hbm_home())
...
class ActiveSessionLease:
    ...
    enabled: bool = True
    released: bool = False
    # Pinned at acquisition: a lease taken under the root HBM_HOME must release
    ...
    with _FileLock(lock_path):
        ...
        if loaded is None:
            return None, ActiveSessionRefusal(
                "HBM AGENT could not read the active-session registry at "
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile HBM_HOME override.
    # See #85431.
