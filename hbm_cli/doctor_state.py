"""HBM_HOME state checks for hbm-agent doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
from hbm_cli.doctor_report import (
    Finding, _fail_and_issue, _section, check_bool, check_info, check_ok, check_warn, doctor_check, ensure_dir,
    warn_on_error,
)
from hbm_cli.sizefmt import format_bytes as _human_bytes
from hbm_state_common import FTS_STORAGE_VERSION
...
def _doctor_memory_config(hbm_home: Path | None = None) -> dict:
    """Return the effective memory section used by doctor diagnostics."""
    from hbm_cli.doctor import HBM_HOME
    try:
        from hbm_cli.config_effective import load_user_config_effective
        config_path = (hbm_home if hbm_home is not None else HBM_HOME) / "config.yaml"
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        if deferral.get("futile"):
            ...
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'hbm-agent sessions optimize-storage' with every holder stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'hbm-agent sessions optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
def _memory_store_flags(hbm_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(hbm_home)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """HBM_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from hbm_cli.doctor import HBM_HOME, _DHH
    hbm_home = HBM_HOME
    ensure_dir(f, should_fix, hbm_home, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(hbm_home)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, hbm_home / subdir_name, f"{_DHH}/{subdir_name}/ exists",
                   f"Created {_DHH}/{subdir_name}/", f"{_DHH}/{subdir_name}/ not found")
    # SOUL.md persona file
    soul_path = hbm_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give HBM AGENT a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text("# HBM AGENT Persona\n\n<!-- Edit this file to customize how HBM AGENT communicates. -->\n\n"
                                 "You are HBM AGENT, a helpful AI assistant.\n", encoding="utf-8")
    ...
    memories_dir = hbm_home / "memories"
    if not memory_on:
...
def _write_health_reason(state_db_path: Path, *, should_fix: bool):
    ...
    from hbm_state_repair import _db_opens_cleanly, _live_writer_holds_db
    ...
    if not should_fix and state_db_path.stat().st_size > _WRITE_PROBE_SNAPSHOT_MAX_BYTES:
        check_info("state.db write-health probe skipped: store is held by a live writer and larger than 1 GB "
                   "(run 'hbm-agent doctor --fix' to probe it)")
        return None
    import sqlite3
    import tempfile
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'hbm-agent doctor --fix' (or 'hbm-agent sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'hbm-agent doctor --fix' (or 'hbm-agent sessions repair') to recover hidden sessions"),
}
_STATE_DB_STRUCTURAL_ISSUE = (
    ...
    "cannot repair it. Stop the gateway, then run 'hbm {profile_arg}sessions recover --source {db_path} "
    "--inspect-only' and, if it reports recoverable, 'hbm {profile_arg}sessions recover --source {db_path} "
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    if kind == "structural":
        from hbm_constants import profile_cli_selector
    ...
    from hbm_state_repair import repair_state_db_schema
...
def _report_structural_damage(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, reason) -> bool:
    ...
    from hbm_state_repair import state_db_has_structural_damage
...
def _classify_unreadable_state_db(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, exc: Exception) -> None:
    ...
    from hbm_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from hbm_state_dbfile import collect_state_db_stats, count_db_holders
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'hbm-agent sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            if not should_fix:
                return f.issues.append("Large WAL file — run 'hbm-agent doctor --fix' to checkpoint")
            ...
            from hbm_state_holders import live_writer_holds_db
            from hbm_state_repair import _connect_repair_durable, _exclusive_repair_db_guard
            _SKIP = ("Large WAL file — cannot prove state.db is quiet (stop the profile's gateway first, then "
                     "re-run 'hbm-agent doctor --fix' to checkpoint)")
            if live_writer_holds_db(state_db_path, connect_repair_durable=_connect_repair_durable):
                ...
                check_warn("WAL checkpoint skipped: cannot prove state.db is quiet",
                           ...
                           "and re-run 'hbm-agent doctor --fix')")
            ...
            with _exclusive_repair_db_guard(state_db_path) as (guard, guard_error):
                if guard is None:
                    check_warn("WAL checkpoint skipped: could not take exclusive ownership of state.db",
                               f"({guard_error}; stop the profile's gateway and re-run 'hbm-agent doctor --fix')")
                    return f.issues.append(_SKIP)
...
def _check_state_db(should_fix: bool, f: Finding) -> None:
    ...
    from hbm_cli.doctor import HBM_HOME, _DHH
    state_db_path = HBM_HOME / "state.db"
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    from hbm_cli.doctor import HBM_HOME, _DHH
    hub_dir = HBM_HOME / "skills" / ".hub"
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: hbm-agent skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: hbm-agent memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: hbm-agent memory setup",
                        "No Honcho API key — run 'hbm-agent memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run hbm-agent memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: hbm-agent memory status")
    else:
        check_warn(f"{name} plugin not found", "run: hbm-agent memory setup")


@doctor_check()
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    from hbm_cli.doctor import HBM_HOME
    name = _doctor_memory_config(HBM_HOME).get("provider", "")
    if not name:
...
def _check_profiles(should_fix: bool, f: Finding) -> None:
    from hbm_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
    import re as _re
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"hbm-agent -p (\S+)", wrapper.read_text(encoding="utf-8"))
