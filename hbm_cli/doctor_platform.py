...
from hbm_cli.config import is_nix_install_method, recommended_update_command_for_method
...
from hbm_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from hbm_cli.doctor import PROJECT_ROOT
    from hbm_cli.config import detect_install_method
...
def _hbm_database_paths(hbm_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for HBM AGENT-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from hbm_cli.backup import _QUICK_STATE_FILES
    entries = [(name, hbm_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((hbm_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(hbm_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``hbm_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from hbm_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_journal_modes(hbm_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    try:
        databases = _hbm_database_paths(hbm_home if hbm_home is not None else HBM_HOME)
    except Exception as exc:
        check_warn(f"Could not list HBM AGENT databases: {exc}")
        return
    exposed = []
    for name, path in databases:
        ...
        if error is None and mode == "wal" and configured == "delete":
            ...
            check_warn(f"{name} is in WAL mode ({size}) despite database.journal_mode=delete",
                       ...
                       + ". Stop every HBM AGENT process for this profile, then run a one-time offline "
            ...
            check_warn(f"{name} is in WAL mode on a cross-VM filesystem (virtiofs/9p, {size})",
                       "(WAL can silently corrupt across the VM boundary; stop every HBM AGENT process and run a one-time "
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs hbm_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from hbm_cli import __version__ as init_version
    except Exception:
        return
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from hbm_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-hbm", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from hbm_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
_TCC_STABLE_DETAIL = {
    ...
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `hbm-agent desktop --setup-tcc-identity`)",
}
...
def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/HBM AGENT.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/HBM AGENT.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored HBM AGENT-Setup launcher.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/HBM AGENT.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from hbm_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and HBM AGENT "
                   ...
                   "    then enable your terminal (and HBM AGENT.app if you use Desktop), and restart them once. "
                   "With HBM AGENT' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from hbm_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from hbm_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: HBM AGENT already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "hbm-agent" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(hbm not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    if link.is_symlink():
        ...
        return check_ok(f"{display}/hbm exists (non-symlink)")
    else:
    ...
    check_ok(f"{verb} symlink: {display}/hbm → {venv_bin}")
    f.fixed += 1
