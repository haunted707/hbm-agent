...
from hbm_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from hbm_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from hbm_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.hbm/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_hbm_home() / "logs"
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from hbm_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    Named-profile actions therefore start from HBM AGENT' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``HBM_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from hbm_cli.env_loader import (
            _PROFILE_MANAGED_ENV_KEYS, _env_keys_defined_in_dotenv, get_secret_source_values,
        )
        from hbm_cli.web_server_profiles import _resolve_profile_dir
        from hbm_constants import apply_subprocess_home_env, get_default_hbm_root
        ...
        try:
            source_homes = {str(get_default_hbm_root()), str(get_hbm_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["HBM_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["HBM_NONINTERACTIVE"] = "1"
    ...
    # The dashboard runs inside the gateway process, so os.environ carries _HBM_GATEWAY=1;
    ...
    action_env.pop("_HBM_GATEWAY", None)
    if env_overrides:
...
def _action_targets_system_gateway(subcommand: List[str]) -> bool:
    ...
    from hbm_cli.gateway import _select_systemd_scope
    from hbm_constants import reset_hbm_home_override, set_hbm_home_override

    profile = _named_profile_from_action(subcommand)
    if profile is None:
        return _select_systemd_scope(False)
    # Unit names are derived from HBM_HOME, so a selector-bearing action must be resolved
    ...
    from hbm_cli.web_server_profiles import _resolve_profile_dir
    token = set_hbm_home_override(_resolve_profile_dir(profile))
    try:
        return _select_systemd_scope(False)
    finally:
...
def _spawn_hbm_action(
    ...
    if _action_targets_system_gateway(subcommand):
        ...
        if not _sudo_noninteractive_ok(["-l", "--", *cmd]):
            message = (
                ...
                f"'sudo hbm {' '.join(subcommand)}' on the host, or grant that user NOPASSWD sudo."
            )
    ...
    # dashboard profile's credentials; see _profile_action_environment (also drops _HBM_GATEWAY).
    ...
    action_id = (env_overrides or {}).get("HBM_ACTION_ID")
    if action_id:
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``hbm-agent --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_hbm_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``hbm [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``HBM_HOME`` and hit the same exit-78 refusal."""
    from hbm_cli.web_server_profiles import _profile_cli_args
    ...
    if profile and verb == "restart" and multiplexed_profile_refusal(profile, verb) is not None:
        from hbm_constants import get_process_hbm_home, profile_name_for_home
        args = [] if profile_name_for_home(get_process_hbm_home()) == "default" else ["-p", "default"]
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from hbm_cli.web_server import _spawn_gateway_restart
    try:
