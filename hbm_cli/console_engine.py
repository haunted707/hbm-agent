"""Safe HBM AGENT Console command engine."""
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["HbmConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _split_line(line: str) -> list[str]:
    ...
    # See #78293.
    # See #83934.
    from hbm_cli._subprocess_compat import split_command_line
    try:
...
def _clean_summary(text: str | None) -> str:
    ...
    return "" if summary.startswith("Run `hbm ") else summary
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from hbm_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from hbm_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        parser = _ArgumentParser(prog="hbm-agent", add_help=False)
        subparsers = parser.add_subparsers(dest="_console_command")
        module = importlib.import_module(self.module)
        entry = getattr(module, self.builder)
        main_handler = (
            (lambda: getattr(importlib.import_module("hbm_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"hbm_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"hbm_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "portal": (_CliSurface("adder", "hbm_cli.portal_cli", "add_parser"), "info, tools"),
    "project": (
        _CliSurface("builder", "hbm_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "hbm_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "hbm_cli.send_cmd", "register_send_subparser")


def _register_command_family(
    engine: "HbmConsoleEngine", root: str, surface: _CliSurface, paths: str) -> None:
    ...
    for child_path, mutating in _paths(paths):
        ...
        def handler(_engine: HbmConsoleEngine, args: list[str], fixed=child_path) -> str:
            return _dispatch(surface, root, fixed, args, namespace_update)

        engine.register(
            full_path, usage, summaries.get(full_path) or f"Run `hbm {usage}`.", handler,
            mutating=mutating, confirmation=f"Run `hbm {usage}`?")
...
_BLOCKED_PAIRS = {
    ("config", "edit"): "`config edit` opens an editor and is not available in HBM AGENT Console.",
    ("mcp", "serve"): "`mcp serve` starts a server and is not available in HBM AGENT Console.",
    ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in HBM AGENT Console.",
    ("skills", "config"): "`skills config` is interactive and is not available in HBM AGENT Console.",
    ("skills", "publish"): "`skills publish` is not available in HBM AGENT Console.",
    ("portal", "login"): "`portal login` is interactive and is not available in HBM AGENT Console.",
    ("portal", "open"): "`portal open` opens a browser and is not available in HBM AGENT Console.",
    ("kanban", "tail"): "`kanban tail` streams output and is not available in HBM AGENT Console.",
    ("kanban", "watch"): "`kanban watch` streams output and is not available in HBM AGENT Console.",
    ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in HBM AGENT Console.",
    ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in HBM AGENT Console.",
    ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in HBM AGENT Console.",
    ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in HBM AGENT Console.",
    ("kanban", "specify"): "`kanban specify` starts agent work and is not available in HBM AGENT Console.",
    ("kanban", "gc"): "`kanban gc` is not available in HBM AGENT Console.",
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in HBM AGENT Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in HBM AGENT Console.",
}


class HbmConsoleEngine:
    """Curated line-command executor for HBM AGENT Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "hbm-agent":
                tokens = tokens[1:]
            if not tokens:
                return ConsoleResult("ok", output=self.help_text())
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "HBM AGENT Console does not run shell syntax. Use one supported "
                    "HBM AGENT command at a time.")
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["HBM AGENT Console", "", "Supported commands:"]
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported HBM AGENT Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in HBM AGENT Console."
        if first in _BLOCKED_TOP:
            return f"`hbm {first}` is not available in HBM AGENT Console."
...
def _captured(fn):
    ...
    @functools.wraps(fn)
    def wrapper(engine: HbmConsoleEngine, args: list[str]) -> str:
...
def _simple_command(usage: str, module: str, name: str, make_args=lambda: (), **kwargs):
    ...
    def handler(_engine: HbmConsoleEngine, args: list[str]) -> str:
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in HBM AGENT Console.")
...
_version = _simple_command(
    "version", "hbm_cli._startup_fast", "print_fast_version_info", check_updates=True)


def _status(_engine: HbmConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "status")
    from hbm_cli.status import show_status
...
_doctor = _simple_command(
    "doctor", "hbm_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "hbm_cli.config", "show_config")
_cron_status = _simple_command("cron status", "hbm_cli.cron", "cron_status")


def _logs(_engine: HbmConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in HBM AGENT Console.")
    ns = _parse(
    ...
    from hbm_cli.logs import list_logs, tail_log
    if ns.log_name == "list":
...
def _sessions_list(_engine: HbmConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: HbmConsoleEngine, args: list[str]) -> str:
...
def _config_path(_engine: HbmConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from hbm_cli.config import get_config_path
    return str(get_config_path())


def _config_set(_engine: HbmConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    from hbm_cli.config import set_config_value
    return _capture_output(lambda: set_config_value(args[0], " ".join(args[1:])))


@_captured
def _config_migrate(_engine: HbmConsoleEngine, args: list[str]) -> None:
    _expect_no_args(args, "config migrate")
    from hbm_cli.config import migrate_config
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from hbm_state import SessionExportTooLargeError, resolved_max_export_messages
...
@_captured
def _sessions_export(_engine: HbmConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_rename(_engine: HbmConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_optimize(_engine: HbmConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_repair(_engine: HbmConsoleEngine, args: list[str]) -> int | None:
    ns = _parse(
        "sessions repair", args, (("--check-only",), dict(action="store_true")),
        (("--no-backup",), dict(action="store_true")))
    from hbm_state import DEFAULT_DB_PATH
    from hbm_state_repair import _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HbmConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch(_CLI_FAMILIES["profile"][0], "profile", (), ())


def _cron_list(_engine: HbmConsoleEngine, args: list[str]) -> str:
    ns = _parse("cron list", args, (("--all",), dict(action="store_true")))
    from hbm_cli.cron import cron_list
...
def _cron_pause(_engine: HbmConsoleEngine, args: list[str]) -> str:
...
def _cron_resume(_engine: HbmConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, _hbm_now, rearm_oneshot, resume_job
    try:
        job = (
            rearm_oneshot(ns.job, _hbm_now().isoformat() if ns.run_now else ns.at)
...
def _cron_run(_engine: HbmConsoleEngine, args: list[str]) -> str:
...
_BUILTIN_COMMANDS = (
    (("status",), "status", "Show HBM AGENT component status.", _status, ""),
    (("version",), "version", "Show HBM AGENT version information.", _version, ""),
    (("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, ""),
    (("logs",), "logs [name] [-n N]", "Show recent HBM AGENT logs.", _logs, ""),
    ...
    (("config", "set"), "config set <key> <value>", "Set a configuration value.", _config_set,
     "Update HBM AGENT configuration?"),
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update HBM AGENT configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    ...
    engine = HbmConsoleEngine()

    def say(text: str, **kw) -> None:
        if interactive:
            print(text, file=stdout, **kw)

    say("HBM AGENT Console. Type `help` for commands, `exit` to quit.")
    while True:
        say("hbm> ", end="", flush=True)
        line = stdin.readline()
        if line == "":
            say("")
            return 0
