...
logger = logging.getLogger("hbm_cli.update_cmd")  # log-record parity with the origin module
...
def _detect_venv_python_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str, str]]:
    ...
    The hbm.exe shim guard misses the Desktop backend and anything off ``venv\\Scripts\\python(w).exe``;
    ...
    for proc in proc_iter:
        ...
        name_low = name.lower()
        if not (in_venv or name_low.startswith(("python", "pypy")) or name_low in {"uv.exe", "uvx.exe", "hbm-agent.exe"}):
            continue
        ...
        # its .pyd files — match cmdline (venv path, or `-m hbm_cli.main` + root/cwd).
        if in_venv or venv_prefix in cmdline_low or (
            "hbm_cli.main" in cmdline_low and (root_prefix in cmdline_low or _cwd_prefix(proc).startswith(root_prefix))
        ):
...
def _hbm_holder_subcommand(cmdline: str) -> str | None:
    """The actual HBM AGENT SUBCOMMAND a venv-holder argv runs, or None (callers must NOT guess a label).

    Token-based, never substring (``kanban --preserve-cache`` contains "serve"): find the ``hbm_cli.main`` /
    ``hbm(.exe)`` entry token, return the first following token that isn't a flag or a flag's value.
    ...
    def _is_entry(i: int, token: str) -> bool:
        ...
        return (low.endswith("hbm_cli.main") and i > 0 and tokens[i - 1] == "-m") or (
            low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1] in ("hbm-agent", "hbm-agent.exe"))
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    for pid, name, cmdline in matches[:6]:
        hint = hint_by_subcommand.get(_hbm_holder_subcommand(cmdline) or "", "")
...
def _leftover_pausable_gateway_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    from hbm_cli._scan_venv_blockers import _is_pausable_gateway
    psutil = _psutil()
    pids: list[int] = []
...
def _refuse_gateway_ancestor_tree_kill(pids: list[int], *, gateway_mode: bool) -> bool:
    ...
    if gateway_mode or not pids:
        return False
    def _ancestors():
        from hbm_cli.gateway import _is_pid_ancestor_of_current_process
...
def _ledger_manual_serve_holders(matches: list[tuple[int, str, str]]) -> list[dict]:
    ...
    try:
        from hbm_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    with suppress(Exception):
        ...
        if scripts_dir is not None:
            hbm = next((str(scripts_dir / n) for n in ("hbm-agent.exe", "hbm-agent") if (scripts_dir / n).is_file()), hbm)
    ...
    for entry in entries:
        ...
        commands.append(
            [hbm] + (["--profile", profile] if profile and profile != "default" else [])
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    from hbm_cli.update_cmd import _m, _record_update_step
...
def _orphaned_desktop_backend_pids(matches: list[tuple[int, str, str]]) -> list[tuple[int, int]] | None:
    ...
    would dead-end the update with "HBM AGENT is still running" and zero open windows. Qualifies only if cmdline
    is a HBM AGENT backend AND the parent is demonstrably gone (PID missing or reused). Tree-aware: holders inside
    ...
    it tree-kills its backends and waits for the venv shim before spawning hbm-setup, and the
    ...
    dead-ends the update with "HBM AGENT is still running" while the user stares at zero open windows (ryanc's
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # gateway.status.get_process_start_time so pid_is_hbm round-trips at kill time.
...
def _ledger_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int]:
    ...
    try:
        from hbm_cli.process_identity import REAPABLE_PURPOSES, ledger_entries, spawner_is_dead
...
def _handoff_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    hand-off. Inside the hand-off gate (marker + ``--gateway`` + no live ``hbm.exe`` shim) nothing legitimate
...
def _stop_process_trees(pids: list[int] | list[tuple[int, int]]) -> None:
    ...
    ``taskkill /T /F``: stopping only the parent can leave a ``.hbm-runtime`` child holding the install open.

    See #70026.
    """
    from gateway.status import get_process_start_time
    from hbm_cli._subprocess_compat import pid_is_hbm, windows_hide_flags
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_hbm(pid, expected_start_time=expected_start_time):
                logger.debug("Skipping taskkill of non-HBM AGENT or changed PID %s", pid)
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    ...
    """
    return "hbm_cli.main" in (cmdline or "").lower() and _hbm_holder_subcommand(cmdline) in _BACKEND_PURPOSES
...
def _restore_windows_gateway_service(name: str, *, timeout: float = 60.0) -> None:
    ...
    from hbm_cli.update_cmd import _start_windows_gateway_service
    service = _win_service(name)[1]
...
def _pause_windows_gateway_services(service_gateways, token: dict, profiles: dict, unmapped: list) -> dict:
    ...
    from hbm_cli.update_cmd import _restore_windows_gateway_service, _stop_windows_gateway_service
    paused_services = []
...
def _discover_windows_gateways():
    ...
    from hbm_cli.gateway import find_gateway_pids, find_profile_gateway_processes, find_windows_gateway_services
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    Scheduled/startup gateways run via pythonw.exe, invisible to the hbm.exe instance guard, yet keep files
    ...
    with _abort_on_error("Could not prepare Windows gateway pause for update"):
        ...
        from hbm_cli.gateway import _capture_gateway_argv
    ...
    print("→ Stopping Windows gateway process(es) before updating HBM AGENT...")
...
def _record_attested_cold_start_profiles(token: dict, running_profiles: set) -> None:
    ...
    with _best_effort("Could not evaluate per-profile attested cold-starts before update: %s"):
        if not _desktop_owns_gateway_lifecycle():
            return
        from hbm_cli.profiles import get_active_profile_name, profiles_to_serve
...
def _cold_start_attested_profiles(token: dict) -> None:
    """Spawn each ``cold_start_profiles`` entry under its own HBM_HOME and consume exactly the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    """Overwrite ``$HBM_HOME/bootstrap-cache/install-<ref>.{ps1,sh}`` for *branch* from the fresh checkout.

    Old ``hbm-setup.exe`` builds NEVER re-download a cached branch-ref script, so a stale one runs
    ...
    with _best_effort('Could not refresh bootstrap-cache scripts after update: %s'):
        cache_dir = Path(_m().get_hbm_home()) / "bootstrap-cache"
...
def _relaunch_paused_gateways(token: dict, profiles: dict, unmapped: list) -> tuple[list[str], int]:
    ...
    with _abort_on_error("Could not load Windows gateway restart helper"):
        from hbm_cli.gateway import launch_detached_gateway_restart_by_cmdline, launch_detached_profile_gateway_restart
...
def _in_handoff_without_live_shim(args) -> bool:
    """GUI hand-off gate: ``--gateway`` + update-incomplete marker AND no live ``hbm.exe`` shim.
    ...
    try:
        if not (bool(getattr(args, "gateway", False)) and _m()._update_marker_path().exists()):
            return False
        scripts_dir = _m()._venv_scripts_dir()
        return scripts_dir is not None and not _m()._detect_concurrent_hbm_instances(scripts_dir)
    except Exception:
        return False
...
def _clear_windows_venv_holders_or_exit(args, gateway_mode: bool, _windows_gateway_resume):
    ...
    from hbm_cli.update_cmd import _m, _record_update_step, _refuse_gateway_ancestor_tree_kill
    ...
    for classifier, message in (
        (_m()._ledger_reapable_backend_pids, "ledger-identified orphaned HBM AGENT backend process(es) hold the venv"),
    ...
    if holders and _in_handoff_without_live_shim(args) and (handoff_backends := _m()._handoff_reapable_backend_pids(holders)):
        holders = _reap_and_rescan(
            f"  ⚠ {len(handoff_backends)} HBM AGENT backend process(es) "
