...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from hbm_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _apply_capabilities(rows: list[dict]) -> None:
    ...
    from hbm_cli.models import model_supports_fast_mode
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`hbm_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
...
def _append_unconfigured_rows(
    ...
    from hbm_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (HBM AGENT device flow or Claude Code
    ...
    try:
        ...
        readers = (read_hbm_oauth_credentials, read_claude_code_credentials)
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's HBM AGENT overlay declares it keyless."""
    try:
        from hbm_cli.providers import HBM_OVERLAYS
        overlay = HBM_OVERLAYS.get(slug)
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from hbm_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from hbm_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from hbm_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (hbm_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="hbm-picker-pricing-prewarm", daemon=True)
