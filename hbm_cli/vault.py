"""``hbm-agent vault`` — manage the local encrypted autofill vault.

Subcommands:
- ``hbm-agent vault add``   interactive wizard; the password is read via
  getpass (never echoed, never accepted as argv). The login identifier is
  visible metadata and prompted normally.
- ``hbm-agent vault list``  metadata — labels, kinds, identifiers, origins,
  handles. Passwords are never shown.
- ``hbm-agent vault rm``    remove an item by handle/id.
...
def _cmd_list(args) -> None:
    ...
    `hbm-agent vault list` unlock does not carry into a chat session — unlock there when asked)."""
    ...
    if not rows and not locked:
        c.print("[dim]Vault is empty. Add an item with `hbm-agent vault add`.[/]")
        return
    if rows:
...
def _cmd_sources(args) -> None:
    ...
    for name, cls in classes.items():
        if name in enabled:
            ...
            status = "[dim]turned off[/] (`hbm-agent vault sources --enable {name}` to use it)".format(name=name)
        else:
...
def register_cli(subparser) -> None:
    """Build the ``hbm-agent vault`` argparse tree (called from main.py)."""
    ...
    p_rm.add_argument("handle", help="Item handle (see `hbm-agent vault list`)")
