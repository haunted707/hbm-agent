"""HBM AGENT-managed uv and Python runtime repair.

The Python backing the install is shared by every HBM AGENT profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.hbm-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
def managed_uv_path() -> Path:
    """Path of HBM AGENT' own uv binary (``$HBM_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_hbm_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def pip_install_hint(package: str) -> str:
    ...
    Names HBM AGENT' own uv when it exists: the installer drops it in ``$HBM_HOME/bin``
...
def managed_python_env(
    ...
    """Return a sanitized environment for HBM AGENT-private uv Python commands."""
    target = (
...
def _record_runtime_repair(repair: RuntimeRepairResult) -> None:
    ...
    """
    from hbm_cli.update_receipt import record_skip, record_step
...
def _uv_self_update_stamp() -> Path:
    from hbm_constants import get_hbm_home
    return get_hbm_home() / "cache" / ".uv_self_update_stamp"
...
def _reload_hbm_constants():
    """Re-execute ``hbm_constants`` from disk (the imported one may predate venv_python_path)."""
    import hbm_constants
    return importlib.reload(hbm_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the HBM AGENT generation: %s", python)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("hbm_cli.main")
    ...
    if holders:
        ...
        return True, f"other HBM AGENT processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    updater's own ``python.exe`` (or a waiting ``hbm.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\hbm.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            f"      cd {root}",
            "      <system Python> -m hbm_cli.main update",
            "    Sessions stay protected meanwhile: HBM AGENT keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    into ``$HBM_HOME/bin/`` instead of ``~/.local/bin/``.
    """
