...
Split out of ``hbm_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from hbm_cli.kanban_db import Task
...
# ``HBM_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a HBM AGENT profile. Expected steady-state
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|☤\s*HBM AGENT")
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``hbm_cli.profiles.profile_exists``, or ``None`` when it cannot be
    ...
    """
    try:
        from hbm_cli.profiles import normalize_profile_name, profile_exists
    except Exception:
        return None
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several HBM AGENT homes),
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real HBM AGENT profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (HBM AGENT ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``HBM_KANBAN_DB`` (pins every
...
def _module_hbm_argv() -> list[str]:
    """Interpreter-bound HBM AGENT CLI invocation (``hbm_cli.main`` is the
    console-script target — there is no top-level ``hbm-agent`` package)."""
    return [sys.executable, "-m", "hbm_cli.main"]


def _absolute_hbm_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved HBM AGENT shim."""
...
def _hbm_path_argv(path: str) -> list[str]:
    """argv for a resolved HBM AGENT executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_hbm_argv()
    return [_absolute_hbm_path(path)]


def _resolve_hbm_argv() -> list[str]:
    """Resolve the ``hbm-agent`` invocation as argv for ``Popen``: ``$HBM_BIN``
    ...
    hbm_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("hbm-agent")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``hbm_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``hbm-agent`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_hbm_bin``; local because ``hbm_cli``
    sits below ``gateway`` in the dependency order.
    """
    import importlib.util
    import shutil

    env_bin = os.environ.get("HBM_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _hbm_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _hbm_path_argv(resolved_env_bin)
        return _module_hbm_argv()
    ...
    hbm_bin = _safe_which_no_cwd("hbm-agent") if _kb._IS_WINDOWS else shutil.which("hbm-agent")
    if hbm_bin:
        return _hbm_path_argv(hbm_bin)
    return _module_hbm_argv()
...
def _resolve_worker_cli_toolsets(hbm_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``HBM_KANBAN_TASK`` is set.
    """
    if not hbm_home:
        return None
    try:
        ...
        token = set_hbm_home_override(hbm_home)
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(hbm_home)))
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for HBM_HOME=%r (%s)",
            hbm_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, hbm_home: Optional[str]) -> list[str]:
    ...
    cmd = [
        *_resolve_hbm_argv(),
        ...
        "--cli",
        # Workers run under a profile-scoped HBM_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(hbm_home)
    if worker_toolsets:
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    ``HBM_KANBAN_DB`` / ``HBM_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from hbm_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    if task.tenant:
        env["HBM_TENANT"] = task.tenant
    env["HBM_KANBAN_TASK"] = task.id
    env["HBM_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["HBM_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["HBM_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["HBM_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["HBM_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["HBM_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    env["HBM_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads HBM_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["HBM_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping HBM_TUI covers
    # older hbm builds on PATH that predate the flag's precedence.
    env.pop("HBM_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("HBM_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`hbm-agent` executable not found on PATH. "
            "Install HBM AGENT or activate its venv before running the kanban dispatcher."
        )
...
from hbm_cli import kanban_db as _kb  # noqa: E402
from hbm_cli import kanban_db_connect as _kbc  # noqa: E402
from hbm_cli import kanban_db_workspace as _kbw  # noqa: E402
