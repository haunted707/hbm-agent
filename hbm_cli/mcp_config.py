...
from hbm_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_hbm_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.hbm/.env`` into ``os.environ`` and
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return HbmTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see hbm_cli/main.py for why the dest is renamed).
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_hbm_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def mcp_command(args):
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from hbm_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `hbm-agent plugin`).
