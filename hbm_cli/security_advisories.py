"""Security advisory checker for HBM AGENT.
...
class Advisory:
    ...
    ``{hbm_home}``; ``full_remediation_text`` fills it with ``display_hbm_home()`` at render
    time so a profile / HBM_HOME user is sent to their own .env.
    """
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including hbm when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in {hbm_home}/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: hbm-agent doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'hbm-agent doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def full_remediation_text(hit: AdvisoryHit) -> list[str]:
    ...
    return [
        ...
        "",
        "Remediation:",
        *(f"  {i}. {step.format(hbm_home=display_hbm_home())}" for i, step in enumerate(a.remediation, 1)),
    ]


# ─── Startup-banner gating ────────────────────────────────────────────────────
# Once the banner is seen we cache that in ``~/.hbm/cache/advisory_banner_seen`` (one
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_hbm_home()) / "cache"
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active (IDs: {', '.join(h.advisory.id for h in fresh)}). "
            "Run `hbm-agent doctor` on the gateway host for details.")
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``hbm-agent doctor``.
