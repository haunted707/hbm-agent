...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``hbm_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``hbm_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from hbm_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (hbm_cli.main's version lives behind imports)."""
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (hbm_constants is stdlib-only)."""
    # hbm_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: HBM AGENT root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("hbm-agent", "hbm-agent-acp")
...
def _default_hbm_root() -> Path | None:
    ...
    try:
        return Path(get_default_hbm_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``hbm-agent`` launchers when they vanish.

    On Windows, ``hbm-agent`` resolves through launchers derived from the venv console scripts — never
    ...
    if not windows:
        return []
    root = Path(root)
    home = _default_hbm_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every hbm_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored hbm-agent launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ hbm-agent launchers now resolve from {home_bin} "
...
def _quarantine_running_hbm_exe(
    ...
    """Rename live hbm*.exe shims aside so the installer can rewrite them.
    ...
    names = set(_load_console_script_names(scripts_dir.parent.parent)) or {
        "hbm-agent", "hbm-agent-acp", "hbm-agent-run"}
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_hbm_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `hbm-agent` gone from PATH. Restore only renames back when the
