#!/usr/bin/env python3
"""HBM AGENT CLI - Main entry point.
...
from hbm_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python hbm_cli/main.py`` (script mode: sys.path[0]
# is hbm_cli/, not the repo root) can import hbm_cli._startup_fast.
...
# PID" class it exists for. ``hbm_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from hbm_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from hbm_cli.oneshot import run_oneshot
...
def _warn_if_unsupervised_pid1(pid: "int | None" = None) -> None:
    ...
    children). A Compose service that overrides ``entrypoint:`` to invoke hbm directly makes
    hbm itself PID 1 — nothing then ``wait()``s on those orphans and they accumulate as
    ...
    if (pid if pid is not None else os.getpid()) != 1:
        return
    print(
        "[hbm] WARNING: this process is PID 1 with no init above it "
...
def _set_process_title() -> None:
    """Cosmetic: show 'hbm-agent' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``hbm.exe``). Never fatal.
    """
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("hbm-agent")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"hbm-agent", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"hbm-agent")
    except Exception:
        pass
...
# hbm_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("HBM_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".hbm", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``HBM_TUI=1``, then a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("HBM_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `resetTerminalModes()` is the later cousin. ``HBM_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("HBM_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``hbm-agent --version`` is answered before config/logging imports.
...
from hbm_cli.subcommands.cron import build_cron_parser
from hbm_cli.subcommands.sync import build_sync_parser
from hbm_cli.subcommands.gateway import build_gateway_parser
from hbm_cli.subcommands.profile import build_profile_parser
from hbm_cli.subcommands.model import build_model_parser
from hbm_cli.subcommands.setup import build_setup_parser

from hbm_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from hbm_cli.subcommands.slack import build_slack_parser
from hbm_cli.subcommands.login import build_login_parser
from hbm_cli.subcommands.logout import build_logout_parser
from hbm_cli.subcommands.auth import build_auth_parser
from hbm_cli.subcommands.status import build_status_parser
from hbm_cli.subcommands.pause import build_pause_parser
from hbm_cli.subcommands.webhook import build_webhook_parser
from hbm_cli.subcommands.hooks import build_hooks_parser
from hbm_cli.subcommands.doctor import build_doctor_parser
from hbm_cli.subcommands.verify import build_verify_parser
from hbm_cli.subcommands.security import build_security_parser
from hbm_cli.subcommands.approvals import build_approvals_parser
from hbm_cli.subcommands.dump import build_dump_parser
from hbm_cli.subcommands.debug import build_debug_parser
from hbm_cli.subcommands.backup import build_backup_parser
from hbm_cli.subcommands.import_cmd import build_import_cmd_parser
from hbm_cli.subcommands.import_agent import build_import_agent_parser
from hbm_cli.subcommands.config import build_config_parser
from hbm_cli.subcommands.skin import build_skin_parser
from hbm_cli.subcommands.console import build_console_parser
from hbm_cli.subcommands.update import build_update_parser
from hbm_cli.subcommands.uninstall import build_uninstall_parser
from hbm_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from hbm_cli.subcommands.gui import build_gui_parser
from hbm_cli.subcommands.logs import build_logs_parser
from hbm_cli.subcommands.prompt_size import build_prompt_size_parser
from hbm_cli.subcommands.memory import build_memory_parser
from hbm_cli.subcommands.acp import build_acp_parser
from hbm_cli.subcommands.tools import build_tools_parser
from hbm_cli.subcommands.insights import build_insights_parser
from hbm_cli.subcommands.monitoring import build_monitoring_parser
from hbm_cli.subcommands.skills import build_skills_parser
from hbm_cli.subcommands.pairing import build_pairing_parser
from hbm_cli.subcommands.plugins import build_plugins_parser
from hbm_cli.subcommands.mcp import build_mcp_parser
from hbm_cli.subcommands.claw import build_claw_parser
from hbm_cli.subcommands.vault import build_vault_parser
from hbm_cli.subcommands.moa import build_moa_parser
from hbm_cli.subcommands.fallback import build_fallback_parser
from hbm_cli.subcommands.worktree import build_worktree_parser
from hbm_cli.subcommands.browser import build_browser_parser
from hbm_cli.subcommands.secrets import build_secrets_parser
from hbm_cli.subcommands.egress import build_egress_parser
from hbm_cli.subcommands.migrate import build_migrate_parser
from hbm_cli.subcommands.checkpoints import build_checkpoints_parser
from hbm_cli.subcommands.bundles import build_bundles_parser
from hbm_cli.subcommands.curator import build_curator_parser
from hbm_cli.subcommands.pets import build_pets_parser
from hbm_cli.subcommands.journey import build_journey_parser
from hbm_cli.subcommands.computer_use import build_computer_use_parser
from hbm_cli.subcommands.sessions import build_sessions_parser
from hbm_cli.subcommands.completion import build_completion_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'hbm {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any hbm-agent module import: many modules
# cache HBM_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# HBM_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.hbm/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors hbm_cli.profiles._PROFILE_ID_RE
...
def _inside_mcp_add_args(argv: list, index: int) -> bool:
    ...
    ``--profile``), not to HBM AGENT' own profile selector.
    """
    try:
...
def _looks_like_hbm_invocation() -> bool:
    ...
    while i < len(argv):
        ...
        if arg in {"--profile", "-p"} and i + 1 < len(argv):
            ...
            if re.match(_PROFILE_NAME_RE, value):
                return value, 2, i
            if not saw_subcommand and not _looks_like_option_value(raw) and _looks_like_hbm_invocation():
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    ...
    if name == "default":
        return None
    from hbm_constants import sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    HBM_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    HBM_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), HBM_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("HBM_SUPERVISED_CHILD") or os.environ.get("HBM_S6_SUPERVISED_CHILD"):
        return True
    ...
    return os.environ.get(
        "HBM_GATEWAY_EXTERNAL_SUPERVISOR", ""
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HBM_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # HBM_HOME already set with no explicit flag: trust it only when it
    ...
    if profile_name is None and not _under_gateway_supervisor(argv) and not _desktop_ssh_backend(argv):
        try:
            ...
            active_path = get_default_hbm_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from hbm_cli.profiles import resolve_profile_env

        hbm_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        hbm_home = _resolve_sudo_user_profile_env(profile_name)
        if not hbm_home:
        ...
        # A bug in profiles.py must NEVER prevent hbm from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["HBM_HOME"] = hbm_home
...
# AFTER the profile override on purpose — no hbm-agent module may import before
...
if sys.platform == "win32":
    try:
        from hbm_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.hbm/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from hbm_cli.config import get_hbm_home
from hbm_cli.env_loader import load_hbm_dotenv
...
load_hbm_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets → HBM_REDACT_SECRETS BEFORE hbm_logging
...
try:
    ...
    # serves hbm_logging, hbm_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from hbm_cli.config_effective import load_user_config_effective as _load_effective_early

    _cfg_path = get_hbm_home() / "config.yaml"
    if _cfg_path.exists():
        _early_cfg_raw = _load_effective_early(_cfg_path)
        if "HBM_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["HBM_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from hbm_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from hbm_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if hbm_constants not importable yet

import logging
import threading
from datetime import datetime

from hbm_cli import __version__, __release_date__

from hbm_cli.model_setup_flows import (
...
from hbm_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from hbm_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from hbm_cli.main_dashboard import (
...
from hbm_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from hbm_cli.main_provider_setup import (
...
from hbm_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from hbm_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from hbm_cli.main_desktop import (
    cmd_gui,
)
from hbm_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from hbm_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from hbm_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from hbm_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_hbm_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("HBM_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("HBM_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.hbm/.env assigns a non-empty value to any provider key."""
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from hbm_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    counts only once its identity exists, and the boot bootstrap (``hbm_cli.free_tier_bootstrap``)
    ...
    """
    from hbm_cli.config import DEFAULT_CONFIG, get_env_path, get_hbm_home, load_config
    from hbm_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from hbm_cli.config import split_model_config_default
    ...
    _has_hbm_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_hbm_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once HBM AGENT is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_hbm_config and not strict_profile_scope:
        try:
    ...
    try:
        from hbm_cli.anon_auth import guest_enabled, has_guest
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from hbm_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a read-only ``SessionDB`` (lazy import, so test patches on ``hbm_state``
    ...
    db = None
    try:
        from hbm_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, hbm_bin
        cli_args: the original CLI arguments (everything after 'hbm-agent')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    hbm_bin = container_info["hbm_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo hbm {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo hbm {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _create_titled_session(title: str) -> Optional[str]:
    ...
    db = None
    try:
        from hbm_state_ids import new_session_id as mint_session_id
        from hbm_state_registry import acquire
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from hbm_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from hbm_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: hbm-agent --resume {_imported_id})")
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from hbm_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from hbm_cli.config import load_config as _load_config_for_xai_check
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.hbm/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from hbm_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from hbm_cli.config import get_hbm_home
            skills_dir = Path(get_hbm_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import hbm_cli.banner as _banner_mod
...
def cmd_chat(args):
    ...
    from hbm_cli.stream_json import stream_json_requested
    # Structured stdout is a non-interactive protocol: it overrides HBM_TUI/display.interface too.
    ...
    from hbm_cli.free_tier_bootstrap import run_bootstrap
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["HBM_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["HBM_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older hbm_cli.config) crash
...
def cmd_gateway(args):
    ...
    from hbm_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from hbm_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``hbm <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "hbm_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "hbm_cli.auth", "login_command", doc='Authenticate HBM AGENT CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "hbm_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "hbm_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "hbm_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "hbm_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "hbm_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "hbm_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "hbm_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "hbm_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "hbm_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "hbm_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "hbm_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "hbm_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "hbm_cli.backup", "run_import", doc='Restore a HBM AGENT backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "hbm_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "hbm_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "hbm_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "hbm_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "hbm_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "hbm_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "hbm_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "hbm_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from hbm_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from hbm_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from hbm_cli.auth import AuthError, format_auth_error, resolve_provider
    from hbm_cli.config import get_compatible_custom_providers, get_env_value
    from hbm_cli.providers import custom_provider_aliases, resolve_provider_full
...
def select_provider_and_model(args=None):
    ...
    effective_provider = config_provider or os.getenv("HBM_INFERENCE_PROVIDER") or "auto"
    ...
    from hbm_cli.models import _PROVIDER_LABELS
    ...
    # hbm_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.hbm/.env that poisons
...
# ``hbm_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on hbm_cli.main; importing update_cmd eagerly would cost every ``hbm-agent``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "hbm_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_hbm_modules",
    ...
    "hbm_cli.dashboard_procs": (
        "_detect_concurrent_hbm_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_verify(args):
    ...
    from hbm_cli.verify_cmd import run_verify_command
...
def cmd_config(args):
    ...
    from hbm_cli.config import config_command
...
def cmd_backup(args):
    """Back up HBM AGENT home directory to a zip file."""
    from hbm_cli import backup

    (backup.run_quick_backup if getattr(args, "quick", False) else backup.run_backup)(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `hbm-agent --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall HBM AGENT (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from hbm_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from hbm_cli.uninstall import run_gui_uninstall
    ...
    from hbm_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from hbm_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    if is_managed():
        managed_error("update HBM AGENT")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running HBM AGENT runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from hbm_cli.update_inventory import (
    ...
    from hbm_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from hbm_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update HBM AGENT: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from hbm_cli.update_lock import (
    ...
    from hbm_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from hbm_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        ...
        from hbm_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    (HBM_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    HBM_SERVE_HEADLESS itself.
    """
    if os.environ.get("HBM_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("HBM_WEB_DIST", "")):
            os.environ.pop("HBM_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("HBM_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other HBM AGENT surface.
    try:
        from hbm_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        from hbm_cli.main_dep_hints import missing_optional_deps_message
    ...
    try:
        ...
        # #61115, #65696.
        from hbm_cli.config import apply_terminal_config_to_env
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("HBM_DESKTOP") == "1"
    if not mcp_discovery_after_bind:
        try:
            from hbm_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard(args):
    ...
    from hbm_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from hbm_cli import completion
...
def cmd_logs(args):
    """View and filter HBM AGENT log files."""
    from hbm_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe HBM AGENT command console."""
    from hbm_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``hbm-agent`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``hbm-agent photon`` failing
    ...
    not import it, so the CLI registration never happens and ``hbm-agent photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("HBM_TUI") == "1":
        return True
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: HBM_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from hbm_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from hbm_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from hbm_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["HBM_SAFE_MODE"] = "1"
    os.environ["HBM_IGNORE_USER_CONFIG"] = "1"
    os.environ["HBM_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from hbm_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("HBM_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from hbm_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("HBM_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("HBM_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["HBM_DEFER_AGENT_STARTUP"] = "1"
            os.environ["HBM_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["HBM_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `hbm-agent --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``hbm-agent``) — matching is exact. ``HBM_AGENT`` is the
    HBM AGENT-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. HBM AGENT running inside another agent's terminal).
    """
    os.environ.setdefault("AI_AGENT", "hbm-agent")
    os.environ.setdefault("HBM_AGENT", "true")
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from hbm_cli.plugins import discover_plugins, get_plugin_manager
...
def _build_cli_parser():
    """Build the full ``hbm-agent`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``hbm-agent --help`` order; keep it stable. Groups
    live in ``hbm_cli/subcommands/<group>.py`` with handlers injected so
    ...
    from hbm_cli.send_cmd import register_send_subparser
    ...
    from hbm_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from hbm_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    from hbm_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from hbm_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (HBM AGENT-Setup.exe tells users to run it, so it
...
def main():
    ...
    try:
        from hbm_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``hbm.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_hbm_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from hbm_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('hbm_cli.cli_output', 'line_input'),
}
