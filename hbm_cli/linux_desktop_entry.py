"""Install and remove the Linux desktop entry (``hbm.desktop``).
...
DESKTOP_ENTRY_NAME = "hbm.desktop"
...
READY_FD_ENV_VAR = "HBM_DESKTOP_READY_FD"
...
def _can_import_hbm_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``hbm_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``hbm_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``hbm-agent desktop``.

    Prefer the real ``hbm-agent`` launcher; fall back to ``<python> -m hbm_cli.main desktop``.
    """
    from hbm_cli.relaunch import resolve_hbm_bin

    bin_path = _resolve_hbm_bin_for_desktop_entry(resolve_hbm_bin, checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_hbm_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import hbm_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "hbm_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `hbm-agent` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `hbm-agent` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m hbm_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_hbm_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`hbm_cli.relaunch.resolve_hbm_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from hbm_cli.relaunch import resolve_hbm_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/hbm-agent, a venv console script) wins
    ...
    # /opt/.../bin/hbm-agent from another install method, or a venv console script). It must be evaluated
    ...
    # ``hbm-agent``) must NOT return None here: that skipped the durable-wrapper probe below and persisted
    # the module form, so the entry's bytes flipped on every alternating launch context — and
    # gnome-shell 50.x crashes when hbm.desktop changes while its ShellApp is STARTING (#110885).
    ...
    # argv[0] was checkout-internal AND PATH had no `hbm-agent` — common in stripped systemd user
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``hbm-agent``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "hbm-agent")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/hbm-agent"))
    candidates.append(Path.home() / ".local" / "bin" / "hbm-agent")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=HBM AGENT\n"
        "GenericName=HBM AGENT Desktop\n"
        "Comment=Launch HBM AGENT Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=HBM AGENT\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "hbm.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=hbm`` resolves without an
...
def _launcher_entry_management_enabled() -> bool:
    ...
    rewrite: a hand-edited ``hbm.desktop`` is then left alone instead
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "hbm"
...
class DeferredDesktopEntryInstall:
    ...
    The launcher hands Electron the write end of a pipe (``HBM_DESKTOP_READY_FD``); Electron
