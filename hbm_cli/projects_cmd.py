"""``hbm project`` CLI — manage first-class, multi-folder Projects."""
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``hbm project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print("usage: hbm project <action> [options]\nRun 'hbm project --help' for the full list.", file=sys.stderr)
        return 0
...
def _cmd_list(args, conn):
    ...
    if not projs:
        return "No projects yet. Create one with `hbm project create <name>`."
    for p in projs:
...
def _cmd_set_primary(args, conn, proj):
    if not pdb.set_primary(conn, proj.id, args.path):
        return _err(f"'{args.path}' is not a folder of {proj.slug}; add it first with `hbm project add-folder`.")
