...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from hbm_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"hbm {candidate}", f"hbm_cli.main {candidate}", f"hbm_cli/main.py {candidate}")
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (HBM AGENT installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``hbm-serve.service``), or None.
...
def _loaded_launchd_backend_jobs(
    ...
    if sys.platform != "darwin":
        return []
    import plistlib
    from hbm_cli.gateway import _launchd_print_service_pid
...
def _restart_launchd_job(domain: str, label: str, old_pid: int | None, *, timeout: float = 15.0) -> bool:
    ...
    from hbm_cli.gateway import _wait_for_launchd_service_pid
    try:
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    ...
    log_path = get_hbm_home() / "logs" / "dashboard-restart.log"
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # hbm_cli.config.get_hbm_home to simulate setup failure.
        from hbm_cli.config import get_hbm_home as _get_hbm_home
        logs_dir = _get_hbm_home() / "logs"
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from hbm_cli.web_server import should_require_dashboard_auth
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from hbm_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.hbm/
    # desktop-ssh, independent of HBM_HOME and the active profile. Anchor
    # validation there, NOT get_hbm_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_hbm_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".hbm" / "desktop-ssh"
    try:
...
def _route_named_profile_dashboard(
    ...
    (HBM_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("HBM_DESKTOP") == "1"
    ):
        return
    ...
    # HBM_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping HBM_HOME: in the Docker layout the root is /opt/data, and an
    # unset HBM_HOME would fall back to $HOME/.hbm = /opt/data/.hbm — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_hbm_root() strips a trailing profiles/<name> for both layouts.
    try:
        from hbm_constants import get_default_hbm_root
        env["HBM_HOME"] = str(get_default_hbm_root())
    except Exception:
        env.pop("HBM_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets HBM_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless HBM_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed HBM_WEB_DIST can't be populated.
    """
    from hbm_cli.main import PROJECT_ROOT
    from hbm_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["HBM_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "HBM_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["HBM_WEB_DIST"])
            if "HBM_WEB_DIST" in os.environ
            else PROJECT_ROOT / "hbm_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom HBM_WEB_DIST
            ...
            _recoverable = "HBM_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # HBM_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # HBM_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["HBM_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ HBM_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset HBM_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads HBM_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["HBM_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from HBM_WEB_DIST: {_dist_root}")
