...
from hbm_cli._secrets_common import (
...
from hbm_cli.config import get_env_path, load_config, save_config, save_env_value
...
def __getattr__(name: str):
    ...
    Existing callers (and upstream tests) monkeypatch attributes on ``hbm_cli.secrets_cli.bw`` directly.
    ...
    so ``import hbm_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
# ── Argparse wiring — called from hbm_cli.main ──
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        console.print("\n  This was a dry-run — secrets are picked up automatically on the "
                      "next [cyan]hbm[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
