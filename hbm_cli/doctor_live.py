...
from hbm_cli.doctor import _section, check_info
from hbm_cli.doctor_report import check_fail, check_ok, check_warn
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from hbm_cli.doctor import HBM_HOME, PROJECT_ROOT
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (HBM_HOME / "node" / "bin", HBM_HOME / "node", HBM_HOME / "node_modules" / ".bin"):
    ...
    # probes above. Mirror the rung hbm_cli.doctor uses so this probe can't diverge from it, including
