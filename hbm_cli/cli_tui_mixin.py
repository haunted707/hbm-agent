...
interactive CLI. Mixin bound onto ``HbmCLI`` via the MRO; cli.py-internal symbols are imported
...
class CLITuiMixin:
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from hbm_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from hbm_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def _render_scroll_list_panel(self, state, title, hint, labels, *, min_width, max_width, indent):
        ...
        """
        from cli import HbmCLI, _panel_box_width, _wrap_panel_text
        ...
        scroll_offset, visible = HbmCLI._compute_model_picker_viewport(
    ...
    def _get_model_picker_display_fragments(self):
        ...
        if state.get("stage", "provider") == "provider":
            ...
            from hbm_cli.cli_model_switch_mixin import _picker_reasoning_rows
        ...
        if code := self._sudo_state.get("vault_code"):
            return self._render_sudo_style_panel(
                ...
                [f'{code["site"]} is asking for a one-time code (text message, email or authenticator app).',
                 'Type the code and press Enter; HBM AGENT enters it into the page for you.',
    ...
    def _tui_handle_ignored_terminal_sequence(self, event):
        ...
        hbm_cli.pt_input_extras registers focus reports (CSI I / CSI O) as Keys.Ignore at the
    ...
    def _tui_handle_ctrl_z(self, event):
        ...
        agent_name = get_active_skin().get_branding("agent_name", "HBM AGENT")
    ...
    def _tui_enter_while_busy(self, text: str, images: list, payload) -> None:
        ...
        """
        from cli import CLI_CONFIG, _ACCENT, _DIM, _RST, _cprint, _hbm_home
        ...
        if _effective_mode == "queue":
            ...
            if redirected:
                ...
                try:
                    with open(_hbm_home / "interrupt_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(
        ...
        try:
            ...
            if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                ...
                mark_seen(_hbm_home / "config.yaml", BUSY_INPUT_FLAG)
    ...
    def _tui_enter_overlay(self, event) -> bool:
        ...
        if self._model_picker_state:
            try:
                ...
                from hbm_cli.model_switch import resolve_persist_behavior
    ...
    def _tui_collapse_paste(self, text: str, line_count: int, *, fallback: bool) -> str:
        """Save a large paste under ~/.hbm/pastes and return the placeholder for the buffer."""
        from cli import _hbm_home, datetime, logger
        self._tui_paste_counter += 1
        paste_dir = _hbm_home / "pastes"
    ...
    def _tui_handle_prompt_stash(self, event):
        ...
        """
        from hbm_cli.prompt_stash import ACTION_RESTORED, ACTION_STASHED, resolve_ctrl_s
        buf = event.app.current_buffer
    ...
    def _tui_build_key_bindings(self):
        ...
        from hbm_cli.cli_subagent_monitor import modal_prompt_active, open_monitor, toggle_dock
        for key in ('c-t', 'f6'):
    ...
    def _tui_build_layout(self, kb):
        ...
        cli_ref = self
        from hbm_cli.cli_subagent_monitor import install_dock
        install_dock(self)
