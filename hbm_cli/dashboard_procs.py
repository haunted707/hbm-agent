"""Dashboard process-hygiene helpers — extracted from ``hbm_cli/main.py``.

Helpers defined in ``hbm_cli.main_dashboard`` / ``hbm_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("hbm-agent", "hbm_cli.main", "hbm_cli/main.py"))
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "hbm.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the HBM AGENT
    ...
    try:
        ...
        return []
    # Spawn-ledger augmentation: substring patterns miss profiled launches (`hbm-agent --profile p
    ...
    with contextlib.suppress(Exception):
        ...
        from hbm_cli.process_identity import ledger_entries
...
def _hbm_home_for_pid(pid: int) -> str | None:
    """Best-effort ``HBM_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("HBM_HOME"):
            return home
    try:
...
def _profile_key_for_respawn(argv: list[str], hbm_home: str | None = None) -> str:
    """Stable owner key: ``HBM_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if hbm_home:
        parts = _resolved_home(hbm_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(hbm_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, hbm_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``HBM_HOME`` (the argv-only respawn would come back on this install's home
    ...
    for _pid, argv, hbm_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and hbm_home and _normalized_home_for_compare(hbm_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, hbm_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``HBM_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("HBM_DESKTOP_CHILD_PID", "").split(","):
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_hbm(pid, expected_start_time=expected_start_time):
                failed.append((pid, "not hbm-owned or process identity changed"))
            else:
...
# hbm_cli/web_server.py::_lifespan: stop_hosted_room_service(timeout=5.0) + the startup-thread
...
# and keep the deleted state.db-wal inode open — the next hbm-agent start refuses with a FATAL
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    ...
    """
    from hbm_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. hbm-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "hbm-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if launchd_jobs and (job := _launchd_owner(pid, cmdline)):
                ...
                # Manual process: exact argv + HBM_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot HBM_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _hbm_home_for_pid(pid)
...
def _detect_concurrent_hbm_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``hbm.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from hbm_cli.main_install_repair import _hbm_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _hbm_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _hbm_home_dir() -> Path:
    """The process's HBM AGENT home: remote-backend locks are a process-level asset, so a request scoped
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "hbmPath", "hbmHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated HBM_HOME can't reject a legitimate backend.
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{hbm_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
    (best-effort: a bad record contributes no PID; never raises)."""
    import json
    root = base_dir if base_dir is not None else _hbm_home_dir() / _REMOTE_LOCK_SUBDIR
    owned: set[int] = set()
    try:
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local hbm-agent serve", signal_term=None, signal_kill=None,
    ...
    0/1; not self / parent / HBM_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after HBM_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
