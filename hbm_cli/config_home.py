"""Directory initialization and storage diagnostics for the active HBM AGENT home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool, home: Path) -> None:
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize HBM AGENT directory {path}: {exc}. "
            ...
            "HBM AGENT has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from hbm_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize HBM AGENT home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from hbm_cli.config import ConfigIssue
