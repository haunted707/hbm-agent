"""Standalone venv-process scan for JSON consumption (``python -m hbm_cli._scan_venv_blockers``).
...
def _updater_owned_backend_entry(pid: int, cmdline: str) -> dict | None:
    ...
    """
    try:
        from hbm_cli.update_cmd import _hbm_holder_subcommand  # noqa: PLC0415

        purpose = _hbm_holder_subcommand(cmdline)
    except Exception:
        return None
    if purpose not in _UPDATER_STOPPABLE_PURPOSES:
        return None
    try:
        from hbm_cli.process_identity import ledger_entries, spawner_is_dead  # noqa: PLC0415
