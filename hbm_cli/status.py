...
from hbm_cli.config import get_env_path, get_env_value, get_hbm_home, load_config
from hbm_cli.models import provider_label
from hbm_cli.runtime_provider import resolve_requested_provider
from hbm_cli.vercel_auth import describe_vercel_auth
from hbm_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from hbm_constants import OPENROUTER_MODELS_URL
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'hbm-agent[vercel]')")
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from hbm_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from hbm_cli.gateway_multiplex_served import served_profile_ingress_urls
...
def _render_cron(ctx):
    ...
    jobs_file = get_hbm_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from hbm_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_hbm_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from hbm_cli.active_sessions import (
...
def show_status(args):
    """Show status of all HBM AGENT components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('hbm_cli.nous_account', 'get_nous_portal_account_info'),
