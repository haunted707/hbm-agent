"""Interactive setup wizard for HBM AGENT (config lives in ~/.hbm/).
...
``hbm_cli.setup.<name>`` keep working.
"""
...
from hbm_cli.cli_output import print_error, print_info, print_success, print_warning
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("☤ HBM AGENT Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("hbm_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from hbm_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``HBM_NONINTERACTIVE=1`` (``hbm_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("HBM_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``HBM_NONINTERACTIVE=1`` or a
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from hbm_cli.main import select_provider_and_model
    try:
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into HBM_MAX_ITERATIONS);
    # a stale .env entry silently shadowing it caused the 60-vs-500 bug, so drop it.
    remove_env_value("HBM_MAX_ITERATIONS")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.hbm/logs/tool_calls.log (gateway only)",
)
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives HBM_MAX_ITERATIONS from agent.max_turns.
...
# hbm_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from hbm_cli.setup_tts import setup_tts  # noqa: E402
from hbm_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from hbm_cli.setup_platforms import setup_gateway  # noqa: E402
from hbm_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from hbm_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from hbm_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ☤ HBM AGENT Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, hbm_home, *, is_existing: bool, migration_ran: bool) -> None:
...
def _run_setup_wizard_impl(args):
    ...
    from hbm_cli.auth import get_active_provider
    is_existing = bool(get_env_value("OPENROUTER_API_KEY") or get_env_value("OPENAI_BASE_URL")
                       or get_active_provider() is not None)
    _print_banner("│             ☤ HBM AGENT Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your HBM AGENT installation.       │",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, hbm_home))])
            return
        ...
        migration_ran = _offer_openclaw_migration(hbm_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up HBM AGENT?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from hbm_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, hbm_home, is_existing))])
            return
    _run_full_setup(config, hbm_home, is_existing=is_existing, migration_ran=migration_ran)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('hbm_cli.nous_subscription', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('hbm_constants', 'get_optional_skills_dir'),
