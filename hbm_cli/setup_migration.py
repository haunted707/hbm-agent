...
def _offer_openclaw_migration(hbm_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info("Skipping migration. You can run it later with: hbm claw migrate --dry-run")
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        _info("Migration cancelled. You can run it later with: hbm claw migrate",
    ...
    for key, printer, text in (
        ...
        ("conflict", print_info,
         "Skipped {n} item(s) that already exist in HBM AGENT (use hbm claw migrate --overwrite to force)."),
