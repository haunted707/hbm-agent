...
def _blank_slate_done(config: dict, hbm_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from hbm_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    try:
        from hbm_cli.model_setup_flows import _model_flow_nous
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and HBM AGENT.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every HBM AGENT update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``hbm-agent`` skill
    ...
    try:
        ...
        from hbm_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "hbm-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("hbm-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _run_blank_slate_setup(config: dict, hbm_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `hbm-agent` skill is always kept so the agent",
          "can help you drive and configure HBM AGENT itself.", None)
...
def _blank_slate_walkthrough(config: dict, hbm_home):
...
def _run_quick_setup(config: dict, hbm_home):
    ...
    from hbm_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
