...
Wraps the same config layer the CLI uses (hbm_cli.mcp_config); stdio ``env``
...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from hbm_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.hbm_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import HbmTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_hbm_home_override(flow.hbm_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.hbm_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = HbmTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, hbm_home=flow.hbm_home)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, hbm_home=flow.hbm_home)
                    raise
        finally:
