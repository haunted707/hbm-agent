...
from hbm_cli.update_cmd_common import _best_effort
from hbm_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("hbm_cli.update_cmd")

# Under HBM_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "hbm-gateway*", "hbm-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from hbm_cli.update_cmd import get_hbm_home
    path = get_hbm_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """HBM_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from hbm_cli.update_cmd import get_hbm_home
    return get_hbm_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    from hbm_cli.update_cmd import _capture_head_sha, _m
    try:
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from hbm_cli.update_cmd import _current_checkout_sha
    try:
        from hbm_cli.update_receipt import read_latest_receipt
...
def _live_fleet_covers_receipt(expected_sha: str | None) -> bool:
    ...
    if not expected_sha:
        return False
    from hbm_cli.update_receipt import collect_fleet_versions
...
def _restart_systemd_gateway_units_best_effort(failed: list, listings) -> None:
    """Best-effort ``systemctl restart`` of every hbm-gateway/serve unit."""
    answered = set()
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        ...
        # --- Systemd services (Linux) --- Discover all hbm-gateway* units (default + profiles) plus
        # hbm-serve* units (the Desktop app's backend, #83438).
        if systemd_listings is not None:
            _restart_systemd_gateway_units_best_effort(failed, systemd_listings)
        # --- Launchd services (macOS) --- Restart EVERY ai.hbm.gateway* LaunchAgent, not only the
...
def _defer_fleet_restart_after_update(*, update_complete: bool, resume_incomplete: bool = False) -> None:
    ...
    with suppress(Exception):
        from hbm_cli.update_receipt import finalize_update_receipt
...
def _apply_pending_fleet_restart_catchup(*, defer: bool = False) -> None:
    ...
    """
    from hbm_cli.update_cmd import _run_pending_fleet_restart
...
def _is_hbm_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("hbm-serve")``
    would accept ``hbm-server.service``."""
    return (
        ...
        unit == "hbm-gateway.service"
        or unit.startswith("hbm-gateway-")
        or unit == "hbm-serve.service"
        or unit.startswith("hbm-serve-")
    )


def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each hbm-gateway*/hbm-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_hbm_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``hbm-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``hbm-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``hbm-gatewayd`` is never signalled.

    See #83438.
    """
    return svc_name == "hbm-gateway" or svc_name.startswith("hbm-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from hbm_cli.gateway import is_macos
    if not failed_units:
        return
    ...
    if any(name.startswith("ai.hbm.") for name in ordered):
...
def _restart_macos_launchd_gateways(
    ...
    The pull is shared across profiles, so every ``ai.hbm.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``hbm_cli.gateway`` no longer importing
    ...
    """
    try:
        from hbm_cli.gateway import find_gateway_pids
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither HBM_SPAWN
# nor HBM_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a hbm-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``hbm_cli.gateway`` in the
    ...
    child's ``hbm-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``hbm_cli.gateway`` from the freshly
...
def _gateway_home_for_pid(pid: int):
    """HBM_HOME of the gateway ``pid`` per the fleet inventory, else None (own profile's file)."""
    with suppress(Exception):
        from hbm_cli.update_receipt import _profile_homes
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to hbm-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. hbm-serve has no handler → blunt restart below.
    _main_pid = 0
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active hbm-gateway*/hbm-serve* systemd unit (user + system).
    ...
    """
    from hbm_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
...
class _GatewayRestartOutcome:
    ...
    def record_receipt(self, **extra) -> None:
        ...
        with suppress(Exception):
            from hbm_cli.update_receipt import record_gateway_restart
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from hbm_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from hbm_cli.update_abort_recovery import _owed_stale_serve_rows
    from hbm_cli.update_cmd import (
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from hbm_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/hbm-serve`` vs ``system/hbm-serve`` are different
    ...
    # Purge stale cached HBM AGENT modules FIRST: the import below loads new gateway
    ...
    try:
        ...
        from hbm_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from hbm_cli.gateway import _get_restart_exit_wait_budget
        ...
        # macOS: EVERY ai.hbm.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy hbm.service fights hbm-gateway.service over the bot token; warn on
    every update until migrated."""
    from hbm_cli.gateway import (has_legacy_hbm_units, _find_legacy_hbm_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_hbm_units()):
        return
    print()
    print("⚠ Legacy HBM AGENT gateway unit(s) detected:")
    for name, path, is_sys in _find_legacy_hbm_units():
        scope = "system" if is_sys else "user"
        print(f"    {path}  ({scope} scope)")
    print()
    print("  These pre-rename units (hbm.service) fight the current")
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from hbm_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from hbm_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import hbm_cli.update_receipt as _ur
    ...
    with _best_effort('Multiplex auto-migration after update failed: %s'):
        from hbm_cli.gateway_migrate import maybe_auto_migrate_after_update
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``hbm_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
