...
_ZIP_STAGING_ARTIFACT_SUFFIXES = ".hbm-update-staging", ".hbm-update-old"
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``hbm_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.hbm-update-staging"
    backup = f"{dst}.hbm-update-old"
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.hbm-update-staging`` siblings that git
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``hbm-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"hbm-agent-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, hbm_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ HBM AGENT.exe) exists only in the LIVE tree. Graft the live release
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="hbm-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"hbm-agent-{branch}.zip")
