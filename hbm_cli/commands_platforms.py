...
from hbm_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _iter_plugin_command_entries, _resolve_config_gates)

# Logger name parity with the origin module (tests capture "hbm_cli.commands").
logger = logging.getLogger("hbm_cli.commands")
...
# Canonical commands deliberately routed through ``/hbm <command>`` on Slack only: the registry
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_HBM_ONLY are skipped. ``/hbm`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://hbm-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/hbm`` handler, plugin commands included."""
