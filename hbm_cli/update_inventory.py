...
@dataclass
class RuntimeRecord:
    """One running (or expected) HBM AGENT runtime on this machine."""
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from hbm_cli.gateway import is_macos, supports_systemd_services
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from hbm_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
        ...
        with _probe("Image provenance probe"):
            # See #91277.
            from hbm_cli.image_provenance import read_image_provenance
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from hbm_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from hbm_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from hbm_cli.update_receipt import _socket_identity
    ...
    with _probe("PID-file gateway inventory"):
        from hbm_cli.gateway import find_profile_gateway_processes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running HBM AGENT services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``hbm-serve*``/``hbm-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``hbm-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``hbm-serve-workbench`` — and a scope prefix
    (``user/hbm-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"hbm-serve{suffix}", f"hbm-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``hbm-gateway-foobar.service``.
    Launchd labels are ``ai.hbm.gateway`` / ``ai.hbm.gateway-<profile>`` — they do
    not contain the substring ``hbm-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/hbm-gateway``,
    ``gui/501/ai.hbm.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"hbm-gateway", "ai.hbm.gateway", "gateway", "gateway-default"}
    return name in {f"hbm-gateway-{profile}", f"ai.hbm.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``hbm-gateway*`` name a
...
def record_plan_in_receipt(plan: UpdatePlan) -> None:
    ...
    try:
        import hbm_cli.update_receipt as ur
