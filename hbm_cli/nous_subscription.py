...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "hbm-cli"}
...
def _has_agent_browser() -> bool:
    ...
    from hbm_constants import agent_browser_runnable
    ...
    try:
        ...
        # HBM AGENT-managed Node dirs ($HBM_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_hbm_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("HBM_LOCAL_STT_COMMAND"):
        return True
    try:
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from hbm_cli.setup import prompt_checklist
    except Exception:
        return set()
