...
_UPDATE_RUNTIME_RELOAD_MODULES = "hbm_constants", "tools.environments.local", "tools.lazy_deps"
...
#: ``_reload_updated_runtime_modules`` instead: ``hbm_logging`` (a fresh copy starts a SECOND
#: QueueListener over the same log files while the first keeps running) and ``hbm_constants``
#: (its ``_HBM_HOME_OVERRIDE`` ContextVar — a token taken through the old module cannot reset a
#: fresh module's var, and an override set before the purge would silently vanish).
_STALE_PURGE_PROTECTED = frozenset({"hbm_cli", "hbm_cli.main", "hbm_logging", "hbm_constants"})
...
_STALE_PURGE_PROTECTED_PREFIX = "hbm_cli.update_"
...
def _purge_stale_hbm_modules() -> None:
    """Evict every cached HBM AGENT module after the checkout changed in-place. Never raises.
    ...
    """
    from hbm_cli.update_cmd import _m
    with _best_effort('Could not purge stale HBM AGENT modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale HBM AGENT module(s) after checkout update", len(purged))
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from hbm_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("hbm_cli._subprocess_compat", "hbm_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from hbm_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from hbm_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("HBM_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== hbm-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from hbm_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next HBM AGENT runtime."""
    from hbm_cli.update_cmd import _post_update_sqlite_runtime_status
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from hbm_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `hbm-agent desktop` to retry the desktop rebuild.")
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from hbm_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from hbm_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        print(
            ...
            "handles (or restart HBM AGENT) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from hbm_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from hbm_cli.backup import _sibling_profile_homes
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v hbm-agent'`` resolves."""
    from hbm_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/hbm-agent).
    fhs_link = Path("/usr/local/bin/hbm-agent")
...
def _ensure_acp_launcher() -> None:
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        hbm_cmd = bin_dir / "hbm-agent"
        acp_cmd = bin_dir / "hbm-agent-acp"
        try:
            if not (hbm_cmd.is_file() or hbm_cmd.is_symlink()):
                continue
            ...
            continue
        print(f"  ✓ Installed hbm-agent-acp launcher → {acp_cmd}")
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_hbm_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_hbm_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from hbm_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from hbm_cli.backup import create_pre_update_snapshots_all_profiles
        ...
        if _sibling_snaps:
            ...
            import hbm_cli.update_cmd_config as _cfg
...
def _run_full_backup() -> None:
    ...
    # display_hbm_home so the user sees ~/.hbm/...
    try:
        from hbm_constants import get_hbm_home, display_hbm_home
        display_path = f"{display_hbm_home()}/{out_path.relative_to(get_hbm_home())}"
    except Exception:
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in hbm_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # HBM_HOME, so sync_skills()'s module-level HBM_HOME cache can't skew it.
    with suppress(Exception):
        from hbm_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from hbm_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from hbm_cli.tools_config import install_cua_driver
        print()
...
def _print_plugin_compat_notice() -> None:
    ...
    from hbm_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from hbm_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from hbm_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('hbm-agent-acp launcher self-heal failed: %s', _ensure_acp_launcher),
        ...
        # Legacy HBM_NEMO_RELAY_ATIF_*/ATOF_* vars produce no traces since the Relay cutover;
...
def _migrate_relay_exporter_env() -> None:
    from hbm_cli.relay_plugin_migrate import run_relay_migration_after_update
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from hbm_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    try:
        # See #95596.
        from hbm_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from hbm_cli.model_catalog import seed_cache_from_checkout
