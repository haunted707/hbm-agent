"""HBM AGENT update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``hbm_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``hbm_cli.main`` at call time.
"""
...
from hbm_cli.config import get_hbm_home  # noqa: F401  (re-exported; patched via update_cmd)
from hbm_cli.update_cmd_common import _best_effort
from hbm_constants import get_default_hbm_root, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from hbm_cli.update_abort_recovery import (  # noqa: F401
...
from hbm_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _hbm_holder_subcommand, _holder_value_flags,
...
from hbm_cli.update_cmd_fleet import (  # noqa: F401
...
from hbm_cli.update_cmd_zip import (  # noqa: F401
...
from hbm_cli.update_cmd_stash import (  # noqa: F401
...
from hbm_cli.update_cmd_config import (  # noqa: F401
...
from hbm_cli.update_cmd_deps import (  # noqa: F401
...
from hbm_cli.update_cmd_git import (  # noqa: F401
...
from hbm_cli.update_cmd_maint import (  # noqa: F401
    ...
    _print_verified_update_completion, _purge_stale_hbm_modules, _read_project_version,
...
def _m():
    """Lazy ``hbm_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from hbm_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "hbm_cli/main.py", "hbm_cli/config.py", "hbm_cli/__init__.py",
    "hbm_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "hbm_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from hbm_cli.update_receipt import record_step
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="hbm-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from hbm_constants import get_hbm_home  # noqa: F811  (deliberate: constants variant)
    home = get_hbm_home()
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live HBM AGENT launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["hbm-agent.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically HBM AGENT")
    print("  Desktop, a gateway, or another hbm REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `hbm-agent` command to finish it automatically.")
...
def _invalidate_update_cache():
    ...
    default_home = get_default_hbm_root()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-hbm.exe gate."""
    shim = scripts_dir / "hbm-agent.exe"
    lines = [
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from hbm_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _log_only_write(text: str) -> None:
    ...
    with suppress(Exception):
        if log_file is None:
            log_path = get_hbm_home() / "logs" / "update.log"
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from hbm_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from hbm_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    from hbm_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        from hbm_cli.gitlock import repair_broken_shallow_boundaries, prune_stale_shallow_grafts
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from hbm_cli.banner import _github_compare_behind
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    from hbm_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by hbm.exe...")
    elif not healthy:
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running HBM AGENT gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/hbm-update-backups/orphan-{branch}-"
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway hbm.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from hbm_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running HBM AGENT runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from hbm_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another hbm.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_hbm_instances(scripts_dir) if scripts_dir is not None else []
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `hbm-agent
