"""Shell completion script generation for hbm CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_hbm_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
...
_hbm_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_hbm_profiles)" -- "$cur"))
        return
    fi
...
complete -F _hbm_completion hbm-agent
"""
...
_hbm_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.hbm/profiles" ]]; then
        profiles+=($HOME/.hbm/profiles/*(N/:t))
    fi
...
_hbm() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_hbm_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'hbm-agent command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.hbm/profiles",
        "        for d in $HOME/.hbm/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
