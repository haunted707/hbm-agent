"""Backup and import commands for hbm CLI."""
...
from hbm_constants import (
    LOCAL_RUNTIME_ROOT_DIRS, _get_platform_default_hbm_home, get_default_hbm_root, get_hbm_home,
    display_hbm_home,
)
from hbm_state_dbfile import RETIRED_GENERATION_DIR_SUFFIX
from utils import (
...
def _snapshot_recovery_hint() -> str:
    """How to restore a state snapshot. There is no `hbm-agent snapshot` subcommand — only the /snapshot
    slash command inside a `hbm-agent` session (hbm_cli/commands.py)."""
    return ("To restore a newer snapshot, start `hbm-agent` in a terminal and run `/snapshot list`, then "
            "`/snapshot restore <id>` (CLI only).")

# Directory names to skip (matched against each path component). ``hbm-agent`` only matches at
# the root (``_should_exclude``) so skill dirs like ``skills/.../hbm-agent/`` survive. The
# dependency/cache entries matter: one plugin venv or pip/uv cache under HBM_HOME walked
...
_EXCLUDED_DIRS = {
    "hbm-agent",     # the codebase repo — re-clone instead
...
# HBM AGENT-managed runtime downloads (see ``LOCAL_RUNTIME_ROOT_DIRS``). Matched ONLY at the root of
# HBM_HOME and at ``profiles/<name>/`` — a deeper dir of the same name (a skill's ``models/``)
# is user data.
...
# Reserved archive subtree for memory-provider state OUTSIDE HBM_HOME (e.g. ~/.honcho, via
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the HBM AGENT backup slot."""
...
@contextmanager
def _backup_operation_lock(hbm_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = hbm_home / ".backup.lock"
        ...
        while not acquired:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise BackupInProgressError("another HBM AGENT backup is already running")
                time.sleep(0.05)
        yield
    finally:
        if acquired:
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to hbm-agent root) should be skipped."""
    parts = rel_path.parts
    if _in_excluded_root_dir(rel_path):
        return True
    # ``hbm-agent`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "hbm-agent" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(hbm_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    for dirpath, dirnames, filenames in os.walk(hbm_root, followlinks=False):
        ...
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "hbm-agent" and not is_root))
        ...
        for fname in filenames:
            ...
            # copy data from outside HBM_HOME; never archive the output zip into itself.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart HBM AGENT) and retry.", dst, exc2)
        return False
...
def run_backup(args) -> None:
    """Create a zip backup of the HBM AGENT home directory."""
    hbm_root = get_default_hbm_root()

    if not hbm_root.is_dir():
        print(f"Error: HBM AGENT home directory not found at {hbm_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(hbm_root):
            _run_backup_locked(args, hbm_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, hbm_root: Path) -> None:
    ...
    print(f"Scanning {display_hbm_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(hbm_root, out_path, skipped_dirs))
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_hbm_home()}.")
    if skipped_external:
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a HBM AGENT backup."""
    names = zf.namelist()
    if not names:
        return False, "zip archive is empty"
    # Telltale files a hbm home has — at the root or one level deep (zipped directory).
    if not any(Path(n).name in {"config.yaml", ".env", "state.db"} for n in names):
        return False, "zip does not appear to be a HBM AGENT backup (no config.yaml, .env, or state databases found)"
    return True, ""
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".hbm", "hbm"}:
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    ``None`` means "unknown" (missing, not a HBM AGENT session store, unreadable) — never "zero":
...
def _confirm_import_overwrite(hbm_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((hbm_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has HBM AGENT configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, hbm_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under HBM_HOME; provider configs commonly hold credentials, so tighten to 0600.
...
def run_import(args) -> None:
    """Restore a HBM AGENT backup from a zip file."""
    ...
    # ``get_default_hbm_root()`` would silently retarget a profile restore at the live root.
    hbm_root = get_hbm_home()
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Backup contains {file_count} files\nTarget: {display_hbm_home()}")
        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")
        if not args.force and not _confirm_import_overwrite(hbm_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, hbm_root, file_count)
        elapsed = time.monotonic() - t0
        ...
        if restored_external:
            print(f"\n  Restored {restored_external} memory-provider file(s) to "
                  f"their original location(s) outside {display_hbm_home()}.")
        if errors:
            _print_capped(f"\n  Warnings ({len(errors)} files skipped):", errors, "  ")
        if db_shrunk:
        ...
        restored_profiles = _restore_profile_wrappers(hbm_root)
        print()
...
def _revive_gateway_after_import(hbm_root: Path) -> None:
    ...
    """
    native_default = _get_platform_default_hbm_home()
    if hbm_root != native_default and any(
...
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside HBM_HOME.
...
def _quick_snapshot_root(hbm_home: Optional[Path] = None) -> Path:
    home = hbm_home or get_hbm_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, hbm_home: Optional[Path] = None, keep: Optional[int] = None,
...
def list_quick_snapshots(limit: int = 20, hbm_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(hbm_home)):
...
def restore_quick_snapshot(snapshot_id: str, hbm_home: Optional[Path] = None) -> bool:
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_hbm_home()
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", hbm_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, hbm_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, hbm_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, hbm_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(hbm_home)), keep, shutil.rmtree, "snapshot")
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_hbm_home()}/state-snapshots/\n"
...
def _write_full_zip_backup(out_path: Path, hbm_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``hbm_root`` to ``out_path`` under the backup slot (same rules as
    :func:`run_backup`); None when nothing to back up, another backup running, or write error."""
    try:
        with _backup_operation_lock(hbm_root):
            return _write_full_zip_backup_locked(out_path, hbm_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, hbm_root: Path) -> Optional[Path]:
    scan_started = time.monotonic()
    logger.info("automatic backup phase=scan status=started")
    try:
        files_to_add = list(_iter_backup_files(hbm_root, out_path))
    except OSError as exc:
...
def _create_prefixed_full_backup(
    hbm_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<HBM_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    hbm_root = hbm_home or get_default_hbm_root()
    if not hbm_root.is_dir():
        return None
    backup_dir = hbm_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, hbm_root) is None:
        return None
