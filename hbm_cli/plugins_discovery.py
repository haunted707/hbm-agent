...
Split out of :mod:`hbm_cli.plugins`. Names that tests patch on the origin
...
ENTRY_POINTS_GROUP = "hbm_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "hbm_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``hbm_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from hbm_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    user_dir = get_hbm_home() / "plugins"
    logger.debug("Scanning user plugins: %s", user_dir)
    _scan("user", user_dir, "user")
    if _origin._env_enabled("HBM_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".hbm" / "plugins"
        logger.debug("Scanning project plugins: %s", project_dir)
        _scan("project", project_dir, "project")
    else:
        logger.debug("Project plugins disabled (set HBM_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        error = (
            "removed — Relay lifecycle is owned by HBM AGENT core; configure "
            f"{RELAY_PLUGINS_CONFIG_ENV} instead"
        )
        return _placeholder(error, logging.WARNING, "Refusing to load removed HBM AGENT Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `hbm-agent`
