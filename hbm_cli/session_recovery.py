"""Offline, non-destructive recovery for a damaged HBM AGENT session database.
...
from hbm_state_common import FTS_STORAGE_VERSION, SCHEMA_VERSION
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    guard keeps this path consistent with ``hbm_state_repair._backup_db_file``.
    ...
    ``hbm_cli.sqlite_safe_read`` exists to prevent (see #71724). Holding the lock means no connection can
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="hbm-session-recovery-", dir=str(work_root))
    try:
        snapshot_source, copied = _copy_source_bundle(source, Path(temp_dir.name))
        if _source_fingerprint(source) != before:
            raise SessionRecoverySafetyError(
                "The source database bundle changed while it was being copied. Stop every HBM AGENT process using this "
                "profile and retry. This includes the interactive `hbm-agent` CLI session this command may have been "
                "launched from: a running parent CLI writes session bookkeeping (compression ticks, context "
                "tracking) to state.db in the background and counts as a HBM AGENT process even after the gateway is "
                "stopped. Run the recovery from a fresh shell with no `hbm-agent` session open, or point --source at an "
...
def _salvage_rowid_bounds(source: sqlite3.Connection, table: str) -> dict[str, Any]:
    ...
    # aggregate lets the planner answer from any covering index (every HBM AGENT table has at
...
def _lost_and_found_plausibility_errors(
    ...
    """
    from hbm_cli.session_lost_and_found import _EPOCH_LOW, STUB_TITLE_PREFIX
...
def _recover_via_lost_and_found(
    *, source: Path, snapshot_source: Path, snapshot_dir: Path, output: Path, inspection: dict[str, Any],
    ...
    from hbm_cli.session_lost_and_found import (
