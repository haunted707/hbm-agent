...
def _anthropic_oauth_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: HBM AGENT-managed PKCE file first, then the
    ...
    """
    try:
        from agent.anthropic_credentials import read_hbm_oauth_credentials, _get_hbm_oauth_file
        hbm_creds = read_hbm_oauth_credentials()
    except Exception:
        hbm_creds = None
    if hbm_creds and hbm_creds.get("accessToken"):
        return _token_status("hbm_pkce", f"HBM AGENT PKCE ({_get_hbm_oauth_file()})", hbm_creds)
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain HBM AGENT can't read, so the unverified
    state reads "managed by the Copilot CLI" — never signed out."""
    try:
        from hbm_cli.auth import get_external_process_provider_status
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``HBM_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
        from hbm_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "nous", "name": "Nous Portal", "flow": "device_code", "cli_command": "hbm-agent auth add nous",
     "docs_url": "https://portal.nousresearch.com", "status_fn": None},
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "hbm-agent auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "hbm-agent auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "hbm-agent auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    {"id": "xai-oauth", "name": "xAI Grok OAuth (SuperGrok / Premium+)", "flow": "device_code",
     "cli_command": "hbm-agent auth add xai-oauth",
    ...
    # Login works via the terminal (`hbm-agent auth add anthropic`) or a plain API key.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external", "cli_command": "hbm-agent auth add anthropic",
...
def _nous_plain_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    """
    from hbm_cli.web_server_profiles import _profile_scope
    from hbm_cli.auth import _poll_for_token, persist_nous_credentials, refresh_nous_oauth_from_state
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    via ``_minimax_save_auth_state`` so the system ends up as after ``hbm-agent auth add minimax-oauth``.
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `hbm-agent auth add xai-oauth`: first credential may become active; never overwrite.
