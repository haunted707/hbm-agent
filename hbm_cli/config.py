...
from hbm_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from hbm_cli.secret_prompt import masked_secret_prompt
# Re-export from hbm_constants — canonical definition lives there.
from hbm_constants import get_hbm_home, get_process_hbm_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    "last-known-good": "HBM AGENT is running on the settings it loaded before the edit until it is fixed, so recent changes are not applied.",
    "last-known-good-backup": "HBM AGENT is running on your last good settings until it is fixed, so recent changes are not applied.",
...
def format_config_parse_failure(config_path: Path, exc: Exception, *, fallback: str = "defaults") -> str:
    """User copy for an unparseable config.yaml: what happened, what HBM AGENT is doing, how to fix.
...
def _warn_config_parse_failure(
    ...
    from hbm_cli.config_backups import backup_config
...
# interpreter init (PYTHON*, NODE_*: HBM AGENT restarts through them), PATH (fix tool lookup
...
# and HBM AGENT runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``HBM_*`` overall is NOT blocked — many integration credentials use that prefix
# (HBM_LANGFUSE_PUBLIC_KEY, HBM_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # HBM AGENT runtime location
    "HBM_HOME", "HBM_PROFILE", "HBM_CONFIG", "HBM_ENV",
    "HBM_CONFIG_PATH", "HBM_ENV_PATH",
    ...
    "HBM_COPILOT_ACP_COMMAND", "HBM_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "HBM_YOLO_MODE", "HBM_ACCEPT_HOOKS", "HBM_REDACT_SECRETS",
    "HBM_INTERACTIVE", "HBM_EXEC_ASK", "HBM_GATEWAY_SESSION",
    "HBM_CRON_SESSION", "HBM_SINGLE_QUERY_SESSION",
    "HBM_SESSION_KEY", "HBM_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or HBM AGENT runtime location and security policy (HBM_HOME, HBM_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.hbm/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean HBM_TOOL_PROGRESS variant is unsupported (its only
    ...
    "HBM_LANGFUSE_ENV", "HBM_LANGFUSE_RELEASE", "HBM_LANGFUSE_SAMPLE_RATE",
    "HBM_LANGFUSE_MAX_CHARS", "HBM_LANGFUSE_CAPTURE", "HBM_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "HBM_ACP_AUTH_METHOD", "HBM_ACP_AUTO_APPROVE", "HBM_COPILOT_ACP_COMMAND",
    "HBM_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# HBM_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: HBM_MANAGED env var (systemd service) or a ``.managed`` marker file in
    HBM_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("HBM_MANAGED", "").strip().lower() or None
    managed_marker = get_hbm_home() / ".managed"
...
def is_managed() -> bool:
    """Check if HBM AGENT is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update HBM AGENT through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how HBM AGENT was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    ...
    the code; - the published ``nousresearch/hbm-agent`` image bakes a ``docker`` stamp into
    ``/opt/hbm`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of hbm_cli/), NOT of $HBM_HOME,
    ...
    method = _install_method_stamp(get_hbm_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``hbm_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull nousresearch/hbm-agent:latest",
    "apt": "pkg upgrade hbm-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (hbm_cli/main.py) so the wording never
...
pulling a fresh image and restarting your container instead:
  ...
  docker compose up -d --force-recreate hbm-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/hbm-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/hbm-agent/tags
  • Your config and session history live under ``$HBM_HOME`` (``/opt/data``
...
def format_managed_message(action: str = "modify this HBM AGENT installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this HBM AGENT installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall HBM AGENT.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HBM_HOME/.container-mode.
    ...
    already inside the container, or when HBM_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("HBM_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_hbm_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "hbm-agent"),
        "exec_user": info.get("exec_user", "hbm"),
        "hbm_bin": info.get("hbm_bin", "/data/current-package/bin/hbm-agent")}


# ---- Config paths / HBM_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_hbm_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / HBM_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("HBM_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_hbm_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read HBM_UID / HBM_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns HBM_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running HBM AGENT commonly set these to map the in-container user to a host user so
    ...
    return _env_int("HBM_UID"), _env_int("HBM_GID")


def _chown_to_hbm_uid(path) -> None:
    """Chown ``path`` to ``HBM_UID:HBM_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_hbm_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_hbm_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply HBM_UID/GID ownership. No-op when managed;
    in a container only an explicit HBM_HOME_MODE is applied. HBM_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse HBM_HOME to a served subdirectory without
    directory listings.

    Also applies ``HBM_UID``/``HBM_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    explicit_mode = os.environ.get("HBM_HOME_MODE", "").strip()
    ...
    # start (#10757). An explicit HBM_HOME_MODE is the operator's choice and is still applied.
    if _is_container() and not explicit_mode:
        _chown_to_hbm_uid(path)
        return
    try:
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or HBM_CONTAINER / HBM_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("HBM_CONTAINER") or os.environ.get("HBM_SKIP_CHMOD")
...
def ensure_hbm_home():
    """Ensure the ~/.hbm directory skeleton exists with secure permissions.
    ...
    from hbm_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _HBM_HOME_ENSURED and home.is_dir():
        return
    from hbm_cli.config_home import initialize_home
    initialize_home(home, _HBM_HOME_SUBDIRS, _HBM_HOME_ENSURED)


# ---- Config loading/saving ----

from hbm_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from hbm_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use hbm_cli.config.<name>)
...
# Back-compat re-exports — :mod:`hbm_cli.personality` owns personality/overlay semantics.
from hbm_cli.personality import (  # noqa: E402,F401
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (hbm_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # hbm_cli/tools_config.py toolset-save flow
...
def _validate_timezone(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    ``hbm_time._get_zoneinfo()`` swallows an invalid name behind a single WARNING in the
    ...
    hint = ("Use an IANA zone name such as America/New_York or Asia/Tokyo (see "
            ...
            "schedules silently fall back to server-local time. HBM_TIMEZONE overrides "
            "this key when set.")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from hbm_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — HBM AGENT won't know which provider to use",
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from hbm_cli.config_migrations import (
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from hbm_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from hbm_cli.toolset_validation import validate_platform_toolsets
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.hbm/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_hbm_dotenv()``, in-process
...
# the HBM_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _backups_dir_display() -> str:
    from hbm_constants import display_hbm_home
    return f"{display_hbm_home()}/backups/config/"
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the HBM AGENT host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        ...
        from hbm_cli.config_backups import load_newest_good_backup
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:8] == cache_sig:
            ...
            # load_hbm_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_hbm_dotenv() pins unexpanded literals
...
def save_config(
    ...
    """Save configuration to ~/.hbm/config.yaml.
...
def load_env() -> Dict[str, str]:
    """Load ~/.hbm/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.hbm/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.hbm/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``HBM_CUSTOM_`` prefix keeps the
    ...
    return f"HBM_CUSTOM_{slug}_API_KEY" if slug else "HBM_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.hbm/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from hbm_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.hbm/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to HBM AGENT (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.hbm/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a HBM AGENT-managed credential preferring ``~/.hbm/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale HBM_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("HBM_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from hbm_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ☤ HBM AGENT Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from hbm_cli.auth import get_anthropic_key
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``HBM_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("HBM_MODEL", ""))
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_hbm_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but HBM AGENT may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if _is_env_config_key(key):
        ...
        return
    from hbm_cli.config_env_routing import is_env_setting_key, save_env_setting
    ...
    if is_env_setting_key(key):
        ...
        # here, --force included (#111848). The env writer's denylist (HBM_YOLO_MODE, PATH, ...)
...
def get_config_value(key: str, *, as_json: bool = False, raw: bool = False):
    ...
    from hbm_cli.config_env_routing import is_env_setting_key, read_env_setting
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from hbm_cli.credential_lifecycle import remove_provider_env_credential

        if not remove_provider_env_credential(key.upper()).get("found"):
            _exit_invalid(f"Config key not set: {key}")
        print(f"✓ Unset {key} from {get_env_path()}")
        return
    from hbm_cli.config_env_routing import is_env_setting_key, remove_env_setting
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``hbm_cli/`` — i.e. the git checkout for source
    installs, ``/opt/hbm`` inside the published image. It is a property of
    the running interpreter, NOT of ``$HBM_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$HBM_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/hbm`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('hbm_cli.route_identity', 'normalize_route_base_url'),
}
