...
_MANAGED_FILES_ROOT_ENV = "HBM_DASHBOARD_FILES_ROOT"
...
def _default_hbm_root_is_opt_data() -> bool:
    raw = os.environ.get("HBM_HOME", "").strip()
    if not raw:
        return False
    try:
        from hbm_constants import get_default_hbm_root

        root = get_default_hbm_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    if _default_hbm_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the HBM AGENT
    # root actually IS /opt/data or HBM_DASHBOARD_FILES_ROOT is set.
    if _default_hbm_root_is_opt_data():
