...
``platform_gate_env``) resolves against — whether or not HBM AGENT enumerates it anywhere. Roughly 290
of the ~700 documented variables (``TELEGRAM_GROUP_ALLOWED_USERS``, ``HBM_TIMEZONE``, ...) are
...
lifecycle in ``hbm_cli.credential_lifecycle``.
"""
...
def is_registered_env_name(name: str) -> bool:
    """True when HBM AGENT itself enumerates ``name``: ``OPTIONAL_ENV_VARS`` / ``_EXTRA_ENV_KEYS``, or a
    self-configuring platform suffix so plugin adapters nobody listed (``IRC_HOME_CHANNEL``) count."""
    from hbm_cli.config import _EXTRA_ENV_KEYS, OPTIONAL_ENV_VARS
    from hbm_cli.setup_hidden_env import is_setup_hidden_env
...
def _drop_config_yaml_copies(key: str) -> bool:
    ...
    from hbm_cli.config import _write_user_config, get_config_path, require_readable_config_before_write
...
def remove_env_setting(key: str) -> bool:
    ...
    from hbm_cli.config import remove_env_value
...
def read_env_setting(key: str) -> Optional[str]:
    ...
    from hbm_cli.config import get_env_value, read_raw_config_readonly
