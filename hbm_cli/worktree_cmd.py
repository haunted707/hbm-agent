"""``hbm-agent worktree`` — audit (``list [--json] [--older-than DAYS]``) and reclaim
...
def _list(worktree_gc, repo_root: str, args) -> int:
    ...
    if records:
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `hbm-agent worktree prune`.")
    if external:
        ...
        if any(e.missing and not e.locked for e in external):
            print("  Stale registrations (MISSING) are cleaned by `hbm-agent worktree prune` (metadata only).")
