...
def _resolve_busy_timeout_ms() -> int:
    ...
    return _kb._env_int("HBM_KANBAN_BUSY_TIMEOUT_MS", DEFAULT_BUSY_TIMEOUT_MS, minimum=1)
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from hbm_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _open_configured(path: Path, under_lock) -> tuple[sqlite3.Connection, Any]:
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # DELETE with one ERROR log (see hbm_state_wal._WAL_INCOMPAT_MARKERS).
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    :func:`kanban_db_path` (``HBM_KANBAN_DB`` -> ``HBM_KANBAN_BOARD`` ->
    ...
    with _cross_process_init_lock(path):
        ...
        # See #12508.
        from hbm_state import preflight_db_writability
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"hbm_nested_{secrets.token_hex(8)}"
