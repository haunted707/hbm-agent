...
from hbm_cli.web_server_memory import _normalize_memory_provider_name
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": _select(
        "When the chat app / gateway updates HBM AGENT (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    1. Vendor-name → HBM AGENT-provider: when the provider is not a known provider/alias but the
    ...
    from hbm_cli.models import _AGGREGATOR_PROVIDERS, _KNOWN_PROVIDER_NAMES, normalize_provider
    from hbm_cli.model_normalize import normalize_model_for_provider
    from hbm_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and not is_custom_provider_slug and "/" in model_in:
        ...
        if cur_provider and normalize_provider(cur_provider) in _AGGREGATOR_PROVIDERS:
            ...
            from hbm_cli.models_detect import provider_has_credentials
...
def _apply_main_model_assignment(model_cfg: "Any", result: "ModelSwitchResult", api_key: str = "") -> dict:
    ...
    (``hbm_cli.model_switch.apply_model_selection``). An explicit key for a custom endpoint is
    ...
    Returns a new dict."""
    from hbm_cli.model_switch import apply_model_selection
...
# in hbm_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_skew_restart_hint() -> str:
    ...
    if os.environ.get("HBM_SERVE_HEADLESS") == "1":
        return (
            ...
            "(use Restart backend in HBM AGENT Desktop, or quit and reopen the app)"
        )
    return (
...
def _apply_nous_gateway_defaults(cfg: dict) -> list:
    ...
    *unconfigured* tools through the Nous Tool Gateway. Purely additive — tools with a direct
    ...
    try:
        from hbm_cli.nous_subscription import apply_nous_managed_defaults
...
def _infer_provider_on_model_change(model_val: str, prev_provider: str) -> tuple[str, str]:
    ...
    if not name:
        return "", name
    try:
        from hbm_cli.models import _AGGREGATOR_PROVIDERS, detect_provider_for_model, normalize_provider
    except Exception:
        return "", name
