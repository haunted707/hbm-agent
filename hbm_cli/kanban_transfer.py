...
from hbm_cli.archive_safe import (
    archive_root_dirs,
    copy_regular_files,
    make_targz,
    safe_extract_targz,
...
ARCHIVE_FORMAT = "hbm-kanban-board"
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError("archive is not a HBM AGENT kanban board export (no manifest.json)")
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a HBM AGENT kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this HBM AGENT "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update HBM AGENT and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "hbm_version": manifest.get("hbm_version"),
        },
        "counts": counts,
