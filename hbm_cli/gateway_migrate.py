...
class ProfileGateway:
    ...
    runtime_home: Optional[Path] = None  # HBM_HOME the installed unit pins, when it differs from ``home``
...
def _home_env(home: Path) -> Iterator[None]:
    """Run service-manager helpers as if ``home`` were the active HBM_HOME. Both the contextvar
    override (``get_hbm_home``) and ``os.environ`` (``gateway.status`` identity files, unit
    ...
    previous = os.environ.get("HBM_HOME")
    token = set_hbm_home_override(str(home))
    os.environ["HBM_HOME"] = str(home)
    hbm_constants._default_hbm_root_memo = None
    try:
        yield
    finally:
...
def _stop_gateway_process(home: Path) -> None:
    from hbm_cli.profiles import _stop_gateway_process
...
def _preflight_apply(plan: MigrationPlan, target: Optional[tuple[str, bool]], run_as_user: Optional[str]) -> Optional[str]:
    ...
    from hbm_cli import gateway as gw
    from hbm_cli.config import require_readable_config_before_write
    try:
...
def maybe_auto_migrate_after_update() -> None:
    ...
    from hbm_cli.gateway_migrate_guards import auto_migration_blockers, auto_migration_opted_out
