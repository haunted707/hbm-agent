...
from hbm_cli.config import get_hbm_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    the Dockerfile bakes into ``<project_root>/.hbm_build_sha``."""
    value = _git_output(project_root, "rev-parse", "--short=8", "HEAD")
    if value:
        return value
    try:
        from hbm_cli.build_info import get_build_sha  # deferred: keeps dump cheap on non-dump paths
...
def _gateway_status() -> str:
    ...
    try:
        from hbm_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(hbm_home: Path) -> int:
...
def _cron_summary(hbm_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = hbm_home / "cron" / "jobs.json"
...
def _api_key_lines(show_keys: bool) -> list[str]:
    ...
    for env_var, label in _API_KEYS:
        ...
        # Set in this shell but absent from ~/.hbm/.env: a managed backend loads .env, not the login
...
def run_dump(args):
    ...
    load_hbm_dotenv(hbm_home=get_env_path().parent, project_env=get_project_root() / ".env")
    project_root, hbm_home = get_project_root(), get_hbm_home()
    try:
        config = load_config()
    except Exception:
        config = {}
    ...
    lines = [
        ...
        f"  cron_jobs:          {_cron_summary(hbm_home)}",
        f"  skills:             {_count_skills(hbm_home)}",
    ]
