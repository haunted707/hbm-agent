"""Image-authored deployment provenance for immutable HBM AGENT runtimes.

The image bakes ``/etc/hbm/image-provenance.json`` outside ``$HBM_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/hbm/image-provenance.json")
