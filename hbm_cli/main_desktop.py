"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (hbm-agent gui/desktop).
...
from hbm_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $HBM_HOME."""
    from hbm_constants import get_hbm_home
    return get_hbm_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/HBM AGENT.app/Contents/MacOS/HBM AGENT → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/HBM AGENT.app/Contents/MacOS/HBM AGENT"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "HBM AGENT.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("hbm", "HBM AGENT")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture HBM AGENT.exe to the launcher. Prefer
        ...
        # wrong-architecture HBM AGENT.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# HBM AGENT.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``HBM AGENT.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 hbm-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built HBM AGENT.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working HBM AGENT.exe from backup.")
        print("    Your existing version was kept and still works. Run `hbm-agent desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `hbm-agent desktop --force-build` to rebuild, or re-run the HBM AGENT")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from hbm_constants import find_node_executable, with_hbm_node_path
...
def _desktop_macos_local_codesign(app: Path, *, desktop_dir: Path, identity: str = "-") -> bool:
    ...
    #    the outer HBM AGENT.app component.
...
def _desktop_macos_relaunchable_fixup(
    ...
    "HBM AGENT is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../HBM AGENT.app/Contents/MacOS/HBM AGENT  ->  app bundle = .../HBM AGENT.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="hbm-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:hbmlocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "HBM AGENT Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nousresearch.hbm"
    )
    return True
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ HBM AGENT Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ HBM AGENT Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry(defer: bool = False):
    ...
    from hbm_cli.main import PROJECT_ROOT
    try:
        from hbm_cli.linux_desktop_entry import DeferredDesktopEntryInstall, install_desktop_entry, is_supported
        if not is_supported():
            return None
        if defer:
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from hbm_cli.main import PROJECT_ROOT
    from hbm_cli.main_web_build import _run_npm_install_deterministic
    from hbm_constants import with_hbm_node_path
    ...
    nixos_env = with_hbm_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose HBM AGENT.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # HBM AGENT.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds HBM AGENT.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on HBM AGENT.exe, close any")
            print("  running HBM AGENT desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> hbm-agent desktop --force-build")
...
def _prefer_wsl_d3d12(env: dict) -> None:
    ...
    from hbm_constants import is_wsl
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    from hbm_constants import with_hbm_node_path
    # with_hbm_node_path() copies os.environ when called with no arg.
    env = with_hbm_node_path()
    _prefer_wsl_d3d12(env)
    for attr, key in (
        ("fake_boot", "HBM_DESKTOP_BOOT_FAKE"), ("ignore_existing", "HBM_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "hbm_root", None):
        env["HBM_DESKTOP_HBM_ROOT"] = str(Path(args.hbm_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["HBM_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "HBM_DESKTOP_DISABLE_GPU" not in os.environ:
        env["HBM_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "HBM_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["HBM_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from hbm_cli.main import PROJECT_ROOT
    from hbm_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "HBM AGENT Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  hbm-agent gui")
            sys.exit(1)
    ...
    from hbm_cli.linux_desktop_entry import launched_from_shell
    ...
    if source_mode:
        print("→ Launching HBM AGENT Desktop from source build...")
    ...
    if not source_mode:
        print(f"→ Launching packaged HBM AGENT Desktop: {' '.join(launch_command)}")
    pass_fds: tuple[int, ...] = ()
