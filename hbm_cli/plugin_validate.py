"""``hbm-agent plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``HBM_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "HBM_VALIDATE_JSON:"
...
def _requires_hbm_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_hbm`` spec.

    Unlike :func:`hbm_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from hbm_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_hbm(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_hbm") or "").strip()
    if not spec:
        report.add("requires_hbm", True, "not declared")
        return
    if _requires_hbm_spec_valid(spec):
        report.add("requires_hbm", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_hbm",
            False,
            f"requires_hbm spec {spec!r} does not parse "
...
# from hbm so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "hbm_validate_probe_plugin"
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="hbm-validate-") as scratch:
        env = dict(os.environ)
        env["HBM_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_hbm(report, manifest)
