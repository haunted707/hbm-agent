...
The identity is created in exactly one place, at boot (``hbm_cli.free_tier_bootstrap``), and only
while ``HBM_GUEST_ONBOARDING=1`` (see ``guest_enabled``). The bootstrap mints an anonymous Nous
...
ANON_SECRET_ENV = "HBM_ANON_API_SECRET"
...
GUEST_ONBOARDING_ENV = "HBM_GUEST_ONBOARDING"
...
# fallback (no ``/login``, no ``hbm-agent`` verb, no guest / anonymous / credential). ``retryable``
...
# what is unavailable is using HBM AGENT without signing in, and signing in is free.
_SIGNIN_IS_FREE = "Signing in is free."
ANON_FAILURE_COPY = {
    ANON_GATE_CLOSED: f"This version can't be used without a Nous account. {_SIGNIN_IS_FREE}",
    ANON_GATE_PAUSED: f"Using HBM AGENT without signing in is paused for a moment. {_SIGNIN_IS_FREE}",
...
def current_nous_state() -> Optional[Dict[str, Any]]:
    ...
    from hbm_cli.auth import _load_auth_store, _load_provider_state
    try:
...
EXTRA_WELCOME_HOSTS_ENV = "HBM_EXTRA_WELCOME_HOSTS"


def welcome_hosts() -> frozenset[str]:
    """``WELCOME_HOSTS`` plus any ``HBM_EXTRA_WELCOME_HOSTS`` entries (lowercased hostnames)."""
...
def apply_exchange_to_state(state: Dict[str, Any], exchanged: Dict[str, Any]) -> None:
    ...
    from hbm_cli.auth_nous import _validate_nous_inference_url_from_network
...
def _portal_base_url() -> str:
    from hbm_cli.auth_nous import _nous_portal_env_override
...
def _mint_locked(
    ...
    from hbm_cli.auth import _store_provider_state, _save_auth_store
    from hbm_cli.auth_nous import _write_shared_nous_state
...
def _mint_memo_key() -> str:
    ...
    return "" if get_hbm_home_override() is None else hbm_home_key()
...
def _reconcile_and_provision(*, timeout_seconds: float, carries_inference: bool = True) -> Optional[Dict[str, Any]]:
    ...
    1. The shared store is the identity of record for this HBM AGENT root. If it holds an identity
...
def refresh_guest_state(state: Dict[str, Any], client: httpx.Client) -> None:
    ...
    from hbm_cli.auth import _nous_portal_base_url
...
def clear_dead_guest(reason: str, *, dead_token: Optional[str] = None) -> None:
    ...
    *dead_token* is None the profile's current guest is treated as the failed one.
    ...
    from hbm_cli.auth_nous import _clear_shared_nous_state, _nous_shared_store_lock, _read_shared_nous_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
_WELCOME_ROUTE_REFUSALS = (
    ...
    ("serves anonymous hbm agent accounts only", "named_on_welcome_host"),
...
_WELCOME_ROUTE_COPY = {
    ...
    "tier_disabled": "Using HBM AGENT without signing in is switched off right now. "
...
def welcome_refusal_copy(refusal: Dict[str, Any], *, model: str = "", in_chat: bool = True, door: bool = True) -> str:
    ...
    if reason == "model_not_free":
        ...
        return (f"{what} available without signing in, so HBM AGENT uses {serves} for now. "
    ...
    return f"HBM AGENT couldn't send that without signing in. Signing in is free. {signin}".rstrip()
...
def welcome_route_refusal_copy(kind: str, *, in_chat: bool = True, door: bool = True) -> str:
    template = _WELCOME_ROUTE_COPY.get(kind) or "HBM AGENT couldn't reach the free model on this route."
...
def apply_model_switch(agent: Any) -> Optional[str]:
    ...
    try:
        ...
        if str(model_cfg.get("default") or "").strip() == requested:
            from hbm_cli.auth import _update_config_for_provider
...
def _account_state_from_token(
    ...
    from hbm_cli.auth import PROVIDER_REGISTRY, _coerce_ttl_seconds, _optional_base_url, _tls_state_from_verify
    from hbm_cli.auth_nous import _NOUS_EMPTY_AGENT_KEY_FIELDS, _iso_after, refresh_nous_oauth_from_state
    now = datetime.now(timezone.utc)
...
def settle_after_upgrade(account_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    default for the account's tier (:func:`hbm_cli.models.recommended_nous_default_model`, the
    ...
    model = current
    if on_welcome_model:
        from hbm_cli.models import recommended_nous_default_model
        try:
...
def _poll_for_token(*args, **kwargs) -> Dict[str, Any]:
    ...
    from hbm_cli.auth_device_flow import _poll_for_token as poll
...
def persist_nous_credentials(*args, **kwargs):
    ...
    from hbm_cli.auth_nous import persist_nous_credentials as persist
...
from hbm_cli.anon_sign_in import (  # noqa: E402
...
from hbm_cli.anon_sign_in_cli import (  # noqa: E402
