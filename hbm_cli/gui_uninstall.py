"""HBM AGENT Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "HBM AGENT" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "HBM AGENT"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "HBM AGENT"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "HBM AGENT"


def source_built_gui_artifacts(hbm_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``hbm-agent desktop`` inside the checkout (same ``hbm-agent/`` layout
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "HBM AGENT" product."""
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/HBM AGENT.app"), home / "Applications" / "HBM AGENT.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "HBM AGENT", local_base / "hbm-desktop"] + (
            [Path(program_files) / "HBM AGENT"] if program_files else [])
    ...
    from hbm_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "HBM AGENT.desktop"] + [
        icons / size / "apps" / "hbm.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(hbm_home: Path) -> bool:
    """True when a usable Python agent install exists under HBM_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((hbm_home / "hbm-agent" / sub).is_dir() for sub in ("hbm_cli", "venv", ".venv"))


def gui_is_installed(hbm_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(hbm_home), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(hbm_home: "Path | None" = None) -> dict:
    ...
    return {"hbm_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(hbm_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so HBM AGENT
        # disappears from the launcher.
        try:
            from hbm_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove hbm' or 'sudo dnf remove hbm'). AppImage builds are a single "
