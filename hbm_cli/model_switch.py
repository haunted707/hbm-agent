...
# Re-exported: callers/tests patch hbm_cli.model_switch.<name>.
from hbm_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix HBM AGENT wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by HBM AGENT.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog HBM AGENT itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
_HBM_MODEL_WARNING = (
    "Nous Research HBM AGENT 3 & 4 models are NOT agentic and are not designed "
    "for use with HBM AGENT. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.).")

# Match only the real Nous Research HBM AGENT 3 / 4 chat families; a bare substring check
# false-positived on tool-capable local Modelfiles like ``hbm-brain:qwen3-14b-ctx16k``.
#   match:    NousResearch/HBM AGENT-3-Llama-3.1-70B, hbm-4-405b, openrouter/hbm3:70b
#   no match: hbm-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HBM_NON_AGENTIC_RE = re.compile(r"(?:^|[/:])hbm[-_ ]?[34](?:[-_.:]|$)", re.IGNORECASE)
...
def is_nous_hbm_non_agentic(model_name: str) -> bool:
    """True if *model_name* is a real Nous HBM AGENT 3/4 chat model (single owner; cli.py uses it too)."""
    return bool(model_name and _NOUS_HBM_NON_AGENTIC_RE.search(model_name))


def _check_hbm_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a Nous HBM AGENT 3/4 chat model, else ""."""
    return _HBM_MODEL_WARNING if is_nous_hbm_non_agentic(model_name) else ""
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from hbm_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``HbmCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from hbm_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
            if is_routing_aggregator(_norm_prov(current_provider)):
                from hbm_cli.models import _find_openrouter_slug
    ...
    try:
        from hbm_cli.models import normalize_provider
...
def parse_model_flags_detailed(raw_args: str) -> ModelFlagParseResult:
    ...
    is the raw level word (validated by :func:`hbm_constants.parse_reasoning_effort` at apply
...
def _ollama_configured_base() -> tuple[dict, str]:
    from hbm_cli.models import _get_provider_config_dict
...
def _apply_direct_alias_endpoint(st: "_Switch", da: DirectAlias) -> None:
    ...
    from hbm_cli.models_local import _same_ollama_native_root
...
def _convert_vendor_colon_slug(st: _Switch) -> None:
    ...
    ``left`` names a provider HBM AGENT knows, so ``/model alibaba:qwen3.6-plus`` routes like
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from hbm_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from hbm_cli.config import load_config
        from hbm_cli.moa_config import exact_moa_preset_name, normalize_moa_config
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from hbm_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from hbm_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
    keep_current_ollama_endpoint = False
    ollama_headers: dict[str, str] = {}
    if st.current_provider == "custom" and st.current_base_url:
        try:
            from hbm_cli.models_local import should_use_ollama_native_catalog
...
def _validate_switch(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from hbm_cli.models_local import _get_ollama_request_headers
    from hbm_cli.models_validate import validate_requested_model
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from hbm_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from hbm_cli.models import opencode_model_api_mode
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from hbm_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_hbm_model_warning(st.new_model)) if w]
    ...
    try:
        from hbm_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
def persist_model_selection(result: ModelSwitchResult, config_path: Any = None) -> None:
    ...
    ``HBM_HOME/config.yaml`` — the context override or ``HBM_HOME`` at call time).
    ...
    from hbm_cli.config import get_config_path, read_user_config_raw, warn_unpinned_cron_jobs_after_model_config_change
...
def _scoped_key_env(name: str) -> str:
    ...
    ``$HBM_HOME/.env`` authenticates the ``/model`` verification probe (#109315) and a rotated
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('hbm_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('hbm_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
