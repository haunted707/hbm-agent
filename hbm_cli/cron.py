...
def cron_status():
    ...
    from hbm_cli.gateway import find_gateway_pids, named_profile_served_by_running_multiplexer
    print()
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits HBM_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (HBM_SESSION_KEY); declare the channel stateless so
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _hbm_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _hbm_now().isoformat() if run_now else run_at)
