...
from hbm_cli.subcommands._shared import add_yes_flag
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent", help="Import a Claude Code or Codex CLI setup into HBM AGENT",
        description="One-command import of another coding agent's setup into HBM AGENT. "
    ...
    parser.add_argument(
        ...
        help="Overwrite existing HBM AGENT items on name conflicts (default: skip)")
    ...
    parser.add_argument(
        ...
        help="Re-import every previously imported source whose files changed since the last "
             "import (registered in HBM_HOME/import-sync.json). Runs without prompts; "
