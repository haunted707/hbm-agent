"""``hbm-agent gui`` subcommand parser."""
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description="Launch the HBM AGENT Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        ...
        help="Force Desktop to ignore any hbm CLI already on PATH during backend resolution")
    gui_parser.add_argument(
        "--hbm-root",
        help="Override the HBM AGENT source root used by Desktop (sets HBM_DESKTOP_HBM_ROOT)")
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets HBM_DESKTOP_CWD)")
    gui_parser.add_argument(
    ...
    gui_parser.add_argument(
        "--identity", default="HBM AGENT Local Signing",
        help="Certificate name to create/use for --setup-tcc-identity (default: HBM AGENT Local Signing)",
    )
