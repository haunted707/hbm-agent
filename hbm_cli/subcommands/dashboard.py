...
def _add_server_runtime_args(parser) -> None:
    ...
    parser.add_argument(
        "--status", action="store_true", help="List running HBM AGENT web server processes and exit")
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the HBM AGENT web dashboard for managing config, API keys, and sessions",
    )
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the HBM AGENT backend server (headless; powers the desktop app and remote backends)",
        description="Run the HBM AGENT backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI.")
    ...
    dashboard_register_parser.add_argument(
        ...
        help="Optional public HTTPS OAuth redirect URI for the dashboard, e.g. "
            "https://hbm.example.com/auth/callback. Omit for localhost-only use.")
    dashboard_register_parser.add_argument(
        ...
        help="Override the Nous Portal base URL for registration (default: the "
            ...
            "portal. Also settable via HBM_DASHBOARD_PORTAL_URL. Mainly for "
