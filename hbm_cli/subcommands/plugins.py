...
from hbm_cli.subcommands._shared import add_json_flag
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description="Install, update, remove, list, or validate native HBM AGENT plugins "
    ...
    plugins_search = plugins_subparsers.add_parser(
        "search", help="Search the curated HBM AGENT plugin catalog")
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack", help="Declarative, shareable plugin sets (hbm-pack.yaml)",
    ...
    pack_install.add_argument("source", help="Path to a hbm-pack.yaml file, or an https:// URL")
    ...
    pack_export.add_argument(
        "--name", default="my-hbm-pack", help="Pack name to embed in the exported YAML")

    pack_show = pack_subparsers.add_parser(
        "show", help="Dry-run: parse and display a pack without installing")
    pack_show.add_argument("source", help="Path to a hbm-pack.yaml file, or an https:// URL")
