"""``hbm-agent auth`` subcommand parser."""
...
def build_auth_parser(subparsers, *, cmd_auth: Callable) -> None:
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate HBM AGENT with Spotify via PKCE")
    auth_spotify.add_argument(
        "spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set HBM_SPOTIFY_CLIENT_ID)")
    auth_spotify.add_argument(
