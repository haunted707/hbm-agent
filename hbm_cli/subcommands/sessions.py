"""``hbm-agent sessions`` subcommand parser."""
...
def build_sessions_parser(subparsers, *, cmd_sessions: Callable) -> None:
    ...
    sessions_export.add_argument("output", nargs="?", metavar="OUTPUT",
        help="Where to write. jsonl/html/trace: a file path, or a directory (existing, or ending in /) "
            ...
            "md/qmd: a directory, one file per session (default: <hbm home>/session-exports)")
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import", help="Import a Claude Code or Codex CLI session into HBM AGENT",
        description="Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the HBM AGENT session store "
            "so it can be resumed with 'hbm-agent --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in hbm_cli/sessions_cmd.py; the parser is threaded
