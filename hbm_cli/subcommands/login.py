"""``hbm-agent login`` subcommand parser."""
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``invalid choice``. Registered WITHOUT ``help=`` so it is omitted from ``hbm-agent --help``
    ...
    login_parser = subparsers.add_parser(
        "login",
        description="Deprecated. Use `hbm-agent auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider", default=None, help="(deprecated) Provider name; ignored — see `hbm-agent model`")
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: hbm-cli)")
