"""``hbm-agent gateway`` and ``hbm-agent proxy`` subcommand parsers."""
...
def build_gateway_parser(
    ...
    _flag(gateway_run, "--no-supervise",
        help="Inside the s6-overlay Docker image, normally `gateway run` is "
            ...
            "if HBM_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy", help="Remove legacy hbm.service units from pre-rename installs",
        description="Stop, disable, and remove legacy HBM AGENT gateway unit files "
            "(e.g. hbm.service) left over from older installs. Profile "
            "units (hbm-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser("enroll",
        ...
        description="Redeem a single-use enrollment token with a relay connector. "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.hbm/.env. "
            "Requires being logged in (hbm-agent setup). Not available in managed installs.")
    ...
    gateway_enroll.add_argument("--wake-url", dest="wake_url", default=None,
        help="Phase 5 §5.2 wake URL: a reachable URL the connector pokes "
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.hbm/.env and forwarded at provision. "
    ...
    proxy_start.add_argument("--provider", default="nous",
        help="Upstream provider: nous or xai (default: nous). See `hbm-agent proxy providers`.")
