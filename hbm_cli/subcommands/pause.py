...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "HBM AGENT paused"
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  HBM AGENT resumed — dispatch picks up on the next tick.")
    else:
        print(f"HBM AGENT is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    resume_parser = subparsers.add_parser(
        "resume", help="Lift the emergency stop set by `hbm pause`",
