...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update", help="Update HBM AGENT to the latest version",
    ...
    update_parser.add_argument(
        ...
        help="Show the update plan and exit without changing anything: install "
            "kind (git/docker/nix), every running HBM AGENT service across all "
    ...
    update_parser.add_argument(
        ...
        help="Force a FULL pre-update backup (quick state snapshot + HBM_HOME zip) for this run, regardless of updates.pre_update_backup",
    )
    update_parser.add_argument(
    ...
    update_parser.add_argument(
        ...
        help="Update against this branch instead of the default (main). "
            "If the local checkout is on a different branch, hbm will "
    ...
    update_parser.add_argument(
        ...
        help="Windows: proceed with the update even when another hbm.exe is detected. The concurrent process will likely cause WinError 32 warnings. Does NOT bypass the venv-process guard (see --force-venv).",
    )
    update_parser.add_argument(
