...
Plain mixin for ``hbm_state.SessionDB`` (no ``__init__``/state of its own).
Must never import hbm_state (cycle); shared constants live in hbm_state_common.
"""
...
from hbm_state_fts import _drop_orphan_fts_shadow_tables
from hbm_state_holders import _read_proc_argv
...
# peer (a supervised service on the same HBM_HOME), not a transient one worth waiting out (#106393).
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._hbm_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._hbm_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _defer_stale_fts_for_holders(self, cursor: sqlite3.Cursor, foreign_holders) -> bool:
        ...
        if futile:
            logger.error(
                ...
                "own retry admits the rebuild within %.0fs of the holder leaving. `hbm doctor` shows this.",
            ...
            logger.error(
                ...
                "processes (this process's own retry then rebuilds), or run `hbm sessions optimize-storage` "
                "with every holder stopped. `hbm doctor` reports this degraded state.", attempts, foreign_holders,
            )
        logger.warning(
    ...
    def _trigram_tokenizer_available(self, cursor: sqlite3.Cursor) -> bool:
        ...
        probe = "temp.hbm_fts5_trigram_probe"
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        with contextlib.suppress(Exception):  # missing/corrupt cache → recompute below
            # Late import: resolves a test-patched hbm_constants.get_hbm_home.
            from hbm_constants import get_hbm_home as _home
    ...
    def _run_data_migrations(self, cursor: sqlite3.Cursor, current_version: int, fts5_available: bool) -> None:
        ...
        # install only gets a flag; `hbm sessions optimize-storage` performs it. The FTS
        ...
        # as before, and we only record a flag advertising that the optimization is available. `hbm
    ...
    def _backfill_gateway_metadata_from_sessions_json(self, cursor: sqlite3.Cursor) -> None:
        ...
        sessions_file = get_hbm_home() / "sessions" / "sessions.json"
