...
from hbm_constants import OPENROUTER_BASE_URL, get_hbm_home
from agent.retry_utils import jittered_backoff
from hbm_cli.env_loader import load_hbm_dotenv

# Load .env from HBM_HOME first, then project root as a dev fallback.
load_hbm_dotenv(hbm_home=get_hbm_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: hbm setup")
