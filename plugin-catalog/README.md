# HBM AGENT Plugin Catalog

Curated, Nous-approved HBM AGENT plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`hbm plugins catalog` / `hbm plugins search` and installable with
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `hbm-agent` repository, reviewed and merged by a maintainer. There is
...
3. **No self-updating code.** A listed plugin must not fetch and replace
   ...
   `hbm plugins update <name>`. Keep the updater in the standalone
...
5. **Owner-or-major-contributor submissions, or a maintainer-curated sweep.**
   ...
   HBM AGENT maintainers may also add entries in batches from a reviewed sweep
